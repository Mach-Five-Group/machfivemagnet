...
`)],{type:`text/csv`}),a=URL.createObjectURL(i),o=document.createElement(`a`);o.href=a,o.download=`leads-${K.filter.toLowerCase()}.csv`,document.body.appendChild(o),o.click(),o.remove(),URL.revokeObjectURL(a),i_(`Exported ${e.length} lead${e.length===1?``:`s`}`,`success`)}var $g,e_,K,t_,n_,r_,q,i_,a_,o_,s_,c_,l_,u_,d_,f_,p_,m_,h_,g_,__,v_,y_,b_=o((()=>{w(),$g=u(f(),1),m(),wa(),Aa(),un(),qc(),e_=!1,K={leads:[],magnetNames:{},filter:`ALL`,canManage:!1,book:!1,ownerNames:{},clientFilter:`ALL`,search:``,shown:25,selectedId:null,day:``,sort:{key:`captured`,dir:`desc`},magnetFilter:`ALL`,range:`ALL`,pageSize:25,total:0,truncated:!1},t_=()=>K.pageSize===`ALL`?2**53-1:Number(K.pageSize)||25,n_=[`NEW`,`WORKING`,`QUALIFIED`,`UNQUALIFIED`,`CONVERTED`,`EXPORTED`],r_={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},q=e=>String(e??``).replace(/[&<>"']/g,e=>r_[e]),i_=(e,t=`info`)=>{try{$g.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},a_=()=>C.getConfig?.().BASE_URL||``,o_=e=>[e.first_name,e.last_name].filter(Boolean).join(` `).trim()||e.email||`(no name)`,s_=e=>(o_(e).split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``)||`?`).toUpperCase(),c_=e=>e.created?.created_tz||``,l_=e=>{let t=c_(e);if(!t)return!1;try{return Date.now()-new Date(t).getTime()<864e5}catch{return!1}},u_=e=>K.clientFilter===`ALL`||e.owner_guid===K.clientFilter,d_=e=>!K.day||c_(e).slice(0,10)===K.day,f_=e=>K.magnetFilter===`ALL`||e.magnet_guid===K.magnetFilter,p_={ALL:null,7:7,30:30,90:90},m_={ALL:`All time`,7:`Last 7 days`,30:`Last 30 days`,90:`Last 90 days`},h_={name:{label:`Name`,dir:`asc`,value:e=>o_(e).toLowerCase()},contact:{label:`Contact`,dir:`asc`,value:e=>String(e.email||e.phone||``).toLowerCase()},captured:{label:`Captured`,dir:`desc`,value:e=>c_(e)}},g_=()=>Og(`touch_app`,`Click to see lead information.`),__=[`hubspot`,`zoho`,`salesforce`,`gohighlevel`],v_=()=>`
  ...
  `,document.head.appendChild(e),Fv=!0}function Ev(){return Tv(),`
    <div class="page-shell" data-section="billing">
      ...
      <div class="dashboard-card">
        <div class="card-body" id="bill-plan-card">${Iv(`hourglass_empty`,`Loading your plan…`)}</div>
      </div>
    ...
    </div>`}async function Dv(){await Av()}function Ov(e){if(e.key===`enterprise`)return`Included with your program.`;if(e.key===`grandfathered`)return`All features included.`;let t=e.prices?`$${e.prices.monthly}/month`:``;return t?`${t}, billed monthly.`:``}function kv(e,t,n){let r=n==null,i=r?0:Math.min(100,Math.round(t/Math.max(1,n)*100));return`
    <div class="bill-meter">
      <div class="bill-meter__k">${Pv(e)}</div>
      <div class="bill-meter__v">${Mv(t)}${r?` <small>No limit</small>`:` <small>of ${Mv(n)}</small>`}</div>
      ${r?``:`<div class="bill-bar"><i style="width:${i}%" data-warn="${i>=80}"></i></div>`}
    </div>`}async function Av(){let e=document.getElementById(`bill-plan-card`),t;try{let e=await fetch(`${jv()}/m5t/v5/billing/usage`);if(t=await e.json(),!e.ok||!t.ok)throw Error(t.error||`HTTP ${e.status}`)}catch(t){console.warn(`[billing] load failed:`,t),e&&(e.innerHTML=Iv(`error`,`Could not load billing. Please try again.`));return}let n=t.plan||et.grandfathered,r=t.usage||{};e&&(e.innerHTML=`
      <div class="bill-plan">
        <span class="bill-plan__name">${Pv(n.label)}</span>
        <span class="bill-plan__badge">Current plan</span>
      </div>
      <div class="bill-plan__sub">${Pv(Ov(n))}</div>`);let i=document.getElementById(`bill-meters`);i&&(i.innerHTML=[kv(`Live magnets`,r.magnets,n.limits?.magnets??null),kv(`Leads this month`,r.leads_month,n.limits?.leads_mo??null),kv(`Team members`,r.team_members,n.limits?.team_members??null)].join(``));let a=document.getElementById(`bill-feat-card`),o=document.getElementById(`bill-feats`);if(a&&o){let e=[],t=n.features?.formats||[];e.push(`<div class="bill-feat"><span class="material-symbols-outlined" aria-hidden="true">check_circle</span>${t.length>=4?`All four display formats`:`Chat bubble format`}</div>`);for(let[t,r]of Lv)r&&n.features?.[t]&&e.push(`<div class="bill-feat"><span class="material-symbols-outlined" aria-hidden="true">check_circle</span>${Pv(r)}</div>`);o.innerHTML=`<div class="bill-feats">${e.join(``)}</div>`,a.style.display=``}let s=document.getElementById(`bill-note`);s&&(s.textContent=n.key===`enterprise`?`Usage is informational. Your program covers the platform.`:`Lead counts reset monthly. This period: ${t.period?.start||``} to ${t.period?.end||``}.`)}var jv,Mv,Nv,Pv,Fv,Iv,Lv,Rv=o((()=>{w(),rt(),jv=()=>C.getConfig?.().BASE_URL||``,Mv=e=>Number(e||0).toLocaleString(),Nv={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Pv=e=>String(e??``).replace(/[&<>"']/g,e=>Nv[e]),Fv=!1,Iv=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,Lv=[[`formats`,null],[`hub_scheduler`,`Welcome hub and scheduler`],[`ab_testing`,`A/B testing`],[`journeys`,`Journeys`],[`attribution`,`Campaign attribution`],[`crm_sync`,`CRM sync`],[`booking`,`Booking integrations`],[`webhooks`,`Webhooks and Zapier`],[`csv_export`,`CSV export`],[`ad_reporting`,`Ad performance reporting`]]})),zv=c({init:()=>Hv,render:()=>Vv});function Bv(){if(Fa(),Ic(),ey)return;let e=document.createElement(`style`);e.id=`m5t-traffic-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),ey=!0}function Vv(){return k()?ma({section:`traffic`,title:`Traffic`,description:`Pick a client to view their website traffic.`}):(Bv(),`
    <div class="page-shell" data-section="traffic">
      <div class="page-header">
        ...
        <div class="chart-controls" id="traffic-periods">
          ${ny.map(e=>`<button class="chart-period-btn${e.d===ry?` active`:``}" type="button" data-days="${e.d}">${e.label}</button>`).join(``)}
        </div>
      </div>
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="traffic-body">
          ${ty(`hourglass_empty`,`Loading traffic…`)}
        </div>
      </div>
    ...
    </div>`)}async function Hv(){if(k()){let e=e=>Number(e||0).toLocaleString();return ga(Jv(),[{h:`Leads`,td:t=>e(t.leads.window??t.leads.total)}],{emptyMsg:`No client orgs yet. Connect a client GA4 property to see traffic here.`})}let e=document.getElementById(`traffic-periods`);e?.addEventListener(`click`,t=>{let n=t.target.closest(`button[data-days]`);n&&(ry=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>e.classList.toggle(`active`,Number(e.dataset.days)===ry)),Uv())}),await Uv()}async function Uv(){let e=Jv(),t=document.getElementById(`traffic-body`),n=document.getElementById(`traffic-range`),r=new Date,i=Yv(r),a=Yv(new Date(r.getTime()-(ry-1)*864e5));n&&(n.textContent=`${a} → ${i}`);let o,s=null;try{let[t,n]=await Promise.all([fetch(`${e}/m5t/v5/coreAnalyticsMetrics/overlay?from=${a}&to=${i}`),Sc().catch(()=>null)]);if(!t.ok)throw Error(`HTTP ${t.status}`);o=await t.json(),s=n}catch(e){console.warn(`[traffic] load failed:`,e),t&&(t.innerHTML=ty(`error`,`Could not load traffic. Please try again.`));return}Wv(s);let c=o.totals||{};if(Gv(`traffic-sessions`,Xv(c.sessions)),Gv(`traffic-users`,Xv(c.users)),Gv(`traffic-views`,Xv(c.page_views)),Gv(`traffic-engagement`,Zv(c.engagement_rate)),Gv(`traffic-leads`,Xv(c.magnet_leads)),Gv(`traffic-leadrate`,Zv(c.lead_rate)),!o.connected){t.innerHTML=s&&!s.denied&&yc(s.providers,`google_analytics`)?ty(`cloud_sync`,`GA4 connected. No traffic for this range yet. Metrics appear here once they sync.`):ty(`link_off`,`No GA4 property connected yet. Connect Google Analytics under Settings, Integrations to see traffic here.`);return}let l=o.rows||[];t.innerHTML=Kv(l),qv(l)}function Wv(e){let t=document.getElementById(`traffic-conn`);if(!t)return;if(!e||e.denied){t.innerHTML=``;return}let n=dc.google_analytics,r=yc(e.providers,`google_analytics`);if(!r){t.innerHTML=``;return}let i=bc(n,r,!!e.configured?.google_analytics);if(i!==`connected`&&i!==`broken`){t.innerHTML=``;return}let a=Bc()?` <a href="#/settings/integrations/${n.urlSlug}">Manage</a>`:``;t.innerHTML=`<span>Google Analytics</span> ${xc(i,r)}${a}`}function Gv(e,t){let n=document.getElementById(e);n&&(n.textContent=t)}function Kv(e){return Ia({rows:e,xLabel:e=>String(e.date||``).slice(5),left:{value:e=>e.sessions||0,label:`Sessions`,fmt:e=>Va(e)},right:{value:e=>e.magnet_leads||0,label:`Magnet Leads`},ariaLabel:`Website sessions versus magnet leads over time`,pointLabel:e=>`${e.date}: ${Xv(e.sessions||0)} sessions, ${Xv(e.magnet_leads||0)} leads`,hitData:e=>({day:e.date})})}function qv(e){let t=document.getElementById(`traffic-body`);return t?Ra(t,{rows:e,tipHTML:e=>`
      <div class="m5c-tip__title">${$v(String(e.date||``))}</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--1"></i>${Xv(e.sessions||0)} sessions</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--2"></i>${Xv(e.magnet_leads||0)} leads</div>`}):()=>{}}var Jv,Yv,Xv,Zv,Qv,$v,ey,ty,ny,ry,iy=o((()=>{w(),wa(),qc(),Na(),Ua(),Jv=()=>C.getConfig?.().BASE_URL||``,Yv=e=>e.toISOString().slice(0,10),Xv=e=>Number(e||0).toLocaleString(),Zv=e=>e==null?`—`:`${e}%`,Qv={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},$v=e=>String(e??``).replace(/[&<>"']/g,e=>Qv[e]),ey=!1,ty=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,ny=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],ry=30})),ay=c({default:()=>Ey,init:()=>cy,render:()=>sy});function oy(){if(Fa(),Sy)return;let e=document.createElement(`style`);e.id=`m5t-adperf-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Sy=!0}function sy(){return k()?ma({section:`ad-performance`,title:`Ad Performance`,description:`Pick a client to view their spend and lead performance.`}):(oy(),`
    <div class="page-shell" data-section="ad-performance">
      <div class="page-header">
        ...
        <div class="chart-controls" id="adperf-periods">
          ${wy.map(e=>`<button class="chart-period-btn${e.d===Ty?` active`:``}" type="button" data-days="${e.d}">${e.label}</button>`).join(``)}
        </div>
      </div>
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="adperf-body">
          ${Cy(`hourglass_empty`,`Loading ad performance…`)}
        </div>
      </div>
    ...
    </div>`)}async function cy(){if(k()){let e=e=>Number(e||0).toLocaleString();return ga(_y(),[{h:`Spend`,td:t=>t.ad?`$${e(t.ad.spend)}`:`—`},{h:`Clicks`,td:t=>t.ad?e(t.ad.clicks):`—`},{h:`Leads`,td:t=>e(t.leads.window??t.leads.total)},{h:`CPL`,td:t=>t.ad?.cost_per_lead==null?`—`:`$${e(t.ad.cost_per_lead)}`}],{emptyMsg:`No client orgs yet. Connect a client ad account to see spend here.`})}let e=document.getElementById(`adperf-periods`);e?.addEventListener(`click`,t=>{let n=t.target.closest(`button[data-days]`);n&&(Ty=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>e.classList.toggle(`active`,Number(e.dataset.days)===Ty)),ly())}),document.getElementById(`adperf-sources`)?.addEventListener(`click`,e=>{let t=e.target.closest(`tr[data-campaign]`);t&&(Ta(Oa.LEADS_SEARCH,t.dataset.campaign),!e.target.closest(`a`)&&(window.location.hash=`#/leads`))}),document.getElementById(`adperf-lsa-card`)?.addEventListener(`click`,e=>{let t=e.target.closest(`tr[data-lsalead]`);t&&(Ta(Oa.LEADS_SEARCH,t.dataset.lsalead),!e.target.closest(`a`)&&(window.location.hash=`#/leads`))}),await ly()}async function ly(){let e=_y(),t=document.getElementById(`adperf-body`),n=document.getElementById(`adperf-range`),r=new Date,i=vy(r),a=vy(new Date(r.getTime()-(Ty-1)*864e5));n&&(n.textContent=`${a} → ${i}`);let o,s=[],c=null,l=null,u=null,d=null;try{let[t,n,r,f,p,m]=await Promise.all([fetch(`${e}/m5t/v5/coreAdMetrics/overlay?from=${a}&to=${i}`),fetch(`${e}/m5t/v5/coreAdAccount`),fetch(`${e}/m5t/v5/coreAdMetrics/by-source?from=${a}&to=${i}`),Sc().catch(()=>null),fetch(`${e}/m5t/v5/coreAdMetrics/conversions?from=${a}&to=${i}`).then(e=>e.json()).catch(()=>null),fetch(`${e}/m5t/v5/coreAdMetrics/lsa?from=${a}&to=${i}`).then(e=>e.json()).catch(()=>null)]);if(!t.ok)throw Error(`HTTP ${t.status}`);o=await t.json(),s=((await n.json().catch(()=>({})))?.data||[]).filter(e=>e.status!==`DELETED`),c=await r.json().catch(()=>null),l=f,u=p,d=m}catch(e){console.warn(`[ad-performance] load failed:`,e),t&&(t.innerHTML=Cy(`error`,`Could not load ad performance. Please try again.`));return}uy(l,s),dy(u,o?.totals?.spend??null),fy(d);let f=o.totals||{};if(my(`adperf-spend`,yy(f.spend)),my(`adperf-clicks`,by(f.clicks)),my(`adperf-leads`,by(f.magnet_leads)),my(`adperf-cpl`,yy(f.cost_per_lead)),py(c),!s.length){t.innerHTML=Cy(`link_off`,`No ad account connected yet. Your agency can connect Google Ads or Local Services Ads to see performance here.`);return}if(!o.connected){t.innerHTML=Cy(`cloud_sync`,`Account connected. No ad data for this range yet. Metrics appear here once they sync.`);return}let p=o.rows||[];t.innerHTML=hy(p),gy(p)}function uy(e,t){let n=document.getElementById(`adperf-conn`);if(!n)return;if(!e||e.denied){n.innerHTML=``;return}let r=dc.google_ads,i=yc(e.providers,`google_ads`),a=bc(r,i,!!e.configured?.google_ads),o=Bc()?` <a href="#/settings/integrations/${r.urlSlug}">Manage</a>`:``;if(a===`connected`||a===`broken`){Ic(),n.innerHTML=`<span>Google Ads</span> ${xc(a,i)}${o}`;return}if(t.length){n.innerHTML=`<span>Google Ads</span> <span>${t.length} associated account${t.length===1?``:`s`}</span>`;return}n.innerHTML=``}function dy(e,t){let n=document.getElementById(`adperf-conv-card`),r=document.getElementById(`adperf-conv-body`),i=document.getElementById(`adperf-stats`),a=document.getElementById(`adperf-conv-stat`);a&&a.remove();let o=document.getElementById(`adperf-conv-cpc`);if(o&&o.remove(),!n||!r)return;if(!e?.ok||!e.has_selection){n.style.display=`none`;return}if(i){let n=document.createElement(`div`);n.innerHTML=A({icon:`flag`,label:`Ads Conversions`,value:by(e.totals?.conversions),sub:`Reported by Google Ads`,href:`#/settings/integrations/google-ads`});let r=n.firstElementChild;r.id=`adperf-conv-stat`,i.appendChild(r);let a=Number(e.totals?.conversions||0),o=t!=null&&a>0?Math.round(Number(t)/a*100)/100:null,s=document.createElement(`div`);s.innerHTML=A({icon:`request_quote`,label:`Cost / Conversion`,value:yy(o),sub:`Against the counted actions`,href:`#/analytics/conversions`});let c=s.firstElementChild;c.id=`adperf-conv-cpc`,i.appendChild(c)}let s=e.actions||[];if(n.style.display=``,!s.length){r.innerHTML=Cy(`search_off`,`No conversions in this range for the selected actions.`);return}let c=s.some(e=>e.value>0);r.innerHTML=`
      ...
      <table class="adperf-srctable">
        ...
        <tbody>${s.map(e=>`
          <tr>
            ...
            <td class="num"><b>${by(e.conversions)}</b></td>
            ${c?`<td class="num">${yy(e.value)}</td>`:``}
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>
    <p class="adperf-note">Counted actions are chosen on the Google Ads integration page. These are Google's own numbers and may overlap magnet leads, so they are never added together.</p>`}function fy(e){let t=document.getElementById(`adperf-lsa-card`),n=document.getElementById(`adperf-lsa-body`);if(!t||!n)return;if(!e?.ok||!e.connected){t.style.display=`none`;return}t.style.display=``;let r=e.totals||{},i=e.leads||[],a=e.disputes||[],o=r.answer_rate==null?`No call outcomes yet`:r.missed>0?`${by(r.missed)} missed call${r.missed===1?``:`s`}`:`Every call answered`;n.innerHTML=`
    ${`
    <div class="stats-grid" style="margin-bottom:16px">
      ${A({icon:`payments`,label:`LSA Spend`,value:yy(r.spend),sub:`Reported by Google`})}
      ${A({icon:`person_add`,label:`LSA Leads`,value:by(r.leads),sub:`${by(r.phone_calls)} calls · ${by(r.messages)} messages`})}
      ${A({icon:`paid`,label:`Charged`,value:by(r.charged),sub:`Leads Google billed for`})}
      ${A({icon:`request_quote`,label:`Cost / Charged Lead`,value:yy(r.cost_per_charged_lead)})}
      ${A({icon:`call`,label:`Answer Rate`,value:r.answer_rate==null?`—`:`${r.answer_rate}%`,sub:o})}
      ${A({icon:`event_available`,label:`Booked`,value:by(r.booked),sub:`Google's lead status`})}
    </div>`}
    ${i.length?`
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
    ...
    </div>`:Cy(`search_off`,`No Local Services leads in this range yet.`)}
    ${a.length?`
    <div class="card-header" style="padding:16px 0 0">
      ...
      <span class="live-indicator">${by(a.length)} inside the 30 day credit window</span>
    </div>
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
        ...
        <tbody>${a.map(e=>`
          <tr>
            ...
            <td class="num"><b>${by(e.days_left)}</b></td>
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>
    ...
    <p class="adperf-note">Numbers come from Google's Local Services feed. These leads land in your Leads inbox and do not flow through magnets.</p>`;let s=n.querySelector(`.adperf-srctable tbody`);s&&E(s,{rowSelector:`tr`,noun:`leads`,controlHost:n.querySelector(`.adperf-srcwrap`)||n})}function py(e){let t=document.getElementById(`adperf-sources`),n=document.getElementById(`adperf-src-summary`);if(!t)return;let r=e&&Array.isArray(e.rows)?e.rows:[];if(!r.length){n&&(n.textContent=``),t.innerHTML=Cy(`search_off`,`No leads in this range yet. Campaigns appear here as tagged leads (?utm_campaign=…) come in.`);return}let i=e.total_leads||r.reduce((e,t)=>e+(t.leads||0),0),a=e.tracked_leads||0;n&&(n.textContent=`${by(a)} of ${by(i)} leads tagged to a source`);let o=e=>e.campaign&&e.campaign!==`(none)`?Y(e.campaign):e.source&&e.source!==`(direct)`?Y(e.source):`<span class="adperf-src-untracked">Untracked / direct</span>`,s=e=>e.campaign&&e.campaign!==`(none)`?e.campaign:e.source&&e.source!==`(direct)`?e.source:``;t.innerHTML=`
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
        ...
        <tbody>${r.map(e=>{let t=s(e);return`
          <tr${t?` data-campaign="${Y(t)}" class="adperf-srcrow--link"`:``}>
            ...
            <td class="num">${e.paid_clicks?by(e.paid_clicks):`<span class="adperf-src-sub">—</span>`}</td>
            <td class="num"><b>${by(e.leads)}</b></td>
          </tr>`}).join(``)}
        </tbody>
      </table>
    </div>`;let c=t.querySelector(`.adperf-srctable tbody`);c&&E(c,{rowSelector:`tr`,noun:`sources`,controlHost:t.querySelector(`.adperf-srcwrap`)||t})}function my(e,t){let n=document.getElementById(e);n&&(n.textContent=t)}function hy(e){return Ia({rows:e,xLabel:e=>String(e.date||``).slice(5),left:{value:e=>e.spend||0,label:`Ad Spend`,fmt:e=>`$${Va(e)}`},right:{value:e=>e.magnet_leads||0,label:`Magnet Leads`},ariaLabel:`Ad spend versus magnet leads over time`,pointLabel:e=>`${e.date}: ${yy(e.spend||0)} spend, ${by(e.magnet_leads||0)} leads`,hitData:e=>({day:e.date})})}function gy(e){let t=document.getElementById(`adperf-body`);return t?Ra(t,{rows:e,tipHTML:e=>`
      <div class="m5c-tip__title">${Y(String(e.date||``))}</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--1"></i>${yy(e.spend||0)} spend</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--2"></i>${by(e.magnet_leads||0)} leads</div>`}):()=>{}}var _y,vy,yy,by,xy,Y,Sy,Cy,wy,Ty,Ey,Dy=o((()=>{w(),wa(),qc(),Na(),Ua(),un(),Aa(),_y=()=>C.getConfig?.().BASE_URL||``,vy=e=>e.toISOString().slice(0,10),yy=e=>e==null?`—`:`$`+Number(e).toLocaleString(void 0,{minimumFractionDigits:0,maximumFractionDigits:2}),by=e=>Number(e||0).toLocaleString(),xy={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Y=e=>String(e??``).replace(/[&<>"']/g,e=>xy[e]),Sy=!1,Cy=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,wy=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],Ty=30,Ey={render:sy,init:cy}})),Oy=c({default:()=>Zy,init:()=>My,parseBookingCsv:()=>Wy,render:()=>Ay});function ky(){if(Jy)return;let e=document.createElement(`style`);e.id=`m5t-adacct-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Jy=!0}function Ay(){return k()?ma({section:`ad-accounts`,title:`Ad Accounts`,description:`Pick a client to see their connected ad accounts.`}):(ky(),Ic(),`
    ...
    <div class="adacct-wrap">
      <table class="adacct-table">
        ...
        <tbody>${r.map(e=>`
          <tr>
            <td>${M(Ky[e.provider]||e.provider)}</td>
            <td class="adacct-cid">${M(qy(e.external_customer_id))}</td>
...
`&&a++,n.push(r),r=``,n.some(e=>e.trim()!==``)&&t.push(n),n=[]):r+=o}if(n.push(r),n.some(e=>e.trim()!==``)&&t.push(n),t.length<2)return[];let a=t[0].map(e=>e.trim().toLowerCase()),o=a.findIndex(e=>/e-?mail/.test(e)),s=a.findIndex(e=>/phone|mobile|tel/.test(e)),c=a.findIndex(e=>/date|time|booked|appointment|start/.test(e));return o<0&&s<0?[]:t.slice(1).map(e=>({email:o>=0?(e[o]||``).trim():``,phone:s>=0?(e[s]||``).trim():``,at:c>=0?(e[c]||``).trim():``})).filter(e=>e.email||e.phone).slice(0,5e3)}function Gy(e,t){let n=new Blob([e],{type:`text/csv;charset=utf-8`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.href=r,i.download=t,document.body.appendChild(i),i.click(),i.remove(),setTimeout(()=>URL.revokeObjectURL(r),4e3)}var Ky,qy,Jy,Yy,Xy,Zy,Qy=o((()=>{wa(),un(),qc(),Ky={google_ads:`Google Ads`,google_lsa:`Local Services Ads`},qy=e=>{let t=String(e||``).replace(/\D/g,``);return t.length===10?`${t.slice(0,3)}-${t.slice(3,6)}-${t.slice(6)}`:String(e||``)},Jy=!1,Yy=[],Xy=null,Zy={render:Ay,init:My}})),$y=c({default:()=>xb,init:()=>sb,openInviteModal:()=>db,render:()=>ob});function eb(e,t=`info`){try{mb.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}}function tb(){let e=C.getActiveOrg();if(e&&e.channel===hb)return{guid:e.guid,name:e.name};let t=C.getMemberships().find(e=>e.channel===hb);return t?{guid:t.orgGuid,name:t.name}:null}function nb(e){let t=e?.created_tz;if(!t)return``;let n=new Date(t);return Number.isNaN(n.getTime())?``:n.toLocaleDateString(void 0,{year:`numeric`,month:`short`,day:`numeric`})}function rb(){if(_b)return;let e=document.createElement(`style`);e.id=`m5t-clients-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),mr(),_b=!0}function ib(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function ab(e){let t=e.caption||e.guid,n=e.status||`ENABLED`;return`
    <div class="client-row">
      <span class="client-row__avatar">${X(yb(t))}</span>
      <span class="client-row__main">
        <div class="client-row__name" title="${X(t)}">${X(t)}</div>
        <div class="client-row__meta">Added ${X(nb(e.created))||`—`}</div>
      </span>
    ...
    <div class="page-shell" data-section="clients">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="clients-body">
          ${ib(`hourglass_empty`,`Loading clients…`)}
        </div>
      </div>
    </div>`}async function sb(){let e=tb();document.getElementById(`clients-add-btn`)?.addEventListener(`click`,()=>lb(e)),await cb(e)}async function cb(e){let t=document.getElementById(`clients-body`),n=document.getElementById(`clients-count`);if(t){if(!e?.guid){t.innerHTML=ib(`info`,`Client provisioning is available to reseller / agency accounts.`),n&&(n.textContent=``);return}try{let r=C.getConfig?.().BASE_URL||``,i=await fetch(`${r}/m5t/v5/acctGroup`);if(!i.ok)throw Error(`HTTP ${i.status}`);let a=await i.json(),o=(Array.isArray(a?.data)?a.data:[]).filter(t=>t.class===gb&&(!t.parent||t.parent===e.guid));if(!o.length){t.innerHTML=ib(`group_off`,`No clients yet. Add your first client to start managing magnets on their behalf.`),n&&(n.textContent=``);return}t.innerHTML=`<div class="client-list">${o.map(ab).join(``)}</div>`,E(t.querySelector(`.client-list`),{rowSelector:`.client-row`,noun:`clients`}),n&&(n.textContent=`${o.length} client${o.length===1?``:`s`}`),t.querySelectorAll(`.client-card__invite`).forEach(e=>{e.addEventListener(`click`,()=>{e.dataset.action===`adaccounts`?ub(e.dataset.clientGuid,e.dataset.clientName):e.dataset.action===`questionnaire`?pb(e.dataset.clientGuid,e.dataset.clientName):db(e.dataset.clientGuid,e.dataset.clientName)})})}catch(e){console.warn(`[clients] load failed:`,e),t.innerHTML=ib(`error`,`Couldn’t load clients. Please try again.`),n&&(n.textContent=``)}}}function lb(e){if(!e?.guid){eb(`Only reseller / agency accounts can add clients.`,`error`);return}let t=document.createElement(`div`);t.className=`client-modal-scrim`;let n=document.createElement(`div`);n.className=`client-modal`,n.setAttribute(`role`,`dialog`),n.setAttribute(`aria-modal`,`true`),n.innerHTML=`
    ...
    <div class="client-modal__body" id="qm-body">${ib(`hourglass_empty`,`Loading…`)}</div>`,document.body.appendChild(r),document.body.appendChild(i),requestAnimationFrame(()=>{r.classList.add(`open`),i.classList.add(`open`)});let a=()=>{r.classList.remove(`open`),i.classList.remove(`open`),document.removeEventListener(`keydown`,o),setTimeout(()=>{r.remove(),i.remove()},200)},o=e=>{e.key===`Escape`&&a()};document.addEventListener(`keydown`,o),r.addEventListener(`click`,a),i.querySelector(`.client-modal__close`)?.addEventListener(`click`,a);let s=i.querySelector(`#qm-body`);function c(){s.innerHTML=`
      ...
      </div>`,s.querySelector(`[data-act="done"]`)?.addEventListener(`click`,a),s.querySelector(`[data-act="resend"]`)?.addEventListener(`click`,()=>c())}function d(e,t=!1){let r=e.editability||{editable:e.status===`SUBMITTED`&&!e.locked_tz,reason:e.locked_tz?`finalized`:null,deadline:null},i=!!r.editable,o=!i&&e.status===`SUBMITTED`?r.reason===`finalized`?`<div class="qm-status" data-status="LOCKED"><span class="material-symbols-outlined" aria-hidden="true">lock</span> Finalized${e.locked_tz?` ${X(fb(e.locked_tz))}`:``}${e.locked_by_email?` by ${X(e.locked_by_email)}`:``} — no further edits.</div>`:`<div class="qm-status" data-status="LOCKED"><span class="material-symbols-outlined" aria-hidden="true">lock_clock</span> Editing closed${r.deadline?` ${X(fb(r.deadline))}`:``} — a response locks 14 days after it is submitted.</div>`:``,c=i&&r.deadline?`<p class="client-modal__sub qm-editable-hint">You can edit these answers until ${X(fb(r.deadline))}.</p>`:``;s.innerHTML=`
      <div class="qm-status" data-status="SUBMITTED"><span class="material-symbols-outlined" aria-hidden="true">task_alt</span> Submitted ${e.submitted_tz?X(fb(e.submitted_tz)):``}</div>
      ${o}
      ${pr(e)}
      <form id="qm-form">
      ...
      </div>`,s.querySelector(`[data-act="done"]`)?.addEventListener(`click`,a),s.querySelector(`[data-act="cancel-edit"]`)?.addEventListener(`click`,()=>d(e,!1)),s.querySelector(`[data-act="edit"]`)?.addEventListener(`click`,()=>d(e,!0));let l=s.querySelector(`[data-act="save"]`);l?.addEventListener(`click`,async()=>{let t=fr(s);l.disabled=!0;let r=l.innerHTML;l.innerHTML=`<span class="material-symbols-outlined" aria-hidden="true">progress_activity</span> Saving…`;let i;try{i=await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e.guid)}`,{method:`PUT`,headers:{"Content-Type":`application/json`},body:JSON.stringify({answers:t})});let r=await i.json().catch(()=>({}));if(!i.ok||!r.ok)throw Error(r.error||r.message||`Failed (HTTP ${i.status})`);eb(`Answers updated`,`success`);let a=await(await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e.guid)}`)).json().catch(()=>({}));d(a.ok?a.data:{...e,answers:t},!1)}catch(t){if(i&&i.status===423){eb(t.message||`This questionnaire is now locked.`,`info`);let r=await(await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e.guid)}`)).json().catch(()=>({}));d(r.ok?r.data:e,!1);return}s.querySelector(`#qm-error`).textContent=t.message||`Something went wrong. Please try again.`,l.disabled=!1,l.innerHTML=r}})}try{let t=await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse?owner_guid=${encodeURIComponent(e)}`),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`HTTP ${t.status}`);let i=(r.data||[])[0]||null;if(!i){c();return}if(i.status===`DRAFT`){l(i);return}if(i.status===`CANCELLED`){u(i);return}let a=await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(i.guid)}`),o=await a.json().catch(()=>({}));if(!a.ok||!o.ok)throw Error(o.error||`HTTP ${a.status}`);d(o.data)}catch(e){console.warn(`[clients] questionnaire load failed:`,e),s.innerHTML=`<p class="client-modal__error">Could not load questionnaire status. ${X(e.message||``)}</p>`}}var mb,hb,gb,_b,vb,X,yb,bb,xb,Sb=o((()=>{w(),rt(),mb=u(f(),1),m(),un(),vr(),hb=`reseller_org`,gb=`managed_client`,_b=!1,vb={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},X=e=>String(e??``).replace(/[&<>"']/g,e=>vb[e]),yb=e=>String(e||`?`).trim().split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``).toUpperCase()||`?`,bb=e=>String(e||``).replace(/[^0-9]/g,``),xb={render:ob,init:sb}})),Cb=c({default:()=>Hb,init:()=>Mb,loadTeam:()=>Nb,render:()=>jb});function wb(){let e=C.getActiveOrg(),t=e&&e.channel!==Ib?e:null;if(!t){let e=C.getMemberships().find(e=>e.channel!==Ib);e&&(t={guid:e.orgGuid,channel:e.channel,memberRole:e.memberRole})}if(!t)return null;let n=C.getMemberships().find(e=>e.orgGuid===t.guid)?.name;return{...t,name:n}}function Tb(e){return Object.values(Re).filter(t=>Le(e,t.value)).sort((e,t)=>t.rank-e.rank)}function Eb(){if(Lb)return;let e=document.createElement(`style`);e.id=`m5t-team-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Lb=!0}function Db(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function Ob(e,t,n){let r=e.member_role||`member`,i=e.entity_guid&&e.entity_guid===C.getEntity?.()?.guid,a=n&&!i&&Tb(t.memberRole).some(e=>e.value===r);return`
    <div class="team-row">
      <span class="team-avatar">${zb(Vb(e.name||e.email))}</span>
      <span class="team-main">
        <div class="team-name">${zb(e.name||e.email)}${e.verified?``:` · <span style="color:var(--color-text-subtle,#6C7C9A);font-weight:400;">pending verification</span>`}</div>
        <div class="team-email" title="${zb(e.email)}">${zb(e.email)}</div>
      </span>
      <span class="team-role" data-role="${zb(r)}">${zb(Re[r]?.label||r)}</span>
      ${a?`<button class="team-modal__btn team-modal__btn--ghost team-remove" type="button" data-id="${zb(e.id)}" data-name="${zb(e.name||e.email)}">Remove</button>`:``}
    </div>`}async function kb(e,t){let{uid:n,email:r}=e.dataset;if(window.confirm(`Revoke the invitation for ${r}?\n\nThe invitation link stops working immediately, including any copy already sent. You can invite this address again afterwards.`)){e.disabled=!0;try{let e=C.getConfig?.().BASE_URL||``,i=await fetch(`${e}/m5t/v5/acctGroup/${encodeURIComponent(t.guid)}/invites/${encodeURIComponent(n)}`,{method:`DELETE`}),a=await i.json().catch(()=>({}));if(!i.ok||!a.ok)throw Error(a.message||a.error||`HTTP ${i.status}`);Bb(`Invitation for ${r} revoked`,`success`)}catch(t){e.disabled=!1,Bb(t.message||`Could not revoke that invitation.`,`error`);return}try{await Nb(t)}catch{}}}async function Ab(e,t){let{id:n,name:r}=e.dataset;if(window.confirm(`Remove ${r} from the team?\n\nThey lose access to this workspace immediately. Their account itself is not deleted, and you can invite them again later.`)){e.disabled=!0;try{let e=C.getConfig?.().BASE_URL||``,t=await fetch(`${e}/m5t/v5/acctGroupMember/${encodeURIComponent(n)}`,{method:`DELETE`}),i=await t.json().catch(()=>({}));if(!t.ok||!i.ok)throw Error(i.message||i.error||`HTTP ${t.status}`);Bb(`${r} removed from the team`,`success`)}catch(t){e.disabled=!1,Bb(t.message||`Could not remove that member.`,`error`);return}try{await Nb(t)}catch{}}}function jb(){return Eb(),`
    <div class="page-shell" data-section="team">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="team-body">
          ${Db(`hourglass_empty`,`Loading team…`)}
        </div>
      </div>
    </div>`}async function Mb(){let e=wb(),t=document.getElementById(`team-invite-btn`);if(!e){t&&(t.style.display=`none`),document.getElementById(`team-body`).innerHTML=Db(`info`,`Team management is available for agency and workspace accounts.`);return}t?.addEventListener(`click`,()=>Pb(e)),await Nb(e)}async function Nb(e){let t=document.getElementById(`team-body`),n=document.getElementById(`team-count`);if(t)try{let r=C.getConfig?.().BASE_URL||``,[i,a]=await Promise.all([fetch(`${r}/m5t/v5/acctGroup/${encodeURIComponent(e.guid)}/members`),fetch(`${r}/m5t/v5/acctGroup/${encodeURIComponent(e.guid)}/invites`)]),[o,s]=await Promise.all([i.json().catch(()=>({})),a.json().catch(()=>({}))]);if(!i.ok||!a.ok)throw Error(`Failed to load team (members: ${i.status}, invites: ${a.status})`);let c=o.members||[],l=s.invites||[],u=Tb(e.memberRole).length>0;t.innerHTML=(l.length?`
      <div class="team-pending">
        <div class="team-pending__label">Pending invites</div>
        ${l.map(e=>`
          <div class="team-pending__row">
            <span class="team-pending__email" title="${zb(e.email)}">${zb(e.email)}${e.expired?` · expired`:``}</span>
            <button class="team-modal__btn team-modal__btn--ghost team-copy" type="button" data-url="${zb(e.inviteUrl)}">Copy link</button>
            ${u?`<button class="team-modal__btn team-modal__btn--ghost team-revoke" type="button" data-uid="${zb(e.uid)}" data-email="${zb(e.email)}">Revoke</button>`:``}
          </div>`).join(``)}
      </div>`:``)+(c.length?`<div class="team-list">${c.map(t=>Ob(t,e,u)).join(``)}</div>`:Db(`group`,`No members yet.`)),n&&(n.textContent=`${c.length} member${c.length===1?``:`s`}`),t.querySelectorAll(`.team-copy`).forEach(e=>e.addEventListener(`click`,async()=>{try{await navigator.clipboard.writeText(e.dataset.url),e.textContent=`Copied!`,setTimeout(()=>{e.textContent=`Copy link`},1500)}catch{}})),t.querySelectorAll(`.team-revoke`).forEach(t=>t.addEventListener(`click`,()=>kb(t,e))),t.querySelectorAll(`.team-remove`).forEach(t=>t.addEventListener(`click`,()=>Ab(t,e)));let d=t.querySelector(`.team-list`);d&&E(d,{rowSelector:`.team-row`,noun:`members`});let f=t.querySelector(`.team-pending`);f&&E(f,{rowSelector:`.team-pending__row`,noun:`invites`})}catch(e){console.warn(`[team] load failed:`,e),t.innerHTML=Db(`error`,`Couldn’t load the team. Please try again.`)}}function Pb(e){let t=Tb(e.memberRole);if(!t.length){Bb(`You don’t have permission to invite teammates.`,`error`);return}let n=document.createElement(`div`);n.className=`team-modal-scrim`;let r=document.createElement(`div`);r.className=`team-modal`,r.setAttribute(`role`,`dialog`),r.setAttribute(`aria-modal`,`true`),r.innerHTML=`
    ...
    <div class="team-modal__body" id="team-invite-body">
      <p class="team-modal__sub">They'll get a link to set a password and join <strong>${zb(e.name||`your workspace`)}</strong>.</p>
      ...
      <select class="team-modal__select" id="tm-role">
        ${t.map(e=>`<option value="${e.value}"${e.value===`member`?` selected`:``}>${zb(e.label)}</option>`).join(``)}
      </select>
      ...
      <div class="team-modal__actions">
        ...
        </div>`;let u=r.querySelector(`#tm-link`);r.querySelector(`#tm-copy`)?.addEventListener(`click`,async()=>{try{await navigator.clipboard.writeText(u.value),r.querySelector(`#tm-copy`).textContent=`Copied!`}catch{u.select()}}),r.querySelector(`[data-act="done"]`)?.addEventListener(`click`,()=>{i(),Nb(e)})}catch(e){console.warn(`[team] invite failed:`,e),o.textContent=e.message||`Something went wrong. Please try again.`,s.disabled=!1,s.innerHTML=l}})}var Fb,Ib,Lb,Rb,zb,Bb,Vb,Hb,Ub=o((()=>{w(),Fb=u(f(),1),m(),rt(),un(),Ib=`managed_client`,Lb=!1,Rb={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},zb=e=>String(e??``).replace(/[&<>"']/g,e=>Rb[e]),Bb=(e,t=`info`)=>{try{Fb.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},Vb=e=>String(e||`?`).trim().split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``).toUpperCase()||`?`,Hb={render:jb,init:Mb}}));m(),Ee(),w(),qt(),y(),Nn(),Bn(),st(),window.m5t=Object.create(null),window.m5t.pubsub=b.default,console.log(`StreamSyncEngage SaaS Client - Starting...`),console.log(`Environment:`,`production`),console.log(`Document readyState:`,document.readyState),console.log(`window.mvvLegit:`,typeof window.mvvLegit),console.log(`globalThis.mvvLegit:`,typeof globalThis.mvvLegit);try{console.log(`mvvLegit (direct):`,typeof mvvLegit)}catch(e){console.log(`mvvLegit (direct): ReferenceError -`,e.message)}var Wb=Object.keys(window).filter(e=>e.toLowerCase().includes(`mvv`)||e.toLowerCase().includes(`neodigm`)||e.toLowerCase().includes(`legit`));console.log(`Possible Neodigm globals:`,Wb);var Z=new ee.default(`/`,{hash:!0});window.router=Z;function Q(e){let t=e?`${e.replace(/-/g,`_`)}_route`:`splash_route`;return console.log(`[Router] Checking access: ${e} -> ${t}`),mvvLegit.isRouteAllowed(t)?!0:(console.warn(`[Router] Access denied to route: ${t}`),b.default.publish(p.UI_TOAST,{message:`Please sign in to access this page`,type:`warning`}),!1)}function $(e){console.log(`[Router] Navigating to:`,e),Z.navigate(`/${e}`)}var Gb=new Set([`splash`,`signin`,`signup`,`forgot`,`resethash`,`verf-link`,`accept-invite`,`signout`]),Kb={},qb={},Jb=null,Yb={admin:{caption:`Platform Admin`,icon:`shield_person`},reseller:{caption:`Reseller`,icon:`handshake`},managed:{caption:`Managed`,icon:`visibility`},self_serve:{caption:`Self-Serve`,icon:`rocket_launch`}},Xb=`__portfolio__`;function Zb(){try{Z._setCurrent(null)}catch{}Z.resolve()}function Qb(){let e=document.getElementById(`m5t-side-nav`);if(!e)return;let t=!C.getScope()&&C.isAuthenticated(),n=C.getCapabilities(),r=C.getUiMode(),i=Qt.getSideNav(n,{uiMode:r});Kb={},qb={};let a=e=>{e.route&&(Kb[e.route]={sectionToken:e.token,childToken:null,children:e.children||null},Array.isArray(e.children)&&e.children.forEach(t=>{t.route&&(qb[t.token]=t.route,Kb[t.route]={sectionToken:e.token,childToken:t.token,children:e.children})}))},o=e=>e.forEach(e=>{e.group&&Array.isArray(e.items)?o(e.items):a(e)});o(i);let s=C.getMemberships(),c=C.getActiveOrg(),l=C.getManagedClients(),u=C.getStoredActiveOrg(),d=l.length?[{guid:Xb,name:`Portfolio (all clients)`},...s.map(e=>({guid:e.orgGuid,name:e.name||e.channel,channel:e.channel,group:`Your organization`})),...l.map(e=>({guid:e.orgGuid,name:e.name||e.orgGuid,channel:e.channel,group:`Clients`}))]:s.map(e=>({guid:e.orgGuid,name:e.name||e.channel,channel:e.channel}));e.setState({buttons:i,orgs:d,activeOrg:l.length?u||Xb:c?.guid||null,footerBadge:Yb[r]||null,loading:t}),ex(),Jb&&tx(Jb)}var $b=e=>String(e??``).replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`})[e]);function ex(){let e=document.getElementById(`app-content`);if(!e)return;let t=document.getElementById(`org-context-banner`);t||(t=document.createElement(`div`),t.id=`org-context-banner`,e.insertBefore(t,e.firstChild),wn(t));let n=C.getActiveOrg(),r=C.isPreviewingAsClient(),i=!!n?.viaSubtree&&!!Jb&&!Gb.has(Jb);if(t.hidden=!i,!i){t.innerHTML=``;return}let a=$b(n.name||`Client`);t.innerHTML=`
    <style>
