...
`))}</textarea>`}return`<div class="mag-cta" data-type="${e}" data-kind="${n.kind}">
    ...
    .page-shell .mag-health { appearance: none; border: 0; background: none; cursor: pointer; font: inherit; font-size: 0.84rem; display: inline-flex; align-items: center; gap: 5px; color: var(--color-text-muted, #4A5A78); padding: 5px 8px; border-radius: 8px; white-space: nowrap; }
    ...
    .page-shell .mag-health .material-symbols-outlined { font-size: 17px; }
    ...
    .page-shell .mag-mode { align-self: flex-start; display: inline-flex; align-items: center; gap: 5px; font-size: 0.76rem; font-weight: 600; color: var(--color-text-muted, #4A5A78); background: var(--color-surface-2, #EAF0F7); padding: 3px 10px; border-radius: 999px; }
    .page-shell .mag-mode .material-symbols-outlined { font-size: 15px; }
    .page-shell .mag-stats { display: flex; align-items: center; gap: 13px; font-size: 0.84rem; color: var(--color-text-muted, #4A5A78); }
    .page-shell .mag-stats span { display: inline-flex; align-items: center; gap: 4px; }
    .page-shell .mag-stats .material-symbols-outlined { font-size: 16px; color: var(--color-text-subtle, #6C7C9A); }
  ...
  `,document.head.appendChild(e),_p=!0}function mf(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function hf(e){let t=z.stats[e];if(!t||!t.opens&&!t.leads)return`<div class="mag-stats"><span><span class="material-symbols-outlined" aria-hidden="true">monitoring</span>No activity yet (30d)</span></div>`;let n=e=>Number(e||0).toLocaleString();return`<div class="mag-stats" title="Trailing 30 days">
    ...
    <div class="mag-card${o?` is-open`:``}" data-guid="${B(e.guid)}">
      <div class="mag-line">
        ...
        <div class="mag-right">
          ...
          <div class="mag-rowactions">
            ...
            <button class="mag-btn${o?` is-open`:``}" data-action="install-toggle" data-guid="${B(e.guid)}" aria-expanded="${o}" aria-controls="mag-detail-${B(e.guid)}"><span class="material-symbols-outlined" aria-hidden="true">${o?`expand_less`:`expand_more`}</span>Install</button>
      ...
      <div class="mag-detail" id="mag-detail-${B(e.guid)}" ${o?``:`hidden`}>
        <div class="mag-detail__hd">
          ...
          <span class="mag-detail__n" title="${B(e.name)}">${B(e.name)}</span>
          <button class="mag-detail__x" type="button" data-action="install-toggle" data-guid="${B(e.guid)}" title="Close"><span class="material-symbols-outlined" aria-hidden="true">close</span></button>
        </div>
        <div class="mag-embed">
