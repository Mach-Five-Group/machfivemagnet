...
`))}</textarea>`}return`<div class="mag-cta" data-type="${e}" data-kind="${a.kind}">
    ...
    ${r}
  </div>`}function dr(){if(ma)return;const e=document.createElement("style");e.id="m5t-magnets-styles",e.textContent=`
  ...
  `,document.head.appendChild(e),ma=!0}function yt(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function pr(e){const t=G.stats[e];if(!t||!t.opens&&!t.leads)return'<div class="mag-stats"><span><span class="material-symbols-outlined" aria-hidden="true">monitoring</span>No activity yet (30d)</span></div>';const a=n=>Number(n||0).toLocaleString();return`<div class="mag-stats" title="Trailing 30 days">
  ...
  </div>`}function ur(e){const t=G.owners.length>1||T.getMemberships().length>1,a=(e.domains||[]).filter(Boolean),n=G.canManage;return`
    ...
    </div>`}async function gr(){document.getElementById("mag-new-btn")?.addEventListener("click",()=>rn(null)),document.getElementById("mag-body")?.addEventListener("click",hr),await Me()}async function Me(){const e=document.getElementById("mag-body");if(!e)return;try{const[a,n]=await Promise.all([fetch(`${ve()}/m5t/v5/magnets`),fetch(`${ve()}/m5t/v5/magnets/stats`).catch(()=>null)]),o=await a.json().catch(()=>({}));if(!o.ok)throw new Error(o.message||"Failed to load");G.magnets=o.magnets||[],G.owners=o.owners||[],G.canProvision=!!o.canProvision,G.canManage=!!o.canManage,G.primaryOrgGuid=o.primaryOrgGuid||null;const s=n?await n.json().catch(()=>({})):{};G.stats=s?.stats||{}}catch{e.innerHTML=yt("error","Could not load your magnets. Please try again.");return}const t=document.getElementById("mag-new-btn");if(t&&(t.style.display=G.canProvision?"":"none"),!G.magnets.length){e.innerHTML=yt("widgets",G.canProvision?"No magnets yet. Click “New magnet” to create your first.":"No magnets yet.");return}e.innerHTML=`<div class="mag-grid">${G.magnets.map(ur).join("")}</div>`}function hr(e){const t=e.target.closest("[data-action]");if(!t)return;const a=t.dataset.guid,n=G.magnets.find(s=>s.guid===a),o=t.dataset.action;if(o==="copy")return fr(a);if(n){if(o==="preview")return Ct(()=>kt(n));if(o==="edit")return rn(n);if(o==="flow")return ar(n,Me);if(o==="toggle")return vr(n);if(o==="delete")return _r(n)}}async function fr(e){try{await navigator.clipboard.writeText(nn(e)),fe("Embed snippet copied","success")}catch{fe("Copy failed","danger")}}async function vr(e){const t=e.status==="ENABLED"?"DISABLED":"ENABLED";try{if(!(await(await fetch(`${ve()}/m5t/v5/magnets/${encodeURIComponent(e.guid)}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({status:t})})).json().catch(()=>({}))).ok)throw new Error;fe(t==="ENABLED"?"Magnet enabled":"Magnet disabled","success"),await Me()}catch{fe("Update failed","danger")}}async function _r(e){if(window.confirm(`Delete “${e.name}”? Visitors will no longer see this magnet.`))try{if(!(await(await fetch(`${ve()}/m5t/v5/magnets/${encodeURIComponent(e.guid)}`,{method:"DELETE"})).json().catch(()=>({}))).ok)throw new Error;fe("Magnet deleted","success"),await Me()}catch{fe("Delete failed","danger")}}function ha(e){const t=[];return e.querySelectorAll(".mag-cta").forEach(a=>{if(!a.querySelector(".mag-cta__on").checked)return;const n=a.dataset.type,o=a.dataset.kind,s=a.querySelector(".mag-cta__val"),r=s?s.value.trim():"";if(o==="value")r&&t.push({type:n,value:r});else if(o==="book"){const i={type:n};r&&(/^https?:\/\//i.test(r)?i.value=r:i.config={meeting_label:r}),t.push(i)}else if(o==="support"){const i=(a.querySelector(".mag-cta__opts")?.value||"").split(`
`).map(c=>c.trim()).filter(Boolean).map(c=>({label:c})),l={type:n};i.length&&(l.config={options:i}),t.push(l)}else t.push({type:n})}),{display_mode:e.querySelector('input[name="mag-display"]:checked')?.value||"chat",theme_mode:e.querySelector('input[name="mag-theme"]:checked')?.value||"auto",title:e.querySelector("#mag-f-title").value.trim(),theme_color:e.querySelector("#mag-f-color").value,subtitle:e.querySelector("#mag-f-subtitle").value,launcher_label:e.querySelector("#mag-f-launcher").value.trim(),logo_url:e.querySelector("#mag-f-logo").value.trim(),logo_fit:e.querySelector("#mag-f-logo-fit")?.value==="contain"?"contain":"cover",summary:e.querySelector("#mag-f-summary").checked,summary_heading:e.querySelector("#mag-f-summary-heading").value.trim(),home:e.querySelector("#mag-f-home").checked,welcome:e.querySelector("#mag-f-welcome").value,ab:sn(e),actions:t}}function sn(e){const t={},a=e.querySelector("#mag-f-ab-title")?.value.trim(),n=e.querySelector("#mag-f-ab-welcome")?.value.trim(),o=e.querySelector("#mag-f-ab-color")?.value||"",s=e.querySelector("#mag-f-color")?.value||"";return a&&(t.title=a),n&&(t.welcome=n),o&&o.toLowerCase()!==s.toLowerCase()&&(t.theme_color=o),{enabled:!!e.querySelector("#mag-f-ab")?.checked,split:Number(e.querySelector("#mag-f-ab-split")?.value)||50,b:t}}function rn(e){const t=!!e,a=!t&&G.owners.length>1,n=e?.display_mode||"chat",o=e?.theme_mode||"auto",s={};(e?.actions||[]).forEach(g=>{g&&g.type&&(s[g.type]=g)});const r=t?!!e.home:!0,i=t?e.subtitle!=null?e.subtitle:"":"Typically replies in a few minutes",l=document.createElement("div");l.className="mag-modal-scrim",l.innerHTML=`
  ...
  `,document.head.appendChild(e),fa=!0}function Rt(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function Er(e){return`<select class="leads-status" data-id="${H(e.id)}" data-act="status" onclick="event.stopPropagation()">
    ${ln.map(t=>`<option value="${t}" ${e.status===t?"selected":""}>${t[0]}${t.slice(1).toLowerCase()}</option>`).join("")}
  </select>`}function Ar(e){const t=D.magnetNames[e.magnet_guid]||e.lead_source||"—";return`
    <tr class="leads-row" data-id="${H(e.id)}">
      <td>
        <span class="leads-who">
          <span class="leads-avatar">${H(wr(e))}</span>
      ...
      <td>${D.canManage?Er(e):`<span class="leads-badge" data-s="${H(e.status)}">${H(e.status)}</span>`}</td>
      <td class="leads-muted" style="white-space:nowrap">${H(Ot(e))}</td>
    </tr>`}function cn(){return D.filter==="ALL"?D.leads:D.leads.filter(e=>e.status===D.filter)}function Sr(){return xr(),`
    ...
    <div class="leads-tablewrap">
      <table class="leads-table">
        ...
        <tbody id="leads-tbody">${a.map(Ar).join("")||`<tr><td colspan="6"><div class="leads-muted" style="padding:14px 4px">No ${D.filter.toLowerCase()} leads.</div></td></tr>`}</tbody>
      </table>
    </div>`,e.querySelector(".leads-filter")?.addEventListener("click",n=>{const o=n.target.closest("[data-filter]");o&&(D.filter=o.dataset.filter,$t())}),e.querySelector("#leads-tbody")?.addEventListener("change",Cr),e.querySelector("#leads-tbody")?.addEventListener("click",n=>{if(n.target.closest(".leads-status"))return;const o=n.target.closest(".leads-row");o&&Ir(o.dataset.id)})}async function Cr(e){const t=e.target.closest(".leads-status");if(!t)return;const a=t.dataset.id,n=D.leads.find(r=>String(r.id)===String(a)),o=t.value,s=n?.status;try{if(!(await(await fetch(`${wt()}/m5t/v5/m5mLead/${encodeURIComponent(a)}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({status:o})})).json().catch(()=>({}))).ok)throw new Error;n&&(n.status=o),Ke(`Marked ${o[0]}${o.slice(1).toLowerCase()}`,"success"),D.filter!=="ALL"&&$t()}catch{t.value=s||"NEW",Ke("Could not update status","danger")}}function Ir(e){const t=D.leads.find(r=>String(r.id)===String(e));if(!t)return;const a=D.magnetNames[t.magnet_guid]||t.lead_source||"—",n=(r,i)=>i?`<div class="leads-drawer__field"><span class="leads-drawer__k">${H(r)}</span><span class="leads-drawer__v">${H(i)}</span></div>`:"",o=document.createElement("div");o.className="leads-drawer-scrim",o.innerHTML=`
    ...
    </aside>`,document.body.appendChild(o),requestAnimationFrame(()=>{o.classList.add("open"),o.querySelector(".leads-drawer").classList.add("open")});const s=()=>{o.classList.remove("open"),setTimeout(()=>o.remove(),220)};o.querySelector(".leads-drawer__close").addEventListener("click",s),o.addEventListener("click",r=>{r.target===o&&s()})}function Nr(){const e=cn();if(!e.length){Ke("Nothing to export","info");return}const t=["first_name","last_name","email","phone","company","product_interest","lead_source","utm_source","utm_medium","utm_campaign","gclid","status"],a=l=>`"${String(l??"").replace(/"/g,'""')}"`,o=[[...t,"magnet","captured"].join(",")];for(const l of e)o.push([...t.map(c=>a(l[c])),a(D.magnetNames[l.magnet_guid]||l.lead_source||""),a(Ot(l))].join(","));const s=new Blob([o.join(`
`)],{type:"text/csv"}),r=URL.createObjectURL(s),i=document.createElement("a");i.href=r,i.download=`leads-${D.filter.toLowerCase()}.csv`,document.body.appendChild(i),i.click(),i.remove(),URL.revokeObjectURL(r),Ke(`Exported ${e.length} lead${e.length===1?"":"s"}`,"success")}const Lr=Object.freeze(Object.defineProperty({__proto__:null,init:Tr,render:Sr},Symbol.toStringTag,{value:"Module"})),Or=()=>T.getConfig?.().BASE_URL||"",va=e=>e.toISOString().slice(0,10),_a=e=>e==null?"—":"$"+Number(e).toLocaleString(void 0,{minimumFractionDigits:0,maximumFractionDigits:2}),Se=e=>Number(e||0).toLocaleString(),Rr={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"},Ue=e=>String(e??"").replace(/[&<>"']/g,t=>Rr[t]);let ba=!1;function $r(){if(ba)return;const e=document.createElement("style");e.id="m5t-adperf-styles",e.textContent=`
  ...
  <div class="stat-card">
  ...
  </div>`,Mr=[{d:7,label:"7 Days"},{d:30,label:"30 Days"},{d:90,label:"90 Days"}];let Je=30;function dn(){return $r(),`
    <div class="page-shell" data-section="ad-performance">
      <div class="page-header">
        ...
        <div class="chart-controls" id="adperf-periods">
          ${Mr.map(e=>`<button class="chart-period-btn${e.d===Je?" active":""}" type="button" data-days="${e.d}">${e.label}</button>`).join("")}
        </div>
      </div>
    ...
    </div>`}async function pn(){const e=document.getElementById("adperf-periods");e?.addEventListener("click",t=>{const a=t.target.closest("button[data-days]");a&&(Je=Number(a.dataset.days)||30,e.querySelectorAll(".chart-period-btn").forEach(n=>n.classList.toggle("active",Number(n.dataset.days)===Je)),ya())}),await ya()}async function ya(){const e=Or(),t=document.getElementById("adperf-body"),a=document.getElementById("adperf-range"),n=new Date,o=va(n),s=va(new Date(n.getTime()-(Je-1)*864e5));a&&(a.textContent=`${s} → ${o}`);let r,i=[],l=null;try{const[g,y,h]=await Promise.all([fetch(`${e}/m5t/v5/coreAdMetrics/overlay?from=${s}&to=${o}`),fetch(`${e}/m5t/v5/coreAdAccount`),fetch(`${e}/m5t/v5/coreAdMetrics/by-source?from=${s}&to=${o}`)]);if(!g.ok)throw new Error(`HTTP ${g.status}`);r=await g.json(),i=((await y.json().catch(()=>({})))?.data||[]).filter(b=>b.status!=="DELETED"),l=await h.json().catch(()=>null)}catch(g){console.warn("[ad-performance] load failed:",g),t&&(t.innerHTML=Te("error","Could not load ad performance. Please try again."));return}const c=r.totals||{};if(He("adperf-spend",_a(c.spend)),He("adperf-clicks",Se(c.clicks)),He("adperf-leads",Se(c.magnet_leads)),He("adperf-cpl",_a(c.cost_per_lead)),Fr(l),!i.length){t.innerHTML=Te("link_off","No ad account connected yet. Your agency can connect Google Ads or Local Services Ads to see performance here.");return}if(!r.connected){t.innerHTML=Te("cloud_sync","Account connected — no ad data for this range yet. Metrics appear here once they sync.");return}t.innerHTML=Pr(r.rows||[])}function Fr(e){const t=document.getElementById("adperf-sources"),a=document.getElementById("adperf-src-summary");if(!t)return;const n=e&&Array.isArray(e.rows)?e.rows:[];if(!n.length){a&&(a.textContent=""),t.innerHTML=Te("search_off","No leads in this range yet. Campaigns appear here as tagged leads (?utm_campaign=…) come in.");return}const o=e.total_leads||n.reduce((i,l)=>i+(l.leads||0),0),s=e.tracked_leads||0;a&&(a.textContent=`${Se(s)} of ${Se(o)} leads tagged to a source`);const r=i=>i.campaign&&i.campaign!=="(none)"?Ue(i.campaign):i.source&&i.source!=="(direct)"?Ue(i.source):'<span class="adperf-src-untracked">Untracked / direct</span>';t.innerHTML=`
    ...
    <svg class="adperf-chart" viewBox="0 0 840 340" preserveAspectRatio="xMidYMid meet" role="img" aria-label="Ad spend versus magnet leads over time">
      ${K}${j}${pe}${re}${ye}
    </svg>`}const Dr={render:dn,init:pn},Ur=Object.freeze(Object.defineProperty({__proto__:null,default:Dr,init:pn,render:dn},Symbol.toStringTag,{value:"Module"})),Ea="reseller_org",un="managed_client";let Aa=!1;const Br={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"},U=e=>String(e??"").replace(/[&<>"']/g,t=>Br[t]);function Xe(e,t="info"){try{u.publish(m.UI_TOAST,{message:e,type:t})}catch{}}function Hr(){const e=T.getActiveOrg();if(e&&e.channel===Ea)return{guid:e.guid,name:e.name};const t=T.getMemberships().find(a=>a.channel===Ea);return t?{guid:t.orgGuid,name:t.name}:null}function zr(e){const t=e?.created_tz;if(!t)return"";const a=new Date(t);return Number.isNaN(a.getTime())?"":a.toLocaleDateString(void 0,{year:"numeric",month:"short",day:"numeric"})}function Gr(){if(Aa)return;const e=document.createElement("style");e.id="m5t-clients-styles",e.textContent=`
  ...
  `,document.head.appendChild(e),Aa=!0}const Vr=e=>String(e||"?").trim().split(/\s+/).slice(0,2).map(t=>t[0]||"").join("").toUpperCase()||"?";function ze(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function qr(e){const t=e.caption||e.guid,a=e.status||"ENABLED";return`
    <div class="client-card">
      <span class="client-card__avatar">${U(Vr(t))}</span>
      <span class="client-card__main">
        <div class="client-card__name" title="${U(t)}">${U(t)}</div>
        <div class="client-card__meta">Added ${U(zr(e.created))||"—"}</div>
    ...
    <div class="client-modal__body" id="invite-body">
      ...
      <div class="client-modal__actions">
        ...
        </div>`;const _=n.querySelector("#inv-link");n.querySelector("#inv-copy")?.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(_.value),n.querySelector("#inv-copy").textContent="Copied!"}catch{_.select()}}),n.querySelector('[data-act="done"]')?.addEventListener("click",o)}catch(b){console.warn("[clients] invite failed:",b),i.textContent=b.message||"Something went wrong. Please try again.",l.disabled=!1,l.innerHTML=v}})}const Kr={render:mn,init:gn},Jr=Object.freeze(Object.defineProperty({__proto__:null,default:Kr,init:gn,render:mn},Symbol.toStringTag,{value:"Module"})),Re=Object.freeze({owner:{value:"owner",rank:30,label:"Owner"},admin:{value:"admin",rank:20,label:"Admin"},member:{value:"member",rank:10,label:"Member"},viewer:{value:"viewer",rank:0,label:"Viewer"}}),Xr=Object.freeze({VIEW_DASHBOARD:"view_dashboard",VIEW_MAGNETS:"view_magnets",PROVISION_MAGNETS:"provision_magnets",MANAGE_MAGNETS:"manage_magnets",VIEW_CONVERSATIONS:"view_conversations",VIEW_STATS:"view_stats",VIEW_AD_METRICS:"view_ad_metrics",CONNECT_AD_ACCOUNTS:"connect_ad_accounts",CREATE_CLIENT_ACCOUNTS:"create_client_accounts",MANAGE_TEAM:"manage_team",VIEW_BILLING:"view_billing",MANAGE_BILLING:"manage_billing"}),A=Xr,Qr=Object.freeze(Object.values(A)),Ae=Object.freeze([A.VIEW_DASHBOARD,A.VIEW_MAGNETS,A.VIEW_CONVERSATIONS,A.VIEW_STATS,A.VIEW_AD_METRICS]);Object.freeze({reseller_org:{owner:Qr,admin:[A.VIEW_DASHBOARD,A.VIEW_MAGNETS,A.PROVISION_MAGNETS,A.MANAGE_MAGNETS,A.VIEW_CONVERSATIONS,A.VIEW_STATS,A.VIEW_AD_METRICS,A.CONNECT_AD_ACCOUNTS,A.CREATE_CLIENT_ACCOUNTS,A.MANAGE_TEAM,A.VIEW_BILLING],member:[A.VIEW_DASHBOARD,A.VIEW_MAGNETS,A.PROVISION_MAGNETS,A.MANAGE_MAGNETS,A.VIEW_CONVERSATIONS,A.VIEW_STATS,A.VIEW_AD_METRICS],viewer:Ae},managed_client:{owner:[...Ae,A.VIEW_BILLING],admin:[...Ae,A.VIEW_BILLING],member:Ae,viewer:Ae},saas_org:{owner:[A.VIEW_DASHBOARD,A.VIEW_MAGNETS,A.PROVISION_MAGNETS,A.MANAGE_MAGNETS,A.VIEW_CONVERSATIONS,A.VIEW_STATS,A.VIEW_AD_METRICS,A.CONNECT_AD_ACCOUNTS,A.MANAGE_TEAM,A.VIEW_BILLING,A.MANAGE_BILLING],admin:[A.VIEW_DASHBOARD,A.VIEW_MAGNETS,A.PROVISION_MAGNETS,A.MANAGE_MAGNETS,A.VIEW_CONVERSATIONS,A.VIEW_STATS,A.VIEW_AD_METRICS,A.CONNECT_AD_ACCOUNTS,A.MANAGE_TEAM,A.VIEW_BILLING],member:[A.VIEW_DASHBOARD,A.VIEW_MAGNETS,A.PROVISION_MAGNETS,A.MANAGE_MAGNETS,A.VIEW_CONVERSATIONS,A.VIEW_STATS],viewer:Ae}});function Zr(e,t){const a=Re[e]?.rank??-1,n=Re[t]?.rank??1/0;return a>=Re.admin.rank&&a>=n}const Ta="managed_client";let ka=!1;const ei={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"},W=e=>String(e??"").replace(/[&<>"']/g,t=>ei[t]),Ca=(e,t="info")=>{try{u.publish(m.UI_TOAST,{message:e,type:t})}catch{}},ti=e=>String(e||"?").trim().split(/\s+/).slice(0,2).map(t=>t[0]||"").join("").toUpperCase()||"?";function ai(){const e=T.getActiveOrg();let t=e&&e.channel!==Ta?e:null;if(!t){const n=T.getMemberships().find(o=>o.channel!==Ta);n&&(t={guid:n.orgGuid,channel:n.channel,memberRole:n.memberRole})}if(!t)return null;const a=T.getMemberships().find(n=>n.orgGuid===t.guid)?.name;return{...t,name:a}}function ni(e){return Object.values(Re).filter(t=>Zr(e,t.value)).sort((t,a)=>a.rank-t.rank)}function oi(){if(ka)return;const e=document.createElement("style");e.id="m5t-team-styles",e.textContent=`
  ...
  `,document.head.appendChild(e),ka=!0}function Qe(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function si(e){const t=e.member_role||"member";return`
    <div class="team-row">
      <span class="team-avatar">${W(ti(e.name||e.email))}</span>
