...
<body>
  ...
  `,document.head.appendChild(e),of=!0}function tf(e){return`<div class="pv-hintwrap">
    ...
    </div>`,document.body.appendChild(n),requestAnimationFrame(()=>n.classList.add(`open`)),nf(n);let r=n.querySelector(`.pv-frame`),i=n.querySelector(`.pv-stage`),a=()=>{try{r.srcdoc=$d(e())}catch{}};a();let o=()=>{n.classList.remove(`open`),setTimeout(()=>n.remove(),180)};return n.querySelector(`[data-x="close"]`).addEventListener(`click`,o),n.querySelector(`[data-x="refresh"]`).addEventListener(`click`,a),n.addEventListener(`click`,e=>{e.target===n&&o()}),n.querySelector(`.pv-seg`).addEventListener(`click`,e=>{let r=e.target.closest(`button[data-device]`);!r||r.dataset.device===t||(t=r.dataset.device,n.querySelectorAll(`.pv-seg button`).forEach(e=>e.classList.toggle(`is-on`,e.dataset.device===t)),i.dataset.device=t,a())}),{close:o,rebuild:a}}var of,sf=o((()=>{Zd(),of=!1}));function cf(e,t){let n=pf(e),r=pf(t),i=df.find(e=>pf(e.w)===n&&pf(e.h)===r);return i?i.id:`custom`}function lf(e){let t=df.find(t=>t.id===e);return t?{w:t.w,h:t.h}:null}function uf(e,t){let n=String(e||``).trim();return n?n.length>40?`Keep it under 40 characters.`:!mf.test(n)&&!hf.test(n)?`That isn't a size the widget understands — use a CSS length like 720px or 80vh${t===`width`?` or 100%`:``}, or min()/max()/clamp().`:t===`height`&&/%\s*$/.test(n)?`A percentage height won’t work here — the card would collapse to its content. Use px or vh instead (720px, 80vh).`:``:``}var df,ff,pf,mf,hf,gf=o((()=>{df=Object.freeze([{id:`compact`,name:`Compact`,hint:`Sidebars and narrow slots`,w:`480px`,h:`440px`},{id:`standard`,name:`Standard`,hint:`The default fit`,w:``,h:``},{id:`tall`,name:`Tall`,hint:`Longer flows, content columns`,w:`640px`,h:`min(720px, 82vh)`},{id:`hero`,name:`Hero`,hint:`The magnet is the page`,w:`100%`,h:`min(80vh, 860px)`}]),ff=Object.freeze({compact:{w:26,h:26},standard:{w:34,h:30},tall:{w:34,h:40},hero:{w:52,h:38}}),pf=e=>String(e||``).trim().replace(/\s+/g,` `).toLowerCase(),mf=/^\d+(\.\d+)?(px|rem|em|vh|vw|%)?$/,hf=/^(min|max|clamp)\(\s*[\d.\s,a-z%()+*/-]+\)$/i}));function _f(e){return{id:e.slug,name:e.name,description:e.description,icon:e.icon,category:e.category,tags:e.tags||[],featured:!!e.featured,appearance:e.appearance||{},steps:e.steps||[],appguid:e.appguid}}function vf(){return Nf||Mf}function yf(){return Nf?Promise.resolve(Nf):(Pf||=fetch(`${Gi.baseUrl}/m5t/v5/templates`).then(e=>e.ok?e.json():null).then(e=>(e?.ok&&Array.isArray(e.templates)&&e.templates.length&&(Nf=e.templates.map(_f)),vf())).catch(()=>(Pf=null,vf())),Pf)}function bf(e){let t=[];for(let n of e.steps||[])for(let e of n.step_columns||[])e&&!t.includes(e)&&t.push(e);return t}function xf(e){let t=(e.steps||[])[0]||{},n=(t.step_prompts||[])[0]||``,r=``;return t.step_type===`single_select`||t.step_type===`multi_select`?r=(t.step_options||[]).slice(0,3).map(e=>`<span class="mag-tpl-pv__btn">${Af(e?.label)}</span>`).join(``):Ff[t.step_type]?r=`<span class="mag-tpl-pv__input">${Af(Ff[t.step_type])}</span>`:t.step_type===`scheduler`&&(r=`<span class="mag-tpl-pv__cal"><span class="material-symbols-outlined" aria-hidden="true">calendar_month</span>Pick a time</span>`),`<div class="mag-tpl-card__preview" aria-hidden="true"><span class="mag-tpl-pv__bubble">${Af(n)}</span>${r}</div>`}function Sf(e,{detail:t=!1,gallery:n=!1,canUse:r=!0}={}){let i=t?bf(e):[],a=n?` mag-tpl-card--gallery`:``,o=n?` data-cat="${Af(e.category||``)}"`:``,s=n&&e.featured?`<span class="m5t-badge m5t-badge--featured m5t-featured-corner">Most popular</span>`:``,c=n?xf(e):``,l=n&&(e.tags||[]).length?`<div class="m5t-chip-row mag-tpl-card__tags">${e.tags.map(e=>`<span class="m5t-chip">${Af(e)}</span>`).join(``)}</div>`:``,u=r?`<button class="mag-tpl-card__btn" type="button" data-tpl="${e.id}">
    ...
    </div>`}function Cf(e){xa(wa.MAGNET_TEMPLATE,{id:e,at:Date.now()})}function wf(e){if(!e)return null;if(typeof e==`string`)return vf().find(t=>t.id===e)||null;let{id:t,at:n}=e;return typeof n==`number`&&Date.now()-n>=If?null:vf().find(e=>e.id===t)||null}function Tf(){return wf(Sa(wa.MAGNET_TEMPLATE))}function Ef(){return wf(Ca(wa.MAGNET_TEMPLATE))}function Df(){if(Lf)return;let e=document.createElement(`style`);e.id=`m5t-mag-template-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Lf=!0}function Of(e){yf(),Df();let t=document.createElement(`div`);t.className=`mag-modal-scrim mag-tpl-modal`,t.innerHTML=`
...
`))}</textarea>`}return`<div class="mag-cta" data-type="${e}" data-kind="${n.kind}">
    ...
    ${o}
  </div>`}function np(){if(om)return;let e=document.createElement(`style`);e.id=`m5t-magnets-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),om=!0}function rp(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function ip(e){let t=V.stats[e];if(!t||!t.opens&&!t.leads)return`<div class="mag-statline mag-statline--empty">&mdash;</div><div class="mag-cellsub">No activity yet</div>`;let n=e=>Number(e||0).toLocaleString();return`<div class="mag-statline" title="Trailing 30 days"><b>${n(t.opens)}</b> opens &middot; <b>${n(t.leads)}</b> leads</div>
    ${t.completion_rate==null?``:`<div class="mag-cellsub">${t.completion_rate}% complete the conversation</div>`}`}function ap(e){let t=V.install[e.guid];if(!t)return``;let n=t.unrecognized||[],r=(t.traffic?.opens||0)>0,i=e.can_manage===!0,a=n.length?`
    ...
    <div class="mag-conn__row" data-conn="${H(t.guid)}" data-state="${r.tone}">
      ...
      ${e.can_manage===!0?sp(t)?`<button class="mag-btn" data-action="clear-connection" data-guid="${H(e.guid)}" data-conn="${H(t.guid)}"
    ...
    </div>`}function lp(e){let t=V.pairing[e.guid];if(!t)return``;if(t.error)return`<div class="mag-conn__code" data-state="error">
  ...
  </div>`}function up(e){let t=V.connections[e.guid];if(!t)return``;if(t.failed)return`<div class="mag-conn">
    ...
    </div>`;let n=t.connections||[],r=n.filter(t=>t.status!==`PENDING`||e.can_manage===!0),i=n.filter(e=>e.status===`ACTIVE`).length,a=lp(e),o=r.filter(e=>!sp(e)),s=r.filter(e=>sp(e)),c=o.length?o.map(t=>cp(e,t)).join(``):`<p class="mag-install__hint">No sites connected yet. On WordPress, connecting a site pairs it without pasting the snippet by hand.</p>`,l=s.length?`
  ...
  </div>`}function dp(e){let t=document.querySelector(`.mag-card[data-guid="${CSS.escape(e.guid)}"] .mag-conn-slot`);t&&(t.innerHTML=up(e))}async function fp(e){let t=await(await fetch(`${W()}/m5t/v5/magnets/${encodeURIComponent(e.guid)}/connections`)).json().catch(()=>({}));if(!t.ok)throw Error(t.error||`Could not load connected sites`);return t}async function pp(){await Promise.all(V.magnets.map(async e=>{try{V.connections[e.guid]=await fp(e)}catch(t){console.warn(`[magnets] connections failed:`,e.guid,t),V.connections[e.guid]={failed:!0}}dp(e)}))}function mp(){V.pairTimer&&(clearInterval(V.pairTimer),V.pairTimer=null)}function hp(){mp()}function gp(e){return!!document.querySelector(`.mag-card[data-guid="${CSS.escape(e)}"] .mag-conn-slot`)}function _p(){if(V.pairTimer)return;let e=0;V.pairTimer=setInterval(async()=>{for(let e of Object.keys(V.pairing))gp(e)||delete V.pairing[e];let t=Object.keys(V.pairing);if(!t.length){mp();return}if(!t.some(e=>{let t=V.pairing[e];return!t.connected&&!t.error&&t.expiresAt-Date.now()>0})){t.forEach(e=>{let t=V.magnets.find(t=>t.guid===e);t&&dp(t)}),mp();return}e+=1e3;let n=e>=xm;n&&(e=0);for(let e of t){let t=V.magnets.find(t=>t.guid===e),r=V.pairing[e];if(!t||!r)continue;let i=r.expiresAt-Date.now()<=0;if(n&&!r.connected&&!r.error&&!i)try{let n=await fp(t);V.connections[e]=n;let i=(n.connections||[]).find(e=>e.guid===r.connectionGuid&&e.status===`ACTIVE`);i&&(V.pairing[e]={connected:!0,connectionGuid:r.connectionGuid,connectedHost:i.host},U(`${i.host||`Your site`} connected`,`success`))}catch{}dp(t)}},1e3)}async function vp(e){try{let t=await fetch(`${W()}/m5t/v5/magnets/${encodeURIComponent(e.guid)}/pairing-code`,{method:`POST`,headers:{"Content-Type":`application/json`},body:`{}`}),n=await t.json().catch(()=>({}));if(!t.ok||!n.ok){V.pairing[e.guid]={error:n.message||n.error||`Could not generate a pairing code`},dp(e);return}V.pairing[e.guid]={code:n.pairingCode,connectionGuid:n.connectionGuid,expiresAt:new Date(n.expiresAt).getTime()};try{V.connections[e.guid]=await fp(e)}catch{}dp(e),_p()}catch{V.pairing[e.guid]={error:`Could not reach the server. Try again.`},dp(e)}}async function yp(e){let t=V.pairing[e.guid]?.code;if(t)try{await navigator.clipboard.writeText(t),U(`Pairing code copied`,`success`)}catch{U(`Copy failed — select the code and copy it manually`,`danger`)}}async function bp(e,t,n){if(window.confirm(`Clear the record for ${n||`this connection`}?\n\nIt is removed from this list for good. This only tidies up the history — the site is already disconnected, so nothing about access changes.`))try{let n=await fetch(`${W()}/m5t/v5/magnets/${encodeURIComponent(e.guid)}/connections/${encodeURIComponent(t)}`,{method:`DELETE`}),r=await n.json().catch(()=>({}));if(!n.ok||!r.ok){U(r.message||r.error||`Could not clear that record`,`danger`);return}U(`Record cleared`,`success`);try{V.connections[e.guid]=await fp(e)}catch{}dp(e)}catch{U(`Could not reach the server`,`danger`)}}async function xp(e,t){let n=(V.connections[e.guid]?.connections||[]).find(e=>e.guid===t),r=n?.status===`PENDING`,i=r?`Cancel this pairing code?`:`Disconnect ${n?.host||`this site`}?`,a=r?`The code stops working immediately.`:`Its key stops working immediately and the plugin will show as disconnected. The domain stays on the allow-list — remove it separately if you want the magnet off that site.`;if(window.confirm(`${i}\n\n${a}`))try{let n=await fetch(`${W()}/m5t/v5/magnets/${encodeURIComponent(e.guid)}/connections/${encodeURIComponent(t)}/revoke`,{method:`POST`,headers:{"Content-Type":`application/json`},body:`{}`}),i=await n.json().catch(()=>({}));if(!n.ok||!i.ok){U(i.message||i.error||`Could not disconnect`,`danger`);return}V.pairing[e.guid]?.connectionGuid===t&&delete V.pairing[e.guid],U(r?`Pairing code cancelled`:`Site disconnected`,`success`);try{V.connections[e.guid]=await fp(e)}catch{}dp(e)}catch{U(`Could not reach the server`,`danger`)}}function Sp(e){if(e.status===`DRAFT`)return`draft`;if(e.status!==`ENABLED`)return`off`;let t=V.install[e.guid];return t?(t.traffic?.opens||0)>0?`live`:(t.unrecognized||[]).length?`seen`:`quiet`:`unknown`}function Cp(e){let t=Sp(e),n=V.install[e.guid],r=``,i=``;if(t===`draft`)r=`Draft`,i=`Not live yet`;else if(t===`off`)r=`Disabled`,i=`Hidden from visitors`;else if(t===`live`){let e=n.traffic?.last_open_at?new Date(n.traffic.last_open_at):null,t=e&&e.toDateString()===new Date().toDateString(),a=e?t?e.toLocaleTimeString([],{hour:`numeric`,minute:`2-digit`}):e.toLocaleDateString():``;r=`Live`,i=a?`Last visitor ${H(a)}`:`Serving visitors`}else t===`seen`?(r=`Needs review`,i=`Unrecognised domain`):t===`quiet`&&(r=`Not seen`,i=`No visitors in ${Number(n.window_days||30)} days`);return r?`<div class="mag-status" data-live="${t}"><span class="mag-dot" data-live="${t}" aria-hidden="true"></span>${r}</div>
          ${i?`<div class="mag-cellsub">${i}</div>`:``}`:``}function wp(e){let t=V.owners.length>1||C.getMemberships().length>1,n=(e.domains||[]).filter(Boolean),r=e.can_manage===!0,i=V.detailOpen[e.guid]===!0,a=[dm[e.display_mode]||`Chat bubble`,n.length?n.join(`, `):`No domains set. Add yours to go live.`,t&&e.owner_name?e.owner_name:``].filter(Boolean).join(` · `);return`
    <div class="mag-card${i?` is-open`:``}" data-guid="${H(e.guid)}">
      <div class="mag-line">
        ...
        <div class="mag-cell mag-cell--stats">${ip(e.guid)}</div>
      ...
      <div class="mag-detail" id="mag-detail-${H(e.guid)}" ${i?``:`hidden`}>
        ...
        <div class="mag-embed">
          <input type="text" readonly value="${H(cm(e.guid))}" aria-label="Embed snippet">
        ...
        <div class="mag-install-slot">${ap(e)}</div>
        <div class="mag-conn-slot">${up(e)}</div>
      </div>
    </div>`}function Tp(e,t){let n=typeof t==`boolean`?t:V.detailOpen[e]!==!0;V.detailOpen[e]=n;let r=document.querySelector(`.mag-card[data-guid="${CSS.escape(e)}"]`);if(!r)return;r.classList.toggle(`is-open`,n);let i=r.querySelector(`.mag-detail`);i&&(i.hidden=!n);let a=r.querySelector(`[data-action="install-toggle"][aria-controls]`);if(a){a.setAttribute(`aria-expanded`,String(n)),a.classList.toggle(`is-open`,n);let e=a.querySelector(`.material-symbols-outlined`);e&&(e.textContent=n?`expand_less`:`expand_more`)}}function Ep(){Cm||(Cm=!0,document.addEventListener(`click`,e=>{e.target.closest?.(`.mag-more-wrap`)||Dp()}),document.addEventListener(`keydown`,e=>{e.key===`Escape`&&Dp()}))}function Dp(e){document.querySelectorAll(`.mag-menu`).forEach(t=>{if(t===e)return;t.hidden=!0;let n=t.parentElement?.querySelector(`[data-action="menu"]`);n&&n.setAttribute(`aria-expanded`,`false`)})}function Op(e){let t=document.querySelector(`.mag-card[data-guid="${CSS.escape(e.guid)}"]`);if(!t)return;let n=t.querySelector(`.mag-install-slot`);n&&(n.innerHTML=ap(e));let r=Sp(e),i=t.querySelector(`.mag-cell--status`);i&&(i.innerHTML=Cp(e)),(r===`quiet`||r===`seen`)&&V.detailOpen[e.guid]===void 0&&Tp(e.guid,!0)}function kp(){return A()?ua({section:`magnets`,title:`Your Magnets`,description:`Pick a client to view and manage their magnets.`}):(np(),`
    <div class="page-shell" data-section="magnets">
      ...
      <div class="dashboard-card">
        <div class="card-body" id="mag-body">
          ${rp(`hourglass_empty`,`Loading magnets…`)}
        </div>
      </div>
    </div>`)}async function Ap(){if(A()){let e=Ef();if(e){let t=document.querySelector(`.page-shell .page-header-text .page-description`);t&&(t.textContent=`Template picked: ${e.name}. Choose the client to build it for.`)}let t=e=>Number(e||0).toLocaleString();return fa(W(),[{h:`Magnets`,td:e=>`${t(e.magnets.active)}<span style="color:var(--color-text-subtle,#6C7C9A)">/${t(e.magnets.total)}</span>`},{h:`Leads · wk`,td:e=>t(e.leads.this_week)},{h:`Leads · total`,td:e=>t(e.leads.total)}],{emptyMsg:`No client orgs yet. Create one under Clients, then provision its magnets here.`})}_m=null,vm=null,ym=null,bm=null,document.getElementById(`mag-new-btn`)?.addEventListener(`click`,Rp),document.getElementById(`mag-template-btn`)?.addEventListener(`click`,Bp),document.getElementById(`mag-body`)?.addEventListener(`click`,Vp),Ep(),await Mp();let e=Tf();e&&V.canProvision&&zp(e)}function jp(e){e&&(e.innerHTML=`<div class="mag-grid">
    <div class="mag-head" aria-hidden="true"><span>Magnet</span><span>Status</span><span>Last 30 days</span><span></span></div>
    ${V.magnets.map(wp).join(``)}</div>`,E(e.querySelector(`.mag-grid`),{rowSelector:`.mag-card`,noun:`magnets`}))}async function Mp(){let e=document.getElementById(`mag-body`);if(!e)return;try{let[e,t]=await Promise.all([fetch(`${W()}/m5t/v5/magnets`),fetch(`${W()}/m5t/v5/magnets/stats`).catch(()=>null)]),n=await e.json().catch(()=>({}));if(!n.ok)throw Error(n.message||`Failed to load`);V.magnets=n.magnets||[],V.owners=n.owners||[],V.canProvision=!!n.canProvision,V.canManage=!!n.canManage,V.primaryOrgGuid=n.primaryOrgGuid||null;let r=t?await t.json().catch(()=>({})):{};V.stats=r?.stats||{}}catch{e.innerHTML=rp(`error`,`Could not load your magnets. Please try again.`);return}let t=document.getElementById(`mag-new-btn`);t&&(t.style.display=V.canProvision?``:`none`);let n=document.getElementById(`mag-template-btn`);if(n&&(n.style.display=V.canProvision?``:`none`),!V.magnets.length){e.innerHTML=rp(`widgets`,V.canProvision?`No magnets yet. Click “New magnet” to create your first.`:`No magnets yet.`);return}jp(e),op(),pp()}async function Np(e){let t={"Content-Type":`application/json`,"Idempotency-Key":crypto.randomUUID()},n={status:`DRAFT`};e&&(n.owner_guid=e);let r=await fetch(`${W()}/m5t/v5/magnets`,{method:`POST`,headers:t,body:JSON.stringify(n)}),i={};try{i=await r.json()}catch{}if(!r.ok||!i.ok)throw Error(i.message||`Could not start a new magnet (${r.status})`);return i.magnet}async function Pp(e,t){let n=await fetch(`${W()}/m5t/v5/magnets/${encodeURIComponent(e)}`,{method:`PATCH`,headers:{"Content-Type":`application/json`},body:JSON.stringify({name:t.name,...t.appearance,steps:t.steps})}),r={};try{r=await n.json()}catch{}if(!n.ok||!r.ok)throw Error(r.message||`Could not apply the ${t.name} template (${n.status})`)}async function Fp(e,t){let n=document.getElementById(`mag-new-btn`),r=document.getElementById(`mag-template-btn`);n&&(n.disabled=!0),r&&(r.disabled=!0);let i=null;try{let n=await Np(e);i=n.guid,t?(await Pp(n.guid,t),await Mp(),n=V.magnets.find(e=>e.guid===n.guid)||n):(V.magnets=[n,...V.magnets],jp(document.getElementById(`mag-body`))),im(n,void 0,{freshDraft:!0})}catch(e){i&&fetch(`${W()}/m5t/v5/magnets/${encodeURIComponent(i)}`,{method:`DELETE`}).catch(()=>{}),U(e.message||`Could not start a new magnet`,`danger`)}finally{n&&(n.disabled=!1),r&&(r.disabled=!1)}}function Ip(e){let t=document.createElement(`div`);t.className=`mag-modal-scrim`,t.innerHTML=`
    <div class="mag-modal" role="dialog" aria-modal="true" aria-label="Choose client" style="max-width:420px">
    ...
    </div>`,document.body.appendChild(t),requestAnimationFrame(()=>{t.classList.add(`open`),t.querySelector(`.mag-modal`).classList.add(`open`)});let n=()=>{t.classList.remove(`open`),setTimeout(()=>t.remove(),180),document.removeEventListener(`keydown`,r)};function r(e){e.key===`Escape`&&(e.preventDefault(),n())}document.addEventListener(`keydown`,r),t.querySelector(`.mag-modal__close`).addEventListener(`click`,n),t.querySelector(`[data-x="cancel"]`).addEventListener(`click`,n),t.querySelector(`[data-x="continue"]`).addEventListener(`click`,()=>{let r=t.querySelector(`#mag-owner-pick`).value;n(),Fp(r,e)})}function Lp(){return V.owners.length===1?V.owners[0].guid:null}function Rp(){if(V.owners.length>1){Ip();return}Fp(Lp())}function zp(e){if(V.owners.length>1){Ip(e);return}Fp(Lp(),e)}function Bp(){Of(zp)}function Vp(e){let t=e.target.closest(`[data-action]`);if(!t){if(Dp(),!e.target.closest(`a`)){let t=e.target.closest(`.mag-line`)?.closest(`.mag-card`);t?.dataset.guid&&Tp(t.dataset.guid)}return}let n=t.dataset.guid,r=V.magnets.find(e=>e.guid===n),i=t.dataset.action;if(i===`menu`){let e=t.parentElement?.querySelector(`.mag-menu`);if(!e)return;let n=e.hidden;Dp(e),e.hidden=!n,t.setAttribute(`aria-expanded`,String(n));return}if(Dp(),i===`install-toggle`)return Tp(n);if(i===`copy`)return Hp(n);if(r){if(i===`preview`)return af(()=>Qd(r));if(i===`funnel`)return Wp(r);if(i===`edit`)return im(r);if(i===`flow`)return im(r,`flow`);if(i===`allow-domain`)return Gp(r,t.dataset.domain);if(i===`dismiss-domain`)return Kp(r,t.dataset.domain);if(i===`restore-domain`)return qp(r,t.dataset.domain);if(i===`pair-start`)return vp(r);if(i===`copy-code`)return yp(r);if(i===`revoke-connection`)return xp(r,t.dataset.conn);if(i===`clear-connection`)return bp(r,t.dataset.conn,t.dataset.label);if(i===`toggle`)return Yp(r);if(i===`delete`)return Xp(r)}}async function Hp(e){try{await navigator.clipboard.writeText(cm(e)),U(`Embed snippet copied`,`success`)}catch{U(`Copy failed`,`danger`)}}function Up(e){let t=Number(e?.total_steps)||0,n={};if((Array.isArray(e?.funnel)?e.funnel:[]).forEach(e=>{n[e.position]=Number(e.reached)||0}),!t||!Object.keys(n).length)return`<p class="mag-modal__hint" style="margin:0">No step activity yet in this window. As visitors answer questions, drop-off by step will appear here.</p>`;let r=Math.max(1,n[1]||0),i=[];for(let e=1;e<=t;e++){let a=n[e]||0,o=Math.min(100,Math.round(a/r*100));i.push(`
      ...
      </div>`)}return`<div class="mag-funnel">${i.join(``)}</div>`}async function Wp(e){let t=document.createElement(`div`);t.className=`mag-modal-scrim`,t.innerHTML=`
    ...
    <div class="mag-modal" role="dialog" aria-modal="true" aria-label="${i?`Edit magnet`:`New magnet`}">
      ...
      <div class="mag-modal__main">
        ...
        <select class="mag-modal__select" id="mag-f-trigger">
          ${mm.map(t=>`<option value="${t.value}"${Bf(e)===t.value?` selected`:``}>${H(t.label)}</option>`).join(``)}
        ...
        <div class="mag-perdev" id="mag-f-perdev"${Bf(e)===`__per_device`?``:` hidden`}>
          <div class="mag-perdev__row">
            ...
            <select class="mag-modal__select" id="mag-f-trigger-desktop">
              ${mm.map(t=>`<option value="${t.value}"${Vf(e,`desktop`)===t.value?` selected`:``}>${H(t.label)}</option>`).join(``)}
            </select>
          </div>
          <div class="mag-perdev__row">
            <label class="mag-modal__label" for="mag-f-trigger-mobile">Mobile</label>
            <select class="mag-modal__select" id="mag-f-trigger-mobile">
              ${mm.map(t=>`<option value="${t.value}"${Vf(e,`mobile`)===t.value?` selected`:``}>${H(t.label)}</option>`).join(``)}
            </select>
          </div>
        ...
        <div class="mag-modal__sec">
          ...
          <select class="mag-modal__select" id="mag-f-font">${rm(e?.font||``)}</select>
          ...
          <div id="mag-f-inline-size"${s===`inline`?``:` hidden`}>
            ...
            <div class="mag-size-grid" role="radiogroup" aria-label="Inline size">
              ${(()=>{let t=cf(e?.inline_width,e?.inline_height),n=(e,n,r,i,a)=>`
                  ...
                  </label>`;return df.map(e=>n(e.id,e.name,e.hint,ff[e.id].w,ff[e.id].h)).join(``)+n(`custom`,`Custom`,`Your own CSS sizes`,44,32)})()}
            </div>
            ...
            <div id="mag-f-inline-custom"${cf(e?.inline_width,e?.inline_height)===`custom`?``:` hidden`}>
              <div class="mag-modal__row">
                <div>
...
</details>`,um=e=>{let t=H(cm(e));return`<details class="mag-guide">
...
/&gt;</code></pre>
...
</details>`},dm={chat:`Chat bubble`,right_slide:`Slide-in panel`,modal:`Modal`,inline:`Inline`},fm=[{value:`inline`,icon:`view_agenda`,label:`Inline`},{value:`chat`,icon:`chat`,label:`Chat`},{value:`right_slide`,icon:`vertical_split`,label:`Slide`},{value:`modal`,icon:`web_asset`,label:`Modal`}],pm=[{value:`auto`,icon:`brightness_auto`,label:`Auto`},{value:`sync`,icon:`sync`,label:`Sync`},{value:`light`,icon:`light_mode`,label:`Light`},{value:`dark`,icon:`dark_mode`,label:`Dark`}],mm=[{value:`on_load`,label:`Immediately on page load`},{value:`on_8_seconds`,label:`After 8 seconds`},{value:`on_scroll_80_percent`,label:`At 80% scroll depth`},{value:`on_exit_intent`,label:`On exit intent (mouse leaves the page)`},{value:`none`,label:`Manual only (visitor clicks the launcher)`}],hm={book:{label:`Book`,icon:`event`,kind:`book`},chat:{label:`Chat`,icon:`chat`,kind:`flow`},call:{label:`Call`,icon:`phone`,kind:`value`,placeholder:`Phone number`},email:{label:`Email`,icon:`mail`,kind:`value`,placeholder:`Email address`},support:{label:`Support`,icon:`support_agent`,kind:`support`}},gm=[`book`,`chat`,`call`,`email`,`support`],_m=null,vm=null,ym=null,bm=null,xm=5e3,Sm={ACTIVE:{label:`Connected`,icon:`check_circle`,tone:`live`},PENDING:{label:`Waiting to connect`,icon:`hourglass_empty`,tone:`pending`},REVOKED:{label:`Disconnected`,icon:`link_off`,tone:`off`},EXPIRED:{label:`Expired`,icon:`timer_off`,tone:`off`}},Cm=!1,wm=512,Tm=new Set([`image/png`,`image/jpeg`,`image/gif`,`image/webp`]),Em=8388608,Dm=[{label:`Web fonts (loaded for you)`,opts:[[`Inter, sans-serif`,`Inter`],[`Poppins, sans-serif`,`Poppins`],[`Montserrat, sans-serif`,`Montserrat`],[`Roboto, sans-serif`,`Roboto`],[`"Open Sans", sans-serif`,`Open Sans`],[`Lato, sans-serif`,`Lato`],[`Nunito, sans-serif`,`Nunito`],[`Raleway, sans-serif`,`Raleway`],[`"Work Sans", sans-serif`,`Work Sans`],[`Oswald, sans-serif`,`Oswald`],[`Merriweather, serif`,`Merriweather (serif)`]]},{label:`System / always available`,opts:[[`system-ui, sans-serif`,`System UI`],[`Georgia, serif`,`Georgia (serif)`],[`"Times New Roman", Times, serif`,`Times (serif)`],[`Arial, Helvetica, sans-serif`,`Arial / Helvetica`],[`Verdana, Geneva, sans-serif`,`Verdana`],[`"Courier New", monospace`,`Courier (mono)`]]}]})),km=c({default:()=>Um,init:()=>Fm,render:()=>Nm});function Am(){let e=new Map;for(let t of vf())e.set(t.category,(e.get(t.category)||0)+1);return Object.keys(jf).filter(t=>e.has(t)).map(t=>[t,e.get(t)])}function jm(){if(Df(),Hm)return;let e=document.createElement(`style`);e.id=`m5t-templates-page-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Hm=!0}function Mm(){let e=Bm()||zm();return`
      <div class="m5t-chip-row tpl-filters" id="tpl-filters" role="group" aria-label="Filter templates by category">
        ${Vm(`all`,`All`,vf().length,`true`)}
        ${Am().map(([e,t])=>Vm(e,jf[e],t,`false`)).join(``)}
      </div>

      <div class="mag-tpl-grid mag-tpl-grid--page" id="tpl-grid">
        ${vf().map(t=>Sf(t,{detail:!0,gallery:!0,canUse:e})).join(``)}
      </div>`}function Nm(){jm();let e=Bm();return`
    <div class="page-shell" data-section="templates">
      ...
      ${e||zm()?``:`<div class="settings-note" id="tpl-readonly-note">
        <span class="material-symbols-outlined" aria-hidden="true">lock</span>
        Setup managed by your provider.
      </div>`}

      <div id="tpl-gallery">${Mm()}</div>
    </div>`}function Pm(){let e=document.getElementById(`tpl-grid`);document.getElementById(`tpl-filters`)?.addEventListener(`click`,t=>{let n=t.target.closest(`[data-cat]`);if(!n)return;n.parentElement.querySelectorAll(`[data-cat]`).forEach(e=>{e.setAttribute(`aria-pressed`,String(e===n))});let r=n.dataset.cat;e?.querySelectorAll(`.mag-tpl-card`).forEach(e=>{e.hidden=r!==`all`&&e.dataset.cat!==r})}),e?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-preview]`);if(t){let e=vf().find(e=>e.id===t.dataset.preview);e&&af(()=>Qd({...e.appearance,steps:e.steps,name:e.name}));return}let n=e.target.closest(`[data-tpl]`);n&&(Cf(n.dataset.tpl),window.location.hash=Bm()?`#/signup`:`#/magnets`)})}function Fm(){Pm();let e=vf();yf().then(t=>{let n=document.getElementById(`tpl-gallery`);!n||t===e||(n.innerHTML=Mm(),Pm())})}var Im,Lm,Rm,zm,Bm,Vm,Hm,Um,Wm=o((()=>{w(),sf(),Rf(),Im={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Lm=e=>String(e??``).replace(/[&<>"']/g,e=>Im[e]),Rm=[`provision_magnets`,`manage_magnets`],zm=()=>C.getCapabilities().some(e=>Rm.includes(e)),Bm=()=>!C.isAuthenticated(),Vm=(e,t,n,r)=>`<button class="m5t-chip--filter" type="button" data-cat="${Lm(e)}" aria-pressed="${r}">${Lm(t)}<span class="m5t-chip__count">${n}</span></button>`,Hm=!1,Um={render:Nm,init:Fm}})),Gm=c({default:()=>gh,init:()=>Zm,render:()=>Xm});function Km(){if(dr(),ph)return;let e=document.createElement(`style`);e.id=`m5t-questionnaires-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),ph=!0}function qm(){return fh.clients.map(e=>{let t=fh.responses.filter(t=>t.owner_guid===e.guid),n=t[0]||null;return{guid:e.guid,name:e.caption||e.name||e.guid,latest:n,earlier:Math.max(0,t.length-1),status:n?n.status:`NOT_SENT`}})}function Jm(){let e=fh.search.trim().toLowerCase();return qm().filter(t=>(fh.filter===`ALL`||t.status===fh.filter)&&(!e||t.name.toLowerCase().includes(e)))}function Ym(e){let t=e.status===`SUBMITTED`,n=e.status===`SUBMITTED`?`<button class="qz-act" type="button" data-act="open" data-guid="${oh(e.latest.guid)}">Read answers</button>`:e.status===`DRAFT`?`<button class="qz-act" type="button" data-act="copy" data-url="${oh(e.latest.url||``)}">Copy link</button>`:`<a class="qz-act" href="#/clients">Go to client</a>`;return`<tr class="qz-row${t?` qz-row--open`:``}"${t?` data-open-guid="${oh(e.latest.guid)}"`:``}>
    <td><span class="qz-client">${oh(e.name)}</span>${e.earlier?`<span class="qz-earlier">${e.earlier} earlier</span>`:``}</td>
    <td><span class="qz-badge" data-s="${oh(e.status)}">${oh(dh[e.status]||e.status)}</span></td>
    <td class="qz-when">${oh(hh(e,`created_tz`))}</td>
    <td class="qz-when">${oh(hh(e,`submitted_tz`))}</td>
    <td class="qz-actcell">${n}</td>
  </tr>`}function Xm(){return Km(),`
    <div class="page-shell" data-section="questionnaires">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="qz-body">${mh(`hourglass_empty`,`Loading…`)}</div>
      </div>
    </div>`}async function Zm(){await Qm()}async function Qm(){let e=document.getElementById(`qz-body`);if(e){try{let[e,t]=await Promise.all([fetch(`${sh()}/m5t/v5/acctGroup`),fetch(`${sh()}/m5t/v5/m5mQuestionnaireResponse`)]),n=await e.json().catch(()=>({})),r=await t.json().catch(()=>({}));if(!e.ok||!t.ok)throw Error(`load failed`);fh.clients=(n.data||[]).filter(e=>e.class===lh&&![`DELETED`,`ARCHIVED`].includes(e.status)).sort((e,t)=>String(e.caption||``).localeCompare(String(t.caption||``))),fh.responses=r.data||[]}catch{e.innerHTML=mh(`error`,`Could not load questionnaires. Please try again.`);return}eh()}}function $m(){let e=document.getElementById(`qz-tbody`);e&&E(e,{rowSelector:`tr.qz-row`,noun:`clients`,controlHost:e.closest(`.qz-tablewrap`)||e.parentElement})}function eh(){let e=document.getElementById(`qz-body`);if(!e)return;if(!fh.clients.length){e.innerHTML=mh(`groups`,`No clients yet. Add a client first, then send them a kickoff questionnaire.`);return}let t=qm(),n={ALL:t.length};uh.slice(1).forEach(([e])=>{n[e]=t.filter(t=>t.status===e).length});let r=Jm();e.innerHTML=`
    <div class="qz-bar">
      <div class="qz-filter">
        ${uh.map(([e,t])=>`<button data-filter="${e}" class="${fh.filter===e?`active`:``}">${oh(t)}${n[e]?`<span class="qz-pillnum">${n[e]}</span>`:``}</button>`).join(``)}
      </div>
      <span class="qz-searchwrap">
        <span class="material-symbols-outlined" aria-hidden="true">search</span>
        <input class="qz-search" id="qz-search" type="search" placeholder="Search clients" value="${oh(fh.search)}" aria-label="Search clients" />
      </span>
    ...
    <div class="qz-tablewrap">
      <table class="qz-table">
        ...
        <tbody id="qz-tbody">${r.length?r.map(Ym).join(``):`<tr><td colspan="5"><div class="qz-when" style="padding:14px 4px">${fh.search.trim()?`No clients match your search.`:`No clients with status “${oh(dh[fh.filter]||fh.filter)}”.`}</div></td></tr>`}</tbody>
      </table>
    </div>`,$m(),e.querySelector(`.qz-filter`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-filter]`);t&&(fh.filter=t.dataset.filter,eh())}),e.querySelector(`#qz-search`)?.addEventListener(`input`,e=>{fh.search=e.target.value,th()}),e.querySelector(`#qz-tbody`)?.addEventListener(`click`,nh)}function th(){let e=document.getElementById(`qz-tbody`);if(!e)return;let t=Jm();e.innerHTML=t.length?t.map(Ym).join(``):`<tr><td colspan="5"><div class="qz-when" style="padding:14px 4px">No clients match your search.</div></td></tr>`,$m();let n=document.querySelector(`.qz-count`);n&&(n.textContent=`${t.length} of ${qm().length}`)}function nh(e){let t=e.target.closest(`[data-act="copy"]`);if(t){e.stopPropagation();let n=t.dataset.url;if(!n){ch(`No link on this questionnaire yet.`,`warning`);return}navigator.clipboard?.writeText(n).then(()=>{t.textContent=`Copied!`,setTimeout(()=>{t.textContent=`Copy link`},1500)},()=>ch(`Could not copy the link.`,`error`));return}let n=e.target.closest(`[data-act="open"]`),r=e.target.closest(`[data-open-guid]`),i=n?.dataset.guid||r?.dataset.openGuid;i&&rh(i)}async function rh(e){fh.openGuid=e;let t=document.createElement(`div`);t.className=`qz-drawer-scrim`;let n=document.createElement(`aside`);n.className=`qz-drawer`,n.setAttribute(`role`,`dialog`),n.setAttribute(`aria-label`,`Questionnaire answers`),n.innerHTML=`
    ...
    <div class="qz-drawer__body" id="qz-dbody">${mh(`hourglass_empty`,`Loading answers…`)}</div>`,document.body.append(t,n),requestAnimationFrame(()=>{t.classList.add(`open`),n.classList.add(`open`)});let r=()=>{fh.openGuid=null,t.classList.remove(`open`),n.classList.remove(`open`),setTimeout(()=>{t.remove(),n.remove()},220),document.removeEventListener(`keydown`,i)};function i(e){e.key===`Escape`&&(e.preventDefault(),r())}document.addEventListener(`keydown`,i),t.addEventListener(`click`,r),n.querySelector(`.qz-drawer__close`).addEventListener(`click`,r);try{let t=await fetch(`${sh()}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e)}`),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`load failed`);let i=r.data||{},a=fh.clients.find(e=>e.guid===i.owner_guid)?.caption||`Client`;n.querySelector(`#qz-dtitle`).textContent=a;let o=i.submitted_tz?`Submitted ${D(i.submitted_tz)}`:`Not yet submitted`;n.querySelector(`#qz-dsub`).textContent=i.locked_tz?`${o} · Finalized`:o,n.querySelector(`#qz-dbody`).innerHTML=ur(i)+sr(i,{emptyLabel:`Not answered`})}catch{n.querySelector(`#qz-dbody`).innerHTML=mh(`error`,`Could not load these answers. Please try again.`)}}var ih,ah,oh,sh,ch,lh,uh,dh,fh,ph,mh,hh,gh,_h=o((()=>{w(),ih=u(f(),1),m(),hr(),sn(),ah={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},oh=e=>String(e??``).replace(/[&<>"']/g,e=>ah[e]),sh=()=>C.getConfig?.().BASE_URL||``,ch=(e,t=`info`)=>{try{ih.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},lh=`managed_client`,uh=[[`ALL`,`All`],[`NOT_SENT`,`Not sent`],[`DRAFT`,`Awaiting response`],[`SUBMITTED`,`Submitted`],[`CANCELLED`,`Cancelled`]],dh=Object.fromEntries(uh),fh={clients:[],responses:[],filter:`ALL`,search:``,openGuid:null},ph=!1,mh=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,hh=(e,t)=>e?.latest?.[t]?D(e.latest[t]):`—`,gh={render:Xm,init:Zm}})),vh=c({default:()=>$h,init:()=>wh,render:()=>Ch});function yh(){if(dr(),Yh)return;let e=document.createElement(`style`);e.id=`m5t-deliverables-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Yh=!0}function bh(){let e=K.search.trim().toLowerCase();return K.rows.filter(t=>(K.filter===`ALL`||t.status===K.filter)&&(!e||String(t.owner_name||``).toLowerCase().includes(e)))}function xh(e){return e.status===`DRAFT`?`<button class="dz-act" type="button" data-act="open" data-guid="${G(e.guid)}">Edit</button>`:e.status===`SENT`?`<button class="dz-act" type="button" data-act="copy" data-url="${G(e.url||``)}">Copy link</button>`:e.status===`SUBMITTED`?`<button class="dz-act" type="button" data-act="open" data-guid="${G(e.guid)}">Read review</button>`:`<span class="dz-when">—</span>`}function Sh(e){let t=e.status!==`CANCELLED`;return`<tr class="dz-row${t?` dz-row--open`:``}"${t?` data-open-guid="${G(e.guid)}"`:``}>
    ...
    <td><span class="dz-badge" data-s="${G(e.status)}">${G(Jh[e.status]||e.status)}</span></td>
    <td class="dz-when">${e.sent_tz?G(D(e.sent_tz)):`—`}</td>
    <td class="dz-when">${e.submitted_tz?G(D(e.submitted_tz)):`—`}</td>
    <td class="dz-actcell">${xh(e)}</td>
  </tr>`}function Ch(){return yh(),`
    <div class="page-shell" data-section="deliverables">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="dz-body">${Xh(`hourglass_empty`,`Loading…`)}</div>
      </div>
    </div>`}async function wh(){document.getElementById(`dz-new`)?.addEventListener(`click`,jh),await Th()}async function Th(){let e=document.getElementById(`dz-body`);if(e){try{let[e,t,n]=await Promise.all([fetch(`${Wh()}/m5t/v5/m5mDeliverable`),fetch(`${Wh()}/m5t/v5/acctGroup`),fetch(`${Wh()}/m5t/v5/m5mDeliverable/templates`)]),r=await e.json().catch(()=>({})),i=await t.json().catch(()=>({})),a=await n.json().catch(()=>({}));if(!e.ok||!t.ok||!n.ok)throw Error(`load failed`);K.rows=r.data||[],K.clients=(i.data||[]).filter(e=>e.class===Kh&&![`DELETED`,`ARCHIVED`].includes(e.status)).sort((e,t)=>String(e.caption||``).localeCompare(String(t.caption||``))),K.templates=a.data||[]}catch{e.innerHTML=Xh(`error`,`Could not load deliverables. Please try again.`);return}Dh()}}function Eh(){let e=document.getElementById(`dz-tbody`);e&&E(e,{rowSelector:`tr.dz-row`,noun:`deliverables`,controlHost:e.closest(`.dz-tablewrap`)||e.parentElement})}function Dh(){let e=document.getElementById(`dz-body`);if(!e)return;if(!K.clients.length&&!K.rows.length){e.innerHTML=Xh(`groups`,`No clients yet. Add a client first, then author their first deliverable.`);return}if(!K.rows.length){e.innerHTML=Xh(`approval`,`No deliverables yet. Author the first one with New deliverable.`);return}let t={ALL:K.rows.length};qh.slice(1).forEach(([e])=>{t[e]=K.rows.filter(t=>t.status===e).length});let n=bh();e.innerHTML=`
    <div class="dz-bar">
      <div class="dz-filter">
        ${qh.map(([e,n])=>`<button data-filter="${e}" class="${K.filter===e?`active`:``}">${G(n)}${t[e]?`<span class="dz-pillnum">${t[e]}</span>`:``}</button>`).join(``)}
      </div>
    ...
    <div class="dz-tablewrap">
      <table class="dz-table">
        ...
        <tbody id="dz-tbody">${n.length?n.map(Sh).join(``):`<tr><td colspan="6"><div class="dz-when" style="padding:14px 4px">${K.search.trim()?`No deliverables match your search.`:`No deliverables with status “${G(Jh[K.filter]||K.filter)}”.`}</div></td></tr>`}</tbody>
      </table>
    </div>`,Eh(),e.querySelector(`.dz-filter`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-filter]`);t&&(K.filter=t.dataset.filter,Dh())}),e.querySelector(`#dz-search`)?.addEventListener(`input`,e=>{K.search=e.target.value,Oh()}),e.querySelector(`#dz-tbody`)?.addEventListener(`click`,kh)}function Oh(){let e=document.getElementById(`dz-tbody`);if(!e)return;let t=bh();e.innerHTML=t.length?t.map(Sh).join(``):`<tr><td colspan="6"><div class="dz-when" style="padding:14px 4px">No deliverables match your search.</div></td></tr>`,Eh();let n=document.querySelector(`.dz-count`);n&&(n.textContent=`${t.length} of ${K.rows.length}`)}function kh(e){let t=e.target.closest(`[data-act="copy"]`);if(t){e.stopPropagation();let n=t.dataset.url;if(!n){Gh(`No link on this deliverable yet.`,`warning`);return}navigator.clipboard?.writeText(n).then(()=>{t.textContent=`Copied!`,setTimeout(()=>{t.textContent=`Copy link`},1500)},()=>Gh(`Could not copy the link.`,`error`));return}let n=e.target.closest(`[data-act="open"]`),r=e.target.closest(`[data-open-guid]`),i=n?.dataset.guid||r?.dataset.openGuid;i&&Vh(i)}function Ah(e){let t=document.createElement(`div`);t.className=`dz-drawer-scrim`;let n=document.createElement(`aside`);n.className=`dz-drawer`,n.setAttribute(`role`,`dialog`),n.setAttribute(`aria-label`,e),n.innerHTML=`
    ...
    <div class="qm-qa" data-question-id="${G(e.id)}" data-question-type="${G(e.type)}">
      ...
      </div>`}function Ph(e,t){let n=(e.questions||[]).filter(e=>e.audience===`am`),r=n.filter(e=>!e.group).map(e=>Mh(e,t)).join(``),i=Nr(e);if(i){let n=Pr(e).filter(e=>e.audience===`am`);if(!n.length)return r;let a=Lr(e,t);for(let e=1;e<=a;e++){let a=Fr(n,i,e);r+=Nh(a,Qh(a,`${i}${e}`),t)}let o=Qh(Fr(n,i,a+1),`${i}${a+1}`);return r+=`<button class="dz-btn--ghost dz-addblock" type="button" data-repeat="${G(i)}" data-section-id="${G(e.id)}">+ Add ${G(o)}</button>`,r}let a=[...new Set(n.filter(e=>e.group).map(e=>e.group))];if(!a.length)return r;let o=null;return r+=a.map((e,r)=>{let i=n.filter(t=>t.group===e),a=Qh(i,e),s=r===0||Zh(i,t);return!s&&o===null&&(o={key:e,heading:a}),Nh(i,a,t,{hidden:!s})}).join(``),r+=`<button class="dz-btn--ghost dz-addblock" type="button"${o?``:` hidden`}>${o?`+ Add ${G(o.heading)}`:``}</button>`,r}function Fh(e,t){let n=(e?.sections||[]).filter(e=>(e.questions||[]).some(e=>e.audience===`am`)),r=n.length>1;return`
    ${r?`
    ...
    ${r?`
      ...
      </div>`:``}).join(``)}function Rh(e,t){if(!t)return``;let n=(e?.sections||[]).flatMap(e=>(e.questions||[]).filter(e=>e.audience!==`am`)).map(e=>({q:e,val:or(e,t.answers||{})})).filter(({val:e})=>e);return`
    ...
    </div>`}function zh(e,t){let n=Array.isArray(t)&&t.length?t[t.length-1]:null;return n?Rh(e,n):``}function Bh(e,t){return(Array.isArray(t)?t.slice().reverse():[]).map(t=>Rh(e,t)).join(``)}async function Vh(e){let{drawer:t,close:n}=Ah(`Deliverable`),r=t.querySelector(`#dz-dbody`),i=t.querySelector(`#dz-dfoot`),a;try{let t=await fetch(`${Wh()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}`),n=await t.json().catch(()=>({}));if(!t.ok||!n.ok)throw Error(n.error||`load failed`);a=n.data||{}}catch{r.innerHTML=Xh(`error`,`Could not load this deliverable. Please try again.`);return}let o=K.rows.find(t=>t.guid===e)?.owner_name||K.clients.find(e=>e.guid===a.owner_guid)?.caption||`Client`;t.querySelector(`#dz-dtitle`).textContent=`${o} · ${a.template?.name||`Deliverable`}`;let s=a.status===`DRAFT`||a.status===`SENT`,c=Number(a.revision)>1?` · Rev ${a.revision}`:``,l=a.status===`SUBMITTED`?`Returned ${a.submitted_tz?D(a.submitted_tz):``}${a.locked_tz?` · Finalized`:``}${c}`:a.status===`SENT`?`Sent ${a.sent_tz?D(a.sent_tz):``} to ${a.recipient_email||``} · awaiting approval${c}`:`${Jh[a.status]||a.status}${c}`;if(t.querySelector(`#dz-dsub`).textContent=l,!s){r.innerHTML=ur(a)+Lh(a.template,a.content||{},a.answers||{})+Bh(a.template,a.rounds),a.status===`SUBMITTED`&&!a.locked_tz?(i.innerHTML=`<button class="dz-btn" type="button" id="dz-revise">Revise plan</button>
        <span class="dz-note" style="margin:0">Archives this review and reopens authoring. The client approves the revised plan on a fresh link.</span>`,i.querySelector(`#dz-revise`).addEventListener(`click`,async t=>{let r=t.target;r.disabled=!0;try{let t=await fetch(`${Wh()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}/revise`,{method:`POST`}),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`revise failed`);Gh(`Reopened for revision. The last review is pinned above the form.`,`success`),n(),await Th(),Vh(e)}catch(e){Gh(e.message||`Could not revise.`,`error`),r.disabled=!1}})):i.innerHTML=``;return}r.innerHTML=`
    ${zh(a.template,a.rounds)}
    ${a.url?`
    ...
    <div id="dz-author-form">${Fh(a.template,a.content||{})}</div>`,i.innerHTML=`
    ...
    <button class="dz-btn--ghost dz-btn--danger" type="button" id="dz-cancel">Cancel deliverable</button>`,r.querySelector(`#dz-copylink`)?.addEventListener(`click`,e=>{let t=r.querySelector(`.dz-linkrow input`);navigator.clipboard?.writeText(t?.value||``).then(()=>{e.target.textContent=`Copied!`,setTimeout(()=>{e.target.textContent=`Copy link`},1500)},()=>Gh(`Could not copy the link.`,`error`))});let u=async()=>{let t=lr(r.querySelector(`#dz-author-form`)),i=await fetch(`${Wh()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}`,{method:`PUT`,headers:{"Content-Type":`application/json`},body:JSON.stringify({content:t})}),a=await i.json().catch(()=>({}));if(!i.ok||!a.ok){if(i.status===409)return Gh(a.error||`This deliverable is no longer editable.`,`warning`),n(),await Th(),!1;throw Error(a.error||`save failed`)}return!0};Ih(r.querySelector(`#dz-author-form`),a.template,()=>{u().catch(()=>Gh(`Autosave failed. Use Save draft.`,`warning`))}),i.querySelector(`#dz-preview`).addEventListener(`click`,async e=>{let t=e.target.closest(`button`);if(!a.url){Gh(`No link available for this deliverable.`,`warning`);return}t.disabled=!0;try{await u()&&window.open(a.url,`_blank`)}catch(e){Gh(e.message||`Could not save before previewing.`,`error`)}t.disabled=!1}),i.querySelector(`#dz-save`).addEventListener(`click`,async e=>{let t=e.target;t.disabled=!0;try{await u()&&Gh(`Draft saved.`,`success`)}catch(e){Gh(e.message||`Could not save.`,`error`)}t.disabled=!1}),i.querySelector(`#dz-send`).addEventListener(`click`,async t=>{let r=t.target,a=(i.querySelector(`#dz-email`)?.value||``).trim();if(!a){Gh(`Enter the recipient email first.`,`warning`);return}r.disabled=!0;try{if(!await u())return;let t=await fetch(`${Wh()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}/send`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({recipient_email:a})}),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`send failed`);Gh(r.data?.emailQueued?`Sent. ${a} will receive the review link.`:`Link created, but the email could not be queued. Copy the link and send it yourself.`,r.data?.emailQueued?`success`:`warning`),n(),await Th()}catch(e){Gh(e.message||`Could not send.`,`error`),r.disabled=!1}}),i.querySelector(`#dz-cancel`).addEventListener(`click`,async t=>{let r=t.target;r.disabled=!0;try{let t=await fetch(`${Wh()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}/cancel`,{method:`POST`}),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`cancel failed`);Gh(`Deliverable cancelled. Its link no longer works.`,`info`),n(),await Th()}catch(e){Gh(e.message||`Could not cancel.`,`error`),r.disabled=!1}})}var Hh,Uh,G,Wh,Gh,Kh,qh,Jh,K,Yh,Xh,Zh,Qh,$h,eg=o((()=>{w(),Hh=u(f(),1),m(),hr(),Vr(),sn(),Uh={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},G=e=>String(e??``).replace(/[&<>"']/g,e=>Uh[e]),Wh=()=>C.getConfig?.().BASE_URL||``,Gh=(e,t=`info`)=>{try{Hh.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},Kh=`managed_client`,qh=[[`ALL`,`All`],[`DRAFT`,`In progress`],[`SENT`,`Awaiting approval`],[`SUBMITTED`,`Returned`],[`CANCELLED`,`Cancelled`]],Jh=Object.fromEntries(qh),K={rows:[],clients:[],templates:[],filter:`ALL`,search:``},Yh=!1,Xh=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,Zh=(e,t)=>e.some(e=>{let n=t?.[e.id];return Array.isArray(n)?n.length>0:!!(n&&String(n).trim())}),Qh=(e,t)=>{let n=String(e[0]?.label||``).match(/^(.+?):\s/);return n?n[1]:t.toUpperCase()},$h={render:Ch,init:wh}})),tg=c({default:()=>pg,init:()=>sg,render:()=>og});function ng(e){return e.status===`SENT`?{key:`PENDING`,label:`Pending your review`}:e.status===`SUBMITTED`&&e.signed?{key:`APPROVED`,label:`Approved`}:e.status===`SUBMITTED`?{key:`RETURNED`,label:`Returned with edits`}:null}function rg(){if(dr(),dg)return;let e=document.createElement(`style`);e.id=`m5t-deliverables-client-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),dg=!0}function ig(e,t){return!e.url&&t.key===`PENDING`?``:t.key===`PENDING`?`<button class="dc-act" type="button" data-url="${lg(e.url)}">Review</button>`:t.key!==`APPROVED`&&e.url&&e.editability?.editable?`<button class="dc-act" type="button" data-url="${lg(e.url)}">Update your review</button>`:``}function ag(e){let t=ng(e);return`<tr>
    <td><span class="dc-name">${lg(e.template_name||e.kind||`Deliverable`)}</span>${Number(e.revision)>1?`<span class="dc-when" style="margin-left:6px">Rev ${lg(e.revision)}</span>`:``}</td>
    <td><span class="dc-badge" data-s="${lg(t.key)}">${lg(t.label)}</span></td>
    <td class="dc-when">${e.sent_tz?lg(D(e.sent_tz)):`—`}</td>
    <td class="dc-when">${e.submitted_tz?lg(D(e.submitted_tz)):`—`}</td>
    <td class="dc-actcell">${ig(e,t)}</td>
  </tr>`}function og(){return rg(),`
    <div class="page-shell" data-section="deliverables-client">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="dc-body">${fg(`hourglass_empty`,`Loading…`)}</div>
      </div>
    </div>`}async function sg(){let e=document.getElementById(`dc-body`);if(!e)return;let t;try{let e=await fetch(`${ug()}/m5t/v5/m5mDeliverable`),n=await e.json().catch(()=>({}));if(!e.ok||!n.ok)throw Error(`load failed`);t=(n.data||[]).filter(e=>ng(e))}catch{e.innerHTML=fg(`error`,`Could not load your deliverables. Please try again.`);return}if(!t.length){e.innerHTML=fg(`approval`,`Nothing to review right now.`);return}e.innerHTML=`
    <div class="dc-tablewrap">
      <table class="dc-table">
        <thead><tr><th>Deliverable</th><th>Status</th><th>Received</th><th>Returned</th><th></th></tr></thead>
        <tbody id="dc-tbody">${t.map(ag).join(``)}</tbody>
      </table>
    </div>`,e.querySelector(`#dc-tbody`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-url]`);t?.dataset.url&&window.open(t.dataset.url,`_blank`)})}var cg,lg,ug,dg,fg,pg,mg=o((()=>{w(),hr(),cg={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},lg=e=>String(e??``).replace(/[&<>"']/g,e=>cg[e]),ug=()=>C.getConfig?.().BASE_URL||``,dg=!1,fg=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,pg={render:og,init:sg}})),hg=c({init:()=>Ag,render:()=>kg});function gg(e){let t=Qg(e);if(!t)return``;try{return new Date(t).toLocaleDateString([],{month:`short`,day:`numeric`,year:`numeric`})}catch{return``}}function _g(e){let t=Qg(e);if(!t)return``;try{let n=Date.now()-new Date(t).getTime();if(n<0)return gg(e);let r=n/36e5;return r<1?`${Math.max(1,Math.floor(n/6e4))}m ago`:r<24?`${Math.floor(r)}h ago`:r<48?`Yesterday`:gg(e)}catch{return gg(e)}}function vg(){if(Wg)return;let e=document.createElement(`style`);e.id=`m5t-leads-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Wg=!0}function yg(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function bg(e){return`<select class="leads-status" data-id="${J(e.id)}" data-act="status" onclick="event.stopPropagation()">
    ${Kg.map(t=>`<option value="${t}" ${e.status===t?`selected`:``}>${t[0]}${t.slice(1).toLowerCase()}</option>`).join(``)}
  </select>`}function xg(e){let t=q.magnetNames[e.magnet_guid]||e.lead_source||`—`,n=e.phone?String(e.phone).replace(/[^\d+]/g,``):``;return`
    <tr class="leads-row${String(e.id)===q.selectedId?` selected`:``}" data-id="${J(e.id)}" tabindex="0" role="button" aria-label="Open lead ${J(Xg(e))}">
      <td>
        <span class="leads-who">
          <span class="leads-avatar">${J(Zg(e))}</span>
          <span><span class="leads-name">${J(Xg(e))}</span>${e.company?`<span class="leads-sub">${J(e.company)}</span>`:``}</span>
        </span>
      </td>
      ...
      <td>${q.canManage?bg(e):`<span class="leads-badge" data-s="${J(e.status)}">${J(e.status)}</span>`}</td>
      <td class="num leads-muted" style="white-space:nowrap" title="${J(gg(e))}">${$g(e)?`<span class="leads-fresh" title="Captured in the last 24 hours"></span>`:``}${J(_g(e))}</td>
      <td class="leads-open" aria-hidden="true"><span class="material-symbols-outlined">chevron_right</span></td>
    </tr>`}function Sg(e){let t=q.search.trim().toLowerCase();if(!t)return!0;let n=[e.first_name,e.last_name,e.email,e.company,e.phone,e.product_interest,q.magnetNames[e.magnet_guid],q.book?q.ownerNames[e.owner_guid]:``,e.utm_campaign,e.utm_source].filter(Boolean).join(` `).toLowerCase();return t.split(/\s+/).every(e=>n.includes(e))}function Cg(e){let t=r_[q.range];if(!t)return!0;let n=Qg(e);if(!n)return!1;let r=new Date(Date.now()-(t-1)*864e5).toISOString().slice(0,10);return n.slice(0,10)>=r}function wg(){return q.leads.filter(e=>(q.filter===`ALL`||e.status===q.filter)&&e_(e)&&n_(e)&&Sg(e)&&t_(e)&&Cg(e))}function Tg(){let e=wg(),t=a_[q.sort.key];if(!t)return e;let n=q.sort.dir===`asc`?1:-1;return e.slice().sort((e,r)=>{let i=t.value(e),a=t.value(r);return i===``&&a===``?0:i===``?1:a===``?-1:i===a?0:i<a?-n:n})}function Eg(e,t=``){let n=a_[e],r=q.sort.key===e,i=r?q.sort.dir:``;return`<th class="leads-th--sort${t?` ${t}`:``}${r?` is-sorted`:``}"
    ...
    </th>`}function Dg(e){return a_[e]?(q.sort.key===e?q.sort.dir=q.sort.dir===`asc`?`desc`:`asc`:q.sort={key:e,dir:a_[e].dir},q.shown=Gg(),!0):!1}function Og(){if(!q.day)return``;let e=q.day;try{e=new Date(`${q.day}T00:00:00Z`).toLocaleDateString([],{month:`short`,day:`numeric`,year:`numeric`,timeZone:`UTC`})}catch{}return`<button class="leads-daychip" type="button" id="leads-dayclear" title="Show every day again">
  ...
  </button>`}function kg(){vg(),aa();let e=A();return`
    <div class="page-shell" data-section="leads">
      ...
      <div class="dashboard-card" style="margin-bottom:24px">
        ...
        <div class="card-body" id="pf-picker-body">${yg(`hourglass_empty`,`Loading…`)}</div>
      </div>`:``}
      <div class="page-header">
        <div class="page-header-text">
      ...
      <div class="leads-layout">
        <div class="dashboard-card">
          <div class="card-body" id="leads-body">
            ${yg(`hourglass_empty`,`Loading leads…`)}
          </div>
        </div>
        <aside class="dashboard-card leads-panel" aria-label="Lead detail">
          <div class="card-body" id="leads-panel-body">${o_()}</div>
        </aside>
      </div>
    </div>`}async function Ag(){document.getElementById(`leads-export`)?.addEventListener(`click`,Hg);let e=Sa(wa.LEADS_SEARCH),t=Sa(wa.LEADS_DAY);if((e||t)&&(q.search=e||``,q.day=t||``,q.filter=`ALL`,q.clientFilter=`ALL`,q.magnetFilter=`ALL`,q.range=`ALL`,q.shown=Gg()),A()){let e=e=>Number(e||0).toLocaleString();fa(Yg(),[{h:`Leads · wk`,td:t=>e(t.leads.this_week)},{h:`Leads · total`,td:t=>e(t.leads.total)},{h:`Compl.`,td:e=>e.completion_rate==null?`—`:`${e.completion_rate}%`}],{emptyMsg:`No client orgs yet. Create one under Clients first.`})}await Mg(),jg()}async function jg(){let e=document.getElementById(`leads-crm-slot`);if(e)try{let{denied:t,providers:n,configured:r}=await vc();if(t)return;let i=s_.map(e=>{let t=sc[e],i=hc(n,e);return{p:t,conn:i,status:gc(t,i,!!r?.[e])}}),a=i.filter(e=>e.status===`connected`),o=i.find(e=>e.status===`broken`);if(a.length){let t=a.map(e=>e.p.label).join(` and `),n=a.length===1&&a[0].conn?.external_account?.hub_domain||``;e.innerHTML=`
    ...
    <div class="leads-bar">
      ...
      <select class="leads-clientsel" id="leads-rangesel" aria-label="Filter by date captured">
        ${Object.keys(r_).map(e=>`<option value="${e}" ${q.range===e?`selected`:``}>${J(i_[e])}</option>`).join(``)}
      </select>
      ${Og()}
    ...
    <div id="leads-tablezone"></div>`,e.querySelector(`.leads-filter`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-filter]`);t&&(q.filter=t.dataset.filter,q.shown=Gg(),Ng())}),e.querySelector(`#leads-clientsel`)?.addEventListener(`change`,e=>{q.clientFilter=e.target.value,q.shown=Gg(),Ng()}),e.querySelector(`#leads-magnetsel`)?.addEventListener(`change`,e=>{q.magnetFilter=e.target.value,q.shown=Gg(),Ng()}),e.querySelector(`#leads-rangesel`)?.addEventListener(`change`,e=>{q.range=e.target.value,q.day=``,q.shown=Gg(),Ng()}),e.querySelector(`#leads-dayclear`)?.addEventListener(`click`,()=>{q.day=``,q.shown=Gg(),Ng()}),e.querySelector(`#leads-search`)?.addEventListener(`input`,e=>{q.search=e.target.value,q.shown=Gg(),Pg()}),Pg()}function Pg(){let e=document.getElementById(`leads-tablezone`),t=document.getElementById(`leads-countlbl`);if(!e)return;let n=Tg(),r=n.slice(0,q.shown),i=q.book?8:7;t&&(t.textContent=`${n.length} of ${q.leads.length}`),e.innerHTML=`
    <div class="leads-tablewrap">
      <table class="leads-table${q.book?` leads-table--book`:``}">
        ${c_()}
        <thead><tr>${Eg(`name`)}${q.book?`<th>Client</th>`:``}${Eg(`contact`)}<th>Interest</th><th>Magnet</th><th>Status</th>${Eg(`captured`,`num`)}<th></th></tr></thead>
        <tbody id="leads-tbody">${r.map(xg).join(``)||`<tr><td colspan="${i}"><div class="leads-muted" style="padding:14px 4px">${q.search.trim()?`No leads match your search.`:`No ${q.filter.toLowerCase()} leads.`}</div></td></tr>`}</tbody>
      </table>
    </div>
    ${en({total:n.length,shown:q.shown,size:q.pageSize,noun:`leads`})}
    ${q.truncated?`<div class="leads-muted" style="padding:10px 4px">Showing your ${q.leads.length.toLocaleString()} most recent leads of ${q.total.toLocaleString()}. Search and filters cover these; older leads aren't included yet.</div>`:``}`,e.querySelector(`thead`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-sort]`);t&&Dg(t.dataset.sort)&&Pg()}),tn(e,{total:n.length,onSize:(e,t)=>{q.pageSize=t,q.shown=e,Pg()}}),e.querySelector(`#leads-tbody`)?.addEventListener(`change`,Fg),e.querySelector(`#leads-tbody`)?.addEventListener(`click`,e=>{if(e.target.closest(`.leads-status, a.leads-contactlink`))return;let t=e.target.closest(`.leads-row`);t&&Ig(t.dataset.id)}),e.querySelector(`#leads-tbody`)?.addEventListener(`keydown`,e=>{if(e.key!==`Enter`&&e.key!==` `)return;let t=e.target.closest(`.leads-row`);t&&e.target===t&&(e.preventDefault(),Ig(t.dataset.id))})}async function Fg(e){let t=e.target.closest(`.leads-status`);if(!t)return;let n=t.dataset.id,r=q.leads.find(e=>String(e.id)===String(n)),i=t.value,a=r?.status;try{if(!(await(await fetch(`${Yg()}/m5t/v5/m5mLead/${encodeURIComponent(n)}`,{method:`PUT`,headers:{"Content-Type":`application/json`},body:JSON.stringify({status:i})})).json().catch(()=>({}))).ok)throw Error();r&&(r.status=i),Jg(`Marked ${i[0]}${i.slice(1).toLowerCase()}`,`success`)}catch{t.value=a||`NEW`,Jg(`Could not update status`,`danger`);return}try{String(n)===q.selectedId&&Bg(),q.filter!==`ALL`&&Ng()}catch(e){console.warn(`[leads] re-render after status change failed:`,e)}}function Ig(e){q.selectedId=String(e),document.querySelectorAll(`.leads-row`).forEach(e=>e.classList.toggle(`selected`,e.dataset.id===q.selectedId)),Bg()}function Lg(e,t){let n=Array.isArray(e.tags)?e.tags:[];for(let e=n.length-1;e>=0;e--)if(n[e]&&n[e].tag===t&&n[e].value!=null)return String(n[e].value);return``}function Rg(e){let t=Lg(e,`appointment_start`);if(!t)return null;let n=Lg(e,`appointment_status`)||`booked`,r=Lg(e,`appointment_timezone`),i=Lg(e,`appointment_event_type`),a=new Date(t),o=t;if(!Number.isNaN(a.getTime())){let e={weekday:`short`,month:`short`,day:`numeric`,hour:`numeric`,minute:`2-digit`};try{o=new Intl.DateTimeFormat(void 0,r?{...e,timeZone:r}:e).format(a)}catch{o=new Intl.DateTimeFormat(void 0,e).format(a)}}return{when:o,status:n,type:i,start:t}}function zg(e){let t=Array.isArray(e.tags)?e.tags:[],n=null;for(let e of t)if(!(!e||!e.tag)){if(e.tag===`callrail_call_id`){n={};continue}n&&e.tag.indexOf(`call_`)===0&&e.value!=null&&(n[e.tag]=String(e.value))}if(!n||!n.call_started)return null;let r=n.call_started,i=new Date(r),a=r;Number.isNaN(i.getTime())||(a=new Intl.DateTimeFormat(void 0,{weekday:`short`,month:`short`,day:`numeric`,hour:`numeric`,minute:`2-digit`}).format(i));let o=Number(n.call_duration)||0,s=o?o<60?`${o}s`:`${Math.floor(o/60)}m ${o%60}s`:``;return{when:a,dur:s,source:n.call_source||``,answered:n.call_answered||``,recording:n.call_recording_player||``,started:r}}function Bg(){let e=document.getElementById(`leads-panel-body`);if(!e)return;let t=q.leads.find(e=>String(e.id)===q.selectedId),n=document.querySelector(`.leads-layout`);if(n&&(n.dataset.hasSelection=t?`true`:`false`),!t){e.innerHTML=o_();return}let r=q.magnetNames[t.magnet_guid]||t.lead_source||`—`,i=Rg(t),[a,o]=i?l_[i.status]||l_.booked:[``,``],s=zg(t),c=(e,t)=>t?`<div class="leads-det__field"><span class="leads-det__k">${J(e)}</span><span class="leads-det__v">${J(t)}</span></div>`:``;e.innerHTML=`
    <div class="leads-det__head">
      <span class="leads-avatar leads-det__ava">${J(Zg(t))}</span>
      <div>
        <div class="leads-det__name">${J(Xg(t))}</div>
    ...
    ${q.canManage?`
      ...
      </div>`:``}`,e.querySelector(`[data-action="delete-lead"]`)?.addEventListener(`click`,Vg)}async function Vg(e){let t=e.currentTarget?.dataset?.id,n=q.leads.find(e=>String(e.id)===String(t));if(!n)return;let r=Xg(n);if(!window.confirm(`Remove ${r} from your leads?\n\nThis takes the lead out of your dashboard, lists and exports. The record is kept in our system, and this address can be captured again if they come back through one of your magnets.`))return;let i=e.currentTarget;i.disabled=!0;try{let e=await fetch(`${Yg()}/m5t/v5/m5mLead/${encodeURIComponent(t)}`,{method:`DELETE`}),n=await e.json().catch(()=>({}));if(!e.ok||!n.ok)throw Error(n.error||`HTTP ${e.status}`);q.leads=q.leads.filter(e=>String(e.id)!==String(t)),q.selectedId=null,Jg(`${r} removed`,`success`),Ng()}catch{i.disabled=!1,Jg(`Could not remove that lead. Please try again.`,`danger`)}}function Hg(){let e=Tg();if(!e.length){Jg(`Nothing to export`,`info`);return}let t=[`first_name`,`last_name`,`email`,`phone`,`company`,`product_interest`,`lead_source`,`utm_source`,`utm_medium`,`utm_campaign`,`gclid`,`status`],n=e=>`"${String(e??``).replace(/"/g,`""`)}"`,r=[[...q.book?[`client`]:[],...t,`magnet`,`captured`,`appointment`,`last_call`].join(`,`)];for(let i of e){let e=Rg(i),a=zg(i);r.push([...q.book?[n(q.ownerNames[i.owner_guid]||``)]:[],...t.map(e=>n(i[e])),n(q.magnetNames[i.magnet_guid]||i.lead_source||``),n(gg(i)),n(e?`${e.start} (${e.status})`:``),n(a?`${a.started} (${a.answered===`no`?`missed`:`answered`})`:``)].join(`,`))}let i=new Blob([r.join(`
`)],{type:`text/csv`}),a=URL.createObjectURL(i),o=document.createElement(`a`);o.href=a,o.download=`leads-${q.filter.toLowerCase()}.csv`,document.body.appendChild(o),o.click(),o.remove(),URL.revokeObjectURL(a),Jg(`Exported ${e.length} lead${e.length===1?``:`s`}`,`success`)}var Ug,Wg,q,Gg,Kg,qg,J,Jg,Yg,Xg,Zg,Qg,$g,e_,t_,n_,r_,i_,a_,o_,s_,c_,l_,u_=o((()=>{w(),Ug=u(f(),1),m(),ba(),Ea(),sn(),Uc(),Wg=!1,q={leads:[],magnetNames:{},filter:`ALL`,canManage:!1,book:!1,ownerNames:{},clientFilter:`ALL`,search:``,shown:25,selectedId:null,day:``,sort:{key:`captured`,dir:`desc`},magnetFilter:`ALL`,range:`ALL`,pageSize:25,total:0,truncated:!1},Gg=()=>q.pageSize===`ALL`?2**53-1:Number(q.pageSize)||25,Kg=[`NEW`,`WORKING`,`QUALIFIED`,`UNQUALIFIED`,`CONVERTED`,`EXPORTED`],qg={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},J=e=>String(e??``).replace(/[&<>"']/g,e=>qg[e]),Jg=(e,t=`info`)=>{try{Ug.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},Yg=()=>C.getConfig?.().BASE_URL||``,Xg=e=>[e.first_name,e.last_name].filter(Boolean).join(` `).trim()||e.email||`(no name)`,Zg=e=>(Xg(e).split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``)||`?`).toUpperCase(),Qg=e=>e.created?.created_tz||``,$g=e=>{let t=Qg(e);if(!t)return!1;try{return Date.now()-new Date(t).getTime()<864e5}catch{return!1}},e_=e=>q.clientFilter===`ALL`||e.owner_guid===q.clientFilter,t_=e=>!q.day||Qg(e).slice(0,10)===q.day,n_=e=>q.magnetFilter===`ALL`||e.magnet_guid===q.magnetFilter,r_={ALL:null,7:7,30:30,90:90},i_={ALL:`All time`,7:`Last 7 days`,30:`Last 30 days`,90:`Last 90 days`},a_={name:{label:`Name`,dir:`asc`,value:e=>Xg(e).toLowerCase()},contact:{label:`Contact`,dir:`asc`,value:e=>String(e.email||e.phone||``).toLowerCase()},captured:{label:`Captured`,dir:`desc`,value:e=>Qg(e)}},o_=()=>yg(`touch_app`,`Click to see lead information.`),s_=[`hubspot`,`zoho`,`salesforce`,`gohighlevel`],c_=()=>`
  <colgroup>
    <col style="width:22%">
  ...
  </colgroup>`,l_={booked:[`QUALIFIED`,`Booked`],rescheduled:[`WORKING`,`Rescheduled`],canceled:[`CANCELED`,`Canceled`]}})),d_=c({init:()=>b_,render:()=>g_,tableHTML:()=>h_});function f_(){if(A_)return;let e=document.createElement(`style`);e.id=`m5t-analytics-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),A_=!0}function p_(e,t){let n=t>0?Math.round(e/t*100):0,r=`${w_(e)} of ${w_(t)} leads for the busiest magnet`;return`<svg class="anl-bar" width="72" height="8" viewBox="0 0 72 8" role="img"
      aria-label="${E_(r)}">
      <title>${E_(r)}</title>
    ...
    </svg>`}function m_(e,t,n){let r=M_[e.status]?`<span class="anl-chip">${E_(M_[e.status])}</span>`:``,i=n&&e.owner_name?`<span class="anl-sub">${E_(e.owner_name)}</span>`:``,a=e.ab_tests&&(e.ab_tests.running||e.ab_tests.completed)?`${w_(e.ab_tests.running)} / ${w_(e.ab_tests.completed)}`:`—`;return`<tr data-guid="${E_(e.magnet_guid)}">
    <td><a class="anl-name" href="#/magnets">${E_(e.name)}</a>${r}${i}</td>
    <td>${p_(Number(e.leads?.window||0),t)}</td>
    <td>${w_(e.leads?.window)}</td>
    <td>${w_(e.opens)}</td>
    <td>${D_(e.engagement_rate)}</td>
    <td>${D_(e.completion_rate)}</td>
    <td>${D_(e.lead_rate)}</td>
    <td>${a}</td>
  </tr>`}function h_(e,t){let n=e.reduce((e,t)=>Math.max(e,Number(t.leads?.window||0)),0);return`<div class="anl-wrap"><table class="anl-table">
    <thead><tr>
      <th>Magnet</th><th></th><th>Leads</th><th>Opens</th>
      <th>Engaged</th><th>Completed</th><th>Leads / Open</th><th>A/B run / done</th>
    </tr></thead>
    <tbody>${e.map(e=>m_(e,n,t)).join(``)}</tbody>
  </table></div>`}function g_(){return A()?ua({section:`analytics`,title:`Analytics`,description:`Pick a client to see how their magnets are performing.`}):(f_(),Ma(),`
    <div class="page-shell" data-section="analytics">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="anl-body">
          ${j_(`hourglass_empty`,`Loading magnet performance…`)}
        </div>
      </div>
    ...
    </div>`)}function __(e){try{return new Date(`${e}T00:00:00`).toLocaleDateString(void 0,{month:`short`,day:`numeric`,year:`numeric`})}catch{return String(e||``)}}function v_(e){return Na({rows:e,xLabel:e=>String(e.date||``).slice(5),left:{value:e=>e.opens||0,label:`Magnet Opens`},right:{value:e=>e.leads||0,label:`Leads`},ariaLabel:`Magnet opens versus leads over time`,activatable:e=>Number(e.leads||0)>0,pointLabel:e=>`${__(e.date)}: ${w_(e.opens||0)} opens, ${w_(e.leads||0)} leads`,hitData:e=>({day:e.date,leads:Number(e.leads||0)})})}function y_(e){let t=document.getElementById(`anl-trend`);return t?Fa(t,{rows:e,tipHTML:e=>`
      <div class="m5c-tip__title">${__(e.date)}</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--1"></i>${w_(e.opens||0)} opens</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--2"></i>${w_(e.leads||0)} leads</div>
      ${Number(e.leads||0)>0?`<div class="m5c-tip__go">Open these leads →</div>`:``}`,onActivate:e=>{xa(wa.LEADS_DAY,e.date),window.location.hash=`#/leads`}}):()=>{}}async function b_(){if(A())return fa(S_(),[{h:`Leads`,td:e=>w_(e.leads.window??e.leads.total)},{h:`Opens`,td:e=>w_(e.opens)},{h:`Completed`,td:e=>D_(e.completion_rate)}]);f_(),Ma(),await x_();let e=document.getElementById(`anl-periods`);e&&e.addEventListener(`click`,t=>{let n=t.target.closest(`.chart-period-btn`);n&&(k_=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>{e.classList.toggle(`active`,Number(e.dataset.days)===k_)}),x_())})}async function x_(){let e=document.getElementById(`anl-body`),t=document.getElementById(`anl-range`);if(!e)return;e.innerHTML=j_(`hourglass_empty`,`Loading magnet performance…`);let n=new Date,r=C_(n),i=C_(new Date(n.getTime()-(k_-1)*864e5));t&&(t.textContent=`${i} → ${r}`);let a;try{let e=await fetch(`${S_()}/m5t/v5/dashboard/stats?from=${i}&to=${r}&group_by=magnet`);if(a=await e.json(),!e.ok||!a?.ok)throw Error(a?.error||`HTTP ${e.status}`)}catch(t){e.innerHTML=j_(`cloud_off`,`Could not load analytics. Refresh to try again.`),console.error(`[analytics] load failed:`,t);return}let o=(e,t)=>{let n=document.getElementById(e);n&&(n.textContent=t)};o(`anl-leads`,w_(a.by_day?.reduce((e,t)=>e+Number(t.leads||0),0))),o(`anl-opens`,w_(a.funnel?.opens)),o(`anl-completion`,D_(a.funnel?.completion_rate)),o(`anl-active`,w_(a.magnets?.active));let s=Array.isArray(a.by_day)?a.by_day:[],c=document.getElementById(`anl-trend`);if(c){let e=s.some(e=>Number(e.opens||0)>0||Number(e.leads||0)>0);c.innerHTML=e?v_(s):j_(`show_chart`,`No activity in this range yet. Opens and leads chart here as visitors engage your magnets.`),e&&y_(s)}let l=Array.isArray(a.by_magnet)?a.by_magnet:[];if(!l.length){e.innerHTML=j_(`widgets`,`No magnets yet. Create one and its numbers will show up here.`);return}let u=l.some(e=>Number(e.opens||0)>0||Number(e.leads?.window||0)>0),d=new Set(l.map(e=>e.owner_guid)).size>1;e.innerHTML=(a.by_magnet_truncated?`<p class="anl-warn">Showing the top ${l.length} magnets by leads. You have more than that, so this list is not complete.</p>`:``)+(u?``:`<p class="anl-warn">No opens or leads for any magnet in this period. Try a longer range.</p>`)+h_(l,d);let f=e.querySelector(`.anl-table tbody`);f&&E(f,{rowSelector:`tr`,noun:`magnets`,controlHost:e.querySelector(`.anl-wrap`)||e})}var S_,C_,w_,T_,E_,D_,O_,k_,A_,j_,M_,N_=o((()=>{w(),ba(),Aa(),sn(),Ba(),Ea(),S_=()=>C.getConfig?.().BASE_URL||``,C_=e=>e.toISOString().slice(0,10),w_=e=>Number(e||0).toLocaleString(),T_={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},E_=e=>String(e??``).replace(/[&<>"']/g,e=>T_[e]),D_=e=>e==null?`—`:`${e}%`,O_=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],k_=30,A_=!1,j_=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,M_={DRAFT:`Draft`,DISABLED:`Paused`}})),P_=c({default:()=>fv,init:()=>L_,render:()=>I_});function F_(){if(Ma(),lv)return;let e=document.createElement(`style`);e.id=`m5t-conversions-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),lv=!0}function I_(){return A()?ua({section:`analytics-conversions`,title:`Conversions`,description:`Pick a client to view their conversion reporting.`}):(F_(),Y.canSave=Ic(),`
    <div class="page-shell" data-section="analytics-conversions">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="conv-selector">
          ${uv(`hourglass_empty`,`Loading conversion actions.`)}
        </div>
      </div>
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="conv-chart">
          ${uv(`hourglass_empty`,`Loading conversions.`)}
        </div>
      </div>
    ...
    </div>`)}async function L_(){if(A()){let e=e=>Number(e||0).toLocaleString();return fa(X_(),[{h:`Spend`,td:t=>t.ad?`$${e(t.ad.spend)}`:Q_},{h:`Clicks`,td:t=>t.ad?e(t.ad.clicks):Q_},{h:`Leads`,td:t=>e(t.leads.window??t.leads.total)},{h:`CPL`,td:t=>t.ad?.cost_per_lead==null?Q_:`$${e(t.ad.cost_per_lead)}`}],{emptyMsg:`No client orgs yet. Connect a client ad account to see conversions here.`})}let e=document.getElementById(`conv-periods`);e?.addEventListener(`click`,t=>{let n=t.target.closest(`button[data-days]`);n&&(cv=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>e.classList.toggle(`active`,Number(e.dataset.days)===cv)),R_())}),document.getElementById(`conv-selector`)?.addEventListener(`change`,e=>{let t=e.target.closest(`input[type="checkbox"][data-action-id]`);t&&(t.checked?Y.selection.add(t.value):Y.selection.delete(t.value),G_(),B_())}),await R_()}async function R_(){let e=new Date;Y.to=Z_(e),Y.from=Z_(new Date(e.getTime()-(cv-1)*864e5));let t=document.getElementById(`conv-range`);t&&(t.textContent=`${Y.from} → ${Y.to}`);let n=X_(),r=`from=${Y.from}&to=${Y.to}`,i=null,a=null,o=null,s=[],c=null;try{let e=Ic(),[t,l,u,d,f]=await Promise.all([fetch(`${n}/m5t/v5/coreAdMetrics/overlay?${r}`).then(e=>e.json()).catch(()=>null),fetch(`${n}/m5t/v5/coreAdMetrics/conversions?${r}`).then(e=>e.json()).catch(()=>null),fetch(`${n}/m5t/v5/coreAdMetrics/upload-ledger?${r}`).then(e=>e.json()).catch(()=>null),fetch(`${n}/m5t/v5/coreAdAccount`).then(e=>e.json()).catch(()=>null),e?fetch(`${n}/m5t/v5/integrations/google_ads/conversion_actions`).then(e=>e.json()).catch(()=>null):Promise.resolve(null)]);i=t,a=l,o=u,c=f,s=(d?.data||[]).filter(e=>e.status!==`DELETED`)}catch(e){console.warn(`[analytics-conversions] load failed:`,e),Y_(`conv-chart`,uv(`error`,`Could not load conversions. Please try again.`));return}Y.spend=i?.totals?.spend??null,Y.overlayRows=i?.rows||[],Y.canSave=Ic();let l=s.filter(e=>e.provider===`google_ads`);Y.universe=z_({catalog:c,conv:a,accounts:l});let u=a?.has_selection&&Array.isArray(a.selected_ids)?a.selected_ids.map(String):[],d=Y.universe.filter(e=>e.magnet).map(e=>e.id);if(Y.selection=new Set(u.length?u:d),q_(o),W_({accounts:l}),G_(),!l.length){J_(`conv-spend`,$_(Y.spend)),Y_(`conv-chart`,uv(`link_off`,`No ad account connected yet. Your agency can connect Google Ads to see conversions here.`)),Y_(`conv-actions`,uv(`link_off`,`No ad account connected yet.`));return}u.length||!Y.selection.size?(Y.conv=a,V_()):await B_()}function z_({catalog:e,conv:t,accounts:n}){let r=new Map,i=(e,t)=>{let n=String(e||``);if(!n)return;let i=r.get(n)||{id:n,name:null,category:null,magnet:!1,kind:null};r.set(n,{...i,...Object.fromEntries(Object.entries(t).filter(([,e])=>e!=null&&e!==!1))})};(e?.data?.actions||[]).forEach(e=>i(e.id,{name:e.name,category:e.category})),(t?.actions||[]).forEach(e=>i(e.id,{name:e.name,category:e.category})),n.forEach(e=>{let t=e.account_config?.magnet_actions||{};Object.entries(t).forEach(([e,t])=>{t?.id&&i(t.id,{name:t.name,magnet:!0,kind:e})})}),r.forEach(e=>{!e.magnet&&e.name&&ov.has(e.name)&&(e.magnet=!0)});let a=[...r.values()],o=e=>e.magnet?iv.indexOf(e.kind):-1;return a.sort((e,t)=>{if(e.magnet!==t.magnet)return e.magnet?-1:1;if(e.magnet){let n=o(e),r=o(t);if(n!==r)return(n<0?99:n)-(r<0?99:r)}return String(e.name||e.id).localeCompare(String(t.name||t.id))})}async function B_(){let e=[...Y.selection],t=document.getElementById(`conv-chart`);t?.classList.add(`conv-busy`),Y.busy=!0;try{if(!e.length)Y.conv={ok:!0,has_selection:!1,rows:[],actions:[],totals:{conversions:0,value:0}};else{let t=X_(),n=await fetch(`${t}/m5t/v5/coreAdMetrics/conversions?from=${Y.from}&to=${Y.to}&actions=${encodeURIComponent(e.join(`,`))}`);Y.conv=await n.json()}V_()}catch(e){console.warn(`[analytics-conversions] recompute failed:`,e),Y_(`conv-chart`,uv(`error`,`Could not recompute for that selection. Please try again.`))}finally{Y.busy=!1,t?.classList.remove(`conv-busy`)}}function V_(){let e=Y.conv||{},t=e.totals||{conversions:0,value:0},n=Number(t.conversions||0);J_(`conv-spend`,$_(Y.spend)),J_(`conv-count`,ev(n)),J_(`conv-cpc`,$_(rv(Y.spend,n))),J_(`conv-value`,$_(t.value??0)),H_(e.rows||[]),U_(e.actions||[])}function H_(e){let t=document.getElementById(`conv-chart`);if(!t)return;if(!Y.selection.size){t.innerHTML=uv(`checklist`,`No actions counted. Tick a conversion action to see it here.`),Y.rows=[];return}let n=new Map(e.map(e=>[String(e.date).slice(0,10),e])),r=Y.overlayRows.length?Y.overlayRows.map(e=>String(e.date).slice(0,10)):e.map(e=>String(e.date).slice(0,10)),i=new Map(Y.overlayRows.map(e=>[String(e.date).slice(0,10),Number(e.spend||0)])),a=r.map(e=>({date:e,spend:i.get(e)||0,conversions:Number(n.get(e)?.conversions||0)}));if(Y.rows=a,!a.length){t.innerHTML=uv(`search_off`,`No conversions in this range for the selected actions.`);return}t.innerHTML=Na({rows:a,xLabel:e=>String(e.date||``).slice(5),left:{value:e=>e.spend||0,label:`Ad Spend`,fmt:e=>`$${Ra(e)}`},right:{value:e=>e.conversions||0,label:`Conversions`},ariaLabel:`Ad spend versus counted conversions over time`,pointLabel:e=>`${e.date}: ${$_(e.spend||0)} spend, ${ev(e.conversions||0)} conversions`,hitData:e=>({day:e.date})}),Fa(t,{rows:a,tipHTML:e=>`
      <div class="m5c-tip__title">${nv(String(e.date||``))}</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--1"></i>${$_(e.spend||0)} spend</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--2"></i>${ev(e.conversions||0)} conversions</div>`})}function U_(e){let t=document.getElementById(`conv-actions`);if(!t)return;let n=new Set(Y.universe.filter(e=>e.magnet).map(e=>e.id)),r=e.filter(e=>Y.selection.has(String(e.id)));if(!r.length){t.innerHTML=uv(`search_off`,Y.selection.size?`No conversions in this range for the selected actions.`:`No actions counted. Tick a conversion action to see it here.`);return}t.innerHTML=`
    <div class="conv-wrap">
      <table class="conv-table">
        <thead><tr><th>Conversion action</th><th>Category</th><th class="num">Conversions</th><th class="num">Value</th></tr></thead>
        <tbody>${r.map(e=>`
          <tr>
            <td><span class="conv-name">${nv(e.name||e.id)}</span>${n.has(String(e.id))?` <span class="conv-tag">Magnet</span>`:``}</td>
            <td class="sub">${nv(dv(e.category))}</td>
            <td class="num"><b>${ev(e.conversions)}</b></td>
            <td class="num">${$_(e.value??0)}</td>
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>`;let i=t.querySelector(`.conv-table tbody`);i&&E(i,{rowSelector:`tr`,noun:`actions`,controlHost:t.querySelector(`.conv-wrap`)||t})}function W_({accounts:e}){let t=document.getElementById(`conv-selector`);if(!t)return;if(!e.length){t.innerHTML=uv(`link_off`,`No ad account connected yet. Your agency can connect Google Ads to see conversions here.`);return}if(!Y.universe.length){t.innerHTML=`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">checklist</span>
    ...
    <label class="conv-row">
      <input type="checkbox" data-action-id="${nv(e.id)}" value="${nv(e.id)}"${Y.selection.has(e.id)?` checked`:``}>
      <span class="conv-name">${nv(e.name||e.id)}</span>
      ${e.magnet?`<span class="conv-tag">Magnet</span>`:``}
      ${e.category?`<span class="conv-cat">${nv(dv(e.category))}</span>`:``}
    </label>`;t.innerHTML=`
    ...
    ${Y.canSave?`
      ...
      </div>`:``}`;let a=t.querySelector(`#conv-list-other`);a&&E(a,{rowSelector:`.conv-row`,noun:`actions`}),document.getElementById(`conv-save`)?.addEventListener(`click`,K_)}function G_(){let e=document.getElementById(`conv-selcount`);if(!e)return;let t=Y.selection.size;e.textContent=t?`${t} action${t===1?``:`s`} counted`:`No actions counted`}async function K_(){let e=document.getElementById(`conv-save`),t=document.getElementById(`conv-savemsg`),n=[...Y.selection];e&&(e.disabled=!0),t&&(t.classList.remove(`err`),t.textContent=`Saving.`);try{let e=await(await fetch(`${X_()}/m5t/v5/integrations/google_ads/conversion_actions`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({conversion_action_ids:n})})).json().catch(()=>({}));if(!e.ok)throw Error(e.error||`save failed`);t&&(t.textContent=n.length?`Saved. ${n.length} action${n.length===1?``:`s`} counted by default.`:`Saved. No actions counted by default.`)}catch{t&&(t.classList.add(`err`),t.textContent=`Could not save the selection. Please try again.`)}finally{e&&(e.disabled=!1)}}function q_(e){let t=document.getElementById(`conv-ledger`);if(!t)return;let n=e?.kinds||[];if(!n.length){t.innerHTML=uv(`upload`,`Nothing fed into Google Ads in this range yet.`);return}let r=e=>av[e]||String(e||``).replace(/_/g,` `),i=e=>{let t=iv.indexOf(e.kind);return t<0?99:t};t.innerHTML=`
    <div class="conv-wrap">
      <table class="conv-table" id="conv-ledger-table">
        ...
        <tbody>${[...n].sort((e,t)=>i(e)-i(t)).map(e=>`
          <tr data-kind="${nv(e.kind)}">
            <td><span class="conv-name">${nv(r(e.kind))}</span></td>
            <td class="num"><b>${ev(e.sent)}</b></td>
            <td class="num">${ev(e.queued)}</td>
            <td class="num">${ev(e.failed)}</td>
            <td class="num">${ev(e.skipped)}</td>
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>
    <p class="conv-note">What Magnet fed into Google Ads. Google's attributed numbers above are its own and are never added to these.</p>`}function J_(e,t){let n=document.getElementById(e);n&&(n.textContent=t)}function Y_(e,t){let n=document.getElementById(e);n&&(n.innerHTML=t)}var X_,Z_,Q_,$_,ev,tv,nv,rv,iv,av,ov,sv,cv,Y,lv,uv,dv,fv,pv=o((()=>{w(),ba(),Uc(),Aa(),Ba(),sn(),X_=()=>C.getConfig?.().BASE_URL||``,Z_=e=>e.toISOString().slice(0,10),Q_=`—`,$_=e=>e==null?Q_:`$`+Number(e).toLocaleString(void 0,{minimumFractionDigits:0,maximumFractionDigits:2}),ev=e=>e==null?Q_:Number(e).toLocaleString(),tv={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},nv=e=>String(e??``).replace(/[&<>"']/g,e=>tv[e]),rv=(e,t)=>{let n=Number(e),r=Number(t);return!Number.isFinite(n)||!Number.isFinite(r)||r<=0?null:Math.round(n/r*100)/100},iv=[`magnet_fill`,`qualified_lead`,`call_dni`,`calls_from_ads`],av={magnet_fill:`Magnet fills`,qualified_lead:`Qualified leads`,call_dni:`Website calls`,calls_from_ads:`Calls from ads`},ov=new Set([`Magnet Lead`,`Magnet Qualified Lead`,`Magnet Phone Call (Website)`,`Calls from Ads (Magnet)`]),sv=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],cv=30,Y={from:null,to:null,spend:null,universe:[],selection:new Set,conv:null,rows:[],overlayRows:[],canSave:!1,busy:!1},lv=!1,uv=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,dv=e=>String(e||``).toLowerCase().replace(/_/g,` `)||Q_,fv={render:I_,init:L_}})),mv=c({default:()=>Fv,init:()=>_v,render:()=>gv});function hv(){if(Ma(),jv)return;let e=document.createElement(`style`);e.id=`m5t-adperf-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),jv=!0}function gv(){return A()?ua({section:`ad-performance`,title:`Ad Performance`,description:`Pick a client to view their spend and lead performance.`}):(hv(),`
    <div class="page-shell" data-section="ad-performance">
      <div class="page-header">
        ...
        <div class="chart-controls" id="adperf-periods">
          ${Nv.map(e=>`<button class="chart-period-btn${e.d===Pv?` active`:``}" type="button" data-days="${e.d}">${e.label}</button>`).join(``)}
        </div>
      </div>
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="adperf-body">
          ${Mv(`hourglass_empty`,`Loading ad performance…`)}
        </div>
      </div>
    ...
    </div>`)}async function _v(){if(A()){let e=e=>Number(e||0).toLocaleString();return fa(Tv(),[{h:`Spend`,td:t=>t.ad?`$${e(t.ad.spend)}`:`—`},{h:`Clicks`,td:t=>t.ad?e(t.ad.clicks):`—`},{h:`Leads`,td:t=>e(t.leads.window??t.leads.total)},{h:`CPL`,td:t=>t.ad?.cost_per_lead==null?`—`:`$${e(t.ad.cost_per_lead)}`}],{emptyMsg:`No client orgs yet. Connect a client ad account to see spend here.`})}let e=document.getElementById(`adperf-periods`);e?.addEventListener(`click`,t=>{let n=t.target.closest(`button[data-days]`);n&&(Pv=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>e.classList.toggle(`active`,Number(e.dataset.days)===Pv)),vv())}),document.getElementById(`adperf-sources`)?.addEventListener(`click`,e=>{let t=e.target.closest(`tr[data-campaign]`);t&&(xa(wa.LEADS_SEARCH,t.dataset.campaign),!e.target.closest(`a`)&&(window.location.hash=`#/leads`))}),await vv()}async function vv(){let e=Tv(),t=document.getElementById(`adperf-body`),n=document.getElementById(`adperf-range`),r=new Date,i=Ev(r),a=Ev(new Date(r.getTime()-(Pv-1)*864e5));n&&(n.textContent=`${a} → ${i}`);let o,s=[],c=null,l=null,u=null;try{let[t,n,r,d,f]=await Promise.all([fetch(`${e}/m5t/v5/coreAdMetrics/overlay?from=${a}&to=${i}`),fetch(`${e}/m5t/v5/coreAdAccount`),fetch(`${e}/m5t/v5/coreAdMetrics/by-source?from=${a}&to=${i}`),vc().catch(()=>null),fetch(`${e}/m5t/v5/coreAdMetrics/conversions?from=${a}&to=${i}`).then(e=>e.json()).catch(()=>null)]);if(!t.ok)throw Error(`HTTP ${t.status}`);o=await t.json(),s=((await n.json().catch(()=>({})))?.data||[]).filter(e=>e.status!==`DELETED`),c=await r.json().catch(()=>null),l=d,u=f}catch(e){console.warn(`[ad-performance] load failed:`,e),t&&(t.innerHTML=Mv(`error`,`Could not load ad performance. Please try again.`));return}yv(l,s),bv(u,o?.totals?.spend??null);let d=o.totals||{};if(Sv(`adperf-spend`,Dv(d.spend)),Sv(`adperf-clicks`,Ov(d.clicks)),Sv(`adperf-leads`,Ov(d.magnet_leads)),Sv(`adperf-cpl`,Dv(d.cost_per_lead)),xv(c),!s.length){t.innerHTML=Mv(`link_off`,`No ad account connected yet. Your agency can connect Google Ads or Local Services Ads to see performance here.`);return}if(!o.connected){t.innerHTML=Mv(`cloud_sync`,`Account connected. No ad data for this range yet. Metrics appear here once they sync.`);return}let f=o.rows||[];t.innerHTML=Cv(f),wv(f)}function yv(e,t){let n=document.getElementById(`adperf-conn`);if(!n)return;if(!e||e.denied){n.innerHTML=``;return}let r=sc.google_ads,i=hc(e.providers,`google_ads`),a=gc(r,i,!!e.configured?.google_ads),o=Ic()?` <a href="#/settings/integrations/${r.urlSlug}">Manage</a>`:``;if(a===`connected`||a===`broken`){Mc(),n.innerHTML=`<span>Google Ads</span> ${_c(a,i)}${o}`;return}if(t.length){n.innerHTML=`<span>Google Ads</span> <span>${t.length} associated account${t.length===1?``:`s`}</span>`;return}n.innerHTML=``}function bv(e,t){let n=document.getElementById(`adperf-conv-card`),r=document.getElementById(`adperf-conv-body`),i=document.getElementById(`adperf-stats`),a=document.getElementById(`adperf-conv-stat`);a&&a.remove();let o=document.getElementById(`adperf-conv-cpc`);if(o&&o.remove(),!n||!r)return;if(!e?.ok||!e.has_selection){n.style.display=`none`;return}if(i){let n=document.createElement(`div`);n.innerHTML=Da({icon:`flag`,label:`Ads Conversions`,value:Ov(e.totals?.conversions),sub:`Reported by Google Ads`,href:`#/settings/integrations/google-ads`});let r=n.firstElementChild;r.id=`adperf-conv-stat`,i.appendChild(r);let a=Number(e.totals?.conversions||0),o=t!=null&&a>0?Math.round(Number(t)/a*100)/100:null,s=document.createElement(`div`);s.innerHTML=Da({icon:`request_quote`,label:`Cost / Conversion`,value:Dv(o),sub:`Against the counted actions`,href:`#/analytics/conversions`});let c=s.firstElementChild;c.id=`adperf-conv-cpc`,i.appendChild(c)}let s=e.actions||[];if(n.style.display=``,!s.length){r.innerHTML=Mv(`search_off`,`No conversions in this range for the selected actions.`);return}let c=s.some(e=>e.value>0);r.innerHTML=`
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
        ...
        <tbody>${s.map(e=>`
          <tr>
            <td>${Av(e.name||e.id)}</td>
            <td class="adperf-src-sub">${Av(String(e.category||``).toLowerCase().replace(/_/g,` `)||`—`)}</td>
            <td class="num"><b>${Ov(e.conversions)}</b></td>
            ${c?`<td class="num">${Dv(e.value)}</td>`:``}
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>
    <p class="adperf-note">Counted actions are chosen on the Google Ads integration page. These are Google's own numbers and may overlap magnet leads, so they are never added together.</p>`}function xv(e){let t=document.getElementById(`adperf-sources`),n=document.getElementById(`adperf-src-summary`);if(!t)return;let r=e&&Array.isArray(e.rows)?e.rows:[];if(!r.length){n&&(n.textContent=``),t.innerHTML=Mv(`search_off`,`No leads in this range yet. Campaigns appear here as tagged leads (?utm_campaign=…) come in.`);return}let i=e.total_leads||r.reduce((e,t)=>e+(t.leads||0),0),a=e.tracked_leads||0;n&&(n.textContent=`${Ov(a)} of ${Ov(i)} leads tagged to a source`);let o=e=>e.campaign&&e.campaign!==`(none)`?Av(e.campaign):e.source&&e.source!==`(direct)`?Av(e.source):`<span class="adperf-src-untracked">Untracked / direct</span>`,s=e=>e.campaign&&e.campaign!==`(none)`?e.campaign:e.source&&e.source!==`(direct)`?e.source:``;t.innerHTML=`
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
        ...
        <tbody>${r.map(e=>{let t=s(e);return`
          <tr${t?` data-campaign="${Av(t)}" class="adperf-srcrow--link"`:``}>
            <td>${t?`<a class="adperf-srclink" href="#/leads" title="Show the leads from ${Av(t)}">${o(e)}</a>`:o(e)}</td>
            <td class="adperf-src-sub">${Av(e.source||`—`)}${e.medium?` · ${Av(e.medium)}`:``}</td>
            <td class="num">${e.paid_clicks?Ov(e.paid_clicks):`<span class="adperf-src-sub">—</span>`}</td>
            <td class="num"><b>${Ov(e.leads)}</b></td>
          </tr>`}).join(``)}
        </tbody>
      </table>
    </div>`;let c=t.querySelector(`.adperf-srctable tbody`);c&&E(c,{rowSelector:`tr`,noun:`sources`,controlHost:t.querySelector(`.adperf-srcwrap`)||t})}function Sv(e,t){let n=document.getElementById(e);n&&(n.textContent=t)}function Cv(e){return Na({rows:e,xLabel:e=>String(e.date||``).slice(5),left:{value:e=>e.spend||0,label:`Ad Spend`,fmt:e=>`$${Ra(e)}`},right:{value:e=>e.magnet_leads||0,label:`Magnet Leads`},ariaLabel:`Ad spend versus magnet leads over time`,pointLabel:e=>`${e.date}: ${Dv(e.spend||0)} spend, ${Ov(e.magnet_leads||0)} leads`,hitData:e=>({day:e.date})})}function wv(e){let t=document.getElementById(`adperf-body`);return t?Fa(t,{rows:e,tipHTML:e=>`
      <div class="m5c-tip__title">${Av(String(e.date||``))}</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--1"></i>${Dv(e.spend||0)} spend</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--2"></i>${Ov(e.magnet_leads||0)} leads</div>`}):()=>{}}var Tv,Ev,Dv,Ov,kv,Av,jv,Mv,Nv,Pv,Fv,Iv=o((()=>{w(),ba(),Uc(),Aa(),Ba(),sn(),Ea(),Tv=()=>C.getConfig?.().BASE_URL||``,Ev=e=>e.toISOString().slice(0,10),Dv=e=>e==null?`—`:`$`+Number(e).toLocaleString(void 0,{minimumFractionDigits:0,maximumFractionDigits:2}),Ov=e=>Number(e||0).toLocaleString(),kv={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Av=e=>String(e??``).replace(/[&<>"']/g,e=>kv[e]),jv=!1,Mv=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,Nv=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],Pv=30,Fv={render:gv,init:_v}})),Lv=c({default:()=>iy,init:()=>Bv,parseBookingCsv:()=>Zv,render:()=>zv});function Rv(){if(ty)return;let e=document.createElement(`style`);e.id=`m5t-adacct-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),ty=!0}function zv(){return A()?ua({section:`ad-accounts`,title:`Ad Accounts`,description:`Pick a client to see their connected ad accounts.`}):(Rv(),Mc(),`
...
`&&a++,n.push(r),r=``,n.some(e=>e.trim()!==``)&&t.push(n),n=[]):r+=o}if(n.push(r),n.some(e=>e.trim()!==``)&&t.push(n),t.length<2)return[];let a=t[0].map(e=>e.trim().toLowerCase()),o=a.findIndex(e=>/e-?mail/.test(e)),s=a.findIndex(e=>/phone|mobile|tel/.test(e)),c=a.findIndex(e=>/date|time|booked|appointment|start/.test(e));return o<0&&s<0?[]:t.slice(1).map(e=>({email:o>=0?(e[o]||``).trim():``,phone:s>=0?(e[s]||``).trim():``,at:c>=0?(e[c]||``).trim():``})).filter(e=>e.email||e.phone).slice(0,5e3)}function Qv(e,t){let n=new Blob([e],{type:`text/csv;charset=utf-8`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.href=r,i.download=t,document.body.appendChild(i),i.click(),i.remove(),setTimeout(()=>URL.revokeObjectURL(r),4e3)}var $v,ey,ty,ny,ry,iy,ay=o((()=>{ba(),sn(),Uc(),$v={google_ads:`Google Ads`,google_lsa:`Local Services Ads`},ey=e=>{let t=String(e||``).replace(/\D/g,``);return t.length===10?`${t.slice(0,3)}-${t.slice(3,6)}-${t.slice(6)}`:String(e||``)},ty=!1,ny=[],ry=null,iy={render:zv,init:Bv}})),oy=c({default:()=>Oy,init:()=>my,openInviteModal:()=>vy,render:()=>py});function sy(e,t=`info`){try{xy.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}}function cy(){let e=C.getActiveOrg();if(e&&e.channel===Sy)return{guid:e.guid,name:e.name};let t=C.getMemberships().find(e=>e.channel===Sy);return t?{guid:t.orgGuid,name:t.name}:null}function ly(e){let t=e?.created_tz;if(!t)return``;let n=new Date(t);return Number.isNaN(n.getTime())?``:n.toLocaleDateString(void 0,{year:`numeric`,month:`short`,day:`numeric`})}function uy(){if(wy)return;let e=document.createElement(`style`);e.id=`m5t-clients-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),dr(),wy=!0}function dy(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function fy(e){let t=e.caption||e.guid,n=e.status||`ENABLED`;return`
    <div class="client-row">
      <span class="client-row__avatar">${X(Ey(t))}</span>
    ...
    <div class="page-shell" data-section="clients">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="clients-body">
          ${dy(`hourglass_empty`,`Loading clients…`)}
        </div>
      </div>
    </div>`}async function my(){let e=cy();document.getElementById(`clients-add-btn`)?.addEventListener(`click`,()=>gy(e)),await hy(e)}async function hy(e){let t=document.getElementById(`clients-body`),n=document.getElementById(`clients-count`);if(t){if(!e?.guid){t.innerHTML=dy(`info`,`Client provisioning is available to reseller / agency accounts.`),n&&(n.textContent=``);return}try{let r=C.getConfig?.().BASE_URL||``,i=await fetch(`${r}/m5t/v5/acctGroup`);if(!i.ok)throw Error(`HTTP ${i.status}`);let a=await i.json(),o=(Array.isArray(a?.data)?a.data:[]).filter(t=>t.class===Cy&&(!t.parent||t.parent===e.guid));if(!o.length){t.innerHTML=dy(`group_off`,`No clients yet. Add your first client to start managing magnets on their behalf.`),n&&(n.textContent=``);return}t.innerHTML=`<div class="client-list">${o.map(fy).join(``)}</div>`,E(t.querySelector(`.client-list`),{rowSelector:`.client-row`,noun:`clients`}),n&&(n.textContent=`${o.length} client${o.length===1?``:`s`}`),t.querySelectorAll(`.client-card__invite`).forEach(e=>{e.addEventListener(`click`,()=>{e.dataset.action===`adaccounts`?_y(e.dataset.clientGuid,e.dataset.clientName):e.dataset.action===`questionnaire`?by(e.dataset.clientGuid,e.dataset.clientName):vy(e.dataset.clientGuid,e.dataset.clientName)})})}catch(e){console.warn(`[clients] load failed:`,e),t.innerHTML=dy(`error`,`Couldn’t load clients. Please try again.`),n&&(n.textContent=``)}}}function gy(e){if(!e?.guid){sy(`Only reseller / agency accounts can add clients.`,`error`);return}let t=document.createElement(`div`);t.className=`client-modal-scrim`;let n=document.createElement(`div`);n.className=`client-modal`,n.setAttribute(`role`,`dialog`),n.setAttribute(`aria-modal`,`true`),n.innerHTML=`
    ...
    <div class="client-modal__body" id="qm-body">${dy(`hourglass_empty`,`Loading…`)}</div>`,document.body.appendChild(r),document.body.appendChild(i),requestAnimationFrame(()=>{r.classList.add(`open`),i.classList.add(`open`)});let a=()=>{r.classList.remove(`open`),i.classList.remove(`open`),document.removeEventListener(`keydown`,o),setTimeout(()=>{r.remove(),i.remove()},200)},o=e=>{e.key===`Escape`&&a()};document.addEventListener(`keydown`,o),r.addEventListener(`click`,a),i.querySelector(`.client-modal__close`)?.addEventListener(`click`,a);let s=i.querySelector(`#qm-body`);function c(){s.innerHTML=`
      ...
      </div>`,s.querySelector(`[data-act="done"]`)?.addEventListener(`click`,a),s.querySelector(`[data-act="resend"]`)?.addEventListener(`click`,()=>c())}function d(e,t=!1){let r=e.editability||{editable:e.status===`SUBMITTED`&&!e.locked_tz,reason:e.locked_tz?`finalized`:null,deadline:null},i=!!r.editable,o=!i&&e.status===`SUBMITTED`?r.reason===`finalized`?`<div class="qm-status" data-status="LOCKED"><span class="material-symbols-outlined" aria-hidden="true">lock</span> Finalized${e.locked_tz?` ${X(yy(e.locked_tz))}`:``}${e.locked_by_email?` by ${X(e.locked_by_email)}`:``} — no further edits.</div>`:`<div class="qm-status" data-status="LOCKED"><span class="material-symbols-outlined" aria-hidden="true">lock_clock</span> Editing closed${r.deadline?` ${X(yy(r.deadline))}`:``} — a response locks 14 days after it is submitted.</div>`:``,c=i&&r.deadline?`<p class="client-modal__sub qm-editable-hint">You can edit these answers until ${X(yy(r.deadline))}.</p>`:``;s.innerHTML=`
      <div class="qm-status" data-status="SUBMITTED"><span class="material-symbols-outlined" aria-hidden="true">task_alt</span> Submitted ${e.submitted_tz?X(yy(e.submitted_tz)):``}</div>
      ${o}
      ${ur(e)}
      <form id="qm-form">
      ...
      </div>`,s.querySelector(`[data-act="done"]`)?.addEventListener(`click`,a),s.querySelector(`[data-act="cancel-edit"]`)?.addEventListener(`click`,()=>d(e,!1)),s.querySelector(`[data-act="edit"]`)?.addEventListener(`click`,()=>d(e,!0));let l=s.querySelector(`[data-act="save"]`);l?.addEventListener(`click`,async()=>{let t=lr(s);l.disabled=!0;let r=l.innerHTML;l.innerHTML=`<span class="material-symbols-outlined" aria-hidden="true">progress_activity</span> Saving…`;let i;try{i=await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e.guid)}`,{method:`PUT`,headers:{"Content-Type":`application/json`},body:JSON.stringify({answers:t})});let r=await i.json().catch(()=>({}));if(!i.ok||!r.ok)throw Error(r.error||r.message||`Failed (HTTP ${i.status})`);sy(`Answers updated`,`success`);let a=await(await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e.guid)}`)).json().catch(()=>({}));d(a.ok?a.data:{...e,answers:t},!1)}catch(t){if(i&&i.status===423){sy(t.message||`This questionnaire is now locked.`,`info`);let r=await(await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e.guid)}`)).json().catch(()=>({}));d(r.ok?r.data:e,!1);return}s.querySelector(`#qm-error`).textContent=t.message||`Something went wrong. Please try again.`,l.disabled=!1,l.innerHTML=r}})}try{let t=await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse?owner_guid=${encodeURIComponent(e)}`),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`HTTP ${t.status}`);let i=(r.data||[])[0]||null;if(!i){c();return}if(i.status===`DRAFT`){l(i);return}if(i.status===`CANCELLED`){u(i);return}let a=await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(i.guid)}`),o=await a.json().catch(()=>({}));if(!a.ok||!o.ok)throw Error(o.error||`HTTP ${a.status}`);d(o.data)}catch(e){console.warn(`[clients] questionnaire load failed:`,e),s.innerHTML=`<p class="client-modal__error">Could not load questionnaire status. ${X(e.message||``)}</p>`}}var xy,Sy,Cy,wy,Ty,X,Ey,Dy,Oy,ky=o((()=>{w(),et(),xy=u(f(),1),m(),sn(),hr(),Sy=`reseller_org`,Cy=`managed_client`,wy=!1,Ty={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},X=e=>String(e??``).replace(/[&<>"']/g,e=>Ty[e]),Ey=e=>String(e||`?`).trim().split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``).toUpperCase()||`?`,Dy=e=>String(e||``).replace(/[^0-9]/g,``),Oy={render:py,init:my}})),Ay=c({default:()=>Yy,init:()=>zy,loadTeam:()=>By,render:()=>Ry});function jy(){let e=C.getActiveOrg(),t=e&&e.channel!==Uy?e:null;if(!t){let e=C.getMemberships().find(e=>e.channel!==Uy);e&&(t={guid:e.orgGuid,channel:e.channel,memberRole:e.memberRole})}if(!t)return null;let n=C.getMemberships().find(e=>e.orgGuid===t.guid)?.name;return{...t,name:n}}function My(e){return Object.values(Re).filter(t=>Le(e,t.value)).sort((e,t)=>t.rank-e.rank)}function Ny(){if(Wy)return;let e=document.createElement(`style`);e.id=`m5t-team-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Wy=!0}function Py(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function Fy(e,t,n){let r=e.member_role||`member`,i=e.entity_guid&&e.entity_guid===C.getEntity?.()?.guid,a=n&&!i&&My(t.memberRole).some(e=>e.value===r);return`
    <div class="team-row">
      <span class="team-avatar">${Ky(Jy(e.name||e.email))}</span>
      <span class="team-main">
        <div class="team-name">${Ky(e.name||e.email)}${e.verified?``:` · <span style="color:var(--color-text-subtle,#6C7C9A);font-weight:400;">pending verification</span>`}</div>
        <div class="team-email" title="${Ky(e.email)}">${Ky(e.email)}</div>
      </span>
      <span class="team-role" data-role="${Ky(r)}">${Ky(Re[r]?.label||r)}</span>
      ${a?`<button class="team-modal__btn team-modal__btn--ghost team-remove" type="button" data-id="${Ky(e.id)}" data-name="${Ky(e.name||e.email)}">Remove</button>`:``}
    </div>`}async function Iy(e,t){let{uid:n,email:r}=e.dataset;if(window.confirm(`Revoke the invitation for ${r}?\n\nThe invitation link stops working immediately, including any copy already sent. You can invite this address again afterwards.`)){e.disabled=!0;try{let e=C.getConfig?.().BASE_URL||``,i=await fetch(`${e}/m5t/v5/acctGroup/${encodeURIComponent(t.guid)}/invites/${encodeURIComponent(n)}`,{method:`DELETE`}),a=await i.json().catch(()=>({}));if(!i.ok||!a.ok)throw Error(a.message||a.error||`HTTP ${i.status}`);qy(`Invitation for ${r} revoked`,`success`)}catch(t){e.disabled=!1,qy(t.message||`Could not revoke that invitation.`,`error`);return}try{await By(t)}catch{}}}async function Ly(e,t){let{id:n,name:r}=e.dataset;if(window.confirm(`Remove ${r} from the team?\n\nThey lose access to this workspace immediately. Their account itself is not deleted, and you can invite them again later.`)){e.disabled=!0;try{let e=C.getConfig?.().BASE_URL||``,t=await fetch(`${e}/m5t/v5/acctGroupMember/${encodeURIComponent(n)}`,{method:`DELETE`}),i=await t.json().catch(()=>({}));if(!t.ok||!i.ok)throw Error(i.message||i.error||`HTTP ${t.status}`);qy(`${r} removed from the team`,`success`)}catch(t){e.disabled=!1,qy(t.message||`Could not remove that member.`,`error`);return}try{await By(t)}catch{}}}function Ry(){return Ny(),`
    <div class="page-shell" data-section="team">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="team-body">
          ${Py(`hourglass_empty`,`Loading team…`)}
        </div>
      </div>
    </div>`}async function zy(){let e=jy(),t=document.getElementById(`team-invite-btn`);if(!e){t&&(t.style.display=`none`),document.getElementById(`team-body`).innerHTML=Py(`info`,`Team management is available for agency and workspace accounts.`);return}t?.addEventListener(`click`,()=>Vy(e)),await By(e)}async function By(e){let t=document.getElementById(`team-body`),n=document.getElementById(`team-count`);if(t)try{let r=C.getConfig?.().BASE_URL||``,[i,a]=await Promise.all([fetch(`${r}/m5t/v5/acctGroup/${encodeURIComponent(e.guid)}/members`),fetch(`${r}/m5t/v5/acctGroup/${encodeURIComponent(e.guid)}/invites`)]),[o,s]=await Promise.all([i.json().catch(()=>({})),a.json().catch(()=>({}))]);if(!i.ok||!a.ok)throw Error(`Failed to load team (members: ${i.status}, invites: ${a.status})`);let c=o.members||[],l=s.invites||[],u=My(e.memberRole).length>0;t.innerHTML=(l.length?`
      <div class="team-pending">
        <div class="team-pending__label">Pending invites</div>
        ${l.map(e=>`
          <div class="team-pending__row">
            <span class="team-pending__email" title="${Ky(e.email)}">${Ky(e.email)}${e.expired?` · expired`:``}</span>
            <button class="team-modal__btn team-modal__btn--ghost team-copy" type="button" data-url="${Ky(e.inviteUrl)}">Copy link</button>
            ${u?`<button class="team-modal__btn team-modal__btn--ghost team-revoke" type="button" data-uid="${Ky(e.uid)}" data-email="${Ky(e.email)}">Revoke</button>`:``}
          </div>`).join(``)}
      </div>`:``)+(c.length?`<div class="team-list">${c.map(t=>Fy(t,e,u)).join(``)}</div>`:Py(`group`,`No members yet.`)),n&&(n.textContent=`${c.length} member${c.length===1?``:`s`}`),t.querySelectorAll(`.team-copy`).forEach(e=>e.addEventListener(`click`,async()=>{try{await navigator.clipboard.writeText(e.dataset.url),e.textContent=`Copied!`,setTimeout(()=>{e.textContent=`Copy link`},1500)}catch{}})),t.querySelectorAll(`.team-revoke`).forEach(t=>t.addEventListener(`click`,()=>Iy(t,e))),t.querySelectorAll(`.team-remove`).forEach(t=>t.addEventListener(`click`,()=>Ly(t,e)))}catch(e){console.warn(`[team] load failed:`,e),t.innerHTML=Py(`error`,`Couldn’t load the team. Please try again.`)}}function Vy(e){let t=My(e.memberRole);if(!t.length){qy(`You don’t have permission to invite teammates.`,`error`);return}let n=document.createElement(`div`);n.className=`team-modal-scrim`;let r=document.createElement(`div`);r.className=`team-modal`,r.setAttribute(`role`,`dialog`),r.setAttribute(`aria-modal`,`true`),r.innerHTML=`
    ...
    <div class="team-modal__body" id="team-invite-body">
      <p class="team-modal__sub">They'll get a link to set a password and join <strong>${Ky(e.name||`your workspace`)}</strong>.</p>
      ...
      <select class="team-modal__select" id="tm-role">
        ${t.map(e=>`<option value="${e.value}"${e.value===`member`?` selected`:``}>${Ky(e.label)}</option>`).join(``)}
      </select>
      ...
      <div class="team-modal__actions">
        ...
        </div>`;let u=r.querySelector(`#tm-link`);r.querySelector(`#tm-copy`)?.addEventListener(`click`,async()=>{try{await navigator.clipboard.writeText(u.value),r.querySelector(`#tm-copy`).textContent=`Copied!`}catch{u.select()}}),r.querySelector(`[data-act="done"]`)?.addEventListener(`click`,()=>{i(),By(e)})}catch(e){console.warn(`[team] invite failed:`,e),o.textContent=e.message||`Something went wrong. Please try again.`,s.disabled=!1,s.innerHTML=l}})}var Hy,Uy,Wy,Gy,Ky,qy,Jy,Yy,Xy=o((()=>{w(),Hy=u(f(),1),m(),et(),Uy=`managed_client`,Wy=!1,Gy={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Ky=e=>String(e??``).replace(/[&<>"']/g,e=>Gy[e]),qy=(e,t=`info`)=>{try{Hy.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},Jy=e=>String(e||`?`).trim().split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``).toUpperCase()||`?`,Yy={render:Ry,init:zy}}));m(),Ee(),w(),Wt(),y(),An(),Ln(),it(),window.m5t=Object.create(null),window.m5t.pubsub=b.default,console.log(`StreamSyncEngage SaaS Client - Starting...`),console.log(`Environment:`,`production`),console.log(`Document readyState:`,document.readyState),console.log(`window.mvvLegit:`,typeof window.mvvLegit),console.log(`globalThis.mvvLegit:`,typeof globalThis.mvvLegit);try{console.log(`mvvLegit (direct):`,typeof mvvLegit)}catch(e){console.log(`mvvLegit (direct): ReferenceError -`,e.message)}var Zy=Object.keys(window).filter(e=>e.toLowerCase().includes(`mvv`)||e.toLowerCase().includes(`neodigm`)||e.toLowerCase().includes(`legit`));console.log(`Possible Neodigm globals:`,Zy);var Z=new ee.default(`/`,{hash:!0});window.router=Z;function Q(e){let t=e?`${e.replace(/-/g,`_`)}_route`:`splash_route`;return console.log(`[Router] Checking access: ${e} -> ${t}`),mvvLegit.isRouteAllowed(t)?!0:(console.warn(`[Router] Access denied to route: ${t}`),b.default.publish(p.UI_TOAST,{message:`Please sign in to access this page`,type:`warning`}),!1)}function $(e){console.log(`[Router] Navigating to:`,e),Z.navigate(`/${e}`)}var Qy=new Set([`splash`,`signin`,`signup`,`forgot`,`resethash`,`verf-link`,`accept-invite`,`signout`]),$y={},eb={},tb=null,nb={admin:{caption:`Platform Admin`,icon:`shield_person`},reseller:{caption:`Reseller`,icon:`handshake`},managed:{caption:`Managed`,icon:`visibility`},self_serve:{caption:`Self-Serve`,icon:`rocket_launch`}},rb=`__portfolio__`;function ib(){try{Z._setCurrent(null)}catch{}Z.resolve()}function ab(){let e=document.getElementById(`m5t-side-nav`);if(!e)return;let t=!C.getScope()&&C.isAuthenticated(),n=C.getCapabilities(),r=C.getUiMode(),i=Yt.getSideNav(n,{uiMode:r});$y={},eb={};let a=e=>{e.route&&($y[e.route]={sectionToken:e.token,childToken:null,children:e.children||null},Array.isArray(e.children)&&e.children.forEach(t=>{t.route&&(eb[t.token]=t.route,$y[t.route]={sectionToken:e.token,childToken:t.token,children:e.children})}))},o=e=>e.forEach(e=>{e.group&&Array.isArray(e.items)?o(e.items):a(e)});o(i);let s=C.getMemberships(),c=C.getActiveOrg(),l=C.getManagedClients(),u=C.getStoredActiveOrg(),d=l.length?[{guid:rb,name:`Portfolio (all clients)`},...s.map(e=>({guid:e.orgGuid,name:e.name||e.channel,channel:e.channel,group:`Your organization`})),...l.map(e=>({guid:e.orgGuid,name:e.name||e.orgGuid,channel:e.channel,group:`Clients`}))]:s.map(e=>({guid:e.orgGuid,name:e.name||e.channel,channel:e.channel}));e.setState({buttons:i,orgs:d,activeOrg:l.length?u||rb:c?.guid||null,footerBadge:nb[r]||null,loading:t}),sb(),tb&&cb(tb)}var ob=e=>String(e??``).replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`})[e]);function sb(){let e=document.getElementById(`app-content`);if(!e)return;let t=document.getElementById(`org-context-banner`);t||(t=document.createElement(`div`),t.id=`org-context-banner`,e.insertBefore(t,e.firstChild),xn(t));let n=C.getActiveOrg(),r=C.isPreviewingAsClient(),i=!!n?.viaSubtree&&!!tb&&!Qy.has(tb);if(t.hidden=!i,!i){t.innerHTML=``;return}let a=ob(n.name||`Client`);t.innerHTML=`
    <style>
