...
`)],{type:`text/csv`}),a=URL.createObjectURL(i),o=document.createElement(`a`);o.href=a,o.download=`leads-${J.filter.toLowerCase()}.csv`,document.body.appendChild(o),o.click(),o.remove(),URL.revokeObjectURL(a),dg(`Exported ${e.length} lead${e.length===1?``:`s`}`,`success`)}var og,sg,J,cg,lg,ug,Y,dg,fg,pg,mg,hg,gg,_g,vg,yg,bg,xg,Sg,Cg,wg,Tg,Eg=o((()=>{E(),og=u(f(),1),m(),ga(),Sa(),rn(),sg=!1,J={leads:[],magnetNames:{},filter:`ALL`,canManage:!1,book:!1,ownerNames:{},clientFilter:`ALL`,search:``,shown:25,selectedId:null,day:``,sort:{key:`captured`,dir:`desc`},magnetFilter:`ALL`,range:`ALL`,pageSize:25,total:0,truncated:!1},cg=()=>J.pageSize===`ALL`?2**53-1:Number(J.pageSize)||25,lg=[`NEW`,`WORKING`,`QUALIFIED`,`UNQUALIFIED`,`CONVERTED`,`EXPORTED`],ug={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Y=e=>String(e??``).replace(/[&<>"']/g,e=>ug[e]),dg=(e,t=`info`)=>{try{og.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},fg=()=>T.getConfig?.().BASE_URL||``,pg=e=>[e.first_name,e.last_name].filter(Boolean).join(` `).trim()||e.email||`(no name)`,mg=e=>(pg(e).split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``)||`?`).toUpperCase(),hg=e=>e.created?.created_tz||``,gg=e=>{let t=hg(e);if(!t)return!1;try{return Date.now()-new Date(t).getTime()<864e5}catch{return!1}},_g=e=>J.clientFilter===`ALL`||e.owner_guid===J.clientFilter,vg=e=>!J.day||hg(e).slice(0,10)===J.day,yg=e=>J.magnetFilter===`ALL`||e.magnet_guid===J.magnetFilter,bg={ALL:null,7:7,30:30,90:90},xg={ALL:`All time`,7:`Last 7 days`,30:`Last 30 days`,90:`Last 90 days`},Sg={name:{label:`Name`,dir:`asc`,value:e=>pg(e).toLowerCase()},contact:{label:`Contact`,dir:`asc`,value:e=>String(e.email||e.phone||``).toLowerCase()},captured:{label:`Captured`,dir:`desc`,value:e=>hg(e)}},Cg=()=>Lh(`touch_app`,`Click to see lead information.`),wg=()=>`
  ...
  </table></div>`}function Mg(){return j()?sa({section:`analytics`,title:`Analytics`,description:`Pick a client to see how their magnets are performing.`}):(Og(),`
  ...
  `,document.head.appendChild(e),u_=!0}function Yg(){return j()?sa({section:`ad-performance`,title:`Ad Performance`,description:`Pick a client to view their spend and lead performance.`}):(Jg(),`
  ...
  `,document.head.appendChild(e),P_=!0}function v_(){return j()?sa({section:`ad-accounts`,title:`Ad Accounts`,description:`Pick a client to see their connected ad accounts.`}):(__(),yc(),`
...
`&&a++,n.push(r),r=``,n.some(e=>e.trim()!==``)&&t.push(n),n=[]):r+=o}if(n.push(r),n.some(e=>e.trim()!==``)&&t.push(n),t.length<2)return[];let a=t[0].map(e=>e.trim().toLowerCase()),o=a.findIndex(e=>/e-?mail/.test(e)),s=a.findIndex(e=>/phone|mobile|tel/.test(e)),c=a.findIndex(e=>/date|time|booked|appointment|start/.test(e));return o<0&&s<0?[]:t.slice(1).map(e=>({email:o>=0?(e[o]||``).trim():``,phone:s>=0?(e[s]||``).trim():``,at:c>=0?(e[c]||``).trim():``})).filter(e=>e.email||e.phone).slice(0,5e3)}function j_(e,t){let n=new Blob([e],{type:`text/csv;charset=utf-8`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.href=r,i.download=t,document.body.appendChild(i),i.click(),i.remove(),setTimeout(()=>URL.revokeObjectURL(r),4e3)}var M_,N_,P_,F_,I_,L_,R_=o((()=>{ga(),rn(),Fc(),M_={google_ads:`Google Ads`,google_lsa:`Local Services Ads`},N_=e=>{let t=String(e||``).replace(/\D/g,``);return t.length===10?`${t.slice(0,3)}-${t.slice(3,6)}-${t.slice(6)}`:String(e||``)},P_=!1,F_=[],I_=null,L_={render:v_,init:y_}})),z_=c({default:()=>sv,init:()=>q_,openInviteModal:()=>Z_,render:()=>K_});function B_(e,t=`info`){try{ev.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}}function V_(){let e=T.getActiveOrg();if(e&&e.channel===tv)return{guid:e.guid,name:e.name};let t=T.getMemberships().find(e=>e.channel===tv);return t?{guid:t.orgGuid,name:t.name}:null}function H_(e){let t=e?.created_tz;if(!t)return``;let n=new Date(t);return Number.isNaN(n.getTime())?``:n.toLocaleDateString(void 0,{year:`numeric`,month:`short`,day:`numeric`})}function U_(){if(rv)return;let e=document.createElement(`style`);e.id=`m5t-clients-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),cr(),rv=!0}function W_(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function G_(e){let t=e.caption||e.guid,n=e.status||`ENABLED`;return`
    <div class="client-row">
      <span class="client-row__avatar">${X(av(t))}</span>
      <span class="client-row__main">
        <div class="client-row__name" title="${X(t)}">${X(t)}</div>
        <div class="client-row__meta">Added ${X(H_(e.created))||`—`}</div>
      </span>
    ...
    <div class="client-modal__body" id="qm-body">${W_(`hourglass_empty`,`Loading…`)}</div>`,document.body.appendChild(r),document.body.appendChild(i),requestAnimationFrame(()=>{r.classList.add(`open`),i.classList.add(`open`)});let a=()=>{r.classList.remove(`open`),i.classList.remove(`open`),document.removeEventListener(`keydown`,o),setTimeout(()=>{r.remove(),i.remove()},200)},o=e=>{e.key===`Escape`&&a()};document.addEventListener(`keydown`,o),r.addEventListener(`click`,a),i.querySelector(`.client-modal__close`)?.addEventListener(`click`,a);let s=i.querySelector(`#qm-body`);function c(){s.innerHTML=`
      ...
      <div class="qm-status" data-status="DRAFT"><span class="material-symbols-outlined" aria-hidden="true">hourglass_top</span> ${t} — awaiting response${e.token_expiry?`, link expires ${X(Q_(e.token_expiry))}`:``}</div>
      ...
      <div class="qm-status" data-status="CANCELLED"><span class="material-symbols-outlined" aria-hidden="true">block</span> Cancelled${e.cancelled_tz?` ${X(Q_(e.cancelled_tz))}`:``}${e.cancelled_by_email?` by ${X(e.cancelled_by_email)}`:``}</div>
      ...
      </div>`,s.querySelector(`[data-act="done"]`)?.addEventListener(`click`,a),s.querySelector(`[data-act="resend"]`)?.addEventListener(`click`,()=>c())}function d(e,t=!1){let r=e.editability||{editable:e.status===`SUBMITTED`&&!e.locked_tz,reason:e.locked_tz?`finalized`:null,deadline:null},i=!!r.editable,o=!i&&e.status===`SUBMITTED`?r.reason===`finalized`?`<div class="qm-status" data-status="LOCKED"><span class="material-symbols-outlined" aria-hidden="true">lock</span> Finalized${e.locked_tz?` ${X(Q_(e.locked_tz))}`:``}${e.locked_by_email?` by ${X(e.locked_by_email)}`:``} — no further edits.</div>`:`<div class="qm-status" data-status="LOCKED"><span class="material-symbols-outlined" aria-hidden="true">lock_clock</span> Editing closed${r.deadline?` ${X(Q_(r.deadline))}`:``} — a response locks 14 days after it is submitted.</div>`:``,c=i&&r.deadline?`<p class="client-modal__sub qm-editable-hint">You can edit these answers until ${X(Q_(r.deadline))}.</p>`:``;s.innerHTML=`
      <div class="qm-status" data-status="SUBMITTED"><span class="material-symbols-outlined" aria-hidden="true">task_alt</span> Submitted ${e.submitted_tz?X(Q_(e.submitted_tz)):``}</div>
      ${o}
      ${sr(e)}
      <form id="qm-form">
  ...
  `,document.head.appendChild(e),Cv=!0}function pv(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function mv(e,t,n){let r=e.member_role||`member`,i=e.entity_guid&&e.entity_guid===T.getEntity?.()?.guid,a=n&&!i&&dv(t.memberRole).some(e=>e.value===r);return`
    <div class="team-row">
      <span class="team-avatar">${Tv(Dv(e.name||e.email))}</span>
      <span class="team-main">
        <div class="team-name">${Tv(e.name||e.email)}${e.verified?``:` · <span style="color:var(--color-text-subtle,#6C7C9A);font-weight:400;">pending verification</span>`}</div>
        <div class="team-email" title="${Tv(e.email)}">${Tv(e.email)}</div>
      </span>
      <span class="team-role" data-role="${Tv(r)}">${Tv(Fe[r]?.label||r)}</span>
      ${a?`<button class="team-modal__btn team-modal__btn--ghost team-remove" type="button" data-id="${Tv(e.id)}" data-name="${Tv(e.name||e.email)}">Remove</button>`:``}
    </div>`}async function hv(e,t){let{uid:n,email:r}=e.dataset;if(window.confirm(`Revoke the invitation for ${r}?\n\nThe invitation link stops working immediately, including any copy already sent. You can invite this address again afterwards.`)){e.disabled=!0;try{let e=T.getConfig?.().BASE_URL||``,i=await fetch(`${e}/m5t/v5/acctGroup/${encodeURIComponent(t.guid)}/invites/${encodeURIComponent(n)}`,{method:`DELETE`}),a=await i.json().catch(()=>({}));if(!i.ok||!a.ok)throw Error(a.message||a.error||`HTTP ${i.status}`);Ev(`Invitation for ${r} revoked`,`success`)}catch(t){e.disabled=!1,Ev(t.message||`Could not revoke that invitation.`,`error`);return}try{await yv(t)}catch{}}}async function gv(e,t){let{id:n,name:r}=e.dataset;if(window.confirm(`Remove ${r} from the team?\n\nThey lose access to this workspace immediately. Their account itself is not deleted, and you can invite them again later.`)){e.disabled=!0;try{let e=T.getConfig?.().BASE_URL||``,t=await fetch(`${e}/m5t/v5/acctGroupMember/${encodeURIComponent(n)}`,{method:`DELETE`}),i=await t.json().catch(()=>({}));if(!t.ok||!i.ok)throw Error(i.message||i.error||`HTTP ${t.status}`);Ev(`${r} removed from the team`,`success`)}catch(t){e.disabled=!1,Ev(t.message||`Could not remove that member.`,`error`);return}try{await yv(t)}catch{}}}function _v(){return fv(),`
    ...
    <div class="team-modal__body" id="team-invite-body">
      <p class="team-modal__sub">They'll get a link to set a password and join <strong>${Tv(e.name||`your workspace`)}</strong>.</p>
      ...
      <select class="team-modal__select" id="tm-role">
        ${t.map(e=>`<option value="${e.value}"${e.value===`member`?` selected`:``}>${Tv(e.label)}</option>`).join(``)}
      </select>
      ...
      <div class="team-modal__actions">
        ...
        </div>`;let u=r.querySelector(`#tm-link`);r.querySelector(`#tm-copy`)?.addEventListener(`click`,async()=>{try{await navigator.clipboard.writeText(u.value),r.querySelector(`#tm-copy`).textContent=`Copied!`}catch{u.select()}}),r.querySelector(`[data-act="done"]`)?.addEventListener(`click`,()=>{i(),yv(e)})}catch(e){console.warn(`[team] invite failed:`,e),o.textContent=e.message||`Something went wrong. Please try again.`,s.disabled=!1,s.innerHTML=l}})}var xv,Sv,Cv,wv,Tv,Ev,Dv,Ov,kv=o((()=>{E(),xv=u(f(),1),m(),Ze(),Sv=`managed_client`,Cv=!1,wv={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Tv=e=>String(e??``).replace(/[&<>"']/g,e=>wv[e]),Ev=(e,t=`info`)=>{try{xv.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},Dv=e=>String(e||`?`).trim().split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``).toUpperCase()||`?`,Ov={render:_v,init:vv}}));m(),Ce(),E(),Bt(),y(),Dn(),Pn(),tt(),window.m5t=Object.create(null),window.m5t.pubsub=b.default,console.log(`StreamSyncEngage SaaS Client - Starting...`),console.log(`Environment:`,`production`),console.log(`Document readyState:`,document.readyState),console.log(`window.mvvLegit:`,typeof window.mvvLegit),console.log(`globalThis.mvvLegit:`,typeof globalThis.mvvLegit);try{console.log(`mvvLegit (direct):`,typeof mvvLegit)}catch(e){console.log(`mvvLegit (direct): ReferenceError -`,e.message)}var Av=Object.keys(window).filter(e=>e.toLowerCase().includes(`mvv`)||e.toLowerCase().includes(`neodigm`)||e.toLowerCase().includes(`legit`));console.log(`Possible Neodigm globals:`,Av);var Z=new ee.default(`/`,{hash:!0});window.router=Z;function Q(e){let t=e?`${e.replace(/-/g,`_`)}_route`:`splash_route`;return console.log(`[Router] Checking access: ${e} -> ${t}`),mvvLegit.isRouteAllowed(t)?!0:(console.warn(`[Router] Access denied to route: ${t}`),b.default.publish(p.UI_TOAST,{message:`Please sign in to access this page`,type:`warning`}),!1)}function $(e){console.log(`[Router] Navigating to:`,e),Z.navigate(`/${e}`)}var jv=new Set([`splash`,`signin`,`signup`,`forgot`,`resethash`,`verf-link`,`accept-invite`,`signout`]),Mv={},Nv={},Pv=null,Fv={admin:{caption:`Platform Admin`,icon:`shield_person`},reseller:{caption:`Reseller`,icon:`handshake`},managed:{caption:`Managed`,icon:`visibility`},self_serve:{caption:`Self-Serve`,icon:`rocket_launch`}},Iv=`__portfolio__`;function Lv(){try{Z._setCurrent(null)}catch{}Z.resolve()}function Rv(){let e=document.getElementById(`m5t-side-nav`);if(!e)return;let t=!T.getScope()&&T.isAuthenticated(),n=T.getCapabilities(),r=T.getUiMode(),i=Gt.getSideNav(n,{uiMode:r});Mv={},Nv={};let a=e=>{e.route&&(Mv[e.route]={sectionToken:e.token,childToken:null,children:e.children||null},Array.isArray(e.children)&&e.children.forEach(t=>{t.route&&(Nv[t.token]=t.route,Mv[t.route]={sectionToken:e.token,childToken:t.token,children:e.children})}))},o=e=>e.forEach(e=>{e.group&&Array.isArray(e.items)?o(e.items):a(e)});o(i);let s=T.getMemberships(),c=T.getActiveOrg(),l=T.getManagedClients(),u=T.getStoredActiveOrg(),d=l.length?[{guid:Iv,name:`Portfolio (all clients)`},...s.map(e=>({guid:e.orgGuid,name:e.name||e.channel,channel:e.channel,group:`Your organization`})),...l.map(e=>({guid:e.orgGuid,name:e.name||e.orgGuid,channel:e.channel,group:`Clients`}))]:s.map(e=>({guid:e.orgGuid,name:e.name||e.channel,channel:e.channel}));e.setState({buttons:i,orgs:d,activeOrg:l.length?u||Iv:c?.guid||null,footerBadge:Fv[r]||null,loading:t}),Bv(),Pv&&Vv(Pv)}var zv=e=>String(e??``).replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`})[e]);function Bv(){let e=document.getElementById(`app-content`);if(!e)return;let t=document.getElementById(`org-context-banner`);t||(t=document.createElement(`div`),t.id=`org-context-banner`,e.insertBefore(t,e.firstChild),vn(t));let n=T.getActiveOrg(),r=T.isPreviewingAsClient(),i=!!n?.viaSubtree&&!!Pv&&!jv.has(Pv);if(t.hidden=!i,!i){t.innerHTML=``;return}let a=zv(n.name||`Client`);t.innerHTML=`
    <style>
