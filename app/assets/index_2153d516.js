...
`,t)}function pr(){if(!oa())return va.stubThis(new Promise(t=>t(!1)));const e=globalThis.PublicKeyCredential;return e?.isConditionalMediationAvailable===void 0?va.stubThis(new Promise(t=>t(!1))):va.stubThis(e.isConditionalMediationAvailable())}const va={stubThis:e=>e};function Xc({error:e,options:t}){const{publicKey:a}=t;if(!a)throw Error("options was missing required publicKey property");if(e.name==="AbortError"){if(t.signal instanceof AbortSignal)return new he({message:"Authentication ceremony was sent an abort signal",code:"ERROR_CEREMONY_ABORTED",cause:e})}else{if(e.name==="NotAllowedError")return new he({message:e.message,code:"ERROR_PASSTHROUGH_SEE_CAUSE_PROPERTY",cause:e});if(e.name==="SecurityError"){const n=globalThis.location.hostname;if(cr(n)){if(a.rpId!==n)return new he({message:`The RP ID "${a.rpId}" is invalid for this domain`,code:"ERROR_INVALID_RP_ID",cause:e})}else return new he({message:`${globalThis.location.hostname} is an invalid domain`,code:"ERROR_INVALID_DOMAIN",cause:e})}else if(e.name==="UnknownError")return new he({message:"The authenticator was unable to process the specified options, or could not create a new assertion signature",code:"ERROR_AUTHENTICATOR_GENERAL_ERROR",cause:e})}return e}async function Zc(e){!e.optionsJSON&&e.challenge&&(console.warn("startAuthentication() was not called correctly. It will try to continue with the provided options, but this call should be refactored to use the expected call structure instead. See https://simplewebauthn.dev/docs/packages/browser#typeerror-cannot-read-properties-of-undefined-reading-challenge for more information."),e={optionsJSON:e});const{optionsJSON:t,useBrowserAutofill:a=!1,verifyBrowserAutofillInput:n=!0}=e;if(!oa())throw new Error("WebAuthn is not supported in this browser");let o;t.allowCredentials?.length!==0&&(o=t.allowCredentials?.map(lr));const s={...t,challenge:Xt(t.challenge),allowCredentials:o},r={};if(a){if(!await pr())throw Error("Browser does not support WebAuthn autofill");if(document.querySelectorAll("input[autocomplete$='webauthn']").length<1&&n)throw Error('No <input> with "webauthn" as the only or last value in its `autocomplete` attribute was detected');r.mediation="conditional",s.allowCredentials=[]}r.publicKey=s,r.signal=Wn.createNewAbortSignal();let i;try{i=await navigator.credentials.get(r)}catch(m){throw Xc({error:m,options:r})}if(!i)throw new Error("Authentication was not completed");const{id:l,rawId:c,response:g,type:b}=i;let d;return g.userHandle&&(d=Pe(g.userHandle)),{id:l,rawId:Pe(c),response:{authenticatorData:Pe(g.authenticatorData),clientDataJSON:Pe(g.clientDataJSON),signature:Pe(g.signature),userHandle:d},type:b,clientExtensionResults:i.getClientExtensionResults(),authenticatorAttachment:dr(i.authenticatorAttachment)}}function ed(){return oa()?PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable():new Promise(e=>e(!1))}const ur=Object.freeze(Object.defineProperty({__proto__:null,WebAuthnAbortService:Wn,WebAuthnError:he,_browserSupportsWebAuthnAutofillInternals:va,_browserSupportsWebAuthnInternals:ir,base64URLStringToBuffer:Xt,browserSupportsWebAuthn:oa,browserSupportsWebAuthnAutofill:pr,bufferToBase64URLString:Pe,platformAuthenticatorIsAvailable:ed,startAuthentication:Zc,startRegistration:Qc},Symbol.toStringTag,{value:"Module"}));let To=!1;function td(){if(To)return;const e=document.createElement("style");e.id="m5t-accept-invite-styles",e.textContent=`
  ...
  <div class="dashboard-card${n?" chart-card-wide":""}${o?" chart-card-full":""}">
    ...
    <div class="page-shell" data-section="home">
      ...
      <div class="content-grid">
    ...
    </div>`}async function Cr(){const e=E.getConfig?.().BASE_URL||"";document.getElementById("dash-periods")?.addEventListener("click",t=>{const a=t.target.closest("button[data-days]");a&&(kn=Number(a.dataset.days)||30,document.querySelectorAll("#dash-periods .chart-period-btn").forEach(n=>n.classList.toggle("active",Number(n.dataset.days)===kn)),Do(e))}),document.getElementById("dash-campaigns")?.addEventListener("click",t=>{const a=t.target.closest("[data-campaign]");a&&Zn(bt.LEADS_SEARCH,a.dataset.campaign)}),await Promise.all([Do(e),dp(e)])}const De=(e,t)=>{const a=document.getElementById(e);a&&(a.textContent=t)},Ct=(e,t)=>{const a=document.getElementById(e);a&&(a.innerHTML=t)},Fo=e=>e.toISOString().slice(0,10),ip=()=>typeof window.matchMedia=="function"?window.matchMedia("(hover: hover)").matches:!0;async function Do(e){const t=new Date,a=Fo(new Date(t.getTime()-(kn-1)*864e5)),n=Fo(t),o=re();let s;try{const C=await fetch(`${e}/m5t/v5/dashboard/stats?from=${a}&to=${n}${o?"&group_by=owner":""}`);if(!C.ok)throw new Error(`HTTP ${C.status}`);if(s=await C.json(),!s.ok)throw new Error("stats not ok")}catch(y){console.warn("[dashboard] stats fetch failed:",y),["stat-active-magnets","stat-opens","stat-leads","stat-completion"].forEach(N=>De(N,"—")),["stat-active-magnets-sub","stat-opens-sub","stat-leads-sub","stat-completion-sub"].forEach(N=>De(N,"Unavailable"));const C=document.getElementById("stat-leads-trend");C&&(C.className="stat-trend",C.innerHTML=""),["dash-trend","dash-funnel","dash-campaigns","dash-routes","dash-interactions","dash-portfolio"].forEach(N=>Ct(N,ke("error","Could not load. Please refresh.")));return}o&&lp(s.by_owner||[]),De("stat-active-magnets",ee(s.magnets.active)),De("stat-active-magnets-sub",s.magnets.total>s.magnets.active?`${ee(s.magnets.total-s.magnets.active)} paused`:s.magnets.active?"All running":"No magnets yet");const r=s.funnel||{};De("stat-opens",r.opens?ee(r.opens):"—"),De("stat-opens-sub",r.opens?r.engagement_rate!=null?`${r.engagement_rate}% engage`:`${ee(r.engaged)} engaged`:"No opens in this range yet");const i=s.leads||{};De("stat-leads",ee(i.total));const l=(i.this_week||0)-(i.prev_week||0);De("stat-leads-sub",i.total?`${ee(i.this_week)} this week`:"No leads yet");const c=document.getElementById("stat-leads-trend");if(c)if((i.prev_week||0)>0){const y=l>0?"positive":l<0?"negative":"neutral",C=l>0?"trending_up":l<0?"trending_down":"trending_flat";c.className=`stat-trend ${y}`,c.innerHTML=`<span class="material-symbols-outlined" aria-hidden="true">${C}</span>${l>0?"+":""}${ee(l)}`}else c.className="stat-trend",c.innerHTML="";De("stat-completion",r.completion_rate!=null?`${r.completion_rate}%`:"—"),De("stat-completion-sub",r.opens?`${ee(r.completions)} completed of ${ee(r.opens)} opens`:"Appears once magnets get opened");const g=s.by_day||[],b=g.some(y=>y.opens||y.leads);Ct("dash-trend",b?Tr(g):ke("show_chart","No activity in this range yet. Opens and leads chart here as visitors engage your magnets.")),b&&$r(g),Ct("dash-funnel",r.opens?cp(r):ke("filter_alt","The funnel fills in once your magnets record opens."));const d=s.top_campaigns||[];Ct("dash-campaigns",d.length?`<div class="dash-list">${d.map(y=>`
        ...
        </a>`).join("")}</div>`:ke("source","No tagged campaigns yet. Leads arriving with ?utm_campaign=… rank here."));const m={book:["event","Book"],chat:["chat","Chat"],call:["phone_in_talk","Call"],email:["mail","Email"],support:["support_agent","Support"]},f=Object.entries(r.routes||{}).sort((y,C)=>C[1]-y[1]),p=f.length?f[0][1]:0;Ct("dash-routes",f.length?`<div class="dash-funnel">${f.map(([y,C])=>{const[N,q]=m[y]||["touch_app",y];return`<div class="dash-funnel__row">
          <div class="dash-funnel__meta"><span class="dash-funnel__label"><span class="material-symbols-outlined" aria-hidden="true" style="font-size:15px;vertical-align:-3px">${N}</span> ${be(q)}</span><span class="dash-funnel__val">${ee(C)}</span></div>
          <div class="dash-funnel__track"><div class="dash-funnel__bar" style="width:${p?Math.max(4,Math.round(C/p*100)):0}%"></div></div>
        </div>`}).join("")}</div>`:ke("touch_app","CTA taps (Book, Chat, Call, Support) rank here once visitors use the action bar."));const v={open:["visibility","Magnet opened"],engage:["touch_app","Visitor engaged"],route:["alt_route","Picked a channel"],complete:["check_circle","Completed: lead captured"]},w=s.recent_events||[];Ct("dash-interactions",w.length?`<div class="dash-list">${w.map(y=>{const[C,N]=v[y.event]||["sensors",y.event],q=y.event==="route"&&y.detail?` · ${be(y.detail)}`:"";return`<div class="dash-list__row">
          <span class="dash-list__ico"><span class="material-symbols-outlined" aria-hidden="true">${C}</span></span>
    ...
    <div class="mag-modal" role="dialog" aria-modal="true" aria-label="${o?"Edit magnet":"New magnet"}">
      ...
      <div class="mag-modal__main">
        ...
        <select class="mag-modal__select" id="mag-f-owner">
          ${V.owners.map(S=>`<option value="${x(S.guid)}"${S.guid===V.primaryOrgGuid?" selected":""}>${x(S.name||S.guid)}</option>`).join("")}
        </select>`:""}
        ...
        <select class="mag-modal__select" id="mag-f-trigger">
          ${Oi.map(S=>`<option value="${S.value}"${(e?e.trigger||"none":"on_load")===S.value?" selected":""}>${x(S.label)}</option>`).join("")}
        </select>
