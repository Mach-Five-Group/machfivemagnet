...
`))}</textarea>`}return`<div class="mag-cta" data-type="${e}" data-kind="${n.kind}">
    ...
    ${o}
  </div>`}function ip(){if(cm)return;let e=document.createElement(`style`);e.id=`m5t-magnets-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),cm=!0}function ap(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function op(e){let t=V.stats[e];if(!t||!t.opens&&!t.leads)return`<div class="mag-statline mag-statline--empty">&mdash;</div><div class="mag-cellsub">No activity yet</div>`;let n=e=>Number(e||0).toLocaleString();return`<div class="mag-statline" title="Trailing 30 days"><b>${n(t.opens)}</b> opens &middot; <b>${n(t.leads)}</b> leads</div>
    ${t.completion_rate==null?``:`<div class="mag-cellsub">${t.completion_rate}% complete the conversation</div>`}`}function sp(e){let t=V.install[e.guid];if(!t)return``;let n=t.unrecognized||[],r=(t.traffic?.opens||0)>0,i=e.can_manage===!0,a=n.length?`
    ...
    ${s}
    ${l}
  </div>`}async function cp(){await Promise.all(V.magnets.map(async e=>{try{let t=await(await fetch(`${W()}/m5t/v5/magnets/${encodeURIComponent(e.guid)}/install-activity`)).json().catch(()=>({}));if(!t.ok)return;V.install[e.guid]=t,Ap(e)}catch(t){console.warn(`[magnets] install activity failed:`,e.guid,t)}}))}function lp(e){return e?e.status===`REVOKED`||e.status===`PENDING`&&e.codeLive===!1:!1}function up(e,t){let n=t.status===`PENDING`&&t.codeLive===!1,r=(n?wm.EXPIRED:wm[t.status])||{label:t.status,icon:`help`,tone:`off`},i=t.lastSeen?new Date(t.lastSeen).toLocaleString():``,a=t.host?H(t.host):n||t.status===`REVOKED`?`Code was never used`:`Waiting for a site to use this code`,o=[t.siteName?H(t.siteName):``,i?`last seen ${H(i)}`:``].filter(Boolean).join(` — `);return`
    <div class="mag-conn__row" data-conn="${H(t.guid)}" data-state="${r.tone}">
      ...
      ${e.can_manage===!0?lp(t)?`<button class="mag-btn" data-action="clear-connection" data-guid="${H(e.guid)}" data-conn="${H(t.guid)}"
    ...
    </div>`}function dp(e){let t=V.pairing[e.guid];if(!t)return``;if(t.error)return`<div class="mag-conn__code" data-state="error">
  ...
  </div>`}function fp(e){let t=V.connections[e.guid];if(!t)return``;if(t.failed)return`<div class="mag-conn">
    ...
    </div>`;let n=t.connections||[],r=n.filter(t=>t.status!==`PENDING`||e.can_manage===!0),i=n.filter(e=>e.status===`ACTIVE`).length,a=dp(e),o=r.filter(e=>!lp(e)),s=r.filter(e=>lp(e)),c=o.length?o.map(t=>up(e,t)).join(``):`<p class="mag-install__hint">No sites connected yet. On WordPress, connecting a site pairs it without pasting the snippet by hand.</p>`,l=s.length?`
  ...
  </div>`}function pp(e){let t=document.querySelector(`.mag-card[data-guid="${CSS.escape(e.guid)}"] .mag-conn-slot`);t&&(t.innerHTML=fp(e))}async function mp(e){let t=await(await fetch(`${W()}/m5t/v5/magnets/${encodeURIComponent(e.guid)}/connections`)).json().catch(()=>({}));if(!t.ok)throw Error(t.error||`Could not load connected sites`);return t}async function hp(){await Promise.all(V.magnets.map(async e=>{try{V.connections[e.guid]=await mp(e)}catch(t){console.warn(`[magnets] connections failed:`,e.guid,t),V.connections[e.guid]={failed:!0}}pp(e)}))}function gp(){V.pairTimer&&(clearInterval(V.pairTimer),V.pairTimer=null)}function _p(){gp()}function vp(e){return!!document.querySelector(`.mag-card[data-guid="${CSS.escape(e)}"] .mag-conn-slot`)}function yp(){if(V.pairTimer)return;let e=0;V.pairTimer=setInterval(async()=>{for(let e of Object.keys(V.pairing))vp(e)||delete V.pairing[e];let t=Object.keys(V.pairing);if(!t.length){gp();return}if(!t.some(e=>{let t=V.pairing[e];return!t.connected&&!t.error&&t.expiresAt-Date.now()>0})){t.forEach(e=>{let t=V.magnets.find(t=>t.guid===e);t&&pp(t)}),gp();return}e+=1e3;let n=e>=Cm;n&&(e=0);for(let e of t){let t=V.magnets.find(t=>t.guid===e),r=V.pairing[e];if(!t||!r)continue;let i=r.expiresAt-Date.now()<=0;if(n&&!r.connected&&!r.error&&!i)try{let n=await mp(t);V.connections[e]=n;let i=(n.connections||[]).find(e=>e.guid===r.connectionGuid&&e.status===`ACTIVE`);i&&(V.pairing[e]={connected:!0,connectionGuid:r.connectionGuid,connectedHost:i.host},U(`${i.host||`Your site`} connected`,`success`))}catch{}pp(t)}},1e3)}async function bp(e){try{let t=await fetch(`${W()}/m5t/v5/magnets/${encodeURIComponent(e.guid)}/pairing-code`,{method:`POST`,headers:{"Content-Type":`application/json`},body:`{}`}),n=await t.json().catch(()=>({}));if(!t.ok||!n.ok){V.pairing[e.guid]={error:n.message||n.error||`Could not generate a pairing code`},pp(e);return}V.pairing[e.guid]={code:n.pairingCode,connectionGuid:n.connectionGuid,expiresAt:new Date(n.expiresAt).getTime()};try{V.connections[e.guid]=await mp(e)}catch{}pp(e),yp()}catch{V.pairing[e.guid]={error:`Could not reach the server. Try again.`},pp(e)}}async function xp(e){let t=V.pairing[e.guid]?.code;if(t)try{await navigator.clipboard.writeText(t),U(`Pairing code copied`,`success`)}catch{U(`Copy failed — select the code and copy it manually`,`danger`)}}async function Sp(e,t,n){if(window.confirm(`Clear the record for ${n||`this connection`}?\n\nIt is removed from this list for good. This only tidies up the history — the site is already disconnected, so nothing about access changes.`))try{let n=await fetch(`${W()}/m5t/v5/magnets/${encodeURIComponent(e.guid)}/connections/${encodeURIComponent(t)}`,{method:`DELETE`}),r=await n.json().catch(()=>({}));if(!n.ok||!r.ok){U(r.message||r.error||`Could not clear that record`,`danger`);return}U(`Record cleared`,`success`);try{V.connections[e.guid]=await mp(e)}catch{}pp(e)}catch{U(`Could not reach the server`,`danger`)}}async function Cp(e,t){let n=(V.connections[e.guid]?.connections||[]).find(e=>e.guid===t),r=n?.status===`PENDING`,i=r?`Cancel this pairing code?`:`Disconnect ${n?.host||`this site`}?`,a=r?`The code stops working immediately.`:`Its key stops working immediately and the plugin will show as disconnected. The domain stays on the allow-list — remove it separately if you want the magnet off that site.`;if(window.confirm(`${i}\n\n${a}`))try{let n=await fetch(`${W()}/m5t/v5/magnets/${encodeURIComponent(e.guid)}/connections/${encodeURIComponent(t)}/revoke`,{method:`POST`,headers:{"Content-Type":`application/json`},body:`{}`}),i=await n.json().catch(()=>({}));if(!n.ok||!i.ok){U(i.message||i.error||`Could not disconnect`,`danger`);return}V.pairing[e.guid]?.connectionGuid===t&&delete V.pairing[e.guid],U(r?`Pairing code cancelled`:`Site disconnected`,`success`);try{V.connections[e.guid]=await mp(e)}catch{}pp(e)}catch{U(`Could not reach the server`,`danger`)}}function wp(e){if(e.status===`DRAFT`)return`draft`;if(e.status!==`ENABLED`)return`off`;let t=V.install[e.guid];return t?(t.traffic?.opens||0)>0?`live`:(t.unrecognized||[]).length?`seen`:`quiet`:`unknown`}function Tp(e){let t=wp(e),n=V.install[e.guid],r=``,i=``;if(t===`draft`)r=`Draft`,i=`Not live yet`;else if(t===`off`)r=`Disabled`,i=`Hidden from visitors`;else if(t===`live`){let e=n.traffic?.last_open_at?new Date(n.traffic.last_open_at):null,t=e&&e.toDateString()===new Date().toDateString(),a=e?t?e.toLocaleTimeString([],{hour:`numeric`,minute:`2-digit`}):e.toLocaleDateString():``;r=`Live`,i=a?`Last visitor ${H(a)}`:`Serving visitors`}else t===`seen`?(r=`Needs review`,i=`Unrecognised domain`):t===`quiet`&&(r=`Not seen`,i=`No visitors in ${Number(n.window_days||30)} days`);return r?`<div class="mag-status" data-live="${t}"><span class="mag-dot" data-live="${t}" aria-hidden="true"></span>${r}</div>
          ${i?`<div class="mag-cellsub">${i}</div>`:``}`:``}function Ep(e){let t=V.owners.length>1||C.getMemberships().length>1,n=(e.domains||[]).filter(Boolean),r=e.can_manage===!0,i=V.detailOpen[e.guid]===!0,a=[pm[e.display_mode]||`Chat bubble`,n.length?n.join(`, `):`No domains set. Add yours to go live.`,t&&e.owner_name?e.owner_name:``].filter(Boolean).join(` · `);return`
    <div class="mag-card${i?` is-open`:``}" data-guid="${H(e.guid)}">
      <div class="mag-line">
        ...
        <div class="mag-cell mag-cell--stats">${op(e.guid)}</div>
      ...
      <div class="mag-detail" id="mag-detail-${H(e.guid)}" ${i?``:`hidden`}>
        ...
        <div class="mag-embed">
          <input type="text" readonly value="${H(um(e.guid))}" aria-label="Embed snippet">
        ...
        <div class="mag-install-slot">${sp(e)}</div>
        <div class="mag-conn-slot">${fp(e)}</div>
      </div>
    </div>`}function Dp(e,t){let n=typeof t==`boolean`?t:V.detailOpen[e]!==!0;V.detailOpen[e]=n;let r=document.querySelector(`.mag-card[data-guid="${CSS.escape(e)}"]`);if(!r)return;r.classList.toggle(`is-open`,n);let i=r.querySelector(`.mag-detail`);i&&(i.hidden=!n);let a=r.querySelector(`[data-action="install-toggle"][aria-controls]`);if(a){a.setAttribute(`aria-expanded`,String(n)),a.classList.toggle(`is-open`,n);let e=a.querySelector(`.material-symbols-outlined`);e&&(e.textContent=n?`expand_less`:`expand_more`)}}function Op(){Tm||(Tm=!0,document.addEventListener(`click`,e=>{e.target.closest?.(`.mag-more-wrap`)||kp()}),document.addEventListener(`keydown`,e=>{e.key===`Escape`&&kp()}))}function kp(e){document.querySelectorAll(`.mag-menu`).forEach(t=>{if(t===e)return;t.hidden=!0;let n=t.parentElement?.querySelector(`[data-action="menu"]`);n&&n.setAttribute(`aria-expanded`,`false`)})}function Ap(e){let t=document.querySelector(`.mag-card[data-guid="${CSS.escape(e.guid)}"]`);if(!t)return;let n=t.querySelector(`.mag-install-slot`);n&&(n.innerHTML=sp(e));let r=wp(e),i=t.querySelector(`.mag-cell--status`);i&&(i.innerHTML=Tp(e)),(r===`quiet`||r===`seen`)&&V.detailOpen[e.guid]===void 0&&Dp(e.guid,!0)}function jp(){return A()?ua({section:`magnets`,title:`Your Magnets`,description:`Pick a client to view and manage their magnets.`}):(ip(),`
    <div class="page-shell" data-section="magnets">
      ...
      <div class="dashboard-card">
        <div class="card-body" id="mag-body">
          ${ap(`hourglass_empty`,`Loading magnets…`)}
        </div>
      </div>
    </div>`)}async function Mp(){if(A()){let e=Df();if(e){let t=document.querySelector(`.page-shell .page-header-text .page-description`);t&&(t.textContent=`Template picked: ${e.name}. Choose the client to build it for.`)}let t=e=>Number(e||0).toLocaleString();return fa(W(),[{h:`Magnets`,td:e=>`${t(e.magnets.active)}<span style="color:var(--color-text-subtle,#6C7C9A)">/${t(e.magnets.total)}</span>`},{h:`Leads · wk`,td:e=>t(e.leads.this_week)},{h:`Leads · total`,td:e=>t(e.leads.total)}],{emptyMsg:`No client orgs yet. Create one under Clients, then provision its magnets here.`})}ym=null,bm=null,xm=null,Sm=null,document.getElementById(`mag-new-btn`)?.addEventListener(`click`,Bp),document.getElementById(`mag-template-btn`)?.addEventListener(`click`,Hp),document.getElementById(`mag-body`)?.addEventListener(`click`,Up),Op(),await Pp();let e=Ef();e&&V.canProvision&&Vp(e)}function Np(e){e&&(e.innerHTML=`<div class="mag-grid">
    <div class="mag-head" aria-hidden="true"><span>Magnet</span><span>Status</span><span>Last 30 days</span><span></span></div>
    ${V.magnets.map(Ep).join(``)}</div>`,E(e.querySelector(`.mag-grid`),{rowSelector:`.mag-card`,noun:`magnets`}))}async function Pp(){let e=document.getElementById(`mag-body`);if(!e)return;try{let[e,t]=await Promise.all([fetch(`${W()}/m5t/v5/magnets`),fetch(`${W()}/m5t/v5/magnets/stats`).catch(()=>null)]),n=await e.json().catch(()=>({}));if(!n.ok)throw Error(n.message||`Failed to load`);V.magnets=n.magnets||[],V.owners=n.owners||[],V.canProvision=!!n.canProvision,V.canManage=!!n.canManage,V.primaryOrgGuid=n.primaryOrgGuid||null;let r=t?await t.json().catch(()=>({})):{};V.stats=r?.stats||{}}catch{e.innerHTML=ap(`error`,`Could not load your magnets. Please try again.`);return}let t=document.getElementById(`mag-new-btn`);t&&(t.style.display=V.canProvision?``:`none`);let n=document.getElementById(`mag-template-btn`);if(n&&(n.style.display=V.canProvision?``:`none`),!V.magnets.length){e.innerHTML=ap(`widgets`,V.canProvision?`No magnets yet. Click “New magnet” to create your first.`:`No magnets yet.`);return}Np(e),cp(),hp()}async function Fp(e){let t={"Content-Type":`application/json`,"Idempotency-Key":crypto.randomUUID()},n={status:`DRAFT`};e&&(n.owner_guid=e);let r=await fetch(`${W()}/m5t/v5/magnets`,{method:`POST`,headers:t,body:JSON.stringify(n)}),i={};try{i=await r.json()}catch{}if(!r.ok||!i.ok)throw Error(i.message||`Could not start a new magnet (${r.status})`);return i.magnet}async function Ip(e,t){let n=await fetch(`${W()}/m5t/v5/magnets/${encodeURIComponent(e)}`,{method:`PATCH`,headers:{"Content-Type":`application/json`},body:JSON.stringify({name:t.name,...t.appearance,steps:t.steps})}),r={};try{r=await n.json()}catch{}if(!n.ok||!r.ok)throw Error(r.message||`Could not apply the ${t.name} template (${n.status})`)}async function Lp(e,t){let n=document.getElementById(`mag-new-btn`),r=document.getElementById(`mag-template-btn`);n&&(n.disabled=!0),r&&(r.disabled=!0);let i=null;try{let n=await Fp(e);i=n.guid,t?(await Ip(n.guid,t),await Pp(),n=V.magnets.find(e=>e.guid===n.guid)||n):(V.magnets=[n,...V.magnets],Np(document.getElementById(`mag-body`))),om(n,void 0,{freshDraft:!0})}catch(e){i&&fetch(`${W()}/m5t/v5/magnets/${encodeURIComponent(i)}`,{method:`DELETE`}).catch(()=>{}),U(e.message||`Could not start a new magnet`,`danger`)}finally{n&&(n.disabled=!1),r&&(r.disabled=!1)}}function Rp(e){let t=document.createElement(`div`);t.className=`mag-modal-scrim`,t.innerHTML=`
    ...
    </div>`,document.body.appendChild(t),requestAnimationFrame(()=>{t.classList.add(`open`),t.querySelector(`.mag-modal`).classList.add(`open`)});let n=()=>{t.classList.remove(`open`),setTimeout(()=>t.remove(),180),document.removeEventListener(`keydown`,r)};function r(e){e.key===`Escape`&&(e.preventDefault(),n())}document.addEventListener(`keydown`,r),t.querySelector(`.mag-modal__close`).addEventListener(`click`,n),t.querySelector(`[data-x="cancel"]`).addEventListener(`click`,n),t.querySelector(`[data-x="continue"]`).addEventListener(`click`,()=>{let r=t.querySelector(`#mag-owner-pick`).value;n(),Lp(r,e)})}function zp(){return V.owners.length===1?V.owners[0].guid:null}function Bp(){if(V.owners.length>1){Rp();return}Lp(zp())}function Vp(e){if(V.owners.length>1){Rp(e);return}Lp(zp(),e)}function Hp(){kf(Vp)}function Up(e){let t=e.target.closest(`[data-action]`);if(!t){if(kp(),!e.target.closest(`a`)){let t=e.target.closest(`.mag-line`)?.closest(`.mag-card`);t?.dataset.guid&&Dp(t.dataset.guid)}return}let n=t.dataset.guid,r=V.magnets.find(e=>e.guid===n),i=t.dataset.action;if(i===`menu`){let e=t.parentElement?.querySelector(`.mag-menu`);if(!e)return;let n=e.hidden;kp(e),e.hidden=!n,t.setAttribute(`aria-expanded`,String(n));return}if(kp(),i===`install-toggle`)return Dp(n);if(i===`copy`)return Wp(n);if(r){if(i===`preview`)return of(()=>$d(r));if(i===`funnel`)return Kp(r);if(i===`edit`)return om(r);if(i===`flow`)return om(r,`flow`);if(i===`allow-domain`)return qp(r,t.dataset.domain);if(i===`dismiss-domain`)return Jp(r,t.dataset.domain);if(i===`restore-domain`)return Yp(r,t.dataset.domain);if(i===`pair-start`)return bp(r);if(i===`copy-code`)return xp(r);if(i===`revoke-connection`)return Cp(r,t.dataset.conn);if(i===`clear-connection`)return Sp(r,t.dataset.conn,t.dataset.label);if(i===`toggle`)return Zp(r);if(i===`delete`)return Qp(r)}}async function Wp(e){try{await navigator.clipboard.writeText(um(e)),U(`Embed snippet copied`,`success`)}catch{U(`Copy failed`,`danger`)}}function Gp(e){let t=Number(e?.total_steps)||0,n={};if((Array.isArray(e?.funnel)?e.funnel:[]).forEach(e=>{n[e.position]=Number(e.reached)||0}),!t||!Object.keys(n).length)return`<p class="mag-modal__hint" style="margin:0">No step activity yet in this window. As visitors answer questions, drop-off by step will appear here.</p>`;let r=Math.max(1,n[1]||0),i=[];for(let e=1;e<=t;e++){let a=n[e]||0,o=Math.min(100,Math.round(a/r*100));i.push(`
    ...
    <div class="mag-modal" role="dialog" aria-modal="true" aria-label="${i?`Edit magnet`:`New magnet`}">
      ...
      <div class="mag-modal__main">
        ...
        <select class="mag-modal__select" id="mag-f-trigger">
          ${gm.map(t=>`<option value="${t.value}"${Vf(e)===t.value?` selected`:``}>${H(t.label)}</option>`).join(``)}
        ...
        <div class="mag-perdev" id="mag-f-perdev"${Vf(e)===`__per_device`?``:` hidden`}>
          <div class="mag-perdev__row">
            ...
            <select class="mag-modal__select" id="mag-f-trigger-desktop">
              ${gm.map(t=>`<option value="${t.value}"${Hf(e,`desktop`)===t.value?` selected`:``}>${H(t.label)}</option>`).join(``)}
            </select>
          </div>
          <div class="mag-perdev__row">
            <label class="mag-modal__label" for="mag-f-trigger-mobile">Mobile</label>
            <select class="mag-modal__select" id="mag-f-trigger-mobile">
              ${gm.map(t=>`<option value="${t.value}"${Hf(e,`mobile`)===t.value?` selected`:``}>${H(t.label)}</option>`).join(``)}
            </select>
          </div>
        ...
        <div class="mag-modal__sec">
          ...
          <select class="mag-modal__select" id="mag-f-font">${am(e?.font||``)}</select>
...
</details>`,fm=e=>{let t=H(um(e));return`<details class="mag-guide">
...
/&gt;</code></pre>
...
</details>`},pm={chat:`Chat bubble`,right_slide:`Slide-in panel`,modal:`Modal`,inline:`Inline`},mm=[{value:`inline`,icon:`view_agenda`,label:`Inline`},{value:`chat`,icon:`chat`,label:`Chat`},{value:`right_slide`,icon:`vertical_split`,label:`Slide`},{value:`modal`,icon:`web_asset`,label:`Modal`}],hm=[{value:`auto`,icon:`brightness_auto`,label:`Auto`},{value:`sync`,icon:`sync`,label:`Sync`},{value:`light`,icon:`light_mode`,label:`Light`},{value:`dark`,icon:`dark_mode`,label:`Dark`}],gm=[{value:`on_load`,label:`Immediately on page load`},{value:`on_8_seconds`,label:`After 8 seconds`},{value:`on_scroll_80_percent`,label:`At 80% scroll depth`},{value:`on_exit_intent`,label:`On exit intent (mouse leaves the page)`},{value:`none`,label:`Manual only (visitor clicks the launcher)`}],_m={book:{label:`Book`,icon:`event`,kind:`book`},chat:{label:`Chat`,icon:`chat`,kind:`flow`},call:{label:`Call`,icon:`phone`,kind:`value`,placeholder:`Phone number`},email:{label:`Email`,icon:`mail`,kind:`value`,placeholder:`Email address`},support:{label:`Support`,icon:`support_agent`,kind:`support`}},vm=[`book`,`chat`,`call`,`email`,`support`],ym=null,bm=null,xm=null,Sm=null,Cm=5e3,wm={ACTIVE:{label:`Connected`,icon:`check_circle`,tone:`live`},PENDING:{label:`Waiting to connect`,icon:`hourglass_empty`,tone:`pending`},REVOKED:{label:`Disconnected`,icon:`link_off`,tone:`off`},EXPIRED:{label:`Expired`,icon:`timer_off`,tone:`off`}},Tm=!1,Em=512,Dm=new Set([`image/png`,`image/jpeg`,`image/gif`,`image/webp`]),Om=8388608,km=[{label:`Web fonts (loaded for you)`,opts:[[`Inter, sans-serif`,`Inter`],[`Poppins, sans-serif`,`Poppins`],[`Montserrat, sans-serif`,`Montserrat`],[`Roboto, sans-serif`,`Roboto`],[`"Open Sans", sans-serif`,`Open Sans`],[`Lato, sans-serif`,`Lato`],[`Nunito, sans-serif`,`Nunito`],[`Raleway, sans-serif`,`Raleway`],[`"Work Sans", sans-serif`,`Work Sans`],[`Oswald, sans-serif`,`Oswald`],[`Merriweather, serif`,`Merriweather (serif)`]]},{label:`System / always available`,opts:[[`system-ui, sans-serif`,`System UI`],[`Georgia, serif`,`Georgia (serif)`],[`"Times New Roman", Times, serif`,`Times (serif)`],[`Arial, Helvetica, sans-serif`,`Arial / Helvetica`],[`Verdana, Geneva, sans-serif`,`Verdana`],[`"Courier New", monospace`,`Courier (mono)`]]}]})),jm=c({default:()=>Gm,init:()=>Lm,render:()=>Fm});function Mm(){let e=new Map;for(let t of yf())e.set(t.category,(e.get(t.category)||0)+1);return Object.keys(Mf).filter(t=>e.has(t)).map(t=>[t,e.get(t)])}function Nm(){if(Of(),Wm)return;let e=document.createElement(`style`);e.id=`m5t-templates-page-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Wm=!0}function Pm(){let e=Hm()||Vm();return`
    ...
    <div class="page-shell" data-section="templates">
      ...
      ${e||Vm()?``:`<div class="settings-note" id="tpl-readonly-note">
        <span class="material-symbols-outlined" aria-hidden="true">lock</span>
        Setup managed by your provider.
      </div>`}

      <div id="tpl-gallery">${Pm()}</div>
    </div>`}function Im(){let e=document.getElementById(`tpl-grid`);document.getElementById(`tpl-filters`)?.addEventListener(`click`,t=>{let n=t.target.closest(`[data-cat]`);if(!n)return;n.parentElement.querySelectorAll(`[data-cat]`).forEach(e=>{e.setAttribute(`aria-pressed`,String(e===n))});let r=n.dataset.cat;e?.querySelectorAll(`.mag-tpl-card`).forEach(e=>{e.hidden=r!==`all`&&e.dataset.cat!==r})}),e?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-preview]`);if(t){let e=yf().find(e=>e.id===t.dataset.preview);e&&of(()=>$d({...e.appearance,steps:e.steps,name:e.name}));return}let n=e.target.closest(`[data-tpl]`);n&&(wf(n.dataset.tpl),window.location.hash=Hm()?`#/signup`:`#/magnets`)})}function Lm(){Im();let e=yf();bf().then(t=>{let n=document.getElementById(`tpl-gallery`);!n||t===e||(n.innerHTML=Pm(),Im())})}var Rm,zm,Bm,Vm,Hm,Um,Wm,Gm,Km=o((()=>{w(),cf(),zf(),Rm={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},zm=e=>String(e??``).replace(/[&<>"']/g,e=>Rm[e]),Bm=[`provision_magnets`,`manage_magnets`],Vm=()=>C.getCapabilities().some(e=>Bm.includes(e)),Hm=()=>!C.isAuthenticated(),Um=(e,t,n,r)=>`<button class="m5t-chip--filter" type="button" data-cat="${zm(e)}" aria-pressed="${r}">${zm(t)}<span class="m5t-chip__count">${n}</span></button>`,Wm=!1,Gm={render:Fm,init:Lm}})),qm=c({default:()=>vh,init:()=>$m,render:()=>Qm});function Jm(){if(dr(),hh)return;let e=document.createElement(`style`);e.id=`m5t-questionnaires-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),hh=!0}function Ym(){return mh.clients.map(e=>{let t=mh.responses.filter(t=>t.owner_guid===e.guid),n=t[0]||null;return{guid:e.guid,name:e.caption||e.name||e.guid,latest:n,earlier:Math.max(0,t.length-1),status:n?n.status:`NOT_SENT`}})}function Xm(){let e=mh.search.trim().toLowerCase();return Ym().filter(t=>(mh.filter===`ALL`||t.status===mh.filter)&&(!e||t.name.toLowerCase().includes(e)))}function Zm(e){let t=e.status===`SUBMITTED`,n=e.status===`SUBMITTED`?`<button class="qz-act" type="button" data-act="open" data-guid="${ch(e.latest.guid)}">Read answers</button>`:e.status===`DRAFT`?`<button class="qz-act" type="button" data-act="copy" data-url="${ch(e.latest.url||``)}">Copy link</button>`:`<a class="qz-act" href="#/clients">Go to client</a>`;return`<tr class="qz-row${t?` qz-row--open`:``}"${t?` data-open-guid="${ch(e.latest.guid)}"`:``}>
    <td><span class="qz-client">${ch(e.name)}</span>${e.earlier?`<span class="qz-earlier">${e.earlier} earlier</span>`:``}</td>
    <td><span class="qz-badge" data-s="${ch(e.status)}">${ch(ph[e.status]||e.status)}</span></td>
    <td class="qz-when">${ch(_h(e,`created_tz`))}</td>
    <td class="qz-when">${ch(_h(e,`submitted_tz`))}</td>
    <td class="qz-actcell">${n}</td>
  </tr>`}function Qm(){return Jm(),`
    <div class="page-shell" data-section="questionnaires">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="qz-body">${gh(`hourglass_empty`,`Loading…`)}</div>
      </div>
    </div>`}async function $m(){await eh()}async function eh(){let e=document.getElementById(`qz-body`);if(e){try{let[e,t]=await Promise.all([fetch(`${lh()}/m5t/v5/acctGroup`),fetch(`${lh()}/m5t/v5/m5mQuestionnaireResponse`)]),n=await e.json().catch(()=>({})),r=await t.json().catch(()=>({}));if(!e.ok||!t.ok)throw Error(`load failed`);mh.clients=(n.data||[]).filter(e=>e.class===dh&&![`DELETED`,`ARCHIVED`].includes(e.status)).sort((e,t)=>String(e.caption||``).localeCompare(String(t.caption||``))),mh.responses=r.data||[]}catch{e.innerHTML=gh(`error`,`Could not load questionnaires. Please try again.`);return}nh()}}function th(){let e=document.getElementById(`qz-tbody`);e&&E(e,{rowSelector:`tr.qz-row`,noun:`clients`,controlHost:e.closest(`.qz-tablewrap`)||e.parentElement})}function nh(){let e=document.getElementById(`qz-body`);if(!e)return;if(!mh.clients.length){e.innerHTML=gh(`groups`,`No clients yet. Add a client first, then send them a kickoff questionnaire.`);return}let t=Ym(),n={ALL:t.length};fh.slice(1).forEach(([e])=>{n[e]=t.filter(t=>t.status===e).length});let r=Xm();e.innerHTML=`
    <div class="qz-bar">
      <div class="qz-filter">
        ${fh.map(([e,t])=>`<button data-filter="${e}" class="${mh.filter===e?`active`:``}">${ch(t)}${n[e]?`<span class="qz-pillnum">${n[e]}</span>`:``}</button>`).join(``)}
      </div>
      <span class="qz-searchwrap">
        <span class="material-symbols-outlined" aria-hidden="true">search</span>
        <input class="qz-search" id="qz-search" type="search" placeholder="Search clients" value="${ch(mh.search)}" aria-label="Search clients" />
      </span>
    ...
    <div class="qz-tablewrap">
      <table class="qz-table">
        ...
        <tbody id="qz-tbody">${r.length?r.map(Zm).join(``):`<tr><td colspan="5"><div class="qz-when" style="padding:14px 4px">${mh.search.trim()?`No clients match your search.`:`No clients with status “${ch(ph[mh.filter]||mh.filter)}”.`}</div></td></tr>`}</tbody>
      </table>
    </div>`,th(),e.querySelector(`.qz-filter`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-filter]`);t&&(mh.filter=t.dataset.filter,nh())}),e.querySelector(`#qz-search`)?.addEventListener(`input`,e=>{mh.search=e.target.value,rh()}),e.querySelector(`#qz-tbody`)?.addEventListener(`click`,ih)}function rh(){let e=document.getElementById(`qz-tbody`);if(!e)return;let t=Xm();e.innerHTML=t.length?t.map(Zm).join(``):`<tr><td colspan="5"><div class="qz-when" style="padding:14px 4px">No clients match your search.</div></td></tr>`,th();let n=document.querySelector(`.qz-count`);n&&(n.textContent=`${t.length} of ${Ym().length}`)}function ih(e){let t=e.target.closest(`[data-act="copy"]`);if(t){e.stopPropagation();let n=t.dataset.url;if(!n){uh(`No link on this questionnaire yet.`,`warning`);return}navigator.clipboard?.writeText(n).then(()=>{t.textContent=`Copied!`,setTimeout(()=>{t.textContent=`Copy link`},1500)},()=>uh(`Could not copy the link.`,`error`));return}let n=e.target.closest(`[data-act="open"]`),r=e.target.closest(`[data-open-guid]`),i=n?.dataset.guid||r?.dataset.openGuid;i&&ah(i)}async function ah(e){mh.openGuid=e;let t=document.createElement(`div`);t.className=`qz-drawer-scrim`;let n=document.createElement(`aside`);n.className=`qz-drawer`,n.setAttribute(`role`,`dialog`),n.setAttribute(`aria-label`,`Questionnaire answers`),n.innerHTML=`
    ...
    <div class="qz-drawer__body" id="qz-dbody">${gh(`hourglass_empty`,`Loading answers…`)}</div>`,document.body.append(t,n),requestAnimationFrame(()=>{t.classList.add(`open`),n.classList.add(`open`)});let r=()=>{mh.openGuid=null,t.classList.remove(`open`),n.classList.remove(`open`),setTimeout(()=>{t.remove(),n.remove()},220),document.removeEventListener(`keydown`,i)};function i(e){e.key===`Escape`&&(e.preventDefault(),r())}document.addEventListener(`keydown`,i),t.addEventListener(`click`,r),n.querySelector(`.qz-drawer__close`).addEventListener(`click`,r);try{let t=await fetch(`${lh()}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e)}`),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`load failed`);let i=r.data||{},a=mh.clients.find(e=>e.guid===i.owner_guid)?.caption||`Client`;n.querySelector(`#qz-dtitle`).textContent=a;let o=i.submitted_tz?`Submitted ${D(i.submitted_tz)}`:`Not yet submitted`;n.querySelector(`#qz-dsub`).textContent=i.locked_tz?`${o} · Finalized`:o,n.querySelector(`#qz-dbody`).innerHTML=ur(i)+sr(i,{emptyLabel:`Not answered`})}catch{n.querySelector(`#qz-dbody`).innerHTML=gh(`error`,`Could not load these answers. Please try again.`)}}var oh,sh,ch,lh,uh,dh,fh,ph,mh,hh,gh,_h,vh,yh=o((()=>{w(),oh=u(f(),1),m(),hr(),sn(),sh={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},ch=e=>String(e??``).replace(/[&<>"']/g,e=>sh[e]),lh=()=>C.getConfig?.().BASE_URL||``,uh=(e,t=`info`)=>{try{oh.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},dh=`managed_client`,fh=[[`ALL`,`All`],[`NOT_SENT`,`Not sent`],[`DRAFT`,`Awaiting response`],[`SUBMITTED`,`Submitted`],[`CANCELLED`,`Cancelled`]],ph=Object.fromEntries(fh),mh={clients:[],responses:[],filter:`ALL`,search:``,openGuid:null},hh=!1,gh=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,_h=(e,t)=>e?.latest?.[t]?D(e.latest[t]):`—`,vh={render:Qm,init:$m}})),bh=c({default:()=>tg,init:()=>Eh,render:()=>Th});function xh(){if(dr(),Zh)return;let e=document.createElement(`style`);e.id=`m5t-deliverables-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Zh=!0}function Sh(){let e=K.search.trim().toLowerCase();return K.rows.filter(t=>(K.filter===`ALL`||t.status===K.filter)&&(!e||String(t.owner_name||``).toLowerCase().includes(e)))}function Ch(e){return e.status===`DRAFT`?`<button class="dz-act" type="button" data-act="open" data-guid="${G(e.guid)}">Edit</button>`:e.status===`SENT`?`<button class="dz-act" type="button" data-act="copy" data-url="${G(e.url||``)}">Copy link</button>`:e.status===`SUBMITTED`?`<button class="dz-act" type="button" data-act="open" data-guid="${G(e.guid)}">Read review</button>`:`<span class="dz-when">—</span>`}function wh(e){let t=e.status!==`CANCELLED`;return`<tr class="dz-row${t?` dz-row--open`:``}"${t?` data-open-guid="${G(e.guid)}"`:``}>
    ...
    <td><span class="dz-badge" data-s="${G(e.status)}">${G(Xh[e.status]||e.status)}</span></td>
    <td class="dz-when">${e.sent_tz?G(D(e.sent_tz)):`—`}</td>
    <td class="dz-when">${e.submitted_tz?G(D(e.submitted_tz)):`—`}</td>
    <td class="dz-actcell">${Ch(e)}</td>
  </tr>`}function Th(){return xh(),`
    ...
    </div>`}async function Eh(){document.getElementById(`dz-new`)?.addEventListener(`click`,Nh),await Dh()}async function Dh(){let e=document.getElementById(`dz-body`);if(e){try{let[e,t,n]=await Promise.all([fetch(`${Kh()}/m5t/v5/m5mDeliverable`),fetch(`${Kh()}/m5t/v5/acctGroup`),fetch(`${Kh()}/m5t/v5/m5mDeliverable/templates`)]),r=await e.json().catch(()=>({})),i=await t.json().catch(()=>({})),a=await n.json().catch(()=>({}));if(!e.ok||!t.ok||!n.ok)throw Error(`load failed`);K.rows=r.data||[],K.clients=(i.data||[]).filter(e=>e.class===Jh&&![`DELETED`,`ARCHIVED`].includes(e.status)).sort((e,t)=>String(e.caption||``).localeCompare(String(t.caption||``))),K.templates=a.data||[]}catch{e.innerHTML=Qh(`error`,`Could not load deliverables. Please try again.`);return}kh()}}function Oh(){let e=document.getElementById(`dz-tbody`);e&&E(e,{rowSelector:`tr.dz-row`,noun:`deliverables`,controlHost:e.closest(`.dz-tablewrap`)||e.parentElement})}function kh(){let e=document.getElementById(`dz-body`);if(!e)return;if(!K.clients.length&&!K.rows.length){e.innerHTML=Qh(`groups`,`No clients yet. Add a client first, then author their first deliverable.`);return}if(!K.rows.length){e.innerHTML=Qh(`approval`,`No deliverables yet. Author the first one with New deliverable.`);return}let t={ALL:K.rows.length};Yh.slice(1).forEach(([e])=>{t[e]=K.rows.filter(t=>t.status===e).length});let n=Sh();e.innerHTML=`
    <div class="dz-bar">
      <div class="dz-filter">
        ${Yh.map(([e,n])=>`<button data-filter="${e}" class="${K.filter===e?`active`:``}">${G(n)}${t[e]?`<span class="dz-pillnum">${t[e]}</span>`:``}</button>`).join(``)}
      </div>
    ...
    <div class="dz-tablewrap">
      <table class="dz-table">
        ...
        <tbody id="dz-tbody">${n.length?n.map(wh).join(``):`<tr><td colspan="6"><div class="dz-when" style="padding:14px 4px">${K.search.trim()?`No deliverables match your search.`:`No deliverables with status “${G(Xh[K.filter]||K.filter)}”.`}</div></td></tr>`}</tbody>
      </table>
    </div>`,Oh(),e.querySelector(`.dz-filter`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-filter]`);t&&(K.filter=t.dataset.filter,kh())}),e.querySelector(`#dz-search`)?.addEventListener(`input`,e=>{K.search=e.target.value,Ah()}),e.querySelector(`#dz-tbody`)?.addEventListener(`click`,jh)}function Ah(){let e=document.getElementById(`dz-tbody`);if(!e)return;let t=Sh();e.innerHTML=t.length?t.map(wh).join(``):`<tr><td colspan="6"><div class="dz-when" style="padding:14px 4px">No deliverables match your search.</div></td></tr>`,Oh();let n=document.querySelector(`.dz-count`);n&&(n.textContent=`${t.length} of ${K.rows.length}`)}function jh(e){let t=e.target.closest(`[data-act="copy"]`);if(t){e.stopPropagation();let n=t.dataset.url;if(!n){qh(`No link on this deliverable yet.`,`warning`);return}navigator.clipboard?.writeText(n).then(()=>{t.textContent=`Copied!`,setTimeout(()=>{t.textContent=`Copy link`},1500)},()=>qh(`Could not copy the link.`,`error`));return}let n=e.target.closest(`[data-act="open"]`),r=e.target.closest(`[data-open-guid]`),i=n?.dataset.guid||r?.dataset.openGuid;i&&Uh(i)}function Mh(e){let t=document.createElement(`div`);t.className=`dz-drawer-scrim`;let n=document.createElement(`aside`);n.className=`dz-drawer`,n.setAttribute(`role`,`dialog`),n.setAttribute(`aria-label`,e),n.innerHTML=`
    ...
    </div>`}function Fh(e,t,n,{hidden:r=!1}={}){let i=`${t}: `,a=e[0]?.group||``;return`
      <div class="dz-cblock" data-group="${G(a)}" data-heading="${G(t)}"${r?` hidden`:``}>
        <h4 class="dz-cblock__title">${G(t)}</h4>
        ${e.map(e=>Ph(e,n,e.label.startsWith(i)?e.label.slice(i.length):e.label)).join(``)}
      </div>`}function Ih(e,t){let n=(e.questions||[]).filter(e=>e.audience===`am`),r=n.filter(e=>!e.group).map(e=>Ph(e,t)).join(``),i=Nr(e);if(i){let n=Pr(e).filter(e=>e.audience===`am`);if(!n.length)return r;let a=Lr(e,t);for(let e=1;e<=a;e++){let a=Fr(n,i,e);r+=Fh(a,eg(a,`${i}${e}`),t)}let o=eg(Fr(n,i,a+1),`${i}${a+1}`);return r+=`<button class="dz-btn--ghost dz-addblock" type="button" data-repeat="${G(i)}" data-section-id="${G(e.id)}">+ Add ${G(o)}</button>`,r}let a=[...new Set(n.filter(e=>e.group).map(e=>e.group))];if(!a.length)return r;let o=null;return r+=a.map((e,r)=>{let i=n.filter(t=>t.group===e),a=eg(i,e),s=r===0||$h(i,t);return!s&&o===null&&(o={key:e,heading:a}),Fh(i,a,t,{hidden:!s})}).join(``),r+=`<button class="dz-btn--ghost dz-addblock" type="button"${o?``:` hidden`}>${o?`+ Add ${G(o.heading)}`:``}</button>`,r}function Lh(e,t){let n=(e?.sections||[]).filter(e=>(e.questions||[]).some(e=>e.audience===`am`)),r=n.length>1;return`
    ${r?`
    ...
    ${r?`
      ...
      </div>`:``}).join(``)}function Bh(e,t){if(!t)return``;let n=(e?.sections||[]).flatMap(e=>(e.questions||[]).filter(e=>e.audience!==`am`)).map(e=>({q:e,val:or(e,t.answers||{})})).filter(({val:e})=>e);return`
    ...
    </div>`}function Vh(e,t){let n=Array.isArray(t)&&t.length?t[t.length-1]:null;return n?Bh(e,n):``}function Hh(e,t){return(Array.isArray(t)?t.slice().reverse():[]).map(t=>Bh(e,t)).join(``)}async function Uh(e){let{drawer:t,close:n}=Mh(`Deliverable`),r=t.querySelector(`#dz-dbody`),i=t.querySelector(`#dz-dfoot`),a;try{let t=await fetch(`${Kh()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}`),n=await t.json().catch(()=>({}));if(!t.ok||!n.ok)throw Error(n.error||`load failed`);a=n.data||{}}catch{r.innerHTML=Qh(`error`,`Could not load this deliverable. Please try again.`);return}let o=K.rows.find(t=>t.guid===e)?.owner_name||K.clients.find(e=>e.guid===a.owner_guid)?.caption||`Client`;t.querySelector(`#dz-dtitle`).textContent=`${o} · ${a.template?.name||`Deliverable`}`;let s=a.status===`DRAFT`||a.status===`SENT`,c=Number(a.revision)>1?` · Rev ${a.revision}`:``,l=a.status===`SUBMITTED`?`Returned ${a.submitted_tz?D(a.submitted_tz):``}${a.locked_tz?` · Finalized`:``}${c}`:a.status===`SENT`?`Sent ${a.sent_tz?D(a.sent_tz):``} to ${a.recipient_email||``} · awaiting approval${c}`:`${Xh[a.status]||a.status}${c}`;if(t.querySelector(`#dz-dsub`).textContent=l,!s){r.innerHTML=ur(a)+zh(a.template,a.content||{},a.answers||{})+Hh(a.template,a.rounds),a.status===`SUBMITTED`&&!a.locked_tz?(i.innerHTML=`<button class="dz-btn" type="button" id="dz-revise">Revise plan</button>
        <span class="dz-note" style="margin:0">Archives this review and reopens authoring. The client approves the revised plan on a fresh link.</span>`,i.querySelector(`#dz-revise`).addEventListener(`click`,async t=>{let r=t.target;r.disabled=!0;try{let t=await fetch(`${Kh()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}/revise`,{method:`POST`}),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`revise failed`);qh(`Reopened for revision. The last review is pinned above the form.`,`success`),n(),await Dh(),Uh(e)}catch(e){qh(e.message||`Could not revise.`,`error`),r.disabled=!1}})):i.innerHTML=``;return}r.innerHTML=`
    ${Vh(a.template,a.rounds)}
    ${a.url?`
    ...
    <button class="dz-btn--ghost dz-btn--danger" type="button" id="dz-cancel">Cancel deliverable</button>`,r.querySelector(`#dz-copylink`)?.addEventListener(`click`,e=>{let t=r.querySelector(`.dz-linkrow input`);navigator.clipboard?.writeText(t?.value||``).then(()=>{e.target.textContent=`Copied!`,setTimeout(()=>{e.target.textContent=`Copy link`},1500)},()=>qh(`Could not copy the link.`,`error`))});let u=async()=>{let t=lr(r.querySelector(`#dz-author-form`)),i=await fetch(`${Kh()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}`,{method:`PUT`,headers:{"Content-Type":`application/json`},body:JSON.stringify({content:t})}),a=await i.json().catch(()=>({}));if(!i.ok||!a.ok){if(i.status===409)return qh(a.error||`This deliverable is no longer editable.`,`warning`),n(),await Dh(),!1;throw Error(a.error||`save failed`)}return!0};Rh(r.querySelector(`#dz-author-form`),a.template,()=>{u().catch(()=>qh(`Autosave failed. Use Save draft.`,`warning`))}),i.querySelector(`#dz-preview`).addEventListener(`click`,async e=>{let t=e.target.closest(`button`);if(!a.url){qh(`No link available for this deliverable.`,`warning`);return}t.disabled=!0;try{await u()&&window.open(a.url,`_blank`)}catch(e){qh(e.message||`Could not save before previewing.`,`error`)}t.disabled=!1}),i.querySelector(`#dz-save`).addEventListener(`click`,async e=>{let t=e.target;t.disabled=!0;try{await u()&&qh(`Draft saved.`,`success`)}catch(e){qh(e.message||`Could not save.`,`error`)}t.disabled=!1}),i.querySelector(`#dz-send`).addEventListener(`click`,async t=>{let r=t.target,a=(i.querySelector(`#dz-email`)?.value||``).trim();if(!a){qh(`Enter the recipient email first.`,`warning`);return}r.disabled=!0;try{if(!await u())return;let t=await fetch(`${Kh()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}/send`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({recipient_email:a})}),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`send failed`);qh(r.data?.emailQueued?`Sent. ${a} will receive the review link.`:`Link created, but the email could not be queued. Copy the link and send it yourself.`,r.data?.emailQueued?`success`:`warning`),n(),await Dh()}catch(e){qh(e.message||`Could not send.`,`error`),r.disabled=!1}}),i.querySelector(`#dz-cancel`).addEventListener(`click`,async t=>{let r=t.target;r.disabled=!0;try{let t=await fetch(`${Kh()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}/cancel`,{method:`POST`}),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`cancel failed`);qh(`Deliverable cancelled. Its link no longer works.`,`info`),n(),await Dh()}catch(e){qh(e.message||`Could not cancel.`,`error`),r.disabled=!1}})}var Wh,Gh,G,Kh,qh,Jh,Yh,Xh,K,Zh,Qh,$h,eg,tg,ng=o((()=>{w(),Wh=u(f(),1),m(),hr(),Vr(),sn(),Gh={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},G=e=>String(e??``).replace(/[&<>"']/g,e=>Gh[e]),Kh=()=>C.getConfig?.().BASE_URL||``,qh=(e,t=`info`)=>{try{Wh.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},Jh=`managed_client`,Yh=[[`ALL`,`All`],[`DRAFT`,`In progress`],[`SENT`,`Awaiting approval`],[`SUBMITTED`,`Returned`],[`CANCELLED`,`Cancelled`]],Xh=Object.fromEntries(Yh),K={rows:[],clients:[],templates:[],filter:`ALL`,search:``},Zh=!1,Qh=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,$h=(e,t)=>e.some(e=>{let n=t?.[e.id];return Array.isArray(n)?n.length>0:!!(n&&String(n).trim())}),eg=(e,t)=>{let n=String(e[0]?.label||``).match(/^(.+?):\s/);return n?n[1]:t.toUpperCase()},tg={render:Th,init:Eh}})),rg=c({default:()=>hg,init:()=>lg,render:()=>cg});function ig(e){return e.status===`SENT`?{key:`PENDING`,label:`Pending your review`}:e.status===`SUBMITTED`&&e.signed?{key:`APPROVED`,label:`Approved`}:e.status===`SUBMITTED`?{key:`RETURNED`,label:`Returned with edits`}:null}function ag(){if(dr(),pg)return;let e=document.createElement(`style`);e.id=`m5t-deliverables-client-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Kg=!0}function xg(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function Sg(e){return`<select class="leads-status" data-id="${J(e.id)}" data-act="status" onclick="event.stopPropagation()">
  ...
  </button>`}function jg(){bg(),aa();let e=A();return`
    ...
    </div>`}async function Mg(){document.getElementById(`leads-export`)?.addEventListener(`click`,Wg);let e=Sa(wa.LEADS_SEARCH),t=Sa(wa.LEADS_DAY);if((e||t)&&(q.search=e||``,q.day=t||``,q.filter=`ALL`,q.clientFilter=`ALL`,q.magnetFilter=`ALL`,q.range=`ALL`,q.shown=qg()),A()){let e=e=>Number(e||0).toLocaleString();fa(Zg(),[{h:`Leads · wk`,td:t=>e(t.leads.this_week)},{h:`Leads · total`,td:t=>e(t.leads.total)},{h:`Compl.`,td:e=>e.completion_rate==null?`—`:`${e.completion_rate}%`}],{emptyMsg:`No client orgs yet. Create one under Clients first.`})}await Pg(),Ng()}async function Ng(){let e=document.getElementById(`leads-crm-slot`);if(e)try{let{denied:t,providers:n,configured:r}=await vc();if(t)return;let i=l_.map(e=>{let t=sc[e],i=hc(n,e);return{p:t,conn:i,status:gc(t,i,!!r?.[e])}}),a=i.filter(e=>e.status===`connected`),o=i.find(e=>e.status===`broken`);if(a.length){let t=a.map(e=>e.p.label).join(` and `),n=a.length===1&&a[0].conn?.external_account?.hub_domain||``;e.innerHTML=`
...
`)],{type:`text/csv`}),a=URL.createObjectURL(i),o=document.createElement(`a`);o.href=a,o.download=`leads-${q.filter.toLowerCase()}.csv`,document.body.appendChild(o),o.click(),o.remove(),URL.revokeObjectURL(a),Xg(`Exported ${e.length} lead${e.length===1?``:`s`}`,`success`)}var Gg,Kg,q,qg,Jg,Yg,J,Xg,Zg,Qg,$g,e_,t_,n_,r_,i_,a_,o_,s_,c_,l_,u_,d_,f_=o((()=>{w(),Gg=u(f(),1),m(),ba(),Ea(),sn(),Uc(),Kg=!1,q={leads:[],magnetNames:{},filter:`ALL`,canManage:!1,book:!1,ownerNames:{},clientFilter:`ALL`,search:``,shown:25,selectedId:null,day:``,sort:{key:`captured`,dir:`desc`},magnetFilter:`ALL`,range:`ALL`,pageSize:25,total:0,truncated:!1},qg=()=>q.pageSize===`ALL`?2**53-1:Number(q.pageSize)||25,Jg=[`NEW`,`WORKING`,`QUALIFIED`,`UNQUALIFIED`,`CONVERTED`,`EXPORTED`],Yg={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},J=e=>String(e??``).replace(/[&<>"']/g,e=>Yg[e]),Xg=(e,t=`info`)=>{try{Gg.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},Zg=()=>C.getConfig?.().BASE_URL||``,Qg=e=>[e.first_name,e.last_name].filter(Boolean).join(` `).trim()||e.email||`(no name)`,$g=e=>(Qg(e).split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``)||`?`).toUpperCase(),e_=e=>e.created?.created_tz||``,t_=e=>{let t=e_(e);if(!t)return!1;try{return Date.now()-new Date(t).getTime()<864e5}catch{return!1}},n_=e=>q.clientFilter===`ALL`||e.owner_guid===q.clientFilter,r_=e=>!q.day||e_(e).slice(0,10)===q.day,i_=e=>q.magnetFilter===`ALL`||e.magnet_guid===q.magnetFilter,a_={ALL:null,7:7,30:30,90:90},o_={ALL:`All time`,7:`Last 7 days`,30:`Last 30 days`,90:`Last 90 days`},s_={name:{label:`Name`,dir:`asc`,value:e=>Qg(e).toLowerCase()},contact:{label:`Contact`,dir:`asc`,value:e=>String(e.email||e.phone||``).toLowerCase()},captured:{label:`Captured`,dir:`desc`,value:e=>e_(e)}},c_=()=>xg(`touch_app`,`Click to see lead information.`),l_=[`hubspot`,`zoho`,`salesforce`,`gohighlevel`],u_=()=>`
  ...
  </table></div>`}function v_(){return A()?ua({section:`analytics`,title:`Analytics`,description:`Pick a client to see how their magnets are performing.`}):(m_(),Ma(),`
  ...
  `,document.head.appendChild(e),dv=!0}function R_(){return A()?ua({section:`analytics-conversions`,title:`Conversions`,description:`Pick a client to view their conversion reporting.`}):(L_(),Y.canSave=Ic(),`
    ...
    <p class="conv-note">What Magnet fed into Google Ads. Google's attributed numbers above are its own and are never added to these.</p>`}function X_(e,t){let n=document.getElementById(e);n&&(n.textContent=t)}function Z_(e,t){let n=document.getElementById(e);n&&(n.innerHTML=t)}var Q_,$_,ev,tv,nv,rv,iv,av,ov,sv,cv,lv,uv,Y,dv,fv,pv,mv,hv=o((()=>{w(),ba(),Uc(),Aa(),Ba(),sn(),Q_=()=>C.getConfig?.().BASE_URL||``,$_=e=>e.toISOString().slice(0,10),ev=`—`,tv=e=>e==null?ev:`$`+Number(e).toLocaleString(void 0,{minimumFractionDigits:0,maximumFractionDigits:2}),nv=e=>e==null?ev:Number(e).toLocaleString(),rv={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},iv=e=>String(e??``).replace(/[&<>"']/g,e=>rv[e]),av=(e,t)=>{let n=Number(e),r=Number(t);return!Number.isFinite(n)||!Number.isFinite(r)||r<=0?null:Math.round(n/r*100)/100},ov=[`magnet_fill`,`qualified_lead`,`call_dni`,`calls_from_ads`],sv={magnet_fill:`Magnet fills`,qualified_lead:`Qualified leads`,call_dni:`Website calls`,calls_from_ads:`Calls from ads`},cv=new Set([`Magnet Lead`,`Magnet Qualified Lead`,`Magnet Phone Call (Website)`,`Calls from Ads (Magnet)`]),lv=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],uv=30,Y={from:null,to:null,spend:null,universe:[],selection:new Set,conv:null,rows:[],overlayRows:[],canSave:!1,busy:!1},dv=!1,fv=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,pv=e=>String(e||``).toLowerCase().replace(/_/g,` `)||ev,mv={render:R_,init:z_}})),gv=c({default:()=>Lv,init:()=>yv,render:()=>vv});function _v(){if(Ma(),Nv)return;let e=document.createElement(`style`);e.id=`m5t-adperf-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Nv=!0}function vv(){return A()?ua({section:`ad-performance`,title:`Ad Performance`,description:`Pick a client to view their spend and lead performance.`}):(_v(),`
    ...
    </div>`;let c=t.querySelector(`.adperf-srctable tbody`);c&&E(c,{rowSelector:`tr`,noun:`sources`,controlHost:t.querySelector(`.adperf-srcwrap`)||t})}function wv(e,t){let n=document.getElementById(e);n&&(n.textContent=t)}function Tv(e){return Na({rows:e,xLabel:e=>String(e.date||``).slice(5),left:{value:e=>e.spend||0,label:`Ad Spend`,fmt:e=>`$${Ra(e)}`},right:{value:e=>e.magnet_leads||0,label:`Magnet Leads`},ariaLabel:`Ad spend versus magnet leads over time`,pointLabel:e=>`${e.date}: ${kv(e.spend||0)} spend, ${Av(e.magnet_leads||0)} leads`,hitData:e=>({day:e.date})})}function Ev(e){let t=document.getElementById(`adperf-body`);return t?Fa(t,{rows:e,tipHTML:e=>`
      <div class="m5c-tip__title">${Mv(String(e.date||``))}</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--1"></i>${kv(e.spend||0)} spend</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--2"></i>${Av(e.magnet_leads||0)} leads</div>`}):()=>{}}var Dv,Ov,kv,Av,jv,Mv,Nv,Pv,Fv,Iv,Lv,Rv=o((()=>{w(),ba(),Uc(),Aa(),Ba(),sn(),Ea(),Dv=()=>C.getConfig?.().BASE_URL||``,Ov=e=>e.toISOString().slice(0,10),kv=e=>e==null?`—`:`$`+Number(e).toLocaleString(void 0,{minimumFractionDigits:0,maximumFractionDigits:2}),Av=e=>Number(e||0).toLocaleString(),jv={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Mv=e=>String(e??``).replace(/[&<>"']/g,e=>jv[e]),Nv=!1,Pv=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,Fv=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],Iv=30,Lv={render:vv,init:yv}})),zv=c({default:()=>oy,init:()=>Hv,parseBookingCsv:()=>$v,render:()=>Vv});function Bv(){if(ry)return;let e=document.createElement(`style`);e.id=`m5t-adacct-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),ry=!0}function Vv(){return A()?ua({section:`ad-accounts`,title:`Ad Accounts`,description:`Pick a client to see their connected ad accounts.`}):(Bv(),Mc(),`
