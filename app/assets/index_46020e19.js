...
`))}</textarea>`}return`<div class="mag-cta" data-type="${e}" data-kind="${n.kind}">
    ...
    ${o}
  </div>`}function Ad(){if(Of)return;let e=document.createElement(`style`);e.id=`m5t-magnets-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Of=!0}function jd(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function Md(e){let t=H.stats[e];if(!t||!t.opens&&!t.leads)return`<div class="mag-stats"><span><span class="material-symbols-outlined" aria-hidden="true">monitoring</span>No activity yet (30d)</span></div>`;let n=e=>Number(e||0).toLocaleString();return`<div class="mag-stats" title="Trailing 30 days">
    ...
    <div class="mag-card" data-guid="${U(e.guid)}">
      ...
      <span class="mag-mode"><span class="material-symbols-outlined" aria-hidden="true">${Pf[e.display_mode]||`chat`}</span>${U(Nf[e.display_mode]||`Chat bubble`)}${e.ab?.enabled?`&nbsp;&nbsp;<span class="mag-ab-chip"><span class="material-symbols-outlined" aria-hidden="true">science</span>A/B</span>`:``}${(e.notify_emails||[]).length?`&nbsp;&nbsp;<a class="mag-alert-chip" href="#/settings/notifications" title="Lead alerts to ${(e.notify_emails||[]).length} email${(e.notify_emails||[]).length===1?``:`s`}. Manage in Settings."><span class="material-symbols-outlined" aria-hidden="true">notifications_active</span>${(e.notify_emails||[]).length}</a>`:``}</span>
      ${Md(e.guid)}
      ...
      <div class="mag-embed">
        <input type="text" readonly value="${U(Af(e.guid))}" aria-label="Embed snippet">
    ...
    </div>`}function Zd(){return M()?bi({section:`magnets`,title:`Your Magnets`,description:`Pick a client to view and manage their magnets.`}):(Ad(),`
    ...
    </div>`)}async function Qd(){if(M()){let e=od();if(e){let t=document.querySelector(`.page-shell .page-header-text .page-description`);t&&(t.textContent=`Template picked: ${e.name}. Choose the client to build it for.`)}let t=e=>Number(e||0).toLocaleString();return xi(G(),[{h:`Magnets`,td:e=>`${t(e.magnets.active)}<span style="color:var(--color-text-subtle,#6C7C9A)">/${t(e.magnets.total)}</span>`},{h:`Leads · wk`,td:e=>t(e.leads.this_week)},{h:`Leads · total`,td:e=>t(e.leads.total)}],{emptyMsg:`No client orgs yet. Create one under Clients, then provision its magnets here.`})}Bf=null,Vf=null,document.getElementById(`mag-new-btn`)?.addEventListener(`click`,of),document.getElementById(`mag-template-btn`)?.addEventListener(`click`,cf),document.getElementById(`mag-body`)?.addEventListener(`click`,lf),await $d();let e=ad();e&&H.canProvision&&sf(e)}async function $d(){let e=document.getElementById(`mag-body`);if(!e)return;try{let[e,t]=await Promise.all([fetch(`${G()}/m5t/v5/magnets`),fetch(`${G()}/m5t/v5/magnets/stats`).catch(()=>null)]),n=await e.json().catch(()=>({}));if(!n.ok)throw Error(n.message||`Failed to load`);H.magnets=n.magnets||[],H.owners=n.owners||[],H.canProvision=!!n.canProvision,H.canManage=!!n.canManage,H.primaryOrgGuid=n.primaryOrgGuid||null;let r=t?await t.json().catch(()=>({})):{};H.stats=r?.stats||{}}catch{e.innerHTML=jd(`error`,`Could not load your magnets. Please try again.`);return}let t=document.getElementById(`mag-new-btn`);t&&(t.style.display=H.canProvision?``:`none`);let n=document.getElementById(`mag-template-btn`);if(n&&(n.style.display=H.canProvision?``:`none`),!H.magnets.length){e.innerHTML=jd(`widgets`,H.canProvision?`No magnets yet. Click “New magnet” to create your first.`:`No magnets yet.`);return}e.innerHTML=`<div class="mag-grid">${H.magnets.map(Xd).join(``)}</div>`,Pd(),Vd()}async function ef(e){let t={"Content-Type":`application/json`,"Idempotency-Key":crypto.randomUUID()},n={status:`DRAFT`};e&&(n.owner_guid=e);let r=await fetch(`${G()}/m5t/v5/magnets`,{method:`POST`,headers:t,body:JSON.stringify(n)}),i={};try{i=await r.json()}catch{}if(!r.ok||!i.ok)throw Error(i.message||`Could not start a new magnet (${r.status})`);return i.magnet}async function tf(e,t){let n=await fetch(`${G()}/m5t/v5/magnets/${encodeURIComponent(e)}`,{method:`PATCH`,headers:{"Content-Type":`application/json`},body:JSON.stringify({name:t.name,...t.appearance,steps:t.steps})}),r={};try{r=await n.json()}catch{}if(!n.ok||!r.ok)throw Error(r.message||`Could not apply the ${t.name} template (${n.status})`)}async function nf(e,t){let n=document.getElementById(`mag-new-btn`),r=document.getElementById(`mag-template-btn`);n&&(n.disabled=!0),r&&(r.disabled=!0);let i=null;try{let n=await ef(e);if(i=n.guid,t)await tf(n.guid,t),await $d(),n=H.magnets.find(e=>e.guid===n.guid)||n;else{H.magnets=[n,...H.magnets];let e=document.getElementById(`mag-body`);e&&(e.innerHTML=`<div class="mag-grid">${H.magnets.map(Xd).join(``)}</div>`)}Ef(n,void 0,{freshDraft:!0})}catch(e){i&&fetch(`${G()}/m5t/v5/magnets/${encodeURIComponent(i)}`,{method:`DELETE`}).catch(()=>{}),W(e.message||`Could not start a new magnet`,`danger`)}finally{n&&(n.disabled=!1),r&&(r.disabled=!1)}}function rf(e){let t=document.createElement(`div`);t.className=`mag-modal-scrim`,t.innerHTML=`
    <div class="mag-modal" role="dialog" aria-modal="true" aria-label="Choose client" style="max-width:420px">
    ...
    </div>`,document.body.appendChild(t),requestAnimationFrame(()=>{t.classList.add(`open`),t.querySelector(`.mag-modal`).classList.add(`open`)});let n=()=>{t.classList.remove(`open`),setTimeout(()=>t.remove(),180),document.removeEventListener(`keydown`,r)};function r(e){e.key===`Escape`&&(e.preventDefault(),n())}document.addEventListener(`keydown`,r),t.querySelector(`.mag-modal__close`).addEventListener(`click`,n),t.querySelector(`[data-x="cancel"]`).addEventListener(`click`,n),t.querySelector(`[data-x="continue"]`).addEventListener(`click`,()=>{let r=t.querySelector(`#mag-owner-pick`).value;n(),nf(r,e)})}function af(){return H.owners.length===1?H.owners[0].guid:null}function of(){if(H.owners.length>1){rf();return}nf(af())}function sf(e){if(H.owners.length>1){rf(e);return}nf(af(),e)}function cf(){cd(sf)}function lf(e){let t=e.target.closest(`[data-action]`);if(!t)return;let n=t.dataset.guid,r=H.magnets.find(e=>e.guid===n),i=t.dataset.action;if(i===`copy`)return uf(n);if(r){if(i===`preview`)return Zu(()=>Gu(r));if(i===`funnel`)return ff(r);if(i===`edit`)return Ef(r);if(i===`flow`)return Ef(r,`flow`);if(i===`allow-domain`)return pf(r,t.dataset.domain);if(i===`dismiss-domain`)return mf(r,t.dataset.domain);if(i===`restore-domain`)return hf(r,t.dataset.domain);if(i===`pair-start`)return Kd(r);if(i===`copy-code`)return qd(r);if(i===`revoke-connection`)return Yd(r,t.dataset.conn);if(i===`clear-connection`)return Jd(r,t.dataset.conn,t.dataset.label);if(i===`toggle`)return _f(r);if(i===`delete`)return vf(r)}}async function uf(e){try{await navigator.clipboard.writeText(Af(e)),W(`Embed snippet copied`,`success`)}catch{W(`Copy failed`,`danger`)}}function df(e){let t=Number(e?.total_steps)||0,n={};if((Array.isArray(e?.funnel)?e.funnel:[]).forEach(e=>{n[e.position]=Number(e.reached)||0}),!t||!Object.keys(n).length)return`<p class="mag-modal__hint" style="margin:0">No step activity yet in this window. As visitors answer questions, drop-off by step will appear here.</p>`;let r=Math.max(1,n[1]||0),i=[];for(let e=1;e<=t;e++){let a=n[e]||0,o=Math.min(100,Math.round(a/r*100));i.push(`
      ...
      </div>`)}return`<div class="mag-funnel">${i.join(``)}</div>`}async function ff(e){let t=document.createElement(`div`);t.className=`mag-modal-scrim`,t.innerHTML=`
    ...
    <div class="mag-modal" role="dialog" aria-modal="true" aria-label="${i?`Edit magnet`:`New magnet`}">
      ...
      <div class="mag-modal__main">
        ...
        <select class="mag-modal__select" id="mag-f-trigger">
          ${Lf.map(t=>`<option value="${t.value}"${vd(e)===t.value?` selected`:``}>${U(t.label)}</option>`).join(``)}
        ...
        <div class="mag-perdev" id="mag-f-perdev"${vd(e)===`__per_device`?``:` hidden`}>
          <div class="mag-perdev__row">
            ...
            <select class="mag-modal__select" id="mag-f-trigger-desktop">
              ${Lf.map(t=>`<option value="${t.value}"${yd(e,`desktop`)===t.value?` selected`:``}>${U(t.label)}</option>`).join(``)}
            </select>
          </div>
          <div class="mag-perdev__row">
            <label class="mag-modal__label" for="mag-f-trigger-mobile">Mobile</label>
            <select class="mag-modal__select" id="mag-f-trigger-mobile">
              ${Lf.map(t=>`<option value="${t.value}"${yd(e,`mobile`)===t.value?` selected`:``}>${U(t.label)}</option>`).join(``)}
            </select>
          </div>
        ...
        <div class="mag-modal__sec">
          ...
          <select class="mag-modal__select" id="mag-f-font">${Tf(e?.font||``)}</select>
...
</details>`,Mf=e=>{let t=U(Af(e));return`<details class="mag-guide">
...
/&gt;</code></pre>
...
</details>`},Nf={chat:`Chat bubble`,right_slide:`Slide-in panel`,modal:`Modal`,inline:`Inline`},Pf={chat:`chat`,right_slide:`vertical_split`,modal:`web_asset`,inline:`view_agenda`},Ff=[{value:`inline`,icon:`view_agenda`,label:`Inline`},{value:`chat`,icon:`chat`,label:`Chat`},{value:`right_slide`,icon:`vertical_split`,label:`Slide`},{value:`modal`,icon:`web_asset`,label:`Modal`}],If=[{value:`auto`,icon:`brightness_auto`,label:`Auto`},{value:`sync`,icon:`sync`,label:`Sync`},{value:`light`,icon:`light_mode`,label:`Light`},{value:`dark`,icon:`dark_mode`,label:`Dark`}],Lf=[{value:`on_load`,label:`Immediately on page load`},{value:`on_8_seconds`,label:`After 8 seconds`},{value:`on_scroll_80_percent`,label:`At 80% scroll depth`},{value:`on_exit_intent`,label:`On exit intent (mouse leaves the page)`},{value:`none`,label:`Manual only (visitor clicks the launcher)`}],Rf={book:{label:`Book`,icon:`event`,kind:`book`},chat:{label:`Chat`,icon:`chat`,kind:`flow`},call:{label:`Call`,icon:`phone`,kind:`value`,placeholder:`Phone number`},email:{label:`Email`,icon:`mail`,kind:`value`,placeholder:`Email address`},support:{label:`Support`,icon:`support_agent`,kind:`support`}},zf=[`book`,`chat`,`call`,`email`,`support`],Bf=null,Vf=null,Hf=5e3,Uf={ACTIVE:{label:`Connected`,icon:`check_circle`,tone:`live`},PENDING:{label:`Waiting to connect`,icon:`hourglass_empty`,tone:`pending`},REVOKED:{label:`Disconnected`,icon:`link_off`,tone:`off`},EXPIRED:{label:`Expired`,icon:`timer_off`,tone:`off`}},Wf=512,Gf=new Set([`image/png`,`image/jpeg`,`image/gif`,`image/webp`]),Kf=8388608,qf=[{label:`Web fonts (loaded for you)`,opts:[[`Inter, sans-serif`,`Inter`],[`Poppins, sans-serif`,`Poppins`],[`Montserrat, sans-serif`,`Montserrat`],[`Roboto, sans-serif`,`Roboto`],[`"Open Sans", sans-serif`,`Open Sans`],[`Lato, sans-serif`,`Lato`],[`Nunito, sans-serif`,`Nunito`],[`Raleway, sans-serif`,`Raleway`],[`"Work Sans", sans-serif`,`Work Sans`],[`Oswald, sans-serif`,`Oswald`],[`Merriweather, serif`,`Merriweather (serif)`]]},{label:`System / always available`,opts:[[`system-ui, sans-serif`,`System UI`],[`Georgia, serif`,`Georgia (serif)`],[`"Times New Roman", Times, serif`,`Times (serif)`],[`Arial, Helvetica, sans-serif`,`Arial / Helvetica`],[`Verdana, Geneva, sans-serif`,`Verdana`],[`"Courier New", monospace`,`Courier (mono)`]]}]})),Yf=c({default:()=>op,init:()=>$f,render:()=>Qf});function Xf(){let e=new Map;for(let t of fd)e.set(t.category,(e.get(t.category)||0)+1);return Object.keys(dd).filter(t=>e.has(t)).map(t=>[t,e.get(t)])}function Zf(){if(sd(),ap)return;let e=document.createElement(`style`);e.id=`m5t-templates-page-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),ap=!0}function Qf(){Zf();let e=rp();return`
    <div class="page-shell" data-section="templates">
      ...
      <div class="m5t-chip-row tpl-filters" id="tpl-filters" role="group" aria-label="Filter templates by category">
        ${ip(`all`,`All`,fd.length,`true`)}
        ${Xf().map(([e,t])=>ip(e,dd[e],t,`false`)).join(``)}
      </div>

      <div class="mag-tpl-grid mag-tpl-grid--page" id="tpl-grid">
        ${fd.map(t=>nd(t,{detail:!0,gallery:!0,canUse:e})).join(``)}
      </div>
    </div>`}function $f(){let e=document.getElementById(`tpl-grid`);document.getElementById(`tpl-filters`)?.addEventListener(`click`,t=>{let n=t.target.closest(`[data-cat]`);if(!n)return;n.parentElement.querySelectorAll(`[data-cat]`).forEach(e=>{e.setAttribute(`aria-pressed`,String(e===n))});let r=n.dataset.cat;e?.querySelectorAll(`.mag-tpl-card`).forEach(e=>{e.hidden=r!==`all`&&e.dataset.cat!==r})}),e?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-preview]`);if(t){let e=fd.find(e=>e.id===t.dataset.preview);e&&Zu(()=>Gu({...e.appearance,steps:e.steps,name:e.name}));return}let n=e.target.closest(`[data-tpl]`);n&&(rd(n.dataset.tpl),window.location.hash=`#/magnets`)})}var ep,tp,np,rp,ip,ap,op,sp=o((()=>{D(),$u(),gd(),ep={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},tp=e=>String(e??``).replace(/[&<>"']/g,e=>ep[e]),np=[`provision_magnets`,`manage_magnets`],rp=()=>E.getCapabilities().some(e=>np.includes(e)),ip=(e,t,n,r)=>`<button class="m5t-chip--filter" type="button" data-cat="${tp(e)}" aria-pressed="${r}">${tp(t)}<span class="m5t-chip__count">${n}</span></button>`,ap=!1,op={render:Qf,init:$f}})),cp=c({default:()=>jp,init:()=>mp,render:()=>pp});function lp(){if(Jn(),Op)return;let e=document.createElement(`style`);e.id=`m5t-questionnaires-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Op=!0}function up(){return K.clients.map(e=>{let t=K.responses.filter(t=>t.owner_guid===e.guid),n=t[0]||null;return{guid:e.guid,name:e.caption||e.name||e.guid,latest:n,earlier:Math.max(0,t.length-1),status:n?n.status:`NOT_SENT`}})}function dp(){let e=K.search.trim().toLowerCase();return up().filter(t=>(K.filter===`ALL`||t.status===K.filter)&&(!e||t.name.toLowerCase().includes(e)))}function fp(e){let t=e.status===`SUBMITTED`,n=e.status===`SUBMITTED`?`<button class="qz-act" type="button" data-act="open" data-guid="${Sp(e.latest.guid)}">Read answers</button>`:e.status===`DRAFT`?`<button class="qz-act" type="button" data-act="copy" data-url="${Sp(e.latest.url||``)}">Copy link</button>`:`<a class="qz-act" href="#/clients">Go to client</a>`;return`<tr class="${t?`qz-row--open`:``}"${t?` data-open-guid="${Sp(e.latest.guid)}"`:``}>
    <td><span class="qz-client">${Sp(e.name)}</span>${e.earlier?`<span class="qz-earlier">${e.earlier} earlier</span>`:``}</td>
    <td><span class="qz-badge" data-s="${Sp(e.status)}">${Sp(Dp[e.status]||e.status)}</span></td>
    <td class="qz-when">${Sp(Ap(e,`created_tz`))}</td>
    <td class="qz-when">${Sp(Ap(e,`submitted_tz`))}</td>
    <td class="qz-actcell">${n}</td>
  </tr>`}function pp(){return lp(),`
    <div class="page-shell" data-section="questionnaires">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="qz-body">${kp(`hourglass_empty`,`Loading…`)}</div>
      </div>
    </div>`}async function mp(){await hp()}async function hp(){let e=document.getElementById(`qz-body`);if(e){try{let[e,t]=await Promise.all([fetch(`${Cp()}/m5t/v5/acctGroup`),fetch(`${Cp()}/m5t/v5/m5mQuestionnaireResponse`)]),n=await e.json().catch(()=>({})),r=await t.json().catch(()=>({}));if(!e.ok||!t.ok)throw Error(`load failed`);K.clients=(n.data||[]).filter(e=>e.class===Tp&&![`DELETED`,`ARCHIVED`].includes(e.status)).sort((e,t)=>String(e.caption||``).localeCompare(String(t.caption||``))),K.responses=r.data||[]}catch{e.innerHTML=kp(`error`,`Could not load questionnaires. Please try again.`);return}gp()}}function gp(){let e=document.getElementById(`qz-body`);if(!e)return;if(!K.clients.length){e.innerHTML=kp(`groups`,`No clients yet. Add a client first, then send them a kickoff questionnaire.`);return}let t=up(),n={ALL:t.length};Ep.slice(1).forEach(([e])=>{n[e]=t.filter(t=>t.status===e).length});let r=dp();e.innerHTML=`
    <div class="qz-bar">
      <div class="qz-filter">
        ${Ep.map(([e,t])=>`<button data-filter="${e}" class="${K.filter===e?`active`:``}">${Sp(t)}${n[e]?`<span class="qz-pillnum">${n[e]}</span>`:``}</button>`).join(``)}
      </div>
      <span class="qz-searchwrap">
        <span class="material-symbols-outlined" aria-hidden="true">search</span>
        <input class="qz-search" id="qz-search" type="search" placeholder="Search clients" value="${Sp(K.search)}" aria-label="Search clients" />
      </span>
    ...
    <div class="qz-tablewrap">
      <table class="qz-table">
        ...
        <tbody id="qz-tbody">${r.length?r.map(fp).join(``):`<tr><td colspan="5"><div class="qz-when" style="padding:14px 4px">${K.search.trim()?`No clients match your search.`:`No clients with status “${Sp(Dp[K.filter]||K.filter)}”.`}</div></td></tr>`}</tbody>
      </table>
    </div>`,e.querySelector(`.qz-filter`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-filter]`);t&&(K.filter=t.dataset.filter,gp())}),e.querySelector(`#qz-search`)?.addEventListener(`input`,e=>{K.search=e.target.value,_p()}),e.querySelector(`#qz-tbody`)?.addEventListener(`click`,vp)}function _p(){let e=document.getElementById(`qz-tbody`);if(!e)return;let t=dp();e.innerHTML=t.length?t.map(fp).join(``):`<tr><td colspan="5"><div class="qz-when" style="padding:14px 4px">No clients match your search.</div></td></tr>`;let n=document.querySelector(`.qz-count`);n&&(n.textContent=`${t.length} of ${up().length}`)}function vp(e){let t=e.target.closest(`[data-act="copy"]`);if(t){e.stopPropagation();let n=t.dataset.url;if(!n){wp(`No link on this questionnaire yet.`,`warning`);return}navigator.clipboard?.writeText(n).then(()=>{t.textContent=`Copied!`,setTimeout(()=>{t.textContent=`Copy link`},1500)},()=>wp(`Could not copy the link.`,`error`));return}let n=e.target.closest(`[data-act="open"]`),r=e.target.closest(`[data-open-guid]`),i=n?.dataset.guid||r?.dataset.openGuid;i&&yp(i)}async function yp(e){K.openGuid=e;let t=document.createElement(`div`);t.className=`qz-drawer-scrim`;let n=document.createElement(`aside`);n.className=`qz-drawer`,n.setAttribute(`role`,`dialog`),n.setAttribute(`aria-label`,`Questionnaire answers`),n.innerHTML=`
    ...
    <div class="qz-drawer__body" id="qz-dbody">${kp(`hourglass_empty`,`Loading answers…`)}</div>`,document.body.append(t,n),requestAnimationFrame(()=>{t.classList.add(`open`),n.classList.add(`open`)});let r=()=>{K.openGuid=null,t.classList.remove(`open`),n.classList.remove(`open`),setTimeout(()=>{t.remove(),n.remove()},220),document.removeEventListener(`keydown`,i)};function i(e){e.key===`Escape`&&(e.preventDefault(),r())}document.addEventListener(`keydown`,i),t.addEventListener(`click`,r),n.querySelector(`.qz-drawer__close`).addEventListener(`click`,r);try{let t=await fetch(`${Cp()}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e)}`),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`load failed`);let i=r.data||{},a=K.clients.find(e=>e.guid===i.owner_guid)?.caption||`Client`;n.querySelector(`#qz-dtitle`).textContent=a;let o=i.submitted_tz?`Submitted ${Hn(i.submitted_tz)}`:`Not yet submitted`;n.querySelector(`#qz-dsub`).textContent=i.locked_tz?`${o} · Finalized`:o,n.querySelector(`#qz-dbody`).innerHTML=qn(i)+Wn(i,{emptyLabel:`Not answered`})}catch{n.querySelector(`#qz-dbody`).innerHTML=kp(`error`,`Could not load these answers. Please try again.`)}}var bp,xp,Sp,Cp,wp,Tp,Ep,Dp,K,Op,kp,Ap,jp,Mp=o((()=>{D(),bp=u(f(),1),m(),Zn(),xp={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Sp=e=>String(e??``).replace(/[&<>"']/g,e=>xp[e]),Cp=()=>E.getConfig?.().BASE_URL||``,wp=(e,t=`info`)=>{try{bp.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},Tp=`managed_client`,Ep=[[`ALL`,`All`],[`NOT_SENT`,`Not sent`],[`DRAFT`,`Awaiting response`],[`SUBMITTED`,`Submitted`],[`CANCELLED`,`Cancelled`]],Dp=Object.fromEntries(Ep),K={clients:[],responses:[],filter:`ALL`,search:``,openGuid:null},Op=!1,kp=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,Ap=(e,t)=>e?.latest?.[t]?Hn(e.latest[t]):`—`,jp={render:pp,init:mp}})),Np=c({init:()=>Wp,render:()=>Up});function Pp(e){let t=dm(e);if(!t)return``;try{return new Date(t).toLocaleDateString([],{month:`short`,day:`numeric`,year:`numeric`})}catch{return``}}function Fp(e){let t=dm(e);if(!t)return``;try{let n=Date.now()-new Date(t).getTime();if(n<0)return Pp(e);let r=n/36e5;return r<1?`${Math.max(1,Math.floor(n/6e4))}m ago`:r<24?`${Math.floor(r)}h ago`:r<48?`Yesterday`:Pp(e)}catch{return Pp(e)}}function Ip(){if(rm)return;let e=document.createElement(`style`);e.id=`m5t-leads-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),rm=!0}function Lp(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function Rp(e){return`<select class="leads-status" data-id="${J(e.id)}" data-act="status" onclick="event.stopPropagation()">
    ${am.map(t=>`<option value="${t}" ${e.status===t?`selected`:``}>${t[0]}${t.slice(1).toLowerCase()}</option>`).join(``)}
  </select>`}function zp(e){let t=q.magnetNames[e.magnet_guid]||e.lead_source||`—`,n=e.phone?String(e.phone).replace(/[^\d+]/g,``):``;return`
    <tr class="leads-row${String(e.id)===q.selectedId?` selected`:``}" data-id="${J(e.id)}" tabindex="0" role="button" aria-label="Open lead ${J(lm(e))}">
      <td>
        <span class="leads-who">
          <span class="leads-avatar">${J(um(e))}</span>
          <span><span class="leads-name">${J(lm(e))}</span>${e.company?`<span class="leads-sub">${J(e.company)}</span>`:``}</span>
        </span>
      </td>
      ...
      <td>${q.canManage?Rp(e):`<span class="leads-badge" data-s="${J(e.status)}">${J(e.status)}</span>`}</td>
      <td class="num leads-muted" style="white-space:nowrap" title="${J(Pp(e))}">${fm(e)?`<span class="leads-fresh" title="Captured in the last 24 hours"></span>`:``}${J(Fp(e))}</td>
      <td class="leads-open" aria-hidden="true"><span class="material-symbols-outlined">chevron_right</span></td>
    </tr>`}function Bp(e){let t=q.search.trim().toLowerCase();if(!t)return!0;let n=[e.first_name,e.last_name,e.email,e.company,e.phone,e.product_interest,q.magnetNames[e.magnet_guid],q.book?q.ownerNames[e.owner_guid]:``,e.utm_campaign,e.utm_source].filter(Boolean).join(` `).toLowerCase();return t.split(/\s+/).every(e=>n.includes(e))}function Vp(){return q.leads.filter(e=>(q.filter===`ALL`||e.status===q.filter)&&pm(e)&&Bp(e)&&mm(e))}function Hp(){if(!q.day)return``;let e=q.day;try{e=new Date(`${q.day}T00:00:00Z`).toLocaleDateString([],{month:`short`,day:`numeric`,year:`numeric`,timeZone:`UTC`})}catch{}return`<button class="leads-daychip" type="button" id="leads-dayclear" title="Show every day again">
  ...
  </button>`}function Up(){Ip(),hi();let e=M();return`
    <div class="page-shell" data-section="leads">
      ...
      <div class="dashboard-card" style="margin-bottom:24px">
        ...
        <div class="card-body" id="pf-picker-body">${Lp(`hourglass_empty`,`Loading…`)}</div>
      </div>`:``}
      <div class="page-header">
        <div class="page-header-text">
      ...
      <div class="leads-layout">
        <div class="dashboard-card">
          <div class="card-body" id="leads-body">
            ${Lp(`hourglass_empty`,`Loading leads…`)}
          </div>
        </div>
        <aside class="dashboard-card leads-panel" aria-label="Lead detail">
          <div class="card-body" id="leads-panel-body">${hm()}</div>
        </aside>
      </div>
    </div>`}async function Wp(){document.getElementById(`leads-export`)?.addEventListener(`click`,tm);let e=ji(Ni.LEADS_SEARCH),t=ji(Ni.LEADS_DAY);if((e||t)&&(q.search=e||``,q.day=t||``,q.filter=`ALL`,q.clientFilter=`ALL`,q.shown=im),M()){let e=e=>Number(e||0).toLocaleString();xi(cm(),[{h:`Leads · wk`,td:t=>e(t.leads.this_week)},{h:`Leads · total`,td:t=>e(t.leads.total)},{h:`Compl.`,td:e=>e.completion_rate==null?`—`:`${e.completion_rate}%`}],{emptyMsg:`No client orgs yet. Create one under Clients first.`})}await Gp()}async function Gp(){let e=document.getElementById(`leads-body`);if(e){try{let[e,t]=await Promise.all([fetch(`${cm()}/m5t/v5/m5mLead`),fetch(`${cm()}/m5t/v5/magnets`).catch(()=>null)]),n=await e.json().catch(()=>({}));if(!n.ok)throw Error(n.message||`Failed to load`);if(q.leads=(n.data||[]).filter(e=>![`DELETED`,`ARCHIVED`].includes(e.status)),q.total=Number.isFinite(n.total)?n.total:q.leads.length,q.truncated=q.total>(n.data||[]).length,q.book=M(),q.ownerNames=q.book?Si():{},t){let e=await t.json().catch(()=>({}));q.magnetNames={},(e.magnets||[]).forEach(e=>{q.magnetNames[e.guid]=e.name,e.owner_guid&&e.owner_name&&!q.ownerNames[e.owner_guid]&&(q.ownerNames[e.owner_guid]=e.owner_name)}),q.canManage=!!e.canManage}}catch{e.innerHTML=Lp(`error`,`Could not load your leads. Please try again.`);return}Kp()}}function Kp(){let e=document.getElementById(`leads-body`),t=document.getElementById(`leads-export`);if(!e)return;if(t&&(t.style.display=q.leads.length?``:`none`),!q.leads.length){e.innerHTML=Lp(`inbox`,`No leads captured yet. Publish a magnet and share it to start collecting.`),$p();return}let n=q.leads.filter(pm),r={ALL:n.length};am.forEach(e=>{r[e]=n.filter(t=>t.status===e).length});let i=q.book?[...new Set(q.leads.map(e=>e.owner_guid).filter(Boolean))].map(e=>({guid:e,name:q.ownerNames[e]||e})).sort((e,t)=>e.name.localeCompare(t.name)):[];e.innerHTML=`
    <div class="leads-bar">
      <div class="leads-filter">
        ${[`ALL`,...am].map(e=>`<button data-filter="${e}" class="${q.filter===e?`active`:``}">${e===`ALL`?`All`:e[0]+e.slice(1).toLowerCase()}${r[e]?`<span class="leads-pillnum">${r[e]}</span>`:``}</button>`).join(``)}
      </div>
      ${q.book&&i.length>1?`
    ...
    <div id="leads-tablezone"></div>`,e.querySelector(`.leads-filter`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-filter]`);t&&(q.filter=t.dataset.filter,q.shown=im,Kp())}),e.querySelector(`#leads-clientsel`)?.addEventListener(`change`,e=>{q.clientFilter=e.target.value,q.shown=im,Kp()}),e.querySelector(`#leads-dayclear`)?.addEventListener(`click`,()=>{q.day=``,q.shown=im,Kp()}),e.querySelector(`#leads-search`)?.addEventListener(`input`,e=>{q.search=e.target.value,q.shown=im,qp()}),qp()}function qp(){let e=document.getElementById(`leads-tablezone`),t=document.getElementById(`leads-countlbl`);if(!e)return;let n=Vp(),r=n.slice(0,q.shown),i=q.book?8:7;t&&(t.textContent=`${n.length} of ${q.leads.length}`),e.innerHTML=`
    <div class="leads-tablewrap">
      <table class="leads-table${q.book?` leads-table--book`:``}">
        ...
        <tbody id="leads-tbody">${r.map(zp).join(``)||`<tr><td colspan="${i}"><div class="leads-muted" style="padding:14px 4px">${q.search.trim()?`No leads match your search.`:`No ${q.filter.toLowerCase()} leads.`}</div></td></tr>`}</tbody>
      </table>
    </div>
    ${n.length>q.shown?`<button class="leads-more" type="button" id="leads-more">Show ${Math.min(im,n.length-q.shown)} more (${n.length-q.shown} remaining)</button>`:``}
    ${q.truncated?`<div class="leads-muted" style="padding:10px 4px">Showing your ${q.leads.length.toLocaleString()} most recent leads of ${q.total.toLocaleString()}. Search and filters cover these; older leads aren't included yet.</div>`:``}`,e.querySelector(`#leads-more`)?.addEventListener(`click`,()=>{q.shown+=im,qp()}),e.querySelector(`#leads-tbody`)?.addEventListener(`change`,Jp),e.querySelector(`#leads-tbody`)?.addEventListener(`click`,e=>{if(e.target.closest(`.leads-status, a.leads-contactlink`))return;let t=e.target.closest(`.leads-row`);t&&Yp(t.dataset.id)}),e.querySelector(`#leads-tbody`)?.addEventListener(`keydown`,e=>{if(e.key!==`Enter`&&e.key!==` `)return;let t=e.target.closest(`.leads-row`);t&&e.target===t&&(e.preventDefault(),Yp(t.dataset.id))})}async function Jp(e){let t=e.target.closest(`.leads-status`);if(!t)return;let n=t.dataset.id,r=q.leads.find(e=>String(e.id)===String(n)),i=t.value,a=r?.status;try{if(!(await(await fetch(`${cm()}/m5t/v5/m5mLead/${encodeURIComponent(n)}`,{method:`PUT`,headers:{"Content-Type":`application/json`},body:JSON.stringify({status:i})})).json().catch(()=>({}))).ok)throw Error();r&&(r.status=i),sm(`Marked ${i[0]}${i.slice(1).toLowerCase()}`,`success`)}catch{t.value=a||`NEW`,sm(`Could not update status`,`danger`);return}try{String(n)===q.selectedId&&$p(),q.filter!==`ALL`&&Kp()}catch(e){console.warn(`[leads] re-render after status change failed:`,e)}}function Yp(e){q.selectedId=String(e),document.querySelectorAll(`.leads-row`).forEach(e=>e.classList.toggle(`selected`,e.dataset.id===q.selectedId)),$p()}function Xp(e,t){let n=Array.isArray(e.tags)?e.tags:[];for(let e=n.length-1;e>=0;e--)if(n[e]&&n[e].tag===t&&n[e].value!=null)return String(n[e].value);return``}function Zp(e){let t=Xp(e,`appointment_start`);if(!t)return null;let n=Xp(e,`appointment_status`)||`booked`,r=Xp(e,`appointment_timezone`),i=Xp(e,`appointment_event_type`),a=new Date(t),o=t;if(!Number.isNaN(a.getTime())){let e={weekday:`short`,month:`short`,day:`numeric`,hour:`numeric`,minute:`2-digit`};try{o=new Intl.DateTimeFormat(void 0,r?{...e,timeZone:r}:e).format(a)}catch{o=new Intl.DateTimeFormat(void 0,e).format(a)}}return{when:o,status:n,type:i,start:t}}function Qp(e){let t=Array.isArray(e.tags)?e.tags:[],n=null;for(let e of t)if(!(!e||!e.tag)){if(e.tag===`callrail_call_id`){n={};continue}n&&e.tag.indexOf(`call_`)===0&&e.value!=null&&(n[e.tag]=String(e.value))}if(!n||!n.call_started)return null;let r=n.call_started,i=new Date(r),a=r;Number.isNaN(i.getTime())||(a=new Intl.DateTimeFormat(void 0,{weekday:`short`,month:`short`,day:`numeric`,hour:`numeric`,minute:`2-digit`}).format(i));let o=Number(n.call_duration)||0,s=o?o<60?`${o}s`:`${Math.floor(o/60)}m ${o%60}s`:``;return{when:a,dur:s,source:n.call_source||``,answered:n.call_answered||``,recording:n.call_recording_player||``,started:r}}function $p(){let e=document.getElementById(`leads-panel-body`);if(!e)return;let t=q.leads.find(e=>String(e.id)===q.selectedId),n=document.querySelector(`.leads-layout`);if(n&&(n.dataset.hasSelection=t?`true`:`false`),!t){e.innerHTML=hm();return}let r=q.magnetNames[t.magnet_guid]||t.lead_source||`—`,i=Zp(t),[a,o]=i?_m[i.status]||_m.booked:[``,``],s=Qp(t),c=(e,t)=>t?`<div class="leads-det__field"><span class="leads-det__k">${J(e)}</span><span class="leads-det__v">${J(t)}</span></div>`:``;e.innerHTML=`
    <div class="leads-det__head">
      <span class="leads-avatar leads-det__ava">${J(um(t))}</span>
      <div>
        <div class="leads-det__name">${J(lm(t))}</div>
    ...
    ${q.canManage?`
      ...
      </div>`:``}`,e.querySelector(`[data-action="delete-lead"]`)?.addEventListener(`click`,em)}async function em(e){let t=e.currentTarget?.dataset?.id,n=q.leads.find(e=>String(e.id)===String(t));if(!n)return;let r=lm(n);if(!window.confirm(`Remove ${r} from your leads?\n\nThis takes the lead out of your dashboard, lists and exports. The record is kept in our system, and this address can be captured again if they come back through one of your magnets.`))return;let i=e.currentTarget;i.disabled=!0;try{let e=await fetch(`${cm()}/m5t/v5/m5mLead/${encodeURIComponent(t)}`,{method:`DELETE`}),n=await e.json().catch(()=>({}));if(!e.ok||!n.ok)throw Error(n.error||`HTTP ${e.status}`);q.leads=q.leads.filter(e=>String(e.id)!==String(t)),q.selectedId=null,sm(`${r} removed`,`success`),Kp()}catch{i.disabled=!1,sm(`Could not remove that lead. Please try again.`,`danger`)}}function tm(){let e=Vp();if(!e.length){sm(`Nothing to export`,`info`);return}let t=[`first_name`,`last_name`,`email`,`phone`,`company`,`product_interest`,`lead_source`,`utm_source`,`utm_medium`,`utm_campaign`,`gclid`,`status`],n=e=>`"${String(e??``).replace(/"/g,`""`)}"`,r=[[...q.book?[`client`]:[],...t,`magnet`,`captured`,`appointment`,`last_call`].join(`,`)];for(let i of e){let e=Zp(i),a=Qp(i);r.push([...q.book?[n(q.ownerNames[i.owner_guid]||``)]:[],...t.map(e=>n(i[e])),n(q.magnetNames[i.magnet_guid]||i.lead_source||``),n(Pp(i)),n(e?`${e.start} (${e.status})`:``),n(a?`${a.started} (${a.answered===`no`?`missed`:`answered`})`:``)].join(`,`))}let i=new Blob([r.join(`
`)],{type:`text/csv`}),a=URL.createObjectURL(i),o=document.createElement(`a`);o.href=a,o.download=`leads-${q.filter.toLowerCase()}.csv`,document.body.appendChild(o),o.click(),o.remove(),URL.revokeObjectURL(a),sm(`Exported ${e.length} lead${e.length===1?``:`s`}`,`success`)}var nm,rm,q,im,am,om,J,sm,cm,lm,um,dm,fm,pm,mm,hm,gm,_m,vm=o((()=>{D(),nm=u(f(),1),m(),ki(),Fi(),rm=!1,q={leads:[],magnetNames:{},filter:`ALL`,canManage:!1,book:!1,ownerNames:{},clientFilter:`ALL`,search:``,shown:100,selectedId:null,day:``,total:0,truncated:!1},im=100,am=[`NEW`,`WORKING`,`QUALIFIED`,`UNQUALIFIED`,`CONVERTED`,`EXPORTED`],om={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},J=e=>String(e??``).replace(/[&<>"']/g,e=>om[e]),sm=(e,t=`info`)=>{try{nm.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},cm=()=>E.getConfig?.().BASE_URL||``,lm=e=>[e.first_name,e.last_name].filter(Boolean).join(` `).trim()||e.email||`(no name)`,um=e=>(lm(e).split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``)||`?`).toUpperCase(),dm=e=>e.created?.created_tz||``,fm=e=>{let t=dm(e);if(!t)return!1;try{return Date.now()-new Date(t).getTime()<864e5}catch{return!1}},pm=e=>q.clientFilter===`ALL`||e.owner_guid===q.clientFilter,mm=e=>!q.day||dm(e).slice(0,10)===q.day,hm=()=>Lp(`touch_app`,`Click to see lead information.`),gm=()=>`
  <colgroup>
    <col style="width:22%">
  ...
  </colgroup>`,_m={booked:[`QUALIFIED`,`Booked`],rescheduled:[`WORKING`,`Rescheduled`],canceled:[`CANCELED`,`Canceled`]}})),ym=c({init:()=>Tm,render:()=>wm,tableHTML:()=>Cm});function bm(){if(Fm)return;let e=document.createElement(`style`);e.id=`m5t-analytics-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Fm=!0}function xm(e,t){let n=t>0?Math.round(e/t*100):0;return`<svg class="anl-bar" width="72" height="8" viewBox="0 0 72 8" role="img"
    ...
    </svg>`}function Sm(e,t,n){let r=Lm[e.status]?`<span class="anl-chip">${jm(Lm[e.status])}</span>`:``,i=n&&e.owner_name?`<span class="anl-sub">${jm(e.owner_name)}</span>`:``,a=e.ab_tests&&(e.ab_tests.running||e.ab_tests.completed)?`${km(e.ab_tests.running)} / ${km(e.ab_tests.completed)}`:`—`;return`<tr data-guid="${jm(e.magnet_guid)}">
    <td><a class="anl-name" href="#/magnets">${jm(e.name)}</a>${r}${i}</td>
    <td>${xm(Number(e.leads?.window||0),t)}</td>
    <td>${km(e.leads?.window)}</td>
    <td>${km(e.opens)}</td>
    <td>${Mm(e.engagement_rate)}</td>
    <td>${Mm(e.completion_rate)}</td>
    <td>${Mm(e.lead_rate)}</td>
    <td>${a}</td>
  </tr>`}function Cm(e,t){let n=e.reduce((e,t)=>Math.max(e,Number(t.leads?.window||0)),0);return`<div class="anl-wrap"><table class="anl-table">
    <thead><tr>
      <th>Magnet</th><th></th><th>Leads</th><th>Opens</th>
      <th>Engaged</th><th>Completed</th><th>Leads / Open</th><th>A/B run / done</th>
    </tr></thead>
    <tbody>${e.map(e=>Sm(e,n,t)).join(``)}</tbody>
  </table></div>`}function wm(){return M()?bi({section:`analytics`,title:`Analytics`,description:`Pick a client to see how their magnets are performing.`}):(bm(),`
    <div class="page-shell" data-section="analytics">
      <div class="page-header">
        ...
        <div class="chart-controls" id="anl-periods">
          ${Nm.map(e=>`<button class="chart-period-btn${e.d===Pm?` active`:``}" type="button" data-days="${e.d}">${e.label}</button>`).join(``)}
        </div>
      </div>
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="anl-body">
          ${Im(`hourglass_empty`,`Loading magnet performance…`)}
        </div>
      </div>
    ...
    </div>`)}async function Tm(){if(M())return xi(Dm(),[{h:`Leads`,td:e=>km(e.leads.window??e.leads.total)},{h:`Opens`,td:e=>km(e.opens)},{h:`Completed`,td:e=>Mm(e.completion_rate)}]);bm(),await Em();let e=document.getElementById(`anl-periods`);e&&e.addEventListener(`click`,t=>{let n=t.target.closest(`.chart-period-btn`);n&&(Pm=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>{e.classList.toggle(`active`,Number(e.dataset.days)===Pm)}),Em())})}async function Em(){let e=document.getElementById(`anl-body`),t=document.getElementById(`anl-range`);if(!e)return;e.innerHTML=Im(`hourglass_empty`,`Loading magnet performance…`);let n=new Date,r=Om(n),i=Om(new Date(n.getTime()-(Pm-1)*864e5));t&&(t.textContent=`${i} → ${r}`);let a;try{let e=await fetch(`${Dm()}/m5t/v5/dashboard/stats?from=${i}&to=${r}&group_by=magnet`);if(a=await e.json(),!e.ok||!a?.ok)throw Error(a?.error||`HTTP ${e.status}`)}catch(t){e.innerHTML=Im(`cloud_off`,`Could not load analytics. Refresh to try again.`),console.error(`[analytics] load failed:`,t);return}let o=(e,t)=>{let n=document.getElementById(e);n&&(n.textContent=t)};o(`anl-leads`,km(a.by_day?.reduce((e,t)=>e+Number(t.leads||0),0))),o(`anl-opens`,km(a.funnel?.opens)),o(`anl-completion`,Mm(a.funnel?.completion_rate)),o(`anl-active`,km(a.magnets?.active));let s=Array.isArray(a.by_magnet)?a.by_magnet:[];if(!s.length){e.innerHTML=Im(`widgets`,`No magnets yet. Create one and its numbers will show up here.`);return}let c=s.some(e=>Number(e.opens||0)>0||Number(e.leads?.window||0)>0),l=new Set(s.map(e=>e.owner_guid)).size>1;e.innerHTML=(a.by_magnet_truncated?`<p class="anl-warn">Showing the top ${s.length} magnets by leads. You have more than that, so this list is not complete.</p>`:``)+(c?``:`<p class="anl-warn">No opens or leads for any magnet in this period. Try a longer range.</p>`)+Cm(s,l)}var Dm,Om,km,Am,jm,Mm,Nm,Pm,Fm,Im,Lm,Rm=o((()=>{D(),ki(),zi(),Dm=()=>E.getConfig?.().BASE_URL||``,Om=e=>e.toISOString().slice(0,10),km=e=>Number(e||0).toLocaleString(),Am={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},jm=e=>String(e??``).replace(/[&<>"']/g,e=>Am[e]),Mm=e=>e==null?`—`:`${e}%`,Nm=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],Pm=30,Fm=!1,Im=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,Lm={DRAFT:`Draft`,DISABLED:`Paused`}})),zm=c({default:()=>sh,init:()=>Hm,render:()=>Vm});function Bm(){if(rh)return;let e=document.createElement(`style`);e.id=`m5t-adperf-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),rh=!0}function Vm(){return M()?bi({section:`ad-performance`,title:`Ad Performance`,description:`Pick a client to view their spend and lead performance.`}):(Bm(),`
    <div class="page-shell" data-section="ad-performance">
      <div class="page-header">
        ...
        <div class="chart-controls" id="adperf-periods">
          ${ah.map(e=>`<button class="chart-period-btn${e.d===oh?` active`:``}" type="button" data-days="${e.d}">${e.label}</button>`).join(``)}
        </div>
      </div>
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="adperf-body">
          ${ih(`hourglass_empty`,`Loading ad performance…`)}
        </div>
      </div>
    ...
    </div>`)}async function Hm(){if(M()){let e=e=>Number(e||0).toLocaleString();return xi(Zm(),[{h:`Spend`,td:t=>t.ad?`$${e(t.ad.spend)}`:`—`},{h:`Clicks`,td:t=>t.ad?e(t.ad.clicks):`—`},{h:`Leads`,td:t=>e(t.leads.window??t.leads.total)},{h:`CPL`,td:t=>t.ad?.cost_per_lead==null?`—`:`$${e(t.ad.cost_per_lead)}`}],{emptyMsg:`No client orgs yet. Connect a client ad account to see spend here.`})}let e=document.getElementById(`adperf-periods`);e?.addEventListener(`click`,t=>{let n=t.target.closest(`button[data-days]`);n&&(oh=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>e.classList.toggle(`active`,Number(e.dataset.days)===oh)),Um())}),await Um()}async function Um(){let e=Zm(),t=document.getElementById(`adperf-body`),n=document.getElementById(`adperf-range`),r=new Date,i=Qm(r),a=Qm(new Date(r.getTime()-(oh-1)*864e5));n&&(n.textContent=`${a} → ${i}`);let o,s=[],c=null,l=null,u=null;try{let[t,n,r,d,f]=await Promise.all([fetch(`${e}/m5t/v5/coreAdMetrics/overlay?from=${a}&to=${i}`),fetch(`${e}/m5t/v5/coreAdAccount`),fetch(`${e}/m5t/v5/coreAdMetrics/by-source?from=${a}&to=${i}`),ds().catch(()=>null),fetch(`${e}/m5t/v5/coreAdMetrics/conversions?from=${a}&to=${i}`).then(e=>e.json()).catch(()=>null)]);if(!t.ok)throw Error(`HTTP ${t.status}`);o=await t.json(),s=((await n.json().catch(()=>({})))?.data||[]).filter(e=>e.status!==`DELETED`),c=await r.json().catch(()=>null),l=d,u=f}catch(e){console.warn(`[ad-performance] load failed:`,e),t&&(t.innerHTML=ih(`error`,`Could not load ad performance. Please try again.`));return}Wm(l,s),Gm(u);let d=o.totals||{};if(qm(`adperf-spend`,$m(d.spend)),qm(`adperf-clicks`,eh(d.clicks)),qm(`adperf-leads`,eh(d.magnet_leads)),qm(`adperf-cpl`,$m(d.cost_per_lead)),Km(c),!s.length){t.innerHTML=ih(`link_off`,`No ad account connected yet. Your agency can connect Google Ads or Local Services Ads to see performance here.`);return}if(!o.connected){t.innerHTML=ih(`cloud_sync`,`Account connected. No ad data for this range yet. Metrics appear here once they sync.`);return}t.innerHTML=Xm(o.rows||[])}function Wm(e,t){let n=document.getElementById(`adperf-conn`);if(!n)return;if(!e||e.denied){n.innerHTML=``;return}let r=Ps.google_ads,i=cs(e.providers,`google_ads`),a=ls(r,i,!!e.configured?.google_ads),o=Os()?` <a href="#/settings/integrations/${r.urlSlug}">Manage</a>`:``;if(a===`connected`||a===`broken`){Cs(),n.innerHTML=`<span>Google Ads</span> ${us(a,i)}${o}`;return}if(t.length){n.innerHTML=`<span>Google Ads</span> <span>${t.length} associated account${t.length===1?``:`s`}</span>`;return}n.innerHTML=``}function Gm(e){let t=document.getElementById(`adperf-conv-card`),n=document.getElementById(`adperf-conv-body`),r=document.getElementById(`adperf-stats`),i=document.getElementById(`adperf-conv-stat`);if(i&&i.remove(),!t||!n)return;if(!e?.ok||!e.has_selection){t.style.display=`none`;return}if(r){let t=document.createElement(`div`);t.innerHTML=Ii({icon:`flag`,label:`Ads Conversions`,value:eh(e.totals?.conversions),sub:`Reported by Google Ads`,href:`#/settings/integrations/google-ads`});let n=t.firstElementChild;n.id=`adperf-conv-stat`,r.appendChild(n)}let a=e.actions||[];if(t.style.display=``,!a.length){n.innerHTML=ih(`search_off`,`No conversions in this range for the selected actions.`);return}let o=a.some(e=>e.value>0);n.innerHTML=`
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
        ...
        <tbody>${a.map(e=>`
          <tr>
            <td>${nh(e.name||e.id)}</td>
            <td class="adperf-src-sub">${nh(String(e.category||``).toLowerCase().replace(/_/g,` `)||`—`)}</td>
            <td class="num"><b>${eh(e.conversions)}</b></td>
            ${o?`<td class="num">${$m(e.value)}</td>`:``}
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>
    <p class="adperf-note">Counted actions are chosen on the Google Ads integration page. These are Google's own numbers and may overlap magnet leads, so they are never added together.</p>`}function Km(e){let t=document.getElementById(`adperf-sources`),n=document.getElementById(`adperf-src-summary`);if(!t)return;let r=e&&Array.isArray(e.rows)?e.rows:[];if(!r.length){n&&(n.textContent=``),t.innerHTML=ih(`search_off`,`No leads in this range yet. Campaigns appear here as tagged leads (?utm_campaign=…) come in.`);return}let i=e.total_leads||r.reduce((e,t)=>e+(t.leads||0),0),a=e.tracked_leads||0;n&&(n.textContent=`${eh(a)} of ${eh(i)} leads tagged to a source`);let o=e=>e.campaign&&e.campaign!==`(none)`?nh(e.campaign):e.source&&e.source!==`(direct)`?nh(e.source):`<span class="adperf-src-untracked">Untracked / direct</span>`;t.innerHTML=`
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
        ...
        <tbody>${r.map(e=>`
          <tr>
            <td>${o(e)}</td>
            <td class="adperf-src-sub">${nh(e.source||`—`)}${e.medium?` · ${nh(e.medium)}`:``}</td>
            <td class="num">${e.paid_clicks?eh(e.paid_clicks):`<span class="adperf-src-sub">—</span>`}</td>
            <td class="num"><b>${eh(e.leads)}</b></td>
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>`}function qm(e,t){let n=document.getElementById(e);n&&(n.textContent=t)}function Jm(e){if(e<=0)return 1;let t=10**Math.floor(Math.log10(e)),n=e/t;return(n<=1?1:n<=2?2:n<=5?5:10)*t}function Ym(e){return e=Math.round(e),e>=1e3?(e/1e3).toFixed(e%1e3==0?0:1)+`k`:String(e)}function Xm(e){let t=e.length||1,n=Jm(Math.max(1,...e.map(e=>e.spend||0))),r=Jm(Math.max(1,...e.map(e=>e.magnet_leads||0))),i=738/t,a=Math.min(24,i*.55),o=e=>56+i*(e+.5),s=e=>304-e/n*282,c=e=>304-e/r*282,l=[0,.25,.5,.75,1].map(e=>{let t=304-e*282;return`<line x1="56" y1="${t.toFixed(1)}" x2="794" y2="${t.toFixed(1)}" stroke="#EAF0F7"/><text x="48" y="${(t+3.5).toFixed(1)}" text-anchor="end" class="ax">$${Ym(n*e)}</text><text x="802" y="${(t+3.5).toFixed(1)}" text-anchor="start" class="ax">${Ym(r*e)}</text>`}).join(``),u=e.map((e,t)=>{let n=s(e.spend||0);return`<rect x="${(o(t)-a/2).toFixed(1)}" y="${n.toFixed(1)}" width="${a.toFixed(1)}" height="${Math.max(0,304-n).toFixed(1)}" rx="3" fill="#0072BA"><title>${e.date}: $${e.spend} spend</title></rect>`}).join(``),d=e.map((e,t)=>`${o(t).toFixed(1)},${c(e.magnet_leads||0).toFixed(1)}`).join(` `),f=t>1?`<polyline points="${d}" fill="none" stroke="#009473" stroke-width="2.5"/>`:``,p=e.map((e,t)=>`<circle cx="${o(t).toFixed(1)}" cy="${c(e.magnet_leads||0).toFixed(1)}" r="3.5" fill="#009473"><title>${e.date}: ${e.magnet_leads} leads</title></circle>`).join(``),m=Math.max(1,Math.ceil(t/6));return`
    ...
    </svg>`}var Zm,Qm,$m,eh,th,nh,rh,ih,ah,oh,sh,ch=o((()=>{D(),ki(),zs(),zi(),Zm=()=>E.getConfig?.().BASE_URL||``,Qm=e=>e.toISOString().slice(0,10),$m=e=>e==null?`—`:`$`+Number(e).toLocaleString(void 0,{minimumFractionDigits:0,maximumFractionDigits:2}),eh=e=>Number(e||0).toLocaleString(),th={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},nh=e=>String(e??``).replace(/[&<>"']/g,e=>th[e]),rh=!1,ih=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,ah=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],oh=30,sh={render:Vm,init:Hm}})),lh=c({default:()=>Ah,init:()=>fh,parseBookingCsv:()=>Ch,render:()=>dh});function uh(){if(Dh)return;let e=document.createElement(`style`);e.id=`m5t-adacct-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Dh=!0}function dh(){return M()?bi({section:`ad-accounts`,title:`Ad Accounts`,description:`Pick a client to see their connected ad accounts.`}):(uh(),Cs(),`
...
`&&a++,n.push(r),r=``,n.some(e=>e.trim()!==``)&&t.push(n),n=[]):r+=o}if(n.push(r),n.some(e=>e.trim()!==``)&&t.push(n),t.length<2)return[];let a=t[0].map(e=>e.trim().toLowerCase()),o=a.findIndex(e=>/e-?mail/.test(e)),s=a.findIndex(e=>/phone|mobile|tel/.test(e)),c=a.findIndex(e=>/date|time|booked|appointment|start/.test(e));return o<0&&s<0?[]:t.slice(1).map(e=>({email:o>=0?(e[o]||``).trim():``,phone:s>=0?(e[s]||``).trim():``,at:c>=0?(e[c]||``).trim():``})).filter(e=>e.email||e.phone).slice(0,5e3)}function wh(e,t){let n=new Blob([e],{type:`text/csv;charset=utf-8`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.href=r,i.download=t,document.body.appendChild(i),i.click(),i.remove(),setTimeout(()=>URL.revokeObjectURL(r),4e3)}var Th,Eh,Dh,Oh,kh,Ah,jh=o((()=>{ki(),zs(),Th={google_ads:`Google Ads`,google_lsa:`Local Services Ads`},Eh=e=>{let t=String(e||``).replace(/\D/g,``);return t.length===10?`${t.slice(0,3)}-${t.slice(3,6)}-${t.slice(6)}`:String(e||``)},Dh=!1,Oh=[],kh=null,Ah={render:dh,init:fh}})),Mh=c({default:()=>eg,init:()=>Bh,openInviteModal:()=>Wh,render:()=>zh});function Nh(e,t=`info`){try{qh.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}}function Ph(){let e=E.getActiveOrg();if(e&&e.channel===Jh)return{guid:e.guid,name:e.name};let t=E.getMemberships().find(e=>e.channel===Jh);return t?{guid:t.orgGuid,name:t.name}:null}function Fh(e){let t=e?.created_tz;if(!t)return``;let n=new Date(t);return Number.isNaN(n.getTime())?``:n.toLocaleDateString(void 0,{year:`numeric`,month:`short`,day:`numeric`})}function Ih(){if(Xh)return;let e=document.createElement(`style`);e.id=`m5t-clients-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Jn(),Xh=!0}function Lh(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function Rh(e){let t=e.caption||e.guid,n=e.status||`ENABLED`;return`
    <div class="client-row">
      <span class="client-row__avatar">${Y(Qh(t))}</span>
      <span class="client-row__main">
        <div class="client-row__name" title="${Y(t)}">${Y(t)}</div>
        <div class="client-row__meta">Added ${Y(Fh(e.created))||`—`}</div>
      </span>
    ...
    <div class="page-shell" data-section="clients">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="clients-body">
          ${Lh(`hourglass_empty`,`Loading clients…`)}
        </div>
      </div>
    </div>`}async function Bh(){let e=Ph();document.getElementById(`clients-add-btn`)?.addEventListener(`click`,()=>Hh(e)),await Vh(e)}async function Vh(e){let t=document.getElementById(`clients-body`),n=document.getElementById(`clients-count`);if(t){if(!e?.guid){t.innerHTML=Lh(`info`,`Client provisioning is available to reseller / agency accounts.`),n&&(n.textContent=``);return}try{let r=E.getConfig?.().BASE_URL||``,i=await fetch(`${r}/m5t/v5/acctGroup`);if(!i.ok)throw Error(`HTTP ${i.status}`);let a=await i.json(),o=(Array.isArray(a?.data)?a.data:[]).filter(t=>t.class===Yh&&(!t.parent||t.parent===e.guid));if(!o.length){t.innerHTML=Lh(`group_off`,`No clients yet. Add your first client to start managing magnets on their behalf.`),n&&(n.textContent=``);return}t.innerHTML=`<div class="client-list">${o.map(Rh).join(``)}</div>`,n&&(n.textContent=`${o.length} client${o.length===1?``:`s`}`),t.querySelectorAll(`.client-card__invite`).forEach(e=>{e.addEventListener(`click`,()=>{e.dataset.action===`adaccounts`?Uh(e.dataset.clientGuid,e.dataset.clientName):e.dataset.action===`questionnaire`?Kh(e.dataset.clientGuid,e.dataset.clientName):Wh(e.dataset.clientGuid,e.dataset.clientName)})})}catch(e){console.warn(`[clients] load failed:`,e),t.innerHTML=Lh(`error`,`Couldn’t load clients. Please try again.`),n&&(n.textContent=``)}}}function Hh(e){if(!e?.guid){Nh(`Only reseller / agency accounts can add clients.`,`error`);return}let t=document.createElement(`div`);t.className=`client-modal-scrim`;let n=document.createElement(`div`);n.className=`client-modal`,n.setAttribute(`role`,`dialog`),n.setAttribute(`aria-modal`,`true`),n.innerHTML=`
    ...
    <div class="client-modal__body" id="qm-body">${Lh(`hourglass_empty`,`Loading…`)}</div>`,document.body.appendChild(r),document.body.appendChild(i),requestAnimationFrame(()=>{r.classList.add(`open`),i.classList.add(`open`)});let a=()=>{r.classList.remove(`open`),i.classList.remove(`open`),document.removeEventListener(`keydown`,o),setTimeout(()=>{r.remove(),i.remove()},200)},o=e=>{e.key===`Escape`&&a()};document.addEventListener(`keydown`,o),r.addEventListener(`click`,a),i.querySelector(`.client-modal__close`)?.addEventListener(`click`,a);let s=i.querySelector(`#qm-body`);function c(){s.innerHTML=`
      ...
      </div>`,s.querySelector(`[data-act="done"]`)?.addEventListener(`click`,a),s.querySelector(`[data-act="resend"]`)?.addEventListener(`click`,()=>c())}function d(e,t=!1){let r=e.editability||{editable:e.status===`SUBMITTED`&&!e.locked_tz,reason:e.locked_tz?`finalized`:null,deadline:null},i=!!r.editable,o=!i&&e.status===`SUBMITTED`?r.reason===`finalized`?`<div class="qm-status" data-status="LOCKED"><span class="material-symbols-outlined" aria-hidden="true">lock</span> Finalized${e.locked_tz?` ${Y(Gh(e.locked_tz))}`:``}${e.locked_by_email?` by ${Y(e.locked_by_email)}`:``} — no further edits.</div>`:`<div class="qm-status" data-status="LOCKED"><span class="material-symbols-outlined" aria-hidden="true">lock_clock</span> Editing closed${r.deadline?` ${Y(Gh(r.deadline))}`:``} — a response locks 14 days after it is submitted.</div>`:``,c=i&&r.deadline?`<p class="client-modal__sub qm-editable-hint">You can edit these answers until ${Y(Gh(r.deadline))}.</p>`:``;s.innerHTML=`
      <div class="qm-status" data-status="SUBMITTED"><span class="material-symbols-outlined" aria-hidden="true">task_alt</span> Submitted ${e.submitted_tz?Y(Gh(e.submitted_tz)):``}</div>
      ${o}
      ${qn(e)}
      <form id="qm-form">
      ...
      </div>`,s.querySelector(`[data-act="done"]`)?.addEventListener(`click`,a),s.querySelector(`[data-act="cancel-edit"]`)?.addEventListener(`click`,()=>d(e,!1)),s.querySelector(`[data-act="edit"]`)?.addEventListener(`click`,()=>d(e,!0));let l=s.querySelector(`[data-act="save"]`);l?.addEventListener(`click`,async()=>{let t=Kn(s);l.disabled=!0;let r=l.innerHTML;l.innerHTML=`<span class="material-symbols-outlined" aria-hidden="true">progress_activity</span> Saving…`;let i;try{i=await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e.guid)}`,{method:`PUT`,headers:{"Content-Type":`application/json`},body:JSON.stringify({answers:t})});let r=await i.json().catch(()=>({}));if(!i.ok||!r.ok)throw Error(r.error||r.message||`Failed (HTTP ${i.status})`);Nh(`Answers updated`,`success`);let a=await(await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e.guid)}`)).json().catch(()=>({}));d(a.ok?a.data:{...e,answers:t},!1)}catch(t){if(i&&i.status===423){Nh(t.message||`This questionnaire is now locked.`,`info`);let r=await(await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e.guid)}`)).json().catch(()=>({}));d(r.ok?r.data:e,!1);return}s.querySelector(`#qm-error`).textContent=t.message||`Something went wrong. Please try again.`,l.disabled=!1,l.innerHTML=r}})}try{let t=await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse?owner_guid=${encodeURIComponent(e)}`),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`HTTP ${t.status}`);let i=(r.data||[])[0]||null;if(!i){c();return}if(i.status===`DRAFT`){l(i);return}if(i.status===`CANCELLED`){u(i);return}let a=await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(i.guid)}`),o=await a.json().catch(()=>({}));if(!a.ok||!o.ok)throw Error(o.error||`HTTP ${a.status}`);d(o.data)}catch(e){console.warn(`[clients] questionnaire load failed:`,e),s.innerHTML=`<p class="client-modal__error">Could not load questionnaire status. ${Y(e.message||``)}</p>`}}var qh,Jh,Yh,Xh,Zh,Y,Qh,$h,eg,tg=o((()=>{D(),qe(),qh=u(f(),1),m(),Zn(),Jh=`reseller_org`,Yh=`managed_client`,Xh=!1,Zh={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Y=e=>String(e??``).replace(/[&<>"']/g,e=>Zh[e]),Qh=e=>String(e||`?`).trim().split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``).toUpperCase()||`?`,$h=e=>String(e||``).replace(/[^0-9]/g,``),eg={render:zh,init:Bh}})),ng=c({default:()=>bg,init:()=>dg,loadTeam:()=>fg,render:()=>ug});function rg(){let e=E.getActiveOrg(),t=e&&e.channel!==hg?e:null;if(!t){let e=E.getMemberships().find(e=>e.channel!==hg);e&&(t={guid:e.orgGuid,channel:e.channel,memberRole:e.memberRole})}if(!t)return null;let n=E.getMemberships().find(e=>e.orgGuid===t.guid)?.name;return{...t,name:n}}function ig(e){return Object.values(je).filter(t=>Ae(e,t.value)).sort((e,t)=>t.rank-e.rank)}function ag(){if(gg)return;let e=document.createElement(`style`);e.id=`m5t-team-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),gg=!0}function og(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function sg(e,t,n){let r=e.member_role||`member`,i=e.entity_guid&&e.entity_guid===E.getEntity?.()?.guid,a=n&&!i&&ig(t.memberRole).some(e=>e.value===r);return`
    <div class="team-row">
      <span class="team-avatar">${X(yg(e.name||e.email))}</span>
    ...
    </div>`}async function cg(e,t){let{uid:n,email:r}=e.dataset;if(window.confirm(`Revoke the invitation for ${r}?\n\nThe invitation link stops working immediately, including any copy already sent. You can invite this address again afterwards.`)){e.disabled=!0;try{let e=E.getConfig?.().BASE_URL||``,i=await fetch(`${e}/m5t/v5/acctGroup/${encodeURIComponent(t.guid)}/invites/${encodeURIComponent(n)}`,{method:`DELETE`}),a=await i.json().catch(()=>({}));if(!i.ok||!a.ok)throw Error(a.message||a.error||`HTTP ${i.status}`);vg(`Invitation for ${r} revoked`,`success`)}catch(t){e.disabled=!1,vg(t.message||`Could not revoke that invitation.`,`error`);return}try{await fg(t)}catch{}}}async function lg(e,t){let{id:n,name:r}=e.dataset;if(window.confirm(`Remove ${r} from the team?\n\nThey lose access to this workspace immediately. Their account itself is not deleted, and you can invite them again later.`)){e.disabled=!0;try{let e=E.getConfig?.().BASE_URL||``,t=await fetch(`${e}/m5t/v5/acctGroupMember/${encodeURIComponent(n)}`,{method:`DELETE`}),i=await t.json().catch(()=>({}));if(!t.ok||!i.ok)throw Error(i.message||i.error||`HTTP ${t.status}`);vg(`${r} removed from the team`,`success`)}catch(t){e.disabled=!1,vg(t.message||`Could not remove that member.`,`error`);return}try{await fg(t)}catch{}}}function ug(){return ag(),`
    <div class="page-shell" data-section="team">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="team-body">
          ${og(`hourglass_empty`,`Loading team…`)}
        </div>
      </div>
    </div>`}async function dg(){let e=rg(),t=document.getElementById(`team-invite-btn`);if(!e){t&&(t.style.display=`none`),document.getElementById(`team-body`).innerHTML=og(`info`,`Team management is available for agency and workspace accounts.`);return}t?.addEventListener(`click`,()=>pg(e)),await fg(e)}async function fg(e){let t=document.getElementById(`team-body`),n=document.getElementById(`team-count`);if(t)try{let r=E.getConfig?.().BASE_URL||``,[i,a]=await Promise.all([fetch(`${r}/m5t/v5/acctGroup/${encodeURIComponent(e.guid)}/members`),fetch(`${r}/m5t/v5/acctGroup/${encodeURIComponent(e.guid)}/invites`)]),[o,s]=await Promise.all([i.json().catch(()=>({})),a.json().catch(()=>({}))]);if(!i.ok||!a.ok)throw Error(`Failed to load team (members: ${i.status}, invites: ${a.status})`);let c=o.members||[],l=s.invites||[],u=ig(e.memberRole).length>0;t.innerHTML=(l.length?`
    ...
    <div class="team-modal__body" id="team-invite-body">
      ...
      <div class="team-modal__actions">
        ...
        </div>`;let u=r.querySelector(`#tm-link`);r.querySelector(`#tm-copy`)?.addEventListener(`click`,async()=>{try{await navigator.clipboard.writeText(u.value),r.querySelector(`#tm-copy`).textContent=`Copied!`}catch{u.select()}}),r.querySelector(`[data-act="done"]`)?.addEventListener(`click`,()=>{i(),fg(e)})}catch(e){console.warn(`[team] invite failed:`,e),o.textContent=e.message||`Something went wrong. Please try again.`,s.disabled=!1,s.innerHTML=l}})}var mg,hg,gg,_g,X,vg,yg,bg,xg=o((()=>{D(),mg=u(f(),1),m(),qe(),hg=`managed_client`,gg=!1,_g={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},X=e=>String(e??``).replace(/[&<>"']/g,e=>_g[e]),vg=(e,t=`info`)=>{try{mg.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},yg=e=>String(e||`?`).trim().split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``).toUpperCase()||`?`,bg={render:ug,init:dg}}));m(),ye(),D(),It(),y(),fn(),yn(),Ze(),window.m5t=Object.create(null),window.m5t.pubsub=b.default,console.log(`StreamSyncEngage SaaS Client - Starting...`),console.log(`Environment:`,`production`),console.log(`Document readyState:`,document.readyState),console.log(`window.mvvLegit:`,typeof window.mvvLegit),console.log(`globalThis.mvvLegit:`,typeof globalThis.mvvLegit);try{console.log(`mvvLegit (direct):`,typeof mvvLegit)}catch(e){console.log(`mvvLegit (direct): ReferenceError -`,e.message)}var Sg=Object.keys(window).filter(e=>e.toLowerCase().includes(`mvv`)||e.toLowerCase().includes(`neodigm`)||e.toLowerCase().includes(`legit`));console.log(`Possible Neodigm globals:`,Sg);var Z=new ee.default(`/`,{hash:!0});window.router=Z;function Q(e){let t=e?`${e.replace(/-/g,`_`)}_route`:`splash_route`;return console.log(`[Router] Checking access: ${e} -> ${t}`),mvvLegit.isRouteAllowed(t)?!0:(console.warn(`[Router] Access denied to route: ${t}`),b.default.publish(p.UI_TOAST,{message:`Please sign in to access this page`,type:`warning`}),!1)}function $(e){console.log(`[Router] Navigating to:`,e),Z.navigate(`/${e}`)}var Cg=new Set([`splash`,`signin`,`signup`,`forgot`,`resethash`,`verf-link`,`accept-invite`,`signout`]),wg={},Tg={},Eg=null,Dg={admin:{caption:`Platform Admin`,icon:`shield_person`},reseller:{caption:`Reseller`,icon:`handshake`},managed:{caption:`Managed`,icon:`visibility`},self_serve:{caption:`Self-Serve`,icon:`rocket_launch`}},Og=`__portfolio__`;function kg(){try{Z._setCurrent(null)}catch{}Z.resolve()}function Ag(){let e=document.getElementById(`m5t-side-nav`);if(!e)return;let t=!E.getScope()&&E.isAuthenticated(),n=E.getCapabilities(),r=E.getUiMode(),i=Vt.getSideNav(n,{uiMode:r});wg={},Tg={};let a=e=>{e.route&&(wg[e.route]={sectionToken:e.token,childToken:null,children:e.children||null},Array.isArray(e.children)&&e.children.forEach(t=>{t.route&&(Tg[t.token]=t.route,wg[t.route]={sectionToken:e.token,childToken:t.token,children:e.children})}))},o=e=>e.forEach(e=>{e.group&&Array.isArray(e.items)?o(e.items):a(e)});o(i);let s=E.getMemberships(),c=E.getActiveOrg(),l=E.getManagedClients(),u=E.getStoredActiveOrg(),d=l.length?[{guid:Og,name:`Portfolio (all clients)`},...s.map(e=>({guid:e.orgGuid,name:e.name||e.channel,channel:e.channel,group:`Your organization`})),...l.map(e=>({guid:e.orgGuid,name:e.name||e.orgGuid,channel:e.channel,group:`Clients`}))]:s.map(e=>({guid:e.orgGuid,name:e.name||e.channel,channel:e.channel}));e.setState({buttons:i,orgs:d,activeOrg:l.length?u||Og:c?.guid||null,footerBadge:Dg[r]||null,loading:t}),Mg(),Eg&&Ng(Eg)}var jg=e=>String(e??``).replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`})[e]);function Mg(){let e=document.getElementById(`app-content`);if(!e)return;let t=document.getElementById(`org-context-banner`);t||(t=document.createElement(`div`),t.id=`org-context-banner`,e.insertBefore(t,e.firstChild),nn(t));let n=E.getActiveOrg(),r=E.isPreviewingAsClient(),i=!!n?.viaSubtree&&!!Eg&&!Cg.has(Eg);if(t.hidden=!i,!i){t.innerHTML=``;return}let a=jg(n.name||`Client`);t.innerHTML=`
    <style>
