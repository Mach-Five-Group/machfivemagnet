...
/&gt;</code></pre>
  ...
  `,document.head.appendChild(e),n_=!0}function Yg(e,t){return!e.url&&t.key===`PENDING`?``:t.key===`PENDING`?`<button class="dc-act" type="button" data-url="${e_(e.url)}">Review</button>`:t.key!==`APPROVED`&&e.url&&e.editability?.editable?`<button class="dc-act" type="button" data-url="${e_(e.url)}">Update your review</button>`:``}function Xg(e){let t=qg(e);return`<tr>
    ...
    </div>`,e.querySelector(`#dc-tbody`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-url]`);t?.dataset.url&&window.open(t.dataset.url,`_blank`)})}var $g,e_,t_,n_,r_,i_,a_=o((()=>{T(),Kr(),x(),no(),$g={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},e_=e=>String(e??``).replace(/[&<>"']/g,e=>$g[e]),t_=()=>w.getConfig?.().BASE_URL||``,n_=!1,r_=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,i_={render:Zg,init:Qg}})),o_=c({init:()=>x_,render:()=>b_});function s_(e){let t=W_(e);if(!t)return``;try{return new Date(t).toLocaleDateString([],{month:`short`,day:`numeric`,year:`numeric`})}catch{return``}}function c_(e){let t=W_(e);if(!t)return``;try{let n=Date.now()-new Date(t).getTime();if(n<0)return s_(e);let r=n/36e5;return r<1?`${Math.max(1,Math.floor(n/6e4))}m ago`:r<24?`${Math.floor(r)}h ago`:r<48?`Yesterday`:s_(e)}catch{return s_(e)}}function l_(){if(I_)return;let e=document.createElement(`style`);e.id=`m5t-leads-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),I_=!0}function u_(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function d_(e){return`<select class="leads-status" data-id="${q(e.id)}" data-act="status" onclick="event.stopPropagation()">
    ${R_.map(t=>`<option value="${t}" ${e.status===t?`selected`:``}>${t[0]}${t.slice(1).toLowerCase()}</option>`).join(``)}
  </select>`}function f_(e){let t=K.magnetNames[e.magnet_guid]||e.lead_source||`—`,n=e.phone?String(e.phone).replace(/[^\d+]/g,``):``;return`
    <tr class="leads-row${String(e.id)===K.selectedId?` selected`:``}" data-id="${q(e.id)}" tabindex="0" role="button" aria-label="Open lead ${q(H_(e))}">
      <td>
        <span class="leads-who">
          <span class="leads-avatar">${q(U_(e))}</span>
          <span><span class="leads-name">${q(H_(e))}</span>${e.company?`<span class="leads-sub">${q(e.company)}</span>`:``}</span>
        </span>
      </td>
      ...
      <td class="num leads-muted" style="white-space:nowrap" title="${q(s_(e))}">${G_(e)?`<span class="leads-fresh" title="Captured in the last 24 hours"></span>`:``}${q(c_(e))}</td>
      <td class="leads-open" aria-hidden="true"><span class="material-symbols-outlined">chevron_right</span></td>
    </tr>`}function p_(e){let t=K.search.trim().toLowerCase();if(!t)return!0;let n=[e.first_name,e.last_name,e.email,e.company,e.phone,e.product_interest,K.magnetNames[e.magnet_guid],K.book?K.ownerNames[e.owner_guid]:``,e.utm_campaign,e.utm_source].filter(Boolean).join(` `).toLowerCase();return t.split(/\s+/).every(e=>n.includes(e))}function m_(e){let t=Y_[K.range];if(!t)return!0;let n=W_(e);if(!n)return!1;let r=new Date(Date.now()-(t-1)*864e5).toISOString().slice(0,10);return n.slice(0,10)>=r}function h_(){return K.leads.filter(e=>(K.filter===`ALL`||e.status===K.filter)&&K_(e)&&J_(e)&&p_(e)&&q_(e)&&m_(e))}function g_(){let e=h_(),t=Z_[K.sort.key];if(!t)return e;let n=K.sort.dir===`asc`?1:-1;return e.slice().sort((e,r)=>{let i=t.value(e),a=t.value(r);return i===``&&a===``?0:i===``?1:a===``?-1:i===a?0:i<a?-n:n})}function __(e,t=``){let n=Z_[e],r=K.sort.key===e,i=r?K.sort.dir:``;return`<th class="leads-th--sort${t?` ${t}`:``}${r?` is-sorted`:``}"
    ...
    </th>`}function v_(e){return Z_[e]?(K.sort.key===e?K.sort.dir=K.sort.dir===`asc`?`desc`:`asc`:K.sort={key:e,dir:Z_[e].dir},K.shown=L_(),!0):!1}function y_(){if(!K.day)return``;let e=K.day;try{e=new Date(`${K.day}T00:00:00Z`).toLocaleDateString([],{month:`short`,day:`numeric`,year:`numeric`,timeZone:`UTC`})}catch{}return`<button class="leads-daychip" type="button" id="leads-dayclear" title="Show every day again">
    ...
    <div class="page-shell" data-section="leads">
      ...
      <div class="leads-layout">
        ...
        <aside class="dashboard-card leads-panel" aria-label="Lead detail">
          <div class="card-body" id="leads-panel-body">${Q_()}</div>
        </aside>
      </div>
    </div>`}async function x_(){document.getElementById(`leads-export`)?.addEventListener(`click`,P_),lr(`csv_export`).then(e=>{e||document.getElementById(`leads-export`)?.remove()}).catch(()=>{});let e=io(oo.LEADS_SEARCH),t=io(oo.LEADS_DAY);if((e||t)&&(K.search=e||``,K.day=t||``,K.filter=`ALL`,K.clientFilter=`ALL`,K.magnetFilter=`ALL`,K.range=`ALL`,K.shown=L_()),A()){let e=e=>Number(e||0).toLocaleString();Wa(V_(),[{h:`Leads · wk`,td:t=>e(t.leads.this_week)},{h:`Leads · total`,td:t=>e(t.leads.total)},{h:`Compl.`,td:e=>e.completion_rate==null?`—`:`${e.completion_rate}%`}],{emptyMsg:`No client orgs yet. Create one under Clients first.`})}await C_(),S_()}async function S_(){let e=document.getElementById(`leads-crm-slot`);if(e)try{let{denied:t,providers:n,configured:r}=await nl();if(t)return;let i=$_.map(e=>{let t=Gc[e],i=$c(n,e);return{p:t,conn:i,status:el(t,i,!!r?.[e])}}),a=i.filter(e=>e.status===`connected`),o=i.find(e=>e.status===`broken`);if(a.length){let t=a.map(e=>e.p.label).join(` and `),n=a.length===1&&a[0].conn?.external_account?.hub_domain||``;e.innerHTML=`
    ...
    <div class="leads-bar">
      ...
      <select class="leads-clientsel" id="leads-rangesel" aria-label="Filter by date captured">
        ${Object.keys(Y_).map(e=>`<option value="${e}" ${K.range===e?`selected`:``}>${q(X_[e])}</option>`).join(``)}
      </select>
      ${y_()}
    ...
    <div id="leads-tablezone"></div>`,e.querySelector(`.leads-filter`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-filter]`);t&&(K.filter=t.dataset.filter,K.shown=L_(),w_())}),e.querySelector(`#leads-clientsel`)?.addEventListener(`change`,e=>{K.clientFilter=e.target.value,K.shown=L_(),w_()}),e.querySelector(`#leads-magnetsel`)?.addEventListener(`change`,e=>{K.magnetFilter=e.target.value,K.shown=L_(),w_()}),e.querySelector(`#leads-rangesel`)?.addEventListener(`change`,e=>{K.range=e.target.value,K.day=``,K.shown=L_(),w_()}),e.querySelector(`#leads-dayclear`)?.addEventListener(`click`,()=>{K.day=``,K.shown=L_(),w_()}),e.querySelector(`#leads-search`)?.addEventListener(`input`,e=>{K.search=e.target.value,K.shown=L_(),T_()}),T_()}function T_(){let e=document.getElementById(`leads-tablezone`),t=document.getElementById(`leads-countlbl`);if(!e)return;let n=g_(),r=n.slice(0,K.shown),i=K.book?8:7;t&&(t.textContent=`${n.length} of ${K.leads.length}`),e.innerHTML=`
    <div class="leads-tablewrap">
      <table class="leads-table${K.book?` leads-table--book`:``}">
        ${ev()}
        <thead><tr>${__(`name`)}${K.book?`<th>Client</th>`:``}${__(`contact`)}<th>Interest</th><th>Magnet</th><th>Status</th>${__(`captured`,`num`)}<th></th></tr></thead>
    ...
    ${K.canManage?`
      ...
      </div>`:``}`,e.querySelector(`[data-action="delete-lead"]`)?.addEventListener(`click`,N_)}async function N_(e){let t=e.currentTarget?.dataset?.id,n=K.leads.find(e=>String(e.id)===String(t));if(!n)return;let r=H_(n);if(!window.confirm(`Remove ${r} from your leads?\n\nThis takes the lead out of your dashboard, lists and exports. The record is kept in our system, and this address can be captured again if they come back through one of your magnets.`))return;let i=e.currentTarget;i.disabled=!0;try{let e=await b(`${V_()}/m5t/v5/m5mLead/${encodeURIComponent(t)}`,{method:`DELETE`}),n=await e.json().catch(()=>({}));if(!e.ok||!n.ok)throw Error(n.error||`HTTP ${e.status}`);K.leads=K.leads.filter(e=>String(e.id)!==String(t)),K.selectedId=null,B_(`${r} removed`,`success`),w_()}catch{i.disabled=!1,B_(`Could not remove that lead. Please try again.`,`danger`)}}function P_(){let e=g_();if(!e.length){B_(`Nothing to export`,`info`);return}let t=[`first_name`,`last_name`,`email`,`phone`,`company`,`product_interest`,`lead_source`,`utm_source`,`utm_medium`,`utm_campaign`,`gclid`,`status`],n=e=>`"${String(e??``).replace(/"/g,`""`)}"`,r=[[...K.book?[`client`]:[],...t,`magnet`,`captured`,`appointment`,`last_call`].join(`,`)];for(let i of e){let e=k_(i),a=A_(i);r.push([...K.book?[n(K.ownerNames[i.owner_guid]||``)]:[],...t.map(e=>n(i[e])),n(K.magnetNames[i.magnet_guid]||i.lead_source||``),n(s_(i)),n(e?`${e.start} (${e.status})`:``),n(a?`${a.started} (${a.answered===`no`?`missed`:`answered`})`:``)].join(`,`))}let i=new Blob([r.join(`
`)],{type:`text/csv`}),a=URL.createObjectURL(i),o=document.createElement(`a`);o.href=a,o.download=`leads-${K.filter.toLowerCase()}.csv`,document.body.appendChild(o),o.click(),o.remove(),URL.revokeObjectURL(a),B_(`Exported ${e.length} lead${e.length===1?``:`s`}`,`success`)}var F_,I_,K,L_,R_,z_,q,B_,V_,H_,U_,W_,G_,K_,q_,J_,Y_,X_,Z_,Q_,$_,ev,tv,nv=o((()=>{T(),F_=u(f(),1),m(),Qa(),co(),An(),Ol(),pr(),x(),no(),I_=!1,K={leads:[],magnetNames:{},filter:`ALL`,canManage:!1,book:!1,ownerNames:{},clientFilter:`ALL`,search:``,shown:25,selectedId:null,day:``,sort:{key:`captured`,dir:`desc`},magnetFilter:`ALL`,range:`ALL`,pageSize:25,total:0,truncated:!1},L_=()=>K.pageSize===`ALL`?2**53-1:Number(K.pageSize)||25,R_=[`NEW`,`WORKING`,`QUALIFIED`,`UNQUALIFIED`,`CONVERTED`,`EXPORTED`],z_={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},q=e=>String(e??``).replace(/[&<>"']/g,e=>z_[e]),B_=(e,t=`info`)=>{try{F_.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},V_=()=>w.getConfig?.().BASE_URL||``,H_=e=>[e.first_name,e.last_name].filter(Boolean).join(` `).trim()||e.email||`(no name)`,U_=e=>(H_(e).split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``)||`?`).toUpperCase(),W_=e=>e.captured_at||e.created?.created_tz||``,G_=e=>{let t=W_(e);if(!t)return!1;try{return Date.now()-new Date(t).getTime()<864e5}catch{return!1}},K_=e=>K.clientFilter===`ALL`||e.owner_guid===K.clientFilter,q_=e=>!K.day||W_(e).slice(0,10)===K.day,J_=e=>K.magnetFilter===`ALL`||e.magnet_guid===K.magnetFilter,Y_={ALL:null,7:7,30:30,90:90},X_={ALL:`All time`,7:`Last 7 days`,30:`Last 30 days`,90:`Last 90 days`},Z_={name:{label:`Name`,dir:`asc`,value:e=>H_(e).toLowerCase()},contact:{label:`Contact`,dir:`asc`,value:e=>String(e.email||e.phone||``).toLowerCase()},captured:{label:`Captured`,dir:`desc`,value:e=>W_(e)}},Q_=()=>u_(`touch_app`,`Click to see lead information.`),$_=[`hubspot`,`zoho`,`salesforce`,`gohighlevel`],ev=()=>`
  <colgroup>
    <col style="width:22%">
  ...
  </colgroup>`,tv={booked:[`QUALIFIED`,`Booked`],rescheduled:[`WORKING`,`Rescheduled`],canceled:[`CANCELED`,`Canceled`]}})),rv=c({init:()=>fv,render:()=>cv,tableHTML:()=>sv});function iv(){if(Sv)return;let e=document.createElement(`style`);e.id=`m5t-analytics-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Sv=!0}function av(e,t){let n=t>0?Math.round(e/t*100):0,r=`${gv(e)} of ${gv(t)} leads for the busiest magnet`;return`<svg class="anl-bar" width="72" height="8" viewBox="0 0 72 8" role="img"
      aria-label="${vv(r)}">
      <title>${vv(r)}</title>
    ...
    </svg>`}function ov(e,t,n){let r=wv[e.status]?`<span class="anl-chip">${vv(wv[e.status])}</span>`:``,i=n&&e.owner_name?`<span class="anl-sub">${vv(e.owner_name)}</span>`:``,a=e.ab_tests&&(e.ab_tests.running||e.ab_tests.completed)?`${gv(e.ab_tests.running)} / ${gv(e.ab_tests.completed)}`:`—`;return`<tr data-guid="${vv(e.magnet_guid)}">
    <td><a class="anl-name" href="#/magnets">${vv(e.name)}</a>${r}${i}</td>
    <td>${av(Number(e.leads?.window||0),t)}</td>
    <td>${gv(e.leads?.window)}</td>
    <td>${gv(e.opens)}</td>
    <td>${yv(e.engagement_rate)}</td>
    <td>${yv(e.completion_rate)}</td>
    <td>${yv(e.lead_rate)}</td>
    <td>${a}</td>
  </tr>`}function sv(e,t){let n=e.reduce((e,t)=>Math.max(e,Number(t.leads?.window||0)),0);return`<div class="anl-wrap"><table class="anl-table">
    <thead><tr>
      <th>Magnet</th><th></th><th>Leads</th><th>Opens</th>
      <th>Engaged</th><th>Completed</th><th>Leads / Open</th><th>A/B run / done</th>
    </tr></thead>
    <tbody>${e.map(e=>ov(e,n,t)).join(``)}</tbody>
  </table></div>`}function cv(){return A()?Ha({section:`analytics`,title:`Analytics`,description:`Pick a client to see how their magnets are performing.`}):(iv(),mo(),`
    <div class="page-shell" data-section="analytics">
      <div class="page-header">
        ...
        <div class="chart-controls" id="anl-periods">
          ${bv.map(e=>`<button class="chart-period-btn${e.d===xv?` active`:``}" type="button" data-days="${e.d}">${e.label}</button>`).join(``)}
        </div>
      </div>
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="anl-body">
          ${Cv(`hourglass_empty`,`Loading magnet performance…`)}
        </div>
      </div>
    ...
    </div>`)}function lv(e){try{return new Date(`${e}T00:00:00`).toLocaleDateString(void 0,{month:`short`,day:`numeric`,year:`numeric`})}catch{return String(e||``)}}function uv(e){return ho({rows:e,xLabel:e=>String(e.date||``).slice(5),left:{value:e=>e.opens||0,label:`Magnet Opens`},right:{value:e=>e.leads||0,label:`Leads`},ariaLabel:`Magnet opens versus leads over time`,activatable:e=>Number(e.leads||0)>0,pointLabel:e=>`${lv(e.date)}: ${gv(e.opens||0)} opens, ${gv(e.leads||0)} leads`,hitData:e=>({day:e.date,leads:Number(e.leads||0)})})}function dv(e){let t=document.getElementById(`anl-trend`);return t?_o(t,{rows:e,tipHTML:e=>`
      <div class="m5c-tip__title">${lv(e.date)}</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--1"></i>${gv(e.opens||0)} opens</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--2"></i>${gv(e.leads||0)} leads</div>
      ${Number(e.leads||0)>0?`<div class="m5c-tip__go">Open these leads →</div>`:``}`,onActivate:e=>{ro(oo.LEADS_DAY,e.date),window.location.hash=`#/leads`}}):()=>{}}async function fv(){if(A())return Wa(mv(),[{h:`Leads`,td:e=>gv(e.leads.window??e.leads.total)},{h:`Opens`,td:e=>gv(e.opens)},{h:`Completed`,td:e=>yv(e.completion_rate)}]);iv(),mo(),await pv();let e=document.getElementById(`anl-periods`);e&&e.addEventListener(`click`,t=>{let n=t.target.closest(`.chart-period-btn`);n&&(xv=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>{e.classList.toggle(`active`,Number(e.dataset.days)===xv)}),pv())})}async function pv(){let e=document.getElementById(`anl-body`),t=document.getElementById(`anl-range`);if(!e)return;e.innerHTML=Cv(`hourglass_empty`,`Loading magnet performance…`);let n=new Date,r=hv(n),i=hv(new Date(n.getTime()-(xv-1)*864e5));t&&(t.textContent=`${i} → ${r}`);let a;try{let e=await b(`${mv()}/m5t/v5/dashboard/stats?from=${i}&to=${r}&group_by=magnet`);if(a=await e.json(),j(e,a),!a?.ok)throw Error(a?.error||`Failed to load`)}catch(t){e.innerHTML=$a(t)?eo():Cv(`cloud_off`,`Could not load analytics. Refresh to try again.`),console.error(`[analytics] load failed:`,t);return}let o=(e,t)=>{let n=document.getElementById(e);n&&(n.textContent=t)};o(`anl-leads`,gv(a.by_day?.reduce((e,t)=>e+Number(t.leads||0),0))),o(`anl-opens`,gv(a.funnel?.opens)),o(`anl-completion`,yv(a.funnel?.completion_rate)),o(`anl-active`,gv(a.magnets?.active));let s=Array.isArray(a.by_day)?a.by_day:[],c=document.getElementById(`anl-trend`);if(c){let e=s.some(e=>Number(e.opens||0)>0||Number(e.leads||0)>0);c.innerHTML=e?uv(s):Cv(`show_chart`,`No activity in this range yet. Opens and leads chart here as visitors engage your magnets.`),e&&dv(s)}let l=Array.isArray(a.by_magnet)?a.by_magnet:[];if(!l.length){e.innerHTML=Cv(`widgets`,`No magnets yet. Create one and its numbers will show up here.`);return}let u=l.some(e=>Number(e.opens||0)>0||Number(e.leads?.window||0)>0),d=new Set(l.map(e=>e.owner_guid)).size>1;e.innerHTML=(a.by_magnet_truncated?`<p class="anl-warn">Showing the top ${l.length} magnets by leads. You have more than that, so this list is not complete.</p>`:``)+(u?``:`<p class="anl-warn">No opens or leads for any magnet in this period. Try a longer range.</p>`)+sv(l,d);let f=e.querySelector(`.anl-table tbody`);f&&O(f,{rowSelector:`tr`,noun:`magnets`,controlHost:e.querySelector(`.anl-wrap`)||e})}var mv,hv,gv,_v,vv,yv,bv,xv,Sv,Cv,wv,Tv=o((()=>{T(),Qa(),fo(),An(),So(),co(),x(),no(),mv=()=>w.getConfig?.().BASE_URL||``,hv=e=>e.toISOString().slice(0,10),gv=e=>Number(e||0).toLocaleString(),_v={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},vv=e=>String(e??``).replace(/[&<>"']/g,e=>_v[e]),yv=e=>e==null?`—`:`${e}%`,bv=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],xv=30,Sv=!1,Cv=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,wv={DRAFT:`Draft`,DISABLED:`Paused`}})),Ev=c({default:()=>iy,init:()=>kv,render:()=>Ov});function Dv(){if(mo(),ty)return;let e=document.createElement(`style`);e.id=`m5t-conversions-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),ty=!0}function Ov(){return A()?Ha({section:`analytics-conversions`,title:`Conversions`,description:`Pick a client to view their conversion reporting.`}):(Dv(),J.canSave=bl(),`
    <div class="page-shell" data-section="analytics-conversions">
      <div class="page-header">
        ...
        <div class="chart-controls" id="conv-periods">
          ${$v.map(e=>`<button class="chart-period-btn${e.d===ey?` active`:``}" type="button" data-days="${e.d}">${e.label}</button>`).join(``)}
        </div>
      </div>
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="conv-selector">
          ${ny(`hourglass_empty`,`Loading conversion actions.`)}
        </div>
      </div>
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="conv-chart">
          ${ny(`hourglass_empty`,`Loading conversions.`)}
        </div>
      </div>
    ...
    </div>`)}async function kv(){if(A()){let e=e=>Number(e||0).toLocaleString();return Wa(Hv(),[{h:`Spend`,td:t=>t.ad?`$${e(t.ad.spend)}`:Wv},{h:`Clicks`,td:t=>t.ad?e(t.ad.clicks):Wv},{h:`Leads`,td:t=>e(t.leads.window??t.leads.total)},{h:`CPL`,td:t=>t.ad?.cost_per_lead==null?Wv:`$${e(t.ad.cost_per_lead)}`}],{emptyMsg:`No client orgs yet. Connect a client ad account to see conversions here.`})}let e=document.getElementById(`conv-periods`);e?.addEventListener(`click`,t=>{let n=t.target.closest(`button[data-days]`);n&&(ey=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>e.classList.toggle(`active`,Number(e.dataset.days)===ey)),Av())}),document.getElementById(`conv-selector`)?.addEventListener(`change`,e=>{let t=e.target.closest(`input[type="checkbox"][data-action-id]`);t&&(t.checked?J.selection.add(t.value):J.selection.delete(t.value),Lv(),Mv())}),await Av()}async function Av(){let e=new Date;J.to=Uv(e),J.from=Uv(new Date(e.getTime()-(ey-1)*864e5));let t=document.getElementById(`conv-range`);t&&(t.textContent=`${J.from} → ${J.to}`);let n=Hv(),r=`from=${J.from}&to=${J.to}`,i=null,a=null,o=null,s=[],c=null;try{let e=bl(),[t,l,u,d,f]=await Promise.all([b(`${n}/m5t/v5/coreAdMetrics/overlay?${r}`).then(e=>e.json()).catch(()=>null),b(`${n}/m5t/v5/coreAdMetrics/conversions?${r}`).then(e=>e.json()).catch(()=>null),b(`${n}/m5t/v5/coreAdMetrics/upload-ledger?${r}`).then(e=>e.json()).catch(()=>null),b(`${n}/m5t/v5/coreAdAccount`).then(e=>e.json()).catch(()=>null),e?b(`${n}/m5t/v5/integrations/google_ads/conversion_actions`).then(e=>e.json()).catch(()=>null):Promise.resolve(null)]);i=t,a=l,o=u,c=f,s=(d?.data||[]).filter(e=>e.status!==`DELETED`)}catch(e){console.warn(`[analytics-conversions] load failed:`,e),Vv(`conv-chart`,ny(`error`,`Could not load conversions. Please try again.`));return}J.spend=i?.totals?.spend??null,J.overlayRows=i?.rows||[],J.canSave=bl();let l=s.filter(e=>e.provider===`google_ads`);J.universe=jv({catalog:c,conv:a,accounts:l});let u=a?.has_selection&&Array.isArray(a.selected_ids)?a.selected_ids.map(String):[],d=J.universe.filter(e=>e.magnet).map(e=>e.id);if(J.selection=new Set(u.length?u:d),zv(o),Iv({accounts:l}),Lv(),!l.length){Bv(`conv-spend`,Gv(J.spend)),Vv(`conv-chart`,ny(`link_off`,`No ad account connected yet. Your agency can connect Google Ads to see conversions here.`)),Vv(`conv-actions`,ny(`link_off`,`No ad account connected yet.`));return}u.length||!J.selection.size?(J.conv=a,Nv()):await Mv()}function jv({catalog:e,conv:t,accounts:n}){let r=new Map,i=(e,t)=>{let n=String(e||``);if(!n)return;let i=r.get(n)||{id:n,name:null,category:null,magnet:!1,kind:null};r.set(n,{...i,...Object.fromEntries(Object.entries(t).filter(([,e])=>e!=null&&e!==!1))})};(e?.data?.actions||[]).forEach(e=>i(e.id,{name:e.name,category:e.category})),(t?.actions||[]).forEach(e=>i(e.id,{name:e.name,category:e.category})),(t?.available||[]).forEach(e=>i(e.id,{name:e.name,category:e.category})),n.forEach(e=>{let t=e.account_config?.magnet_actions||{};Object.entries(t).forEach(([e,t])=>{t?.id&&i(t.id,{name:t.name,magnet:!0,kind:e})})}),r.forEach(e=>{!e.magnet&&e.name&&Qv.has(e.name)&&(e.magnet=!0)});let a=[...r.values()],o=e=>e.magnet?Xv.indexOf(e.kind):-1;return a.sort((e,t)=>{if(e.magnet!==t.magnet)return e.magnet?-1:1;if(e.magnet){let n=o(e),r=o(t);if(n!==r)return(n<0?99:n)-(r<0?99:r)}return String(e.name||e.id).localeCompare(String(t.name||t.id))})}async function Mv(){let e=[...J.selection],t=document.getElementById(`conv-chart`);t?.classList.add(`conv-busy`),J.busy=!0;try{if(!e.length)J.conv={ok:!0,has_selection:!1,rows:[],actions:[],totals:{conversions:0,value:0,all_conversions:0,all_value:0}};else{let t=await b(`${Hv()}/m5t/v5/coreAdMetrics/conversions?from=${J.from}&to=${J.to}&actions=${encodeURIComponent(e.join(`,`))}`),n=await t.json().catch(()=>({}));j(t,n),J.conv=n}Nv()}catch(e){console.warn(`[analytics-conversions] recompute failed:`,e),Vv(`conv-chart`,$a(e)?eo():ny(`error`,`Could not recompute for that selection. Please try again.`))}finally{J.busy=!1,t?.classList.remove(`conv-busy`)}}function Nv(){let e=J.conv||{},t=e.totals||{conversions:0,value:0},n=Number(t.all_conversions??t.conversions??0);Bv(`conv-spend`,Gv(J.spend)),Bv(`conv-count`,Kv(n)),Bv(`conv-cpc`,Gv(Yv(J.spend,n))),Bv(`conv-value`,Gv(t.all_value??t.value??0)),Pv(e.rows||[]),Fv(e.actions||[])}function Pv(e){let t=document.getElementById(`conv-chart`);if(!t)return;if(!J.selection.size){t.innerHTML=ny(`checklist`,`No actions counted. Tick a conversion action to see it here.`),J.rows=[];return}let n=new Map(e.map(e=>[String(e.date).slice(0,10),e])),r=J.overlayRows.length?J.overlayRows.map(e=>String(e.date).slice(0,10)):e.map(e=>String(e.date).slice(0,10)),i=new Map(J.overlayRows.map(e=>[String(e.date).slice(0,10),Number(e.spend||0)])),a=r.map(e=>({date:e,spend:i.get(e)||0,conversions:Number(n.get(e)?.all_conversions??n.get(e)?.conversions??0)}));if(J.rows=a,!a.length){t.innerHTML=ny(`search_off`,`No conversions in this range for the selected actions.`);return}t.innerHTML=ho({rows:a,xLabel:e=>String(e.date||``).slice(5),left:{value:e=>e.spend||0,label:`Ad Spend`,fmt:e=>`$${bo(e)}`},right:{value:e=>e.conversions||0,label:`Conversions`},ariaLabel:`Ad spend versus counted conversions over time`,pointLabel:e=>`${e.date}: ${Gv(e.spend||0)} spend, ${Kv(e.conversions||0)} conversions`,hitData:e=>({day:e.date})}),_o(t,{rows:a,tipHTML:e=>`
      <div class="m5c-tip__title">${Jv(String(e.date||``))}</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--1"></i>${Gv(e.spend||0)} spend</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--2"></i>${Kv(e.conversions||0)} conversions</div>`})}function Fv(e){let t=document.getElementById(`conv-actions`);if(!t)return;let n=new Set(J.universe.filter(e=>e.magnet).map(e=>e.id)),r=e.filter(e=>J.selection.has(String(e.id)));if(!r.length){t.innerHTML=ny(`search_off`,J.selection.size?`No conversions in this range for the selected actions.`:`No actions counted. Tick a conversion action to see it here.`);return}t.innerHTML=`
    <div class="conv-wrap">
      <table class="conv-table">
        ...
        <tbody>${r.map(e=>`
          <tr>
            <td><span class="conv-name">${Jv(e.name||e.id)}</span>${n.has(String(e.id))?` <span class="conv-tag">Magnet</span>`:``}</td>
            <td class="sub">${Jv(ry(e.category))}</td>
            <td class="num"><b>${Kv(e.all_conversions??e.conversions)}</b></td>
            <td class="num">${Kv(e.conversions)}</td>
            <td class="num">${Gv(e.all_value??e.value??0)}</td>
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>`;let i=t.querySelector(`.conv-table tbody`);i&&O(i,{rowSelector:`tr`,noun:`actions`,controlHost:t.querySelector(`.conv-wrap`)||t})}function Iv({accounts:e}){let t=document.getElementById(`conv-selector`);if(!t)return;if(!e.length){t.innerHTML=ny(`link_off`,`No ad account connected yet. Your agency can connect Google Ads to see conversions here.`);return}if(!J.universe.length){t.innerHTML=`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">checklist</span>
    ...
    <label class="conv-row">
      <input type="checkbox" data-action-id="${Jv(e.id)}" value="${Jv(e.id)}"${J.selection.has(e.id)?` checked`:``}>
      <span class="conv-name">${Jv(e.name||e.id)}</span>
      ${e.magnet?`<span class="conv-tag">Magnet</span>`:``}
      ${e.category?`<span class="conv-cat">${Jv(ry(e.category))}</span>`:``}
    </label>`;t.innerHTML=`
    ...
    ${J.canSave?`
      ...
      </div>`:``}`;let a=t.querySelector(`#conv-list-other`);a&&O(a,{rowSelector:`.conv-row`,noun:`actions`}),document.getElementById(`conv-save`)?.addEventListener(`click`,Rv)}function Lv(){let e=document.getElementById(`conv-selcount`);if(!e)return;let t=J.selection.size;e.textContent=t?`${t} action${t===1?``:`s`} counted`:`No actions counted`}async function Rv(){let e=document.getElementById(`conv-save`),t=document.getElementById(`conv-savemsg`),n=[...J.selection];e&&(e.disabled=!0),t&&(t.classList.remove(`err`),t.textContent=`Saving.`);try{let e=await(await b(`${Hv()}/m5t/v5/integrations/google_ads/conversion_actions`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({conversion_action_ids:n})})).json().catch(()=>({}));if(!e.ok)throw Error(e.error||`save failed`);t&&(t.textContent=n.length?`Saved. ${n.length} action${n.length===1?``:`s`} counted by default.`:`Saved. No actions counted by default.`)}catch{t&&(t.classList.add(`err`),t.textContent=`Could not save the selection. Please try again.`)}finally{e&&(e.disabled=!1)}}function zv(e){let t=document.getElementById(`conv-ledger`);if(!t)return;let n=e?.kinds||[];if(!n.length){t.innerHTML=ny(`upload`,`Nothing fed into Google Ads in this range yet.`);return}let r=e=>Zv[e]||String(e||``).replace(/_/g,` `),i=e=>{let t=Xv.indexOf(e.kind);return t<0?99:t};t.innerHTML=`
    <div class="conv-wrap">
      <table class="conv-table" id="conv-ledger-table">
        ...
        <tbody>${[...n].sort((e,t)=>i(e)-i(t)).map(e=>`
          <tr data-kind="${Jv(e.kind)}">
            <td><span class="conv-name">${Jv(r(e.kind))}</span></td>
            <td class="num"><b>${Kv(e.sent)}</b></td>
            <td class="num">${Kv(e.queued)}</td>
            <td class="num">${Kv(e.failed)}</td>
            <td class="num">${Kv(e.skipped)}</td>
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>
    <p class="conv-note">What Magnet fed into Google Ads. Google's attributed numbers above are its own and are never added to these.</p>`}function Bv(e,t){let n=document.getElementById(e);n&&(n.textContent=t)}function Vv(e,t){let n=document.getElementById(e);n&&(n.innerHTML=t)}var Hv,Uv,Wv,Gv,Kv,qv,Jv,Yv,Xv,Zv,Qv,$v,ey,J,ty,ny,ry,iy,ay=o((()=>{T(),Qa(),Ol(),fo(),So(),An(),x(),no(),Hv=()=>w.getConfig?.().BASE_URL||``,Uv=e=>e.toISOString().slice(0,10),Wv=`—`,Gv=e=>e==null?Wv:`$`+Number(e).toLocaleString(void 0,{minimumFractionDigits:0,maximumFractionDigits:2}),Kv=e=>e==null?Wv:Number(e).toLocaleString(),qv={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Jv=e=>String(e??``).replace(/[&<>"']/g,e=>qv[e]),Yv=(e,t)=>{let n=Number(e),r=Number(t);return!Number.isFinite(n)||!Number.isFinite(r)||r<=0?null:Math.round(n/r*100)/100},Xv=[`magnet_fill`,`qualified_lead`,`call_dni`,`calls_from_ads`],Zv={magnet_fill:`Magnet fills`,qualified_lead:`Qualified leads`,call_dni:`Website calls`,calls_from_ads:`Ad calls`},Qv=new Set([`Magnet Lead`,`Magnet Qualified Lead`,`Magnet Phone Call (Website)`,`Ad Calls (Magnet reporting)`,`Calls from Ads (Magnet)`]),$v=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],ey=30,J={from:null,to:null,spend:null,universe:[],selection:new Set,conv:null,rows:[],overlayRows:[],canSave:!1,busy:!1},de(()=>{J.busy=!1}),ty=!1,ny=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,ry=e=>String(e||``).toLowerCase().replace(/_/g,` `)||Wv,iy={render:Ov,init:kv}})),oy=c({init:()=>ly,render:()=>cy});function sy(){if(yy)return;let e=document.createElement(`style`);e.id=`m5t-billing-styles`,e.textContent=`
    ...
    .page-shell .chart-period-btn { cursor: pointer; }
    .page-shell .chart-period-btn.active { background: var(--color-primary-600, #0072BA); color: #fff; }
  `,document.head.appendChild(e),yy=!0}function cy(){return sy(),`
    <div class="page-shell" data-section="billing">
      ...
      <div class="dashboard-card">
        <div class="card-body" id="bill-plan-card">${by(`hourglass_empty`,`Loading your plan…`)}</div>
      </div>
    ...
    </div>`}async function ly(){document.getElementById(`bill-interval`)?.addEventListener(`click`,e=>{let t=e.target.closest(`button[data-interval]`);t&&(Sy=t.dataset.interval,document.querySelectorAll(`#bill-interval .chart-period-btn`).forEach(e=>e.classList.toggle(`active`,e.dataset.interval===Sy)),Cy&&uy(Cy))}),document.getElementById(`bill-upgrade-card`)?.addEventListener(`click`,async e=>{let t=e.target.closest(`button[data-plan]`);if(t){t.disabled=!0;try{let e=await(await b(`${hy()}/m5t/v5/billing/checkout`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({plan:t.dataset.plan,interval:Sy})})).json();if(!e.ok||!e.data?.url)throw Error(e.error||`checkout failed`);window.location.href=e.data.url}catch(e){console.warn(`[billing] checkout failed:`,e),t.disabled=!1,t.textContent=`Could not start checkout. Try again.`}}}),document.getElementById(`bill-plan-card`)?.addEventListener(`click`,async e=>{let t=e.target.closest(`#bill-manage`);if(t){t.disabled=!0;try{let e=await(await b(`${hy()}/m5t/v5/billing/portal`,{method:`POST`,headers:{"Content-Type":`application/json`},body:`{}`})).json();if(!e.ok||!e.data?.url)throw Error(e.error||`portal failed`);window.location.href=e.data.url}catch(e){console.warn(`[billing] portal failed:`,e),t.disabled=!1}}}),await my()}function uy(e){let t=document.getElementById(`bill-upgrade-card`),n=document.getElementById(`bill-upgrade`);if(!t||!n)return;let r=e.plan?.key===`free`&&e.checkout_ready;if(t.style.display=r?``:`none`,!r)return;let i=e=>`
    <div class="bill-meter">
      <div class="bill-meter__k">${vy(e.label)}</div>
      <div class="bill-meter__v">$${e.prices[Sy]}<small>/month${Sy===`annual`?`, billed yearly`:``}</small></div>
      ...
      <div style="margin-top:12px"><button type="button" class="bill-btn" data-plan="${e.key}"><span class="material-symbols-outlined" aria-hidden="true">arrow_forward</span>Choose ${vy(e.label)}</button></div>
    </div>`;n.innerHTML=`<div class="bill-meters" style="margin-top:0">${i(et.pro)}${i(et.enterprise)}</div>`}function dy(e,t=null){return e.key===`enterprise`&&!t?`Included with your program.`:e.key===`grandfathered`?`All features included.`:t&&e.prices?`${t.interval===`annual`?`$${e.prices.annual}/month, billed yearly`:`$${e.prices.monthly}/month`}.`:``}function fy(e){let t=`for every display format, integrations, and ${gy(et.pro.limits.leads_mo)} leads a month.`,n=e?`<a href="#/billing" id="bill-up-link">Upgrade to Pro</a>`:`Upgrade to Pro`;return`${vy(`Design a free magnet for your business and track its leads.`)} ${n} ${vy(t)}`}function py(e,t,n,{hint:r=``,consumption:i=!1}={}){let a=n==null,o=a?0:Math.min(100,Math.round(t/Math.max(1,n)*100)),s=i&&o>=80;return`
    <div class="bill-meter">
      <div class="bill-meter__k">${vy(e)}</div>
      <div class="bill-meter__v">${gy(t)}${a?` <small>No limit</small>`:` <small>of ${gy(n)}</small>`}</div>
      ${a?``:`<div class="bill-bar"><i style="width:${o}%" data-warn="${s}"></i></div>`}
      ${r?`<div class="bill-meter__hint">${vy(r)}</div>`:``}
    </div>`}async function my(){let e=document.getElementById(`bill-plan-card`),t;try{let e=await b(`${hy()}/m5t/v5/billing/usage`);if(t=await e.json(),j(e,t),!t.ok)throw Error(t.error||`load failed`)}catch(t){console.warn(`[billing] load failed:`,t),e&&(e.innerHTML=$a(t)?eo():by(`error`,`Could not load billing. Please try again.`));return}let n=t.plan||et.grandfathered,r=t.usage||{},i=t.subscription||null;if(Cy=t,e){let r=i?.current_period_end?`${i.cancel_at_period_end?`Ends`:`Renews`} ${new Date(i.current_period_end).toLocaleDateString([],{month:`short`,day:`numeric`,year:`numeric`})}.`:``,a=i?.status===`PAST_DUE`?` <span class="bill-plan__badge" style="background: var(--color-danger, #DD4124)">Payment needed</span>`:``;e.innerHTML=`
      <div class="bill-plan">
        <span class="bill-plan__name">${vy(n.label)}</span>
      ...
      <div class="bill-plan__sub">${n.key===`free`?fy(!!t.checkout_ready):vy([dy(n,i),r].filter(Boolean).join(` `))}</div>`,e.querySelector(`#bill-up-link`)?.addEventListener(`click`,e=>{e.preventDefault(),document.getElementById(`bill-upgrade-card`)?.scrollIntoView({behavior:`smooth`,block:`start`})})}uy(t);let a=document.getElementById(`bill-meters`);if(a){let e=n.key===`free`;a.innerHTML=[py(`Live magnets`,r.magnets,n.limits?.magnets??null,{hint:e?`Pro includes unlimited magnets.`:``}),py(`Leads this month`,r.leads_month,n.limits?.leads_mo??null,{consumption:!0,hint:e?`Pro includes ${gy(et.pro.limits.leads_mo)} a month.`:``}),py(`Team members`,r.team_members,n.limits?.team_members??null,{hint:e?`Pro includes ${gy(et.pro.limits.team_members)} seats.`:``})].join(``)}let o=document.getElementById(`bill-feat-card`),s=document.getElementById(`bill-feats`);if(o&&s){let e=[],t=n.features?.formats||[];e.push(`<div class="bill-feat"><span class="material-symbols-outlined" aria-hidden="true">check_circle</span>${t.length>=4?`All four display formats`:`Chat bubble format`}</div>`);for(let[t,r]of xy)r&&n.features?.[t]&&e.push(`<div class="bill-feat"><span class="material-symbols-outlined" aria-hidden="true">check_circle</span>${vy(r)}</div>`);let r=``;if(n.key===`free`){let e=[];(n.features?.formats||[]).length<4&&(et.pro.features.formats||[]).length>=4&&e.push(`All four display formats`);for(let[t,r]of xy)r&&!n.features?.[t]&&et.pro.features?.[t]&&e.push(r);e.length&&(r=`<div class="bill-feats__pro-label">Included with Pro</div>
          <div class="bill-feats">${e.map(e=>`<div class="bill-feat bill-feat--pro"><span class="material-symbols-outlined" aria-hidden="true">add_circle</span>${vy(e)}</div>`).join(``)}</div>`)}s.innerHTML=`<div class="bill-feats">${e.join(``)}</div>${r}`,o.style.display=``}let c=document.getElementById(`bill-note`);c&&(c.textContent=n.key===`enterprise`?`Usage is informational. Your program covers the platform.`:`Lead counts reset monthly. This period: ${t.period?.start||``} to ${t.period?.end||``}.`)}var hy,gy,_y,vy,yy,by,xy,Sy,Cy,wy=o((()=>{T(),rt(),x(),no(),hy=()=>w.getConfig?.().BASE_URL||``,gy=e=>Number(e||0).toLocaleString(),_y={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},vy=e=>String(e??``).replace(/[&<>"']/g,e=>_y[e]),yy=!1,by=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,xy=[[`formats`,null],[`hub_scheduler`,`Welcome hub and scheduler`],[`ab_testing`,`A/B testing`],[`journeys`,`Journeys`],[`attribution`,`Campaign attribution`],[`crm_sync`,`CRM sync`],[`booking`,`Booking integrations`],[`webhooks`,`Webhooks and Zapier`],[`csv_export`,`CSV export`],[`ad_reporting`,`Ad performance reporting`]],Sy=`monthly`,Cy=null})),Ty=c({init:()=>Oy,render:()=>Dy});function Ey(){if(mo(),gl(),By)return;let e=document.createElement(`style`);e.id=`m5t-traffic-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),By=!0}function Dy(){return A()?Ha({section:`traffic`,title:`Traffic`,description:`Pick a client to view their website traffic.`}):(Ey(),`
    <div class="page-shell" data-section="traffic">
      <div class="page-header">
        ...
        <div class="chart-controls" id="traffic-periods">
          ${Hy.map(e=>`<button class="chart-period-btn${e.d===Uy?` active`:``}" type="button" data-days="${e.d}">${e.label}</button>`).join(``)}
        </div>
      </div>
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="traffic-body">
          ${Vy(`hourglass_empty`,`Loading traffic…`)}
        </div>
      </div>
    ...
    </div>`)}async function Oy(){if(A()){let e=e=>Number(e||0).toLocaleString();return Wa(Py(),[{h:`Leads`,td:t=>e(t.leads.window??t.leads.total)}],{emptyMsg:`No client orgs yet. Connect a client GA4 property to see traffic here.`})}let e=document.getElementById(`traffic-periods`);e?.addEventListener(`click`,t=>{let n=t.target.closest(`button[data-days]`);n&&(Uy=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>e.classList.toggle(`active`,Number(e.dataset.days)===Uy)),ky())}),await ky()}async function ky(){let e=Py(),t=document.getElementById(`traffic-body`),n=document.getElementById(`traffic-range`),r=new Date,i=Fy(r),a=Fy(new Date(r.getTime()-(Uy-1)*864e5));n&&(n.textContent=`${a} → ${i}`);let o,s=null;try{let[t,n]=await Promise.all([b(`${e}/m5t/v5/coreAnalyticsMetrics/overlay?from=${a}&to=${i}`),nl().catch(()=>null)]);j(t),o=await t.json(),s=n}catch(e){console.warn(`[traffic] load failed:`,e),t&&(t.innerHTML=$a(e)?eo():Vy(`error`,`Could not load traffic. Please try again.`));return}Ay(s);let c=o.totals||{};if(jy(`traffic-sessions`,Iy(c.sessions)),jy(`traffic-users`,Iy(c.users)),jy(`traffic-views`,Iy(c.page_views)),jy(`traffic-engagement`,Ly(c.engagement_rate)),jy(`traffic-leads`,Iy(c.magnet_leads)),jy(`traffic-leadrate`,Ly(c.lead_rate)),!o.connected){t.innerHTML=s&&!s.denied&&$c(s.providers,`google_analytics`)?Vy(`cloud_sync`,`GA4 connected. No traffic for this range yet. Metrics appear here once they sync.`):Vy(`link_off`,`No GA4 property connected yet. Connect Google Analytics under Settings, Integrations to see traffic here.`);return}let l=o.rows||[];t.innerHTML=My(l),Ny(l)}function Ay(e){let t=document.getElementById(`traffic-conn`);if(!t)return;if(!e||e.denied){t.innerHTML=``;return}let n=Gc.google_analytics,r=$c(e.providers,`google_analytics`);if(!r){t.innerHTML=``;return}let i=el(n,r,!!e.configured?.google_analytics);if(i!==`connected`&&i!==`broken`){t.innerHTML=``;return}let a=bl()?` <a href="#/settings/integrations/${n.urlSlug}">Manage</a>`:``;t.innerHTML=`<span>Google Analytics</span> ${tl(i,r)}${a}`}function jy(e,t){let n=document.getElementById(e);n&&(n.textContent=t)}function My(e){return ho({rows:e,xLabel:e=>String(e.date||``).slice(5),left:{value:e=>e.sessions||0,label:`Sessions`,fmt:e=>bo(e)},right:{value:e=>e.magnet_leads||0,label:`Magnet Leads`},ariaLabel:`Website sessions versus magnet leads over time`,pointLabel:e=>`${e.date}: ${Iy(e.sessions||0)} sessions, ${Iy(e.magnet_leads||0)} leads`,hitData:e=>({day:e.date})})}function Ny(e){let t=document.getElementById(`traffic-body`);return t?_o(t,{rows:e,tipHTML:e=>`
      <div class="m5c-tip__title">${zy(String(e.date||``))}</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--1"></i>${Iy(e.sessions||0)} sessions</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--2"></i>${Iy(e.magnet_leads||0)} leads</div>`}):()=>{}}var Py,Fy,Iy,Ly,Ry,zy,By,Vy,Hy,Uy,Wy=o((()=>{T(),Qa(),Ol(),fo(),So(),x(),no(),Py=()=>w.getConfig?.().BASE_URL||``,Fy=e=>e.toISOString().slice(0,10),Iy=e=>Number(e||0).toLocaleString(),Ly=e=>e==null?`—`:`${e}%`,Ry={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},zy=e=>String(e??``).replace(/[&<>"']/g,e=>Ry[e]),By=!1,Vy=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,Hy=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],Uy=30})),Gy=c({default:()=>_b,init:()=>Jy,render:()=>qy});function Ky(){if(mo(),db)return;let e=document.createElement(`style`);e.id=`m5t-adperf-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),db=!0}function qy(){return A()?Ha({section:`ad-performance`,title:`Ad Performance`,description:`Pick a client to view their spend and lead performance.`}):(Ky(),`
    <div class="page-shell" data-section="ad-performance">
      <div class="page-header">
        ...
        <div class="chart-controls" id="adperf-periods">
          ${pb.map(e=>`<button class="chart-period-btn${e.d===mb?` active`:``}" type="button" data-days="${e.d}">${e.label}</button>`).join(``)}
        </div>
      </div>
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="adperf-body">
          ${fb(`hourglass_empty`,`Loading ad performance…`)}
        </div>
      </div>
    ...
    </div>`)}async function Jy(){if(A()){let e=e=>Number(e||0).toLocaleString();return Wa(ab(),[{h:`Spend`,td:t=>t.ad?`$${e(t.ad.spend)}`:`—`},{h:`Clicks`,td:t=>t.ad?e(t.ad.clicks):`—`},{h:`Leads`,td:t=>e(t.leads.window??t.leads.total)},{h:`CPL`,td:t=>t.ad?.cost_per_lead==null?`—`:`$${e(t.ad.cost_per_lead)}`}],{emptyMsg:`No client orgs yet. Connect a client ad account to see spend here.`})}let e=document.getElementById(`adperf-periods`);e?.addEventListener(`click`,t=>{let n=t.target.closest(`button[data-days]`);n&&(mb=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>e.classList.toggle(`active`,Number(e.dataset.days)===mb)),Yy())}),document.getElementById(`adperf-sources`)?.addEventListener(`click`,e=>{let t=e.target.closest(`tr[data-campaign]`);t&&(ro(oo.LEADS_SEARCH,t.dataset.campaign),!e.target.closest(`a`)&&(window.location.hash=`#/leads`))}),document.getElementById(`adperf-conv-scope`)?.addEventListener(`change`,e=>{hb=e.target.value||`all`,Qy()}),document.getElementById(`adperf-lsa-card`)?.addEventListener(`click`,e=>{let t=e.target.closest(`tr[data-lsalead]`);t&&(ro(oo.LEADS_SEARCH,t.dataset.lsalead),!e.target.closest(`a`)&&(window.location.hash=`#/leads`))}),await Yy()}async function Yy(){let e=ab(),t=document.getElementById(`adperf-body`),n=document.getElementById(`adperf-range`),r=new Date,i=ob(r),a=ob(new Date(r.getTime()-(mb-1)*864e5));n&&(n.textContent=`${a} → ${i}`);let o,s=[],c=null,l=null,u=null,d=null;try{let[t,n,r,f,p,m]=await Promise.all([b(`${e}/m5t/v5/coreAdMetrics/overlay?from=${a}&to=${i}`),b(`${e}/m5t/v5/coreAdAccount`),b(`${e}/m5t/v5/coreAdMetrics/by-source?from=${a}&to=${i}`),nl().catch(()=>null),b(`${e}/m5t/v5/coreAdMetrics/conversions?from=${a}&to=${i}${Zy()}`).then(e=>e.json()).catch(()=>null),b(`${e}/m5t/v5/coreAdMetrics/lsa?from=${a}&to=${i}`).then(e=>e.json()).catch(()=>null)]);j(t),o=await t.json(),s=((await n.json().catch(()=>({})))?.data||[]).filter(e=>e.status!==`DELETED`),c=await r.json().catch(()=>null),l=f,u=p,d=m}catch(e){console.warn(`[ad-performance] load failed:`,e),t&&(t.innerHTML=$a(e)?eo():fb(`error`,`Could not load ad performance. Please try again.`));return}Xy(l,s),gb=o?.totals?.spend??null,$y(u,gb),eb(d);let f=o.totals||{};if(nb(`adperf-spend`,sb(f.spend)),nb(`adperf-clicks`,cb(f.clicks)),nb(`adperf-leads`,cb(f.magnet_leads)),nb(`adperf-cpl`,sb(f.cost_per_lead)),tb(c),!s.length){t.innerHTML=fb(`link_off`,`No ad account connected yet. Your agency can connect Google Ads or Local Services Ads to see performance here.`);return}if(!o.connected){t.innerHTML=fb(`cloud_sync`,`Account connected. No ad data for this range yet. Metrics appear here once they sync.`);return}let p=o.rows||[];t.innerHTML=rb(p),ib(p)}function Xy(e,t){let n=document.getElementById(`adperf-conn`);if(!n)return;if(!e||e.denied){n.innerHTML=``;return}let r=Gc.google_ads,i=$c(e.providers,`google_ads`),a=el(r,i,!!e.configured?.google_ads),o=bl()?` <a href="#/settings/integrations/${r.urlSlug}">Manage</a>`:``;if(a===`connected`||a===`broken`){gl(),n.innerHTML=`<span>Google Ads</span> ${tl(a,i)}${o}`;return}if(t.length){n.innerHTML=`<span>Google Ads</span> <span>${t.length} associated account${t.length===1?``:`s`}</span>`;return}n.innerHTML=``}function Zy(){return hb===`saved`?``:`&actions=${encodeURIComponent(hb)}`}async function Qy(){let e=ab(),t=new Date,n=ob(t);$y(await b(`${e}/m5t/v5/coreAdMetrics/conversions?from=${ob(new Date(t.getTime()-(mb-1)*864e5))}&to=${n}${Zy()}`).then(e=>e.json()).catch(()=>null),gb)}function $y(e,t){let n=document.getElementById(`adperf-conv-card`),r=document.getElementById(`adperf-conv-body`),i=document.getElementById(`adperf-stats`),a=document.getElementById(`adperf-conv-stat`);a&&a.remove();let o=document.getElementById(`adperf-conv-cpc`);if(o&&o.remove(),!n||!r)return;let s=e?.available||[],c=e?.saved_ids||[],l=e?.actions||[];if(!e?.ok||!s.length&&!c.length&&!l.length){n.style.display=`none`;return}let u=document.getElementById(`adperf-conv-scope`);if(u){let e=[{v:`all`,label:`All conversion actions`}];c.length&&e.push({v:`saved`,label:`Counted actions`});for(let t of s)e.push({v:String(t.id),label:t.name||String(t.id)});e.some(e=>e.v===hb)||(hb=`all`),u.innerHTML=e.map(e=>`<option value="${ub(e.v)}"${e.v===hb?` selected`:``}>${ub(e.label)}</option>`).join(``)}let d=Number(e.totals?.all_conversions??e.totals?.conversions??0);if(i){let e=document.createElement(`div`);e.innerHTML=M({icon:`flag`,label:`Ads Conversions`,value:cb(d),sub:`Reported by Google Ads`,href:`#/settings/integrations/google-ads`});let n=e.firstElementChild;n.id=`adperf-conv-stat`,i.appendChild(n);let r=t!=null&&d>0?Math.round(Number(t)/d*100)/100:null,a=document.createElement(`div`);a.innerHTML=M({icon:`request_quote`,label:`Cost / Conversion`,value:sb(r),sub:`Against the selected actions`,href:`#/analytics/conversions`});let o=a.firstElementChild;o.id=`adperf-conv-cpc`,i.appendChild(o)}if(n.style.display=``,!l.length){r.innerHTML=fb(`search_off`,`No conversions in this range for the selected actions.`);return}let f=l.some(e=>(e.all_value??e.value)>0);r.innerHTML=`
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
        ...
        <tbody>${l.map(e=>`
          <tr>
            <td>${ub(e.name||e.id)}</td>
            <td class="adperf-src-sub">${ub(String(e.category||``).toLowerCase().replace(/_/g,` `)||`—`)}</td>
            <td class="num"><b>${cb(e.all_conversions??e.conversions)}</b></td>
            <td class="num">${cb(e.conversions)}</td>
            ${f?`<td class="num">${sb(e.all_value??e.value)}</td>`:``}
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>
    <p class="adperf-note">Counted actions are chosen on the Google Ads integration page. All conv. counts every conversion action Google recorded, including actions outside the campaign bidding goals. These are Google's own numbers and may overlap magnet leads, so they are never added together.</p>`}function eb(e){let t=document.getElementById(`adperf-lsa-card`),n=document.getElementById(`adperf-lsa-body`);if(!t||!n)return;if(!e?.ok||!e.connected){t.style.display=`none`;return}t.style.display=``;let r=e.totals||{},i=e.leads||[],a=e.disputes||[],o=r.answer_rate==null?`No call outcomes yet`:r.missed>0?`${cb(r.missed)} missed call${r.missed===1?``:`s`}`:`Every call answered`;n.innerHTML=`
    ${`
    <div class="stats-grid" style="margin-bottom:16px">
      ${M({icon:`payments`,label:`LSA Spend`,value:sb(r.spend),sub:`Reported by Google`})}
      ${M({icon:`person_add`,label:`LSA Leads`,value:cb(r.leads),sub:`${cb(r.phone_calls)} calls · ${cb(r.messages)} messages`})}
      ${M({icon:`paid`,label:`Charged`,value:cb(r.charged),sub:`Leads Google billed for`})}
      ${M({icon:`request_quote`,label:`Cost / Charged Lead`,value:sb(r.cost_per_charged_lead)})}
      ${M({icon:`call`,label:`Answer Rate`,value:r.answer_rate==null?`—`:`${r.answer_rate}%`,sub:o})}
      ${M({icon:`event_available`,label:`Booked`,value:cb(r.booked),sub:`Google's lead status`})}
    </div>`}
    ${i.length?`
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
        ...
        <tbody>${i.map(e=>{let t=e.phone||e.name,n=e.captured_at?new Date(e.captured_at).toLocaleDateString([],{month:`short`,day:`numeric`}):`—`;return`
          <tr${t?` data-lsalead="${ub(t)}" class="adperf-srcrow--link"`:``}>
            <td>${t?`<a class="adperf-srclink" href="#/leads" title="Open this lead">${ub(e.name)}</a>`:ub(e.name)}</td>
            <td class="adperf-src-sub">${ub([e.phone,e.email].filter(Boolean).join(` · `)||`—`)}</td>
            <td>${ub(e.type||`—`)}${e.missed?` <span class="adperf-src-untracked">· missed</span>`:``}</td>
            <td class="adperf-src-sub">${ub(e.category||`—`)}</td>
            <td class="adperf-src-sub">${ub(String(e.lsa_status||`—`).toLowerCase())}${e.dispute?` <span class="adperf-src-untracked">· dispute candidate</span>`:``}</td>
            <td class="num">${e.charged?`<b>yes</b>`:`<span class="adperf-src-sub">no</span>`}</td>
            <td class="adperf-src-sub">${ub(n)}</td>
          </tr>`}).join(``)}
        </tbody>
      </table>
    </div>`:fb(`search_off`,`No Local Services leads in this range yet.`)}
    ${a.length?`
    <div class="card-header" style="padding:16px 0 0">
      ...
      <span class="live-indicator">${cb(a.length)} inside the 30 day credit window</span>
    </div>
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
        ...
        <tbody>${a.map(e=>`
          <tr>
            <td>${ub(e.name)}${e.phone?` <span class="adperf-src-sub">${ub(e.phone)}</span>`:``}</td>
            <td class="adperf-src-sub">${ub(e.reason)}</td>
            <td class="num"><b>${cb(e.days_left)}</b></td>
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>
    ...
    <p class="adperf-note">Numbers come from Google's Local Services feed. These leads land in your Leads inbox and do not flow through magnets.</p>`;let s=n.querySelector(`.adperf-srctable tbody`);s&&O(s,{rowSelector:`tr`,noun:`leads`,controlHost:n.querySelector(`.adperf-srcwrap`)||n})}function tb(e){let t=document.getElementById(`adperf-sources`),n=document.getElementById(`adperf-src-summary`);if(!t)return;let r=e&&Array.isArray(e.rows)?e.rows:[];if(!r.length){n&&(n.textContent=``),t.innerHTML=fb(`search_off`,`No leads in this range yet. Campaigns appear here as tagged leads (?utm_campaign=…) come in.`);return}let i=e.total_leads||r.reduce((e,t)=>e+(t.leads||0),0),a=e.tracked_leads||0;n&&(n.textContent=`${cb(a)} of ${cb(i)} leads tagged to a source`);let o=e=>e.campaign&&e.campaign!==`(none)`?ub(e.campaign):e.source&&e.source!==`(direct)`?ub(e.source):`<span class="adperf-src-untracked">Untracked / direct</span>`,s=e=>e.campaign&&e.campaign!==`(none)`?e.campaign:e.source&&e.source!==`(direct)`?e.source:``;t.innerHTML=`
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
        ...
        <tbody>${r.map(e=>{let t=s(e);return`
          <tr${t?` data-campaign="${ub(t)}" class="adperf-srcrow--link"`:``}>
            <td>${t?`<a class="adperf-srclink" href="#/leads" title="Show the leads from ${ub(t)}">${o(e)}</a>`:o(e)}</td>
            <td class="adperf-src-sub">${ub(e.source||`—`)}${e.medium?` · ${ub(e.medium)}`:``}</td>
            <td class="num">${e.paid_clicks?cb(e.paid_clicks):`<span class="adperf-src-sub">—</span>`}</td>
            <td class="num"><b>${cb(e.leads)}</b></td>
          </tr>`}).join(``)}
        </tbody>
      </table>
    </div>`;let c=t.querySelector(`.adperf-srctable tbody`);c&&O(c,{rowSelector:`tr`,noun:`sources`,controlHost:t.querySelector(`.adperf-srcwrap`)||t})}function nb(e,t){let n=document.getElementById(e);n&&(n.textContent=t)}function rb(e){return ho({rows:e,xLabel:e=>String(e.date||``).slice(5),left:{value:e=>e.spend||0,label:`Ad Spend`,fmt:e=>`$${bo(e)}`},right:{value:e=>e.magnet_leads||0,label:`Magnet Leads`},ariaLabel:`Ad spend versus magnet leads over time`,pointLabel:e=>`${e.date}: ${sb(e.spend||0)} spend, ${cb(e.magnet_leads||0)} leads`,hitData:e=>({day:e.date})})}function ib(e){let t=document.getElementById(`adperf-body`);return t?_o(t,{rows:e,tipHTML:e=>`
      <div class="m5c-tip__title">${ub(String(e.date||``))}</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--1"></i>${sb(e.spend||0)} spend</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--2"></i>${cb(e.magnet_leads||0)} leads</div>`}):()=>{}}var ab,ob,sb,cb,lb,ub,db,fb,pb,mb,hb,gb,_b,vb=o((()=>{T(),Qa(),Ol(),fo(),So(),An(),co(),x(),no(),ab=()=>w.getConfig?.().BASE_URL||``,ob=e=>e.toISOString().slice(0,10),sb=e=>e==null?`—`:`$`+Number(e).toLocaleString(void 0,{minimumFractionDigits:0,maximumFractionDigits:2}),cb=e=>Number(e||0).toLocaleString(),lb={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},ub=e=>String(e??``).replace(/[&<>"']/g,e=>lb[e]),db=!1,fb=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,pb=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],mb=30,hb=`all`,gb=null,_b={render:qy,init:Jy}})),yb=c({default:()=>Hb,init:()=>Cb,parseBookingCsv:()=>Fb,render:()=>xb});function bb(){if(zb)return;let e=document.createElement(`style`);e.id=`m5t-adacct-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),zb=!0}function xb(){return A()?Ha({section:`ad-accounts`,title:`Ad Accounts`,description:`Pick a client to see their connected ad accounts.`}):(bb(),gl(),`
    ...
    <div class="adacct-wrap">
      <table class="adacct-table">
        ...
        <tbody>${r.map(e=>`
          <tr>
            <td>${P(Lb[e.provider]||e.provider)}</td>
            <td class="adacct-cid">${P(Rb(e.external_customer_id))}</td>
...
`&&a++,n.push(r),r=``,n.some(e=>e.trim()!==``)&&t.push(n),n=[]):r+=o}if(n.push(r),n.some(e=>e.trim()!==``)&&t.push(n),t.length<2)return[];let a=t[0].map(e=>e.trim().toLowerCase()),o=a.findIndex(e=>/e-?mail/.test(e)),s=a.findIndex(e=>/phone|mobile|tel/.test(e)),c=a.findIndex(e=>/date|time|booked|appointment|start/.test(e));return o<0&&s<0?[]:t.slice(1).map(e=>({email:o>=0?(e[o]||``).trim():``,phone:s>=0?(e[s]||``).trim():``,at:c>=0?(e[c]||``).trim():``})).filter(e=>e.email||e.phone).slice(0,5e3)}function Ib(e,t){let n=new Blob([e],{type:`text/csv;charset=utf-8`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.href=r,i.download=t,document.body.appendChild(i),i.click(),i.remove(),setTimeout(()=>URL.revokeObjectURL(r),4e3)}var Lb,Rb,zb,Bb,Vb,Hb,Ub=o((()=>{Qa(),An(),Ol(),x(),Lb={google_ads:`Google Ads`,google_lsa:`Local Services Ads`},Rb=e=>{let t=String(e||``).replace(/\D/g,``);return t.length===10?`${t.slice(0,3)}-${t.slice(3,6)}-${t.slice(6)}`:String(e||``)},zb=!1,Bb=[],Vb=null,Hb={render:xb,init:Cb}})),Wb=c({default:()=>fx,init:()=>Qb,openInviteModal:()=>nx,render:()=>Zb});function Gb(e,t=`info`){try{ax.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}}function Kb(){let e=w.getActiveOrg();if(e&&e.channel===ox)return{guid:e.guid,name:e.name};let t=w.getMemberships().find(e=>e.channel===ox);return t?{guid:t.orgGuid,name:t.name}:null}function qb(e){let t=e?.created_tz;if(!t)return``;let n=new Date(t);return Number.isNaN(n.getTime())?``:n.toLocaleDateString(void 0,{year:`numeric`,month:`short`,day:`numeric`})}function Jb(){if(cx)return;let e=document.createElement(`style`);e.id=`m5t-clients-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Hr(),cx=!0}function Yb(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function Xb(e){let t=e.caption||e.guid,n=e.status||`ENABLED`;return`
    <div class="client-row">
      <span class="client-row__avatar">${Y(ux(t))}</span>
    ...
    <div class="page-shell" data-section="clients">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="clients-body">
          ${Yb(`hourglass_empty`,`Loading clients…`)}
        </div>
      </div>
    </div>`}async function Qb(){let e=Kb();document.getElementById(`clients-add-btn`)?.addEventListener(`click`,()=>ex(e)),await $b(e)}async function $b(e){let t=document.getElementById(`clients-body`),n=document.getElementById(`clients-count`);if(t){if(!e?.guid){t.innerHTML=Yb(`info`,`Client provisioning is available to reseller / agency accounts.`),n&&(n.textContent=``);return}try{let r=await b(`${w.getConfig?.().BASE_URL||``}/m5t/v5/acctGroup`);j(r);let i=await r.json(),a=(Array.isArray(i?.data)?i.data:[]).filter(t=>t.class===sx&&(!t.parent||t.parent===e.guid));if(!a.length){t.innerHTML=Yb(`group_off`,`No clients yet. Add your first client to start managing magnets on their behalf.`),n&&(n.textContent=``);return}t.innerHTML=`<div class="client-list">${a.map(Xb).join(``)}</div>`,O(t.querySelector(`.client-list`),{rowSelector:`.client-row`,noun:`clients`}),n&&(n.textContent=`${a.length} client${a.length===1?``:`s`}`),t.querySelectorAll(`.client-card__invite`).forEach(e=>{e.addEventListener(`click`,()=>{e.dataset.action===`adaccounts`?tx(e.dataset.clientGuid,e.dataset.clientName):e.dataset.action===`questionnaire`?ix(e.dataset.clientGuid,e.dataset.clientName):nx(e.dataset.clientGuid,e.dataset.clientName)})})}catch(e){console.warn(`[clients] load failed:`,e),t.innerHTML=$a(e)?eo():Yb(`error`,`Couldn’t load clients. Please try again.`),n&&(n.textContent=``)}}}function ex(e){if(!e?.guid){Gb(`Only reseller / agency accounts can add clients.`,`error`);return}let t=document.createElement(`div`);t.className=`client-modal-scrim`;let n=document.createElement(`div`);n.className=`client-modal`,n.setAttribute(`role`,`dialog`),n.setAttribute(`aria-modal`,`true`),n.innerHTML=`
    ...
    <div class="client-modal__body" id="qm-body">${Yb(`hourglass_empty`,`Loading…`)}</div>`,document.body.appendChild(r),document.body.appendChild(i),requestAnimationFrame(()=>{r.classList.add(`open`),i.classList.add(`open`)});let a=()=>{r.classList.remove(`open`),i.classList.remove(`open`),document.removeEventListener(`keydown`,o),setTimeout(()=>{r.remove(),i.remove()},200)},o=e=>{e.key===`Escape`&&a()};document.addEventListener(`keydown`,o),r.addEventListener(`click`,a),i.querySelector(`.client-modal__close`)?.addEventListener(`click`,a);let s=i.querySelector(`#qm-body`);function c(){s.innerHTML=`
      ...
      </div>`,s.querySelector(`[data-act="done"]`)?.addEventListener(`click`,a),s.querySelector(`[data-act="resend"]`)?.addEventListener(`click`,()=>c())}function d(e,t=!1){let r=e.editability||{editable:e.status===`SUBMITTED`&&!e.locked_tz,reason:e.locked_tz?`finalized`:null,deadline:null},i=!!r.editable,o=!i&&e.status===`SUBMITTED`?r.reason===`finalized`?`<div class="qm-status" data-status="LOCKED"><span class="material-symbols-outlined" aria-hidden="true">lock</span> Finalized${e.locked_tz?` ${Y(rx(e.locked_tz))}`:``}${e.locked_by_email?` by ${Y(e.locked_by_email)}`:``} — no further edits.</div>`:`<div class="qm-status" data-status="LOCKED"><span class="material-symbols-outlined" aria-hidden="true">lock_clock</span> Editing closed${r.deadline?` ${Y(rx(r.deadline))}`:``} — a response locks 14 days after it is submitted.</div>`:``,c=i&&r.deadline?`<p class="client-modal__sub qm-editable-hint">You can edit these answers until ${Y(rx(r.deadline))}.</p>`:``;s.innerHTML=`
      <div class="qm-status" data-status="SUBMITTED"><span class="material-symbols-outlined" aria-hidden="true">task_alt</span> Submitted ${e.submitted_tz?Y(rx(e.submitted_tz)):``}</div>
      ${o}
      ${Vr(e)}
      <form id="qm-form">
      ...
      </div>`,s.querySelector(`[data-act="done"]`)?.addEventListener(`click`,a),s.querySelector(`[data-act="cancel-edit"]`)?.addEventListener(`click`,()=>d(e,!1)),s.querySelector(`[data-act="edit"]`)?.addEventListener(`click`,()=>d(e,!0));let l=s.querySelector(`[data-act="save"]`);l?.addEventListener(`click`,async()=>{let t=Br(s);l.disabled=!0;let r=l.innerHTML;l.innerHTML=`<span class="material-symbols-outlined" aria-hidden="true">progress_activity</span> Saving…`;let i;try{i=await b(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e.guid)}`,{method:`PUT`,headers:{"Content-Type":`application/json`},body:JSON.stringify({answers:t})});let r=await i.json().catch(()=>({}));if(!i.ok||!r.ok)throw Error(r.error||r.message||`Failed (HTTP ${i.status})`);Gb(`Answers updated`,`success`);let a=await(await b(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e.guid)}`)).json().catch(()=>({}));d(a.ok?a.data:{...e,answers:t},!1)}catch(t){if(i&&i.status===423){Gb(t.message||`This questionnaire is now locked.`,`info`);let r=await(await b(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e.guid)}`)).json().catch(()=>({}));d(r.ok?r.data:e,!1);return}s.querySelector(`#qm-error`).textContent=t.message||`Something went wrong. Please try again.`,l.disabled=!1,l.innerHTML=r}})}try{let t=await b(`${n}/m5t/v5/m5mQuestionnaireResponse?owner_guid=${encodeURIComponent(e)}`),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`HTTP ${t.status}`);let i=(r.data||[])[0]||null;if(!i){c();return}if(i.status===`DRAFT`){l(i);return}if(i.status===`CANCELLED`){u(i);return}let a=await b(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(i.guid)}`),o=await a.json().catch(()=>({}));if(!a.ok||!o.ok)throw Error(o.error||`HTTP ${a.status}`);d(o.data)}catch(e){console.warn(`[clients] questionnaire load failed:`,e),s.innerHTML=`<p class="client-modal__error">Could not load questionnaire status. ${Y(e.message||``)}</p>`}}var ax,ox,sx,cx,lx,Y,ux,dx,fx,px=o((()=>{T(),rt(),ax=u(f(),1),m(),An(),Kr(),x(),no(),ox=`reseller_org`,sx=`managed_client`,cx=!1,lx={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Y=e=>String(e??``).replace(/[&<>"']/g,e=>lx[e]),ux=e=>String(e||`?`).trim().split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``).toUpperCase()||`?`,dx=e=>String(e||``).replace(/[^0-9]/g,``),fx={render:Zb,init:Qb}})),mx=c({default:()=>Nx,init:()=>Cx,loadTeam:()=>wx,render:()=>Sx});function hx(){let e=w.getActiveOrg(),t=e&&e.channel!==Ox?e:null;if(!t){let e=w.getMemberships().find(e=>e.channel!==Ox);e&&(t={guid:e.orgGuid,channel:e.channel,memberRole:e.memberRole})}if(!t)return null;let n=w.getMemberships().find(e=>e.orgGuid===t.guid)?.name;return{...t,name:n}}function gx(e){return Object.values(Re).filter(t=>Le(e,t.value)).sort((e,t)=>t.rank-e.rank)}function _x(){if(kx)return;let e=document.createElement(`style`);e.id=`m5t-team-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),kx=!0}function vx(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function yx(e,t,n){let r=e.member_role||`member`,i=e.entity_guid&&e.entity_guid===w.getEntity?.()?.guid,a=n&&!i&&gx(t.memberRole).some(e=>e.value===r),o=t.channel===`reseller_org`&&a&&r!==`owner`;return`
    <div class="team-row">
      <span class="team-avatar">${X(Mx(e.name||e.email))}</span>
    ...
    </div>`}async function bx(e,t){let{uid:n,email:r}=e.dataset;if(window.confirm(`Revoke the invitation for ${r}?\n\nThe invitation link stops working immediately, including any copy already sent. You can invite this address again afterwards.`)){e.disabled=!0;try{let e=await b(`${w.getConfig?.().BASE_URL||``}/m5t/v5/acctGroup/${encodeURIComponent(t.guid)}/invites/${encodeURIComponent(n)}`,{method:`DELETE`}),i=await e.json().catch(()=>({}));if(!e.ok||!i.ok)throw Error(i.message||i.error||`HTTP ${e.status}`);jx(`Invitation for ${r} revoked`,`success`)}catch(t){e.disabled=!1,jx(t.message||`Could not revoke that invitation.`,`error`);return}try{await wx(t)}catch{}}}async function xx(e,t){let{id:n,name:r}=e.dataset;if(window.confirm(`Remove ${r} from the team?\n\nThey lose access to this workspace immediately. Their account itself is not deleted, and you can invite them again later.`)){e.disabled=!0;try{let e=await b(`${w.getConfig?.().BASE_URL||``}/m5t/v5/acctGroupMember/${encodeURIComponent(n)}`,{method:`DELETE`}),t=await e.json().catch(()=>({}));if(!e.ok||!t.ok)throw Error(t.message||t.error||`HTTP ${e.status}`);jx(`${r} removed from the team`,`success`)}catch(t){e.disabled=!1,jx(t.message||`Could not remove that member.`,`error`);return}try{await wx(t)}catch{}}}function Sx(){return _x(),`
    <div class="page-shell" data-section="team">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="team-body">
          ${vx(`hourglass_empty`,`Loading team…`)}
        </div>
      </div>
    </div>`}async function Cx(){let e=hx(),t=document.getElementById(`team-invite-btn`);if(!e){t&&(t.style.display=`none`),document.getElementById(`team-body`).innerHTML=vx(`info`,`Team management is available for agency and workspace accounts.`);return}t?.addEventListener(`click`,()=>Tx(e)),await wx(e)}async function wx(e){let t=document.getElementById(`team-body`),n=document.getElementById(`team-count`);if(t)try{let r=w.getConfig?.().BASE_URL||``,[i,a]=await Promise.all([b(`${r}/m5t/v5/acctGroup/${encodeURIComponent(e.guid)}/members`),b(`${r}/m5t/v5/acctGroup/${encodeURIComponent(e.guid)}/invites`)]),[o,s]=await Promise.all([i.json().catch(()=>({})),a.json().catch(()=>({}))]);j(i,o),j(a,s);let c=o.members||[],l=s.invites||[],u=gx(e.memberRole).length>0;t.innerHTML=(l.length?`
    ...
    <div class="team-modal__body" id="team-invite-body">
      ...
      <div class="team-modal__actions">
        ...
        </div>`;let l=r.querySelector(`#tm-link`);r.querySelector(`#tm-copy`)?.addEventListener(`click`,async()=>{try{await navigator.clipboard.writeText(l.value),r.querySelector(`#tm-copy`).textContent=`Copied!`}catch{l.select()}}),r.querySelector(`[data-act="done"]`)?.addEventListener(`click`,()=>{i(),wx(e)})}catch(e){console.warn(`[team] invite failed:`,e),o.textContent=e.message||`Something went wrong. Please try again.`,s.disabled=!1,s.innerHTML=l}})}async function Ex(e,t,n){let r=w.getConfig?.().BASE_URL||``,i;try{let t=await b(`${r}/m5t/v5/acctGroupMember/${encodeURIComponent(e)}/client-scope`),n=await t.json().catch(()=>({}));j(t,n),i=n.data||{}}catch(e){console.warn(`[team] client-scope load failed:`,e),jx(`Could not load client access. Please try again.`,`error`);return}if(!i.restrictable){jx(`This member always sees all clients.`,`info`);return}let a=i.clients||[],o=new Set(i.allowed||[]),s=!!i.restricted,c=document.createElement(`div`);c.className=`team-modal-scrim`;let l=document.createElement(`div`);l.className=`team-modal`,l.setAttribute(`role`,`dialog`),l.setAttribute(`aria-modal`,`true`),l.innerHTML=`
    <div class="team-modal__head">
    ...
    </div>`,document.body.appendChild(c),document.body.appendChild(l),requestAnimationFrame(()=>{c.classList.add(`open`),l.classList.add(`open`)});let u=()=>{c.classList.remove(`open`),l.classList.remove(`open`),document.removeEventListener(`keydown`,d),setTimeout(()=>{c.remove(),l.remove()},200)},d=e=>{e.key===`Escape`&&u()};document.addEventListener(`keydown`,d),c.addEventListener(`click`,u),l.querySelector(`.team-modal__close`)?.addEventListener(`click`,u),l.querySelector(`[data-act="cancel"]`)?.addEventListener(`click`,u);let f=l.querySelector(`#tm-clients`);l.querySelectorAll(`input[name="tm-scope"]`).forEach(e=>e.addEventListener(`change`,()=>{f.classList.toggle(`open`,l.querySelector(`input[name="tm-scope"]:checked`)?.value===`assigned`)}));let p=l.querySelector(`#tm-scope-error`),m=l.querySelector(`[data-act="save"]`);m?.addEventListener(`click`,async()=>{p.textContent=``;let t=l.querySelector(`input[name="tm-scope"]:checked`)?.value,i=[...l.querySelectorAll(`input[data-client]:checked`)].map(e=>e.dataset.client);if(t===`assigned`&&!i.length){p.textContent=`Tick at least one client, or choose All clients.`;return}m.disabled=!0;try{let a=await b(`${r}/m5t/v5/acctGroupMember/${encodeURIComponent(e)}/client-scope`,{method:`PUT`,headers:{"Content-Type":`application/json`},body:JSON.stringify({client_guids:t===`assigned`?i:[]})}),o=await a.json().catch(()=>({}));if(j(a,o),!o.ok)throw Error(o.message||o.error||`Save failed`);jx(`Client access updated.`,`success`),u(),wx(n)}catch(e){console.warn(`[team] client-scope save failed:`,e),p.textContent=e.message||`Something went wrong. Please try again.`,m.disabled=!1}})}var Dx,Ox,kx,Ax,X,jx,Mx,Nx,Px=o((()=>{T(),Dx=u(f(),1),m(),rt(),An(),x(),no(),Ox=`managed_client`,kx=!1,Ax={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},X=e=>String(e??``).replace(/[&<>"']/g,e=>Ax[e]),jx=(e,t=`info`)=>{try{Dx.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},Mx=e=>String(e||`?`).trim().split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``).toUpperCase()||`?`,Nx={render:Sx,init:Cx}}));m(),vt(),T(),dn(),y(),$n(),or(),St(),window.m5t=Object.create(null),window.m5t.pubsub=E.default,console.log(`StreamSyncEngage SaaS Client - Starting...`),console.log(`Environment:`,`production`),console.log(`Document readyState:`,document.readyState),console.log(`window.mvvLegit:`,typeof window.mvvLegit),console.log(`globalThis.mvvLegit:`,typeof globalThis.mvvLegit);try{console.log(`mvvLegit (direct):`,typeof mvvLegit)}catch(e){console.log(`mvvLegit (direct): ReferenceError -`,e.message)}var Fx=Object.keys(window).filter(e=>e.toLowerCase().includes(`mvv`)||e.toLowerCase().includes(`neodigm`)||e.toLowerCase().includes(`legit`));console.log(`Possible Neodigm globals:`,Fx);var Z=new ee.default(`/`,{hash:!0});window.router=Z;function Q(e){let t=e?`${e.replace(/-/g,`_`)}_route`:`splash_route`;return console.log(`[Router] Checking access: ${e} -> ${t}`),mvvLegit.isRouteAllowed(t)?!0:(console.warn(`[Router] Access denied to route: ${t}`),E.default.publish(p.UI_TOAST,{message:`Please sign in to access this page`,type:`warning`}),!1)}function $(e){console.log(`[Router] Navigating to:`,e),Z.navigate(`/${e}`)}var Ix=new Set([`splash`,`signin`,`signup`,`forgot`,`resethash`,`verf-link`,`accept-invite`,`signout`]),Lx={},Rx={},zx=null,Bx={admin:{caption:`Platform Admin`,icon:`shield_person`},reseller:{caption:`Reseller`,icon:`handshake`},managed:{caption:`Managed`,icon:`visibility`},self_serve:{caption:`Self-Serve`,icon:`rocket_launch`}},Vx={free:`rocket_launch`,pro:`workspace_premium`,enterprise:`diamond`,grandfathered:`all_inclusive`},Hx=null,Ux=!1;function Wx(){return Hx||(Ux||(Ux=!0,C(async()=>{let{fetchMyPlan:e}=await Promise.resolve().then(()=>(pr(),sr));return{fetchMyPlan:e}},void 0,import.meta.url).then(({fetchMyPlan:e})=>e()).then(e=>{let t=e?.plan;!t?.label||!t?.key||(Hx={caption:t.key===`grandfathered`?t.label:`${t.label} plan`,icon:Vx[t.key]||`rocket_launch`},qx())}).catch(()=>{})),Bx.self_serve)}var Gx=`__portfolio__`;function Kx(){try{Z._setCurrent(null)}catch{}Z.resolve()}function qx(){let e=document.getElementById(`m5t-side-nav`);if(!e)return;let t=!w.getScope()&&w.isAuthenticated(),n=w.getCapabilities(),r=w.getUiMode(),i=gn.getSideNav(n,{uiMode:r});Lx={},Rx={};let a=e=>{e.route&&(Lx[e.route]={sectionToken:e.token,childToken:null,children:e.children||null},Array.isArray(e.children)&&e.children.forEach(t=>{t.route&&(Rx[t.token]=t.route,Lx[t.route]={sectionToken:e.token,childToken:t.token,children:e.children})}))},o=e=>e.forEach(e=>{e.group&&Array.isArray(e.items)?o(e.items):a(e)});o(i);let s=w.getMemberships(),c=w.getActiveOrg(),l=w.getManagedClients(),u=w.getStoredActiveOrg(),d=l.length?[{guid:Gx,name:Cn(w.isClientScopeRestricted?.())},...s.map(e=>({guid:e.orgGuid,name:e.name||e.channel,channel:e.channel,group:`Your organization`})),...l.map(e=>({guid:e.orgGuid,name:e.name||e.orgGuid,channel:e.channel,group:`Clients`}))]:s.map(e=>({guid:e.orgGuid,name:e.name||e.channel,channel:e.channel}));e.setState({buttons:i,orgs:d,activeOrg:l.length?u||Gx:c?.guid||null,footerBadge:r===`self_serve`?Wx():Bx[r]||null,loading:t}),Yx(),zx&&Xx(zx)}var Jx=e=>String(e??``).replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`})[e]);function Yx(){let e=document.getElementById(`app-content`);if(!e)return;let t=document.getElementById(`org-context-banner`);t||(t=document.createElement(`div`),t.id=`org-context-banner`,e.insertBefore(t,e.firstChild),Wn(t));let n=w.getActiveOrg(),r=w.isPreviewingAsClient(),i=!!n?.viaSubtree&&!!zx&&!Ix.has(zx);if(t.hidden=!i,!i){t.innerHTML=``;return}let a=Jx(n.name||`Client`);t.innerHTML=`
    <style>
