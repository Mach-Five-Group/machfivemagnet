...
`))}</textarea>`}return`<div class="mag-cta" data-type="${e}" data-kind="${n.kind}">
    ...
    ${o}
  </div>`}function pf(){if(dp)return;let e=document.createElement(`style`);e.id=`m5t-magnets-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),dp=!0}function mf(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function hf(e){let t=z.stats[e];if(!t||!t.opens&&!t.leads)return`<div class="mag-stats"><span><span class="material-symbols-outlined" aria-hidden="true">monitoring</span>No activity yet (30d)</span></div>`;let n=e=>Number(e||0).toLocaleString();return`<div class="mag-stats" title="Trailing 30 days">
    ...
    <div class="mag-card" data-guid="${B(e.guid)}">
      ...
      <span class="mag-mode"><span class="material-symbols-outlined" aria-hidden="true">${_p[e.display_mode]||`chat`}</span>${B(gp[e.display_mode]||`Chat bubble`)}${e.ab?.enabled?`&nbsp;&nbsp;<span class="mag-ab-chip"><span class="material-symbols-outlined" aria-hidden="true">science</span>A/B</span>`:``}${(e.notify_emails||[]).length?`&nbsp;&nbsp;<a class="mag-alert-chip" href="#/settings/notifications" title="Lead alerts to ${(e.notify_emails||[]).length} email${(e.notify_emails||[]).length===1?``:`s`}. Manage in Settings."><span class="material-symbols-outlined" aria-hidden="true">notifications_active</span>${(e.notify_emails||[]).length}</a>`:``}</span>
      ${hf(e.guid)}
      ...
      <div class="mag-embed">
        <input type="text" readonly value="${B(pp(e.guid))}" aria-label="Embed snippet">
    ...
    </div>`}function Pf(){return A()?ta({section:`magnets`,title:`Your Magnets`,description:`Pick a client to view and manage their magnets.`}):(pf(),`
    ...
    </div>`)}async function Ff(){if(A()){let e=Hd();if(e){let t=document.querySelector(`.page-shell .page-header-text .page-description`);t&&(t.textContent=`Template picked: ${e.name}. Choose the client to build it for.`)}let t=e=>Number(e||0).toLocaleString();return na(H(),[{h:`Magnets`,td:e=>`${t(e.magnets.active)}<span style="color:var(--color-text-subtle,#6C7C9A)">/${t(e.magnets.total)}</span>`},{h:`Leads · wk`,td:e=>t(e.leads.this_week)},{h:`Leads · total`,td:e=>t(e.leads.total)}],{emptyMsg:`No client orgs yet. Create one under Clients, then provision its magnets here.`})}Cp=null,wp=null,document.getElementById(`mag-new-btn`)?.addEventListener(`click`,Uf),document.getElementById(`mag-template-btn`)?.addEventListener(`click`,Gf),document.getElementById(`mag-body`)?.addEventListener(`click`,Kf),await Lf();let e=Vd();e&&z.canProvision&&Wf(e)}function If(e){e&&(e.innerHTML=`<div class="mag-grid">${z.magnets.map(Nf).join(``)}</div>`,Yt(e.querySelector(`.mag-grid`),{rowSelector:`.mag-card`,noun:`magnets`}))}async function Lf(){let e=document.getElementById(`mag-body`);if(!e)return;try{let[e,t]=await Promise.all([fetch(`${H()}/m5t/v5/magnets`),fetch(`${H()}/m5t/v5/magnets/stats`).catch(()=>null)]),n=await e.json().catch(()=>({}));if(!n.ok)throw Error(n.message||`Failed to load`);z.magnets=n.magnets||[],z.owners=n.owners||[],z.canProvision=!!n.canProvision,z.canManage=!!n.canManage,z.primaryOrgGuid=n.primaryOrgGuid||null;let r=t?await t.json().catch(()=>({})):{};z.stats=r?.stats||{}}catch{e.innerHTML=mf(`error`,`Could not load your magnets. Please try again.`);return}let t=document.getElementById(`mag-new-btn`);t&&(t.style.display=z.canProvision?``:`none`);let n=document.getElementById(`mag-template-btn`);if(n&&(n.style.display=z.canProvision?``:`none`),!z.magnets.length){e.innerHTML=mf(`widgets`,z.canProvision?`No magnets yet. Click “New magnet” to create your first.`:`No magnets yet.`);return}If(e),_f(),wf()}async function Rf(e){let t={"Content-Type":`application/json`,"Idempotency-Key":crypto.randomUUID()},n={status:`DRAFT`};e&&(n.owner_guid=e);let r=await fetch(`${H()}/m5t/v5/magnets`,{method:`POST`,headers:t,body:JSON.stringify(n)}),i={};try{i=await r.json()}catch{}if(!r.ok||!i.ok)throw Error(i.message||`Could not start a new magnet (${r.status})`);return i.magnet}async function zf(e,t){let n=await fetch(`${H()}/m5t/v5/magnets/${encodeURIComponent(e)}`,{method:`PATCH`,headers:{"Content-Type":`application/json`},body:JSON.stringify({name:t.name,...t.appearance,steps:t.steps})}),r={};try{r=await n.json()}catch{}if(!n.ok||!r.ok)throw Error(r.message||`Could not apply the ${t.name} template (${n.status})`)}async function Bf(e,t){let n=document.getElementById(`mag-new-btn`),r=document.getElementById(`mag-template-btn`);n&&(n.disabled=!0),r&&(r.disabled=!0);let i=null;try{let n=await Rf(e);i=n.guid,t?(await zf(n.guid,t),await Lf(),n=z.magnets.find(e=>e.guid===n.guid)||n):(z.magnets=[n,...z.magnets],If(document.getElementById(`mag-body`))),lp(n,void 0,{freshDraft:!0})}catch(e){i&&fetch(`${H()}/m5t/v5/magnets/${encodeURIComponent(i)}`,{method:`DELETE`}).catch(()=>{}),V(e.message||`Could not start a new magnet`,`danger`)}finally{n&&(n.disabled=!1),r&&(r.disabled=!1)}}function Vf(e){let t=document.createElement(`div`);t.className=`mag-modal-scrim`,t.innerHTML=`
    <div class="mag-modal" role="dialog" aria-modal="true" aria-label="Choose client" style="max-width:420px">
    ...
    </div>`,document.body.appendChild(t),requestAnimationFrame(()=>{t.classList.add(`open`),t.querySelector(`.mag-modal`).classList.add(`open`)});let n=()=>{t.classList.remove(`open`),setTimeout(()=>t.remove(),180),document.removeEventListener(`keydown`,r)};function r(e){e.key===`Escape`&&(e.preventDefault(),n())}document.addEventListener(`keydown`,r),t.querySelector(`.mag-modal__close`).addEventListener(`click`,n),t.querySelector(`[data-x="cancel"]`).addEventListener(`click`,n),t.querySelector(`[data-x="continue"]`).addEventListener(`click`,()=>{let r=t.querySelector(`#mag-owner-pick`).value;n(),Bf(r,e)})}function Hf(){return z.owners.length===1?z.owners[0].guid:null}function Uf(){if(z.owners.length>1){Vf();return}Bf(Hf())}function Wf(e){if(z.owners.length>1){Vf(e);return}Bf(Hf(),e)}function Gf(){Wd(Wf)}function Kf(e){let t=e.target.closest(`[data-action]`);if(!t)return;let n=t.dataset.guid,r=z.magnets.find(e=>e.guid===n),i=t.dataset.action;if(i===`copy`)return qf(n);if(r){if(i===`preview`)return Nd(()=>Dd(r));if(i===`funnel`)return Yf(r);if(i===`edit`)return lp(r);if(i===`flow`)return lp(r,`flow`);if(i===`allow-domain`)return Xf(r,t.dataset.domain);if(i===`dismiss-domain`)return Zf(r,t.dataset.domain);if(i===`restore-domain`)return Qf(r,t.dataset.domain);if(i===`pair-start`)return kf(r);if(i===`copy-code`)return Af(r);if(i===`revoke-connection`)return Mf(r,t.dataset.conn);if(i===`clear-connection`)return jf(r,t.dataset.conn,t.dataset.label);if(i===`toggle`)return ep(r);if(i===`delete`)return tp(r)}}async function qf(e){try{await navigator.clipboard.writeText(pp(e)),V(`Embed snippet copied`,`success`)}catch{V(`Copy failed`,`danger`)}}function Jf(e){let t=Number(e?.total_steps)||0,n={};if((Array.isArray(e?.funnel)?e.funnel:[]).forEach(e=>{n[e.position]=Number(e.reached)||0}),!t||!Object.keys(n).length)return`<p class="mag-modal__hint" style="margin:0">No step activity yet in this window. As visitors answer questions, drop-off by step will appear here.</p>`;let r=Math.max(1,n[1]||0),i=[];for(let e=1;e<=t;e++){let a=n[e]||0,o=Math.min(100,Math.round(a/r*100));i.push(`
      ...
      </div>`)}return`<div class="mag-funnel">${i.join(``)}</div>`}async function Yf(e){let t=document.createElement(`div`);t.className=`mag-modal-scrim`,t.innerHTML=`
    ...
    <div class="mag-modal" role="dialog" aria-modal="true" aria-label="${i?`Edit magnet`:`New magnet`}">
      ...
      <div class="mag-modal__main">
        ...
        <select class="mag-modal__select" id="mag-f-trigger">
          ${bp.map(t=>`<option value="${t.value}"${ef(e)===t.value?` selected`:``}>${B(t.label)}</option>`).join(``)}
        ...
        <div class="mag-perdev" id="mag-f-perdev"${ef(e)===`__per_device`?``:` hidden`}>
          <div class="mag-perdev__row">
            ...
            <select class="mag-modal__select" id="mag-f-trigger-desktop">
              ${bp.map(t=>`<option value="${t.value}"${tf(e,`desktop`)===t.value?` selected`:``}>${B(t.label)}</option>`).join(``)}
            </select>
          </div>
          <div class="mag-perdev__row">
            <label class="mag-modal__label" for="mag-f-trigger-mobile">Mobile</label>
            <select class="mag-modal__select" id="mag-f-trigger-mobile">
              ${bp.map(t=>`<option value="${t.value}"${tf(e,`mobile`)===t.value?` selected`:``}>${B(t.label)}</option>`).join(``)}
            </select>
          </div>
...
</details>`,hp=e=>{let t=B(pp(e));return`<details class="mag-guide">
...
/&gt;</code></pre>
...
</details>`},gp={chat:`Chat bubble`,right_slide:`Slide-in panel`,modal:`Modal`,inline:`Inline`},_p={chat:`chat`,right_slide:`vertical_split`,modal:`web_asset`,inline:`view_agenda`},vp=[{value:`inline`,icon:`view_agenda`,label:`Inline`},{value:`chat`,icon:`chat`,label:`Chat`},{value:`right_slide`,icon:`vertical_split`,label:`Slide`},{value:`modal`,icon:`web_asset`,label:`Modal`}],yp=[{value:`auto`,icon:`brightness_auto`,label:`Auto`},{value:`sync`,icon:`sync`,label:`Sync`},{value:`light`,icon:`light_mode`,label:`Light`},{value:`dark`,icon:`dark_mode`,label:`Dark`}],bp=[{value:`on_load`,label:`Immediately on page load`},{value:`on_8_seconds`,label:`After 8 seconds`},{value:`on_scroll_80_percent`,label:`At 80% scroll depth`},{value:`on_exit_intent`,label:`On exit intent (mouse leaves the page)`},{value:`none`,label:`Manual only (visitor clicks the launcher)`}],xp={book:{label:`Book`,icon:`event`,kind:`book`},chat:{label:`Chat`,icon:`chat`,kind:`flow`},call:{label:`Call`,icon:`phone`,kind:`value`,placeholder:`Phone number`},email:{label:`Email`,icon:`mail`,kind:`value`,placeholder:`Email address`},support:{label:`Support`,icon:`support_agent`,kind:`support`}},Sp=[`book`,`chat`,`call`,`email`,`support`],Cp=null,wp=null,Tp=5e3,Ep={ACTIVE:{label:`Connected`,icon:`check_circle`,tone:`live`},PENDING:{label:`Waiting to connect`,icon:`hourglass_empty`,tone:`pending`},REVOKED:{label:`Disconnected`,icon:`link_off`,tone:`off`},EXPIRED:{label:`Expired`,icon:`timer_off`,tone:`off`}},Dp=512,Op=new Set([`image/png`,`image/jpeg`,`image/gif`,`image/webp`]),kp=8388608,Ap=[{label:`Web fonts (loaded for you)`,opts:[[`Inter, sans-serif`,`Inter`],[`Poppins, sans-serif`,`Poppins`],[`Montserrat, sans-serif`,`Montserrat`],[`Roboto, sans-serif`,`Roboto`],[`"Open Sans", sans-serif`,`Open Sans`],[`Lato, sans-serif`,`Lato`],[`Nunito, sans-serif`,`Nunito`],[`Raleway, sans-serif`,`Raleway`],[`"Work Sans", sans-serif`,`Work Sans`],[`Oswald, sans-serif`,`Oswald`],[`Merriweather, serif`,`Merriweather (serif)`]]},{label:`System / always available`,opts:[[`system-ui, sans-serif`,`System UI`],[`Georgia, serif`,`Georgia (serif)`],[`"Times New Roman", Times, serif`,`Times (serif)`],[`Arial, Helvetica, sans-serif`,`Arial / Helvetica`],[`Verdana, Geneva, sans-serif`,`Verdana`],[`"Courier New", monospace`,`Courier (mono)`]]}]})),Mp=c({default:()=>Up,init:()=>Ip,render:()=>Fp});function Np(){let e=new Map;for(let t of Jd)e.set(t.category,(e.get(t.category)||0)+1);return Object.keys(qd).filter(t=>e.has(t)).map(t=>[t,e.get(t)])}function Pp(){if(Ud(),Hp)return;let e=document.createElement(`style`);e.id=`m5t-templates-page-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Hp=!0}function Fp(){Pp();let e=Bp();return`
    <div class="page-shell" data-section="templates">
      ...
      <div class="m5t-chip-row tpl-filters" id="tpl-filters" role="group" aria-label="Filter templates by category">
        ${Vp(`all`,`All`,Jd.length,`true`)}
        ${Np().map(([e,t])=>Vp(e,qd[e],t,`false`)).join(``)}
      </div>

      <div class="mag-tpl-grid mag-tpl-grid--page" id="tpl-grid">
        ${Jd.map(t=>Rd(t,{detail:!0,gallery:!0,canUse:e})).join(``)}
      </div>
    </div>`}function Ip(){let e=document.getElementById(`tpl-grid`);document.getElementById(`tpl-filters`)?.addEventListener(`click`,t=>{let n=t.target.closest(`[data-cat]`);if(!n)return;n.parentElement.querySelectorAll(`[data-cat]`).forEach(e=>{e.setAttribute(`aria-pressed`,String(e===n))});let r=n.dataset.cat;e?.querySelectorAll(`.mag-tpl-card`).forEach(e=>{e.hidden=r!==`all`&&e.dataset.cat!==r})}),e?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-preview]`);if(t){let e=Jd.find(e=>e.id===t.dataset.preview);e&&Nd(()=>Dd({...e.appearance,steps:e.steps,name:e.name}));return}let n=e.target.closest(`[data-tpl]`);n&&(zd(n.dataset.tpl),window.location.hash=`#/magnets`)})}var Lp,Rp,zp,Bp,Vp,Hp,Up,Wp=o((()=>{T(),Fd(),Qd(),Lp={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Rp=e=>String(e??``).replace(/[&<>"']/g,e=>Lp[e]),zp=[`provision_magnets`,`manage_magnets`],Bp=()=>w.getCapabilities().some(e=>zp.includes(e)),Vp=(e,t,n,r)=>`<button class="m5t-chip--filter" type="button" data-cat="${Rp(e)}" aria-pressed="${r}">${Rp(t)}<span class="m5t-chip__count">${n}</span></button>`,Hp=!1,Up={render:Fp,init:Ip}})),Gp=c({default:()=>hm,init:()=>Zp,render:()=>Xp});function Kp(){if(nr(),fm)return;let e=document.createElement(`style`);e.id=`m5t-questionnaires-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),fm=!0}function qp(){return U.clients.map(e=>{let t=U.responses.filter(t=>t.owner_guid===e.guid),n=t[0]||null;return{guid:e.guid,name:e.caption||e.name||e.guid,latest:n,earlier:Math.max(0,t.length-1),status:n?n.status:`NOT_SENT`}})}function Jp(){let e=U.search.trim().toLowerCase();return qp().filter(t=>(U.filter===`ALL`||t.status===U.filter)&&(!e||t.name.toLowerCase().includes(e)))}function Yp(e){let t=e.status===`SUBMITTED`,n=e.status===`SUBMITTED`?`<button class="qz-act" type="button" data-act="open" data-guid="${om(e.latest.guid)}">Read answers</button>`:e.status===`DRAFT`?`<button class="qz-act" type="button" data-act="copy" data-url="${om(e.latest.url||``)}">Copy link</button>`:`<a class="qz-act" href="#/clients">Go to client</a>`;return`<tr class="qz-row${t?` qz-row--open`:``}"${t?` data-open-guid="${om(e.latest.guid)}"`:``}>
    <td><span class="qz-client">${om(e.name)}</span>${e.earlier?`<span class="qz-earlier">${e.earlier} earlier</span>`:``}</td>
    <td><span class="qz-badge" data-s="${om(e.status)}">${om(dm[e.status]||e.status)}</span></td>
    <td class="qz-when">${om(mm(e,`created_tz`))}</td>
    <td class="qz-when">${om(mm(e,`submitted_tz`))}</td>
    <td class="qz-actcell">${n}</td>
  </tr>`}function Xp(){return Kp(),`
    ...
    </div>`}async function Zp(){await Qp()}async function Qp(){let e=document.getElementById(`qz-body`);if(e){try{let[e,t]=await Promise.all([fetch(`${sm()}/m5t/v5/acctGroup`),fetch(`${sm()}/m5t/v5/m5mQuestionnaireResponse`)]),n=await e.json().catch(()=>({})),r=await t.json().catch(()=>({}));if(!e.ok||!t.ok)throw Error(`load failed`);U.clients=(n.data||[]).filter(e=>e.class===lm&&![`DELETED`,`ARCHIVED`].includes(e.status)).sort((e,t)=>String(e.caption||``).localeCompare(String(t.caption||``))),U.responses=r.data||[]}catch{e.innerHTML=pm(`error`,`Could not load questionnaires. Please try again.`);return}em()}}function $p(){let e=document.getElementById(`qz-tbody`);e&&Yt(e,{rowSelector:`tr.qz-row`,noun:`clients`,controlHost:e.closest(`.qz-tablewrap`)||e.parentElement})}function em(){let e=document.getElementById(`qz-body`);if(!e)return;if(!U.clients.length){e.innerHTML=pm(`groups`,`No clients yet. Add a client first, then send them a kickoff questionnaire.`);return}let t=qp(),n={ALL:t.length};um.slice(1).forEach(([e])=>{n[e]=t.filter(t=>t.status===e).length});let r=Jp();e.innerHTML=`
    <div class="qz-bar">
      <div class="qz-filter">
        ${um.map(([e,t])=>`<button data-filter="${e}" class="${U.filter===e?`active`:``}">${om(t)}${n[e]?`<span class="qz-pillnum">${n[e]}</span>`:``}</button>`).join(``)}
      </div>
      <span class="qz-searchwrap">
        <span class="material-symbols-outlined" aria-hidden="true">search</span>
        <input class="qz-search" id="qz-search" type="search" placeholder="Search clients" value="${om(U.search)}" aria-label="Search clients" />
      </span>
    ...
    <div class="qz-tablewrap">
      <table class="qz-table">
        ...
        <tbody id="qz-tbody">${r.length?r.map(Yp).join(``):`<tr><td colspan="5"><div class="qz-when" style="padding:14px 4px">${U.search.trim()?`No clients match your search.`:`No clients with status “${om(dm[U.filter]||U.filter)}”.`}</div></td></tr>`}</tbody>
      </table>
    </div>`,$p(),e.querySelector(`.qz-filter`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-filter]`);t&&(U.filter=t.dataset.filter,em())}),e.querySelector(`#qz-search`)?.addEventListener(`input`,e=>{U.search=e.target.value,tm()}),e.querySelector(`#qz-tbody`)?.addEventListener(`click`,nm)}function tm(){let e=document.getElementById(`qz-tbody`);if(!e)return;let t=Jp();e.innerHTML=t.length?t.map(Yp).join(``):`<tr><td colspan="5"><div class="qz-when" style="padding:14px 4px">No clients match your search.</div></td></tr>`,$p();let n=document.querySelector(`.qz-count`);n&&(n.textContent=`${t.length} of ${qp().length}`)}function nm(e){let t=e.target.closest(`[data-act="copy"]`);if(t){e.stopPropagation();let n=t.dataset.url;if(!n){cm(`No link on this questionnaire yet.`,`warning`);return}navigator.clipboard?.writeText(n).then(()=>{t.textContent=`Copied!`,setTimeout(()=>{t.textContent=`Copy link`},1500)},()=>cm(`Could not copy the link.`,`error`));return}let n=e.target.closest(`[data-act="open"]`),r=e.target.closest(`[data-open-guid]`),i=n?.dataset.guid||r?.dataset.openGuid;i&&rm(i)}async function rm(e){U.openGuid=e;let t=document.createElement(`div`);t.className=`qz-drawer-scrim`;let n=document.createElement(`aside`);n.className=`qz-drawer`,n.setAttribute(`role`,`dialog`),n.setAttribute(`aria-label`,`Questionnaire answers`),n.innerHTML=`
    ...
    <div class="qz-drawer__body" id="qz-dbody">${pm(`hourglass_empty`,`Loading answers…`)}</div>`,document.body.append(t,n),requestAnimationFrame(()=>{t.classList.add(`open`),n.classList.add(`open`)});let r=()=>{U.openGuid=null,t.classList.remove(`open`),n.classList.remove(`open`),setTimeout(()=>{t.remove(),n.remove()},220),document.removeEventListener(`keydown`,i)};function i(e){e.key===`Escape`&&(e.preventDefault(),r())}document.addEventListener(`keydown`,i),t.addEventListener(`click`,r),n.querySelector(`.qz-drawer__close`).addEventListener(`click`,r);try{let t=await fetch(`${sm()}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e)}`),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`load failed`);let i=r.data||{},a=U.clients.find(e=>e.guid===i.owner_guid)?.caption||`Client`;n.querySelector(`#qz-dtitle`).textContent=a;let o=i.submitted_tz?`Submitted ${D(i.submitted_tz)}`:`Not yet submitted`;n.querySelector(`#qz-dsub`).textContent=i.locked_tz?`${o} · Finalized`:o,n.querySelector(`#qz-dbody`).innerHTML=tr(i)+Qn(i,{emptyLabel:`Not answered`})}catch{n.querySelector(`#qz-dbody`).innerHTML=pm(`error`,`Could not load these answers. Please try again.`)}}var im,am,om,sm,cm,lm,um,dm,U,fm,pm,mm,hm,gm=o((()=>{T(),im=u(f(),1),m(),or(),Zt(),am={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},om=e=>String(e??``).replace(/[&<>"']/g,e=>am[e]),sm=()=>w.getConfig?.().BASE_URL||``,cm=(e,t=`info`)=>{try{im.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},lm=`managed_client`,um=[[`ALL`,`All`],[`NOT_SENT`,`Not sent`],[`DRAFT`,`Awaiting response`],[`SUBMITTED`,`Submitted`],[`CANCELLED`,`Cancelled`]],dm=Object.fromEntries(um),U={clients:[],responses:[],filter:`ALL`,search:``,openGuid:null},fm=!1,pm=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,mm=(e,t)=>e?.latest?.[t]?D(e.latest[t]):`—`,hm={render:Xp,init:Zp}})),_m=c({default:()=>Ym,init:()=>Cm,render:()=>Sm});function vm(){if(nr(),Gm)return;let e=document.createElement(`style`);e.id=`m5t-deliverables-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Gm=!0}function ym(){let e=K.search.trim().toLowerCase();return K.rows.filter(t=>(K.filter===`ALL`||t.status===K.filter)&&(!e||String(t.owner_name||``).toLowerCase().includes(e)))}function bm(e){return e.status===`DRAFT`?`<button class="dz-act" type="button" data-act="open" data-guid="${W(e.guid)}">Edit</button>`:e.status===`SENT`?`<button class="dz-act" type="button" data-act="copy" data-url="${W(e.url||``)}">Copy link</button>`:e.status===`SUBMITTED`?`<button class="dz-act" type="button" data-act="open" data-guid="${W(e.guid)}">Read review</button>`:`<span class="dz-when">—</span>`}function xm(e){let t=e.status!==`CANCELLED`;return`<tr class="dz-row${t?` dz-row--open`:``}"${t?` data-open-guid="${W(e.guid)}"`:``}>
    ...
    <td><span class="dz-badge" data-s="${W(e.status)}">${W(Wm[e.status]||e.status)}</span></td>
    <td class="dz-when">${e.sent_tz?W(D(e.sent_tz)):`—`}</td>
    <td class="dz-when">${e.submitted_tz?W(D(e.submitted_tz)):`—`}</td>
    <td class="dz-actcell">${bm(e)}</td>
  </tr>`}function Sm(){return vm(),`
    <div class="page-shell" data-section="deliverables">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="dz-body">${Km(`hourglass_empty`,`Loading…`)}</div>
      </div>
    </div>`}async function Cm(){document.getElementById(`dz-new`)?.addEventListener(`click`,Am),await wm()}async function wm(){let e=document.getElementById(`dz-body`);if(e){try{let[e,t,n]=await Promise.all([fetch(`${Vm()}/m5t/v5/m5mDeliverable`),fetch(`${Vm()}/m5t/v5/acctGroup`),fetch(`${Vm()}/m5t/v5/m5mDeliverable/templates`)]),r=await e.json().catch(()=>({})),i=await t.json().catch(()=>({})),a=await n.json().catch(()=>({}));if(!e.ok||!t.ok||!n.ok)throw Error(`load failed`);K.rows=r.data||[],K.clients=(i.data||[]).filter(e=>e.class===Hm&&![`DELETED`,`ARCHIVED`].includes(e.status)).sort((e,t)=>String(e.caption||``).localeCompare(String(t.caption||``))),K.templates=a.data||[]}catch{e.innerHTML=Km(`error`,`Could not load deliverables. Please try again.`);return}Em()}}function Tm(){let e=document.getElementById(`dz-tbody`);e&&Yt(e,{rowSelector:`tr.dz-row`,noun:`deliverables`,controlHost:e.closest(`.dz-tablewrap`)||e.parentElement})}function Em(){let e=document.getElementById(`dz-body`);if(!e)return;if(!K.clients.length&&!K.rows.length){e.innerHTML=Km(`groups`,`No clients yet. Add a client first, then author their first deliverable.`);return}if(!K.rows.length){e.innerHTML=Km(`approval`,`No deliverables yet. Author the first one with New deliverable.`);return}let t={ALL:K.rows.length};Um.slice(1).forEach(([e])=>{t[e]=K.rows.filter(t=>t.status===e).length});let n=ym();e.innerHTML=`
    <div class="dz-bar">
      <div class="dz-filter">
        ${Um.map(([e,n])=>`<button data-filter="${e}" class="${K.filter===e?`active`:``}">${W(n)}${t[e]?`<span class="dz-pillnum">${t[e]}</span>`:``}</button>`).join(``)}
      </div>
    ...
    <div class="dz-tablewrap">
      <table class="dz-table">
        ...
        <tbody id="dz-tbody">${n.length?n.map(xm).join(``):`<tr><td colspan="6"><div class="dz-when" style="padding:14px 4px">${K.search.trim()?`No deliverables match your search.`:`No deliverables with status “${W(Wm[K.filter]||K.filter)}”.`}</div></td></tr>`}</tbody>
      </table>
    </div>`,Tm(),e.querySelector(`.dz-filter`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-filter]`);t&&(K.filter=t.dataset.filter,Em())}),e.querySelector(`#dz-search`)?.addEventListener(`input`,e=>{K.search=e.target.value,Dm()}),e.querySelector(`#dz-tbody`)?.addEventListener(`click`,Om)}function Dm(){let e=document.getElementById(`dz-tbody`);if(!e)return;let t=ym();e.innerHTML=t.length?t.map(xm).join(``):`<tr><td colspan="6"><div class="dz-when" style="padding:14px 4px">No deliverables match your search.</div></td></tr>`,Tm();let n=document.querySelector(`.dz-count`);n&&(n.textContent=`${t.length} of ${K.rows.length}`)}function Om(e){let t=e.target.closest(`[data-act="copy"]`);if(t){e.stopPropagation();let n=t.dataset.url;if(!n){G(`No link on this deliverable yet.`,`warning`);return}navigator.clipboard?.writeText(n).then(()=>{t.textContent=`Copied!`,setTimeout(()=>{t.textContent=`Copy link`},1500)},()=>G(`Could not copy the link.`,`error`));return}let n=e.target.closest(`[data-act="open"]`),r=e.target.closest(`[data-open-guid]`),i=n?.dataset.guid||r?.dataset.openGuid;i&&Rm(i)}function km(e){let t=document.createElement(`div`);t.className=`dz-drawer-scrim`;let n=document.createElement(`aside`);n.className=`dz-drawer`,n.setAttribute(`role`,`dialog`),n.setAttribute(`aria-label`,e),n.innerHTML=`
    ...
    </div>`}function Mm(e,t,n,{hidden:r=!1}={}){let i=`${t}: `,a=e[0]?.group||``;return`
      <div class="dz-cblock" data-group="${W(a)}" data-heading="${W(t)}"${r?` hidden`:``}>
        <h4 class="dz-cblock__title">${W(t)}</h4>
        ${e.map(e=>jm(e,n,e.label.startsWith(i)?e.label.slice(i.length):e.label)).join(``)}
      </div>`}function Nm(e,t){let n=(e.questions||[]).filter(e=>e.audience===`am`),r=n.filter(e=>!e.group).map(e=>jm(e,t)).join(``),i=wr(e);if(i){let n=Tr(e).filter(e=>e.audience===`am`);if(!n.length)return r;let a=Or(e,t);for(let e=1;e<=a;e++){let a=Er(n,i,e);r+=Mm(a,Jm(a,`${i}${e}`),t)}let o=Jm(Er(n,i,a+1),`${i}${a+1}`);return r+=`<button class="dz-btn--ghost dz-addblock" type="button" data-repeat="${W(i)}" data-section-id="${W(e.id)}">+ Add ${W(o)}</button>`,r}let a=[...new Set(n.filter(e=>e.group).map(e=>e.group))];if(!a.length)return r;let o=null;return r+=a.map((e,r)=>{let i=n.filter(t=>t.group===e),a=Jm(i,e),s=r===0||qm(i,t);return!s&&o===null&&(o={key:e,heading:a}),Mm(i,a,t,{hidden:!s})}).join(``),r+=`<button class="dz-btn--ghost dz-addblock" type="button"${o?``:` hidden`}>${o?`+ Add ${W(o.heading)}`:``}</button>`,r}function Pm(e,t){let n=(e?.sections||[]).filter(e=>(e.questions||[]).some(e=>e.audience===`am`)),r=n.length>1;return`
    ${r?`
    ...
    ${r?`
      ...
      </div>`:``}).join(``)}function Lm(e,t){let n=Array.isArray(t)&&t.length?t[t.length-1]:null;if(!n)return``;let r=(e?.sections||[]).flatMap(e=>(e.questions||[]).filter(e=>e.audience!==`am`)).map(e=>({q:e,val:Zn(e,n.answers||{})})).filter(({val:e})=>e);return r.length?`
    ...
    </div>`:``}async function Rm(e){let{drawer:t,close:n}=km(`Deliverable`),r=t.querySelector(`#dz-dbody`),i=t.querySelector(`#dz-dfoot`),a;try{let t=await fetch(`${Vm()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}`),n=await t.json().catch(()=>({}));if(!t.ok||!n.ok)throw Error(n.error||`load failed`);a=n.data||{}}catch{r.innerHTML=Km(`error`,`Could not load this deliverable. Please try again.`);return}let o=K.rows.find(t=>t.guid===e)?.owner_name||K.clients.find(e=>e.guid===a.owner_guid)?.caption||`Client`;t.querySelector(`#dz-dtitle`).textContent=`${o} · ${a.template?.name||`Deliverable`}`;let s=a.status===`DRAFT`||a.status===`SENT`,c=Number(a.revision)>1?` · Rev ${a.revision}`:``,l=a.status===`SUBMITTED`?`Returned ${a.submitted_tz?D(a.submitted_tz):``}${a.locked_tz?` · Finalized`:``}${c}`:a.status===`SENT`?`Sent ${a.sent_tz?D(a.sent_tz):``} to ${a.recipient_email||``} · awaiting approval${c}`:`${Wm[a.status]||a.status}${c}`;if(t.querySelector(`#dz-dsub`).textContent=l,!s){r.innerHTML=tr(a)+Im(a.template,a.content||{},a.answers||{}),a.status===`SUBMITTED`&&!a.locked_tz?(i.innerHTML=`<button class="dz-btn" type="button" id="dz-revise">Revise plan</button>
        <span class="dz-note" style="margin:0">Archives this review and reopens authoring. The client approves the revised plan on a fresh link.</span>`,i.querySelector(`#dz-revise`).addEventListener(`click`,async t=>{let r=t.target;r.disabled=!0;try{let t=await fetch(`${Vm()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}/revise`,{method:`POST`}),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`revise failed`);G(`Reopened for revision. The last review is pinned above the form.`,`success`),n(),await wm(),Rm(e)}catch(e){G(e.message||`Could not revise.`,`error`),r.disabled=!1}})):i.innerHTML=``;return}r.innerHTML=`
    ${Lm(a.template,a.rounds)}
    ${a.url?`
    ...
    <div id="dz-author-form">${Pm(a.template,a.content||{})}</div>`,i.innerHTML=`
    ...
    <button class="dz-btn--ghost dz-btn--danger" type="button" id="dz-cancel">Cancel deliverable</button>`,r.querySelector(`#dz-copylink`)?.addEventListener(`click`,e=>{let t=r.querySelector(`.dz-linkrow input`);navigator.clipboard?.writeText(t?.value||``).then(()=>{e.target.textContent=`Copied!`,setTimeout(()=>{e.target.textContent=`Copy link`},1500)},()=>G(`Could not copy the link.`,`error`))});let u=async()=>{let t=er(r.querySelector(`#dz-author-form`)),i=await fetch(`${Vm()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}`,{method:`PUT`,headers:{"Content-Type":`application/json`},body:JSON.stringify({content:t})}),a=await i.json().catch(()=>({}));if(!i.ok||!a.ok){if(i.status===409)return G(a.error||`This deliverable is no longer editable.`,`warning`),n(),await wm(),!1;throw Error(a.error||`save failed`)}return!0};Fm(r.querySelector(`#dz-author-form`),a.template,()=>{u().catch(()=>G(`Autosave failed. Use Save draft.`,`warning`))}),i.querySelector(`#dz-preview`).addEventListener(`click`,async e=>{let t=e.target.closest(`button`);if(!a.url){G(`No link available for this deliverable.`,`warning`);return}t.disabled=!0;try{await u()&&window.open(a.url,`_blank`)}catch(e){G(e.message||`Could not save before previewing.`,`error`)}t.disabled=!1}),i.querySelector(`#dz-save`).addEventListener(`click`,async e=>{let t=e.target;t.disabled=!0;try{await u()&&G(`Draft saved.`,`success`)}catch(e){G(e.message||`Could not save.`,`error`)}t.disabled=!1}),i.querySelector(`#dz-send`).addEventListener(`click`,async t=>{let r=t.target,a=(i.querySelector(`#dz-email`)?.value||``).trim();if(!a){G(`Enter the recipient email first.`,`warning`);return}r.disabled=!0;try{if(!await u())return;let t=await fetch(`${Vm()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}/send`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({recipient_email:a})}),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`send failed`);G(r.data?.emailQueued?`Sent. ${a} will receive the review link.`:`Link created, but the email could not be queued. Copy the link and send it yourself.`,r.data?.emailQueued?`success`:`warning`),n(),await wm()}catch(e){G(e.message||`Could not send.`,`error`),r.disabled=!1}}),i.querySelector(`#dz-cancel`).addEventListener(`click`,async t=>{let r=t.target;r.disabled=!0;try{let t=await fetch(`${Vm()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}/cancel`,{method:`POST`}),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`cancel failed`);G(`Deliverable cancelled. Its link no longer works.`,`info`),n(),await wm()}catch(e){G(e.message||`Could not cancel.`,`error`),r.disabled=!1}})}var zm,Bm,W,Vm,G,Hm,Um,Wm,K,Gm,Km,qm,Jm,Ym,Xm=o((()=>{T(),zm=u(f(),1),m(),or(),Mr(),Zt(),Bm={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},W=e=>String(e??``).replace(/[&<>"']/g,e=>Bm[e]),Vm=()=>w.getConfig?.().BASE_URL||``,G=(e,t=`info`)=>{try{zm.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},Hm=`managed_client`,Um=[[`ALL`,`All`],[`DRAFT`,`In progress`],[`SENT`,`Awaiting approval`],[`SUBMITTED`,`Returned`],[`CANCELLED`,`Cancelled`]],Wm=Object.fromEntries(Um),K={rows:[],clients:[],templates:[],filter:`ALL`,search:``},Gm=!1,Km=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,qm=(e,t)=>e.some(e=>{let n=t?.[e.id];return Array.isArray(n)?n.length>0:!!(n&&String(n).trim())}),Jm=(e,t)=>{let n=String(e[0]?.label||``).match(/^(.+?):\s/);return n?n[1]:t.toUpperCase()},Ym={render:Sm,init:Cm}})),Zm=c({default:()=>lh,init:()=>rh,render:()=>nh});function Qm(e){return e.status===`SENT`?{key:`PENDING`,label:`Pending your review`}:e.status===`SUBMITTED`&&e.signed?{key:`APPROVED`,label:`Approved`}:e.status===`SUBMITTED`?{key:`RETURNED`,label:`Returned with edits`}:null}function $m(){if(nr(),sh)return;let e=document.createElement(`style`);e.id=`m5t-deliverables-client-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),sh=!0}function eh(e,t){return!e.url&&t.key===`PENDING`?``:t.key===`PENDING`?`<button class="dc-act" type="button" data-url="${ah(e.url)}">Review</button>`:t.key!==`APPROVED`&&e.url&&e.editability?.editable?`<button class="dc-act" type="button" data-url="${ah(e.url)}">Update your review</button>`:``}function th(e){let t=Qm(e);return`<tr>
    <td><span class="dc-name">${ah(e.template_name||e.kind||`Deliverable`)}</span>${Number(e.revision)>1?`<span class="dc-when" style="margin-left:6px">Rev ${ah(e.revision)}</span>`:``}</td>
    <td><span class="dc-badge" data-s="${ah(t.key)}">${ah(t.label)}</span></td>
    <td class="dc-when">${e.sent_tz?ah(D(e.sent_tz)):`—`}</td>
    <td class="dc-when">${e.submitted_tz?ah(D(e.submitted_tz)):`—`}</td>
    <td class="dc-actcell">${eh(e,t)}</td>
  </tr>`}function nh(){return $m(),`
    <div class="page-shell" data-section="deliverables-client">
      <div class="page-header">
        <div class="page-header-text">
          <h1 class="page-title">Deliverables</h1>
          <p class="page-description">Plans from your account manager.</p>
        </div>
      </div>
      <div class="dashboard-card">
        <div class="card-header">
          <h2 class="card-title"><span class="material-symbols-outlined" aria-hidden="true">approval</span> Your plans</h2>
        </div>
        <div class="card-body" id="dc-body">${ch(`hourglass_empty`,`Loading…`)}</div>
      </div>
    </div>`}async function rh(){let e=document.getElementById(`dc-body`);if(!e)return;let t;try{let e=await fetch(`${oh()}/m5t/v5/m5mDeliverable`),n=await e.json().catch(()=>({}));if(!e.ok||!n.ok)throw Error(`load failed`);t=(n.data||[]).filter(e=>Qm(e))}catch{e.innerHTML=ch(`error`,`Could not load your deliverables. Please try again.`);return}if(!t.length){e.innerHTML=ch(`approval`,`Nothing to review right now. Plans your account manager sends appear here.`);return}e.innerHTML=`
    ...
    </div>`,e.querySelector(`#dc-tbody`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-url]`);t?.dataset.url&&window.open(t.dataset.url,`_blank`)})}var ih,ah,oh,sh,ch,lh,uh=o((()=>{T(),or(),ih={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},ah=e=>String(e??``).replace(/[&<>"']/g,e=>ih[e]),oh=()=>w.getConfig?.().BASE_URL||``,sh=!1,ch=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,lh={render:nh,init:rh}})),dh=c({init:()=>Sh,render:()=>xh});function fh(e){let t=Uh(e);if(!t)return``;try{return new Date(t).toLocaleDateString([],{month:`short`,day:`numeric`,year:`numeric`})}catch{return``}}function ph(e){let t=Uh(e);if(!t)return``;try{let n=Date.now()-new Date(t).getTime();if(n<0)return fh(e);let r=n/36e5;return r<1?`${Math.max(1,Math.floor(n/6e4))}m ago`:r<24?`${Math.floor(r)}h ago`:r<48?`Yesterday`:fh(e)}catch{return fh(e)}}function mh(){if(Fh)return;let e=document.createElement(`style`);e.id=`m5t-leads-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Fh=!0}function hh(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function gh(e){return`<select class="leads-status" data-id="${J(e.id)}" data-act="status" onclick="event.stopPropagation()">
    ${Lh.map(t=>`<option value="${t}" ${e.status===t?`selected`:``}>${t[0]}${t.slice(1).toLowerCase()}</option>`).join(``)}
  </select>`}function _h(e){let t=q.magnetNames[e.magnet_guid]||e.lead_source||`—`,n=e.phone?String(e.phone).replace(/[^\d+]/g,``):``;return`
    <tr class="leads-row${String(e.id)===q.selectedId?` selected`:``}" data-id="${J(e.id)}" tabindex="0" role="button" aria-label="Open lead ${J(Vh(e))}">
      <td>
        <span class="leads-who">
          <span class="leads-avatar">${J(Hh(e))}</span>
          <span><span class="leads-name">${J(Vh(e))}</span>${e.company?`<span class="leads-sub">${J(e.company)}</span>`:``}</span>
        </span>
      </td>
      ...
      <td>${q.canManage?gh(e):`<span class="leads-badge" data-s="${J(e.status)}">${J(e.status)}</span>`}</td>
      <td class="num leads-muted" style="white-space:nowrap" title="${J(fh(e))}">${Wh(e)?`<span class="leads-fresh" title="Captured in the last 24 hours"></span>`:``}${J(ph(e))}</td>
      <td class="leads-open" aria-hidden="true"><span class="material-symbols-outlined">chevron_right</span></td>
    </tr>`}function vh(e){let t=q.search.trim().toLowerCase();if(!t)return!0;let n=[e.first_name,e.last_name,e.email,e.company,e.phone,e.product_interest,q.magnetNames[e.magnet_guid],q.book?q.ownerNames[e.owner_guid]:``,e.utm_campaign,e.utm_source].filter(Boolean).join(` `).toLowerCase();return t.split(/\s+/).every(e=>n.includes(e))}function yh(){return q.leads.filter(e=>(q.filter===`ALL`||e.status===q.filter)&&Gh(e)&&vh(e)&&Kh(e))}function bh(){if(!q.day)return``;let e=q.day;try{e=new Date(`${q.day}T00:00:00Z`).toLocaleDateString([],{month:`short`,day:`numeric`,year:`numeric`,timeZone:`UTC`})}catch{}return`<button class="leads-daychip" type="button" id="leads-dayclear" title="Show every day again">
  ...
  </button>`}function xh(){mh(),Xi();let e=A();return`
    <div class="page-shell" data-section="leads">
      ...
      <div class="dashboard-card" style="margin-bottom:24px">
        ...
        <div class="card-body" id="pf-picker-body">${hh(`hourglass_empty`,`Loading…`)}</div>
      </div>`:``}
      <div class="page-header">
        <div class="page-header-text">
      ...
      <div class="leads-layout">
        <div class="dashboard-card">
          <div class="card-body" id="leads-body">
            ${hh(`hourglass_empty`,`Loading leads…`)}
          </div>
        </div>
        <aside class="dashboard-card leads-panel" aria-label="Lead detail">
          <div class="card-body" id="leads-panel-body">${qh()}</div>
        </aside>
      </div>
    </div>`}async function Sh(){document.getElementById(`leads-export`)?.addEventListener(`click`,Nh);let e=fa(ma.LEADS_SEARCH),t=fa(ma.LEADS_DAY);if((e||t)&&(q.search=e||``,q.day=t||``,q.filter=`ALL`,q.clientFilter=`ALL`,q.shown=Ih),A()){let e=e=>Number(e||0).toLocaleString();na(Bh(),[{h:`Leads · wk`,td:t=>e(t.leads.this_week)},{h:`Leads · total`,td:t=>e(t.leads.total)},{h:`Compl.`,td:e=>e.completion_rate==null?`—`:`${e.completion_rate}%`}],{emptyMsg:`No client orgs yet. Create one under Clients first.`})}await Ch()}async function Ch(){let e=document.getElementById(`leads-body`);if(e){try{let[e,t]=await Promise.all([fetch(`${Bh()}/m5t/v5/m5mLead`),fetch(`${Bh()}/m5t/v5/magnets`).catch(()=>null)]),n=await e.json().catch(()=>({}));if(!n.ok)throw Error(n.message||`Failed to load`);if(q.leads=(n.data||[]).filter(e=>![`DELETED`,`ARCHIVED`].includes(e.status)),q.total=Number.isFinite(n.total)?n.total:q.leads.length,q.truncated=q.total>(n.data||[]).length,q.book=A(),q.ownerNames=q.book?ra():{},t){let e=await t.json().catch(()=>({}));q.magnetNames={},(e.magnets||[]).forEach(e=>{q.magnetNames[e.guid]=e.name,e.owner_guid&&e.owner_name&&!q.ownerNames[e.owner_guid]&&(q.ownerNames[e.owner_guid]=e.owner_name)}),q.canManage=!!e.canManage}}catch{e.innerHTML=hh(`error`,`Could not load your leads. Please try again.`);return}wh()}}function wh(){let e=document.getElementById(`leads-body`),t=document.getElementById(`leads-export`);if(!e)return;if(t&&(t.style.display=q.leads.length?``:`none`),!q.leads.length){e.innerHTML=hh(`inbox`,`No leads captured yet. Publish a magnet and share it to start collecting.`),jh();return}let n=q.leads.filter(Gh),r={ALL:n.length};Lh.forEach(e=>{r[e]=n.filter(t=>t.status===e).length});let i=q.book?[...new Set(q.leads.map(e=>e.owner_guid).filter(Boolean))].map(e=>({guid:e,name:q.ownerNames[e]||e})).sort((e,t)=>e.name.localeCompare(t.name)):[];e.innerHTML=`
    <div class="leads-bar">
      <div class="leads-filter">
        ${[`ALL`,...Lh].map(e=>`<button data-filter="${e}" class="${q.filter===e?`active`:``}">${e===`ALL`?`All`:e[0]+e.slice(1).toLowerCase()}${r[e]?`<span class="leads-pillnum">${r[e]}</span>`:``}</button>`).join(``)}
      </div>
      ${q.book&&i.length>1?`
    ...
    <div id="leads-tablezone"></div>`,e.querySelector(`.leads-filter`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-filter]`);t&&(q.filter=t.dataset.filter,q.shown=Ih,wh())}),e.querySelector(`#leads-clientsel`)?.addEventListener(`change`,e=>{q.clientFilter=e.target.value,q.shown=Ih,wh()}),e.querySelector(`#leads-dayclear`)?.addEventListener(`click`,()=>{q.day=``,q.shown=Ih,wh()}),e.querySelector(`#leads-search`)?.addEventListener(`input`,e=>{q.search=e.target.value,q.shown=Ih,Th()}),Th()}function Th(){let e=document.getElementById(`leads-tablezone`),t=document.getElementById(`leads-countlbl`);if(!e)return;let n=yh(),r=n.slice(0,q.shown),i=q.book?8:7;t&&(t.textContent=`${n.length} of ${q.leads.length}`),e.innerHTML=`
    <div class="leads-tablewrap">
      <table class="leads-table${q.book?` leads-table--book`:``}">
        ...
        <tbody id="leads-tbody">${r.map(_h).join(``)||`<tr><td colspan="${i}"><div class="leads-muted" style="padding:14px 4px">${q.search.trim()?`No leads match your search.`:`No ${q.filter.toLowerCase()} leads.`}</div></td></tr>`}</tbody>
      </table>
    </div>
    ${Jt({total:n.length,shown:q.shown,noun:`leads`})}
    ${q.truncated?`<div class="leads-muted" style="padding:10px 4px">Showing your ${q.leads.length.toLocaleString()} most recent leads of ${q.total.toLocaleString()}. Search and filters cover these; older leads aren't included yet.</div>`:``}`,e.querySelector(`[data-action="list-expand"]`)?.addEventListener(`click`,()=>{q.shown+=25,Th()}),e.querySelector(`#leads-tbody`)?.addEventListener(`change`,Eh),e.querySelector(`#leads-tbody`)?.addEventListener(`click`,e=>{if(e.target.closest(`.leads-status, a.leads-contactlink`))return;let t=e.target.closest(`.leads-row`);t&&Dh(t.dataset.id)}),e.querySelector(`#leads-tbody`)?.addEventListener(`keydown`,e=>{if(e.key!==`Enter`&&e.key!==` `)return;let t=e.target.closest(`.leads-row`);t&&e.target===t&&(e.preventDefault(),Dh(t.dataset.id))})}async function Eh(e){let t=e.target.closest(`.leads-status`);if(!t)return;let n=t.dataset.id,r=q.leads.find(e=>String(e.id)===String(n)),i=t.value,a=r?.status;try{if(!(await(await fetch(`${Bh()}/m5t/v5/m5mLead/${encodeURIComponent(n)}`,{method:`PUT`,headers:{"Content-Type":`application/json`},body:JSON.stringify({status:i})})).json().catch(()=>({}))).ok)throw Error();r&&(r.status=i),zh(`Marked ${i[0]}${i.slice(1).toLowerCase()}`,`success`)}catch{t.value=a||`NEW`,zh(`Could not update status`,`danger`);return}try{String(n)===q.selectedId&&jh(),q.filter!==`ALL`&&wh()}catch(e){console.warn(`[leads] re-render after status change failed:`,e)}}function Dh(e){q.selectedId=String(e),document.querySelectorAll(`.leads-row`).forEach(e=>e.classList.toggle(`selected`,e.dataset.id===q.selectedId)),jh()}function Oh(e,t){let n=Array.isArray(e.tags)?e.tags:[];for(let e=n.length-1;e>=0;e--)if(n[e]&&n[e].tag===t&&n[e].value!=null)return String(n[e].value);return``}function kh(e){let t=Oh(e,`appointment_start`);if(!t)return null;let n=Oh(e,`appointment_status`)||`booked`,r=Oh(e,`appointment_timezone`),i=Oh(e,`appointment_event_type`),a=new Date(t),o=t;if(!Number.isNaN(a.getTime())){let e={weekday:`short`,month:`short`,day:`numeric`,hour:`numeric`,minute:`2-digit`};try{o=new Intl.DateTimeFormat(void 0,r?{...e,timeZone:r}:e).format(a)}catch{o=new Intl.DateTimeFormat(void 0,e).format(a)}}return{when:o,status:n,type:i,start:t}}function Ah(e){let t=Array.isArray(e.tags)?e.tags:[],n=null;for(let e of t)if(!(!e||!e.tag)){if(e.tag===`callrail_call_id`){n={};continue}n&&e.tag.indexOf(`call_`)===0&&e.value!=null&&(n[e.tag]=String(e.value))}if(!n||!n.call_started)return null;let r=n.call_started,i=new Date(r),a=r;Number.isNaN(i.getTime())||(a=new Intl.DateTimeFormat(void 0,{weekday:`short`,month:`short`,day:`numeric`,hour:`numeric`,minute:`2-digit`}).format(i));let o=Number(n.call_duration)||0,s=o?o<60?`${o}s`:`${Math.floor(o/60)}m ${o%60}s`:``;return{when:a,dur:s,source:n.call_source||``,answered:n.call_answered||``,recording:n.call_recording_player||``,started:r}}function jh(){let e=document.getElementById(`leads-panel-body`);if(!e)return;let t=q.leads.find(e=>String(e.id)===q.selectedId),n=document.querySelector(`.leads-layout`);if(n&&(n.dataset.hasSelection=t?`true`:`false`),!t){e.innerHTML=qh();return}let r=q.magnetNames[t.magnet_guid]||t.lead_source||`—`,i=kh(t),[a,o]=i?Yh[i.status]||Yh.booked:[``,``],s=Ah(t),c=(e,t)=>t?`<div class="leads-det__field"><span class="leads-det__k">${J(e)}</span><span class="leads-det__v">${J(t)}</span></div>`:``;e.innerHTML=`
    <div class="leads-det__head">
      <span class="leads-avatar leads-det__ava">${J(Hh(t))}</span>
      <div>
        <div class="leads-det__name">${J(Vh(t))}</div>
    ...
    ${q.canManage?`
      ...
      </div>`:``}`,e.querySelector(`[data-action="delete-lead"]`)?.addEventListener(`click`,Mh)}async function Mh(e){let t=e.currentTarget?.dataset?.id,n=q.leads.find(e=>String(e.id)===String(t));if(!n)return;let r=Vh(n);if(!window.confirm(`Remove ${r} from your leads?\n\nThis takes the lead out of your dashboard, lists and exports. The record is kept in our system, and this address can be captured again if they come back through one of your magnets.`))return;let i=e.currentTarget;i.disabled=!0;try{let e=await fetch(`${Bh()}/m5t/v5/m5mLead/${encodeURIComponent(t)}`,{method:`DELETE`}),n=await e.json().catch(()=>({}));if(!e.ok||!n.ok)throw Error(n.error||`HTTP ${e.status}`);q.leads=q.leads.filter(e=>String(e.id)!==String(t)),q.selectedId=null,zh(`${r} removed`,`success`),wh()}catch{i.disabled=!1,zh(`Could not remove that lead. Please try again.`,`danger`)}}function Nh(){let e=yh();if(!e.length){zh(`Nothing to export`,`info`);return}let t=[`first_name`,`last_name`,`email`,`phone`,`company`,`product_interest`,`lead_source`,`utm_source`,`utm_medium`,`utm_campaign`,`gclid`,`status`],n=e=>`"${String(e??``).replace(/"/g,`""`)}"`,r=[[...q.book?[`client`]:[],...t,`magnet`,`captured`,`appointment`,`last_call`].join(`,`)];for(let i of e){let e=kh(i),a=Ah(i);r.push([...q.book?[n(q.ownerNames[i.owner_guid]||``)]:[],...t.map(e=>n(i[e])),n(q.magnetNames[i.magnet_guid]||i.lead_source||``),n(fh(i)),n(e?`${e.start} (${e.status})`:``),n(a?`${a.started} (${a.answered===`no`?`missed`:`answered`})`:``)].join(`,`))}let i=new Blob([r.join(`
`)],{type:`text/csv`}),a=URL.createObjectURL(i),o=document.createElement(`a`);o.href=a,o.download=`leads-${q.filter.toLowerCase()}.csv`,document.body.appendChild(o),o.click(),o.remove(),URL.revokeObjectURL(a),zh(`Exported ${e.length} lead${e.length===1?``:`s`}`,`success`)}var Ph,Fh,q,Ih,Lh,Rh,J,zh,Bh,Vh,Hh,Uh,Wh,Gh,Kh,qh,Jh,Yh,Xh=o((()=>{T(),Ph=u(f(),1),m(),ua(),ga(),Zt(),Fh=!1,q={leads:[],magnetNames:{},filter:`ALL`,canManage:!1,book:!1,ownerNames:{},clientFilter:`ALL`,search:``,shown:25,selectedId:null,day:``,total:0,truncated:!1},Ih=25,Lh=[`NEW`,`WORKING`,`QUALIFIED`,`UNQUALIFIED`,`CONVERTED`,`EXPORTED`],Rh={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},J=e=>String(e??``).replace(/[&<>"']/g,e=>Rh[e]),zh=(e,t=`info`)=>{try{Ph.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},Bh=()=>w.getConfig?.().BASE_URL||``,Vh=e=>[e.first_name,e.last_name].filter(Boolean).join(` `).trim()||e.email||`(no name)`,Hh=e=>(Vh(e).split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``)||`?`).toUpperCase(),Uh=e=>e.created?.created_tz||``,Wh=e=>{let t=Uh(e);if(!t)return!1;try{return Date.now()-new Date(t).getTime()<864e5}catch{return!1}},Gh=e=>q.clientFilter===`ALL`||e.owner_guid===q.clientFilter,Kh=e=>!q.day||Uh(e).slice(0,10)===q.day,qh=()=>hh(`touch_app`,`Click to see lead information.`),Jh=()=>`
  <colgroup>
    <col style="width:22%">
  ...
  </colgroup>`,Yh={booked:[`QUALIFIED`,`Booked`],rescheduled:[`WORKING`,`Rescheduled`],canceled:[`CANCELED`,`Canceled`]}})),Zh=c({init:()=>rg,render:()=>ng,tableHTML:()=>tg});function Qh(){if(pg)return;let e=document.createElement(`style`);e.id=`m5t-analytics-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),pg=!0}function $h(e,t){let n=t>0?Math.round(e/t*100):0;return`<svg class="anl-bar" width="72" height="8" viewBox="0 0 72 8" role="img"
    ...
    </svg>`}function eg(e,t,n){let r=hg[e.status]?`<span class="anl-chip">${lg(hg[e.status])}</span>`:``,i=n&&e.owner_name?`<span class="anl-sub">${lg(e.owner_name)}</span>`:``,a=e.ab_tests&&(e.ab_tests.running||e.ab_tests.completed)?`${sg(e.ab_tests.running)} / ${sg(e.ab_tests.completed)}`:`—`;return`<tr data-guid="${lg(e.magnet_guid)}">
    <td><a class="anl-name" href="#/magnets">${lg(e.name)}</a>${r}${i}</td>
    <td>${$h(Number(e.leads?.window||0),t)}</td>
    <td>${sg(e.leads?.window)}</td>
    <td>${sg(e.opens)}</td>
    <td>${ug(e.engagement_rate)}</td>
    <td>${ug(e.completion_rate)}</td>
    <td>${ug(e.lead_rate)}</td>
    <td>${a}</td>
  </tr>`}function tg(e,t){let n=e.reduce((e,t)=>Math.max(e,Number(t.leads?.window||0)),0);return`<div class="anl-wrap"><table class="anl-table">
    <thead><tr>
      <th>Magnet</th><th></th><th>Leads</th><th>Opens</th>
      <th>Engaged</th><th>Completed</th><th>Leads / Open</th><th>A/B run / done</th>
    </tr></thead>
    <tbody>${e.map(e=>eg(e,n,t)).join(``)}</tbody>
  </table></div>`}function ng(){return A()?ta({section:`analytics`,title:`Analytics`,description:`Pick a client to see how their magnets are performing.`}):(Qh(),`
    <div class="page-shell" data-section="analytics">
      <div class="page-header">
        ...
        <div class="chart-controls" id="anl-periods">
          ${dg.map(e=>`<button class="chart-period-btn${e.d===fg?` active`:``}" type="button" data-days="${e.d}">${e.label}</button>`).join(``)}
        </div>
      </div>
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="anl-body">
          ${mg(`hourglass_empty`,`Loading magnet performance…`)}
        </div>
      </div>
    ...
    </div>`)}async function rg(){if(A())return na(ag(),[{h:`Leads`,td:e=>sg(e.leads.window??e.leads.total)},{h:`Opens`,td:e=>sg(e.opens)},{h:`Completed`,td:e=>ug(e.completion_rate)}]);Qh(),await ig();let e=document.getElementById(`anl-periods`);e&&e.addEventListener(`click`,t=>{let n=t.target.closest(`.chart-period-btn`);n&&(fg=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>{e.classList.toggle(`active`,Number(e.dataset.days)===fg)}),ig())})}async function ig(){let e=document.getElementById(`anl-body`),t=document.getElementById(`anl-range`);if(!e)return;e.innerHTML=mg(`hourglass_empty`,`Loading magnet performance…`);let n=new Date,r=og(n),i=og(new Date(n.getTime()-(fg-1)*864e5));t&&(t.textContent=`${i} → ${r}`);let a;try{let e=await fetch(`${ag()}/m5t/v5/dashboard/stats?from=${i}&to=${r}&group_by=magnet`);if(a=await e.json(),!e.ok||!a?.ok)throw Error(a?.error||`HTTP ${e.status}`)}catch(t){e.innerHTML=mg(`cloud_off`,`Could not load analytics. Refresh to try again.`),console.error(`[analytics] load failed:`,t);return}let o=(e,t)=>{let n=document.getElementById(e);n&&(n.textContent=t)};o(`anl-leads`,sg(a.by_day?.reduce((e,t)=>e+Number(t.leads||0),0))),o(`anl-opens`,sg(a.funnel?.opens)),o(`anl-completion`,ug(a.funnel?.completion_rate)),o(`anl-active`,sg(a.magnets?.active));let s=Array.isArray(a.by_magnet)?a.by_magnet:[];if(!s.length){e.innerHTML=mg(`widgets`,`No magnets yet. Create one and its numbers will show up here.`);return}let c=s.some(e=>Number(e.opens||0)>0||Number(e.leads?.window||0)>0),l=new Set(s.map(e=>e.owner_guid)).size>1;e.innerHTML=(a.by_magnet_truncated?`<p class="anl-warn">Showing the top ${s.length} magnets by leads. You have more than that, so this list is not complete.</p>`:``)+(c?``:`<p class="anl-warn">No opens or leads for any magnet in this period. Try a longer range.</p>`)+tg(s,l);let u=e.querySelector(`.anl-table tbody`);u&&Yt(u,{rowSelector:`tr`,noun:`magnets`,controlHost:e.querySelector(`.anl-wrap`)||e})}var ag,og,sg,cg,lg,ug,dg,fg,pg,mg,hg,gg=o((()=>{T(),ua(),ba(),Zt(),ag=()=>w.getConfig?.().BASE_URL||``,og=e=>e.toISOString().slice(0,10),sg=e=>Number(e||0).toLocaleString(),cg={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},lg=e=>String(e??``).replace(/[&<>"']/g,e=>cg[e]),ug=e=>e==null?`—`:`${e}%`,dg=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],fg=30,pg=!1,mg=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,hg={DRAFT:`Draft`,DISABLED:`Paused`}})),_g=c({default:()=>zg,init:()=>bg,render:()=>yg});function vg(){if(Fg)return;let e=document.createElement(`style`);e.id=`m5t-adperf-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Fg=!0}function yg(){return A()?ta({section:`ad-performance`,title:`Ad Performance`,description:`Pick a client to view their spend and lead performance.`}):(vg(),`
    <div class="page-shell" data-section="ad-performance">
      <div class="page-header">
        ...
        <div class="chart-controls" id="adperf-periods">
          ${Lg.map(e=>`<button class="chart-period-btn${e.d===Rg?` active`:``}" type="button" data-days="${e.d}">${e.label}</button>`).join(``)}
        </div>
      </div>
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="adperf-body">
          ${Ig(`hourglass_empty`,`Loading ad performance…`)}
        </div>
      </div>
    ...
    </div>`)}async function bg(){if(A()){let e=e=>Number(e||0).toLocaleString();return na(kg(),[{h:`Spend`,td:t=>t.ad?`$${e(t.ad.spend)}`:`—`},{h:`Clicks`,td:t=>t.ad?e(t.ad.clicks):`—`},{h:`Leads`,td:t=>e(t.leads.window??t.leads.total)},{h:`CPL`,td:t=>t.ad?.cost_per_lead==null?`—`:`$${e(t.ad.cost_per_lead)}`}],{emptyMsg:`No client orgs yet. Connect a client ad account to see spend here.`})}let e=document.getElementById(`adperf-periods`);e?.addEventListener(`click`,t=>{let n=t.target.closest(`button[data-days]`);n&&(Rg=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>e.classList.toggle(`active`,Number(e.dataset.days)===Rg)),xg())}),await xg()}async function xg(){let e=kg(),t=document.getElementById(`adperf-body`),n=document.getElementById(`adperf-range`),r=new Date,i=Ag(r),a=Ag(new Date(r.getTime()-(Rg-1)*864e5));n&&(n.textContent=`${a} → ${i}`);let o,s=[],c=null,l=null,u=null;try{let[t,n,r,d,f]=await Promise.all([fetch(`${e}/m5t/v5/coreAdMetrics/overlay?from=${a}&to=${i}`),fetch(`${e}/m5t/v5/coreAdAccount`),fetch(`${e}/m5t/v5/coreAdMetrics/by-source?from=${a}&to=${i}`),qs().catch(()=>null),fetch(`${e}/m5t/v5/coreAdMetrics/conversions?from=${a}&to=${i}`).then(e=>e.json()).catch(()=>null)]);if(!t.ok)throw Error(`HTTP ${t.status}`);o=await t.json(),s=((await n.json().catch(()=>({})))?.data||[]).filter(e=>e.status!==`DELETED`),c=await r.json().catch(()=>null),l=d,u=f}catch(e){console.warn(`[ad-performance] load failed:`,e),t&&(t.innerHTML=Ig(`error`,`Could not load ad performance. Please try again.`));return}Sg(l,s),Cg(u);let d=o.totals||{};if(Tg(`adperf-spend`,jg(d.spend)),Tg(`adperf-clicks`,Mg(d.clicks)),Tg(`adperf-leads`,Mg(d.magnet_leads)),Tg(`adperf-cpl`,jg(d.cost_per_lead)),wg(c),!s.length){t.innerHTML=Ig(`link_off`,`No ad account connected yet. Your agency can connect Google Ads or Local Services Ads to see performance here.`);return}if(!o.connected){t.innerHTML=Ig(`cloud_sync`,`Account connected. No ad data for this range yet. Metrics appear here once they sync.`);return}t.innerHTML=Og(o.rows||[])}function Sg(e,t){let n=document.getElementById(`adperf-conn`);if(!n)return;if(!e||e.denied){n.innerHTML=``;return}let r=gc.google_ads,i=Ws(e.providers,`google_ads`),a=Gs(r,i,!!e.configured?.google_ads),o=uc()?` <a href="#/settings/integrations/${r.urlSlug}">Manage</a>`:``;if(a===`connected`||a===`broken`){ac(),n.innerHTML=`<span>Google Ads</span> ${Ks(a,i)}${o}`;return}if(t.length){n.innerHTML=`<span>Google Ads</span> <span>${t.length} associated account${t.length===1?``:`s`}</span>`;return}n.innerHTML=``}function Cg(e){let t=document.getElementById(`adperf-conv-card`),n=document.getElementById(`adperf-conv-body`),r=document.getElementById(`adperf-stats`),i=document.getElementById(`adperf-conv-stat`);if(i&&i.remove(),!t||!n)return;if(!e?.ok||!e.has_selection){t.style.display=`none`;return}if(r){let t=document.createElement(`div`);t.innerHTML=_a({icon:`flag`,label:`Ads Conversions`,value:Mg(e.totals?.conversions),sub:`Reported by Google Ads`,href:`#/settings/integrations/google-ads`});let n=t.firstElementChild;n.id=`adperf-conv-stat`,r.appendChild(n)}let a=e.actions||[];if(t.style.display=``,!a.length){n.innerHTML=Ig(`search_off`,`No conversions in this range for the selected actions.`);return}let o=a.some(e=>e.value>0);n.innerHTML=`
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
        ...
        <tbody>${a.map(e=>`
          <tr>
            <td>${Pg(e.name||e.id)}</td>
            <td class="adperf-src-sub">${Pg(String(e.category||``).toLowerCase().replace(/_/g,` `)||`—`)}</td>
            <td class="num"><b>${Mg(e.conversions)}</b></td>
            ${o?`<td class="num">${jg(e.value)}</td>`:``}
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>
    <p class="adperf-note">Counted actions are chosen on the Google Ads integration page. These are Google's own numbers and may overlap magnet leads, so they are never added together.</p>`}function wg(e){let t=document.getElementById(`adperf-sources`),n=document.getElementById(`adperf-src-summary`);if(!t)return;let r=e&&Array.isArray(e.rows)?e.rows:[];if(!r.length){n&&(n.textContent=``),t.innerHTML=Ig(`search_off`,`No leads in this range yet. Campaigns appear here as tagged leads (?utm_campaign=…) come in.`);return}let i=e.total_leads||r.reduce((e,t)=>e+(t.leads||0),0),a=e.tracked_leads||0;n&&(n.textContent=`${Mg(a)} of ${Mg(i)} leads tagged to a source`);let o=e=>e.campaign&&e.campaign!==`(none)`?Pg(e.campaign):e.source&&e.source!==`(direct)`?Pg(e.source):`<span class="adperf-src-untracked">Untracked / direct</span>`;t.innerHTML=`
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
        ...
        <tbody>${r.map(e=>`
          <tr>
            <td>${o(e)}</td>
            <td class="adperf-src-sub">${Pg(e.source||`—`)}${e.medium?` · ${Pg(e.medium)}`:``}</td>
            <td class="num">${e.paid_clicks?Mg(e.paid_clicks):`<span class="adperf-src-sub">—</span>`}</td>
            <td class="num"><b>${Mg(e.leads)}</b></td>
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>`;let s=t.querySelector(`.adperf-srctable tbody`);s&&Yt(s,{rowSelector:`tr`,noun:`sources`,controlHost:t.querySelector(`.adperf-srcwrap`)||t})}function Tg(e,t){let n=document.getElementById(e);n&&(n.textContent=t)}function Eg(e){if(e<=0)return 1;let t=10**Math.floor(Math.log10(e)),n=e/t;return(n<=1?1:n<=2?2:n<=5?5:10)*t}function Dg(e){return e=Math.round(e),e>=1e3?(e/1e3).toFixed(e%1e3==0?0:1)+`k`:String(e)}function Og(e){let t=e.length||1,n=Eg(Math.max(1,...e.map(e=>e.spend||0))),r=Eg(Math.max(1,...e.map(e=>e.magnet_leads||0))),i=738/t,a=Math.min(24,i*.55),o=e=>56+i*(e+.5),s=e=>304-e/n*282,c=e=>304-e/r*282,l=[0,.25,.5,.75,1].map(e=>{let t=304-e*282;return`<line x1="56" y1="${t.toFixed(1)}" x2="794" y2="${t.toFixed(1)}" stroke="#EAF0F7"/><text x="48" y="${(t+3.5).toFixed(1)}" text-anchor="end" class="ax">$${Dg(n*e)}</text><text x="802" y="${(t+3.5).toFixed(1)}" text-anchor="start" class="ax">${Dg(r*e)}</text>`}).join(``),u=e.map((e,t)=>{let n=s(e.spend||0);return`<rect x="${(o(t)-a/2).toFixed(1)}" y="${n.toFixed(1)}" width="${a.toFixed(1)}" height="${Math.max(0,304-n).toFixed(1)}" rx="3" fill="#0072BA"><title>${e.date}: $${e.spend} spend</title></rect>`}).join(``),d=e.map((e,t)=>`${o(t).toFixed(1)},${c(e.magnet_leads||0).toFixed(1)}`).join(` `),f=t>1?`<polyline points="${d}" fill="none" stroke="#009473" stroke-width="2.5"/>`:``,p=e.map((e,t)=>`<circle cx="${o(t).toFixed(1)}" cy="${c(e.magnet_leads||0).toFixed(1)}" r="3.5" fill="#009473"><title>${e.date}: ${e.magnet_leads} leads</title></circle>`).join(``),m=Math.max(1,Math.ceil(t/6));return`
    ...
    </svg>`}var kg,Ag,jg,Mg,Ng,Pg,Fg,Ig,Lg,Rg,zg,Bg=o((()=>{T(),ua(),xc(),ba(),Zt(),kg=()=>w.getConfig?.().BASE_URL||``,Ag=e=>e.toISOString().slice(0,10),jg=e=>e==null?`—`:`$`+Number(e).toLocaleString(void 0,{minimumFractionDigits:0,maximumFractionDigits:2}),Mg=e=>Number(e||0).toLocaleString(),Ng={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Pg=e=>String(e??``).replace(/[&<>"']/g,e=>Ng[e]),Fg=!1,Ig=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,Lg=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],Rg=30,zg={render:yg,init:bg}})),Vg=c({default:()=>c_,init:()=>Wg,parseBookingCsv:()=>t_,render:()=>Ug});function Hg(){if(a_)return;let e=document.createElement(`style`);e.id=`m5t-adacct-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),a_=!0}function Ug(){return A()?ta({section:`ad-accounts`,title:`Ad Accounts`,description:`Pick a client to see their connected ad accounts.`}):(Hg(),ac(),`
...
`&&a++,n.push(r),r=``,n.some(e=>e.trim()!==``)&&t.push(n),n=[]):r+=o}if(n.push(r),n.some(e=>e.trim()!==``)&&t.push(n),t.length<2)return[];let a=t[0].map(e=>e.trim().toLowerCase()),o=a.findIndex(e=>/e-?mail/.test(e)),s=a.findIndex(e=>/phone|mobile|tel/.test(e)),c=a.findIndex(e=>/date|time|booked|appointment|start/.test(e));return o<0&&s<0?[]:t.slice(1).map(e=>({email:o>=0?(e[o]||``).trim():``,phone:s>=0?(e[s]||``).trim():``,at:c>=0?(e[c]||``).trim():``})).filter(e=>e.email||e.phone).slice(0,5e3)}function n_(e,t){let n=new Blob([e],{type:`text/csv;charset=utf-8`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.href=r,i.download=t,document.body.appendChild(i),i.click(),i.remove(),setTimeout(()=>URL.revokeObjectURL(r),4e3)}var r_,i_,a_,o_,s_,c_,l_=o((()=>{ua(),Zt(),xc(),r_={google_ads:`Google Ads`,google_lsa:`Local Services Ads`},i_=e=>{let t=String(e||``).replace(/\D/g,``);return t.length===10?`${t.slice(0,3)}-${t.slice(3,6)}-${t.slice(6)}`:String(e||``)},a_=!1,o_=[],s_=null,c_={render:Ug,init:Wg}})),u_=c({default:()=>M_,init:()=>v_,openInviteModal:()=>S_,render:()=>__});function d_(e,t=`info`){try{T_.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}}function f_(){let e=w.getActiveOrg();if(e&&e.channel===E_)return{guid:e.guid,name:e.name};let t=w.getMemberships().find(e=>e.channel===E_);return t?{guid:t.orgGuid,name:t.name}:null}function p_(e){let t=e?.created_tz;if(!t)return``;let n=new Date(t);return Number.isNaN(n.getTime())?``:n.toLocaleDateString(void 0,{year:`numeric`,month:`short`,day:`numeric`})}function m_(){if(O_)return;let e=document.createElement(`style`);e.id=`m5t-clients-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),nr(),O_=!0}function h_(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function g_(e){let t=e.caption||e.guid,n=e.status||`ENABLED`;return`
    <div class="client-row">
      <span class="client-row__avatar">${Y(A_(t))}</span>
      <span class="client-row__main">
        <div class="client-row__name" title="${Y(t)}">${Y(t)}</div>
        <div class="client-row__meta">Added ${Y(p_(e.created))||`—`}</div>
      </span>
    ...
    <div class="page-shell" data-section="clients">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="clients-body">
          ${h_(`hourglass_empty`,`Loading clients…`)}
        </div>
      </div>
    </div>`}async function v_(){let e=f_();document.getElementById(`clients-add-btn`)?.addEventListener(`click`,()=>b_(e)),await y_(e)}async function y_(e){let t=document.getElementById(`clients-body`),n=document.getElementById(`clients-count`);if(t){if(!e?.guid){t.innerHTML=h_(`info`,`Client provisioning is available to reseller / agency accounts.`),n&&(n.textContent=``);return}try{let r=w.getConfig?.().BASE_URL||``,i=await fetch(`${r}/m5t/v5/acctGroup`);if(!i.ok)throw Error(`HTTP ${i.status}`);let a=await i.json(),o=(Array.isArray(a?.data)?a.data:[]).filter(t=>t.class===D_&&(!t.parent||t.parent===e.guid));if(!o.length){t.innerHTML=h_(`group_off`,`No clients yet. Add your first client to start managing magnets on their behalf.`),n&&(n.textContent=``);return}t.innerHTML=`<div class="client-list">${o.map(g_).join(``)}</div>`,Yt(t.querySelector(`.client-list`),{rowSelector:`.client-row`,noun:`clients`}),n&&(n.textContent=`${o.length} client${o.length===1?``:`s`}`),t.querySelectorAll(`.client-card__invite`).forEach(e=>{e.addEventListener(`click`,()=>{e.dataset.action===`adaccounts`?x_(e.dataset.clientGuid,e.dataset.clientName):e.dataset.action===`questionnaire`?w_(e.dataset.clientGuid,e.dataset.clientName):S_(e.dataset.clientGuid,e.dataset.clientName)})})}catch(e){console.warn(`[clients] load failed:`,e),t.innerHTML=h_(`error`,`Couldn’t load clients. Please try again.`),n&&(n.textContent=``)}}}function b_(e){if(!e?.guid){d_(`Only reseller / agency accounts can add clients.`,`error`);return}let t=document.createElement(`div`);t.className=`client-modal-scrim`;let n=document.createElement(`div`);n.className=`client-modal`,n.setAttribute(`role`,`dialog`),n.setAttribute(`aria-modal`,`true`),n.innerHTML=`
    ...
    <div class="client-modal__body" id="qm-body">${h_(`hourglass_empty`,`Loading…`)}</div>`,document.body.appendChild(r),document.body.appendChild(i),requestAnimationFrame(()=>{r.classList.add(`open`),i.classList.add(`open`)});let a=()=>{r.classList.remove(`open`),i.classList.remove(`open`),document.removeEventListener(`keydown`,o),setTimeout(()=>{r.remove(),i.remove()},200)},o=e=>{e.key===`Escape`&&a()};document.addEventListener(`keydown`,o),r.addEventListener(`click`,a),i.querySelector(`.client-modal__close`)?.addEventListener(`click`,a);let s=i.querySelector(`#qm-body`);function c(){s.innerHTML=`
      ...
      </div>`,s.querySelector(`[data-act="done"]`)?.addEventListener(`click`,a),s.querySelector(`[data-act="resend"]`)?.addEventListener(`click`,()=>c())}function d(e,t=!1){let r=e.editability||{editable:e.status===`SUBMITTED`&&!e.locked_tz,reason:e.locked_tz?`finalized`:null,deadline:null},i=!!r.editable,o=!i&&e.status===`SUBMITTED`?r.reason===`finalized`?`<div class="qm-status" data-status="LOCKED"><span class="material-symbols-outlined" aria-hidden="true">lock</span> Finalized${e.locked_tz?` ${Y(C_(e.locked_tz))}`:``}${e.locked_by_email?` by ${Y(e.locked_by_email)}`:``} — no further edits.</div>`:`<div class="qm-status" data-status="LOCKED"><span class="material-symbols-outlined" aria-hidden="true">lock_clock</span> Editing closed${r.deadline?` ${Y(C_(r.deadline))}`:``} — a response locks 14 days after it is submitted.</div>`:``,c=i&&r.deadline?`<p class="client-modal__sub qm-editable-hint">You can edit these answers until ${Y(C_(r.deadline))}.</p>`:``;s.innerHTML=`
      <div class="qm-status" data-status="SUBMITTED"><span class="material-symbols-outlined" aria-hidden="true">task_alt</span> Submitted ${e.submitted_tz?Y(C_(e.submitted_tz)):``}</div>
      ${o}
      ${tr(e)}
      <form id="qm-form">
      ...
      </div>`,s.querySelector(`[data-act="done"]`)?.addEventListener(`click`,a),s.querySelector(`[data-act="cancel-edit"]`)?.addEventListener(`click`,()=>d(e,!1)),s.querySelector(`[data-act="edit"]`)?.addEventListener(`click`,()=>d(e,!0));let l=s.querySelector(`[data-act="save"]`);l?.addEventListener(`click`,async()=>{let t=er(s);l.disabled=!0;let r=l.innerHTML;l.innerHTML=`<span class="material-symbols-outlined" aria-hidden="true">progress_activity</span> Saving…`;let i;try{i=await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e.guid)}`,{method:`PUT`,headers:{"Content-Type":`application/json`},body:JSON.stringify({answers:t})});let r=await i.json().catch(()=>({}));if(!i.ok||!r.ok)throw Error(r.error||r.message||`Failed (HTTP ${i.status})`);d_(`Answers updated`,`success`);let a=await(await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e.guid)}`)).json().catch(()=>({}));d(a.ok?a.data:{...e,answers:t},!1)}catch(t){if(i&&i.status===423){d_(t.message||`This questionnaire is now locked.`,`info`);let r=await(await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e.guid)}`)).json().catch(()=>({}));d(r.ok?r.data:e,!1);return}s.querySelector(`#qm-error`).textContent=t.message||`Something went wrong. Please try again.`,l.disabled=!1,l.innerHTML=r}})}try{let t=await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse?owner_guid=${encodeURIComponent(e)}`),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`HTTP ${t.status}`);let i=(r.data||[])[0]||null;if(!i){c();return}if(i.status===`DRAFT`){l(i);return}if(i.status===`CANCELLED`){u(i);return}let a=await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(i.guid)}`),o=await a.json().catch(()=>({}));if(!a.ok||!o.ok)throw Error(o.error||`HTTP ${a.status}`);d(o.data)}catch(e){console.warn(`[clients] questionnaire load failed:`,e),s.innerHTML=`<p class="client-modal__error">Could not load questionnaire status. ${Y(e.message||``)}</p>`}}var T_,E_,D_,O_,k_,Y,A_,j_,M_,N_=o((()=>{T(),Qe(),T_=u(f(),1),m(),Zt(),or(),E_=`reseller_org`,D_=`managed_client`,O_=!1,k_={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Y=e=>String(e??``).replace(/[&<>"']/g,e=>k_[e]),A_=e=>String(e||`?`).trim().split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``).toUpperCase()||`?`,j_=e=>String(e||``).replace(/[^0-9]/g,``),M_={render:__,init:v_}})),P_=c({default:()=>Q_,init:()=>U_,loadTeam:()=>W_,render:()=>H_});function F_(){let e=w.getActiveOrg(),t=e&&e.channel!==q_?e:null;if(!t){let e=w.getMemberships().find(e=>e.channel!==q_);e&&(t={guid:e.orgGuid,channel:e.channel,memberRole:e.memberRole})}if(!t)return null;let n=w.getMemberships().find(e=>e.orgGuid===t.guid)?.name;return{...t,name:n}}function I_(e){return Object.values(Ie).filter(t=>Fe(e,t.value)).sort((e,t)=>t.rank-e.rank)}function L_(){if(J_)return;let e=document.createElement(`style`);e.id=`m5t-team-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),J_=!0}function R_(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function z_(e,t,n){let r=e.member_role||`member`,i=e.entity_guid&&e.entity_guid===w.getEntity?.()?.guid,a=n&&!i&&I_(t.memberRole).some(e=>e.value===r);return`
    <div class="team-row">
      <span class="team-avatar">${X(Z_(e.name||e.email))}</span>
    ...
    </div>`}async function B_(e,t){let{uid:n,email:r}=e.dataset;if(window.confirm(`Revoke the invitation for ${r}?\n\nThe invitation link stops working immediately, including any copy already sent. You can invite this address again afterwards.`)){e.disabled=!0;try{let e=w.getConfig?.().BASE_URL||``,i=await fetch(`${e}/m5t/v5/acctGroup/${encodeURIComponent(t.guid)}/invites/${encodeURIComponent(n)}`,{method:`DELETE`}),a=await i.json().catch(()=>({}));if(!i.ok||!a.ok)throw Error(a.message||a.error||`HTTP ${i.status}`);X_(`Invitation for ${r} revoked`,`success`)}catch(t){e.disabled=!1,X_(t.message||`Could not revoke that invitation.`,`error`);return}try{await W_(t)}catch{}}}async function V_(e,t){let{id:n,name:r}=e.dataset;if(window.confirm(`Remove ${r} from the team?\n\nThey lose access to this workspace immediately. Their account itself is not deleted, and you can invite them again later.`)){e.disabled=!0;try{let e=w.getConfig?.().BASE_URL||``,t=await fetch(`${e}/m5t/v5/acctGroupMember/${encodeURIComponent(n)}`,{method:`DELETE`}),i=await t.json().catch(()=>({}));if(!t.ok||!i.ok)throw Error(i.message||i.error||`HTTP ${t.status}`);X_(`${r} removed from the team`,`success`)}catch(t){e.disabled=!1,X_(t.message||`Could not remove that member.`,`error`);return}try{await W_(t)}catch{}}}function H_(){return L_(),`
    <div class="page-shell" data-section="team">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="team-body">
          ${R_(`hourglass_empty`,`Loading team…`)}
        </div>
      </div>
    </div>`}async function U_(){let e=F_(),t=document.getElementById(`team-invite-btn`);if(!e){t&&(t.style.display=`none`),document.getElementById(`team-body`).innerHTML=R_(`info`,`Team management is available for agency and workspace accounts.`);return}t?.addEventListener(`click`,()=>G_(e)),await W_(e)}async function W_(e){let t=document.getElementById(`team-body`),n=document.getElementById(`team-count`);if(t)try{let r=w.getConfig?.().BASE_URL||``,[i,a]=await Promise.all([fetch(`${r}/m5t/v5/acctGroup/${encodeURIComponent(e.guid)}/members`),fetch(`${r}/m5t/v5/acctGroup/${encodeURIComponent(e.guid)}/invites`)]),[o,s]=await Promise.all([i.json().catch(()=>({})),a.json().catch(()=>({}))]);if(!i.ok||!a.ok)throw Error(`Failed to load team (members: ${i.status}, invites: ${a.status})`);let c=o.members||[],l=s.invites||[],u=I_(e.memberRole).length>0;t.innerHTML=(l.length?`
    ...
    <div class="team-modal__body" id="team-invite-body">
      ...
      <div class="team-modal__actions">
        ...
        </div>`;let u=r.querySelector(`#tm-link`);r.querySelector(`#tm-copy`)?.addEventListener(`click`,async()=>{try{await navigator.clipboard.writeText(u.value),r.querySelector(`#tm-copy`).textContent=`Copied!`}catch{u.select()}}),r.querySelector(`[data-act="done"]`)?.addEventListener(`click`,()=>{i(),W_(e)})}catch(e){console.warn(`[team] invite failed:`,e),o.textContent=e.message||`Something went wrong. Please try again.`,s.disabled=!1,s.innerHTML=l}})}var K_,q_,J_,Y_,X,X_,Z_,Q_,$_=o((()=>{T(),K_=u(f(),1),m(),Qe(),q_=`managed_client`,J_=!1,Y_={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},X=e=>String(e??``).replace(/[&<>"']/g,e=>Y_[e]),X_=(e,t=`info`)=>{try{K_.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},Z_=e=>String(e||`?`).trim().split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``).toUpperCase()||`?`,Q_={render:H_,init:U_}}));m(),we(),T(),Vt(),v(),xn(),On(),nt(),window.m5t=Object.create(null),window.m5t.pubsub=y.default,console.log(`StreamSyncEngage SaaS Client - Starting...`),console.log(`Environment:`,`production`),console.log(`Document readyState:`,document.readyState),console.log(`window.mvvLegit:`,typeof window.mvvLegit),console.log(`globalThis.mvvLegit:`,typeof globalThis.mvvLegit);try{console.log(`mvvLegit (direct):`,typeof mvvLegit)}catch(e){console.log(`mvvLegit (direct): ReferenceError -`,e.message)}var ev=Object.keys(window).filter(e=>e.toLowerCase().includes(`mvv`)||e.toLowerCase().includes(`neodigm`)||e.toLowerCase().includes(`legit`));console.log(`Possible Neodigm globals:`,ev);var Z=new te.default(`/`,{hash:!0});window.router=Z;function Q(e){let t=e?`${e.replace(/-/g,`_`)}_route`:`splash_route`;return console.log(`[Router] Checking access: ${e} -> ${t}`),mvvLegit.isRouteAllowed(t)?!0:(console.warn(`[Router] Access denied to route: ${t}`),y.default.publish(p.UI_TOAST,{message:`Please sign in to access this page`,type:`warning`}),!1)}function $(e){console.log(`[Router] Navigating to:`,e),Z.navigate(`/${e}`)}var tv=new Set([`splash`,`signin`,`signup`,`forgot`,`resethash`,`verf-link`,`accept-invite`,`signout`]),nv={},rv={},iv=null,av={admin:{caption:`Platform Admin`,icon:`shield_person`},reseller:{caption:`Reseller`,icon:`handshake`},managed:{caption:`Managed`,icon:`visibility`},self_serve:{caption:`Self-Serve`,icon:`rocket_launch`}},ov=`__portfolio__`;function sv(){try{Z._setCurrent(null)}catch{}Z.resolve()}function cv(){let e=document.getElementById(`m5t-side-nav`);if(!e)return;let t=!w.getScope()&&w.isAuthenticated(),n=w.getCapabilities(),r=w.getUiMode(),i=Kt.getSideNav(n,{uiMode:r});nv={},rv={};let a=e=>{e.route&&(nv[e.route]={sectionToken:e.token,childToken:null,children:e.children||null},Array.isArray(e.children)&&e.children.forEach(t=>{t.route&&(rv[t.token]=t.route,nv[t.route]={sectionToken:e.token,childToken:t.token,children:e.children})}))},o=e=>e.forEach(e=>{e.group&&Array.isArray(e.items)?o(e.items):a(e)});o(i);let s=w.getMemberships(),c=w.getActiveOrg(),l=w.getManagedClients(),u=w.getStoredActiveOrg(),d=l.length?[{guid:ov,name:`Portfolio (all clients)`},...s.map(e=>({guid:e.orgGuid,name:e.name||e.channel,channel:e.channel,group:`Your organization`})),...l.map(e=>({guid:e.orgGuid,name:e.name||e.orgGuid,channel:e.channel,group:`Clients`}))]:s.map(e=>({guid:e.orgGuid,name:e.name||e.channel,channel:e.channel}));e.setState({buttons:i,orgs:d,activeOrg:l.length?u||ov:c?.guid||null,footerBadge:av[r]||null,loading:t}),uv(),iv&&dv(iv)}var lv=e=>String(e??``).replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`})[e]);function uv(){let e=document.getElementById(`app-content`);if(!e)return;let t=document.getElementById(`org-context-banner`);t||(t=document.createElement(`div`),t.id=`org-context-banner`,e.insertBefore(t,e.firstChild),fn(t));let n=w.getActiveOrg(),r=w.isPreviewingAsClient(),i=!!n?.viaSubtree&&!!iv&&!tv.has(iv);if(t.hidden=!i,!i){t.innerHTML=``;return}let a=lv(n.name||`Client`);t.innerHTML=`
    <style>
