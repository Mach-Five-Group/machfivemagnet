...
/&gt;</code></pre>
  ...
  `,document.head.appendChild(e),L_=!0}function u_(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function d_(e){return`<select class="leads-status" data-id="${q(e.id)}" data-act="status" onclick="event.stopPropagation()">
    ...
    </tr>`}function p_(e){let t=K.search.trim().toLowerCase();if(!t)return!0;let n=[e.first_name,e.last_name,e.email,e.company,e.phone,e.product_interest,K.magnetNames[e.magnet_guid],K.book?K.ownerNames[e.owner_guid]:``,e.utm_campaign,e.utm_source].filter(Boolean).join(` `).toLowerCase();return t.split(/\s+/).every(e=>n.includes(e))}function m_(e){let t=ev[K.range];if(!t)return!0;let n=Y_(e);if(!n)return!1;let r=new Date(Date.now()-(t-1)*864e5).toISOString().slice(0,10);return n.slice(0,10)>=r}function h_(){return K.leads.filter(e=>(K.filter===`ALL`||e.status===K.filter)&&Z_(e)&&$_(e)&&p_(e)&&Q_(e)&&m_(e))}function g_(){let e=h_(),t=nv[K.sort.key];if(!t)return e;let n=K.sort.dir===`asc`?1:-1;return e.slice().sort((e,r)=>{let i=t.value(e),a=t.value(r);return i===``&&a===``?0:i===``?1:a===``?-1:i===a?0:i<a?-n:n})}function __(e,t=``){let n=nv[e],r=K.sort.key===e,i=r?K.sort.dir:``;return`<th class="leads-th--sort${t?` ${t}`:``}${r?` is-sorted`:``}"
    ...
    </th>`}function v_(e){return nv[e]?(K.sort.key===e?K.sort.dir=K.sort.dir===`asc`?`desc`:`asc`:K.sort={key:e,dir:nv[e].dir},K.shown=R_(),!0):!1}function y_(){if(!K.day)return``;let e=K.day;try{e=new Date(`${K.day}T00:00:00Z`).toLocaleDateString([],{month:`short`,day:`numeric`,year:`numeric`,timeZone:`UTC`})}catch{}return`<button class="leads-daychip" type="button" id="leads-dayclear" title="Show every day again">
    ...
    <div class="page-shell" data-section="leads">
      ...
      <div class="leads-layout">
        ...
        <aside class="dashboard-card leads-panel" aria-label="Lead detail">
          <div class="card-body" id="leads-panel-body">${rv()}</div>
        </aside>
      </div>
    </div>`}async function x_(){document.getElementById(`leads-export`)?.addEventListener(`click`,F_),lr(`csv_export`).then(e=>{e||document.getElementById(`leads-export`)?.remove()}).catch(()=>{});let e=io(oo.LEADS_SEARCH),t=io(oo.LEADS_DAY);if((e||t)&&(K.search=e||``,K.day=t||``,K.filter=`ALL`,K.clientFilter=`ALL`,K.magnetFilter=`ALL`,K.range=`ALL`,K.shown=R_()),A()){let e=e=>Number(e||0).toLocaleString();Wa(H_(),[{h:`Leads · wk`,td:t=>e(t.leads.this_week)},{h:`Leads · total`,td:t=>e(t.leads.total)},{h:`Compl.`,td:e=>e.completion_rate==null?`—`:`${e.completion_rate}%`}],{emptyMsg:`No client orgs yet. Create one under Clients first.`})}await C_(),S_()}async function S_(){let e=document.getElementById(`leads-crm-slot`);if(e)try{let{denied:t,providers:n,configured:r}=await nl();if(t)return;let i=iv.map(e=>{let t=Gc[e],i=$c(n,e);return{p:t,conn:i,status:el(t,i,!!r?.[e])}}),a=i.filter(e=>e.status===`connected`),o=i.find(e=>e.status===`broken`);if(a.length){let t=a.map(e=>e.p.label).join(` and `),n=a.length===1&&a[0].conn?.external_account?.hub_domain||``;e.innerHTML=`
    ...
    <div class="leads-bar">
      ...
      <select class="leads-clientsel" id="leads-rangesel" aria-label="Filter by date captured">
        ${Object.keys(ev).map(e=>`<option value="${e}" ${K.range===e?`selected`:``}>${q(tv[e])}</option>`).join(``)}
      </select>
      ${y_()}
...
`)],{type:`text/csv`}),a=URL.createObjectURL(i),o=document.createElement(`a`);o.href=a,o.download=`leads-${K.filter.toLowerCase()}.csv`,document.body.appendChild(o),o.click(),o.remove(),URL.revokeObjectURL(a),V_(`Exported ${e.length} lead${e.length===1?``:`s`}`,`success`)}var I_,L_,K,R_,z_,B_,q,V_,H_,U_,W_,G_,K_,q_,J_,Y_,X_,Z_,Q_,$_,ev,tv,nv,rv,iv,av,ov,sv=o((()=>{T(),I_=u(f(),1),m(),Qa(),co(),An(),Ol(),pr(),x(),no(),L_=!1,K={leads:[],magnetNames:{},filter:`ALL`,canManage:!1,book:!1,ownerNames:{},clientFilter:`ALL`,search:``,shown:25,selectedId:null,day:``,sort:{key:`captured`,dir:`desc`},magnetFilter:`ALL`,range:`ALL`,pageSize:25,total:0,truncated:!1},R_=()=>K.pageSize===`ALL`?2**53-1:Number(K.pageSize)||25,z_=[`NEW`,`WORKING`,`QUALIFIED`,`UNQUALIFIED`,`CONVERTED`,`EXPORTED`],B_={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},q=e=>String(e??``).replace(/[&<>"']/g,e=>B_[e]),V_=(e,t=`info`)=>{try{I_.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},H_=()=>w.getConfig?.().BASE_URL||``,U_=e=>[e.first_name,e.last_name].filter(Boolean).join(` `).trim()||e.email||`(no name)`,W_=e=>(U_(e).split(/\s+/).map(e=>e.replace(/[^A-Za-z0-9]/g,``)).filter(Boolean).slice(0,2).map(e=>e[0]).join(``)||`?`).toUpperCase(),G_={"Google Ads call":`Ads call`,"Google LSA":`Local Services`},K_=e=>K.magnetNames[e.magnet_guid]||G_[e.lead_source]||e.lead_source||`—`,q_={"Google Ads call":`call`},J_=e=>{let t=!e.magnet_guid&&q_[e.lead_source];return t?`<span class="material-symbols-outlined" style="font-size:18px" aria-hidden="true">${t}</span>`:q(W_(e))},Y_=e=>e.captured_at||e.created?.created_tz||``,X_=e=>{let t=Y_(e);if(!t)return!1;try{return Date.now()-new Date(t).getTime()<864e5}catch{return!1}},Z_=e=>K.clientFilter===`ALL`||e.owner_guid===K.clientFilter,Q_=e=>!K.day||Y_(e).slice(0,10)===K.day,$_=e=>K.magnetFilter===`ALL`||e.magnet_guid===K.magnetFilter,ev={ALL:null,7:7,30:30,90:90},tv={ALL:`All time`,7:`Last 7 days`,30:`Last 30 days`,90:`Last 90 days`},nv={name:{label:`Name`,dir:`asc`,value:e=>U_(e).toLowerCase()},contact:{label:`Contact`,dir:`asc`,value:e=>String(e.email||e.phone||``).toLowerCase()},captured:{label:`Captured`,dir:`desc`,value:e=>Y_(e)}},rv=()=>u_(`touch_app`,`Click to see lead information.`),iv=[`hubspot`,`zoho`,`salesforce`,`gohighlevel`],av=()=>`
  <colgroup>
    <col style="width:22%">
  ...
  </colgroup>`,ov={booked:[`QUALIFIED`,`Booked`],rescheduled:[`WORKING`,`Rescheduled`],canceled:[`CANCELED`,`Canceled`]}})),cv=c({init:()=>_v,render:()=>pv,tableHTML:()=>fv});function lv(){if(Dv)return;let e=document.createElement(`style`);e.id=`m5t-analytics-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Dv=!0}function uv(e,t){let n=t>0?Math.round(e/t*100):0,r=`${xv(e)} of ${xv(t)} leads for the busiest magnet`;return`<svg class="anl-bar" width="72" height="8" viewBox="0 0 72 8" role="img"
      aria-label="${Cv(r)}">
      <title>${Cv(r)}</title>
    ...
    </svg>`}function dv(e,t,n){let r=kv[e.status]?`<span class="anl-chip">${Cv(kv[e.status])}</span>`:``,i=n&&e.owner_name?`<span class="anl-sub">${Cv(e.owner_name)}</span>`:``,a=e.ab_tests&&(e.ab_tests.running||e.ab_tests.completed)?`${xv(e.ab_tests.running)} / ${xv(e.ab_tests.completed)}`:`—`;return`<tr data-guid="${Cv(e.magnet_guid)}">
    <td><a class="anl-name" href="#/magnets">${Cv(e.name)}</a>${r}${i}</td>
    <td>${uv(Number(e.leads?.window||0),t)}</td>
    <td>${xv(e.leads?.window)}</td>
    <td>${xv(e.opens)}</td>
    <td>${wv(e.engagement_rate)}</td>
    <td>${wv(e.completion_rate)}</td>
    <td>${wv(e.lead_rate)}</td>
    <td>${a}</td>
  </tr>`}function fv(e,t){let n=e.reduce((e,t)=>Math.max(e,Number(t.leads?.window||0)),0);return`<div class="anl-wrap"><table class="anl-table">
    <thead><tr>
      <th>Magnet</th><th></th><th>Leads</th><th>Opens</th>
      <th>Engaged</th><th>Completed</th><th>Leads / Open</th><th>A/B run / done</th>
    </tr></thead>
    <tbody>${e.map(e=>dv(e,n,t)).join(``)}</tbody>
  </table></div>`}function pv(){return A()?Ha({section:`analytics`,title:`Analytics`,description:`Pick a client to see how their magnets are performing.`}):(lv(),mo(),`
    <div class="page-shell" data-section="analytics">
      <div class="page-header">
        ...
        <div class="chart-controls" id="anl-periods">
          ${Tv.map(e=>`<button class="chart-period-btn${e.d===Ev?` active`:``}" type="button" data-days="${e.d}">${e.label}</button>`).join(``)}
        </div>
      </div>
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="anl-body">
          ${Ov(`hourglass_empty`,`Loading magnet performance…`)}
        </div>
      </div>
    ...
    </div>`)}function mv(e){try{return new Date(`${e}T00:00:00`).toLocaleDateString(void 0,{month:`short`,day:`numeric`,year:`numeric`})}catch{return String(e||``)}}function hv(e){return ho({rows:e,xLabel:e=>String(e.date||``).slice(5),left:{value:e=>e.opens||0,label:`Magnet Opens`},right:{value:e=>e.leads||0,label:`Leads`},ariaLabel:`Magnet opens versus leads over time`,activatable:e=>Number(e.leads||0)>0,pointLabel:e=>`${mv(e.date)}: ${xv(e.opens||0)} opens, ${xv(e.leads||0)} leads`,hitData:e=>({day:e.date,leads:Number(e.leads||0)})})}function gv(e){let t=document.getElementById(`anl-trend`);return t?_o(t,{rows:e,tipHTML:e=>`
      <div class="m5c-tip__title">${mv(e.date)}</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--1"></i>${xv(e.opens||0)} opens</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--2"></i>${xv(e.leads||0)} leads</div>
      ${Number(e.leads||0)>0?`<div class="m5c-tip__go">Open these leads →</div>`:``}`,onActivate:e=>{ro(oo.LEADS_DAY,e.date),window.location.hash=`#/leads`}}):()=>{}}async function _v(){if(A())return Wa(yv(),[{h:`Leads`,td:e=>xv(e.leads.window??e.leads.total)},{h:`Opens`,td:e=>xv(e.opens)},{h:`Completed`,td:e=>wv(e.completion_rate)}]);lv(),mo(),await vv();let e=document.getElementById(`anl-periods`);e&&e.addEventListener(`click`,t=>{let n=t.target.closest(`.chart-period-btn`);n&&(Ev=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>{e.classList.toggle(`active`,Number(e.dataset.days)===Ev)}),vv())})}async function vv(){let e=document.getElementById(`anl-body`),t=document.getElementById(`anl-range`);if(!e)return;e.innerHTML=Ov(`hourglass_empty`,`Loading magnet performance…`);let n=new Date,r=bv(n),i=bv(new Date(n.getTime()-(Ev-1)*864e5));t&&(t.textContent=`${i} → ${r}`);let a;try{let e=await b(`${yv()}/m5t/v5/dashboard/stats?from=${i}&to=${r}&group_by=magnet`);if(a=await e.json(),j(e,a),!a?.ok)throw Error(a?.error||`Failed to load`)}catch(t){e.innerHTML=$a(t)?eo():Ov(`cloud_off`,`Could not load analytics. Refresh to try again.`),console.error(`[analytics] load failed:`,t);return}let o=(e,t)=>{let n=document.getElementById(e);n&&(n.textContent=t)};o(`anl-leads`,xv(a.by_day?.reduce((e,t)=>e+Number(t.leads||0),0))),o(`anl-opens`,xv(a.funnel?.opens)),o(`anl-completion`,wv(a.funnel?.completion_rate)),o(`anl-active`,xv(a.magnets?.active));let s=Array.isArray(a.by_day)?a.by_day:[],c=document.getElementById(`anl-trend`);if(c){let e=s.some(e=>Number(e.opens||0)>0||Number(e.leads||0)>0);c.innerHTML=e?hv(s):Ov(`show_chart`,`No activity in this range yet. Opens and leads chart here as visitors engage your magnets.`),e&&gv(s)}let l=Array.isArray(a.by_magnet)?a.by_magnet:[];if(!l.length){e.innerHTML=Ov(`widgets`,`No magnets yet. Create one and its numbers will show up here.`);return}let u=l.some(e=>Number(e.opens||0)>0||Number(e.leads?.window||0)>0),d=new Set(l.map(e=>e.owner_guid)).size>1;e.innerHTML=(a.by_magnet_truncated?`<p class="anl-warn">Showing the top ${l.length} magnets by leads. You have more than that, so this list is not complete.</p>`:``)+(u?``:`<p class="anl-warn">No opens or leads for any magnet in this period. Try a longer range.</p>`)+fv(l,d);let f=e.querySelector(`.anl-table tbody`);f&&O(f,{rowSelector:`tr`,noun:`magnets`,controlHost:e.querySelector(`.anl-wrap`)||e})}var yv,bv,xv,Sv,Cv,wv,Tv,Ev,Dv,Ov,kv,Av=o((()=>{T(),Qa(),fo(),An(),So(),co(),x(),no(),yv=()=>w.getConfig?.().BASE_URL||``,bv=e=>e.toISOString().slice(0,10),xv=e=>Number(e||0).toLocaleString(),Sv={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Cv=e=>String(e??``).replace(/[&<>"']/g,e=>Sv[e]),wv=e=>e==null?`—`:`${e}%`,Tv=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],Ev=30,Dv=!1,Ov=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,kv={DRAFT:`Draft`,DISABLED:`Paused`}})),jv=c({default:()=>ly,init:()=>Pv,render:()=>Nv});function Mv(){if(mo(),oy)return;let e=document.createElement(`style`);e.id=`m5t-conversions-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),oy=!0}function Nv(){return A()?Ha({section:`analytics-conversions`,title:`Conversions`,description:`Pick a client to view their conversion reporting.`}):(Mv(),J.canSave=bl(),`
    <div class="page-shell" data-section="analytics-conversions">
      <div class="page-header">
        ...
        <div class="chart-controls" id="conv-periods">
          ${iy.map(e=>`<button class="chart-period-btn${e.d===ay?` active`:``}" type="button" data-days="${e.d}">${e.label}</button>`).join(``)}
        </div>
      </div>
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="conv-selector">
          ${sy(`hourglass_empty`,`Loading conversion actions.`)}
        </div>
      </div>
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="conv-chart">
          ${sy(`hourglass_empty`,`Loading conversions.`)}
        </div>
      </div>
    ...
    </div>`)}async function Pv(){if(A()){let e=e=>Number(e||0).toLocaleString();return Wa(qv(),[{h:`Spend`,td:t=>t.ad?`$${e(t.ad.spend)}`:Yv},{h:`Clicks`,td:t=>t.ad?e(t.ad.clicks):Yv},{h:`Leads`,td:t=>e(t.leads.window??t.leads.total)},{h:`CPL`,td:t=>t.ad?.cost_per_lead==null?Yv:`$${e(t.ad.cost_per_lead)}`}],{emptyMsg:`No client orgs yet. Connect a client ad account to see conversions here.`})}let e=document.getElementById(`conv-periods`);e?.addEventListener(`click`,t=>{let n=t.target.closest(`button[data-days]`);n&&(ay=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>e.classList.toggle(`active`,Number(e.dataset.days)===ay)),Fv())}),document.getElementById(`conv-selector`)?.addEventListener(`change`,e=>{let t=e.target.closest(`input[type="checkbox"][data-action-id]`);t&&(t.checked?J.selection.add(t.value):J.selection.delete(t.value),Hv(),Lv())}),await Fv()}async function Fv(){let e=new Date;J.to=Jv(e),J.from=Jv(new Date(e.getTime()-(ay-1)*864e5));let t=document.getElementById(`conv-range`);t&&(t.textContent=`${J.from} → ${J.to}`);let n=qv(),r=`from=${J.from}&to=${J.to}`,i=null,a=null,o=null,s=[],c=null;try{let e=bl(),[t,l,u,d,f]=await Promise.all([b(`${n}/m5t/v5/coreAdMetrics/overlay?${r}`).then(e=>e.json()).catch(()=>null),b(`${n}/m5t/v5/coreAdMetrics/conversions?${r}`).then(e=>e.json()).catch(()=>null),b(`${n}/m5t/v5/coreAdMetrics/upload-ledger?${r}`).then(e=>e.json()).catch(()=>null),b(`${n}/m5t/v5/coreAdAccount`).then(e=>e.json()).catch(()=>null),e?b(`${n}/m5t/v5/integrations/google_ads/conversion_actions`).then(e=>e.json()).catch(()=>null):Promise.resolve(null)]);i=t,a=l,o=u,c=f,s=(d?.data||[]).filter(e=>e.status!==`DELETED`)}catch(e){console.warn(`[analytics-conversions] load failed:`,e),Kv(`conv-chart`,sy(`error`,`Could not load conversions. Please try again.`));return}J.spend=i?.totals?.spend??null,J.overlayRows=i?.rows||[],J.canSave=bl();let l=s.filter(e=>e.provider===`google_ads`);J.universe=Iv({catalog:c,conv:a,accounts:l});let u=a?.has_selection&&Array.isArray(a.selected_ids)?a.selected_ids.map(String):[],d=J.universe.filter(e=>e.magnet).map(e=>e.id);if(J.selection=new Set(u.length?u:d),Wv(o),Vv({accounts:l}),Hv(),!l.length){Gv(`conv-spend`,Xv(J.spend)),Kv(`conv-chart`,sy(`link_off`,`No ad account connected yet. Your agency can connect Google Ads to see conversions here.`)),Kv(`conv-actions`,sy(`link_off`,`No ad account connected yet.`));return}u.length||!J.selection.size?(J.conv=a,Rv()):await Lv()}function Iv({catalog:e,conv:t,accounts:n}){let r=new Map,i=(e,t)=>{let n=String(e||``);if(!n)return;let i=r.get(n)||{id:n,name:null,category:null,magnet:!1,kind:null};r.set(n,{...i,...Object.fromEntries(Object.entries(t).filter(([,e])=>e!=null&&e!==!1))})};(e?.data?.actions||[]).forEach(e=>i(e.id,{name:e.name,category:e.category})),(t?.actions||[]).forEach(e=>i(e.id,{name:e.name,category:e.category})),(t?.available||[]).forEach(e=>i(e.id,{name:e.name,category:e.category})),n.forEach(e=>{let t=e.account_config?.magnet_actions||{};Object.entries(t).forEach(([e,t])=>{t?.id&&i(t.id,{name:t.name,magnet:!0,kind:e})})}),r.forEach(e=>{!e.magnet&&e.name&&ry.has(e.name)&&(e.magnet=!0)});let a=[...r.values()],o=e=>e.magnet?ty.indexOf(e.kind):-1;return a.sort((e,t)=>{if(e.magnet!==t.magnet)return e.magnet?-1:1;if(e.magnet){let n=o(e),r=o(t);if(n!==r)return(n<0?99:n)-(r<0?99:r)}return String(e.name||e.id).localeCompare(String(t.name||t.id))})}async function Lv(){let e=[...J.selection],t=document.getElementById(`conv-chart`);t?.classList.add(`conv-busy`),J.busy=!0;try{if(!e.length)J.conv={ok:!0,has_selection:!1,rows:[],actions:[],totals:{conversions:0,value:0,all_conversions:0,all_value:0}};else{let t=await b(`${qv()}/m5t/v5/coreAdMetrics/conversions?from=${J.from}&to=${J.to}&actions=${encodeURIComponent(e.join(`,`))}`),n=await t.json().catch(()=>({}));j(t,n),J.conv=n}Rv()}catch(e){console.warn(`[analytics-conversions] recompute failed:`,e),Kv(`conv-chart`,$a(e)?eo():sy(`error`,`Could not recompute for that selection. Please try again.`))}finally{J.busy=!1,t?.classList.remove(`conv-busy`)}}function Rv(){let e=J.conv||{},t=e.totals||{conversions:0,value:0},n=Number(t.all_conversions??t.conversions??0);Gv(`conv-spend`,Xv(J.spend)),Gv(`conv-count`,Zv(n)),Gv(`conv-cpc`,Xv(ey(J.spend,n))),Gv(`conv-value`,Xv(t.all_value??t.value??0)),zv(e.rows||[]),Bv(e.actions||[])}function zv(e){let t=document.getElementById(`conv-chart`);if(!t)return;if(!J.selection.size){t.innerHTML=sy(`checklist`,`No actions counted. Tick a conversion action to see it here.`),J.rows=[];return}let n=new Map(e.map(e=>[String(e.date).slice(0,10),e])),r=J.overlayRows.length?J.overlayRows.map(e=>String(e.date).slice(0,10)):e.map(e=>String(e.date).slice(0,10)),i=new Map(J.overlayRows.map(e=>[String(e.date).slice(0,10),Number(e.spend||0)])),a=r.map(e=>({date:e,spend:i.get(e)||0,conversions:Number(n.get(e)?.all_conversions??n.get(e)?.conversions??0)}));if(J.rows=a,!a.length){t.innerHTML=sy(`search_off`,`No conversions in this range for the selected actions.`);return}t.innerHTML=ho({rows:a,xLabel:e=>String(e.date||``).slice(5),left:{value:e=>e.spend||0,label:`Ad Spend`,fmt:e=>`$${bo(e)}`},right:{value:e=>e.conversions||0,label:`Conversions`},ariaLabel:`Ad spend versus counted conversions over time`,pointLabel:e=>`${e.date}: ${Xv(e.spend||0)} spend, ${Zv(e.conversions||0)} conversions`,hitData:e=>({day:e.date})}),_o(t,{rows:a,tipHTML:e=>`
      <div class="m5c-tip__title">${$v(String(e.date||``))}</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--1"></i>${Xv(e.spend||0)} spend</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--2"></i>${Zv(e.conversions||0)} conversions</div>`})}function Bv(e){let t=document.getElementById(`conv-actions`);if(!t)return;let n=new Set(J.universe.filter(e=>e.magnet).map(e=>e.id)),r=e.filter(e=>J.selection.has(String(e.id)));if(!r.length){t.innerHTML=sy(`search_off`,J.selection.size?`No conversions in this range for the selected actions.`:`No actions counted. Tick a conversion action to see it here.`);return}t.innerHTML=`
    <div class="conv-wrap">
      <table class="conv-table">
        ...
        <tbody>${r.map(e=>`
          <tr>
            <td><span class="conv-name">${$v(e.name||e.id)}</span>${n.has(String(e.id))?` <span class="conv-tag">Magnet</span>`:``}</td>
            <td class="sub">${$v(cy(e.category))}</td>
            <td class="num"><b>${Zv(e.all_conversions??e.conversions)}</b></td>
            <td class="num">${Zv(e.conversions)}</td>
            <td class="num">${Xv(e.all_value??e.value??0)}</td>
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>`;let i=t.querySelector(`.conv-table tbody`);i&&O(i,{rowSelector:`tr`,noun:`actions`,controlHost:t.querySelector(`.conv-wrap`)||t})}function Vv({accounts:e}){let t=document.getElementById(`conv-selector`);if(!t)return;if(!e.length){t.innerHTML=sy(`link_off`,`No ad account connected yet. Your agency can connect Google Ads to see conversions here.`);return}if(!J.universe.length){t.innerHTML=`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">checklist</span>
    ...
    <label class="conv-row">
      <input type="checkbox" data-action-id="${$v(e.id)}" value="${$v(e.id)}"${J.selection.has(e.id)?` checked`:``}>
      <span class="conv-name">${$v(e.name||e.id)}</span>
      ${e.magnet?`<span class="conv-tag">Magnet</span>`:``}
      ${e.category?`<span class="conv-cat">${$v(cy(e.category))}</span>`:``}
    </label>`;t.innerHTML=`
    ...
    ${J.canSave?`
      ...
      </div>`:``}`;let a=t.querySelector(`#conv-list-other`);a&&O(a,{rowSelector:`.conv-row`,noun:`actions`}),document.getElementById(`conv-save`)?.addEventListener(`click`,Uv)}function Hv(){let e=document.getElementById(`conv-selcount`);if(!e)return;let t=J.selection.size;e.textContent=t?`${t} action${t===1?``:`s`} counted`:`No actions counted`}async function Uv(){let e=document.getElementById(`conv-save`),t=document.getElementById(`conv-savemsg`),n=[...J.selection];e&&(e.disabled=!0),t&&(t.classList.remove(`err`),t.textContent=`Saving.`);try{let e=await(await b(`${qv()}/m5t/v5/integrations/google_ads/conversion_actions`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({conversion_action_ids:n})})).json().catch(()=>({}));if(!e.ok)throw Error(e.error||`save failed`);t&&(t.textContent=n.length?`Saved. ${n.length} action${n.length===1?``:`s`} counted by default.`:`Saved. No actions counted by default.`)}catch{t&&(t.classList.add(`err`),t.textContent=`Could not save the selection. Please try again.`)}finally{e&&(e.disabled=!1)}}function Wv(e){let t=document.getElementById(`conv-ledger`);if(!t)return;let n=e?.kinds||[];if(!n.length){t.innerHTML=sy(`upload`,`Nothing fed into Google Ads in this range yet.`);return}let r=e=>ny[e]||String(e||``).replace(/_/g,` `),i=e=>{let t=ty.indexOf(e.kind);return t<0?99:t};t.innerHTML=`
    <div class="conv-wrap">
      <table class="conv-table" id="conv-ledger-table">
        ...
        <tbody>${[...n].sort((e,t)=>i(e)-i(t)).map(e=>`
          <tr data-kind="${$v(e.kind)}">
            <td><span class="conv-name">${$v(r(e.kind))}</span></td>
            <td class="num"><b>${Zv(e.sent)}</b></td>
            <td class="num">${Zv(e.queued)}</td>
            <td class="num">${Zv(e.failed)}</td>
            <td class="num">${Zv(e.skipped)}</td>
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>
    <p class="conv-note">What Magnet fed into Google Ads. Google's attributed numbers above are its own and are never added to these.</p>`}function Gv(e,t){let n=document.getElementById(e);n&&(n.textContent=t)}function Kv(e,t){let n=document.getElementById(e);n&&(n.innerHTML=t)}var qv,Jv,Yv,Xv,Zv,Qv,$v,ey,ty,ny,ry,iy,ay,J,oy,sy,cy,ly,uy=o((()=>{T(),Qa(),Ol(),fo(),So(),An(),x(),no(),qv=()=>w.getConfig?.().BASE_URL||``,Jv=e=>e.toISOString().slice(0,10),Yv=`—`,Xv=e=>e==null?Yv:`$`+Number(e).toLocaleString(void 0,{minimumFractionDigits:0,maximumFractionDigits:2}),Zv=e=>e==null?Yv:Number(e).toLocaleString(),Qv={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},$v=e=>String(e??``).replace(/[&<>"']/g,e=>Qv[e]),ey=(e,t)=>{let n=Number(e),r=Number(t);return!Number.isFinite(n)||!Number.isFinite(r)||r<=0?null:Math.round(n/r*100)/100},ty=[`magnet_fill`,`qualified_lead`,`call_dni`,`calls_from_ads`],ny={magnet_fill:`Magnet fills`,qualified_lead:`Qualified leads`,call_dni:`Website calls`,calls_from_ads:`Ad calls`},ry=new Set([`Magnet Lead`,`Magnet Qualified Lead`,`Magnet Phone Call (Website)`,`Ad Calls (Magnet reporting)`,`Calls from Ads (Magnet)`]),iy=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],ay=30,J={from:null,to:null,spend:null,universe:[],selection:new Set,conv:null,rows:[],overlayRows:[],canSave:!1,busy:!1},de(()=>{J.busy=!1}),oy=!1,sy=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,cy=e=>String(e||``).toLowerCase().replace(/_/g,` `)||Yv,ly={render:Nv,init:Pv}})),dy=c({init:()=>my,render:()=>py});function fy(){if(wy)return;let e=document.createElement(`style`);e.id=`m5t-billing-styles`,e.textContent=`
    ...
    .page-shell .chart-period-btn { cursor: pointer; }
    .page-shell .chart-period-btn.active { background: var(--color-primary-600, #0072BA); color: #fff; }
  `,document.head.appendChild(e),wy=!0}function py(){return fy(),`
    <div class="page-shell" data-section="billing">
      ...
      <div class="dashboard-card">
        <div class="card-body" id="bill-plan-card">${Ty(`hourglass_empty`,`Loading your plan…`)}</div>
      </div>
    ...
    </div>`}async function my(){document.getElementById(`bill-interval`)?.addEventListener(`click`,e=>{let t=e.target.closest(`button[data-interval]`);t&&(Dy=t.dataset.interval,document.querySelectorAll(`#bill-interval .chart-period-btn`).forEach(e=>e.classList.toggle(`active`,e.dataset.interval===Dy)),Oy&&hy(Oy))}),document.getElementById(`bill-upgrade-card`)?.addEventListener(`click`,async e=>{let t=e.target.closest(`button[data-plan]`);if(t){t.disabled=!0;try{let e=await(await b(`${by()}/m5t/v5/billing/checkout`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({plan:t.dataset.plan,interval:Dy})})).json();if(!e.ok||!e.data?.url)throw Error(e.error||`checkout failed`);window.location.href=e.data.url}catch(e){console.warn(`[billing] checkout failed:`,e),t.disabled=!1,t.textContent=`Could not start checkout. Try again.`}}}),document.getElementById(`bill-plan-card`)?.addEventListener(`click`,async e=>{let t=e.target.closest(`#bill-manage`);if(t){t.disabled=!0;try{let e=await(await b(`${by()}/m5t/v5/billing/portal`,{method:`POST`,headers:{"Content-Type":`application/json`},body:`{}`})).json();if(!e.ok||!e.data?.url)throw Error(e.error||`portal failed`);window.location.href=e.data.url}catch(e){console.warn(`[billing] portal failed:`,e),t.disabled=!1}}}),await yy()}function hy(e){let t=document.getElementById(`bill-upgrade-card`),n=document.getElementById(`bill-upgrade`);if(!t||!n)return;let r=e.plan?.key===`free`&&e.checkout_ready;if(t.style.display=r?``:`none`,!r)return;let i=e=>`
    <div class="bill-meter">
      <div class="bill-meter__k">${Cy(e.label)}</div>
      <div class="bill-meter__v">$${e.prices[Dy]}<small>/month${Dy===`annual`?`, billed yearly`:``}</small></div>
      ...
      <div style="margin-top:12px"><button type="button" class="bill-btn" data-plan="${e.key}"><span class="material-symbols-outlined" aria-hidden="true">arrow_forward</span>Choose ${Cy(e.label)}</button></div>
    </div>`;n.innerHTML=`<div class="bill-meters" style="margin-top:0">${i(et.pro)}${i(et.enterprise)}</div>`}function gy(e,t=null){return e.key===`enterprise`&&!t?`Included with your program.`:e.key===`grandfathered`?`All features included.`:t&&e.prices?`${t.interval===`annual`?`$${e.prices.annual}/month, billed yearly`:`$${e.prices.monthly}/month`}.`:``}function _y(e){let t=`for every display format, integrations, and ${xy(et.pro.limits.leads_mo)} leads a month.`,n=e?`<a href="#/billing" id="bill-up-link">Upgrade to Pro</a>`:`Upgrade to Pro`;return`${Cy(`Design a free magnet for your business and track its leads.`)} ${n} ${Cy(t)}`}function vy(e,t,n,{hint:r=``,consumption:i=!1}={}){let a=n==null,o=a?0:Math.min(100,Math.round(t/Math.max(1,n)*100)),s=i&&o>=80;return`
    <div class="bill-meter">
      <div class="bill-meter__k">${Cy(e)}</div>
      <div class="bill-meter__v">${xy(t)}${a?` <small>No limit</small>`:` <small>of ${xy(n)}</small>`}</div>
      ${a?``:`<div class="bill-bar"><i style="width:${o}%" data-warn="${s}"></i></div>`}
      ${r?`<div class="bill-meter__hint">${Cy(r)}</div>`:``}
    </div>`}async function yy(){let e=document.getElementById(`bill-plan-card`),t;try{let e=await b(`${by()}/m5t/v5/billing/usage`);if(t=await e.json(),j(e,t),!t.ok)throw Error(t.error||`load failed`)}catch(t){console.warn(`[billing] load failed:`,t),e&&(e.innerHTML=$a(t)?eo():Ty(`error`,`Could not load billing. Please try again.`));return}let n=t.plan||et.grandfathered,r=t.usage||{},i=t.subscription||null;if(Oy=t,e){let r=i?.current_period_end?`${i.cancel_at_period_end?`Ends`:`Renews`} ${new Date(i.current_period_end).toLocaleDateString([],{month:`short`,day:`numeric`,year:`numeric`})}.`:``,a=i?.status===`PAST_DUE`?` <span class="bill-plan__badge" style="background: var(--color-danger, #DD4124)">Payment needed</span>`:``;e.innerHTML=`
      <div class="bill-plan">
        <span class="bill-plan__name">${Cy(n.label)}</span>
      ...
      <div class="bill-plan__sub">${n.key===`free`?_y(!!t.checkout_ready):Cy([gy(n,i),r].filter(Boolean).join(` `))}</div>`,e.querySelector(`#bill-up-link`)?.addEventListener(`click`,e=>{e.preventDefault(),document.getElementById(`bill-upgrade-card`)?.scrollIntoView({behavior:`smooth`,block:`start`})})}hy(t);let a=document.getElementById(`bill-meters`);if(a){let e=n.key===`free`;a.innerHTML=[vy(`Live magnets`,r.magnets,n.limits?.magnets??null,{hint:e?`Pro includes unlimited magnets.`:``}),vy(`Leads this month`,r.leads_month,n.limits?.leads_mo??null,{consumption:!0,hint:e?`Pro includes ${xy(et.pro.limits.leads_mo)} a month.`:``}),vy(`Team members`,r.team_members,n.limits?.team_members??null,{hint:e?`Pro includes ${xy(et.pro.limits.team_members)} seats.`:``})].join(``)}let o=document.getElementById(`bill-feat-card`),s=document.getElementById(`bill-feats`);if(o&&s){let e=[],t=n.features?.formats||[];e.push(`<div class="bill-feat"><span class="material-symbols-outlined" aria-hidden="true">check_circle</span>${t.length>=4?`All four display formats`:`Chat bubble format`}</div>`);for(let[t,r]of Ey)r&&n.features?.[t]&&e.push(`<div class="bill-feat"><span class="material-symbols-outlined" aria-hidden="true">check_circle</span>${Cy(r)}</div>`);let r=``;if(n.key===`free`){let e=[];(n.features?.formats||[]).length<4&&(et.pro.features.formats||[]).length>=4&&e.push(`All four display formats`);for(let[t,r]of Ey)r&&!n.features?.[t]&&et.pro.features?.[t]&&e.push(r);e.length&&(r=`<div class="bill-feats__pro-label">Included with Pro</div>
          <div class="bill-feats">${e.map(e=>`<div class="bill-feat bill-feat--pro"><span class="material-symbols-outlined" aria-hidden="true">add_circle</span>${Cy(e)}</div>`).join(``)}</div>`)}s.innerHTML=`<div class="bill-feats">${e.join(``)}</div>${r}`,o.style.display=``}let c=document.getElementById(`bill-note`);c&&(c.textContent=n.key===`enterprise`?`Usage is informational. Your program covers the platform.`:`Lead counts reset monthly. This period: ${t.period?.start||``} to ${t.period?.end||``}.`)}var by,xy,Sy,Cy,wy,Ty,Ey,Dy,Oy,ky=o((()=>{T(),rt(),x(),no(),by=()=>w.getConfig?.().BASE_URL||``,xy=e=>Number(e||0).toLocaleString(),Sy={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Cy=e=>String(e??``).replace(/[&<>"']/g,e=>Sy[e]),wy=!1,Ty=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,Ey=[[`formats`,null],[`hub_scheduler`,`Welcome hub and scheduler`],[`ab_testing`,`A/B testing`],[`journeys`,`Journeys`],[`attribution`,`Campaign attribution`],[`crm_sync`,`CRM sync`],[`booking`,`Booking integrations`],[`webhooks`,`Webhooks and Zapier`],[`csv_export`,`CSV export`],[`ad_reporting`,`Ad performance reporting`]],Dy=`monthly`,Oy=null})),Ay=c({init:()=>Ny,render:()=>My});function jy(){if(mo(),gl(),Gy)return;let e=document.createElement(`style`);e.id=`m5t-traffic-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Gy=!0}function My(){return A()?Ha({section:`traffic`,title:`Traffic`,description:`Pick a client to view their website traffic.`}):(jy(),`
    <div class="page-shell" data-section="traffic">
      <div class="page-header">
        ...
        <div class="chart-controls" id="traffic-periods">
          ${qy.map(e=>`<button class="chart-period-btn${e.d===Jy?` active`:``}" type="button" data-days="${e.d}">${e.label}</button>`).join(``)}
        </div>
      </div>
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="traffic-body">
          ${Ky(`hourglass_empty`,`Loading traffic…`)}
        </div>
      </div>
    ...
    </div>`)}async function Ny(){if(A()){let e=e=>Number(e||0).toLocaleString();return Wa(zy(),[{h:`Leads`,td:t=>e(t.leads.window??t.leads.total)}],{emptyMsg:`No client orgs yet. Connect a client GA4 property to see traffic here.`})}let e=document.getElementById(`traffic-periods`);e?.addEventListener(`click`,t=>{let n=t.target.closest(`button[data-days]`);n&&(Jy=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>e.classList.toggle(`active`,Number(e.dataset.days)===Jy)),Py())}),await Py()}async function Py(){let e=zy(),t=document.getElementById(`traffic-body`),n=document.getElementById(`traffic-range`),r=new Date,i=By(r),a=By(new Date(r.getTime()-(Jy-1)*864e5));n&&(n.textContent=`${a} → ${i}`);let o,s=null;try{let[t,n]=await Promise.all([b(`${e}/m5t/v5/coreAnalyticsMetrics/overlay?from=${a}&to=${i}`),nl().catch(()=>null)]);j(t),o=await t.json(),s=n}catch(e){console.warn(`[traffic] load failed:`,e),t&&(t.innerHTML=$a(e)?eo():Ky(`error`,`Could not load traffic. Please try again.`));return}Fy(s);let c=o.totals||{};if(Iy(`traffic-sessions`,Vy(c.sessions)),Iy(`traffic-users`,Vy(c.users)),Iy(`traffic-views`,Vy(c.page_views)),Iy(`traffic-engagement`,Hy(c.engagement_rate)),Iy(`traffic-leads`,Vy(c.magnet_leads)),Iy(`traffic-leadrate`,Hy(c.lead_rate)),!o.connected){t.innerHTML=s&&!s.denied&&$c(s.providers,`google_analytics`)?Ky(`cloud_sync`,`GA4 connected. No traffic for this range yet. Metrics appear here once they sync.`):Ky(`link_off`,`No GA4 property connected yet. Connect Google Analytics under Settings, Integrations to see traffic here.`);return}let l=o.rows||[];t.innerHTML=Ly(l),Ry(l)}function Fy(e){let t=document.getElementById(`traffic-conn`);if(!t)return;if(!e||e.denied){t.innerHTML=``;return}let n=Gc.google_analytics,r=$c(e.providers,`google_analytics`);if(!r){t.innerHTML=``;return}let i=el(n,r,!!e.configured?.google_analytics);if(i!==`connected`&&i!==`broken`){t.innerHTML=``;return}let a=bl()?` <a href="#/settings/integrations/${n.urlSlug}">Manage</a>`:``;t.innerHTML=`<span>Google Analytics</span> ${tl(i,r)}${a}`}function Iy(e,t){let n=document.getElementById(e);n&&(n.textContent=t)}function Ly(e){return ho({rows:e,xLabel:e=>String(e.date||``).slice(5),left:{value:e=>e.sessions||0,label:`Sessions`,fmt:e=>bo(e)},right:{value:e=>e.magnet_leads||0,label:`Magnet Leads`},ariaLabel:`Website sessions versus magnet leads over time`,pointLabel:e=>`${e.date}: ${Vy(e.sessions||0)} sessions, ${Vy(e.magnet_leads||0)} leads`,hitData:e=>({day:e.date})})}function Ry(e){let t=document.getElementById(`traffic-body`);return t?_o(t,{rows:e,tipHTML:e=>`
      <div class="m5c-tip__title">${Wy(String(e.date||``))}</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--1"></i>${Vy(e.sessions||0)} sessions</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--2"></i>${Vy(e.magnet_leads||0)} leads</div>`}):()=>{}}var zy,By,Vy,Hy,Uy,Wy,Gy,Ky,qy,Jy,Yy=o((()=>{T(),Qa(),Ol(),fo(),So(),x(),no(),zy=()=>w.getConfig?.().BASE_URL||``,By=e=>e.toISOString().slice(0,10),Vy=e=>Number(e||0).toLocaleString(),Hy=e=>e==null?`—`:`${e}%`,Uy={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Wy=e=>String(e??``).replace(/[&<>"']/g,e=>Uy[e]),Gy=!1,Ky=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,qy=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],Jy=30})),Xy=c({default:()=>Sb,init:()=>$y,render:()=>Qy});function Zy(){if(mo(),gb)return;let e=document.createElement(`style`);e.id=`m5t-adperf-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),gb=!0}function Qy(){return A()?Ha({section:`ad-performance`,title:`Ad Performance`,description:`Pick a client to view their spend and lead performance.`}):(Zy(),`
    <div class="page-shell" data-section="ad-performance">
      <div class="page-header">
        ...
        <div class="chart-controls" id="adperf-periods">
          ${vb.map(e=>`<button class="chart-period-btn${e.d===yb?` active`:``}" type="button" data-days="${e.d}">${e.label}</button>`).join(``)}
        </div>
      </div>
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="adperf-body">
          ${_b(`hourglass_empty`,`Loading ad performance…`)}
        </div>
      </div>
    ...
    </div>`)}async function $y(){if(A()){let e=e=>Number(e||0).toLocaleString();return Wa(ub(),[{h:`Spend`,td:t=>t.ad?`$${e(t.ad.spend)}`:`—`},{h:`Clicks`,td:t=>t.ad?e(t.ad.clicks):`—`},{h:`Leads`,td:t=>e(t.leads.window??t.leads.total)},{h:`CPL`,td:t=>t.ad?.cost_per_lead==null?`—`:`$${e(t.ad.cost_per_lead)}`}],{emptyMsg:`No client orgs yet. Connect a client ad account to see spend here.`})}let e=document.getElementById(`adperf-periods`);e?.addEventListener(`click`,t=>{let n=t.target.closest(`button[data-days]`);n&&(yb=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>e.classList.toggle(`active`,Number(e.dataset.days)===yb)),eb())}),document.getElementById(`adperf-sources`)?.addEventListener(`click`,e=>{let t=e.target.closest(`tr[data-campaign]`);t&&(ro(oo.LEADS_SEARCH,t.dataset.campaign),!e.target.closest(`a`)&&(window.location.hash=`#/leads`))}),document.getElementById(`adperf-conv-scope`)?.addEventListener(`change`,e=>{bb=e.target.value||`all`,rb()}),document.getElementById(`adperf-lsa-card`)?.addEventListener(`click`,e=>{let t=e.target.closest(`tr[data-lsalead]`);t&&(ro(oo.LEADS_SEARCH,t.dataset.lsalead),!e.target.closest(`a`)&&(window.location.hash=`#/leads`))}),await eb()}async function eb(){let e=ub(),t=document.getElementById(`adperf-body`),n=document.getElementById(`adperf-range`),r=new Date,i=db(r),a=db(new Date(r.getTime()-(yb-1)*864e5));n&&(n.textContent=`${a} → ${i}`);let o,s=[],c=null,l=null,u=null,d=null;try{let[t,n,r,f,p,m]=await Promise.all([b(`${e}/m5t/v5/coreAdMetrics/overlay?from=${a}&to=${i}`),b(`${e}/m5t/v5/coreAdAccount`),b(`${e}/m5t/v5/coreAdMetrics/by-source?from=${a}&to=${i}`),nl().catch(()=>null),b(`${e}/m5t/v5/coreAdMetrics/conversions?from=${a}&to=${i}${nb()}`).then(e=>e.json()).catch(()=>null),b(`${e}/m5t/v5/coreAdMetrics/lsa?from=${a}&to=${i}`).then(e=>e.json()).catch(()=>null)]);j(t),o=await t.json(),s=((await n.json().catch(()=>({})))?.data||[]).filter(e=>e.status!==`DELETED`),c=await r.json().catch(()=>null),l=f,u=p,d=m}catch(e){console.warn(`[ad-performance] load failed:`,e),t&&(t.innerHTML=$a(e)?eo():_b(`error`,`Could not load ad performance. Please try again.`));return}tb(l,s),xb=o?.totals?.spend??null,ib(u,xb),ab(d);let f=o.totals||{};if(sb(`adperf-spend`,fb(f.spend)),sb(`adperf-clicks`,pb(f.clicks)),sb(`adperf-leads`,pb(f.magnet_leads)),sb(`adperf-cpl`,fb(f.cost_per_lead)),ob(c),!s.length){t.innerHTML=_b(`link_off`,`No ad account connected yet. Your agency can connect Google Ads or Local Services Ads to see performance here.`);return}if(!o.connected){t.innerHTML=_b(`cloud_sync`,`Account connected. No ad data for this range yet. Metrics appear here once they sync.`);return}let p=o.rows||[];t.innerHTML=cb(p),lb(p)}function tb(e,t){let n=document.getElementById(`adperf-conn`);if(!n)return;if(!e||e.denied){n.innerHTML=``;return}let r=Gc.google_ads,i=$c(e.providers,`google_ads`),a=el(r,i,!!e.configured?.google_ads),o=bl()?` <a href="#/settings/integrations/${r.urlSlug}">Manage</a>`:``;if(a===`connected`||a===`broken`){gl(),n.innerHTML=`<span>Google Ads</span> ${tl(a,i)}${o}`;return}if(t.length){n.innerHTML=`<span>Google Ads</span> <span>${t.length} associated account${t.length===1?``:`s`}</span>`;return}n.innerHTML=``}function nb(){return bb===`saved`?``:`&actions=${encodeURIComponent(bb)}`}async function rb(){let e=ub(),t=new Date,n=db(t);ib(await b(`${e}/m5t/v5/coreAdMetrics/conversions?from=${db(new Date(t.getTime()-(yb-1)*864e5))}&to=${n}${nb()}`).then(e=>e.json()).catch(()=>null),xb)}function ib(e,t){let n=document.getElementById(`adperf-conv-card`),r=document.getElementById(`adperf-conv-body`),i=document.getElementById(`adperf-stats`),a=document.getElementById(`adperf-conv-stat`);a&&a.remove();let o=document.getElementById(`adperf-conv-cpc`);if(o&&o.remove(),!n||!r)return;let s=e?.available||[],c=e?.saved_ids||[],l=e?.actions||[];if(!e?.ok||!s.length&&!c.length&&!l.length){n.style.display=`none`;return}let u=document.getElementById(`adperf-conv-scope`);if(u){let e=[{v:`all`,label:`All conversion actions`}];c.length&&e.push({v:`saved`,label:`Counted actions`});for(let t of s)e.push({v:String(t.id),label:t.name||String(t.id)});e.some(e=>e.v===bb)||(bb=`all`),u.innerHTML=e.map(e=>`<option value="${hb(e.v)}"${e.v===bb?` selected`:``}>${hb(e.label)}</option>`).join(``)}let d=Number(e.totals?.all_conversions??e.totals?.conversions??0);if(i){let e=document.createElement(`div`);e.innerHTML=M({icon:`flag`,label:`Ads Conversions`,value:pb(d),sub:`Reported by Google Ads`,href:`#/settings/integrations/google-ads`});let n=e.firstElementChild;n.id=`adperf-conv-stat`,i.appendChild(n);let r=t!=null&&d>0?Math.round(Number(t)/d*100)/100:null,a=document.createElement(`div`);a.innerHTML=M({icon:`request_quote`,label:`Cost / Conversion`,value:fb(r),sub:`Against the selected actions`,href:`#/analytics/conversions`});let o=a.firstElementChild;o.id=`adperf-conv-cpc`,i.appendChild(o)}if(n.style.display=``,!l.length){r.innerHTML=_b(`search_off`,`No conversions in this range for the selected actions.`);return}let f=l.some(e=>(e.all_value??e.value)>0);r.innerHTML=`
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
        ...
        <tbody>${l.map(e=>`
          <tr>
            <td>${hb(e.name||e.id)}</td>
            <td class="adperf-src-sub">${hb(String(e.category||``).toLowerCase().replace(/_/g,` `)||`—`)}</td>
            <td class="num"><b>${pb(e.all_conversions??e.conversions)}</b></td>
            <td class="num">${pb(e.conversions)}</td>
            ${f?`<td class="num">${fb(e.all_value??e.value)}</td>`:``}
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>
    <p class="adperf-note">Counted actions are chosen on the Google Ads integration page. All conv. counts every conversion action Google recorded, including actions outside the campaign bidding goals. These are Google's own numbers and may overlap magnet leads, so they are never added together.</p>`}function ab(e){let t=document.getElementById(`adperf-lsa-card`),n=document.getElementById(`adperf-lsa-body`);if(!t||!n)return;if(!e?.ok||!e.connected){t.style.display=`none`;return}t.style.display=``;let r=e.totals||{},i=e.leads||[],a=e.disputes||[],o=r.answer_rate==null?`No call outcomes yet`:r.missed>0?`${pb(r.missed)} missed call${r.missed===1?``:`s`}`:`Every call answered`;n.innerHTML=`
    ${`
    <div class="stats-grid" style="margin-bottom:16px">
      ${M({icon:`payments`,label:`LSA Spend`,value:fb(r.spend),sub:`Reported by Google`})}
      ${M({icon:`person_add`,label:`LSA Leads`,value:pb(r.leads),sub:`${pb(r.phone_calls)} calls · ${pb(r.messages)} messages`})}
      ${M({icon:`paid`,label:`Charged`,value:pb(r.charged),sub:`Leads Google billed for`})}
      ${M({icon:`request_quote`,label:`Cost / Charged Lead`,value:fb(r.cost_per_charged_lead)})}
      ${M({icon:`call`,label:`Answer Rate`,value:r.answer_rate==null?`—`:`${r.answer_rate}%`,sub:o})}
      ${M({icon:`event_available`,label:`Booked`,value:pb(r.booked),sub:`Google's lead status`})}
    </div>`}
    ${i.length?`
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
        ...
        <tbody>${i.map(e=>{let t=e.phone||e.name,n=e.captured_at?new Date(e.captured_at).toLocaleDateString([],{month:`short`,day:`numeric`}):`—`;return`
          <tr${t?` data-lsalead="${hb(t)}" class="adperf-srcrow--link"`:``}>
            <td>${t?`<a class="adperf-srclink" href="#/leads" title="Open this lead">${hb(e.name)}</a>`:hb(e.name)}</td>
            <td class="adperf-src-sub">${hb([e.phone,e.email].filter(Boolean).join(` · `)||`—`)}</td>
            <td>${hb(e.type||`—`)}${e.missed?` <span class="adperf-src-untracked">· missed</span>`:``}</td>
            <td class="adperf-src-sub">${hb(e.category||`—`)}</td>
            <td class="adperf-src-sub">${hb(String(e.lsa_status||`—`).toLowerCase())}${e.dispute?` <span class="adperf-src-untracked">· dispute candidate</span>`:``}</td>
            <td class="num">${e.charged?`<b>yes</b>`:`<span class="adperf-src-sub">no</span>`}</td>
            <td class="adperf-src-sub">${hb(n)}</td>
          </tr>`}).join(``)}
        </tbody>
      </table>
    </div>`:_b(`search_off`,`No Local Services leads in this range yet.`)}
    ${a.length?`
    <div class="card-header" style="padding:16px 0 0">
      ...
      <span class="live-indicator">${pb(a.length)} inside the 30 day credit window</span>
    </div>
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
        ...
        <tbody>${a.map(e=>`
          <tr>
            <td>${hb(e.name)}${e.phone?` <span class="adperf-src-sub">${hb(e.phone)}</span>`:``}</td>
            <td class="adperf-src-sub">${hb(e.reason)}</td>
            <td class="num"><b>${pb(e.days_left)}</b></td>
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>
    ...
    <p class="adperf-note">Numbers come from Google's Local Services feed. These leads land in your Leads inbox and do not flow through magnets.</p>`;let s=n.querySelector(`.adperf-srctable tbody`);s&&O(s,{rowSelector:`tr`,noun:`leads`,controlHost:n.querySelector(`.adperf-srcwrap`)||n})}function ob(e){let t=document.getElementById(`adperf-sources`),n=document.getElementById(`adperf-src-summary`);if(!t)return;let r=e&&Array.isArray(e.rows)?e.rows:[];if(!r.length){n&&(n.textContent=``),t.innerHTML=_b(`search_off`,`No leads in this range yet. Campaigns appear here as tagged leads (?utm_campaign=…) come in.`);return}let i=e.total_leads||r.reduce((e,t)=>e+(t.leads||0),0),a=e.tracked_leads||0;n&&(n.textContent=`${pb(a)} of ${pb(i)} leads tagged to a source`);let o=e=>e.campaign&&e.campaign!==`(none)`?hb(e.campaign):e.source&&e.source!==`(direct)`?hb(e.source):`<span class="adperf-src-untracked">Untracked / direct</span>`,s=e=>e.campaign&&e.campaign!==`(none)`?e.campaign:e.source&&e.source!==`(direct)`?e.source:``;t.innerHTML=`
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
        ...
        <tbody>${r.map(e=>{let t=s(e);return`
          <tr${t?` data-campaign="${hb(t)}" class="adperf-srcrow--link"`:``}>
            <td>${t?`<a class="adperf-srclink" href="#/leads" title="Show the leads from ${hb(t)}">${o(e)}</a>`:o(e)}</td>
            <td class="adperf-src-sub">${hb(e.source||`—`)}${e.medium?` · ${hb(e.medium)}`:``}</td>
            <td class="num">${e.paid_clicks?pb(e.paid_clicks):`<span class="adperf-src-sub">—</span>`}</td>
            <td class="num"><b>${pb(e.leads)}</b></td>
          </tr>`}).join(``)}
        </tbody>
      </table>
    </div>`;let c=t.querySelector(`.adperf-srctable tbody`);c&&O(c,{rowSelector:`tr`,noun:`sources`,controlHost:t.querySelector(`.adperf-srcwrap`)||t})}function sb(e,t){let n=document.getElementById(e);n&&(n.textContent=t)}function cb(e){return ho({rows:e,xLabel:e=>String(e.date||``).slice(5),left:{value:e=>e.spend||0,label:`Ad Spend`,fmt:e=>`$${bo(e)}`},right:{value:e=>e.magnet_leads||0,label:`Magnet Leads`},ariaLabel:`Ad spend versus magnet leads over time`,pointLabel:e=>`${e.date}: ${fb(e.spend||0)} spend, ${pb(e.magnet_leads||0)} leads`,hitData:e=>({day:e.date})})}function lb(e){let t=document.getElementById(`adperf-body`);return t?_o(t,{rows:e,tipHTML:e=>`
      <div class="m5c-tip__title">${hb(String(e.date||``))}</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--1"></i>${fb(e.spend||0)} spend</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--2"></i>${pb(e.magnet_leads||0)} leads</div>`}):()=>{}}var ub,db,fb,pb,mb,hb,gb,_b,vb,yb,bb,xb,Sb,Cb=o((()=>{T(),Qa(),Ol(),fo(),So(),An(),co(),x(),no(),ub=()=>w.getConfig?.().BASE_URL||``,db=e=>e.toISOString().slice(0,10),fb=e=>e==null?`—`:`$`+Number(e).toLocaleString(void 0,{minimumFractionDigits:0,maximumFractionDigits:2}),pb=e=>Number(e||0).toLocaleString(),mb={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},hb=e=>String(e??``).replace(/[&<>"']/g,e=>mb[e]),gb=!1,_b=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,vb=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],yb=30,bb=`all`,xb=null,Sb={render:Qy,init:$y}})),wb=c({default:()=>qb,init:()=>Ob,parseBookingCsv:()=>Bb,render:()=>Eb});function Tb(){if(Wb)return;let e=document.createElement(`style`);e.id=`m5t-adacct-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Wb=!0}function Eb(){return A()?Ha({section:`ad-accounts`,title:`Ad Accounts`,description:`Pick a client to see their connected ad accounts.`}):(Tb(),gl(),`
    ...
    <div class="adacct-wrap">
      <table class="adacct-table">
        ...
        <tbody>${r.map(e=>`
          <tr>
            <td>${P(Hb[e.provider]||e.provider)}</td>
            <td class="adacct-cid">${P(Ub(e.external_customer_id))}</td>
...
`&&a++,n.push(r),r=``,n.some(e=>e.trim()!==``)&&t.push(n),n=[]):r+=o}if(n.push(r),n.some(e=>e.trim()!==``)&&t.push(n),t.length<2)return[];let a=t[0].map(e=>e.trim().toLowerCase()),o=a.findIndex(e=>/e-?mail/.test(e)),s=a.findIndex(e=>/phone|mobile|tel/.test(e)),c=a.findIndex(e=>/date|time|booked|appointment|start/.test(e));return o<0&&s<0?[]:t.slice(1).map(e=>({email:o>=0?(e[o]||``).trim():``,phone:s>=0?(e[s]||``).trim():``,at:c>=0?(e[c]||``).trim():``})).filter(e=>e.email||e.phone).slice(0,5e3)}function Vb(e,t){let n=new Blob([e],{type:`text/csv;charset=utf-8`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.href=r,i.download=t,document.body.appendChild(i),i.click(),i.remove(),setTimeout(()=>URL.revokeObjectURL(r),4e3)}var Hb,Ub,Wb,Gb,Kb,qb,Jb=o((()=>{Qa(),An(),Ol(),x(),Hb={google_ads:`Google Ads`,google_lsa:`Local Services Ads`},Ub=e=>{let t=String(e||``).replace(/\D/g,``);return t.length===10?`${t.slice(0,3)}-${t.slice(3,6)}-${t.slice(6)}`:String(e||``)},Wb=!1,Gb=[],Kb=null,qb={render:Eb,init:Ob}})),Yb=c({default:()=>_x,init:()=>rx,openInviteModal:()=>sx,render:()=>nx});function Xb(e,t=`info`){try{ux.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}}function Zb(){let e=w.getActiveOrg();if(e&&e.channel===dx)return{guid:e.guid,name:e.name};let t=w.getMemberships().find(e=>e.channel===dx);return t?{guid:t.orgGuid,name:t.name}:null}function Qb(e){let t=e?.created_tz;if(!t)return``;let n=new Date(t);return Number.isNaN(n.getTime())?``:n.toLocaleDateString(void 0,{year:`numeric`,month:`short`,day:`numeric`})}function $b(){if(px)return;let e=document.createElement(`style`);e.id=`m5t-clients-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Hr(),px=!0}function ex(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function tx(e){let t=e.caption||e.guid,n=e.status||`ENABLED`;return`
    <div class="client-row">
      <span class="client-row__avatar">${Y(hx(t))}</span>
      <span class="client-row__main">
        <div class="client-row__name" title="${Y(t)}">${Y(t)}</div>
        <div class="client-row__meta">Added ${Y(Qb(e.created))||`—`}</div>
      </span>
    ...
    <div class="page-shell" data-section="clients">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="clients-body">
          ${ex(`hourglass_empty`,`Loading clients…`)}
        </div>
      </div>
    </div>`}async function rx(){let e=Zb();document.getElementById(`clients-add-btn`)?.addEventListener(`click`,()=>ax(e)),await ix(e)}async function ix(e){let t=document.getElementById(`clients-body`),n=document.getElementById(`clients-count`);if(t){if(!e?.guid){t.innerHTML=ex(`info`,`Client provisioning is available to reseller / agency accounts.`),n&&(n.textContent=``);return}try{let r=await b(`${w.getConfig?.().BASE_URL||``}/m5t/v5/acctGroup`);j(r);let i=await r.json(),a=(Array.isArray(i?.data)?i.data:[]).filter(t=>t.class===fx&&(!t.parent||t.parent===e.guid));if(!a.length){t.innerHTML=ex(`group_off`,`No clients yet. Add your first client to start managing magnets on their behalf.`),n&&(n.textContent=``);return}t.innerHTML=`<div class="client-list">${a.map(tx).join(``)}</div>`,O(t.querySelector(`.client-list`),{rowSelector:`.client-row`,noun:`clients`}),n&&(n.textContent=`${a.length} client${a.length===1?``:`s`}`),t.querySelectorAll(`.client-card__invite`).forEach(e=>{e.addEventListener(`click`,()=>{e.dataset.action===`adaccounts`?ox(e.dataset.clientGuid,e.dataset.clientName):e.dataset.action===`questionnaire`?lx(e.dataset.clientGuid,e.dataset.clientName):sx(e.dataset.clientGuid,e.dataset.clientName)})})}catch(e){console.warn(`[clients] load failed:`,e),t.innerHTML=$a(e)?eo():ex(`error`,`Couldn’t load clients. Please try again.`),n&&(n.textContent=``)}}}function ax(e){if(!e?.guid){Xb(`Only reseller / agency accounts can add clients.`,`error`);return}let t=document.createElement(`div`);t.className=`client-modal-scrim`;let n=document.createElement(`div`);n.className=`client-modal`,n.setAttribute(`role`,`dialog`),n.setAttribute(`aria-modal`,`true`),n.innerHTML=`
    ...
    <div class="client-modal__body" id="qm-body">${ex(`hourglass_empty`,`Loading…`)}</div>`,document.body.appendChild(r),document.body.appendChild(i),requestAnimationFrame(()=>{r.classList.add(`open`),i.classList.add(`open`)});let a=()=>{r.classList.remove(`open`),i.classList.remove(`open`),document.removeEventListener(`keydown`,o),setTimeout(()=>{r.remove(),i.remove()},200)},o=e=>{e.key===`Escape`&&a()};document.addEventListener(`keydown`,o),r.addEventListener(`click`,a),i.querySelector(`.client-modal__close`)?.addEventListener(`click`,a);let s=i.querySelector(`#qm-body`);function c(){s.innerHTML=`
      ...
      </div>`,s.querySelector(`[data-act="done"]`)?.addEventListener(`click`,a),s.querySelector(`[data-act="resend"]`)?.addEventListener(`click`,()=>c())}function d(e,t=!1){let r=e.editability||{editable:e.status===`SUBMITTED`&&!e.locked_tz,reason:e.locked_tz?`finalized`:null,deadline:null},i=!!r.editable,o=!i&&e.status===`SUBMITTED`?r.reason===`finalized`?`<div class="qm-status" data-status="LOCKED"><span class="material-symbols-outlined" aria-hidden="true">lock</span> Finalized${e.locked_tz?` ${Y(cx(e.locked_tz))}`:``}${e.locked_by_email?` by ${Y(e.locked_by_email)}`:``} — no further edits.</div>`:`<div class="qm-status" data-status="LOCKED"><span class="material-symbols-outlined" aria-hidden="true">lock_clock</span> Editing closed${r.deadline?` ${Y(cx(r.deadline))}`:``} — a response locks 14 days after it is submitted.</div>`:``,c=i&&r.deadline?`<p class="client-modal__sub qm-editable-hint">You can edit these answers until ${Y(cx(r.deadline))}.</p>`:``;s.innerHTML=`
      <div class="qm-status" data-status="SUBMITTED"><span class="material-symbols-outlined" aria-hidden="true">task_alt</span> Submitted ${e.submitted_tz?Y(cx(e.submitted_tz)):``}</div>
      ${o}
      ${Vr(e)}
      <form id="qm-form">
      ...
      </div>`,s.querySelector(`[data-act="done"]`)?.addEventListener(`click`,a),s.querySelector(`[data-act="cancel-edit"]`)?.addEventListener(`click`,()=>d(e,!1)),s.querySelector(`[data-act="edit"]`)?.addEventListener(`click`,()=>d(e,!0));let l=s.querySelector(`[data-act="save"]`);l?.addEventListener(`click`,async()=>{let t=Br(s);l.disabled=!0;let r=l.innerHTML;l.innerHTML=`<span class="material-symbols-outlined" aria-hidden="true">progress_activity</span> Saving…`;let i;try{i=await b(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e.guid)}`,{method:`PUT`,headers:{"Content-Type":`application/json`},body:JSON.stringify({answers:t})});let r=await i.json().catch(()=>({}));if(!i.ok||!r.ok)throw Error(r.error||r.message||`Failed (HTTP ${i.status})`);Xb(`Answers updated`,`success`);let a=await(await b(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e.guid)}`)).json().catch(()=>({}));d(a.ok?a.data:{...e,answers:t},!1)}catch(t){if(i&&i.status===423){Xb(t.message||`This questionnaire is now locked.`,`info`);let r=await(await b(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e.guid)}`)).json().catch(()=>({}));d(r.ok?r.data:e,!1);return}s.querySelector(`#qm-error`).textContent=t.message||`Something went wrong. Please try again.`,l.disabled=!1,l.innerHTML=r}})}try{let t=await b(`${n}/m5t/v5/m5mQuestionnaireResponse?owner_guid=${encodeURIComponent(e)}`),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`HTTP ${t.status}`);let i=(r.data||[])[0]||null;if(!i){c();return}if(i.status===`DRAFT`){l(i);return}if(i.status===`CANCELLED`){u(i);return}let a=await b(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(i.guid)}`),o=await a.json().catch(()=>({}));if(!a.ok||!o.ok)throw Error(o.error||`HTTP ${a.status}`);d(o.data)}catch(e){console.warn(`[clients] questionnaire load failed:`,e),s.innerHTML=`<p class="client-modal__error">Could not load questionnaire status. ${Y(e.message||``)}</p>`}}var ux,dx,fx,px,mx,Y,hx,gx,_x,vx=o((()=>{T(),rt(),ux=u(f(),1),m(),An(),Kr(),x(),no(),dx=`reseller_org`,fx=`managed_client`,px=!1,mx={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Y=e=>String(e??``).replace(/[&<>"']/g,e=>mx[e]),hx=e=>String(e||`?`).trim().split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``).toUpperCase()||`?`,gx=e=>String(e||``).replace(/[^0-9]/g,``),_x={render:nx,init:rx}})),yx=c({default:()=>Rx,init:()=>Ox,loadTeam:()=>kx,render:()=>Dx});function bx(){let e=w.getActiveOrg(),t=e&&e.channel!==Nx?e:null;if(!t){let e=w.getMemberships().find(e=>e.channel!==Nx);e&&(t={guid:e.orgGuid,channel:e.channel,memberRole:e.memberRole})}if(!t)return null;let n=w.getMemberships().find(e=>e.orgGuid===t.guid)?.name;return{...t,name:n}}function xx(e){return Object.values(Re).filter(t=>Le(e,t.value)).sort((e,t)=>t.rank-e.rank)}function Sx(){if(Px)return;let e=document.createElement(`style`);e.id=`m5t-team-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Px=!0}function Cx(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function wx(e,t,n){let r=e.member_role||`member`,i=e.entity_guid&&e.entity_guid===w.getEntity?.()?.guid,a=n&&!i&&xx(t.memberRole).some(e=>e.value===r),o=t.channel===`reseller_org`&&a&&r!==`owner`;return`
    <div class="team-row">
      <span class="team-avatar">${X(Lx(e.name||e.email))}</span>
    ...
    </div>`}async function Tx(e,t){let{uid:n,email:r}=e.dataset;if(window.confirm(`Revoke the invitation for ${r}?\n\nThe invitation link stops working immediately, including any copy already sent. You can invite this address again afterwards.`)){e.disabled=!0;try{let e=await b(`${w.getConfig?.().BASE_URL||``}/m5t/v5/acctGroup/${encodeURIComponent(t.guid)}/invites/${encodeURIComponent(n)}`,{method:`DELETE`}),i=await e.json().catch(()=>({}));if(!e.ok||!i.ok)throw Error(i.message||i.error||`HTTP ${e.status}`);Ix(`Invitation for ${r} revoked`,`success`)}catch(t){e.disabled=!1,Ix(t.message||`Could not revoke that invitation.`,`error`);return}try{await kx(t)}catch{}}}async function Ex(e,t){let{id:n,name:r}=e.dataset;if(window.confirm(`Remove ${r} from the team?\n\nThey lose access to this workspace immediately. Their account itself is not deleted, and you can invite them again later.`)){e.disabled=!0;try{let e=await b(`${w.getConfig?.().BASE_URL||``}/m5t/v5/acctGroupMember/${encodeURIComponent(n)}`,{method:`DELETE`}),t=await e.json().catch(()=>({}));if(!e.ok||!t.ok)throw Error(t.message||t.error||`HTTP ${e.status}`);Ix(`${r} removed from the team`,`success`)}catch(t){e.disabled=!1,Ix(t.message||`Could not remove that member.`,`error`);return}try{await kx(t)}catch{}}}function Dx(){return Sx(),`
    <div class="page-shell" data-section="team">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="team-body">
          ${Cx(`hourglass_empty`,`Loading team…`)}
        </div>
      </div>
    </div>`}async function Ox(){let e=bx(),t=document.getElementById(`team-invite-btn`);if(!e){t&&(t.style.display=`none`),document.getElementById(`team-body`).innerHTML=Cx(`info`,`Team management is available for agency and workspace accounts.`);return}t?.addEventListener(`click`,()=>Ax(e)),await kx(e)}async function kx(e){let t=document.getElementById(`team-body`),n=document.getElementById(`team-count`);if(t)try{let r=w.getConfig?.().BASE_URL||``,[i,a]=await Promise.all([b(`${r}/m5t/v5/acctGroup/${encodeURIComponent(e.guid)}/members`),b(`${r}/m5t/v5/acctGroup/${encodeURIComponent(e.guid)}/invites`)]),[o,s]=await Promise.all([i.json().catch(()=>({})),a.json().catch(()=>({}))]);j(i,o),j(a,s);let c=o.members||[],l=s.invites||[],u=xx(e.memberRole).length>0;t.innerHTML=(l.length?`
    ...
    <div class="team-modal__body" id="team-invite-body">
      ...
      <div class="team-modal__actions">
        ...
        </div>`;let l=r.querySelector(`#tm-link`);r.querySelector(`#tm-copy`)?.addEventListener(`click`,async()=>{try{await navigator.clipboard.writeText(l.value),r.querySelector(`#tm-copy`).textContent=`Copied!`}catch{l.select()}}),r.querySelector(`[data-act="done"]`)?.addEventListener(`click`,()=>{i(),kx(e)})}catch(e){console.warn(`[team] invite failed:`,e),o.textContent=e.message||`Something went wrong. Please try again.`,s.disabled=!1,s.innerHTML=l}})}async function jx(e,t,n){let r=w.getConfig?.().BASE_URL||``,i;try{let t=await b(`${r}/m5t/v5/acctGroupMember/${encodeURIComponent(e)}/client-scope`),n=await t.json().catch(()=>({}));j(t,n),i=n.data||{}}catch(e){console.warn(`[team] client-scope load failed:`,e),Ix(`Could not load client access. Please try again.`,`error`);return}if(!i.restrictable){Ix(`This member always sees all clients.`,`info`);return}let a=i.clients||[],o=new Set(i.allowed||[]),s=!!i.restricted,c=document.createElement(`div`);c.className=`team-modal-scrim`;let l=document.createElement(`div`);l.className=`team-modal`,l.setAttribute(`role`,`dialog`),l.setAttribute(`aria-modal`,`true`),l.innerHTML=`
    <div class="team-modal__head">
    ...
    </div>`,document.body.appendChild(c),document.body.appendChild(l),requestAnimationFrame(()=>{c.classList.add(`open`),l.classList.add(`open`)});let u=()=>{c.classList.remove(`open`),l.classList.remove(`open`),document.removeEventListener(`keydown`,d),setTimeout(()=>{c.remove(),l.remove()},200)},d=e=>{e.key===`Escape`&&u()};document.addEventListener(`keydown`,d),c.addEventListener(`click`,u),l.querySelector(`.team-modal__close`)?.addEventListener(`click`,u),l.querySelector(`[data-act="cancel"]`)?.addEventListener(`click`,u);let f=l.querySelector(`#tm-clients`);l.querySelectorAll(`input[name="tm-scope"]`).forEach(e=>e.addEventListener(`change`,()=>{f.classList.toggle(`open`,l.querySelector(`input[name="tm-scope"]:checked`)?.value===`assigned`)}));let p=l.querySelector(`#tm-scope-error`),m=l.querySelector(`[data-act="save"]`);m?.addEventListener(`click`,async()=>{p.textContent=``;let t=l.querySelector(`input[name="tm-scope"]:checked`)?.value,i=[...l.querySelectorAll(`input[data-client]:checked`)].map(e=>e.dataset.client);if(t===`assigned`&&!i.length){p.textContent=`Tick at least one client, or choose All clients.`;return}m.disabled=!0;try{let a=await b(`${r}/m5t/v5/acctGroupMember/${encodeURIComponent(e)}/client-scope`,{method:`PUT`,headers:{"Content-Type":`application/json`},body:JSON.stringify({client_guids:t===`assigned`?i:[]})}),o=await a.json().catch(()=>({}));if(j(a,o),!o.ok)throw Error(o.message||o.error||`Save failed`);Ix(`Client access updated.`,`success`),u(),kx(n)}catch(e){console.warn(`[team] client-scope save failed:`,e),p.textContent=e.message||`Something went wrong. Please try again.`,m.disabled=!1}})}var Mx,Nx,Px,Fx,X,Ix,Lx,Rx,zx=o((()=>{T(),Mx=u(f(),1),m(),rt(),An(),x(),no(),Nx=`managed_client`,Px=!1,Fx={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},X=e=>String(e??``).replace(/[&<>"']/g,e=>Fx[e]),Ix=(e,t=`info`)=>{try{Mx.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},Lx=e=>String(e||`?`).trim().split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``).toUpperCase()||`?`,Rx={render:Dx,init:Ox}}));m(),vt(),T(),dn(),y(),$n(),or(),St(),window.m5t=Object.create(null),window.m5t.pubsub=E.default,console.log(`StreamSyncEngage SaaS Client - Starting...`),console.log(`Environment:`,`production`),console.log(`Document readyState:`,document.readyState),console.log(`window.mvvLegit:`,typeof window.mvvLegit),console.log(`globalThis.mvvLegit:`,typeof globalThis.mvvLegit);try{console.log(`mvvLegit (direct):`,typeof mvvLegit)}catch(e){console.log(`mvvLegit (direct): ReferenceError -`,e.message)}var Bx=Object.keys(window).filter(e=>e.toLowerCase().includes(`mvv`)||e.toLowerCase().includes(`neodigm`)||e.toLowerCase().includes(`legit`));console.log(`Possible Neodigm globals:`,Bx);var Z=new ee.default(`/`,{hash:!0});window.router=Z;function Q(e){let t=e?`${e.replace(/-/g,`_`)}_route`:`splash_route`;return console.log(`[Router] Checking access: ${e} -> ${t}`),mvvLegit.isRouteAllowed(t)?!0:(console.warn(`[Router] Access denied to route: ${t}`),E.default.publish(p.UI_TOAST,{message:`Please sign in to access this page`,type:`warning`}),!1)}function $(e){console.log(`[Router] Navigating to:`,e),Z.navigate(`/${e}`)}var Vx=new Set([`splash`,`signin`,`signup`,`forgot`,`resethash`,`verf-link`,`accept-invite`,`signout`]),Hx={},Ux={},Wx=null,Gx={admin:{caption:`Platform Admin`,icon:`shield_person`},reseller:{caption:`Reseller`,icon:`handshake`},managed:{caption:`Managed`,icon:`visibility`},self_serve:{caption:`Self-Serve`,icon:`rocket_launch`}},Kx={free:`rocket_launch`,pro:`workspace_premium`,enterprise:`diamond`,grandfathered:`all_inclusive`},qx=null,Jx=!1;function Yx(){return qx||(Jx||(Jx=!0,C(async()=>{let{fetchMyPlan:e}=await Promise.resolve().then(()=>(pr(),sr));return{fetchMyPlan:e}},void 0,import.meta.url).then(({fetchMyPlan:e})=>e()).then(e=>{let t=e?.plan;!t?.label||!t?.key||(qx={caption:t.key===`grandfathered`?t.label:`${t.label} plan`,icon:Kx[t.key]||`rocket_launch`},Qx())}).catch(()=>{})),Gx.self_serve)}var Xx=`__portfolio__`;function Zx(){try{Z._setCurrent(null)}catch{}Z.resolve()}function Qx(){let e=document.getElementById(`m5t-side-nav`);if(!e)return;let t=!w.getScope()&&w.isAuthenticated(),n=w.getCapabilities(),r=w.getUiMode(),i=gn.getSideNav(n,{uiMode:r});Hx={},Ux={};let a=e=>{e.route&&(Hx[e.route]={sectionToken:e.token,childToken:null,children:e.children||null},Array.isArray(e.children)&&e.children.forEach(t=>{t.route&&(Ux[t.token]=t.route,Hx[t.route]={sectionToken:e.token,childToken:t.token,children:e.children})}))},o=e=>e.forEach(e=>{e.group&&Array.isArray(e.items)?o(e.items):a(e)});o(i);let s=w.getMemberships(),c=w.getActiveOrg(),l=w.getManagedClients(),u=w.getStoredActiveOrg(),d=l.length?[{guid:Xx,name:Cn(w.isClientScopeRestricted?.())},...s.map(e=>({guid:e.orgGuid,name:e.name||e.channel,channel:e.channel,group:`Your organization`})),...l.map(e=>({guid:e.orgGuid,name:e.name||e.orgGuid,channel:e.channel,group:`Clients`}))]:s.map(e=>({guid:e.orgGuid,name:e.name||e.channel,channel:e.channel}));e.setState({buttons:i,orgs:d,activeOrg:l.length?u||Xx:c?.guid||null,footerBadge:r===`self_serve`?Yx():Gx[r]||null,loading:t}),eS(),Wx&&tS(Wx)}var $x=e=>String(e??``).replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`})[e]);function eS(){let e=document.getElementById(`app-content`);if(!e)return;let t=document.getElementById(`org-context-banner`);t||(t=document.createElement(`div`),t.id=`org-context-banner`,e.insertBefore(t,e.firstChild),Wn(t));let n=w.getActiveOrg(),r=w.isPreviewingAsClient(),i=!!n?.viaSubtree&&!!Wx&&!Vx.has(Wx);if(t.hidden=!i,!i){t.innerHTML=``;return}let a=$x(n.name||`Client`);t.innerHTML=`
    <style>
