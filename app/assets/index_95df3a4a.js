...
<body>
  ...
  `,document.head.appendChild(e),af=!0}function ef(e){return`<div class="pv-hintwrap">
    ...
    </div>`,document.body.appendChild(n),requestAnimationFrame(()=>n.classList.add(`open`)),tf(n);let r=n.querySelector(`.pv-frame`),i=n.querySelector(`.pv-stage`),a=()=>{try{r.srcdoc=Qd(e())}catch{}};a();let o=()=>{n.classList.remove(`open`),setTimeout(()=>n.remove(),180)};return n.querySelector(`[data-x="close"]`).addEventListener(`click`,o),n.querySelector(`[data-x="refresh"]`).addEventListener(`click`,a),n.addEventListener(`click`,e=>{e.target===n&&o()}),n.querySelector(`.pv-seg`).addEventListener(`click`,e=>{let r=e.target.closest(`button[data-device]`);!r||r.dataset.device===t||(t=r.dataset.device,n.querySelectorAll(`.pv-seg button`).forEach(e=>e.classList.toggle(`is-on`,e.dataset.device===t)),i.dataset.device=t,a())}),{close:o,rebuild:a}}var af,of=o((()=>{Xd(),af=!1}));function sf(e){let t=[];for(let n of e.steps||[])for(let e of n.step_columns||[])e&&!t.includes(e)&&t.push(e);return t}function cf(e){let t=(e.steps||[])[0]||{},n=(t.step_prompts||[])[0]||``,r=``;return t.step_type===`single_select`||t.step_type===`multi_select`?r=(t.step_options||[]).slice(0,3).map(e=>`<span class="mag-tpl-pv__btn">${_f(e?.label)}</span>`).join(``):bf[t.step_type]?r=`<span class="mag-tpl-pv__input">${_f(bf[t.step_type])}</span>`:t.step_type===`scheduler`&&(r=`<span class="mag-tpl-pv__cal"><span class="material-symbols-outlined" aria-hidden="true">calendar_month</span>Pick a time</span>`),`<div class="mag-tpl-card__preview" aria-hidden="true"><span class="mag-tpl-pv__bubble">${_f(n)}</span>${r}</div>`}function lf(e,{detail:t=!1,gallery:n=!1,canUse:r=!0}={}){let i=t?sf(e):[],a=n?` mag-tpl-card--gallery`:``,o=n?` data-cat="${_f(e.category||``)}"`:``,s=n&&e.featured?`<span class="m5t-badge m5t-badge--featured m5t-featured-corner">Most popular</span>`:``,c=n?cf(e):``,l=n&&(e.tags||[]).length?`<div class="m5t-chip-row mag-tpl-card__tags">${e.tags.map(e=>`<span class="m5t-chip">${_f(e)}</span>`).join(``)}</div>`:``,u=r?`<button class="mag-tpl-card__btn" type="button" data-tpl="${e.id}">
    ...
    </div>`}function uf(e){ba(Ca.MAGNET_TEMPLATE,{id:e,at:Date.now()})}function df(e){if(!e)return null;if(typeof e==`string`)return yf.find(t=>t.id===e)||null;let{id:t,at:n}=e;return typeof n==`number`&&Date.now()-n>=xf?null:yf.find(e=>e.id===t)||null}function ff(){return df(xa(Ca.MAGNET_TEMPLATE))}function pf(){return df(Sa(Ca.MAGNET_TEMPLATE))}function mf(){if(Sf)return;let e=document.createElement(`style`);e.id=`m5t-mag-template-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Sf=!0}function hf(e){mf();let t=document.createElement(`div`);t.className=`mag-modal-scrim mag-tpl-modal`,t.innerHTML=`
...
`))}</textarea>`}return`<div class="mag-cta" data-type="${e}" data-kind="${n.kind}">
    ...
    ${o}
  </div>`}function Vf(){if(Gp)return;let e=document.createElement(`style`);e.id=`m5t-magnets-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Gp=!0}function Hf(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function Uf(e){let t=B.stats[e];if(!t||!t.opens&&!t.leads)return`<div class="mag-statline mag-statline--empty">&mdash;</div><div class="mag-cellsub">No activity yet</div>`;let n=e=>Number(e||0).toLocaleString();return`<div class="mag-statline" title="Trailing 30 days"><b>${n(t.opens)}</b> opens &middot; <b>${n(t.leads)}</b> leads</div>
    ${t.completion_rate==null?``:`<div class="mag-cellsub">${t.completion_rate}% complete the conversation</div>`}`}function Wf(e){let t=B.install[e.guid];if(!t)return``;let n=t.unrecognized||[],r=(t.traffic?.opens||0)>0,i=e.can_manage===!0,a=n.length?`
    ...
    ${s}
    ${l}
  </div>`}async function Gf(){await Promise.all(B.magnets.map(async e=>{try{let t=await(await fetch(`${U()}/m5t/v5/magnets/${encodeURIComponent(e.guid)}/install-activity`)).json().catch(()=>({}));if(!t.ok)return;B.install[e.guid]=t,pp(e)}catch(t){console.warn(`[magnets] install activity failed:`,e.guid,t)}}))}function Kf(e){return e?e.status===`REVOKED`||e.status===`PENDING`&&e.codeLive===!1:!1}function qf(e,t){let n=t.status===`PENDING`&&t.codeLive===!1,r=(n?sm.EXPIRED:sm[t.status])||{label:t.status,icon:`help`,tone:`off`},i=t.lastSeen?new Date(t.lastSeen).toLocaleString():``,a=t.host?V(t.host):n||t.status===`REVOKED`?`Code was never used`:`Waiting for a site to use this code`,o=[t.siteName?V(t.siteName):``,i?`last seen ${V(i)}`:``].filter(Boolean).join(` — `);return`
    <div class="mag-conn__row" data-conn="${V(t.guid)}" data-state="${r.tone}">
      ...
      ${e.can_manage===!0?Kf(t)?`<button class="mag-btn" data-action="clear-connection" data-guid="${V(e.guid)}" data-conn="${V(t.guid)}"
    ...
    </div>`}function Jf(e){let t=B.pairing[e.guid];if(!t)return``;if(t.error)return`<div class="mag-conn__code" data-state="error">
  ...
  </div>`}function Yf(e){let t=B.connections[e.guid];if(!t)return``;if(t.failed)return`<div class="mag-conn">
    ...
    </div>`;let n=t.connections||[],r=n.filter(t=>t.status!==`PENDING`||e.can_manage===!0),i=n.filter(e=>e.status===`ACTIVE`).length,a=Jf(e),o=r.filter(e=>!Kf(e)),s=r.filter(e=>Kf(e)),c=o.length?o.map(t=>qf(e,t)).join(``):`<p class="mag-install__hint">No sites connected yet. On WordPress, connecting a site pairs it without pasting the snippet by hand.</p>`,l=s.length?`
  ...
  </div>`}function Xf(e){let t=document.querySelector(`.mag-card[data-guid="${CSS.escape(e.guid)}"] .mag-conn-slot`);t&&(t.innerHTML=Yf(e))}async function Zf(e){let t=await(await fetch(`${U()}/m5t/v5/magnets/${encodeURIComponent(e.guid)}/connections`)).json().catch(()=>({}));if(!t.ok)throw Error(t.error||`Could not load connected sites`);return t}async function Qf(){await Promise.all(B.magnets.map(async e=>{try{B.connections[e.guid]=await Zf(e)}catch(t){console.warn(`[magnets] connections failed:`,e.guid,t),B.connections[e.guid]={failed:!0}}Xf(e)}))}function $f(){B.pairTimer&&(clearInterval(B.pairTimer),B.pairTimer=null)}function ep(){$f()}function tp(e){return!!document.querySelector(`.mag-card[data-guid="${CSS.escape(e)}"] .mag-conn-slot`)}function np(){if(B.pairTimer)return;let e=0;B.pairTimer=setInterval(async()=>{for(let e of Object.keys(B.pairing))tp(e)||delete B.pairing[e];let t=Object.keys(B.pairing);if(!t.length){$f();return}if(!t.some(e=>{let t=B.pairing[e];return!t.connected&&!t.error&&t.expiresAt-Date.now()>0})){t.forEach(e=>{let t=B.magnets.find(t=>t.guid===e);t&&Xf(t)}),$f();return}e+=1e3;let n=e>=om;n&&(e=0);for(let e of t){let t=B.magnets.find(t=>t.guid===e),r=B.pairing[e];if(!t||!r)continue;let i=r.expiresAt-Date.now()<=0;if(n&&!r.connected&&!r.error&&!i)try{let n=await Zf(t);B.connections[e]=n;let i=(n.connections||[]).find(e=>e.guid===r.connectionGuid&&e.status===`ACTIVE`);i&&(B.pairing[e]={connected:!0,connectionGuid:r.connectionGuid,connectedHost:i.host},H(`${i.host||`Your site`} connected`,`success`))}catch{}Xf(t)}},1e3)}async function rp(e){try{let t=await fetch(`${U()}/m5t/v5/magnets/${encodeURIComponent(e.guid)}/pairing-code`,{method:`POST`,headers:{"Content-Type":`application/json`},body:`{}`}),n=await t.json().catch(()=>({}));if(!t.ok||!n.ok){B.pairing[e.guid]={error:n.message||n.error||`Could not generate a pairing code`},Xf(e);return}B.pairing[e.guid]={code:n.pairingCode,connectionGuid:n.connectionGuid,expiresAt:new Date(n.expiresAt).getTime()};try{B.connections[e.guid]=await Zf(e)}catch{}Xf(e),np()}catch{B.pairing[e.guid]={error:`Could not reach the server. Try again.`},Xf(e)}}async function ip(e){let t=B.pairing[e.guid]?.code;if(t)try{await navigator.clipboard.writeText(t),H(`Pairing code copied`,`success`)}catch{H(`Copy failed — select the code and copy it manually`,`danger`)}}async function ap(e,t,n){if(window.confirm(`Clear the record for ${n||`this connection`}?\n\nIt is removed from this list for good. This only tidies up the history — the site is already disconnected, so nothing about access changes.`))try{let n=await fetch(`${U()}/m5t/v5/magnets/${encodeURIComponent(e.guid)}/connections/${encodeURIComponent(t)}`,{method:`DELETE`}),r=await n.json().catch(()=>({}));if(!n.ok||!r.ok){H(r.message||r.error||`Could not clear that record`,`danger`);return}H(`Record cleared`,`success`);try{B.connections[e.guid]=await Zf(e)}catch{}Xf(e)}catch{H(`Could not reach the server`,`danger`)}}async function op(e,t){let n=(B.connections[e.guid]?.connections||[]).find(e=>e.guid===t),r=n?.status===`PENDING`,i=r?`Cancel this pairing code?`:`Disconnect ${n?.host||`this site`}?`,a=r?`The code stops working immediately.`:`Its key stops working immediately and the plugin will show as disconnected. The domain stays on the allow-list — remove it separately if you want the magnet off that site.`;if(window.confirm(`${i}\n\n${a}`))try{let n=await fetch(`${U()}/m5t/v5/magnets/${encodeURIComponent(e.guid)}/connections/${encodeURIComponent(t)}/revoke`,{method:`POST`,headers:{"Content-Type":`application/json`},body:`{}`}),i=await n.json().catch(()=>({}));if(!n.ok||!i.ok){H(i.message||i.error||`Could not disconnect`,`danger`);return}B.pairing[e.guid]?.connectionGuid===t&&delete B.pairing[e.guid],H(r?`Pairing code cancelled`:`Site disconnected`,`success`);try{B.connections[e.guid]=await Zf(e)}catch{}Xf(e)}catch{H(`Could not reach the server`,`danger`)}}function sp(e){if(e.status===`DRAFT`)return`draft`;if(e.status!==`ENABLED`)return`off`;let t=B.install[e.guid];return t?(t.traffic?.opens||0)>0?`live`:(t.unrecognized||[]).length?`seen`:`quiet`:`unknown`}function cp(e){let t=sp(e),n=B.install[e.guid],r=``,i=``;if(t===`draft`)r=`Draft`,i=`Not live yet`;else if(t===`off`)r=`Disabled`,i=`Hidden from visitors`;else if(t===`live`){let e=n.traffic?.last_open_at?new Date(n.traffic.last_open_at):null,t=e&&e.toDateString()===new Date().toDateString(),a=e?t?e.toLocaleTimeString([],{hour:`numeric`,minute:`2-digit`}):e.toLocaleDateString():``;r=`Live`,i=a?`Last visitor ${V(a)}`:`Serving visitors`}else t===`seen`?(r=`Needs review`,i=`Unrecognised domain`):t===`quiet`&&(r=`Not seen`,i=`No visitors in ${Number(n.window_days||30)} days`);return r?`<div class="mag-status" data-live="${t}"><span class="mag-dot" data-live="${t}" aria-hidden="true"></span>${r}</div>
          ${i?`<div class="mag-cellsub">${i}</div>`:``}`:``}function lp(e){let t=B.owners.length>1||C.getMemberships().length>1,n=(e.domains||[]).filter(Boolean),r=e.can_manage===!0,i=B.detailOpen[e.guid]===!0,a=[Xp[e.display_mode]||`Chat bubble`,n.length?n.join(`, `):`No domains set. Add yours to go live.`,t&&e.owner_name?e.owner_name:``].filter(Boolean).join(` · `);return`
    <div class="mag-card${i?` is-open`:``}" data-guid="${V(e.guid)}">
      <div class="mag-line">
        ...
        <div class="mag-cell mag-cell--status">${cp(e)}</div>
        <div class="mag-cell mag-cell--stats">${Uf(e.guid)}</div>
      ...
      <div class="mag-detail" id="mag-detail-${V(e.guid)}" ${i?``:`hidden`}>
        ...
        <div class="mag-conn-slot">${Yf(e)}</div>
      </div>
    </div>`}function up(e,t){let n=typeof t==`boolean`?t:B.detailOpen[e]!==!0;B.detailOpen[e]=n;let r=document.querySelector(`.mag-card[data-guid="${CSS.escape(e)}"]`);if(!r)return;r.classList.toggle(`is-open`,n);let i=r.querySelector(`.mag-detail`);i&&(i.hidden=!n);let a=r.querySelector(`[data-action="install-toggle"][aria-controls]`);if(a){a.setAttribute(`aria-expanded`,String(n)),a.classList.toggle(`is-open`,n);let e=a.querySelector(`.material-symbols-outlined`);e&&(e.textContent=n?`expand_less`:`expand_more`)}}function dp(){cm||(cm=!0,document.addEventListener(`click`,e=>{e.target.closest?.(`.mag-more-wrap`)||fp()}),document.addEventListener(`keydown`,e=>{e.key===`Escape`&&fp()}))}function fp(e){document.querySelectorAll(`.mag-menu`).forEach(t=>{if(t===e)return;t.hidden=!0;let n=t.parentElement?.querySelector(`[data-action="menu"]`);n&&n.setAttribute(`aria-expanded`,`false`)})}function pp(e){let t=document.querySelector(`.mag-card[data-guid="${CSS.escape(e.guid)}"]`);if(!t)return;let n=t.querySelector(`.mag-install-slot`);n&&(n.innerHTML=Wf(e));let r=sp(e),i=t.querySelector(`.mag-cell--status`);i&&(i.innerHTML=cp(e)),(r===`quiet`||r===`seen`)&&B.detailOpen[e.guid]===void 0&&up(e.guid,!0)}function mp(){return k()?la({section:`magnets`,title:`Your Magnets`,description:`Pick a client to view and manage their magnets.`}):(Vf(),`
    ...
    </div>`)}async function hp(){if(k()){let e=pf();if(e){let t=document.querySelector(`.page-shell .page-header-text .page-description`);t&&(t.textContent=`Template picked: ${e.name}. Choose the client to build it for.`)}let t=e=>Number(e||0).toLocaleString();return da(U(),[{h:`Magnets`,td:e=>`${t(e.magnets.active)}<span style="color:var(--color-text-subtle,#6C7C9A)">/${t(e.magnets.total)}</span>`},{h:`Leads · wk`,td:e=>t(e.leads.this_week)},{h:`Leads · total`,td:e=>t(e.leads.total)}],{emptyMsg:`No client orgs yet. Create one under Clients, then provision its magnets here.`})}nm=null,rm=null,im=null,am=null,document.getElementById(`mag-new-btn`)?.addEventListener(`click`,Cp),document.getElementById(`mag-template-btn`)?.addEventListener(`click`,Tp),document.getElementById(`mag-body`)?.addEventListener(`click`,Ep),dp(),await _p();let e=ff();e&&B.canProvision&&wp(e)}function gp(e){e&&(e.innerHTML=`<div class="mag-grid">
    <div class="mag-head" aria-hidden="true"><span>Magnet</span><span>Status</span><span>Last 30 days</span><span></span></div>
    ${B.magnets.map(lp).join(``)}</div>`,E(e.querySelector(`.mag-grid`),{rowSelector:`.mag-card`,noun:`magnets`}))}async function _p(){let e=document.getElementById(`mag-body`);if(!e)return;try{let[e,t]=await Promise.all([fetch(`${U()}/m5t/v5/magnets`),fetch(`${U()}/m5t/v5/magnets/stats`).catch(()=>null)]),n=await e.json().catch(()=>({}));if(!n.ok)throw Error(n.message||`Failed to load`);B.magnets=n.magnets||[],B.owners=n.owners||[],B.canProvision=!!n.canProvision,B.canManage=!!n.canManage,B.primaryOrgGuid=n.primaryOrgGuid||null;let r=t?await t.json().catch(()=>({})):{};B.stats=r?.stats||{}}catch{e.innerHTML=Hf(`error`,`Could not load your magnets. Please try again.`);return}let t=document.getElementById(`mag-new-btn`);t&&(t.style.display=B.canProvision?``:`none`);let n=document.getElementById(`mag-template-btn`);if(n&&(n.style.display=B.canProvision?``:`none`),!B.magnets.length){e.innerHTML=Hf(`widgets`,B.canProvision?`No magnets yet. Click “New magnet” to create your first.`:`No magnets yet.`);return}gp(e),Gf(),Qf()}async function vp(e){let t={"Content-Type":`application/json`,"Idempotency-Key":crypto.randomUUID()},n={status:`DRAFT`};e&&(n.owner_guid=e);let r=await fetch(`${U()}/m5t/v5/magnets`,{method:`POST`,headers:t,body:JSON.stringify(n)}),i={};try{i=await r.json()}catch{}if(!r.ok||!i.ok)throw Error(i.message||`Could not start a new magnet (${r.status})`);return i.magnet}async function yp(e,t){let n=await fetch(`${U()}/m5t/v5/magnets/${encodeURIComponent(e)}`,{method:`PATCH`,headers:{"Content-Type":`application/json`},body:JSON.stringify({name:t.name,...t.appearance,steps:t.steps})}),r={};try{r=await n.json()}catch{}if(!n.ok||!r.ok)throw Error(r.message||`Could not apply the ${t.name} template (${n.status})`)}async function bp(e,t){let n=document.getElementById(`mag-new-btn`),r=document.getElementById(`mag-template-btn`);n&&(n.disabled=!0),r&&(r.disabled=!0);let i=null;try{let n=await vp(e);i=n.guid,t?(await yp(n.guid,t),await _p(),n=B.magnets.find(e=>e.guid===n.guid)||n):(B.magnets=[n,...B.magnets],gp(document.getElementById(`mag-body`))),Up(n,void 0,{freshDraft:!0})}catch(e){i&&fetch(`${U()}/m5t/v5/magnets/${encodeURIComponent(i)}`,{method:`DELETE`}).catch(()=>{}),H(e.message||`Could not start a new magnet`,`danger`)}finally{n&&(n.disabled=!1),r&&(r.disabled=!1)}}function xp(e){let t=document.createElement(`div`);t.className=`mag-modal-scrim`,t.innerHTML=`
    <div class="mag-modal" role="dialog" aria-modal="true" aria-label="Choose client" style="max-width:420px">
    ...
    </div>`,document.body.appendChild(t),requestAnimationFrame(()=>{t.classList.add(`open`),t.querySelector(`.mag-modal`).classList.add(`open`)});let n=()=>{t.classList.remove(`open`),setTimeout(()=>t.remove(),180),document.removeEventListener(`keydown`,r)};function r(e){e.key===`Escape`&&(e.preventDefault(),n())}document.addEventListener(`keydown`,r),t.querySelector(`.mag-modal__close`).addEventListener(`click`,n),t.querySelector(`[data-x="cancel"]`).addEventListener(`click`,n),t.querySelector(`[data-x="continue"]`).addEventListener(`click`,()=>{let r=t.querySelector(`#mag-owner-pick`).value;n(),bp(r,e)})}function Sp(){return B.owners.length===1?B.owners[0].guid:null}function Cp(){if(B.owners.length>1){xp();return}bp(Sp())}function wp(e){if(B.owners.length>1){xp(e);return}bp(Sp(),e)}function Tp(){hf(wp)}function Ep(e){let t=e.target.closest(`[data-action]`);if(!t){if(fp(),!e.target.closest(`a`)){let t=e.target.closest(`.mag-line`)?.closest(`.mag-card`);t?.dataset.guid&&up(t.dataset.guid)}return}let n=t.dataset.guid,r=B.magnets.find(e=>e.guid===n),i=t.dataset.action;if(i===`menu`){let e=t.parentElement?.querySelector(`.mag-menu`);if(!e)return;let n=e.hidden;fp(e),e.hidden=!n,t.setAttribute(`aria-expanded`,String(n));return}if(fp(),i===`install-toggle`)return up(n);if(i===`copy`)return Dp(n);if(r){if(i===`preview`)return rf(()=>Zd(r));if(i===`funnel`)return kp(r);if(i===`edit`)return Up(r);if(i===`flow`)return Up(r,`flow`);if(i===`allow-domain`)return Ap(r,t.dataset.domain);if(i===`dismiss-domain`)return jp(r,t.dataset.domain);if(i===`restore-domain`)return Mp(r,t.dataset.domain);if(i===`pair-start`)return rp(r);if(i===`copy-code`)return ip(r);if(i===`revoke-connection`)return op(r,t.dataset.conn);if(i===`clear-connection`)return ap(r,t.dataset.conn,t.dataset.label);if(i===`toggle`)return Pp(r);if(i===`delete`)return Fp(r)}}async function Dp(e){try{await navigator.clipboard.writeText(qp(e)),H(`Embed snippet copied`,`success`)}catch{H(`Copy failed`,`danger`)}}function Op(e){let t=Number(e?.total_steps)||0,n={};if((Array.isArray(e?.funnel)?e.funnel:[]).forEach(e=>{n[e.position]=Number(e.reached)||0}),!t||!Object.keys(n).length)return`<p class="mag-modal__hint" style="margin:0">No step activity yet in this window. As visitors answer questions, drop-off by step will appear here.</p>`;let r=Math.max(1,n[1]||0),i=[];for(let e=1;e<=t;e++){let a=n[e]||0,o=Math.min(100,Math.round(a/r*100));i.push(`
      ...
      </div>`)}return`<div class="mag-funnel">${i.join(``)}</div>`}async function kp(e){let t=document.createElement(`div`);t.className=`mag-modal-scrim`,t.innerHTML=`
    ...
    <div class="mag-modal" role="dialog" aria-modal="true" aria-label="${i?`Edit magnet`:`New magnet`}">
      ...
      <div class="mag-modal__main">
        ...
        <select class="mag-modal__select" id="mag-f-trigger">
          ${$p.map(t=>`<option value="${t.value}"${Tf(e)===t.value?` selected`:``}>${V(t.label)}</option>`).join(``)}
        ...
        <div class="mag-perdev" id="mag-f-perdev"${Tf(e)===`__per_device`?``:` hidden`}>
          <div class="mag-perdev__row">
            ...
            <select class="mag-modal__select" id="mag-f-trigger-desktop">
              ${$p.map(t=>`<option value="${t.value}"${Ef(e,`desktop`)===t.value?` selected`:``}>${V(t.label)}</option>`).join(``)}
            </select>
          </div>
          <div class="mag-perdev__row">
            <label class="mag-modal__label" for="mag-f-trigger-mobile">Mobile</label>
            <select class="mag-modal__select" id="mag-f-trigger-mobile">
              ${$p.map(t=>`<option value="${t.value}"${Ef(e,`mobile`)===t.value?` selected`:``}>${V(t.label)}</option>`).join(``)}
            </select>
          </div>
        ...
        <div class="mag-modal__sec">
          ...
          <select class="mag-modal__select" id="mag-f-font">${Hp(e?.font||``)}</select>
...
</details>`,Yp=e=>{let t=V(qp(e));return`<details class="mag-guide">
...
/&gt;</code></pre>
...
</details>`},Xp={chat:`Chat bubble`,right_slide:`Slide-in panel`,modal:`Modal`,inline:`Inline`},Zp=[{value:`inline`,icon:`view_agenda`,label:`Inline`},{value:`chat`,icon:`chat`,label:`Chat`},{value:`right_slide`,icon:`vertical_split`,label:`Slide`},{value:`modal`,icon:`web_asset`,label:`Modal`}],Qp=[{value:`auto`,icon:`brightness_auto`,label:`Auto`},{value:`sync`,icon:`sync`,label:`Sync`},{value:`light`,icon:`light_mode`,label:`Light`},{value:`dark`,icon:`dark_mode`,label:`Dark`}],$p=[{value:`on_load`,label:`Immediately on page load`},{value:`on_8_seconds`,label:`After 8 seconds`},{value:`on_scroll_80_percent`,label:`At 80% scroll depth`},{value:`on_exit_intent`,label:`On exit intent (mouse leaves the page)`},{value:`none`,label:`Manual only (visitor clicks the launcher)`}],em={book:{label:`Book`,icon:`event`,kind:`book`},chat:{label:`Chat`,icon:`chat`,kind:`flow`},call:{label:`Call`,icon:`phone`,kind:`value`,placeholder:`Phone number`},email:{label:`Email`,icon:`mail`,kind:`value`,placeholder:`Email address`},support:{label:`Support`,icon:`support_agent`,kind:`support`}},tm=[`book`,`chat`,`call`,`email`,`support`],nm=null,rm=null,im=null,am=null,om=5e3,sm={ACTIVE:{label:`Connected`,icon:`check_circle`,tone:`live`},PENDING:{label:`Waiting to connect`,icon:`hourglass_empty`,tone:`pending`},REVOKED:{label:`Disconnected`,icon:`link_off`,tone:`off`},EXPIRED:{label:`Expired`,icon:`timer_off`,tone:`off`}},cm=!1,lm=512,um=new Set([`image/png`,`image/jpeg`,`image/gif`,`image/webp`]),dm=8388608,fm=[{label:`Web fonts (loaded for you)`,opts:[[`Inter, sans-serif`,`Inter`],[`Poppins, sans-serif`,`Poppins`],[`Montserrat, sans-serif`,`Montserrat`],[`Roboto, sans-serif`,`Roboto`],[`"Open Sans", sans-serif`,`Open Sans`],[`Lato, sans-serif`,`Lato`],[`Nunito, sans-serif`,`Nunito`],[`Raleway, sans-serif`,`Raleway`],[`"Work Sans", sans-serif`,`Work Sans`],[`Oswald, sans-serif`,`Oswald`],[`Merriweather, serif`,`Merriweather (serif)`]]},{label:`System / always available`,opts:[[`system-ui, sans-serif`,`System UI`],[`Georgia, serif`,`Georgia (serif)`],[`"Times New Roman", Times, serif`,`Times (serif)`],[`Arial, Helvetica, sans-serif`,`Arial / Helvetica`],[`Verdana, Geneva, sans-serif`,`Verdana`],[`"Courier New", monospace`,`Courier (mono)`]]}]})),mm=c({default:()=>Tm,init:()=>vm,render:()=>_m});function hm(){let e=new Map;for(let t of yf)e.set(t.category,(e.get(t.category)||0)+1);return Object.keys(vf).filter(t=>e.has(t)).map(t=>[t,e.get(t)])}function gm(){if(mf(),wm)return;let e=document.createElement(`style`);e.id=`m5t-templates-page-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),wm=!0}function _m(){gm();let e=Sm();return`
    <div class="page-shell" data-section="templates">
      ...
      <div class="m5t-chip-row tpl-filters" id="tpl-filters" role="group" aria-label="Filter templates by category">
        ${Cm(`all`,`All`,yf.length,`true`)}
        ${hm().map(([e,t])=>Cm(e,vf[e],t,`false`)).join(``)}
      </div>

      <div class="mag-tpl-grid mag-tpl-grid--page" id="tpl-grid">
        ${yf.map(t=>lf(t,{detail:!0,gallery:!0,canUse:e})).join(``)}
      </div>
    </div>`}function vm(){let e=document.getElementById(`tpl-grid`);document.getElementById(`tpl-filters`)?.addEventListener(`click`,t=>{let n=t.target.closest(`[data-cat]`);if(!n)return;n.parentElement.querySelectorAll(`[data-cat]`).forEach(e=>{e.setAttribute(`aria-pressed`,String(e===n))});let r=n.dataset.cat;e?.querySelectorAll(`.mag-tpl-card`).forEach(e=>{e.hidden=r!==`all`&&e.dataset.cat!==r})}),e?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-preview]`);if(t){let e=yf.find(e=>e.id===t.dataset.preview);e&&rf(()=>Zd({...e.appearance,steps:e.steps,name:e.name}));return}let n=e.target.closest(`[data-tpl]`);n&&(uf(n.dataset.tpl),window.location.hash=`#/magnets`)})}var ym,bm,xm,Sm,Cm,wm,Tm,Em=o((()=>{w(),of(),Cf(),ym={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},bm=e=>String(e??``).replace(/[&<>"']/g,e=>ym[e]),xm=[`provision_magnets`,`manage_magnets`],Sm=()=>C.getCapabilities().some(e=>xm.includes(e)),Cm=(e,t,n,r)=>`<button class="m5t-chip--filter" type="button" data-cat="${bm(e)}" aria-pressed="${r}">${bm(t)}<span class="m5t-chip__count">${n}</span></button>`,wm=!1,Tm={render:_m,init:vm}})),Dm=c({default:()=>Qm,init:()=>Nm,render:()=>Mm});function Om(){if(lr(),Ym)return;let e=document.createElement(`style`);e.id=`m5t-questionnaires-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Ym=!0}function km(){return Jm.clients.map(e=>{let t=Jm.responses.filter(t=>t.owner_guid===e.guid),n=t[0]||null;return{guid:e.guid,name:e.caption||e.name||e.guid,latest:n,earlier:Math.max(0,t.length-1),status:n?n.status:`NOT_SENT`}})}function Am(){let e=Jm.search.trim().toLowerCase();return km().filter(t=>(Jm.filter===`ALL`||t.status===Jm.filter)&&(!e||t.name.toLowerCase().includes(e)))}function jm(e){let t=e.status===`SUBMITTED`,n=e.status===`SUBMITTED`?`<button class="qz-act" type="button" data-act="open" data-guid="${Hm(e.latest.guid)}">Read answers</button>`:e.status===`DRAFT`?`<button class="qz-act" type="button" data-act="copy" data-url="${Hm(e.latest.url||``)}">Copy link</button>`:`<a class="qz-act" href="#/clients">Go to client</a>`;return`<tr class="qz-row${t?` qz-row--open`:``}"${t?` data-open-guid="${Hm(e.latest.guid)}"`:``}>
    <td><span class="qz-client">${Hm(e.name)}</span>${e.earlier?`<span class="qz-earlier">${e.earlier} earlier</span>`:``}</td>
    <td><span class="qz-badge" data-s="${Hm(e.status)}">${Hm(qm[e.status]||e.status)}</span></td>
    <td class="qz-when">${Hm(Zm(e,`created_tz`))}</td>
    <td class="qz-when">${Hm(Zm(e,`submitted_tz`))}</td>
    <td class="qz-actcell">${n}</td>
  </tr>`}function Mm(){return Om(),`
    <div class="page-shell" data-section="questionnaires">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="qz-body">${Xm(`hourglass_empty`,`Loading…`)}</div>
      </div>
    </div>`}async function Nm(){await Pm()}async function Pm(){let e=document.getElementById(`qz-body`);if(e){try{let[e,t]=await Promise.all([fetch(`${Um()}/m5t/v5/acctGroup`),fetch(`${Um()}/m5t/v5/m5mQuestionnaireResponse`)]),n=await e.json().catch(()=>({})),r=await t.json().catch(()=>({}));if(!e.ok||!t.ok)throw Error(`load failed`);Jm.clients=(n.data||[]).filter(e=>e.class===Gm&&![`DELETED`,`ARCHIVED`].includes(e.status)).sort((e,t)=>String(e.caption||``).localeCompare(String(t.caption||``))),Jm.responses=r.data||[]}catch{e.innerHTML=Xm(`error`,`Could not load questionnaires. Please try again.`);return}Im()}}function Fm(){let e=document.getElementById(`qz-tbody`);e&&E(e,{rowSelector:`tr.qz-row`,noun:`clients`,controlHost:e.closest(`.qz-tablewrap`)||e.parentElement})}function Im(){let e=document.getElementById(`qz-body`);if(!e)return;if(!Jm.clients.length){e.innerHTML=Xm(`groups`,`No clients yet. Add a client first, then send them a kickoff questionnaire.`);return}let t=km(),n={ALL:t.length};Km.slice(1).forEach(([e])=>{n[e]=t.filter(t=>t.status===e).length});let r=Am();e.innerHTML=`
    <div class="qz-bar">
      <div class="qz-filter">
        ${Km.map(([e,t])=>`<button data-filter="${e}" class="${Jm.filter===e?`active`:``}">${Hm(t)}${n[e]?`<span class="qz-pillnum">${n[e]}</span>`:``}</button>`).join(``)}
      </div>
      <span class="qz-searchwrap">
        <span class="material-symbols-outlined" aria-hidden="true">search</span>
        <input class="qz-search" id="qz-search" type="search" placeholder="Search clients" value="${Hm(Jm.search)}" aria-label="Search clients" />
      </span>
    ...
    <div class="qz-tablewrap">
      <table class="qz-table">
        ...
        <tbody id="qz-tbody">${r.length?r.map(jm).join(``):`<tr><td colspan="5"><div class="qz-when" style="padding:14px 4px">${Jm.search.trim()?`No clients match your search.`:`No clients with status “${Hm(qm[Jm.filter]||Jm.filter)}”.`}</div></td></tr>`}</tbody>
      </table>
    </div>`,Fm(),e.querySelector(`.qz-filter`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-filter]`);t&&(Jm.filter=t.dataset.filter,Im())}),e.querySelector(`#qz-search`)?.addEventListener(`input`,e=>{Jm.search=e.target.value,Lm()}),e.querySelector(`#qz-tbody`)?.addEventListener(`click`,Rm)}function Lm(){let e=document.getElementById(`qz-tbody`);if(!e)return;let t=Am();e.innerHTML=t.length?t.map(jm).join(``):`<tr><td colspan="5"><div class="qz-when" style="padding:14px 4px">No clients match your search.</div></td></tr>`,Fm();let n=document.querySelector(`.qz-count`);n&&(n.textContent=`${t.length} of ${km().length}`)}function Rm(e){let t=e.target.closest(`[data-act="copy"]`);if(t){e.stopPropagation();let n=t.dataset.url;if(!n){Wm(`No link on this questionnaire yet.`,`warning`);return}navigator.clipboard?.writeText(n).then(()=>{t.textContent=`Copied!`,setTimeout(()=>{t.textContent=`Copy link`},1500)},()=>Wm(`Could not copy the link.`,`error`));return}let n=e.target.closest(`[data-act="open"]`),r=e.target.closest(`[data-open-guid]`),i=n?.dataset.guid||r?.dataset.openGuid;i&&zm(i)}async function zm(e){Jm.openGuid=e;let t=document.createElement(`div`);t.className=`qz-drawer-scrim`;let n=document.createElement(`aside`);n.className=`qz-drawer`,n.setAttribute(`role`,`dialog`),n.setAttribute(`aria-label`,`Questionnaire answers`),n.innerHTML=`
    ...
    <div class="qz-drawer__body" id="qz-dbody">${Xm(`hourglass_empty`,`Loading answers…`)}</div>`,document.body.append(t,n),requestAnimationFrame(()=>{t.classList.add(`open`),n.classList.add(`open`)});let r=()=>{Jm.openGuid=null,t.classList.remove(`open`),n.classList.remove(`open`),setTimeout(()=>{t.remove(),n.remove()},220),document.removeEventListener(`keydown`,i)};function i(e){e.key===`Escape`&&(e.preventDefault(),r())}document.addEventListener(`keydown`,i),t.addEventListener(`click`,r),n.querySelector(`.qz-drawer__close`).addEventListener(`click`,r);try{let t=await fetch(`${Um()}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e)}`),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`load failed`);let i=r.data||{},a=Jm.clients.find(e=>e.guid===i.owner_guid)?.caption||`Client`;n.querySelector(`#qz-dtitle`).textContent=a;let o=i.submitted_tz?`Submitted ${D(i.submitted_tz)}`:`Not yet submitted`;n.querySelector(`#qz-dsub`).textContent=i.locked_tz?`${o} · Finalized`:o,n.querySelector(`#qz-dbody`).innerHTML=cr(i)+ar(i,{emptyLabel:`Not answered`})}catch{n.querySelector(`#qz-dbody`).innerHTML=Xm(`error`,`Could not load these answers. Please try again.`)}}var Bm,Vm,Hm,Um,Wm,Gm,Km,qm,Jm,Ym,Xm,Zm,Qm,$m=o((()=>{w(),Bm=u(f(),1),m(),pr(),an(),Vm={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Hm=e=>String(e??``).replace(/[&<>"']/g,e=>Vm[e]),Um=()=>C.getConfig?.().BASE_URL||``,Wm=(e,t=`info`)=>{try{Bm.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},Gm=`managed_client`,Km=[[`ALL`,`All`],[`NOT_SENT`,`Not sent`],[`DRAFT`,`Awaiting response`],[`SUBMITTED`,`Submitted`],[`CANCELLED`,`Cancelled`]],qm=Object.fromEntries(Km),Jm={clients:[],responses:[],filter:`ALL`,search:``,openGuid:null},Ym=!1,Xm=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,Zm=(e,t)=>e?.latest?.[t]?D(e.latest[t]):`—`,Qm={render:Mm,init:Nm}})),eh=c({default:()=>Ph,init:()=>oh,render:()=>ah});function th(){if(lr(),Ah)return;let e=document.createElement(`style`);e.id=`m5t-deliverables-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Ah=!0}function nh(){let e=K.search.trim().toLowerCase();return K.rows.filter(t=>(K.filter===`ALL`||t.status===K.filter)&&(!e||String(t.owner_name||``).toLowerCase().includes(e)))}function rh(e){return e.status===`DRAFT`?`<button class="dz-act" type="button" data-act="open" data-guid="${W(e.guid)}">Edit</button>`:e.status===`SENT`?`<button class="dz-act" type="button" data-act="copy" data-url="${W(e.url||``)}">Copy link</button>`:e.status===`SUBMITTED`?`<button class="dz-act" type="button" data-act="open" data-guid="${W(e.guid)}">Read review</button>`:`<span class="dz-when">—</span>`}function ih(e){let t=e.status!==`CANCELLED`;return`<tr class="dz-row${t?` dz-row--open`:``}"${t?` data-open-guid="${W(e.guid)}"`:``}>
    ...
    <td><span class="dz-badge" data-s="${W(e.status)}">${W(kh[e.status]||e.status)}</span></td>
    <td class="dz-when">${e.sent_tz?W(D(e.sent_tz)):`—`}</td>
    <td class="dz-when">${e.submitted_tz?W(D(e.submitted_tz)):`—`}</td>
    <td class="dz-actcell">${rh(e)}</td>
    ...
    <div class="page-shell" data-section="deliverables">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="dz-body">${jh(`hourglass_empty`,`Loading…`)}</div>
      </div>
    </div>`}async function oh(){document.getElementById(`dz-new`)?.addEventListener(`click`,ph),await sh()}async function sh(){let e=document.getElementById(`dz-body`);if(e){try{let[e,t,n]=await Promise.all([fetch(`${Eh()}/m5t/v5/m5mDeliverable`),fetch(`${Eh()}/m5t/v5/acctGroup`),fetch(`${Eh()}/m5t/v5/m5mDeliverable/templates`)]),r=await e.json().catch(()=>({})),i=await t.json().catch(()=>({})),a=await n.json().catch(()=>({}));if(!e.ok||!t.ok||!n.ok)throw Error(`load failed`);K.rows=r.data||[],K.clients=(i.data||[]).filter(e=>e.class===Dh&&![`DELETED`,`ARCHIVED`].includes(e.status)).sort((e,t)=>String(e.caption||``).localeCompare(String(t.caption||``))),K.templates=a.data||[]}catch{e.innerHTML=jh(`error`,`Could not load deliverables. Please try again.`);return}lh()}}function ch(){let e=document.getElementById(`dz-tbody`);e&&E(e,{rowSelector:`tr.dz-row`,noun:`deliverables`,controlHost:e.closest(`.dz-tablewrap`)||e.parentElement})}function lh(){let e=document.getElementById(`dz-body`);if(!e)return;if(!K.clients.length&&!K.rows.length){e.innerHTML=jh(`groups`,`No clients yet. Add a client first, then author their first deliverable.`);return}if(!K.rows.length){e.innerHTML=jh(`approval`,`No deliverables yet. Author the first one with New deliverable.`);return}let t={ALL:K.rows.length};Oh.slice(1).forEach(([e])=>{t[e]=K.rows.filter(t=>t.status===e).length});let n=nh();e.innerHTML=`
    <div class="dz-bar">
      <div class="dz-filter">
        ${Oh.map(([e,n])=>`<button data-filter="${e}" class="${K.filter===e?`active`:``}">${W(n)}${t[e]?`<span class="dz-pillnum">${t[e]}</span>`:``}</button>`).join(``)}
      </div>
    ...
    <div class="dz-tablewrap">
      <table class="dz-table">
        ...
        <tbody id="dz-tbody">${n.length?n.map(ih).join(``):`<tr><td colspan="6"><div class="dz-when" style="padding:14px 4px">${K.search.trim()?`No deliverables match your search.`:`No deliverables with status “${W(kh[K.filter]||K.filter)}”.`}</div></td></tr>`}</tbody>
      </table>
    </div>`,ch(),e.querySelector(`.dz-filter`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-filter]`);t&&(K.filter=t.dataset.filter,lh())}),e.querySelector(`#dz-search`)?.addEventListener(`input`,e=>{K.search=e.target.value,uh()}),e.querySelector(`#dz-tbody`)?.addEventListener(`click`,dh)}function uh(){let e=document.getElementById(`dz-tbody`);if(!e)return;let t=nh();e.innerHTML=t.length?t.map(ih).join(``):`<tr><td colspan="6"><div class="dz-when" style="padding:14px 4px">No deliverables match your search.</div></td></tr>`,ch();let n=document.querySelector(`.dz-count`);n&&(n.textContent=`${t.length} of ${K.rows.length}`)}function dh(e){let t=e.target.closest(`[data-act="copy"]`);if(t){e.stopPropagation();let n=t.dataset.url;if(!n){G(`No link on this deliverable yet.`,`warning`);return}navigator.clipboard?.writeText(n).then(()=>{t.textContent=`Copied!`,setTimeout(()=>{t.textContent=`Copy link`},1500)},()=>G(`Could not copy the link.`,`error`));return}let n=e.target.closest(`[data-act="open"]`),r=e.target.closest(`[data-open-guid]`),i=n?.dataset.guid||r?.dataset.openGuid;i&&Ch(i)}function fh(e){let t=document.createElement(`div`);t.className=`dz-drawer-scrim`;let n=document.createElement(`aside`);n.className=`dz-drawer`,n.setAttribute(`role`,`dialog`),n.setAttribute(`aria-label`,e),n.innerHTML=`
    ...
    <div class="qm-qa" data-question-id="${W(e.id)}" data-question-type="${W(e.type)}">
      ...
      <div class="dz-cblock" data-group="${W(a)}" data-heading="${W(t)}"${r?` hidden`:``}>
        ...
        ${e.map(e=>mh(e,n,e.label.startsWith(i)?e.label.slice(i.length):e.label)).join(``)}
      </div>`}function gh(e,t){let n=(e.questions||[]).filter(e=>e.audience===`am`),r=n.filter(e=>!e.group).map(e=>mh(e,t)).join(``),i=Mr(e);if(i){let n=Nr(e).filter(e=>e.audience===`am`);if(!n.length)return r;let a=Ir(e,t);for(let e=1;e<=a;e++){let a=Pr(n,i,e);r+=hh(a,Nh(a,`${i}${e}`),t)}let o=Nh(Pr(n,i,a+1),`${i}${a+1}`);return r+=`<button class="dz-btn--ghost dz-addblock" type="button" data-repeat="${W(i)}" data-section-id="${W(e.id)}">+ Add ${W(o)}</button>`,r}let a=[...new Set(n.filter(e=>e.group).map(e=>e.group))];if(!a.length)return r;let o=null;return r+=a.map((e,r)=>{let i=n.filter(t=>t.group===e),a=Nh(i,e),s=r===0||Mh(i,t);return!s&&o===null&&(o={key:e,heading:a}),hh(i,a,t,{hidden:!s})}).join(``),r+=`<button class="dz-btn--ghost dz-addblock" type="button"${o?``:` hidden`}>${o?`+ Add ${W(o.heading)}`:``}</button>`,r}function _h(e,t){let n=(e?.sections||[]).filter(e=>(e.questions||[]).some(e=>e.audience===`am`)),r=n.length>1;return`
    ${r?`
    ...
    ${r?`
      ...
      </div>`:``}).join(``)}function bh(e,t){if(!t)return``;let n=(e?.sections||[]).flatMap(e=>(e.questions||[]).filter(e=>e.audience!==`am`)).map(e=>({q:e,val:ir(e,t.answers||{})})).filter(({val:e})=>e);return`
    ...
    </div>`}function xh(e,t){let n=Array.isArray(t)&&t.length?t[t.length-1]:null;return n?bh(e,n):``}function Sh(e,t){return(Array.isArray(t)?t.slice().reverse():[]).map(t=>bh(e,t)).join(``)}async function Ch(e){let{drawer:t,close:n}=fh(`Deliverable`),r=t.querySelector(`#dz-dbody`),i=t.querySelector(`#dz-dfoot`),a;try{let t=await fetch(`${Eh()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}`),n=await t.json().catch(()=>({}));if(!t.ok||!n.ok)throw Error(n.error||`load failed`);a=n.data||{}}catch{r.innerHTML=jh(`error`,`Could not load this deliverable. Please try again.`);return}let o=K.rows.find(t=>t.guid===e)?.owner_name||K.clients.find(e=>e.guid===a.owner_guid)?.caption||`Client`;t.querySelector(`#dz-dtitle`).textContent=`${o} · ${a.template?.name||`Deliverable`}`;let s=a.status===`DRAFT`||a.status===`SENT`,c=Number(a.revision)>1?` · Rev ${a.revision}`:``,l=a.status===`SUBMITTED`?`Returned ${a.submitted_tz?D(a.submitted_tz):``}${a.locked_tz?` · Finalized`:``}${c}`:a.status===`SENT`?`Sent ${a.sent_tz?D(a.sent_tz):``} to ${a.recipient_email||``} · awaiting approval${c}`:`${kh[a.status]||a.status}${c}`;if(t.querySelector(`#dz-dsub`).textContent=l,!s){r.innerHTML=cr(a)+yh(a.template,a.content||{},a.answers||{})+Sh(a.template,a.rounds),a.status===`SUBMITTED`&&!a.locked_tz?(i.innerHTML=`<button class="dz-btn" type="button" id="dz-revise">Revise plan</button>
        <span class="dz-note" style="margin:0">Archives this review and reopens authoring. The client approves the revised plan on a fresh link.</span>`,i.querySelector(`#dz-revise`).addEventListener(`click`,async t=>{let r=t.target;r.disabled=!0;try{let t=await fetch(`${Eh()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}/revise`,{method:`POST`}),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`revise failed`);G(`Reopened for revision. The last review is pinned above the form.`,`success`),n(),await sh(),Ch(e)}catch(e){G(e.message||`Could not revise.`,`error`),r.disabled=!1}})):i.innerHTML=``;return}r.innerHTML=`
    ${xh(a.template,a.rounds)}
    ${a.url?`
    ...
    <div id="dz-author-form">${_h(a.template,a.content||{})}</div>`,i.innerHTML=`
    ...
    <button class="dz-btn--ghost dz-btn--danger" type="button" id="dz-cancel">Cancel deliverable</button>`,r.querySelector(`#dz-copylink`)?.addEventListener(`click`,e=>{let t=r.querySelector(`.dz-linkrow input`);navigator.clipboard?.writeText(t?.value||``).then(()=>{e.target.textContent=`Copied!`,setTimeout(()=>{e.target.textContent=`Copy link`},1500)},()=>G(`Could not copy the link.`,`error`))});let u=async()=>{let t=sr(r.querySelector(`#dz-author-form`)),i=await fetch(`${Eh()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}`,{method:`PUT`,headers:{"Content-Type":`application/json`},body:JSON.stringify({content:t})}),a=await i.json().catch(()=>({}));if(!i.ok||!a.ok){if(i.status===409)return G(a.error||`This deliverable is no longer editable.`,`warning`),n(),await sh(),!1;throw Error(a.error||`save failed`)}return!0};vh(r.querySelector(`#dz-author-form`),a.template,()=>{u().catch(()=>G(`Autosave failed. Use Save draft.`,`warning`))}),i.querySelector(`#dz-preview`).addEventListener(`click`,async e=>{let t=e.target.closest(`button`);if(!a.url){G(`No link available for this deliverable.`,`warning`);return}t.disabled=!0;try{await u()&&window.open(a.url,`_blank`)}catch(e){G(e.message||`Could not save before previewing.`,`error`)}t.disabled=!1}),i.querySelector(`#dz-save`).addEventListener(`click`,async e=>{let t=e.target;t.disabled=!0;try{await u()&&G(`Draft saved.`,`success`)}catch(e){G(e.message||`Could not save.`,`error`)}t.disabled=!1}),i.querySelector(`#dz-send`).addEventListener(`click`,async t=>{let r=t.target,a=(i.querySelector(`#dz-email`)?.value||``).trim();if(!a){G(`Enter the recipient email first.`,`warning`);return}r.disabled=!0;try{if(!await u())return;let t=await fetch(`${Eh()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}/send`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({recipient_email:a})}),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`send failed`);G(r.data?.emailQueued?`Sent. ${a} will receive the review link.`:`Link created, but the email could not be queued. Copy the link and send it yourself.`,r.data?.emailQueued?`success`:`warning`),n(),await sh()}catch(e){G(e.message||`Could not send.`,`error`),r.disabled=!1}}),i.querySelector(`#dz-cancel`).addEventListener(`click`,async t=>{let r=t.target;r.disabled=!0;try{let t=await fetch(`${Eh()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}/cancel`,{method:`POST`}),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`cancel failed`);G(`Deliverable cancelled. Its link no longer works.`,`info`),n(),await sh()}catch(e){G(e.message||`Could not cancel.`,`error`),r.disabled=!1}})}var wh,Th,W,Eh,G,Dh,Oh,kh,K,Ah,jh,Mh,Nh,Ph,Fh=o((()=>{w(),wh=u(f(),1),m(),pr(),Br(),an(),Th={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},W=e=>String(e??``).replace(/[&<>"']/g,e=>Th[e]),Eh=()=>C.getConfig?.().BASE_URL||``,G=(e,t=`info`)=>{try{wh.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},Dh=`managed_client`,Oh=[[`ALL`,`All`],[`DRAFT`,`In progress`],[`SENT`,`Awaiting approval`],[`SUBMITTED`,`Returned`],[`CANCELLED`,`Cancelled`]],kh=Object.fromEntries(Oh),K={rows:[],clients:[],templates:[],filter:`ALL`,search:``},Ah=!1,jh=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,Mh=(e,t)=>e.some(e=>{let n=t?.[e.id];return Array.isArray(n)?n.length>0:!!(n&&String(n).trim())}),Nh=(e,t)=>{let n=String(e[0]?.label||``).match(/^(.+?):\s/);return n?n[1]:t.toUpperCase()},Ph={render:ah,init:oh}})),Ih=c({default:()=>Jh,init:()=>Hh,render:()=>Vh});function Lh(e){return e.status===`SENT`?{key:`PENDING`,label:`Pending your review`}:e.status===`SUBMITTED`&&e.signed?{key:`APPROVED`,label:`Approved`}:e.status===`SUBMITTED`?{key:`RETURNED`,label:`Returned with edits`}:null}function Rh(){if(lr(),Kh)return;let e=document.createElement(`style`);e.id=`m5t-deliverables-client-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Kh=!0}function zh(e,t){return!e.url&&t.key===`PENDING`?``:t.key===`PENDING`?`<button class="dc-act" type="button" data-url="${Wh(e.url)}">Review</button>`:t.key!==`APPROVED`&&e.url&&e.editability?.editable?`<button class="dc-act" type="button" data-url="${Wh(e.url)}">Update your review</button>`:``}function Bh(e){let t=Lh(e);return`<tr>
    <td><span class="dc-name">${Wh(e.template_name||e.kind||`Deliverable`)}</span>${Number(e.revision)>1?`<span class="dc-when" style="margin-left:6px">Rev ${Wh(e.revision)}</span>`:``}</td>
    <td><span class="dc-badge" data-s="${Wh(t.key)}">${Wh(t.label)}</span></td>
    <td class="dc-when">${e.sent_tz?Wh(D(e.sent_tz)):`—`}</td>
    <td class="dc-when">${e.submitted_tz?Wh(D(e.submitted_tz)):`—`}</td>
    <td class="dc-actcell">${zh(e,t)}</td>
  </tr>`}function Vh(){return Rh(),`
    <div class="page-shell" data-section="deliverables-client">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="dc-body">${qh(`hourglass_empty`,`Loading…`)}</div>
      </div>
    </div>`}async function Hh(){let e=document.getElementById(`dc-body`);if(!e)return;let t;try{let e=await fetch(`${Gh()}/m5t/v5/m5mDeliverable`),n=await e.json().catch(()=>({}));if(!e.ok||!n.ok)throw Error(`load failed`);t=(n.data||[]).filter(e=>Lh(e))}catch{e.innerHTML=qh(`error`,`Could not load your deliverables. Please try again.`);return}if(!t.length){e.innerHTML=qh(`approval`,`Nothing to review right now.`);return}e.innerHTML=`
    <div class="dc-tablewrap">
      <table class="dc-table">
        <thead><tr><th>Deliverable</th><th>Status</th><th>Received</th><th>Returned</th><th></th></tr></thead>
        <tbody id="dc-tbody">${t.map(Bh).join(``)}</tbody>
      </table>
    </div>`,e.querySelector(`#dc-tbody`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-url]`);t?.dataset.url&&window.open(t.dataset.url,`_blank`)})}var Uh,Wh,Gh,Kh,qh,Jh,Yh=o((()=>{w(),pr(),Uh={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Wh=e=>String(e??``).replace(/[&<>"']/g,e=>Uh[e]),Gh=()=>C.getConfig?.().BASE_URL||``,Kh=!1,qh=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,Jh={render:Vh,init:Hh}})),Xh=c({init:()=>dg,render:()=>ug});function Zh(e){let t=Ng(e);if(!t)return``;try{return new Date(t).toLocaleDateString([],{month:`short`,day:`numeric`,year:`numeric`})}catch{return``}}function Qh(e){let t=Ng(e);if(!t)return``;try{let n=Date.now()-new Date(t).getTime();if(n<0)return Zh(e);let r=n/36e5;return r<1?`${Math.max(1,Math.floor(n/6e4))}m ago`:r<24?`${Math.floor(r)}h ago`:r<48?`Yesterday`:Zh(e)}catch{return Zh(e)}}function $h(){if(Tg)return;let e=document.createElement(`style`);e.id=`m5t-leads-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Tg=!0}function eg(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function tg(e){return`<select class="leads-status" data-id="${J(e.id)}" data-act="status" onclick="event.stopPropagation()">
    ${Dg.map(t=>`<option value="${t}" ${e.status===t?`selected`:``}>${t[0]}${t.slice(1).toLowerCase()}</option>`).join(``)}
  </select>`}function ng(e){let t=q.magnetNames[e.magnet_guid]||e.lead_source||`—`,n=e.phone?String(e.phone).replace(/[^\d+]/g,``):``;return`
    <tr class="leads-row${String(e.id)===q.selectedId?` selected`:``}" data-id="${J(e.id)}" tabindex="0" role="button" aria-label="Open lead ${J(jg(e))}">
      <td>
        <span class="leads-who">
          <span class="leads-avatar">${J(Mg(e))}</span>
          <span><span class="leads-name">${J(jg(e))}</span>${e.company?`<span class="leads-sub">${J(e.company)}</span>`:``}</span>
        </span>
      </td>
      ...
      <td>${q.canManage?tg(e):`<span class="leads-badge" data-s="${J(e.status)}">${J(e.status)}</span>`}</td>
      <td class="num leads-muted" style="white-space:nowrap" title="${J(Zh(e))}">${Pg(e)?`<span class="leads-fresh" title="Captured in the last 24 hours"></span>`:``}${J(Qh(e))}</td>
      <td class="leads-open" aria-hidden="true"><span class="material-symbols-outlined">chevron_right</span></td>
    </tr>`}function rg(e){let t=q.search.trim().toLowerCase();if(!t)return!0;let n=[e.first_name,e.last_name,e.email,e.company,e.phone,e.product_interest,q.magnetNames[e.magnet_guid],q.book?q.ownerNames[e.owner_guid]:``,e.utm_campaign,e.utm_source].filter(Boolean).join(` `).toLowerCase();return t.split(/\s+/).every(e=>n.includes(e))}function ig(e){let t=Rg[q.range];if(!t)return!0;let n=Ng(e);if(!n)return!1;let r=new Date(Date.now()-(t-1)*864e5).toISOString().slice(0,10);return n.slice(0,10)>=r}function ag(){return q.leads.filter(e=>(q.filter===`ALL`||e.status===q.filter)&&Fg(e)&&Lg(e)&&rg(e)&&Ig(e)&&ig(e))}function og(){let e=ag(),t=Bg[q.sort.key];if(!t)return e;let n=q.sort.dir===`asc`?1:-1;return e.slice().sort((e,r)=>{let i=t.value(e),a=t.value(r);return i===``&&a===``?0:i===``?1:a===``?-1:i===a?0:i<a?-n:n})}function sg(e,t=``){let n=Bg[e],r=q.sort.key===e,i=r?q.sort.dir:``;return`<th class="leads-th--sort${t?` ${t}`:``}${r?` is-sorted`:``}"
    ...
    </th>`}function cg(e){return Bg[e]?(q.sort.key===e?q.sort.dir=q.sort.dir===`asc`?`desc`:`asc`:q.sort={key:e,dir:Bg[e].dir},q.shown=Eg(),!0):!1}function lg(){if(!q.day)return``;let e=q.day;try{e=new Date(`${q.day}T00:00:00Z`).toLocaleDateString([],{month:`short`,day:`numeric`,year:`numeric`,timeZone:`UTC`})}catch{}return`<button class="leads-daychip" type="button" id="leads-dayclear" title="Show every day again">
  ...
  </button>`}function ug(){$h(),ia();let e=k();return`
    <div class="page-shell" data-section="leads">
      ...
      <div class="dashboard-card" style="margin-bottom:24px">
        ...
        <div class="card-body" id="pf-picker-body">${eg(`hourglass_empty`,`Loading…`)}</div>
      </div>`:``}
      <div class="page-header">
        <div class="page-header-text">
      ...
      <div class="leads-layout">
        <div class="dashboard-card">
          <div class="card-body" id="leads-body">
            ${eg(`hourglass_empty`,`Loading leads…`)}
          </div>
        </div>
        <aside class="dashboard-card leads-panel" aria-label="Lead detail">
          <div class="card-body" id="leads-panel-body">${Vg()}</div>
        </aside>
      </div>
    </div>`}async function dg(){document.getElementById(`leads-export`)?.addEventListener(`click`,Cg);let e=xa(Ca.LEADS_SEARCH),t=xa(Ca.LEADS_DAY);if((e||t)&&(q.search=e||``,q.day=t||``,q.filter=`ALL`,q.clientFilter=`ALL`,q.magnetFilter=`ALL`,q.range=`ALL`,q.shown=Eg()),k()){let e=e=>Number(e||0).toLocaleString();da(Ag(),[{h:`Leads · wk`,td:t=>e(t.leads.this_week)},{h:`Leads · total`,td:t=>e(t.leads.total)},{h:`Compl.`,td:e=>e.completion_rate==null?`—`:`${e.completion_rate}%`}],{emptyMsg:`No client orgs yet. Create one under Clients first.`})}await pg(),fg()}async function fg(){let e=document.getElementById(`leads-crm-slot`);if(e)try{let{denied:t,providers:n,configured:r}=await _c();if(t)return;let i=Hg.map(e=>{let t=oc[e],i=mc(n,e);return{p:t,conn:i,status:hc(t,i,!!r?.[e])}}),a=i.filter(e=>e.status===`connected`),o=i.find(e=>e.status===`broken`);if(a.length){let t=a.map(e=>e.p.label).join(` and `),n=a.length===1&&a[0].conn?.external_account?.hub_domain||``;e.innerHTML=`
    ...
    <div class="leads-bar">
      ...
      <select class="leads-clientsel" id="leads-rangesel" aria-label="Filter by date captured">
        ${Object.keys(Rg).map(e=>`<option value="${e}" ${q.range===e?`selected`:``}>${J(zg[e])}</option>`).join(``)}
      </select>
      ${lg()}
    ...
    <div id="leads-tablezone"></div>`,e.querySelector(`.leads-filter`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-filter]`);t&&(q.filter=t.dataset.filter,q.shown=Eg(),mg())}),e.querySelector(`#leads-clientsel`)?.addEventListener(`change`,e=>{q.clientFilter=e.target.value,q.shown=Eg(),mg()}),e.querySelector(`#leads-magnetsel`)?.addEventListener(`change`,e=>{q.magnetFilter=e.target.value,q.shown=Eg(),mg()}),e.querySelector(`#leads-rangesel`)?.addEventListener(`change`,e=>{q.range=e.target.value,q.day=``,q.shown=Eg(),mg()}),e.querySelector(`#leads-dayclear`)?.addEventListener(`click`,()=>{q.day=``,q.shown=Eg(),mg()}),e.querySelector(`#leads-search`)?.addEventListener(`input`,e=>{q.search=e.target.value,q.shown=Eg(),hg()}),hg()}function hg(){let e=document.getElementById(`leads-tablezone`),t=document.getElementById(`leads-countlbl`);if(!e)return;let n=og(),r=n.slice(0,q.shown),i=q.book?8:7;t&&(t.textContent=`${n.length} of ${q.leads.length}`),e.innerHTML=`
    <div class="leads-tablewrap">
      <table class="leads-table${q.book?` leads-table--book`:``}">
        ${Ug()}
        <thead><tr>${sg(`name`)}${q.book?`<th>Client</th>`:``}${sg(`contact`)}<th>Interest</th><th>Magnet</th><th>Status</th>${sg(`captured`,`num`)}<th></th></tr></thead>
        <tbody id="leads-tbody">${r.map(ng).join(``)||`<tr><td colspan="${i}"><div class="leads-muted" style="padding:14px 4px">${q.search.trim()?`No leads match your search.`:`No ${q.filter.toLowerCase()} leads.`}</div></td></tr>`}</tbody>
      </table>
    </div>
    ${Qt({total:n.length,shown:q.shown,size:q.pageSize,noun:`leads`})}
    ${q.truncated?`<div class="leads-muted" style="padding:10px 4px">Showing your ${q.leads.length.toLocaleString()} most recent leads of ${q.total.toLocaleString()}. Search and filters cover these; older leads aren't included yet.</div>`:``}`,e.querySelector(`thead`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-sort]`);t&&cg(t.dataset.sort)&&hg()}),$t(e,{total:n.length,onSize:(e,t)=>{q.pageSize=t,q.shown=e,hg()}}),e.querySelector(`#leads-tbody`)?.addEventListener(`change`,gg),e.querySelector(`#leads-tbody`)?.addEventListener(`click`,e=>{if(e.target.closest(`.leads-status, a.leads-contactlink`))return;let t=e.target.closest(`.leads-row`);t&&_g(t.dataset.id)}),e.querySelector(`#leads-tbody`)?.addEventListener(`keydown`,e=>{if(e.key!==`Enter`&&e.key!==` `)return;let t=e.target.closest(`.leads-row`);t&&e.target===t&&(e.preventDefault(),_g(t.dataset.id))})}async function gg(e){let t=e.target.closest(`.leads-status`);if(!t)return;let n=t.dataset.id,r=q.leads.find(e=>String(e.id)===String(n)),i=t.value,a=r?.status;try{if(!(await(await fetch(`${Ag()}/m5t/v5/m5mLead/${encodeURIComponent(n)}`,{method:`PUT`,headers:{"Content-Type":`application/json`},body:JSON.stringify({status:i})})).json().catch(()=>({}))).ok)throw Error();r&&(r.status=i),kg(`Marked ${i[0]}${i.slice(1).toLowerCase()}`,`success`)}catch{t.value=a||`NEW`,kg(`Could not update status`,`danger`);return}try{String(n)===q.selectedId&&xg(),q.filter!==`ALL`&&mg()}catch(e){console.warn(`[leads] re-render after status change failed:`,e)}}function _g(e){q.selectedId=String(e),document.querySelectorAll(`.leads-row`).forEach(e=>e.classList.toggle(`selected`,e.dataset.id===q.selectedId)),xg()}function vg(e,t){let n=Array.isArray(e.tags)?e.tags:[];for(let e=n.length-1;e>=0;e--)if(n[e]&&n[e].tag===t&&n[e].value!=null)return String(n[e].value);return``}function yg(e){let t=vg(e,`appointment_start`);if(!t)return null;let n=vg(e,`appointment_status`)||`booked`,r=vg(e,`appointment_timezone`),i=vg(e,`appointment_event_type`),a=new Date(t),o=t;if(!Number.isNaN(a.getTime())){let e={weekday:`short`,month:`short`,day:`numeric`,hour:`numeric`,minute:`2-digit`};try{o=new Intl.DateTimeFormat(void 0,r?{...e,timeZone:r}:e).format(a)}catch{o=new Intl.DateTimeFormat(void 0,e).format(a)}}return{when:o,status:n,type:i,start:t}}function bg(e){let t=Array.isArray(e.tags)?e.tags:[],n=null;for(let e of t)if(!(!e||!e.tag)){if(e.tag===`callrail_call_id`){n={};continue}n&&e.tag.indexOf(`call_`)===0&&e.value!=null&&(n[e.tag]=String(e.value))}if(!n||!n.call_started)return null;let r=n.call_started,i=new Date(r),a=r;Number.isNaN(i.getTime())||(a=new Intl.DateTimeFormat(void 0,{weekday:`short`,month:`short`,day:`numeric`,hour:`numeric`,minute:`2-digit`}).format(i));let o=Number(n.call_duration)||0,s=o?o<60?`${o}s`:`${Math.floor(o/60)}m ${o%60}s`:``;return{when:a,dur:s,source:n.call_source||``,answered:n.call_answered||``,recording:n.call_recording_player||``,started:r}}function xg(){let e=document.getElementById(`leads-panel-body`);if(!e)return;let t=q.leads.find(e=>String(e.id)===q.selectedId),n=document.querySelector(`.leads-layout`);if(n&&(n.dataset.hasSelection=t?`true`:`false`),!t){e.innerHTML=Vg();return}let r=q.magnetNames[t.magnet_guid]||t.lead_source||`—`,i=yg(t),[a,o]=i?Wg[i.status]||Wg.booked:[``,``],s=bg(t),c=(e,t)=>t?`<div class="leads-det__field"><span class="leads-det__k">${J(e)}</span><span class="leads-det__v">${J(t)}</span></div>`:``;e.innerHTML=`
    <div class="leads-det__head">
      <span class="leads-avatar leads-det__ava">${J(Mg(t))}</span>
      <div>
        <div class="leads-det__name">${J(jg(t))}</div>
    ...
    ${q.canManage?`
      ...
      </div>`:``}`,e.querySelector(`[data-action="delete-lead"]`)?.addEventListener(`click`,Sg)}async function Sg(e){let t=e.currentTarget?.dataset?.id,n=q.leads.find(e=>String(e.id)===String(t));if(!n)return;let r=jg(n);if(!window.confirm(`Remove ${r} from your leads?\n\nThis takes the lead out of your dashboard, lists and exports. The record is kept in our system, and this address can be captured again if they come back through one of your magnets.`))return;let i=e.currentTarget;i.disabled=!0;try{let e=await fetch(`${Ag()}/m5t/v5/m5mLead/${encodeURIComponent(t)}`,{method:`DELETE`}),n=await e.json().catch(()=>({}));if(!e.ok||!n.ok)throw Error(n.error||`HTTP ${e.status}`);q.leads=q.leads.filter(e=>String(e.id)!==String(t)),q.selectedId=null,kg(`${r} removed`,`success`),mg()}catch{i.disabled=!1,kg(`Could not remove that lead. Please try again.`,`danger`)}}function Cg(){let e=og();if(!e.length){kg(`Nothing to export`,`info`);return}let t=[`first_name`,`last_name`,`email`,`phone`,`company`,`product_interest`,`lead_source`,`utm_source`,`utm_medium`,`utm_campaign`,`gclid`,`status`],n=e=>`"${String(e??``).replace(/"/g,`""`)}"`,r=[[...q.book?[`client`]:[],...t,`magnet`,`captured`,`appointment`,`last_call`].join(`,`)];for(let i of e){let e=yg(i),a=bg(i);r.push([...q.book?[n(q.ownerNames[i.owner_guid]||``)]:[],...t.map(e=>n(i[e])),n(q.magnetNames[i.magnet_guid]||i.lead_source||``),n(Zh(i)),n(e?`${e.start} (${e.status})`:``),n(a?`${a.started} (${a.answered===`no`?`missed`:`answered`})`:``)].join(`,`))}let i=new Blob([r.join(`
`)],{type:`text/csv`}),a=URL.createObjectURL(i),o=document.createElement(`a`);o.href=a,o.download=`leads-${q.filter.toLowerCase()}.csv`,document.body.appendChild(o),o.click(),o.remove(),URL.revokeObjectURL(a),kg(`Exported ${e.length} lead${e.length===1?``:`s`}`,`success`)}var wg,Tg,q,Eg,Dg,Og,J,kg,Ag,jg,Mg,Ng,Pg,Fg,Ig,Lg,Rg,zg,Bg,Vg,Hg,Ug,Wg,Gg=o((()=>{w(),wg=u(f(),1),m(),ya(),Ta(),an(),Hc(),Tg=!1,q={leads:[],magnetNames:{},filter:`ALL`,canManage:!1,book:!1,ownerNames:{},clientFilter:`ALL`,search:``,shown:25,selectedId:null,day:``,sort:{key:`captured`,dir:`desc`},magnetFilter:`ALL`,range:`ALL`,pageSize:25,total:0,truncated:!1},Eg=()=>q.pageSize===`ALL`?2**53-1:Number(q.pageSize)||25,Dg=[`NEW`,`WORKING`,`QUALIFIED`,`UNQUALIFIED`,`CONVERTED`,`EXPORTED`],Og={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},J=e=>String(e??``).replace(/[&<>"']/g,e=>Og[e]),kg=(e,t=`info`)=>{try{wg.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},Ag=()=>C.getConfig?.().BASE_URL||``,jg=e=>[e.first_name,e.last_name].filter(Boolean).join(` `).trim()||e.email||`(no name)`,Mg=e=>(jg(e).split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``)||`?`).toUpperCase(),Ng=e=>e.created?.created_tz||``,Pg=e=>{let t=Ng(e);if(!t)return!1;try{return Date.now()-new Date(t).getTime()<864e5}catch{return!1}},Fg=e=>q.clientFilter===`ALL`||e.owner_guid===q.clientFilter,Ig=e=>!q.day||Ng(e).slice(0,10)===q.day,Lg=e=>q.magnetFilter===`ALL`||e.magnet_guid===q.magnetFilter,Rg={ALL:null,7:7,30:30,90:90},zg={ALL:`All time`,7:`Last 7 days`,30:`Last 30 days`,90:`Last 90 days`},Bg={name:{label:`Name`,dir:`asc`,value:e=>jg(e).toLowerCase()},contact:{label:`Contact`,dir:`asc`,value:e=>String(e.email||e.phone||``).toLowerCase()},captured:{label:`Captured`,dir:`desc`,value:e=>Ng(e)}},Vg=()=>eg(`touch_app`,`Click to see lead information.`),Hg=[`hubspot`,`zoho`,`salesforce`,`gohighlevel`],Ug=()=>`
  <colgroup>
    <col style="width:22%">
  ...
  </colgroup>`,Wg={booked:[`QUALIFIED`,`Booked`],rescheduled:[`WORKING`,`Rescheduled`],canceled:[`CANCELED`,`Canceled`]}})),Kg=c({init:()=>t_,render:()=>Zg,tableHTML:()=>Xg});function qg(){if(d_)return;let e=document.createElement(`style`);e.id=`m5t-analytics-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),d_=!0}function Jg(e,t){let n=t>0?Math.round(e/t*100):0,r=`${a_(e)} of ${a_(t)} leads for the busiest magnet`;return`<svg class="anl-bar" width="72" height="8" viewBox="0 0 72 8" role="img"
      aria-label="${s_(r)}">
      <title>${s_(r)}</title>
    ...
    </svg>`}function Yg(e,t,n){let r=p_[e.status]?`<span class="anl-chip">${s_(p_[e.status])}</span>`:``,i=n&&e.owner_name?`<span class="anl-sub">${s_(e.owner_name)}</span>`:``,a=e.ab_tests&&(e.ab_tests.running||e.ab_tests.completed)?`${a_(e.ab_tests.running)} / ${a_(e.ab_tests.completed)}`:`—`;return`<tr data-guid="${s_(e.magnet_guid)}">
    <td><a class="anl-name" href="#/magnets">${s_(e.name)}</a>${r}${i}</td>
    <td>${Jg(Number(e.leads?.window||0),t)}</td>
    <td>${a_(e.leads?.window)}</td>
    <td>${a_(e.opens)}</td>
    <td>${c_(e.engagement_rate)}</td>
    <td>${c_(e.completion_rate)}</td>
    <td>${c_(e.lead_rate)}</td>
    <td>${a}</td>
  </tr>`}function Xg(e,t){let n=e.reduce((e,t)=>Math.max(e,Number(t.leads?.window||0)),0);return`<div class="anl-wrap"><table class="anl-table">
    <thead><tr>
      <th>Magnet</th><th></th><th>Leads</th><th>Opens</th>
      <th>Engaged</th><th>Completed</th><th>Leads / Open</th><th>A/B run / done</th>
    </tr></thead>
    <tbody>${e.map(e=>Yg(e,n,t)).join(``)}</tbody>
  </table></div>`}function Zg(){return k()?la({section:`analytics`,title:`Analytics`,description:`Pick a client to see how their magnets are performing.`}):(qg(),ja(),`
    <div class="page-shell" data-section="analytics">
      <div class="page-header">
        ...
        <div class="chart-controls" id="anl-periods">
          ${l_.map(e=>`<button class="chart-period-btn${e.d===u_?` active`:``}" type="button" data-days="${e.d}">${e.label}</button>`).join(``)}
        </div>
      </div>
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="anl-body">
          ${f_(`hourglass_empty`,`Loading magnet performance…`)}
        </div>
      </div>
    ...
    </div>`)}function Qg(e){try{return new Date(`${e}T00:00:00`).toLocaleDateString(void 0,{month:`short`,day:`numeric`,year:`numeric`})}catch{return String(e||``)}}function $g(e){return Ma({rows:e,xLabel:e=>String(e.date||``).slice(5),left:{value:e=>e.opens||0,label:`Magnet Opens`},right:{value:e=>e.leads||0,label:`Leads`},ariaLabel:`Magnet opens versus leads over time`,activatable:e=>Number(e.leads||0)>0,pointLabel:e=>`${Qg(e.date)}: ${a_(e.opens||0)} opens, ${a_(e.leads||0)} leads`,hitData:e=>({day:e.date,leads:Number(e.leads||0)})})}function e_(e){let t=document.getElementById(`anl-trend`);return t?Pa(t,{rows:e,tipHTML:e=>`
      <div class="m5c-tip__title">${Qg(e.date)}</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--1"></i>${a_(e.opens||0)} opens</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--2"></i>${a_(e.leads||0)} leads</div>
      ${Number(e.leads||0)>0?`<div class="m5c-tip__go">Open these leads →</div>`:``}`,onActivate:e=>{ba(Ca.LEADS_DAY,e.date),window.location.hash=`#/leads`}}):()=>{}}async function t_(){if(k())return da(r_(),[{h:`Leads`,td:e=>a_(e.leads.window??e.leads.total)},{h:`Opens`,td:e=>a_(e.opens)},{h:`Completed`,td:e=>c_(e.completion_rate)}]);qg(),ja(),await n_();let e=document.getElementById(`anl-periods`);e&&e.addEventListener(`click`,t=>{let n=t.target.closest(`.chart-period-btn`);n&&(u_=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>{e.classList.toggle(`active`,Number(e.dataset.days)===u_)}),n_())})}async function n_(){let e=document.getElementById(`anl-body`),t=document.getElementById(`anl-range`);if(!e)return;e.innerHTML=f_(`hourglass_empty`,`Loading magnet performance…`);let n=new Date,r=i_(n),i=i_(new Date(n.getTime()-(u_-1)*864e5));t&&(t.textContent=`${i} → ${r}`);let a;try{let e=await fetch(`${r_()}/m5t/v5/dashboard/stats?from=${i}&to=${r}&group_by=magnet`);if(a=await e.json(),!e.ok||!a?.ok)throw Error(a?.error||`HTTP ${e.status}`)}catch(t){e.innerHTML=f_(`cloud_off`,`Could not load analytics. Refresh to try again.`),console.error(`[analytics] load failed:`,t);return}let o=(e,t)=>{let n=document.getElementById(e);n&&(n.textContent=t)};o(`anl-leads`,a_(a.by_day?.reduce((e,t)=>e+Number(t.leads||0),0))),o(`anl-opens`,a_(a.funnel?.opens)),o(`anl-completion`,c_(a.funnel?.completion_rate)),o(`anl-active`,a_(a.magnets?.active));let s=Array.isArray(a.by_day)?a.by_day:[],c=document.getElementById(`anl-trend`);if(c){let e=s.some(e=>Number(e.opens||0)>0||Number(e.leads||0)>0);c.innerHTML=e?$g(s):f_(`show_chart`,`No activity in this range yet. Opens and leads chart here as visitors engage your magnets.`),e&&e_(s)}let l=Array.isArray(a.by_magnet)?a.by_magnet:[];if(!l.length){e.innerHTML=f_(`widgets`,`No magnets yet. Create one and its numbers will show up here.`);return}let u=l.some(e=>Number(e.opens||0)>0||Number(e.leads?.window||0)>0),d=new Set(l.map(e=>e.owner_guid)).size>1;e.innerHTML=(a.by_magnet_truncated?`<p class="anl-warn">Showing the top ${l.length} magnets by leads. You have more than that, so this list is not complete.</p>`:``)+(u?``:`<p class="anl-warn">No opens or leads for any magnet in this period. Try a longer range.</p>`)+Xg(l,d);let f=e.querySelector(`.anl-table tbody`);f&&E(f,{rowSelector:`tr`,noun:`magnets`,controlHost:e.querySelector(`.anl-wrap`)||e})}var r_,i_,a_,o_,s_,c_,l_,u_,d_,f_,p_,m_=o((()=>{w(),ya(),ka(),an(),za(),Ta(),r_=()=>C.getConfig?.().BASE_URL||``,i_=e=>e.toISOString().slice(0,10),a_=e=>Number(e||0).toLocaleString(),o_={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},s_=e=>String(e??``).replace(/[&<>"']/g,e=>o_[e]),c_=e=>e==null?`—`:`${e}%`,l_=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],u_=30,d_=!1,f_=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,p_={DRAFT:`Draft`,DISABLED:`Paused`}})),h_=c({default:()=>q_,init:()=>v_,render:()=>__});function g_(){if(ja(),W_)return;let e=document.createElement(`style`);e.id=`m5t-conversions-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),W_=!0}function __(){return k()?la({section:`analytics-conversions`,title:`Conversions`,description:`Pick a client to view their conversion reporting.`}):(g_(),Y.canSave=Fc(),`
    <div class="page-shell" data-section="analytics-conversions">
      <div class="page-header">
        ...
        <div class="chart-controls" id="conv-periods">
          ${H_.map(e=>`<button class="chart-period-btn${e.d===U_?` active`:``}" type="button" data-days="${e.d}">${e.label}</button>`).join(``)}
        </div>
      </div>
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="conv-selector">
          ${G_(`hourglass_empty`,`Loading conversion actions.`)}
        </div>
      </div>
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="conv-chart">
          ${G_(`hourglass_empty`,`Loading conversions.`)}
        </div>
      </div>
    ...
    </div>`)}async function v_(){if(k()){let e=e=>Number(e||0).toLocaleString();return da(j_(),[{h:`Spend`,td:t=>t.ad?`$${e(t.ad.spend)}`:N_},{h:`Clicks`,td:t=>t.ad?e(t.ad.clicks):N_},{h:`Leads`,td:t=>e(t.leads.window??t.leads.total)},{h:`CPL`,td:t=>t.ad?.cost_per_lead==null?N_:`$${e(t.ad.cost_per_lead)}`}],{emptyMsg:`No client orgs yet. Connect a client ad account to see conversions here.`})}let e=document.getElementById(`conv-periods`);e?.addEventListener(`click`,t=>{let n=t.target.closest(`button[data-days]`);n&&(U_=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>e.classList.toggle(`active`,Number(e.dataset.days)===U_)),y_())}),document.getElementById(`conv-selector`)?.addEventListener(`change`,e=>{let t=e.target.closest(`input[type="checkbox"][data-action-id]`);t&&(t.checked?Y.selection.add(t.value):Y.selection.delete(t.value),E_(),x_())}),await y_()}async function y_(){let e=new Date;Y.to=M_(e),Y.from=M_(new Date(e.getTime()-(U_-1)*864e5));let t=document.getElementById(`conv-range`);t&&(t.textContent=`${Y.from} → ${Y.to}`);let n=j_(),r=`from=${Y.from}&to=${Y.to}`,i=null,a=null,o=null,s=[],c=null;try{let e=Fc(),[t,l,u,d,f]=await Promise.all([fetch(`${n}/m5t/v5/coreAdMetrics/overlay?${r}`).then(e=>e.json()).catch(()=>null),fetch(`${n}/m5t/v5/coreAdMetrics/conversions?${r}`).then(e=>e.json()).catch(()=>null),fetch(`${n}/m5t/v5/coreAdMetrics/upload-ledger?${r}`).then(e=>e.json()).catch(()=>null),fetch(`${n}/m5t/v5/coreAdAccount`).then(e=>e.json()).catch(()=>null),e?fetch(`${n}/m5t/v5/integrations/google_ads/conversion_actions`).then(e=>e.json()).catch(()=>null):Promise.resolve(null)]);i=t,a=l,o=u,c=f,s=(d?.data||[]).filter(e=>e.status!==`DELETED`)}catch(e){console.warn(`[analytics-conversions] load failed:`,e),A_(`conv-chart`,G_(`error`,`Could not load conversions. Please try again.`));return}Y.spend=i?.totals?.spend??null,Y.overlayRows=i?.rows||[],Y.canSave=Fc();let l=s.filter(e=>e.provider===`google_ads`);Y.universe=b_({catalog:c,conv:a,accounts:l});let u=a?.has_selection&&Array.isArray(a.selected_ids)?a.selected_ids.map(String):[],d=Y.universe.filter(e=>e.magnet).map(e=>e.id);if(Y.selection=new Set(u.length?u:d),O_(o),T_({accounts:l}),E_(),!l.length){k_(`conv-spend`,P_(Y.spend)),A_(`conv-chart`,G_(`link_off`,`No ad account connected yet. Your agency can connect Google Ads to see conversions here.`)),A_(`conv-actions`,G_(`link_off`,`No ad account connected yet.`));return}u.length||!Y.selection.size?(Y.conv=a,S_()):await x_()}function b_({catalog:e,conv:t,accounts:n}){let r=new Map,i=(e,t)=>{let n=String(e||``);if(!n)return;let i=r.get(n)||{id:n,name:null,category:null,magnet:!1,kind:null};r.set(n,{...i,...Object.fromEntries(Object.entries(t).filter(([,e])=>e!=null&&e!==!1))})};(e?.data?.actions||[]).forEach(e=>i(e.id,{name:e.name,category:e.category})),(t?.actions||[]).forEach(e=>i(e.id,{name:e.name,category:e.category})),n.forEach(e=>{let t=e.account_config?.magnet_actions||{};Object.entries(t).forEach(([e,t])=>{t?.id&&i(t.id,{name:t.name,magnet:!0,kind:e})})}),r.forEach(e=>{!e.magnet&&e.name&&V_.has(e.name)&&(e.magnet=!0)});let a=[...r.values()],o=e=>e.magnet?z_.indexOf(e.kind):-1;return a.sort((e,t)=>{if(e.magnet!==t.magnet)return e.magnet?-1:1;if(e.magnet){let n=o(e),r=o(t);if(n!==r)return(n<0?99:n)-(r<0?99:r)}return String(e.name||e.id).localeCompare(String(t.name||t.id))})}async function x_(){let e=[...Y.selection],t=document.getElementById(`conv-chart`);t?.classList.add(`conv-busy`),Y.busy=!0;try{if(!e.length)Y.conv={ok:!0,has_selection:!1,rows:[],actions:[],totals:{conversions:0,value:0}};else{let t=j_(),n=await fetch(`${t}/m5t/v5/coreAdMetrics/conversions?from=${Y.from}&to=${Y.to}&actions=${encodeURIComponent(e.join(`,`))}`);Y.conv=await n.json()}S_()}catch(e){console.warn(`[analytics-conversions] recompute failed:`,e),A_(`conv-chart`,G_(`error`,`Could not recompute for that selection. Please try again.`))}finally{Y.busy=!1,t?.classList.remove(`conv-busy`)}}function S_(){let e=Y.conv||{},t=e.totals||{conversions:0,value:0},n=Number(t.conversions||0);k_(`conv-spend`,P_(Y.spend)),k_(`conv-count`,F_(n)),k_(`conv-cpc`,P_(R_(Y.spend,n))),k_(`conv-value`,P_(t.value??0)),C_(e.rows||[]),w_(e.actions||[])}function C_(e){let t=document.getElementById(`conv-chart`);if(!t)return;if(!Y.selection.size){t.innerHTML=G_(`checklist`,`No actions counted. Tick a conversion action to see it here.`),Y.rows=[];return}let n=new Map(e.map(e=>[String(e.date).slice(0,10),e])),r=Y.overlayRows.length?Y.overlayRows.map(e=>String(e.date).slice(0,10)):e.map(e=>String(e.date).slice(0,10)),i=new Map(Y.overlayRows.map(e=>[String(e.date).slice(0,10),Number(e.spend||0)])),a=r.map(e=>({date:e,spend:i.get(e)||0,conversions:Number(n.get(e)?.conversions||0)}));if(Y.rows=a,!a.length){t.innerHTML=G_(`search_off`,`No conversions in this range for the selected actions.`);return}t.innerHTML=Ma({rows:a,xLabel:e=>String(e.date||``).slice(5),left:{value:e=>e.spend||0,label:`Ad Spend`,fmt:e=>`$${La(e)}`},right:{value:e=>e.conversions||0,label:`Conversions`},ariaLabel:`Ad spend versus counted conversions over time`,pointLabel:e=>`${e.date}: ${P_(e.spend||0)} spend, ${F_(e.conversions||0)} conversions`,hitData:e=>({day:e.date})}),Pa(t,{rows:a,tipHTML:e=>`
      <div class="m5c-tip__title">${L_(String(e.date||``))}</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--1"></i>${P_(e.spend||0)} spend</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--2"></i>${F_(e.conversions||0)} conversions</div>`})}function w_(e){let t=document.getElementById(`conv-actions`);if(!t)return;let n=new Set(Y.universe.filter(e=>e.magnet).map(e=>e.id)),r=e.filter(e=>Y.selection.has(String(e.id)));if(!r.length){t.innerHTML=G_(`search_off`,Y.selection.size?`No conversions in this range for the selected actions.`:`No actions counted. Tick a conversion action to see it here.`);return}t.innerHTML=`
    <div class="conv-wrap">
      <table class="conv-table">
        <thead><tr><th>Conversion action</th><th>Category</th><th class="num">Conversions</th><th class="num">Value</th></tr></thead>
        <tbody>${r.map(e=>`
          <tr>
            <td><span class="conv-name">${L_(e.name||e.id)}</span>${n.has(String(e.id))?` <span class="conv-tag">Magnet</span>`:``}</td>
            <td class="sub">${L_(K_(e.category))}</td>
            <td class="num"><b>${F_(e.conversions)}</b></td>
            <td class="num">${P_(e.value??0)}</td>
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>`;let i=t.querySelector(`.conv-table tbody`);i&&E(i,{rowSelector:`tr`,noun:`actions`,controlHost:t.querySelector(`.conv-wrap`)||t})}function T_({accounts:e}){let t=document.getElementById(`conv-selector`);if(!t)return;if(!e.length){t.innerHTML=G_(`link_off`,`No ad account connected yet. Your agency can connect Google Ads to see conversions here.`);return}if(!Y.universe.length){t.innerHTML=`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">checklist</span>
    ...
    <label class="conv-row">
      <input type="checkbox" data-action-id="${L_(e.id)}" value="${L_(e.id)}"${Y.selection.has(e.id)?` checked`:``}>
      <span class="conv-name">${L_(e.name||e.id)}</span>
      ${e.magnet?`<span class="conv-tag">Magnet</span>`:``}
      ${e.category?`<span class="conv-cat">${L_(K_(e.category))}</span>`:``}
    </label>`;t.innerHTML=`
    ...
    ${Y.canSave?`
      ...
      </div>`:``}`;let a=t.querySelector(`#conv-list-other`);a&&E(a,{rowSelector:`.conv-row`,noun:`actions`}),document.getElementById(`conv-save`)?.addEventListener(`click`,D_)}function E_(){let e=document.getElementById(`conv-selcount`);if(!e)return;let t=Y.selection.size;e.textContent=t?`${t} action${t===1?``:`s`} counted`:`No actions counted`}async function D_(){let e=document.getElementById(`conv-save`),t=document.getElementById(`conv-savemsg`),n=[...Y.selection];e&&(e.disabled=!0),t&&(t.classList.remove(`err`),t.textContent=`Saving.`);try{let e=await(await fetch(`${j_()}/m5t/v5/integrations/google_ads/conversion_actions`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({conversion_action_ids:n})})).json().catch(()=>({}));if(!e.ok)throw Error(e.error||`save failed`);t&&(t.textContent=n.length?`Saved. ${n.length} action${n.length===1?``:`s`} counted by default.`:`Saved. No actions counted by default.`)}catch{t&&(t.classList.add(`err`),t.textContent=`Could not save the selection. Please try again.`)}finally{e&&(e.disabled=!1)}}function O_(e){let t=document.getElementById(`conv-ledger`);if(!t)return;let n=e?.kinds||[];if(!n.length){t.innerHTML=G_(`upload`,`Nothing fed into Google Ads in this range yet.`);return}let r=e=>B_[e]||String(e||``).replace(/_/g,` `),i=e=>{let t=z_.indexOf(e.kind);return t<0?99:t};t.innerHTML=`
    <div class="conv-wrap">
      <table class="conv-table" id="conv-ledger-table">
        ...
        <tbody>${[...n].sort((e,t)=>i(e)-i(t)).map(e=>`
          <tr data-kind="${L_(e.kind)}">
            <td><span class="conv-name">${L_(r(e.kind))}</span></td>
            <td class="num"><b>${F_(e.sent)}</b></td>
            <td class="num">${F_(e.queued)}</td>
            <td class="num">${F_(e.failed)}</td>
            <td class="num">${F_(e.skipped)}</td>
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>
    <p class="conv-note">What Magnet fed into Google Ads. Google's attributed numbers above are its own and are never added to these.</p>`}function k_(e,t){let n=document.getElementById(e);n&&(n.textContent=t)}function A_(e,t){let n=document.getElementById(e);n&&(n.innerHTML=t)}var j_,M_,N_,P_,F_,I_,L_,R_,z_,B_,V_,H_,U_,Y,W_,G_,K_,q_,J_=o((()=>{w(),ya(),Hc(),ka(),za(),an(),j_=()=>C.getConfig?.().BASE_URL||``,M_=e=>e.toISOString().slice(0,10),N_=`—`,P_=e=>e==null?N_:`$`+Number(e).toLocaleString(void 0,{minimumFractionDigits:0,maximumFractionDigits:2}),F_=e=>e==null?N_:Number(e).toLocaleString(),I_={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},L_=e=>String(e??``).replace(/[&<>"']/g,e=>I_[e]),R_=(e,t)=>{let n=Number(e),r=Number(t);return!Number.isFinite(n)||!Number.isFinite(r)||r<=0?null:Math.round(n/r*100)/100},z_=[`magnet_fill`,`qualified_lead`,`call_dni`,`calls_from_ads`],B_={magnet_fill:`Magnet fills`,qualified_lead:`Qualified leads`,call_dni:`Website calls`,calls_from_ads:`Calls from ads`},V_=new Set([`Magnet Lead`,`Magnet Qualified Lead`,`Magnet Phone Call (Website)`,`Calls from Ads (Magnet)`]),H_=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],U_=30,Y={from:null,to:null,spend:null,universe:[],selection:new Set,conv:null,rows:[],overlayRows:[],canSave:!1,busy:!1},W_=!1,G_=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,K_=e=>String(e||``).toLowerCase().replace(/_/g,` `)||N_,q_={render:__,init:v_}})),Y_=c({default:()=>gv,init:()=>Q_,render:()=>Z_});function X_(){if(ja(),fv)return;let e=document.createElement(`style`);e.id=`m5t-adperf-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),fv=!0}function Z_(){return k()?la({section:`ad-performance`,title:`Ad Performance`,description:`Pick a client to view their spend and lead performance.`}):(X_(),`
    <div class="page-shell" data-section="ad-performance">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="adperf-body">
          ${pv(`hourglass_empty`,`Loading ad performance…`)}
        </div>
      </div>
    ...
    </div>`)}async function Q_(){if(k()){let e=e=>Number(e||0).toLocaleString();return da(ov(),[{h:`Spend`,td:t=>t.ad?`$${e(t.ad.spend)}`:`—`},{h:`Clicks`,td:t=>t.ad?e(t.ad.clicks):`—`},{h:`Leads`,td:t=>e(t.leads.window??t.leads.total)},{h:`CPL`,td:t=>t.ad?.cost_per_lead==null?`—`:`$${e(t.ad.cost_per_lead)}`}],{emptyMsg:`No client orgs yet. Connect a client ad account to see spend here.`})}let e=document.getElementById(`adperf-periods`);e?.addEventListener(`click`,t=>{let n=t.target.closest(`button[data-days]`);n&&(hv=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>e.classList.toggle(`active`,Number(e.dataset.days)===hv)),$_())}),document.getElementById(`adperf-sources`)?.addEventListener(`click`,e=>{let t=e.target.closest(`tr[data-campaign]`);t&&(ba(Ca.LEADS_SEARCH,t.dataset.campaign),!e.target.closest(`a`)&&(window.location.hash=`#/leads`))}),await $_()}async function $_(){let e=ov(),t=document.getElementById(`adperf-body`),n=document.getElementById(`adperf-range`),r=new Date,i=sv(r),a=sv(new Date(r.getTime()-(hv-1)*864e5));n&&(n.textContent=`${a} → ${i}`);let o,s=[],c=null,l=null,u=null;try{let[t,n,r,d,f]=await Promise.all([fetch(`${e}/m5t/v5/coreAdMetrics/overlay?from=${a}&to=${i}`),fetch(`${e}/m5t/v5/coreAdAccount`),fetch(`${e}/m5t/v5/coreAdMetrics/by-source?from=${a}&to=${i}`),_c().catch(()=>null),fetch(`${e}/m5t/v5/coreAdMetrics/conversions?from=${a}&to=${i}`).then(e=>e.json()).catch(()=>null)]);if(!t.ok)throw Error(`HTTP ${t.status}`);o=await t.json(),s=((await n.json().catch(()=>({})))?.data||[]).filter(e=>e.status!==`DELETED`),c=await r.json().catch(()=>null),l=d,u=f}catch(e){console.warn(`[ad-performance] load failed:`,e),t&&(t.innerHTML=pv(`error`,`Could not load ad performance. Please try again.`));return}ev(l,s),tv(u,o?.totals?.spend??null);let d=o.totals||{};if(rv(`adperf-spend`,cv(d.spend)),rv(`adperf-clicks`,lv(d.clicks)),rv(`adperf-leads`,lv(d.magnet_leads)),rv(`adperf-cpl`,cv(d.cost_per_lead)),nv(c),!s.length){t.innerHTML=pv(`link_off`,`No ad account connected yet. Your agency can connect Google Ads or Local Services Ads to see performance here.`);return}if(!o.connected){t.innerHTML=pv(`cloud_sync`,`Account connected. No ad data for this range yet. Metrics appear here once they sync.`);return}let f=o.rows||[];t.innerHTML=iv(f),av(f)}function ev(e,t){let n=document.getElementById(`adperf-conn`);if(!n)return;if(!e||e.denied){n.innerHTML=``;return}let r=oc.google_ads,i=mc(e.providers,`google_ads`),a=hc(r,i,!!e.configured?.google_ads),o=Fc()?` <a href="#/settings/integrations/${r.urlSlug}">Manage</a>`:``;if(a===`connected`||a===`broken`){jc(),n.innerHTML=`<span>Google Ads</span> ${gc(a,i)}${o}`;return}if(t.length){n.innerHTML=`<span>Google Ads</span> <span>${t.length} associated account${t.length===1?``:`s`}</span>`;return}n.innerHTML=``}function tv(e,t){let n=document.getElementById(`adperf-conv-card`),r=document.getElementById(`adperf-conv-body`),i=document.getElementById(`adperf-stats`),a=document.getElementById(`adperf-conv-stat`);a&&a.remove();let o=document.getElementById(`adperf-conv-cpc`);if(o&&o.remove(),!n||!r)return;if(!e?.ok||!e.has_selection){n.style.display=`none`;return}if(i){let n=document.createElement(`div`);n.innerHTML=Ea({icon:`flag`,label:`Ads Conversions`,value:lv(e.totals?.conversions),sub:`Reported by Google Ads`,href:`#/settings/integrations/google-ads`});let r=n.firstElementChild;r.id=`adperf-conv-stat`,i.appendChild(r);let a=Number(e.totals?.conversions||0),o=t!=null&&a>0?Math.round(Number(t)/a*100)/100:null,s=document.createElement(`div`);s.innerHTML=Ea({icon:`request_quote`,label:`Cost / Conversion`,value:cv(o),sub:`Against the counted actions`,href:`#/analytics/conversions`});let c=s.firstElementChild;c.id=`adperf-conv-cpc`,i.appendChild(c)}let s=e.actions||[];if(n.style.display=``,!s.length){r.innerHTML=pv(`search_off`,`No conversions in this range for the selected actions.`);return}let c=s.some(e=>e.value>0);r.innerHTML=`
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
        ...
        <tbody>${s.map(e=>`
          <tr>
            <td>${dv(e.name||e.id)}</td>
            <td class="adperf-src-sub">${dv(String(e.category||``).toLowerCase().replace(/_/g,` `)||`—`)}</td>
            <td class="num"><b>${lv(e.conversions)}</b></td>
            ${c?`<td class="num">${cv(e.value)}</td>`:``}
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>
    <p class="adperf-note">Counted actions are chosen on the Google Ads integration page. These are Google's own numbers and may overlap magnet leads, so they are never added together.</p>`}function nv(e){let t=document.getElementById(`adperf-sources`),n=document.getElementById(`adperf-src-summary`);if(!t)return;let r=e&&Array.isArray(e.rows)?e.rows:[];if(!r.length){n&&(n.textContent=``),t.innerHTML=pv(`search_off`,`No leads in this range yet. Campaigns appear here as tagged leads (?utm_campaign=…) come in.`);return}let i=e.total_leads||r.reduce((e,t)=>e+(t.leads||0),0),a=e.tracked_leads||0;n&&(n.textContent=`${lv(a)} of ${lv(i)} leads tagged to a source`);let o=e=>e.campaign&&e.campaign!==`(none)`?dv(e.campaign):e.source&&e.source!==`(direct)`?dv(e.source):`<span class="adperf-src-untracked">Untracked / direct</span>`,s=e=>e.campaign&&e.campaign!==`(none)`?e.campaign:e.source&&e.source!==`(direct)`?e.source:``;t.innerHTML=`
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
        ...
        <tbody>${r.map(e=>{let t=s(e);return`
          <tr${t?` data-campaign="${dv(t)}" class="adperf-srcrow--link"`:``}>
            <td>${t?`<a class="adperf-srclink" href="#/leads" title="Show the leads from ${dv(t)}">${o(e)}</a>`:o(e)}</td>
            <td class="adperf-src-sub">${dv(e.source||`—`)}${e.medium?` · ${dv(e.medium)}`:``}</td>
            <td class="num">${e.paid_clicks?lv(e.paid_clicks):`<span class="adperf-src-sub">—</span>`}</td>
            <td class="num"><b>${lv(e.leads)}</b></td>
          </tr>`}).join(``)}
        </tbody>
      </table>
    </div>`;let c=t.querySelector(`.adperf-srctable tbody`);c&&E(c,{rowSelector:`tr`,noun:`sources`,controlHost:t.querySelector(`.adperf-srcwrap`)||t})}function rv(e,t){let n=document.getElementById(e);n&&(n.textContent=t)}function iv(e){return Ma({rows:e,xLabel:e=>String(e.date||``).slice(5),left:{value:e=>e.spend||0,label:`Ad Spend`,fmt:e=>`$${La(e)}`},right:{value:e=>e.magnet_leads||0,label:`Magnet Leads`},ariaLabel:`Ad spend versus magnet leads over time`,pointLabel:e=>`${e.date}: ${cv(e.spend||0)} spend, ${lv(e.magnet_leads||0)} leads`,hitData:e=>({day:e.date})})}function av(e){let t=document.getElementById(`adperf-body`);return t?Pa(t,{rows:e,tipHTML:e=>`
      <div class="m5c-tip__title">${dv(String(e.date||``))}</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--1"></i>${cv(e.spend||0)} spend</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--2"></i>${lv(e.magnet_leads||0)} leads</div>`}):()=>{}}var ov,sv,cv,lv,uv,dv,fv,pv,mv,hv,gv,_v=o((()=>{w(),ya(),Hc(),ka(),za(),an(),Ta(),ov=()=>C.getConfig?.().BASE_URL||``,sv=e=>e.toISOString().slice(0,10),cv=e=>e==null?`—`:`$`+Number(e).toLocaleString(void 0,{minimumFractionDigits:0,maximumFractionDigits:2}),lv=e=>Number(e||0).toLocaleString(),uv={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},dv=e=>String(e??``).replace(/[&<>"']/g,e=>uv[e]),fv=!1,pv=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,mv=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],hv=30,gv={render:Z_,init:Q_}})),vv=c({default:()=>zv,init:()=>xv,parseBookingCsv:()=>Mv,render:()=>bv});function yv(){if(Iv)return;let e=document.createElement(`style`);e.id=`m5t-adacct-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Iv=!0}function bv(){return k()?la({section:`ad-accounts`,title:`Ad Accounts`,description:`Pick a client to see their connected ad accounts.`}):(yv(),jc(),`
...
`&&a++,n.push(r),r=``,n.some(e=>e.trim()!==``)&&t.push(n),n=[]):r+=o}if(n.push(r),n.some(e=>e.trim()!==``)&&t.push(n),t.length<2)return[];let a=t[0].map(e=>e.trim().toLowerCase()),o=a.findIndex(e=>/e-?mail/.test(e)),s=a.findIndex(e=>/phone|mobile|tel/.test(e)),c=a.findIndex(e=>/date|time|booked|appointment|start/.test(e));return o<0&&s<0?[]:t.slice(1).map(e=>({email:o>=0?(e[o]||``).trim():``,phone:s>=0?(e[s]||``).trim():``,at:c>=0?(e[c]||``).trim():``})).filter(e=>e.email||e.phone).slice(0,5e3)}function Nv(e,t){let n=new Blob([e],{type:`text/csv;charset=utf-8`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.href=r,i.download=t,document.body.appendChild(i),i.click(),i.remove(),setTimeout(()=>URL.revokeObjectURL(r),4e3)}var Pv,Fv,Iv,Lv,Rv,zv,Bv=o((()=>{ya(),an(),Hc(),Pv={google_ads:`Google Ads`,google_lsa:`Local Services Ads`},Fv=e=>{let t=String(e||``).replace(/\D/g,``);return t.length===10?`${t.slice(0,3)}-${t.slice(3,6)}-${t.slice(6)}`:String(e||``)},Iv=!1,Lv=[],Rv=null,zv={render:bv,init:xv}})),Vv=c({default:()=>ly,init:()=>Yv,openInviteModal:()=>$v,render:()=>Jv});function Hv(e,t=`info`){try{ny.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}}function Uv(){let e=C.getActiveOrg();if(e&&e.channel===ry)return{guid:e.guid,name:e.name};let t=C.getMemberships().find(e=>e.channel===ry);return t?{guid:t.orgGuid,name:t.name}:null}function Wv(e){let t=e?.created_tz;if(!t)return``;let n=new Date(t);return Number.isNaN(n.getTime())?``:n.toLocaleDateString(void 0,{year:`numeric`,month:`short`,day:`numeric`})}function Gv(){if(ay)return;let e=document.createElement(`style`);e.id=`m5t-clients-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),lr(),ay=!0}function Kv(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function qv(e){let t=e.caption||e.guid,n=e.status||`ENABLED`;return`
    <div class="client-row">
      <span class="client-row__avatar">${X(sy(t))}</span>
      <span class="client-row__main">
        <div class="client-row__name" title="${X(t)}">${X(t)}</div>
        <div class="client-row__meta">Added ${X(Wv(e.created))||`—`}</div>
      </span>
    ...
    </div>`}async function Yv(){let e=Uv();document.getElementById(`clients-add-btn`)?.addEventListener(`click`,()=>Zv(e)),await Xv(e)}async function Xv(e){let t=document.getElementById(`clients-body`),n=document.getElementById(`clients-count`);if(t){if(!e?.guid){t.innerHTML=Kv(`info`,`Client provisioning is available to reseller / agency accounts.`),n&&(n.textContent=``);return}try{let r=C.getConfig?.().BASE_URL||``,i=await fetch(`${r}/m5t/v5/acctGroup`);if(!i.ok)throw Error(`HTTP ${i.status}`);let a=await i.json(),o=(Array.isArray(a?.data)?a.data:[]).filter(t=>t.class===iy&&(!t.parent||t.parent===e.guid));if(!o.length){t.innerHTML=Kv(`group_off`,`No clients yet. Add your first client to start managing magnets on their behalf.`),n&&(n.textContent=``);return}t.innerHTML=`<div class="client-list">${o.map(qv).join(``)}</div>`,E(t.querySelector(`.client-list`),{rowSelector:`.client-row`,noun:`clients`}),n&&(n.textContent=`${o.length} client${o.length===1?``:`s`}`),t.querySelectorAll(`.client-card__invite`).forEach(e=>{e.addEventListener(`click`,()=>{e.dataset.action===`adaccounts`?Qv(e.dataset.clientGuid,e.dataset.clientName):e.dataset.action===`questionnaire`?ty(e.dataset.clientGuid,e.dataset.clientName):$v(e.dataset.clientGuid,e.dataset.clientName)})})}catch(e){console.warn(`[clients] load failed:`,e),t.innerHTML=Kv(`error`,`Couldn’t load clients. Please try again.`),n&&(n.textContent=``)}}}function Zv(e){if(!e?.guid){Hv(`Only reseller / agency accounts can add clients.`,`error`);return}let t=document.createElement(`div`);t.className=`client-modal-scrim`;let n=document.createElement(`div`);n.className=`client-modal`,n.setAttribute(`role`,`dialog`),n.setAttribute(`aria-modal`,`true`),n.innerHTML=`
    ...
    <div class="client-modal__body" id="qm-body">${Kv(`hourglass_empty`,`Loading…`)}</div>`,document.body.appendChild(r),document.body.appendChild(i),requestAnimationFrame(()=>{r.classList.add(`open`),i.classList.add(`open`)});let a=()=>{r.classList.remove(`open`),i.classList.remove(`open`),document.removeEventListener(`keydown`,o),setTimeout(()=>{r.remove(),i.remove()},200)},o=e=>{e.key===`Escape`&&a()};document.addEventListener(`keydown`,o),r.addEventListener(`click`,a),i.querySelector(`.client-modal__close`)?.addEventListener(`click`,a);let s=i.querySelector(`#qm-body`);function c(){s.innerHTML=`
      ...
      </div>`,s.querySelector(`[data-act="done"]`)?.addEventListener(`click`,a),s.querySelector(`[data-act="resend"]`)?.addEventListener(`click`,()=>c())}function d(e,t=!1){let r=e.editability||{editable:e.status===`SUBMITTED`&&!e.locked_tz,reason:e.locked_tz?`finalized`:null,deadline:null},i=!!r.editable,o=!i&&e.status===`SUBMITTED`?r.reason===`finalized`?`<div class="qm-status" data-status="LOCKED"><span class="material-symbols-outlined" aria-hidden="true">lock</span> Finalized${e.locked_tz?` ${X(ey(e.locked_tz))}`:``}${e.locked_by_email?` by ${X(e.locked_by_email)}`:``} — no further edits.</div>`:`<div class="qm-status" data-status="LOCKED"><span class="material-symbols-outlined" aria-hidden="true">lock_clock</span> Editing closed${r.deadline?` ${X(ey(r.deadline))}`:``} — a response locks 14 days after it is submitted.</div>`:``,c=i&&r.deadline?`<p class="client-modal__sub qm-editable-hint">You can edit these answers until ${X(ey(r.deadline))}.</p>`:``;s.innerHTML=`
      <div class="qm-status" data-status="SUBMITTED"><span class="material-symbols-outlined" aria-hidden="true">task_alt</span> Submitted ${e.submitted_tz?X(ey(e.submitted_tz)):``}</div>
      ${o}
      ${cr(e)}
      <form id="qm-form">
      ...
      </div>`,s.querySelector(`[data-act="done"]`)?.addEventListener(`click`,a),s.querySelector(`[data-act="cancel-edit"]`)?.addEventListener(`click`,()=>d(e,!1)),s.querySelector(`[data-act="edit"]`)?.addEventListener(`click`,()=>d(e,!0));let l=s.querySelector(`[data-act="save"]`);l?.addEventListener(`click`,async()=>{let t=sr(s);l.disabled=!0;let r=l.innerHTML;l.innerHTML=`<span class="material-symbols-outlined" aria-hidden="true">progress_activity</span> Saving…`;let i;try{i=await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e.guid)}`,{method:`PUT`,headers:{"Content-Type":`application/json`},body:JSON.stringify({answers:t})});let r=await i.json().catch(()=>({}));if(!i.ok||!r.ok)throw Error(r.error||r.message||`Failed (HTTP ${i.status})`);Hv(`Answers updated`,`success`);let a=await(await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e.guid)}`)).json().catch(()=>({}));d(a.ok?a.data:{...e,answers:t},!1)}catch(t){if(i&&i.status===423){Hv(t.message||`This questionnaire is now locked.`,`info`);let r=await(await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e.guid)}`)).json().catch(()=>({}));d(r.ok?r.data:e,!1);return}s.querySelector(`#qm-error`).textContent=t.message||`Something went wrong. Please try again.`,l.disabled=!1,l.innerHTML=r}})}try{let t=await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse?owner_guid=${encodeURIComponent(e)}`),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`HTTP ${t.status}`);let i=(r.data||[])[0]||null;if(!i){c();return}if(i.status===`DRAFT`){l(i);return}if(i.status===`CANCELLED`){u(i);return}let a=await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(i.guid)}`),o=await a.json().catch(()=>({}));if(!a.ok||!o.ok)throw Error(o.error||`HTTP ${a.status}`);d(o.data)}catch(e){console.warn(`[clients] questionnaire load failed:`,e),s.innerHTML=`<p class="client-modal__error">Could not load questionnaire status. ${X(e.message||``)}</p>`}}var ny,ry,iy,ay,oy,X,sy,cy,ly,uy=o((()=>{w(),Qe(),ny=u(f(),1),m(),an(),pr(),ry=`reseller_org`,iy=`managed_client`,ay=!1,oy={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},X=e=>String(e??``).replace(/[&<>"']/g,e=>oy[e]),sy=e=>String(e||`?`).trim().split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``).toUpperCase()||`?`,cy=e=>String(e||``).replace(/[^0-9]/g,``),ly={render:Jv,init:Yv}})),dy=c({default:()=>Ay,init:()=>by,loadTeam:()=>xy,render:()=>yy});function fy(){let e=C.getActiveOrg(),t=e&&e.channel!==wy?e:null;if(!t){let e=C.getMemberships().find(e=>e.channel!==wy);e&&(t={guid:e.orgGuid,channel:e.channel,memberRole:e.memberRole})}if(!t)return null;let n=C.getMemberships().find(e=>e.orgGuid===t.guid)?.name;return{...t,name:n}}function py(e){return Object.values(Ie).filter(t=>Fe(e,t.value)).sort((e,t)=>t.rank-e.rank)}function my(){if(Ty)return;let e=document.createElement(`style`);e.id=`m5t-team-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Ty=!0}function hy(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function gy(e,t,n){let r=e.member_role||`member`,i=e.entity_guid&&e.entity_guid===C.getEntity?.()?.guid,a=n&&!i&&py(t.memberRole).some(e=>e.value===r);return`
    <div class="team-row">
      <span class="team-avatar">${Dy(ky(e.name||e.email))}</span>
      <span class="team-main">
        <div class="team-name">${Dy(e.name||e.email)}${e.verified?``:` · <span style="color:var(--color-text-subtle,#6C7C9A);font-weight:400;">pending verification</span>`}</div>
        <div class="team-email" title="${Dy(e.email)}">${Dy(e.email)}</div>
      </span>
      <span class="team-role" data-role="${Dy(r)}">${Dy(Ie[r]?.label||r)}</span>
      ${a?`<button class="team-modal__btn team-modal__btn--ghost team-remove" type="button" data-id="${Dy(e.id)}" data-name="${Dy(e.name||e.email)}">Remove</button>`:``}
    </div>`}async function _y(e,t){let{uid:n,email:r}=e.dataset;if(window.confirm(`Revoke the invitation for ${r}?\n\nThe invitation link stops working immediately, including any copy already sent. You can invite this address again afterwards.`)){e.disabled=!0;try{let e=C.getConfig?.().BASE_URL||``,i=await fetch(`${e}/m5t/v5/acctGroup/${encodeURIComponent(t.guid)}/invites/${encodeURIComponent(n)}`,{method:`DELETE`}),a=await i.json().catch(()=>({}));if(!i.ok||!a.ok)throw Error(a.message||a.error||`HTTP ${i.status}`);Oy(`Invitation for ${r} revoked`,`success`)}catch(t){e.disabled=!1,Oy(t.message||`Could not revoke that invitation.`,`error`);return}try{await xy(t)}catch{}}}async function vy(e,t){let{id:n,name:r}=e.dataset;if(window.confirm(`Remove ${r} from the team?\n\nThey lose access to this workspace immediately. Their account itself is not deleted, and you can invite them again later.`)){e.disabled=!0;try{let e=C.getConfig?.().BASE_URL||``,t=await fetch(`${e}/m5t/v5/acctGroupMember/${encodeURIComponent(n)}`,{method:`DELETE`}),i=await t.json().catch(()=>({}));if(!t.ok||!i.ok)throw Error(i.message||i.error||`HTTP ${t.status}`);Oy(`${r} removed from the team`,`success`)}catch(t){e.disabled=!1,Oy(t.message||`Could not remove that member.`,`error`);return}try{await xy(t)}catch{}}}function yy(){return my(),`
    <div class="page-shell" data-section="team">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="team-body">
          ${hy(`hourglass_empty`,`Loading team…`)}
        </div>
      </div>
    </div>`}async function by(){let e=fy(),t=document.getElementById(`team-invite-btn`);if(!e){t&&(t.style.display=`none`),document.getElementById(`team-body`).innerHTML=hy(`info`,`Team management is available for agency and workspace accounts.`);return}t?.addEventListener(`click`,()=>Sy(e)),await xy(e)}async function xy(e){let t=document.getElementById(`team-body`),n=document.getElementById(`team-count`);if(t)try{let r=C.getConfig?.().BASE_URL||``,[i,a]=await Promise.all([fetch(`${r}/m5t/v5/acctGroup/${encodeURIComponent(e.guid)}/members`),fetch(`${r}/m5t/v5/acctGroup/${encodeURIComponent(e.guid)}/invites`)]),[o,s]=await Promise.all([i.json().catch(()=>({})),a.json().catch(()=>({}))]);if(!i.ok||!a.ok)throw Error(`Failed to load team (members: ${i.status}, invites: ${a.status})`);let c=o.members||[],l=s.invites||[],u=py(e.memberRole).length>0;t.innerHTML=(l.length?`
      <div class="team-pending">
        <div class="team-pending__label">Pending invites</div>
        ${l.map(e=>`
          <div class="team-pending__row">
            <span class="team-pending__email" title="${Dy(e.email)}">${Dy(e.email)}${e.expired?` · expired`:``}</span>
            <button class="team-modal__btn team-modal__btn--ghost team-copy" type="button" data-url="${Dy(e.inviteUrl)}">Copy link</button>
            ${u?`<button class="team-modal__btn team-modal__btn--ghost team-revoke" type="button" data-uid="${Dy(e.uid)}" data-email="${Dy(e.email)}">Revoke</button>`:``}
          </div>`).join(``)}
      </div>`:``)+(c.length?`<div class="team-list">${c.map(t=>gy(t,e,u)).join(``)}</div>`:hy(`group`,`No members yet.`)),n&&(n.textContent=`${c.length} member${c.length===1?``:`s`}`),t.querySelectorAll(`.team-copy`).forEach(e=>e.addEventListener(`click`,async()=>{try{await navigator.clipboard.writeText(e.dataset.url),e.textContent=`Copied!`,setTimeout(()=>{e.textContent=`Copy link`},1500)}catch{}})),t.querySelectorAll(`.team-revoke`).forEach(t=>t.addEventListener(`click`,()=>_y(t,e))),t.querySelectorAll(`.team-remove`).forEach(t=>t.addEventListener(`click`,()=>vy(t,e)))}catch(e){console.warn(`[team] load failed:`,e),t.innerHTML=hy(`error`,`Couldn’t load the team. Please try again.`)}}function Sy(e){let t=py(e.memberRole);if(!t.length){Oy(`You don’t have permission to invite teammates.`,`error`);return}let n=document.createElement(`div`);n.className=`team-modal-scrim`;let r=document.createElement(`div`);r.className=`team-modal`,r.setAttribute(`role`,`dialog`),r.setAttribute(`aria-modal`,`true`),r.innerHTML=`
    ...
    <div class="team-modal__body" id="team-invite-body">
      <p class="team-modal__sub">They'll get a link to set a password and join <strong>${Dy(e.name||`your workspace`)}</strong>.</p>
      ...
      <select class="team-modal__select" id="tm-role">
        ${t.map(e=>`<option value="${e.value}"${e.value===`member`?` selected`:``}>${Dy(e.label)}</option>`).join(``)}
      </select>
      ...
      <div class="team-modal__actions">
        ...
        </div>`;let u=r.querySelector(`#tm-link`);r.querySelector(`#tm-copy`)?.addEventListener(`click`,async()=>{try{await navigator.clipboard.writeText(u.value),r.querySelector(`#tm-copy`).textContent=`Copied!`}catch{u.select()}}),r.querySelector(`[data-act="done"]`)?.addEventListener(`click`,()=>{i(),xy(e)})}catch(e){console.warn(`[team] invite failed:`,e),o.textContent=e.message||`Something went wrong. Please try again.`,s.disabled=!1,s.innerHTML=l}})}var Cy,wy,Ty,Ey,Dy,Oy,ky,Ay,jy=o((()=>{w(),Cy=u(f(),1),m(),Qe(),wy=`managed_client`,Ty=!1,Ey={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Dy=e=>String(e??``).replace(/[&<>"']/g,e=>Ey[e]),Oy=(e,t=`info`)=>{try{Cy.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},ky=e=>String(e||`?`).trim().split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``).toUpperCase()||`?`,Ay={render:yy,init:by}}));m(),we(),w(),Ht(),y(),On(),Fn(),nt(),window.m5t=Object.create(null),window.m5t.pubsub=b.default,console.log(`StreamSyncEngage SaaS Client - Starting...`),console.log(`Environment:`,`production`),console.log(`Document readyState:`,document.readyState),console.log(`window.mvvLegit:`,typeof window.mvvLegit),console.log(`globalThis.mvvLegit:`,typeof globalThis.mvvLegit);try{console.log(`mvvLegit (direct):`,typeof mvvLegit)}catch(e){console.log(`mvvLegit (direct): ReferenceError -`,e.message)}var My=Object.keys(window).filter(e=>e.toLowerCase().includes(`mvv`)||e.toLowerCase().includes(`neodigm`)||e.toLowerCase().includes(`legit`));console.log(`Possible Neodigm globals:`,My);var Z=new ee.default(`/`,{hash:!0});window.router=Z;function Q(e){let t=e?`${e.replace(/-/g,`_`)}_route`:`splash_route`;return console.log(`[Router] Checking access: ${e} -> ${t}`),mvvLegit.isRouteAllowed(t)?!0:(console.warn(`[Router] Access denied to route: ${t}`),b.default.publish(p.UI_TOAST,{message:`Please sign in to access this page`,type:`warning`}),!1)}function $(e){console.log(`[Router] Navigating to:`,e),Z.navigate(`/${e}`)}var Ny=new Set([`splash`,`signin`,`signup`,`forgot`,`resethash`,`verf-link`,`accept-invite`,`signout`]),Py={},Fy={},Iy=null,Ly={admin:{caption:`Platform Admin`,icon:`shield_person`},reseller:{caption:`Reseller`,icon:`handshake`},managed:{caption:`Managed`,icon:`visibility`},self_serve:{caption:`Self-Serve`,icon:`rocket_launch`}},Ry=`__portfolio__`;function zy(){try{Z._setCurrent(null)}catch{}Z.resolve()}function By(){let e=document.getElementById(`m5t-side-nav`);if(!e)return;let t=!C.getScope()&&C.isAuthenticated(),n=C.getCapabilities(),r=C.getUiMode(),i=qt.getSideNav(n,{uiMode:r});Py={},Fy={};let a=e=>{e.route&&(Py[e.route]={sectionToken:e.token,childToken:null,children:e.children||null},Array.isArray(e.children)&&e.children.forEach(t=>{t.route&&(Fy[t.token]=t.route,Py[t.route]={sectionToken:e.token,childToken:t.token,children:e.children})}))},o=e=>e.forEach(e=>{e.group&&Array.isArray(e.items)?o(e.items):a(e)});o(i);let s=C.getMemberships(),c=C.getActiveOrg(),l=C.getManagedClients(),u=C.getStoredActiveOrg(),d=l.length?[{guid:Ry,name:`Portfolio (all clients)`},...s.map(e=>({guid:e.orgGuid,name:e.name||e.channel,channel:e.channel,group:`Your organization`})),...l.map(e=>({guid:e.orgGuid,name:e.name||e.orgGuid,channel:e.channel,group:`Clients`}))]:s.map(e=>({guid:e.orgGuid,name:e.name||e.channel,channel:e.channel}));e.setState({buttons:i,orgs:d,activeOrg:l.length?u||Ry:c?.guid||null,footerBadge:Ly[r]||null,loading:t}),Hy(),Iy&&Uy(Iy)}var Vy=e=>String(e??``).replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`})[e]);function Hy(){let e=document.getElementById(`app-content`);if(!e)return;let t=document.getElementById(`org-context-banner`);t||(t=document.createElement(`div`),t.id=`org-context-banner`,e.insertBefore(t,e.firstChild),yn(t));let n=C.getActiveOrg(),r=C.isPreviewingAsClient(),i=!!n?.viaSubtree&&!!Iy&&!Ny.has(Iy);if(t.hidden=!i,!i){t.innerHTML=``;return}let a=Vy(n.name||`Client`);t.innerHTML=`
    <style>
    ...
    <div class="ocb" role="status">
      ...
      `,b.default.publish(p.APP_ERROR,{message:`Failed to load authentication library`}))},100)}document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,Xy):Xy();async function Zy(){if(console.log(`[App] Neodigm 55 loaded, initializing...`),An(),Ky(),ue(),Vt.initialize(),console.log(`[App] I18n Controller initialized`),!await C.init({BASE_URL:`https://machfivemagnet-saas.onrender.com`,navigateFunction:e=>{console.log(`[AuthController] Navigation requested:`,e),$(e)}})){console.error(`[App] AuthController initialization failed`);return}let e=C.getEntity();e&&(console.log(`[App] Session restored for:`,e.email),b.default.publish(p.APP_USER_LOADED,{entity:e}),C.isAuthenticated()&&(localStorage.getItem(C.CONFIG.LS_KEYS.ACTIVE_ORG)?await C.fetchScope():C.fetchScope())),b.default.publish(p.APP_READY,{timestamp:new Date().toISOString(),sessionUser:e}),qy(),Cn(),Z.resolve(),window.AuthController=C,window.I18nController=Vt,window.MachVive=Ut,console.log(`[App] StreamSyncEngage client initialized (AuthController, I18nController, MachVive available globally)`)}b.default.subscribe(p.ROUTE_NAV_AFTER,(e,t)=>{Vt!==void 0&&Vt.refresh&&setTimeout(()=>{Vt.refresh()},100)});
