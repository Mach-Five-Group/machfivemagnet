...
`)],{type:`text/csv`}),a=URL.createObjectURL(i),o=document.createElement(`a`);o.href=a,o.download=`leads-${K.filter.toLowerCase()}.csv`,document.body.appendChild(o),o.click(),o.remove(),URL.revokeObjectURL(a),t_(`Exported ${e.length} lead${e.length===1?``:`s`}`,`success`)}var Xg,Zg,K,Qg,$g,e_,q,t_,n_,r_,i_,a_,o_,s_,c_,l_,u_,d_,f_,p_,m_,h_,g_,__=o((()=>{w(),Xg=u(f(),1),m(),xa(),Da(),sn(),Wc(),Zg=!1,K={leads:[],magnetNames:{},filter:`ALL`,canManage:!1,book:!1,ownerNames:{},clientFilter:`ALL`,search:``,shown:25,selectedId:null,day:``,sort:{key:`captured`,dir:`desc`},magnetFilter:`ALL`,range:`ALL`,pageSize:25,total:0,truncated:!1},Qg=()=>K.pageSize===`ALL`?2**53-1:Number(K.pageSize)||25,$g=[`NEW`,`WORKING`,`QUALIFIED`,`UNQUALIFIED`,`CONVERTED`,`EXPORTED`],e_={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},q=e=>String(e??``).replace(/[&<>"']/g,e=>e_[e]),t_=(e,t=`info`)=>{try{Xg.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},n_=()=>C.getConfig?.().BASE_URL||``,r_=e=>[e.first_name,e.last_name].filter(Boolean).join(` `).trim()||e.email||`(no name)`,i_=e=>(r_(e).split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``)||`?`).toUpperCase(),a_=e=>e.created?.created_tz||``,o_=e=>{let t=a_(e);if(!t)return!1;try{return Date.now()-new Date(t).getTime()<864e5}catch{return!1}},s_=e=>K.clientFilter===`ALL`||e.owner_guid===K.clientFilter,c_=e=>!K.day||a_(e).slice(0,10)===K.day,l_=e=>K.magnetFilter===`ALL`||e.magnet_guid===K.magnetFilter,u_={ALL:null,7:7,30:30,90:90},d_={ALL:`All time`,7:`Last 7 days`,30:`Last 30 days`,90:`Last 90 days`},f_={name:{label:`Name`,dir:`asc`,value:e=>r_(e).toLowerCase()},contact:{label:`Contact`,dir:`asc`,value:e=>String(e.email||e.phone||``).toLowerCase()},captured:{label:`Captured`,dir:`desc`,value:e=>a_(e)}},p_=()=>Tg(`touch_app`,`Click to see lead information.`),m_=[`hubspot`,`zoho`,`salesforce`,`gohighlevel`],h_=()=>`
  ...
  `,document.head.appendChild(e),Rv=!0}function Cv(){return k()?da({section:`ad-performance`,title:`Ad Performance`,description:`Pick a client to view their spend and lead performance.`}):(Sv(),`
    ...
    </div>`)}async function wv(){if(k()){let e=e=>Number(e||0).toLocaleString();return pa(Nv(),[{h:`Spend`,td:t=>t.ad?`$${e(t.ad.spend)}`:`—`},{h:`Clicks`,td:t=>t.ad?e(t.ad.clicks):`—`},{h:`Leads`,td:t=>e(t.leads.window??t.leads.total)},{h:`CPL`,td:t=>t.ad?.cost_per_lead==null?`—`:`$${e(t.ad.cost_per_lead)}`}],{emptyMsg:`No client orgs yet. Connect a client ad account to see spend here.`})}let e=document.getElementById(`adperf-periods`);e?.addEventListener(`click`,t=>{let n=t.target.closest(`button[data-days]`);n&&(Vv=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>e.classList.toggle(`active`,Number(e.dataset.days)===Vv)),Tv())}),document.getElementById(`adperf-sources`)?.addEventListener(`click`,e=>{let t=e.target.closest(`tr[data-campaign]`);t&&(Sa(Ta.LEADS_SEARCH,t.dataset.campaign),!e.target.closest(`a`)&&(window.location.hash=`#/leads`))}),document.getElementById(`adperf-lsa-card`)?.addEventListener(`click`,e=>{let t=e.target.closest(`tr[data-lsalead]`);t&&(Sa(Ta.LEADS_SEARCH,t.dataset.lsalead),!e.target.closest(`a`)&&(window.location.hash=`#/leads`))}),await Tv()}async function Tv(){let e=Nv(),t=document.getElementById(`adperf-body`),n=document.getElementById(`adperf-range`),r=new Date,i=Pv(r),a=Pv(new Date(r.getTime()-(Vv-1)*864e5));n&&(n.textContent=`${a} → ${i}`);let o,s=[],c=null,l=null,u=null,d=null;try{let[t,n,r,f,p,m]=await Promise.all([fetch(`${e}/m5t/v5/coreAdMetrics/overlay?from=${a}&to=${i}`),fetch(`${e}/m5t/v5/coreAdAccount`),fetch(`${e}/m5t/v5/coreAdMetrics/by-source?from=${a}&to=${i}`),yc().catch(()=>null),fetch(`${e}/m5t/v5/coreAdMetrics/conversions?from=${a}&to=${i}`).then(e=>e.json()).catch(()=>null),fetch(`${e}/m5t/v5/coreAdMetrics/lsa?from=${a}&to=${i}`).then(e=>e.json()).catch(()=>null)]);if(!t.ok)throw Error(`HTTP ${t.status}`);o=await t.json(),s=((await n.json().catch(()=>({})))?.data||[]).filter(e=>e.status!==`DELETED`),c=await r.json().catch(()=>null),l=f,u=p,d=m}catch(e){console.warn(`[ad-performance] load failed:`,e),t&&(t.innerHTML=zv(`error`,`Could not load ad performance. Please try again.`));return}Ev(l,s),Dv(u,o?.totals?.spend??null),Ov(d);let f=o.totals||{};if(Av(`adperf-spend`,Fv(f.spend)),Av(`adperf-clicks`,Iv(f.clicks)),Av(`adperf-leads`,Iv(f.magnet_leads)),Av(`adperf-cpl`,Fv(f.cost_per_lead)),kv(c),!s.length){t.innerHTML=zv(`link_off`,`No ad account connected yet. Your agency can connect Google Ads or Local Services Ads to see performance here.`);return}if(!o.connected){t.innerHTML=zv(`cloud_sync`,`Account connected. No ad data for this range yet. Metrics appear here once they sync.`);return}let p=o.rows||[];t.innerHTML=jv(p),Mv(p)}function Ev(e,t){let n=document.getElementById(`adperf-conn`);if(!n)return;if(!e||e.denied){n.innerHTML=``;return}let r=cc.google_ads,i=gc(e.providers,`google_ads`),a=_c(r,i,!!e.configured?.google_ads),o=Lc()?` <a href="#/settings/integrations/${r.urlSlug}">Manage</a>`:``;if(a===`connected`||a===`broken`){Nc(),n.innerHTML=`<span>Google Ads</span> ${vc(a,i)}${o}`;return}if(t.length){n.innerHTML=`<span>Google Ads</span> <span>${t.length} associated account${t.length===1?``:`s`}</span>`;return}n.innerHTML=``}function Dv(e,t){let n=document.getElementById(`adperf-conv-card`),r=document.getElementById(`adperf-conv-body`),i=document.getElementById(`adperf-stats`),a=document.getElementById(`adperf-conv-stat`);a&&a.remove();let o=document.getElementById(`adperf-conv-cpc`);if(o&&o.remove(),!n||!r)return;if(!e?.ok||!e.has_selection){n.style.display=`none`;return}if(i){let n=document.createElement(`div`);n.innerHTML=A({icon:`flag`,label:`Ads Conversions`,value:Iv(e.totals?.conversions),sub:`Reported by Google Ads`,href:`#/settings/integrations/google-ads`});let r=n.firstElementChild;r.id=`adperf-conv-stat`,i.appendChild(r);let a=Number(e.totals?.conversions||0),o=t!=null&&a>0?Math.round(Number(t)/a*100)/100:null,s=document.createElement(`div`);s.innerHTML=A({icon:`request_quote`,label:`Cost / Conversion`,value:Fv(o),sub:`Against the counted actions`,href:`#/analytics/conversions`});let c=s.firstElementChild;c.id=`adperf-conv-cpc`,i.appendChild(c)}let s=e.actions||[];if(n.style.display=``,!s.length){r.innerHTML=zv(`search_off`,`No conversions in this range for the selected actions.`);return}let c=s.some(e=>e.value>0);r.innerHTML=`
      ...
      <table class="adperf-srctable">
        ...
        <tbody>${s.map(e=>`
          <tr>
            ...
            ${c?`<td class="num">${Fv(e.value)}</td>`:``}
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>
    <p class="adperf-note">Counted actions are chosen on the Google Ads integration page. These are Google's own numbers and may overlap magnet leads, so they are never added together.</p>`}function Ov(e){let t=document.getElementById(`adperf-lsa-card`),n=document.getElementById(`adperf-lsa-body`);if(!t||!n)return;if(!e?.ok||!e.connected){t.style.display=`none`;return}t.style.display=``;let r=e.totals||{},i=e.leads||[],a=e.disputes||[],o=r.answer_rate==null?`No call outcomes yet`:r.missed>0?`${Iv(r.missed)} missed call${r.missed===1?``:`s`}`:`Every call answered`;n.innerHTML=`
    ${`
    <div class="stats-grid" style="margin-bottom:16px">
      ${A({icon:`payments`,label:`LSA Spend`,value:Fv(r.spend),sub:`Reported by Google`})}
      ${A({icon:`person_add`,label:`LSA Leads`,value:Iv(r.leads),sub:`${Iv(r.phone_calls)} calls · ${Iv(r.messages)} messages`})}
      ${A({icon:`paid`,label:`Charged`,value:Iv(r.charged),sub:`Leads Google billed for`})}
      ${A({icon:`request_quote`,label:`Cost / Charged Lead`,value:Fv(r.cost_per_charged_lead)})}
      ${A({icon:`call`,label:`Answer Rate`,value:r.answer_rate==null?`—`:`${r.answer_rate}%`,sub:o})}
      ${A({icon:`event_available`,label:`Booked`,value:Iv(r.booked),sub:`Google's lead status`})}
    </div>`}
    ${i.length?`
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
    ...
    <div class="card-header" style="padding:16px 0 0">
      ...
      <span class="live-indicator">${Iv(a.length)} inside the 30 day credit window</span>
    </div>
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
        ...
        <tbody>${a.map(e=>`
          <tr>
            ...
            <td class="num"><b>${Iv(e.days_left)}</b></td>
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>
    ...
    <p class="adperf-note">Numbers come from Google's Local Services feed. These leads land in your Leads inbox and do not flow through magnets.</p>`;let s=n.querySelector(`.adperf-srctable tbody`);s&&E(s,{rowSelector:`tr`,noun:`leads`,controlHost:n.querySelector(`.adperf-srcwrap`)||n})}function kv(e){let t=document.getElementById(`adperf-sources`),n=document.getElementById(`adperf-src-summary`);if(!t)return;let r=e&&Array.isArray(e.rows)?e.rows:[];if(!r.length){n&&(n.textContent=``),t.innerHTML=zv(`search_off`,`No leads in this range yet. Campaigns appear here as tagged leads (?utm_campaign=…) come in.`);return}let i=e.total_leads||r.reduce((e,t)=>e+(t.leads||0),0),a=e.tracked_leads||0;n&&(n.textContent=`${Iv(a)} of ${Iv(i)} leads tagged to a source`);let o=e=>e.campaign&&e.campaign!==`(none)`?Y(e.campaign):e.source&&e.source!==`(direct)`?Y(e.source):`<span class="adperf-src-untracked">Untracked / direct</span>`,s=e=>e.campaign&&e.campaign!==`(none)`?e.campaign:e.source&&e.source!==`(direct)`?e.source:``;t.innerHTML=`
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
        ...
        <tbody>${r.map(e=>{let t=s(e);return`
          <tr${t?` data-campaign="${Y(t)}" class="adperf-srcrow--link"`:``}>
            ...
            <td class="num">${e.paid_clicks?Iv(e.paid_clicks):`<span class="adperf-src-sub">—</span>`}</td>
            <td class="num"><b>${Iv(e.leads)}</b></td>
          </tr>`}).join(``)}
        </tbody>
      </table>
    </div>`;let c=t.querySelector(`.adperf-srctable tbody`);c&&E(c,{rowSelector:`tr`,noun:`sources`,controlHost:t.querySelector(`.adperf-srcwrap`)||t})}function Av(e,t){let n=document.getElementById(e);n&&(n.textContent=t)}function jv(e){return Na({rows:e,xLabel:e=>String(e.date||``).slice(5),left:{value:e=>e.spend||0,label:`Ad Spend`,fmt:e=>`$${Ra(e)}`},right:{value:e=>e.magnet_leads||0,label:`Magnet Leads`},ariaLabel:`Ad spend versus magnet leads over time`,pointLabel:e=>`${e.date}: ${Fv(e.spend||0)} spend, ${Iv(e.magnet_leads||0)} leads`,hitData:e=>({day:e.date})})}function Mv(e){let t=document.getElementById(`adperf-body`);return t?Fa(t,{rows:e,tipHTML:e=>`
      <div class="m5c-tip__title">${Y(String(e.date||``))}</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--1"></i>${Fv(e.spend||0)} spend</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--2"></i>${Iv(e.magnet_leads||0)} leads</div>`}):()=>{}}var Nv,Pv,Fv,Iv,Lv,Y,Rv,zv,Bv,Vv,Hv,Uv=o((()=>{w(),xa(),Wc(),Aa(),Ba(),sn(),Da(),Nv=()=>C.getConfig?.().BASE_URL||``,Pv=e=>e.toISOString().slice(0,10),Fv=e=>e==null?`—`:`$`+Number(e).toLocaleString(void 0,{minimumFractionDigits:0,maximumFractionDigits:2}),Iv=e=>Number(e||0).toLocaleString(),Lv={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Y=e=>String(e??``).replace(/[&<>"']/g,e=>Lv[e]),Rv=!1,zv=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,Bv=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],Vv=30,Hv={render:Cv,init:wv}})),Wv=c({default:()=>dy,init:()=>qv,parseBookingCsv:()=>iy,render:()=>Kv});function Gv(){if(cy)return;let e=document.createElement(`style`);e.id=`m5t-adacct-styles`,e.textContent=`
