...
`))}</textarea>`}return`<div class="mag-cta" data-type="${e}" data-kind="${n.kind}">
  ...
  `,document.head.appendChild(e),Tp=!0}function Sf(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function Cf(e){let t=V.stats[e];if(!t||!t.opens&&!t.leads)return`<div class="mag-statline mag-statline--empty">&mdash;</div><div class="mag-cellsub">No activity yet</div>`;let n=e=>Number(e||0).toLocaleString();return`<div class="mag-statline" title="Trailing 30 days"><b>${n(t.opens)}</b> opens &middot; <b>${n(t.leads)}</b> leads</div>
    ${t.completion_rate==null?``:`<div class="mag-cellsub">${t.completion_rate}% complete the conversation</div>`}`}function wf(e){let t=V.install[e.guid];if(!t)return``;let n=t.unrecognized||[],r=(t.traffic?.opens||0)>0,i=e.can_manage===!0,a=n.length?`
    ...
    <div class="mag-card${i?` is-open`:``}" data-guid="${H(e.guid)}">
      <div class="mag-line">
        ...
        <div class="mag-cell mag-cell--status">${Hf(e)}</div>
        <div class="mag-cell mag-cell--stats">${Cf(e.guid)}</div>
    ...
    </div>`}function Wf(e,t){let n=typeof t==`boolean`?t:V.detailOpen[e]!==!0;V.detailOpen[e]=n;let r=document.querySelector(`.mag-card[data-guid="${CSS.escape(e)}"]`);if(!r)return;r.classList.toggle(`is-open`,n);let i=r.querySelector(`.mag-detail`);i&&(i.hidden=!n);let a=r.querySelector(`[data-action="install-toggle"][aria-controls]`);if(a){a.setAttribute(`aria-expanded`,String(n)),a.classList.toggle(`is-open`,n);let e=a.querySelector(`.material-symbols-outlined`);e&&(e.textContent=n?`expand_less`:`expand_more`)}}function Gf(){Bp||(Bp=!0,document.addEventListener(`click`,e=>{e.target.closest?.(`.mag-more-wrap`)||Kf()}),document.addEventListener(`keydown`,e=>{e.key===`Escape`&&Kf()}))}function Kf(e){document.querySelectorAll(`.mag-menu`).forEach(t=>{if(t===e)return;t.hidden=!0;let n=t.parentElement?.querySelector(`[data-action="menu"]`);n&&n.setAttribute(`aria-expanded`,`false`)})}function qf(e){let t=document.querySelector(`.mag-card[data-guid="${CSS.escape(e.guid)}"]`);if(!t)return;let n=t.querySelector(`.mag-install-slot`);n&&(n.innerHTML=wf(e));let r=Vf(e),i=t.querySelector(`.mag-cell--status`);i&&(i.innerHTML=Hf(e)),(r===`quiet`||r===`seen`)&&V.detailOpen[e.guid]===void 0&&Wf(e.guid,!0)}function Jf(){return M()?ta({section:`magnets`,title:`Your Magnets`,description:`Pick a client to view and manage their magnets.`}):(xf(),`
    ...
    <div class="mag-modal" role="dialog" aria-modal="true" aria-label="${i?`Edit magnet`:`New magnet`}">
      ...
      <div class="mag-modal__main">
        ...
        <select class="mag-modal__select" id="mag-f-trigger">
          ${Np.map(t=>`<option value="${t.value}"${lf(e)===t.value?` selected`:``}>${H(t.label)}</option>`).join(``)}
        ...
        <div class="mag-perdev" id="mag-f-perdev"${lf(e)===`__per_device`?``:` hidden`}>
          <div class="mag-perdev__row">
            ...
            <select class="mag-modal__select" id="mag-f-trigger-desktop">
              ${Np.map(t=>`<option value="${t.value}"${uf(e,`desktop`)===t.value?` selected`:``}>${H(t.label)}</option>`).join(``)}
            </select>
          </div>
          <div class="mag-perdev__row">
            <label class="mag-modal__label" for="mag-f-trigger-mobile">Mobile</label>
            <select class="mag-modal__select" id="mag-f-trigger-mobile">
              ${Np.map(t=>`<option value="${t.value}"${uf(e,`mobile`)===t.value?` selected`:``}>${H(t.label)}</option>`).join(``)}
            </select>
          </div>
...
/&gt;</code></pre>
...
</details>`},Ap={chat:`Chat bubble`,right_slide:`Slide-in panel`,modal:`Modal`,inline:`Inline`},jp=[{value:`inline`,icon:`view_agenda`,label:`Inline`},{value:`chat`,icon:`chat`,label:`Chat`},{value:`right_slide`,icon:`vertical_split`,label:`Slide`},{value:`modal`,icon:`web_asset`,label:`Modal`}],Mp=[{value:`auto`,icon:`brightness_auto`,label:`Auto`},{value:`sync`,icon:`sync`,label:`Sync`},{value:`light`,icon:`light_mode`,label:`Light`},{value:`dark`,icon:`dark_mode`,label:`Dark`}],Np=[{value:`on_load`,label:`Immediately on page load`},{value:`on_8_seconds`,label:`After 8 seconds`},{value:`on_scroll_80_percent`,label:`At 80% scroll depth`},{value:`on_exit_intent`,label:`On exit intent (mouse leaves the page)`},{value:`none`,label:`Manual only (visitor clicks the launcher)`}],Pp={book:{label:`Book`,icon:`event`,kind:`book`},chat:{label:`Chat`,icon:`chat`,kind:`flow`},call:{label:`Call`,icon:`phone`,kind:`value`,placeholder:`Phone number`},email:{label:`Email`,icon:`mail`,kind:`value`,placeholder:`Email address`},support:{label:`Support`,icon:`support_agent`,kind:`support`}},Fp=[`book`,`chat`,`call`,`email`,`support`],Ip=null,Lp=null,Rp=5e3,zp={ACTIVE:{label:`Connected`,icon:`check_circle`,tone:`live`},PENDING:{label:`Waiting to connect`,icon:`hourglass_empty`,tone:`pending`},REVOKED:{label:`Disconnected`,icon:`link_off`,tone:`off`},EXPIRED:{label:`Expired`,icon:`timer_off`,tone:`off`}},Bp=!1,Vp=512,Hp=new Set([`image/png`,`image/jpeg`,`image/gif`,`image/webp`]),Up=8388608,Wp=[{label:`Web fonts (loaded for you)`,opts:[[`Inter, sans-serif`,`Inter`],[`Poppins, sans-serif`,`Poppins`],[`Montserrat, sans-serif`,`Montserrat`],[`Roboto, sans-serif`,`Roboto`],[`"Open Sans", sans-serif`,`Open Sans`],[`Lato, sans-serif`,`Lato`],[`Nunito, sans-serif`,`Nunito`],[`Raleway, sans-serif`,`Raleway`],[`"Work Sans", sans-serif`,`Work Sans`],[`Oswald, sans-serif`,`Oswald`],[`Merriweather, serif`,`Merriweather (serif)`]]},{label:`System / always available`,opts:[[`system-ui, sans-serif`,`System UI`],[`Georgia, serif`,`Georgia (serif)`],[`"Times New Roman", Times, serif`,`Times (serif)`],[`Arial, Helvetica, sans-serif`,`Arial / Helvetica`],[`Verdana, Geneva, sans-serif`,`Verdana`],[`"Courier New", monospace`,`Courier (mono)`]]}]})),Kp=c({default:()=>rm,init:()=>Xp,render:()=>Yp});function qp(){let e=new Map;for(let t of nf)e.set(t.category,(e.get(t.category)||0)+1);return Object.keys(tf).filter(t=>e.has(t)).map(t=>[t,e.get(t)])}function Jp(){if(Zd(),nm)return;let e=document.createElement(`style`);e.id=`m5t-templates-page-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),nm=!0}function Yp(){Jp();let e=em();return`
    <div class="page-shell" data-section="templates">
      ...
      <div class="m5t-chip-row tpl-filters" id="tpl-filters" role="group" aria-label="Filter templates by category">
        ${tm(`all`,`All`,nf.length,`true`)}
        ${qp().map(([e,t])=>tm(e,tf[e],t,`false`)).join(``)}
      </div>

      <div class="mag-tpl-grid mag-tpl-grid--page" id="tpl-grid">
        ${nf.map(t=>Kd(t,{detail:!0,gallery:!0,canUse:e})).join(``)}
      </div>
    </div>`}function Xp(){let e=document.getElementById(`tpl-grid`);document.getElementById(`tpl-filters`)?.addEventListener(`click`,t=>{let n=t.target.closest(`[data-cat]`);if(!n)return;n.parentElement.querySelectorAll(`[data-cat]`).forEach(e=>{e.setAttribute(`aria-pressed`,String(e===n))});let r=n.dataset.cat;e?.querySelectorAll(`.mag-tpl-card`).forEach(e=>{e.hidden=r!==`all`&&e.dataset.cat!==r})}),e?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-preview]`);if(t){let e=nf.find(e=>e.id===t.dataset.preview);e&&Vd(()=>Fd({...e.appearance,steps:e.steps,name:e.name}));return}let n=e.target.closest(`[data-tpl]`);n&&(qd(n.dataset.tpl),window.location.hash=`#/magnets`)})}var Zp,Qp,$p,em,tm,nm,rm,im=o((()=>{D(),Ud(),sf(),Zp={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Qp=e=>String(e??``).replace(/[&<>"']/g,e=>Zp[e]),$p=[`provision_magnets`,`manage_magnets`],em=()=>E.getCapabilities().some(e=>$p.includes(e)),tm=(e,t,n,r)=>`<button class="m5t-chip--filter" type="button" data-cat="${Qp(e)}" aria-pressed="${r}">${Qp(t)}<span class="m5t-chip__count">${n}</span></button>`,nm=!1,rm={render:Yp,init:Xp}})),am=c({default:()=>Am,init:()=>dm,render:()=>um});function om(){if(nr(),Dm)return;let e=document.createElement(`style`);e.id=`m5t-questionnaires-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Dm=!0}function sm(){return Em.clients.map(e=>{let t=Em.responses.filter(t=>t.owner_guid===e.guid),n=t[0]||null;return{guid:e.guid,name:e.caption||e.name||e.guid,latest:n,earlier:Math.max(0,t.length-1),status:n?n.status:`NOT_SENT`}})}function cm(){let e=Em.search.trim().toLowerCase();return sm().filter(t=>(Em.filter===`ALL`||t.status===Em.filter)&&(!e||t.name.toLowerCase().includes(e)))}function lm(e){let t=e.status===`SUBMITTED`,n=e.status===`SUBMITTED`?`<button class="qz-act" type="button" data-act="open" data-guid="${bm(e.latest.guid)}">Read answers</button>`:e.status===`DRAFT`?`<button class="qz-act" type="button" data-act="copy" data-url="${bm(e.latest.url||``)}">Copy link</button>`:`<a class="qz-act" href="#/clients">Go to client</a>`;return`<tr class="qz-row${t?` qz-row--open`:``}"${t?` data-open-guid="${bm(e.latest.guid)}"`:``}>
    <td><span class="qz-client">${bm(e.name)}</span>${e.earlier?`<span class="qz-earlier">${e.earlier} earlier</span>`:``}</td>
    <td><span class="qz-badge" data-s="${bm(e.status)}">${bm(Tm[e.status]||e.status)}</span></td>
    <td class="qz-when">${bm(km(e,`created_tz`))}</td>
    <td class="qz-when">${bm(km(e,`submitted_tz`))}</td>
    <td class="qz-actcell">${n}</td>
  </tr>`}function um(){return om(),`
    <div class="page-shell" data-section="questionnaires">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="qz-body">${Om(`hourglass_empty`,`Loading…`)}</div>
      </div>
    </div>`}async function dm(){await fm()}async function fm(){let e=document.getElementById(`qz-body`);if(e){try{let[e,t]=await Promise.all([fetch(`${xm()}/m5t/v5/acctGroup`),fetch(`${xm()}/m5t/v5/m5mQuestionnaireResponse`)]),n=await e.json().catch(()=>({})),r=await t.json().catch(()=>({}));if(!e.ok||!t.ok)throw Error(`load failed`);Em.clients=(n.data||[]).filter(e=>e.class===Cm&&![`DELETED`,`ARCHIVED`].includes(e.status)).sort((e,t)=>String(e.caption||``).localeCompare(String(t.caption||``))),Em.responses=r.data||[]}catch{e.innerHTML=Om(`error`,`Could not load questionnaires. Please try again.`);return}mm()}}function pm(){let e=document.getElementById(`qz-tbody`);e&&Yt(e,{rowSelector:`tr.qz-row`,noun:`clients`,controlHost:e.closest(`.qz-tablewrap`)||e.parentElement})}function mm(){let e=document.getElementById(`qz-body`);if(!e)return;if(!Em.clients.length){e.innerHTML=Om(`groups`,`No clients yet. Add a client first, then send them a kickoff questionnaire.`);return}let t=sm(),n={ALL:t.length};wm.slice(1).forEach(([e])=>{n[e]=t.filter(t=>t.status===e).length});let r=cm();e.innerHTML=`
    <div class="qz-bar">
      <div class="qz-filter">
        ${wm.map(([e,t])=>`<button data-filter="${e}" class="${Em.filter===e?`active`:``}">${bm(t)}${n[e]?`<span class="qz-pillnum">${n[e]}</span>`:``}</button>`).join(``)}
      </div>
      <span class="qz-searchwrap">
        <span class="material-symbols-outlined" aria-hidden="true">search</span>
        <input class="qz-search" id="qz-search" type="search" placeholder="Search clients" value="${bm(Em.search)}" aria-label="Search clients" />
      </span>
    ...
    <div class="qz-tablewrap">
      <table class="qz-table">
        ...
        <tbody id="qz-tbody">${r.length?r.map(lm).join(``):`<tr><td colspan="5"><div class="qz-when" style="padding:14px 4px">${Em.search.trim()?`No clients match your search.`:`No clients with status “${bm(Tm[Em.filter]||Em.filter)}”.`}</div></td></tr>`}</tbody>
      </table>
    </div>`,pm(),e.querySelector(`.qz-filter`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-filter]`);t&&(Em.filter=t.dataset.filter,mm())}),e.querySelector(`#qz-search`)?.addEventListener(`input`,e=>{Em.search=e.target.value,hm()}),e.querySelector(`#qz-tbody`)?.addEventListener(`click`,gm)}function hm(){let e=document.getElementById(`qz-tbody`);if(!e)return;let t=cm();e.innerHTML=t.length?t.map(lm).join(``):`<tr><td colspan="5"><div class="qz-when" style="padding:14px 4px">No clients match your search.</div></td></tr>`,pm();let n=document.querySelector(`.qz-count`);n&&(n.textContent=`${t.length} of ${sm().length}`)}function gm(e){let t=e.target.closest(`[data-act="copy"]`);if(t){e.stopPropagation();let n=t.dataset.url;if(!n){Sm(`No link on this questionnaire yet.`,`warning`);return}navigator.clipboard?.writeText(n).then(()=>{t.textContent=`Copied!`,setTimeout(()=>{t.textContent=`Copy link`},1500)},()=>Sm(`Could not copy the link.`,`error`));return}let n=e.target.closest(`[data-act="open"]`),r=e.target.closest(`[data-open-guid]`),i=n?.dataset.guid||r?.dataset.openGuid;i&&_m(i)}async function _m(e){Em.openGuid=e;let t=document.createElement(`div`);t.className=`qz-drawer-scrim`;let n=document.createElement(`aside`);n.className=`qz-drawer`,n.setAttribute(`role`,`dialog`),n.setAttribute(`aria-label`,`Questionnaire answers`),n.innerHTML=`
    ...
    <div class="qz-drawer__body" id="qz-dbody">${Om(`hourglass_empty`,`Loading answers…`)}</div>`,document.body.append(t,n),requestAnimationFrame(()=>{t.classList.add(`open`),n.classList.add(`open`)});let r=()=>{Em.openGuid=null,t.classList.remove(`open`),n.classList.remove(`open`),setTimeout(()=>{t.remove(),n.remove()},220),document.removeEventListener(`keydown`,i)};function i(e){e.key===`Escape`&&(e.preventDefault(),r())}document.addEventListener(`keydown`,i),t.addEventListener(`click`,r),n.querySelector(`.qz-drawer__close`).addEventListener(`click`,r);try{let t=await fetch(`${xm()}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e)}`),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`load failed`);let i=r.data||{},a=Em.clients.find(e=>e.guid===i.owner_guid)?.caption||`Client`;n.querySelector(`#qz-dtitle`).textContent=a;let o=i.submitted_tz?`Submitted ${k(i.submitted_tz)}`:`Not yet submitted`;n.querySelector(`#qz-dsub`).textContent=i.locked_tz?`${o} · Finalized`:o,n.querySelector(`#qz-dbody`).innerHTML=tr(i)+Qn(i,{emptyLabel:`Not answered`})}catch{n.querySelector(`#qz-dbody`).innerHTML=Om(`error`,`Could not load these answers. Please try again.`)}}var vm,ym,bm,xm,Sm,Cm,wm,Tm,Em,Dm,Om,km,Am,jm=o((()=>{D(),vm=u(f(),1),m(),or(),Zt(),ym={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},bm=e=>String(e??``).replace(/[&<>"']/g,e=>ym[e]),xm=()=>E.getConfig?.().BASE_URL||``,Sm=(e,t=`info`)=>{try{vm.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},Cm=`managed_client`,wm=[[`ALL`,`All`],[`NOT_SENT`,`Not sent`],[`DRAFT`,`Awaiting response`],[`SUBMITTED`,`Submitted`],[`CANCELLED`,`Cancelled`]],Tm=Object.fromEntries(wm),Em={clients:[],responses:[],filter:`ALL`,search:``,openGuid:null},Dm=!1,Om=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,km=(e,t)=>e?.latest?.[t]?k(e.latest[t]):`—`,Am={render:um,init:dm}})),Mm=c({default:()=>fh,init:()=>Rm,render:()=>Lm});function Nm(){if(nr(),ch)return;let e=document.createElement(`style`);e.id=`m5t-deliverables-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),ch=!0}function Pm(){let e=q.search.trim().toLowerCase();return q.rows.filter(t=>(q.filter===`ALL`||t.status===q.filter)&&(!e||String(t.owner_name||``).toLowerCase().includes(e)))}function Fm(e){return e.status===`DRAFT`?`<button class="dz-act" type="button" data-act="open" data-guid="${G(e.guid)}">Edit</button>`:e.status===`SENT`?`<button class="dz-act" type="button" data-act="copy" data-url="${G(e.url||``)}">Copy link</button>`:e.status===`SUBMITTED`?`<button class="dz-act" type="button" data-act="open" data-guid="${G(e.guid)}">Read review</button>`:`<span class="dz-when">—</span>`}function Im(e){let t=e.status!==`CANCELLED`;return`<tr class="dz-row${t?` dz-row--open`:``}"${t?` data-open-guid="${G(e.guid)}"`:``}>
    ...
    <td><span class="dz-badge" data-s="${G(e.status)}">${G(sh[e.status]||e.status)}</span></td>
    <td class="dz-when">${e.sent_tz?G(k(e.sent_tz)):`—`}</td>
    <td class="dz-when">${e.submitted_tz?G(k(e.submitted_tz)):`—`}</td>
    <td class="dz-actcell">${Fm(e)}</td>
  </tr>`}function Lm(){return Nm(),`
    <div class="page-shell" data-section="deliverables">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="dz-body">${lh(`hourglass_empty`,`Loading…`)}</div>
      </div>
    </div>`}async function Rm(){document.getElementById(`dz-new`)?.addEventListener(`click`,Gm),await zm()}async function zm(){let e=document.getElementById(`dz-body`);if(e){try{let[e,t,n]=await Promise.all([fetch(`${ih()}/m5t/v5/m5mDeliverable`),fetch(`${ih()}/m5t/v5/acctGroup`),fetch(`${ih()}/m5t/v5/m5mDeliverable/templates`)]),r=await e.json().catch(()=>({})),i=await t.json().catch(()=>({})),a=await n.json().catch(()=>({}));if(!e.ok||!t.ok||!n.ok)throw Error(`load failed`);q.rows=r.data||[],q.clients=(i.data||[]).filter(e=>e.class===ah&&![`DELETED`,`ARCHIVED`].includes(e.status)).sort((e,t)=>String(e.caption||``).localeCompare(String(t.caption||``))),q.templates=a.data||[]}catch{e.innerHTML=lh(`error`,`Could not load deliverables. Please try again.`);return}Vm()}}function Bm(){let e=document.getElementById(`dz-tbody`);e&&Yt(e,{rowSelector:`tr.dz-row`,noun:`deliverables`,controlHost:e.closest(`.dz-tablewrap`)||e.parentElement})}function Vm(){let e=document.getElementById(`dz-body`);if(!e)return;if(!q.clients.length&&!q.rows.length){e.innerHTML=lh(`groups`,`No clients yet. Add a client first, then author their first deliverable.`);return}if(!q.rows.length){e.innerHTML=lh(`approval`,`No deliverables yet. Author the first one with New deliverable.`);return}let t={ALL:q.rows.length};oh.slice(1).forEach(([e])=>{t[e]=q.rows.filter(t=>t.status===e).length});let n=Pm();e.innerHTML=`
    <div class="dz-bar">
      <div class="dz-filter">
        ${oh.map(([e,n])=>`<button data-filter="${e}" class="${q.filter===e?`active`:``}">${G(n)}${t[e]?`<span class="dz-pillnum">${t[e]}</span>`:``}</button>`).join(``)}
      </div>
    ...
    <div class="dz-tablewrap">
      <table class="dz-table">
        ...
        <tbody id="dz-tbody">${n.length?n.map(Im).join(``):`<tr><td colspan="6"><div class="dz-when" style="padding:14px 4px">${q.search.trim()?`No deliverables match your search.`:`No deliverables with status “${G(sh[q.filter]||q.filter)}”.`}</div></td></tr>`}</tbody>
      </table>
    </div>`,Bm(),e.querySelector(`.dz-filter`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-filter]`);t&&(q.filter=t.dataset.filter,Vm())}),e.querySelector(`#dz-search`)?.addEventListener(`input`,e=>{q.search=e.target.value,Hm()}),e.querySelector(`#dz-tbody`)?.addEventListener(`click`,Um)}function Hm(){let e=document.getElementById(`dz-tbody`);if(!e)return;let t=Pm();e.innerHTML=t.length?t.map(Im).join(``):`<tr><td colspan="6"><div class="dz-when" style="padding:14px 4px">No deliverables match your search.</div></td></tr>`,Bm();let n=document.querySelector(`.dz-count`);n&&(n.textContent=`${t.length} of ${q.rows.length}`)}function Um(e){let t=e.target.closest(`[data-act="copy"]`);if(t){e.stopPropagation();let n=t.dataset.url;if(!n){K(`No link on this deliverable yet.`,`warning`);return}navigator.clipboard?.writeText(n).then(()=>{t.textContent=`Copied!`,setTimeout(()=>{t.textContent=`Copy link`},1500)},()=>K(`Could not copy the link.`,`error`));return}let n=e.target.closest(`[data-act="open"]`),r=e.target.closest(`[data-open-guid]`),i=n?.dataset.guid||r?.dataset.openGuid;i&&th(i)}function Wm(e){let t=document.createElement(`div`);t.className=`dz-drawer-scrim`;let n=document.createElement(`aside`);n.className=`dz-drawer`,n.setAttribute(`role`,`dialog`),n.setAttribute(`aria-label`,e),n.innerHTML=`
    ...
    </div>`}function qm(e,t,n,{hidden:r=!1}={}){let i=`${t}: `,a=e[0]?.group||``;return`
      <div class="dz-cblock" data-group="${G(a)}" data-heading="${G(t)}"${r?` hidden`:``}>
        <h4 class="dz-cblock__title">${G(t)}</h4>
        ${e.map(e=>Km(e,n,e.label.startsWith(i)?e.label.slice(i.length):e.label)).join(``)}
      </div>`}function Jm(e,t){let n=(e.questions||[]).filter(e=>e.audience===`am`),r=n.filter(e=>!e.group).map(e=>Km(e,t)).join(``),i=wr(e);if(i){let n=Tr(e).filter(e=>e.audience===`am`);if(!n.length)return r;let a=Or(e,t);for(let e=1;e<=a;e++){let a=Er(n,i,e);r+=qm(a,dh(a,`${i}${e}`),t)}let o=dh(Er(n,i,a+1),`${i}${a+1}`);return r+=`<button class="dz-btn--ghost dz-addblock" type="button" data-repeat="${G(i)}" data-section-id="${G(e.id)}">+ Add ${G(o)}</button>`,r}let a=[...new Set(n.filter(e=>e.group).map(e=>e.group))];if(!a.length)return r;let o=null;return r+=a.map((e,r)=>{let i=n.filter(t=>t.group===e),a=dh(i,e),s=r===0||uh(i,t);return!s&&o===null&&(o={key:e,heading:a}),qm(i,a,t,{hidden:!s})}).join(``),r+=`<button class="dz-btn--ghost dz-addblock" type="button"${o?``:` hidden`}>${o?`+ Add ${G(o.heading)}`:``}</button>`,r}function Ym(e,t){let n=(e?.sections||[]).filter(e=>(e.questions||[]).some(e=>e.audience===`am`)),r=n.length>1;return`
    ${r?`
    ...
    ${r?`
      ...
      </div>`:``}).join(``)}function Qm(e,t){if(!t)return``;let n=(e?.sections||[]).flatMap(e=>(e.questions||[]).filter(e=>e.audience!==`am`)).map(e=>({q:e,val:Zn(e,t.answers||{})})).filter(({val:e})=>e);return`
    ...
    </div>`}function $m(e,t){let n=Array.isArray(t)&&t.length?t[t.length-1]:null;return n?Qm(e,n):``}function eh(e,t){return(Array.isArray(t)?t.slice().reverse():[]).map(t=>Qm(e,t)).join(``)}async function th(e){let{drawer:t,close:n}=Wm(`Deliverable`),r=t.querySelector(`#dz-dbody`),i=t.querySelector(`#dz-dfoot`),a;try{let t=await fetch(`${ih()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}`),n=await t.json().catch(()=>({}));if(!t.ok||!n.ok)throw Error(n.error||`load failed`);a=n.data||{}}catch{r.innerHTML=lh(`error`,`Could not load this deliverable. Please try again.`);return}let o=q.rows.find(t=>t.guid===e)?.owner_name||q.clients.find(e=>e.guid===a.owner_guid)?.caption||`Client`;t.querySelector(`#dz-dtitle`).textContent=`${o} · ${a.template?.name||`Deliverable`}`;let s=a.status===`DRAFT`||a.status===`SENT`,c=Number(a.revision)>1?` · Rev ${a.revision}`:``,l=a.status===`SUBMITTED`?`Returned ${a.submitted_tz?k(a.submitted_tz):``}${a.locked_tz?` · Finalized`:``}${c}`:a.status===`SENT`?`Sent ${a.sent_tz?k(a.sent_tz):``} to ${a.recipient_email||``} · awaiting approval${c}`:`${sh[a.status]||a.status}${c}`;if(t.querySelector(`#dz-dsub`).textContent=l,!s){r.innerHTML=tr(a)+Zm(a.template,a.content||{},a.answers||{})+eh(a.template,a.rounds),a.status===`SUBMITTED`&&!a.locked_tz?(i.innerHTML=`<button class="dz-btn" type="button" id="dz-revise">Revise plan</button>
        <span class="dz-note" style="margin:0">Archives this review and reopens authoring. The client approves the revised plan on a fresh link.</span>`,i.querySelector(`#dz-revise`).addEventListener(`click`,async t=>{let r=t.target;r.disabled=!0;try{let t=await fetch(`${ih()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}/revise`,{method:`POST`}),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`revise failed`);K(`Reopened for revision. The last review is pinned above the form.`,`success`),n(),await zm(),th(e)}catch(e){K(e.message||`Could not revise.`,`error`),r.disabled=!1}})):i.innerHTML=``;return}r.innerHTML=`
    ${$m(a.template,a.rounds)}
    ${a.url?`
    ...
    <div id="dz-author-form">${Ym(a.template,a.content||{})}</div>`,i.innerHTML=`
    ...
    <button class="dz-btn--ghost dz-btn--danger" type="button" id="dz-cancel">Cancel deliverable</button>`,r.querySelector(`#dz-copylink`)?.addEventListener(`click`,e=>{let t=r.querySelector(`.dz-linkrow input`);navigator.clipboard?.writeText(t?.value||``).then(()=>{e.target.textContent=`Copied!`,setTimeout(()=>{e.target.textContent=`Copy link`},1500)},()=>K(`Could not copy the link.`,`error`))});let u=async()=>{let t=er(r.querySelector(`#dz-author-form`)),i=await fetch(`${ih()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}`,{method:`PUT`,headers:{"Content-Type":`application/json`},body:JSON.stringify({content:t})}),a=await i.json().catch(()=>({}));if(!i.ok||!a.ok){if(i.status===409)return K(a.error||`This deliverable is no longer editable.`,`warning`),n(),await zm(),!1;throw Error(a.error||`save failed`)}return!0};Xm(r.querySelector(`#dz-author-form`),a.template,()=>{u().catch(()=>K(`Autosave failed. Use Save draft.`,`warning`))}),i.querySelector(`#dz-preview`).addEventListener(`click`,async e=>{let t=e.target.closest(`button`);if(!a.url){K(`No link available for this deliverable.`,`warning`);return}t.disabled=!0;try{await u()&&window.open(a.url,`_blank`)}catch(e){K(e.message||`Could not save before previewing.`,`error`)}t.disabled=!1}),i.querySelector(`#dz-save`).addEventListener(`click`,async e=>{let t=e.target;t.disabled=!0;try{await u()&&K(`Draft saved.`,`success`)}catch(e){K(e.message||`Could not save.`,`error`)}t.disabled=!1}),i.querySelector(`#dz-send`).addEventListener(`click`,async t=>{let r=t.target,a=(i.querySelector(`#dz-email`)?.value||``).trim();if(!a){K(`Enter the recipient email first.`,`warning`);return}r.disabled=!0;try{if(!await u())return;let t=await fetch(`${ih()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}/send`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({recipient_email:a})}),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`send failed`);K(r.data?.emailQueued?`Sent. ${a} will receive the review link.`:`Link created, but the email could not be queued. Copy the link and send it yourself.`,r.data?.emailQueued?`success`:`warning`),n(),await zm()}catch(e){K(e.message||`Could not send.`,`error`),r.disabled=!1}}),i.querySelector(`#dz-cancel`).addEventListener(`click`,async t=>{let r=t.target;r.disabled=!0;try{let t=await fetch(`${ih()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}/cancel`,{method:`POST`}),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`cancel failed`);K(`Deliverable cancelled. Its link no longer works.`,`info`),n(),await zm()}catch(e){K(e.message||`Could not cancel.`,`error`),r.disabled=!1}})}var nh,rh,G,ih,K,ah,oh,sh,q,ch,lh,uh,dh,fh,ph=o((()=>{D(),nh=u(f(),1),m(),or(),Mr(),Zt(),rh={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},G=e=>String(e??``).replace(/[&<>"']/g,e=>rh[e]),ih=()=>E.getConfig?.().BASE_URL||``,K=(e,t=`info`)=>{try{nh.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},ah=`managed_client`,oh=[[`ALL`,`All`],[`DRAFT`,`In progress`],[`SENT`,`Awaiting approval`],[`SUBMITTED`,`Returned`],[`CANCELLED`,`Cancelled`]],sh=Object.fromEntries(oh),q={rows:[],clients:[],templates:[],filter:`ALL`,search:``},ch=!1,lh=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,uh=(e,t)=>e.some(e=>{let n=t?.[e.id];return Array.isArray(n)?n.length>0:!!(n&&String(n).trim())}),dh=(e,t)=>{let n=String(e[0]?.label||``).match(/^(.+?):\s/);return n?n[1]:t.toUpperCase()},fh={render:Lm,init:Rm}})),mh=c({default:()=>Eh,init:()=>bh,render:()=>yh});function hh(e){return e.status===`SENT`?{key:`PENDING`,label:`Pending your review`}:e.status===`SUBMITTED`&&e.signed?{key:`APPROVED`,label:`Approved`}:e.status===`SUBMITTED`?{key:`RETURNED`,label:`Returned with edits`}:null}function gh(){if(nr(),wh)return;let e=document.createElement(`style`);e.id=`m5t-deliverables-client-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),wh=!0}function _h(e,t){return!e.url&&t.key===`PENDING`?``:t.key===`PENDING`?`<button class="dc-act" type="button" data-url="${Sh(e.url)}">Review</button>`:t.key!==`APPROVED`&&e.url&&e.editability?.editable?`<button class="dc-act" type="button" data-url="${Sh(e.url)}">Update your review</button>`:``}function vh(e){let t=hh(e);return`<tr>
    <td><span class="dc-name">${Sh(e.template_name||e.kind||`Deliverable`)}</span>${Number(e.revision)>1?`<span class="dc-when" style="margin-left:6px">Rev ${Sh(e.revision)}</span>`:``}</td>
    <td><span class="dc-badge" data-s="${Sh(t.key)}">${Sh(t.label)}</span></td>
    <td class="dc-when">${e.sent_tz?Sh(k(e.sent_tz)):`—`}</td>
    <td class="dc-when">${e.submitted_tz?Sh(k(e.submitted_tz)):`—`}</td>
    <td class="dc-actcell">${_h(e,t)}</td>
  </tr>`}function yh(){return gh(),`
    <div class="page-shell" data-section="deliverables-client">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="dc-body">${Th(`hourglass_empty`,`Loading…`)}</div>
      </div>
    </div>`}async function bh(){let e=document.getElementById(`dc-body`);if(!e)return;let t;try{let e=await fetch(`${Ch()}/m5t/v5/m5mDeliverable`),n=await e.json().catch(()=>({}));if(!e.ok||!n.ok)throw Error(`load failed`);t=(n.data||[]).filter(e=>hh(e))}catch{e.innerHTML=Th(`error`,`Could not load your deliverables. Please try again.`);return}if(!t.length){e.innerHTML=Th(`approval`,`Nothing to review right now.`);return}e.innerHTML=`
    <div class="dc-tablewrap">
      <table class="dc-table">
        <thead><tr><th>Deliverable</th><th>Status</th><th>Received</th><th>Returned</th><th></th></tr></thead>
        <tbody id="dc-tbody">${t.map(vh).join(``)}</tbody>
      </table>
    </div>`,e.querySelector(`#dc-tbody`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-url]`);t?.dataset.url&&window.open(t.dataset.url,`_blank`)})}var xh,Sh,Ch,wh,Th,Eh,Dh=o((()=>{D(),or(),xh={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Sh=e=>String(e??``).replace(/[&<>"']/g,e=>xh[e]),Ch=()=>E.getConfig?.().BASE_URL||``,wh=!1,Th=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,Eh={render:yh,init:bh}})),Oh=c({init:()=>zh,render:()=>Rh});function kh(e){let t=og(e);if(!t)return``;try{return new Date(t).toLocaleDateString([],{month:`short`,day:`numeric`,year:`numeric`})}catch{return``}}function Ah(e){let t=og(e);if(!t)return``;try{let n=Date.now()-new Date(t).getTime();if(n<0)return kh(e);let r=n/36e5;return r<1?`${Math.max(1,Math.floor(n/6e4))}m ago`:r<24?`${Math.floor(r)}h ago`:r<48?`Yesterday`:kh(e)}catch{return kh(e)}}function jh(){if(Qh)return;let e=document.createElement(`style`);e.id=`m5t-leads-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Qh=!0}function Mh(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function Nh(e){return`<select class="leads-status" data-id="${Y(e.id)}" data-act="status" onclick="event.stopPropagation()">
    ${eg.map(t=>`<option value="${t}" ${e.status===t?`selected`:``}>${t[0]}${t.slice(1).toLowerCase()}</option>`).join(``)}
  </select>`}function Ph(e){let t=J.magnetNames[e.magnet_guid]||e.lead_source||`—`,n=e.phone?String(e.phone).replace(/[^\d+]/g,``):``;return`
    <tr class="leads-row${String(e.id)===J.selectedId?` selected`:``}" data-id="${Y(e.id)}" tabindex="0" role="button" aria-label="Open lead ${Y(ig(e))}">
      <td>
        <span class="leads-who">
          <span class="leads-avatar">${Y(ag(e))}</span>
          <span><span class="leads-name">${Y(ig(e))}</span>${e.company?`<span class="leads-sub">${Y(e.company)}</span>`:``}</span>
        </span>
      </td>
      ...
      <td>${J.canManage?Nh(e):`<span class="leads-badge" data-s="${Y(e.status)}">${Y(e.status)}</span>`}</td>
      <td class="num leads-muted" style="white-space:nowrap" title="${Y(kh(e))}">${sg(e)?`<span class="leads-fresh" title="Captured in the last 24 hours"></span>`:``}${Y(Ah(e))}</td>
      <td class="leads-open" aria-hidden="true"><span class="material-symbols-outlined">chevron_right</span></td>
    </tr>`}function Fh(e){let t=J.search.trim().toLowerCase();if(!t)return!0;let n=[e.first_name,e.last_name,e.email,e.company,e.phone,e.product_interest,J.magnetNames[e.magnet_guid],J.book?J.ownerNames[e.owner_guid]:``,e.utm_campaign,e.utm_source].filter(Boolean).join(` `).toLowerCase();return t.split(/\s+/).every(e=>n.includes(e))}function Ih(){return J.leads.filter(e=>(J.filter===`ALL`||e.status===J.filter)&&cg(e)&&Fh(e)&&lg(e))}function Lh(){if(!J.day)return``;let e=J.day;try{e=new Date(`${J.day}T00:00:00Z`).toLocaleDateString([],{month:`short`,day:`numeric`,year:`numeric`,timeZone:`UTC`})}catch{}return`<button class="leads-daychip" type="button" id="leads-dayclear" title="Show every day again">
  ...
  </button>`}function Rh(){jh(),Xi();let e=M();return`
    <div class="page-shell" data-section="leads">
      ...
      <div class="dashboard-card" style="margin-bottom:24px">
        ...
        <div class="card-body" id="pf-picker-body">${Mh(`hourglass_empty`,`Loading…`)}</div>
      </div>`:``}
      <div class="page-header">
        <div class="page-header-text">
      ...
      <div class="leads-layout">
        <div class="dashboard-card">
          <div class="card-body" id="leads-body">
            ${Mh(`hourglass_empty`,`Loading leads…`)}
          </div>
        </div>
        <aside class="dashboard-card leads-panel" aria-label="Lead detail">
          <div class="card-body" id="leads-panel-body">${ug()}</div>
        </aside>
      </div>
    </div>`}async function zh(){document.getElementById(`leads-export`)?.addEventListener(`click`,Xh);let e=fa(ma.LEADS_SEARCH),t=fa(ma.LEADS_DAY);if((e||t)&&(J.search=e||``,J.day=t||``,J.filter=`ALL`,J.clientFilter=`ALL`,J.shown=$h),M()){let e=e=>Number(e||0).toLocaleString();na(rg(),[{h:`Leads · wk`,td:t=>e(t.leads.this_week)},{h:`Leads · total`,td:t=>e(t.leads.total)},{h:`Compl.`,td:e=>e.completion_rate==null?`—`:`${e.completion_rate}%`}],{emptyMsg:`No client orgs yet. Create one under Clients first.`})}await Bh()}async function Bh(){let e=document.getElementById(`leads-body`);if(e){try{let[e,t]=await Promise.all([fetch(`${rg()}/m5t/v5/m5mLead`),fetch(`${rg()}/m5t/v5/magnets`).catch(()=>null)]),n=await e.json().catch(()=>({}));if(!n.ok)throw Error(n.message||`Failed to load`);if(J.leads=(n.data||[]).filter(e=>![`DELETED`,`ARCHIVED`].includes(e.status)),J.total=Number.isFinite(n.total)?n.total:J.leads.length,J.truncated=J.total>(n.data||[]).length,J.book=M(),J.ownerNames=J.book?ra():{},t){let e=await t.json().catch(()=>({}));J.magnetNames={},(e.magnets||[]).forEach(e=>{J.magnetNames[e.guid]=e.name,e.owner_guid&&e.owner_name&&!J.ownerNames[e.owner_guid]&&(J.ownerNames[e.owner_guid]=e.owner_name)}),J.canManage=!!e.canManage}}catch{e.innerHTML=Mh(`error`,`Could not load your leads. Please try again.`);return}Vh()}}function Vh(){let e=document.getElementById(`leads-body`),t=document.getElementById(`leads-export`);if(!e)return;if(t&&(t.style.display=J.leads.length?``:`none`),!J.leads.length){e.innerHTML=Mh(`inbox`,`No leads captured yet. Publish a magnet and share it to start collecting.`),Jh();return}let n=J.leads.filter(cg),r={ALL:n.length};eg.forEach(e=>{r[e]=n.filter(t=>t.status===e).length});let i=J.book?[...new Set(J.leads.map(e=>e.owner_guid).filter(Boolean))].map(e=>({guid:e,name:J.ownerNames[e]||e})).sort((e,t)=>e.name.localeCompare(t.name)):[];e.innerHTML=`
    <div class="leads-bar">
      <div class="leads-filter">
        ${[`ALL`,...eg].map(e=>`<button data-filter="${e}" class="${J.filter===e?`active`:``}">${e===`ALL`?`All`:e[0]+e.slice(1).toLowerCase()}${r[e]?`<span class="leads-pillnum">${r[e]}</span>`:``}</button>`).join(``)}
      </div>
      ${J.book&&i.length>1?`
    ...
    <div id="leads-tablezone"></div>`,e.querySelector(`.leads-filter`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-filter]`);t&&(J.filter=t.dataset.filter,J.shown=$h,Vh())}),e.querySelector(`#leads-clientsel`)?.addEventListener(`change`,e=>{J.clientFilter=e.target.value,J.shown=$h,Vh()}),e.querySelector(`#leads-dayclear`)?.addEventListener(`click`,()=>{J.day=``,J.shown=$h,Vh()}),e.querySelector(`#leads-search`)?.addEventListener(`input`,e=>{J.search=e.target.value,J.shown=$h,Hh()}),Hh()}function Hh(){let e=document.getElementById(`leads-tablezone`),t=document.getElementById(`leads-countlbl`);if(!e)return;let n=Ih(),r=n.slice(0,J.shown),i=J.book?8:7;t&&(t.textContent=`${n.length} of ${J.leads.length}`),e.innerHTML=`
    <div class="leads-tablewrap">
      <table class="leads-table${J.book?` leads-table--book`:``}">
        ...
        <tbody id="leads-tbody">${r.map(Ph).join(``)||`<tr><td colspan="${i}"><div class="leads-muted" style="padding:14px 4px">${J.search.trim()?`No leads match your search.`:`No ${J.filter.toLowerCase()} leads.`}</div></td></tr>`}</tbody>
      </table>
    </div>
    ${Jt({total:n.length,shown:J.shown,noun:`leads`})}
    ${J.truncated?`<div class="leads-muted" style="padding:10px 4px">Showing your ${J.leads.length.toLocaleString()} most recent leads of ${J.total.toLocaleString()}. Search and filters cover these; older leads aren't included yet.</div>`:``}`,e.querySelector(`[data-action="list-expand"]`)?.addEventListener(`click`,()=>{J.shown+=25,Hh()}),e.querySelector(`#leads-tbody`)?.addEventListener(`change`,Uh),e.querySelector(`#leads-tbody`)?.addEventListener(`click`,e=>{if(e.target.closest(`.leads-status, a.leads-contactlink`))return;let t=e.target.closest(`.leads-row`);t&&Wh(t.dataset.id)}),e.querySelector(`#leads-tbody`)?.addEventListener(`keydown`,e=>{if(e.key!==`Enter`&&e.key!==` `)return;let t=e.target.closest(`.leads-row`);t&&e.target===t&&(e.preventDefault(),Wh(t.dataset.id))})}async function Uh(e){let t=e.target.closest(`.leads-status`);if(!t)return;let n=t.dataset.id,r=J.leads.find(e=>String(e.id)===String(n)),i=t.value,a=r?.status;try{if(!(await(await fetch(`${rg()}/m5t/v5/m5mLead/${encodeURIComponent(n)}`,{method:`PUT`,headers:{"Content-Type":`application/json`},body:JSON.stringify({status:i})})).json().catch(()=>({}))).ok)throw Error();r&&(r.status=i),ng(`Marked ${i[0]}${i.slice(1).toLowerCase()}`,`success`)}catch{t.value=a||`NEW`,ng(`Could not update status`,`danger`);return}try{String(n)===J.selectedId&&Jh(),J.filter!==`ALL`&&Vh()}catch(e){console.warn(`[leads] re-render after status change failed:`,e)}}function Wh(e){J.selectedId=String(e),document.querySelectorAll(`.leads-row`).forEach(e=>e.classList.toggle(`selected`,e.dataset.id===J.selectedId)),Jh()}function Gh(e,t){let n=Array.isArray(e.tags)?e.tags:[];for(let e=n.length-1;e>=0;e--)if(n[e]&&n[e].tag===t&&n[e].value!=null)return String(n[e].value);return``}function Kh(e){let t=Gh(e,`appointment_start`);if(!t)return null;let n=Gh(e,`appointment_status`)||`booked`,r=Gh(e,`appointment_timezone`),i=Gh(e,`appointment_event_type`),a=new Date(t),o=t;if(!Number.isNaN(a.getTime())){let e={weekday:`short`,month:`short`,day:`numeric`,hour:`numeric`,minute:`2-digit`};try{o=new Intl.DateTimeFormat(void 0,r?{...e,timeZone:r}:e).format(a)}catch{o=new Intl.DateTimeFormat(void 0,e).format(a)}}return{when:o,status:n,type:i,start:t}}function qh(e){let t=Array.isArray(e.tags)?e.tags:[],n=null;for(let e of t)if(!(!e||!e.tag)){if(e.tag===`callrail_call_id`){n={};continue}n&&e.tag.indexOf(`call_`)===0&&e.value!=null&&(n[e.tag]=String(e.value))}if(!n||!n.call_started)return null;let r=n.call_started,i=new Date(r),a=r;Number.isNaN(i.getTime())||(a=new Intl.DateTimeFormat(void 0,{weekday:`short`,month:`short`,day:`numeric`,hour:`numeric`,minute:`2-digit`}).format(i));let o=Number(n.call_duration)||0,s=o?o<60?`${o}s`:`${Math.floor(o/60)}m ${o%60}s`:``;return{when:a,dur:s,source:n.call_source||``,answered:n.call_answered||``,recording:n.call_recording_player||``,started:r}}function Jh(){let e=document.getElementById(`leads-panel-body`);if(!e)return;let t=J.leads.find(e=>String(e.id)===J.selectedId),n=document.querySelector(`.leads-layout`);if(n&&(n.dataset.hasSelection=t?`true`:`false`),!t){e.innerHTML=ug();return}let r=J.magnetNames[t.magnet_guid]||t.lead_source||`—`,i=Kh(t),[a,o]=i?fg[i.status]||fg.booked:[``,``],s=qh(t),c=(e,t)=>t?`<div class="leads-det__field"><span class="leads-det__k">${Y(e)}</span><span class="leads-det__v">${Y(t)}</span></div>`:``;e.innerHTML=`
    <div class="leads-det__head">
      <span class="leads-avatar leads-det__ava">${Y(ag(t))}</span>
      <div>
        <div class="leads-det__name">${Y(ig(t))}</div>
    ...
    ${J.canManage?`
      ...
      </div>`:``}`,e.querySelector(`[data-action="delete-lead"]`)?.addEventListener(`click`,Yh)}async function Yh(e){let t=e.currentTarget?.dataset?.id,n=J.leads.find(e=>String(e.id)===String(t));if(!n)return;let r=ig(n);if(!window.confirm(`Remove ${r} from your leads?\n\nThis takes the lead out of your dashboard, lists and exports. The record is kept in our system, and this address can be captured again if they come back through one of your magnets.`))return;let i=e.currentTarget;i.disabled=!0;try{let e=await fetch(`${rg()}/m5t/v5/m5mLead/${encodeURIComponent(t)}`,{method:`DELETE`}),n=await e.json().catch(()=>({}));if(!e.ok||!n.ok)throw Error(n.error||`HTTP ${e.status}`);J.leads=J.leads.filter(e=>String(e.id)!==String(t)),J.selectedId=null,ng(`${r} removed`,`success`),Vh()}catch{i.disabled=!1,ng(`Could not remove that lead. Please try again.`,`danger`)}}function Xh(){let e=Ih();if(!e.length){ng(`Nothing to export`,`info`);return}let t=[`first_name`,`last_name`,`email`,`phone`,`company`,`product_interest`,`lead_source`,`utm_source`,`utm_medium`,`utm_campaign`,`gclid`,`status`],n=e=>`"${String(e??``).replace(/"/g,`""`)}"`,r=[[...J.book?[`client`]:[],...t,`magnet`,`captured`,`appointment`,`last_call`].join(`,`)];for(let i of e){let e=Kh(i),a=qh(i);r.push([...J.book?[n(J.ownerNames[i.owner_guid]||``)]:[],...t.map(e=>n(i[e])),n(J.magnetNames[i.magnet_guid]||i.lead_source||``),n(kh(i)),n(e?`${e.start} (${e.status})`:``),n(a?`${a.started} (${a.answered===`no`?`missed`:`answered`})`:``)].join(`,`))}let i=new Blob([r.join(`
`)],{type:`text/csv`}),a=URL.createObjectURL(i),o=document.createElement(`a`);o.href=a,o.download=`leads-${J.filter.toLowerCase()}.csv`,document.body.appendChild(o),o.click(),o.remove(),URL.revokeObjectURL(a),ng(`Exported ${e.length} lead${e.length===1?``:`s`}`,`success`)}var Zh,Qh,J,$h,eg,tg,Y,ng,rg,ig,ag,og,sg,cg,lg,ug,dg,fg,pg=o((()=>{D(),Zh=u(f(),1),m(),ua(),ga(),Zt(),Qh=!1,J={leads:[],magnetNames:{},filter:`ALL`,canManage:!1,book:!1,ownerNames:{},clientFilter:`ALL`,search:``,shown:25,selectedId:null,day:``,total:0,truncated:!1},$h=25,eg=[`NEW`,`WORKING`,`QUALIFIED`,`UNQUALIFIED`,`CONVERTED`,`EXPORTED`],tg={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Y=e=>String(e??``).replace(/[&<>"']/g,e=>tg[e]),ng=(e,t=`info`)=>{try{Zh.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},rg=()=>E.getConfig?.().BASE_URL||``,ig=e=>[e.first_name,e.last_name].filter(Boolean).join(` `).trim()||e.email||`(no name)`,ag=e=>(ig(e).split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``)||`?`).toUpperCase(),og=e=>e.created?.created_tz||``,sg=e=>{let t=og(e);if(!t)return!1;try{return Date.now()-new Date(t).getTime()<864e5}catch{return!1}},cg=e=>J.clientFilter===`ALL`||e.owner_guid===J.clientFilter,lg=e=>!J.day||og(e).slice(0,10)===J.day,ug=()=>Mh(`touch_app`,`Click to see lead information.`),dg=()=>`
  <colgroup>
    <col style="width:22%">
  ...
  </colgroup>`,fg={booked:[`QUALIFIED`,`Booked`],rescheduled:[`WORKING`,`Rescheduled`],canceled:[`CANCELED`,`Canceled`]}})),mg=c({init:()=>bg,render:()=>yg,tableHTML:()=>vg});function hg(){if(Ag)return;let e=document.createElement(`style`);e.id=`m5t-analytics-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Ag=!0}function gg(e,t){let n=t>0?Math.round(e/t*100):0,r=`${wg(e)} of ${wg(t)} leads for the busiest magnet`;return`<svg class="anl-bar" width="72" height="8" viewBox="0 0 72 8" role="img"
      aria-label="${Eg(r)}">
      <title>${Eg(r)}</title>
    ...
    </svg>`}function _g(e,t,n){let r=Mg[e.status]?`<span class="anl-chip">${Eg(Mg[e.status])}</span>`:``,i=n&&e.owner_name?`<span class="anl-sub">${Eg(e.owner_name)}</span>`:``,a=e.ab_tests&&(e.ab_tests.running||e.ab_tests.completed)?`${wg(e.ab_tests.running)} / ${wg(e.ab_tests.completed)}`:`—`;return`<tr data-guid="${Eg(e.magnet_guid)}">
    <td><a class="anl-name" href="#/magnets">${Eg(e.name)}</a>${r}${i}</td>
    <td>${gg(Number(e.leads?.window||0),t)}</td>
    <td>${wg(e.leads?.window)}</td>
    <td>${wg(e.opens)}</td>
    <td>${Dg(e.engagement_rate)}</td>
    <td>${Dg(e.completion_rate)}</td>
    <td>${Dg(e.lead_rate)}</td>
    <td>${a}</td>
  </tr>`}function vg(e,t){let n=e.reduce((e,t)=>Math.max(e,Number(t.leads?.window||0)),0);return`<div class="anl-wrap"><table class="anl-table">
    <thead><tr>
      <th>Magnet</th><th></th><th>Leads</th><th>Opens</th>
      <th>Engaged</th><th>Completed</th><th>Leads / Open</th><th>A/B run / done</th>
    </tr></thead>
    <tbody>${e.map(e=>_g(e,n,t)).join(``)}</tbody>
  </table></div>`}function yg(){return M()?ta({section:`analytics`,title:`Analytics`,description:`Pick a client to see how their magnets are performing.`}):(hg(),`
    <div class="page-shell" data-section="analytics">
      <div class="page-header">
        ...
        <div class="chart-controls" id="anl-periods">
          ${Og.map(e=>`<button class="chart-period-btn${e.d===kg?` active`:``}" type="button" data-days="${e.d}">${e.label}</button>`).join(``)}
        </div>
      </div>
    ...
    </div>`)}async function bg(){if(M())return na(Sg(),[{h:`Leads`,td:e=>wg(e.leads.window??e.leads.total)},{h:`Opens`,td:e=>wg(e.opens)},{h:`Completed`,td:e=>Dg(e.completion_rate)}]);hg(),await xg();let e=document.getElementById(`anl-periods`);e&&e.addEventListener(`click`,t=>{let n=t.target.closest(`.chart-period-btn`);n&&(kg=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>{e.classList.toggle(`active`,Number(e.dataset.days)===kg)}),xg())})}async function xg(){let e=document.getElementById(`anl-body`),t=document.getElementById(`anl-range`);if(!e)return;e.innerHTML=jg(`hourglass_empty`,`Loading magnet performance…`);let n=new Date,r=Cg(n),i=Cg(new Date(n.getTime()-(kg-1)*864e5));t&&(t.textContent=`${i} → ${r}`);let a;try{let e=await fetch(`${Sg()}/m5t/v5/dashboard/stats?from=${i}&to=${r}&group_by=magnet`);if(a=await e.json(),!e.ok||!a?.ok)throw Error(a?.error||`HTTP ${e.status}`)}catch(t){e.innerHTML=jg(`cloud_off`,`Could not load analytics. Refresh to try again.`),console.error(`[analytics] load failed:`,t);return}let o=(e,t)=>{let n=document.getElementById(e);n&&(n.textContent=t)};o(`anl-leads`,wg(a.by_day?.reduce((e,t)=>e+Number(t.leads||0),0))),o(`anl-opens`,wg(a.funnel?.opens)),o(`anl-completion`,Dg(a.funnel?.completion_rate)),o(`anl-active`,wg(a.magnets?.active));let s=Array.isArray(a.by_magnet)?a.by_magnet:[];if(!s.length){e.innerHTML=jg(`widgets`,`No magnets yet. Create one and its numbers will show up here.`);return}let c=s.some(e=>Number(e.opens||0)>0||Number(e.leads?.window||0)>0),l=new Set(s.map(e=>e.owner_guid)).size>1;e.innerHTML=(a.by_magnet_truncated?`<p class="anl-warn">Showing the top ${s.length} magnets by leads. You have more than that, so this list is not complete.</p>`:``)+(c?``:`<p class="anl-warn">No opens or leads for any magnet in this period. Try a longer range.</p>`)+vg(s,l);let u=e.querySelector(`.anl-table tbody`);u&&Yt(u,{rowSelector:`tr`,noun:`magnets`,controlHost:e.querySelector(`.anl-wrap`)||e})}var Sg,Cg,wg,Tg,Eg,Dg,Og,kg,Ag,jg,Mg,Ng=o((()=>{D(),ua(),ba(),Zt(),Sg=()=>E.getConfig?.().BASE_URL||``,Cg=e=>e.toISOString().slice(0,10),wg=e=>Number(e||0).toLocaleString(),Tg={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Eg=e=>String(e??``).replace(/[&<>"']/g,e=>Tg[e]),Dg=e=>e==null?`—`:`${e}%`,Og=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],kg=30,Ag=!1,jg=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,Mg={DRAFT:`Draft`,DISABLED:`Paused`}})),Pg=c({default:()=>t_,init:()=>Lg,render:()=>Ig});function Fg(){if(Sa(),Zg)return;let e=document.createElement(`style`);e.id=`m5t-adperf-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),Zg=!0}function Ig(){return M()?ta({section:`ad-performance`,title:`Ad Performance`,description:`Pick a client to view their spend and lead performance.`}):(Fg(),`
    <div class="page-shell" data-section="ad-performance">
      <div class="page-header">
        ...
        <div class="chart-controls" id="adperf-periods">
          ${$g.map(e=>`<button class="chart-period-btn${e.d===e_?` active`:``}" type="button" data-days="${e.d}">${e.label}</button>`).join(``)}
        </div>
      </div>
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="adperf-body">
          ${Qg(`hourglass_empty`,`Loading ad performance…`)}
        </div>
      </div>
    ...
    </div>`)}async function Lg(){if(M()){let e=e=>Number(e||0).toLocaleString();return na(Gg(),[{h:`Spend`,td:t=>t.ad?`$${e(t.ad.spend)}`:`—`},{h:`Clicks`,td:t=>t.ad?e(t.ad.clicks):`—`},{h:`Leads`,td:t=>e(t.leads.window??t.leads.total)},{h:`CPL`,td:t=>t.ad?.cost_per_lead==null?`—`:`$${e(t.ad.cost_per_lead)}`}],{emptyMsg:`No client orgs yet. Connect a client ad account to see spend here.`})}let e=document.getElementById(`adperf-periods`);e?.addEventListener(`click`,t=>{let n=t.target.closest(`button[data-days]`);n&&(e_=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>e.classList.toggle(`active`,Number(e.dataset.days)===e_)),Rg())}),document.getElementById(`adperf-sources`)?.addEventListener(`click`,e=>{let t=e.target.closest(`tr[data-campaign]`);t&&(da(ma.LEADS_SEARCH,t.dataset.campaign),!e.target.closest(`a`)&&(window.location.hash=`#/leads`))}),await Rg()}async function Rg(){let e=Gg(),t=document.getElementById(`adperf-body`),n=document.getElementById(`adperf-range`),r=new Date,i=Kg(r),a=Kg(new Date(r.getTime()-(e_-1)*864e5));n&&(n.textContent=`${a} → ${i}`);let o,s=[],c=null,l=null,u=null;try{let[t,n,r,d,f]=await Promise.all([fetch(`${e}/m5t/v5/coreAdMetrics/overlay?from=${a}&to=${i}`),fetch(`${e}/m5t/v5/coreAdAccount`),fetch(`${e}/m5t/v5/coreAdMetrics/by-source?from=${a}&to=${i}`),tc().catch(()=>null),fetch(`${e}/m5t/v5/coreAdMetrics/conversions?from=${a}&to=${i}`).then(e=>e.json()).catch(()=>null)]);if(!t.ok)throw Error(`HTTP ${t.status}`);o=await t.json(),s=((await n.json().catch(()=>({})))?.data||[]).filter(e=>e.status!==`DELETED`),c=await r.json().catch(()=>null),l=d,u=f}catch(e){console.warn(`[ad-performance] load failed:`,e),t&&(t.innerHTML=Qg(`error`,`Could not load ad performance. Please try again.`));return}zg(l,s),Bg(u);let d=o.totals||{};if(Hg(`adperf-spend`,qg(d.spend)),Hg(`adperf-clicks`,Jg(d.clicks)),Hg(`adperf-leads`,Jg(d.magnet_leads)),Hg(`adperf-cpl`,qg(d.cost_per_lead)),Vg(c),!s.length){t.innerHTML=Qg(`link_off`,`No ad account connected yet. Your agency can connect Google Ads or Local Services Ads to see performance here.`);return}if(!o.connected){t.innerHTML=Qg(`cloud_sync`,`Account connected. No ad data for this range yet. Metrics appear here once they sync.`);return}let f=o.rows||[];t.innerHTML=Ug(f),Wg(f)}function zg(e,t){let n=document.getElementById(`adperf-conn`);if(!n)return;if(!e||e.denied){n.innerHTML=``;return}let r=wc.google_ads,i=Qs(e.providers,`google_ads`),a=$s(r,i,!!e.configured?.google_ads),o=vc()?` <a href="#/settings/integrations/${r.urlSlug}">Manage</a>`:``;if(a===`connected`||a===`broken`){pc(),n.innerHTML=`<span>Google Ads</span> ${ec(a,i)}${o}`;return}if(t.length){n.innerHTML=`<span>Google Ads</span> <span>${t.length} associated account${t.length===1?``:`s`}</span>`;return}n.innerHTML=``}function Bg(e){let t=document.getElementById(`adperf-conv-card`),n=document.getElementById(`adperf-conv-body`),r=document.getElementById(`adperf-stats`),i=document.getElementById(`adperf-conv-stat`);if(i&&i.remove(),!t||!n)return;if(!e?.ok||!e.has_selection){t.style.display=`none`;return}if(r){let t=document.createElement(`div`);t.innerHTML=_a({icon:`flag`,label:`Ads Conversions`,value:Jg(e.totals?.conversions),sub:`Reported by Google Ads`,href:`#/settings/integrations/google-ads`});let n=t.firstElementChild;n.id=`adperf-conv-stat`,r.appendChild(n)}let a=e.actions||[];if(t.style.display=``,!a.length){n.innerHTML=Qg(`search_off`,`No conversions in this range for the selected actions.`);return}let o=a.some(e=>e.value>0);n.innerHTML=`
      ...
      <table class="adperf-srctable">
        ...
        <tbody>${a.map(e=>`
          <tr>
            ...
            <td class="num"><b>${Jg(e.conversions)}</b></td>
            ${o?`<td class="num">${qg(e.value)}</td>`:``}
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>
    <p class="adperf-note">Counted actions are chosen on the Google Ads integration page. These are Google's own numbers and may overlap magnet leads, so they are never added together.</p>`}function Vg(e){let t=document.getElementById(`adperf-sources`),n=document.getElementById(`adperf-src-summary`);if(!t)return;let r=e&&Array.isArray(e.rows)?e.rows:[];if(!r.length){n&&(n.textContent=``),t.innerHTML=Qg(`search_off`,`No leads in this range yet. Campaigns appear here as tagged leads (?utm_campaign=…) come in.`);return}let i=e.total_leads||r.reduce((e,t)=>e+(t.leads||0),0),a=e.tracked_leads||0;n&&(n.textContent=`${Jg(a)} of ${Jg(i)} leads tagged to a source`);let o=e=>e.campaign&&e.campaign!==`(none)`?Xg(e.campaign):e.source&&e.source!==`(direct)`?Xg(e.source):`<span class="adperf-src-untracked">Untracked / direct</span>`,s=e=>e.campaign&&e.campaign!==`(none)`?e.campaign:e.source&&e.source!==`(direct)`?e.source:``;t.innerHTML=`
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
        ...
        <tbody>${r.map(e=>{let t=s(e);return`
          <tr${t?` data-campaign="${Xg(t)}" class="adperf-srcrow--link"`:``}>
            <td>${t?`<a class="adperf-srclink" href="#/leads" title="Show the leads from ${Xg(t)}">${o(e)}</a>`:o(e)}</td>
            <td class="adperf-src-sub">${Xg(e.source||`—`)}${e.medium?` · ${Xg(e.medium)}`:``}</td>
            <td class="num">${e.paid_clicks?Jg(e.paid_clicks):`<span class="adperf-src-sub">—</span>`}</td>
            <td class="num"><b>${Jg(e.leads)}</b></td>
          </tr>`}).join(``)}
        </tbody>
      </table>
    </div>`;let c=t.querySelector(`.adperf-srctable tbody`);c&&Yt(c,{rowSelector:`tr`,noun:`sources`,controlHost:t.querySelector(`.adperf-srcwrap`)||t})}function Hg(e,t){let n=document.getElementById(e);n&&(n.textContent=t)}function Ug(e){return Ca({rows:e,xLabel:e=>String(e.date||``).slice(5),left:{value:e=>e.spend||0,label:`Ad Spend`,fmt:e=>`$${Oa(e)}`},right:{value:e=>e.magnet_leads||0,label:`Magnet Leads`},ariaLabel:`Ad spend versus magnet leads over time`,pointLabel:e=>`${e.date}: ${qg(e.spend||0)} spend, ${Jg(e.magnet_leads||0)} leads`,hitData:e=>({day:e.date})})}function Wg(e){let t=document.getElementById(`adperf-body`);return t?Ta(t,{rows:e,tipHTML:e=>`
      <div class="m5c-tip__title">${Xg(String(e.date||``))}</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--1"></i>${qg(e.spend||0)} spend</div>
      <div class="m5c-tip__row"><i class="m5c-sw m5c-sw--2"></i>${Jg(e.magnet_leads||0)} leads</div>`}):()=>{}}var Gg,Kg,qg,Jg,Yg,Xg,Zg,Qg,$g,e_,t_,n_=o((()=>{D(),ua(),kc(),ba(),Aa(),Zt(),ga(),Gg=()=>E.getConfig?.().BASE_URL||``,Kg=e=>e.toISOString().slice(0,10),qg=e=>e==null?`—`:`$`+Number(e).toLocaleString(void 0,{minimumFractionDigits:0,maximumFractionDigits:2}),Jg=e=>Number(e||0).toLocaleString(),Yg={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Xg=e=>String(e??``).replace(/[&<>"']/g,e=>Yg[e]),Zg=!1,Qg=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,$g=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],e_=30,t_={render:Ig,init:Lg}})),r_=c({default:()=>w_,init:()=>o_,parseBookingCsv:()=>__,render:()=>a_});function i_(){if(x_)return;let e=document.createElement(`style`);e.id=`m5t-adacct-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),x_=!0}function a_(){return M()?ta({section:`ad-accounts`,title:`Ad Accounts`,description:`Pick a client to see their connected ad accounts.`}):(i_(),pc(),`
...
`&&a++,n.push(r),r=``,n.some(e=>e.trim()!==``)&&t.push(n),n=[]):r+=o}if(n.push(r),n.some(e=>e.trim()!==``)&&t.push(n),t.length<2)return[];let a=t[0].map(e=>e.trim().toLowerCase()),o=a.findIndex(e=>/e-?mail/.test(e)),s=a.findIndex(e=>/phone|mobile|tel/.test(e)),c=a.findIndex(e=>/date|time|booked|appointment|start/.test(e));return o<0&&s<0?[]:t.slice(1).map(e=>({email:o>=0?(e[o]||``).trim():``,phone:s>=0?(e[s]||``).trim():``,at:c>=0?(e[c]||``).trim():``})).filter(e=>e.email||e.phone).slice(0,5e3)}function v_(e,t){let n=new Blob([e],{type:`text/csv;charset=utf-8`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.href=r,i.download=t,document.body.appendChild(i),i.click(),i.remove(),setTimeout(()=>URL.revokeObjectURL(r),4e3)}var y_,b_,x_,S_,C_,w_,T_=o((()=>{ua(),Zt(),kc(),y_={google_ads:`Google Ads`,google_lsa:`Local Services Ads`},b_=e=>{let t=String(e||``).replace(/\D/g,``);return t.length===10?`${t.slice(0,3)}-${t.slice(3,6)}-${t.slice(6)}`:String(e||``)},x_=!1,S_=[],C_=null,w_={render:a_,init:o_}})),E_=c({default:()=>J_,init:()=>P_,openInviteModal:()=>R_,render:()=>N_});function D_(e,t=`info`){try{V_.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}}function O_(){let e=E.getActiveOrg();if(e&&e.channel===H_)return{guid:e.guid,name:e.name};let t=E.getMemberships().find(e=>e.channel===H_);return t?{guid:t.orgGuid,name:t.name}:null}function k_(e){let t=e?.created_tz;if(!t)return``;let n=new Date(t);return Number.isNaN(n.getTime())?``:n.toLocaleDateString(void 0,{year:`numeric`,month:`short`,day:`numeric`})}function A_(){if(W_)return;let e=document.createElement(`style`);e.id=`m5t-clients-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),nr(),W_=!0}function j_(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function M_(e){let t=e.caption||e.guid,n=e.status||`ENABLED`;return`
    <div class="client-row">
      <span class="client-row__avatar">${X(K_(t))}</span>
      <span class="client-row__main">
        <div class="client-row__name" title="${X(t)}">${X(t)}</div>
        <div class="client-row__meta">Added ${X(k_(e.created))||`—`}</div>
      </span>
    ...
    <div class="page-shell" data-section="clients">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="clients-body">
          ${j_(`hourglass_empty`,`Loading clients…`)}
        </div>
      </div>
    </div>`}async function P_(){let e=O_();document.getElementById(`clients-add-btn`)?.addEventListener(`click`,()=>I_(e)),await F_(e)}async function F_(e){let t=document.getElementById(`clients-body`),n=document.getElementById(`clients-count`);if(t){if(!e?.guid){t.innerHTML=j_(`info`,`Client provisioning is available to reseller / agency accounts.`),n&&(n.textContent=``);return}try{let r=E.getConfig?.().BASE_URL||``,i=await fetch(`${r}/m5t/v5/acctGroup`);if(!i.ok)throw Error(`HTTP ${i.status}`);let a=await i.json(),o=(Array.isArray(a?.data)?a.data:[]).filter(t=>t.class===U_&&(!t.parent||t.parent===e.guid));if(!o.length){t.innerHTML=j_(`group_off`,`No clients yet. Add your first client to start managing magnets on their behalf.`),n&&(n.textContent=``);return}t.innerHTML=`<div class="client-list">${o.map(M_).join(``)}</div>`,Yt(t.querySelector(`.client-list`),{rowSelector:`.client-row`,noun:`clients`}),n&&(n.textContent=`${o.length} client${o.length===1?``:`s`}`),t.querySelectorAll(`.client-card__invite`).forEach(e=>{e.addEventListener(`click`,()=>{e.dataset.action===`adaccounts`?L_(e.dataset.clientGuid,e.dataset.clientName):e.dataset.action===`questionnaire`?B_(e.dataset.clientGuid,e.dataset.clientName):R_(e.dataset.clientGuid,e.dataset.clientName)})})}catch(e){console.warn(`[clients] load failed:`,e),t.innerHTML=j_(`error`,`Couldn’t load clients. Please try again.`),n&&(n.textContent=``)}}}function I_(e){if(!e?.guid){D_(`Only reseller / agency accounts can add clients.`,`error`);return}let t=document.createElement(`div`);t.className=`client-modal-scrim`;let n=document.createElement(`div`);n.className=`client-modal`,n.setAttribute(`role`,`dialog`),n.setAttribute(`aria-modal`,`true`),n.innerHTML=`
    ...
    <div class="client-modal__body" id="qm-body">${j_(`hourglass_empty`,`Loading…`)}</div>`,document.body.appendChild(r),document.body.appendChild(i),requestAnimationFrame(()=>{r.classList.add(`open`),i.classList.add(`open`)});let a=()=>{r.classList.remove(`open`),i.classList.remove(`open`),document.removeEventListener(`keydown`,o),setTimeout(()=>{r.remove(),i.remove()},200)},o=e=>{e.key===`Escape`&&a()};document.addEventListener(`keydown`,o),r.addEventListener(`click`,a),i.querySelector(`.client-modal__close`)?.addEventListener(`click`,a);let s=i.querySelector(`#qm-body`);function c(){s.innerHTML=`
      ...
      <div class="qm-status" data-status="DRAFT"><span class="material-symbols-outlined" aria-hidden="true">hourglass_top</span> ${t} — awaiting response${e.token_expiry?`, link expires ${X(z_(e.token_expiry))}`:``}</div>
      ...
      <div class="qm-status" data-status="CANCELLED"><span class="material-symbols-outlined" aria-hidden="true">block</span> Cancelled${e.cancelled_tz?` ${X(z_(e.cancelled_tz))}`:``}${e.cancelled_by_email?` by ${X(e.cancelled_by_email)}`:``}</div>
      ...
      </div>`,s.querySelector(`[data-act="done"]`)?.addEventListener(`click`,a),s.querySelector(`[data-act="resend"]`)?.addEventListener(`click`,()=>c())}function d(e,t=!1){let r=e.editability||{editable:e.status===`SUBMITTED`&&!e.locked_tz,reason:e.locked_tz?`finalized`:null,deadline:null},i=!!r.editable,o=!i&&e.status===`SUBMITTED`?r.reason===`finalized`?`<div class="qm-status" data-status="LOCKED"><span class="material-symbols-outlined" aria-hidden="true">lock</span> Finalized${e.locked_tz?` ${X(z_(e.locked_tz))}`:``}${e.locked_by_email?` by ${X(e.locked_by_email)}`:``} — no further edits.</div>`:`<div class="qm-status" data-status="LOCKED"><span class="material-symbols-outlined" aria-hidden="true">lock_clock</span> Editing closed${r.deadline?` ${X(z_(r.deadline))}`:``} — a response locks 14 days after it is submitted.</div>`:``,c=i&&r.deadline?`<p class="client-modal__sub qm-editable-hint">You can edit these answers until ${X(z_(r.deadline))}.</p>`:``;s.innerHTML=`
      <div class="qm-status" data-status="SUBMITTED"><span class="material-symbols-outlined" aria-hidden="true">task_alt</span> Submitted ${e.submitted_tz?X(z_(e.submitted_tz)):``}</div>
      ${o}
      ${tr(e)}
      <form id="qm-form">
  ...
  `,document.head.appendChild(e),uv=!0}function ev(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function tv(e,t,n){let r=e.member_role||`member`,i=e.entity_guid&&e.entity_guid===E.getEntity?.()?.guid,a=n&&!i&&Q_(t.memberRole).some(e=>e.value===r);return`
    <div class="team-row">
      <span class="team-avatar">${fv(mv(e.name||e.email))}</span>
      <span class="team-main">
        <div class="team-name">${fv(e.name||e.email)}${e.verified?``:` · <span style="color:var(--color-text-subtle,#6C7C9A);font-weight:400;">pending verification</span>`}</div>
        <div class="team-email" title="${fv(e.email)}">${fv(e.email)}</div>
      </span>
      <span class="team-role" data-role="${fv(r)}">${fv(Pe[r]?.label||r)}</span>
      ${a?`<button class="team-modal__btn team-modal__btn--ghost team-remove" type="button" data-id="${fv(e.id)}" data-name="${fv(e.name||e.email)}">Remove</button>`:``}
    </div>`}async function nv(e,t){let{uid:n,email:r}=e.dataset;if(window.confirm(`Revoke the invitation for ${r}?\n\nThe invitation link stops working immediately, including any copy already sent. You can invite this address again afterwards.`)){e.disabled=!0;try{let e=E.getConfig?.().BASE_URL||``,i=await fetch(`${e}/m5t/v5/acctGroup/${encodeURIComponent(t.guid)}/invites/${encodeURIComponent(n)}`,{method:`DELETE`}),a=await i.json().catch(()=>({}));if(!i.ok||!a.ok)throw Error(a.message||a.error||`HTTP ${i.status}`);pv(`Invitation for ${r} revoked`,`success`)}catch(t){e.disabled=!1,pv(t.message||`Could not revoke that invitation.`,`error`);return}try{await ov(t)}catch{}}}async function rv(e,t){let{id:n,name:r}=e.dataset;if(window.confirm(`Remove ${r} from the team?\n\nThey lose access to this workspace immediately. Their account itself is not deleted, and you can invite them again later.`)){e.disabled=!0;try{let e=E.getConfig?.().BASE_URL||``,t=await fetch(`${e}/m5t/v5/acctGroupMember/${encodeURIComponent(n)}`,{method:`DELETE`}),i=await t.json().catch(()=>({}));if(!t.ok||!i.ok)throw Error(i.message||i.error||`HTTP ${t.status}`);pv(`${r} removed from the team`,`success`)}catch(t){e.disabled=!1,pv(t.message||`Could not remove that member.`,`error`);return}try{await ov(t)}catch{}}}function iv(){return $_(),`
    <div class="page-shell" data-section="team">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="team-body">
          ${ev(`hourglass_empty`,`Loading team…`)}
        </div>
      </div>
    </div>`}async function av(){let e=Z_(),t=document.getElementById(`team-invite-btn`);if(!e){t&&(t.style.display=`none`),document.getElementById(`team-body`).innerHTML=ev(`info`,`Team management is available for agency and workspace accounts.`);return}t?.addEventListener(`click`,()=>sv(e)),await ov(e)}async function ov(e){let t=document.getElementById(`team-body`),n=document.getElementById(`team-count`);if(t)try{let r=E.getConfig?.().BASE_URL||``,[i,a]=await Promise.all([fetch(`${r}/m5t/v5/acctGroup/${encodeURIComponent(e.guid)}/members`),fetch(`${r}/m5t/v5/acctGroup/${encodeURIComponent(e.guid)}/invites`)]),[o,s]=await Promise.all([i.json().catch(()=>({})),a.json().catch(()=>({}))]);if(!i.ok||!a.ok)throw Error(`Failed to load team (members: ${i.status}, invites: ${a.status})`);let c=o.members||[],l=s.invites||[],u=Q_(e.memberRole).length>0;t.innerHTML=(l.length?`
      <div class="team-pending">
        <div class="team-pending__label">Pending invites</div>
        ${l.map(e=>`
          <div class="team-pending__row">
            <span class="team-pending__email" title="${fv(e.email)}">${fv(e.email)}${e.expired?` · expired`:``}</span>
            <button class="team-modal__btn team-modal__btn--ghost team-copy" type="button" data-url="${fv(e.inviteUrl)}">Copy link</button>
            ${u?`<button class="team-modal__btn team-modal__btn--ghost team-revoke" type="button" data-uid="${fv(e.uid)}" data-email="${fv(e.email)}">Revoke</button>`:``}
          </div>`).join(``)}
      </div>`:``)+(c.length?`<div class="team-list">${c.map(t=>tv(t,e,u)).join(``)}</div>`:ev(`group`,`No members yet.`)),n&&(n.textContent=`${c.length} member${c.length===1?``:`s`}`),t.querySelectorAll(`.team-copy`).forEach(e=>e.addEventListener(`click`,async()=>{try{await navigator.clipboard.writeText(e.dataset.url),e.textContent=`Copied!`,setTimeout(()=>{e.textContent=`Copy link`},1500)}catch{}})),t.querySelectorAll(`.team-revoke`).forEach(t=>t.addEventListener(`click`,()=>nv(t,e))),t.querySelectorAll(`.team-remove`).forEach(t=>t.addEventListener(`click`,()=>rv(t,e)))}catch(e){console.warn(`[team] load failed:`,e),t.innerHTML=ev(`error`,`Couldn’t load the team. Please try again.`)}}function sv(e){let t=Q_(e.memberRole);if(!t.length){pv(`You don’t have permission to invite teammates.`,`error`);return}let n=document.createElement(`div`);n.className=`team-modal-scrim`;let r=document.createElement(`div`);r.className=`team-modal`,r.setAttribute(`role`,`dialog`),r.setAttribute(`aria-modal`,`true`),r.innerHTML=`
    ...
    <div class="team-modal__body" id="team-invite-body">
      <p class="team-modal__sub">They'll get a link to set a password and join <strong>${fv(e.name||`your workspace`)}</strong>.</p>
      ...
      <select class="team-modal__select" id="tm-role">
        ${t.map(e=>`<option value="${e.value}"${e.value===`member`?` selected`:``}>${fv(e.label)}</option>`).join(``)}
      </select>
      ...
      <div class="team-modal__actions">
        ...
        </div>`;let u=r.querySelector(`#tm-link`);r.querySelector(`#tm-copy`)?.addEventListener(`click`,async()=>{try{await navigator.clipboard.writeText(u.value),r.querySelector(`#tm-copy`).textContent=`Copied!`}catch{u.select()}}),r.querySelector(`[data-act="done"]`)?.addEventListener(`click`,()=>{i(),ov(e)})}catch(e){console.warn(`[team] invite failed:`,e),o.textContent=e.message||`Something went wrong. Please try again.`,s.disabled=!1,s.innerHTML=l}})}var cv,lv,uv,dv,fv,pv,mv,hv,gv=o((()=>{D(),cv=u(f(),1),m(),Xe(),lv=`managed_client`,uv=!1,dv={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},fv=e=>String(e??``).replace(/[&<>"']/g,e=>dv[e]),pv=(e,t=`info`)=>{try{cv.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},mv=e=>String(e||`?`).trim().split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``).toUpperCase()||`?`,hv={render:iv,init:av}}));m(),Se(),D(),zt(),y(),xn(),On(),et(),window.m5t=Object.create(null),window.m5t.pubsub=b.default,console.log(`StreamSyncEngage SaaS Client - Starting...`),console.log(`Environment:`,`production`),console.log(`Document readyState:`,document.readyState),console.log(`window.mvvLegit:`,typeof window.mvvLegit),console.log(`globalThis.mvvLegit:`,typeof globalThis.mvvLegit);try{console.log(`mvvLegit (direct):`,typeof mvvLegit)}catch(e){console.log(`mvvLegit (direct): ReferenceError -`,e.message)}var _v=Object.keys(window).filter(e=>e.toLowerCase().includes(`mvv`)||e.toLowerCase().includes(`neodigm`)||e.toLowerCase().includes(`legit`));console.log(`Possible Neodigm globals:`,_v);var Z=new ee.default(`/`,{hash:!0});window.router=Z;function Q(e){let t=e?`${e.replace(/-/g,`_`)}_route`:`splash_route`;return console.log(`[Router] Checking access: ${e} -> ${t}`),mvvLegit.isRouteAllowed(t)?!0:(console.warn(`[Router] Access denied to route: ${t}`),b.default.publish(p.UI_TOAST,{message:`Please sign in to access this page`,type:`warning`}),!1)}function $(e){console.log(`[Router] Navigating to:`,e),Z.navigate(`/${e}`)}var vv=new Set([`splash`,`signin`,`signup`,`forgot`,`resethash`,`verf-link`,`accept-invite`,`signout`]),yv={},bv={},xv=null,Sv={admin:{caption:`Platform Admin`,icon:`shield_person`},reseller:{caption:`Reseller`,icon:`handshake`},managed:{caption:`Managed`,icon:`visibility`},self_serve:{caption:`Self-Serve`,icon:`rocket_launch`}},Cv=`__portfolio__`;function wv(){try{Z._setCurrent(null)}catch{}Z.resolve()}function Tv(){let e=document.getElementById(`m5t-side-nav`);if(!e)return;let t=!E.getScope()&&E.isAuthenticated(),n=E.getCapabilities(),r=E.getUiMode(),i=Wt.getSideNav(n,{uiMode:r});yv={},bv={};let a=e=>{e.route&&(yv[e.route]={sectionToken:e.token,childToken:null,children:e.children||null},Array.isArray(e.children)&&e.children.forEach(t=>{t.route&&(bv[t.token]=t.route,yv[t.route]={sectionToken:e.token,childToken:t.token,children:e.children})}))},o=e=>e.forEach(e=>{e.group&&Array.isArray(e.items)?o(e.items):a(e)});o(i);let s=E.getMemberships(),c=E.getActiveOrg(),l=E.getManagedClients(),u=E.getStoredActiveOrg(),d=l.length?[{guid:Cv,name:`Portfolio (all clients)`},...s.map(e=>({guid:e.orgGuid,name:e.name||e.channel,channel:e.channel,group:`Your organization`})),...l.map(e=>({guid:e.orgGuid,name:e.name||e.orgGuid,channel:e.channel,group:`Clients`}))]:s.map(e=>({guid:e.orgGuid,name:e.name||e.channel,channel:e.channel}));e.setState({buttons:i,orgs:d,activeOrg:l.length?u||Cv:c?.guid||null,footerBadge:Sv[r]||null,loading:t}),Dv(),xv&&Ov(xv)}var Ev=e=>String(e??``).replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`})[e]);function Dv(){let e=document.getElementById(`app-content`);if(!e)return;let t=document.getElementById(`org-context-banner`);t||(t=document.createElement(`div`),t.id=`org-context-banner`,e.insertBefore(t,e.firstChild),fn(t));let n=E.getActiveOrg(),r=E.isPreviewingAsClient(),i=!!n?.viaSubtree&&!!xv&&!vv.has(xv);if(t.hidden=!i,!i){t.innerHTML=``;return}let a=Ev(n.name||`Client`);t.innerHTML=`
    <style>
