  ...
  `,document.head.appendChild(e),Rn=!0}const Ta=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,st=(e,t="")=>`<div class="intg-alert ${t}"><span class="material-symbols-outlined" aria-hidden="true">${t==="error"?"error":"info"}</span><span>${e}</span></div>`;function xl(){return wl(),ct(),se()?At({section:"settings-integrations",title:"Integrations",description:"Pick a client to manage their connections."}):`
    ...
    <div class="dashboard-card intg-card">
      <div class="card-body">
        ...
        <button type="button" class="intg-btn danger" id="intg-r-disconnect"><span class="material-symbols-outlined" aria-hidden="true">link_off</span>Disconnect</button>`;const m=a.webhook||{};if(m.state==="active"){const h=m.last_received_tz?` Last call received ${me(String(m.last_received_tz).slice(0,10))}.`:"";r=st(`Call sync is on. New calls appear within minutes; a daily sweep catches anything missed.${h}`)}else r=st("Automatic call sync could not be set up in CallRail. Calls still arrive via the daily sweep and Sync now. Reconnect to retry the setup.","error")}else n='<span class="intg-state warn"><span class="material-symbols-outlined" aria-hidden="true">priority_high</span>Connection error</span>',o="The connection stopped working. Reconnect with a current CallRail API key.",s=`
...
`))}</textarea>`}return`<div class="mag-cta" data-type="${e}" data-kind="${a.kind}">
  ...
  `,document.head.appendChild(e),Jn=!0}function Kt(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function nd(e){const t=B.stats[e];if(!t||!t.opens&&!t.leads)return'<div class="mag-stats"><span><span class="material-symbols-outlined" aria-hidden="true">monitoring</span>No activity yet (30d)</span></div>';const a=n=>Number(n||0).toLocaleString();return`<div class="mag-stats" title="Trailing 30 days">
    ...
    <div class="mag-card" data-guid="${_(e.guid)}">
      ...
      <span class="mag-mode"><span class="material-symbols-outlined" aria-hidden="true">${jc[e.display_mode]||"chat"}</span>${_(Vc[e.display_mode]||"Chat bubble")}${e.ab?.enabled?'&nbsp;&nbsp;<span class="mag-ab-chip"><span class="material-symbols-outlined" aria-hidden="true">science</span>A/B</span>':""}${(e.notify_emails||[]).length?`&nbsp;&nbsp;<a class="mag-alert-chip" href="#/settings/notifications" title="Lead alerts to ${(e.notify_emails||[]).length} email${(e.notify_emails||[]).length===1?"":"s"}. Manage in Settings."><span class="material-symbols-outlined" aria-hidden="true">notifications_active</span>${(e.notify_emails||[]).length}</a>`:""}</span>
      ${nd(e.guid)}
      <div class="mag-domains">${a.length?a.map(o=>_(o)).join(", "):"<span>No domains set. Add yours to go live.</span>"}</div>
      <div class="mag-embed">
        <input type="text" readonly value="${_(Es(e.guid))}" aria-label="Embed snippet">
    ...
    <div class="mag-modal" role="dialog" aria-modal="true" aria-label="${o?"Edit magnet":"New magnet"}">
      ...
      <div class="mag-modal__main">
        ...
        <select class="mag-modal__select" id="mag-f-trigger">
          ${Jc.map(A=>`<option value="${A.value}"${(e?e.trigger||"none":"on_load")===A.value?" selected":""}>${_(A.label)}</option>`).join("")}
        </select>
      ...
      <div class="team-pending">
        ...
        ${c.map(h=>`
          <div class="team-pending__row">
            <span class="team-pending__email" title="${pe(h.email)}">${pe(h.email)}${h.expired?" · expired":""}</span>
            <button class="team-modal__btn team-modal__btn--ghost team-copy" type="button" data-url="${pe(h.inviteUrl)}">Copy link</button>
          </div>`).join("")}
