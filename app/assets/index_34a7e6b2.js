...
/&gt;</code></pre>
  ...
  `,document.head.appendChild(e),Nm=!0}function lm(){let e=K.search.trim().toLowerCase();return K.rows.filter(t=>(K.filter===`ALL`||t.status===K.filter)&&(!e||String(t.owner_name||``).toLowerCase().includes(e)))}function um(e){return e.status===`DRAFT`?`<button class="dz-act" type="button" data-act="open" data-guid="${G(e.guid)}">Edit</button>`:e.status===`SENT`?`<button class="dz-act" type="button" data-act="copy" data-url="${G(e.url||``)}">Copy link</button>`:e.status===`SUBMITTED`?`<button class="dz-act" type="button" data-act="open" data-guid="${G(e.guid)}">Read review</button>`:`<span class="dz-when">—</span>`}function dm(e){let t=e.status!==`CANCELLED`;return`<tr class="dz-row${t?` dz-row--open`:``}"${t?` data-open-guid="${G(e.guid)}"`:``}>
    ...
    <td><span class="dz-badge" data-s="${G(e.status)}">${G(Mm[e.status]||e.status)}</span></td>
    ...
    <div class="page-shell" data-section="deliverables">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="dz-body">${Pm(`hourglass_empty`,`Loading…`)}</div>
      </div>
    </div>`}async function pm(){document.getElementById(`dz-new`)?.addEventListener(`click`,bm),await mm()}async function mm(){let e=document.getElementById(`dz-body`);if(e){try{let[e,t,n]=await Promise.all([fetch(`${Om()}/m5t/v5/m5mDeliverable`),fetch(`${Om()}/m5t/v5/acctGroup`),fetch(`${Om()}/m5t/v5/m5mDeliverable/templates`)]),r=await e.json().catch(()=>({})),i=await t.json().catch(()=>({})),a=await n.json().catch(()=>({}));if(!e.ok||!t.ok||!n.ok)throw Error(`load failed`);K.rows=r.data||[],K.clients=(i.data||[]).filter(e=>e.class===Am&&![`DELETED`,`ARCHIVED`].includes(e.status)).sort((e,t)=>String(e.caption||``).localeCompare(String(t.caption||``))),K.templates=a.data||[]}catch{e.innerHTML=Pm(`error`,`Could not load deliverables. Please try again.`);return}gm()}}function hm(){let e=document.getElementById(`dz-tbody`);e&&D(e,{rowSelector:`tr.dz-row`,noun:`deliverables`,controlHost:e.closest(`.dz-tablewrap`)||e.parentElement})}function gm(){let e=document.getElementById(`dz-body`);if(!e)return;if(!K.clients.length&&!K.rows.length){e.innerHTML=Pm(`groups`,`No clients yet. Add a client first, then author their first deliverable.`);return}if(!K.rows.length){e.innerHTML=Pm(`approval`,`No deliverables yet. Author the first one with New deliverable.`);return}let t={ALL:K.rows.length};jm.slice(1).forEach(([e])=>{t[e]=K.rows.filter(t=>t.status===e).length});let n=lm();e.innerHTML=`
    <div class="dz-bar">
      <div class="dz-filter">
        ${jm.map(([e,n])=>`<button data-filter="${e}" class="${K.filter===e?`active`:``}">${G(n)}${t[e]?`<span class="dz-pillnum">${t[e]}</span>`:``}</button>`).join(``)}
      </div>
    ...
    <p class="dz-note">Creates a draft. Nothing is sent until you press Send to client.</p>`;let n=e.querySelector(`#dz-dfoot`);n.innerHTML=`<button class="dz-btn" type="button" id="dz-create">Start authoring</button>`,n.querySelector(`#dz-create`).addEventListener(`click`,async()=>{let r=e.querySelector(`#dz-new-client`)?.value,i=e.querySelector(`#dz-new-kind`)?.value,a=n.querySelector(`#dz-create`);a.disabled=!0;try{let e=await fetch(`${Om()}/m5t/v5/m5mDeliverable`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({owner_guid:r,kind:i})}),n=await e.json().catch(()=>({}));if(!e.ok||!n.ok)throw Error(n.error||`create failed`);t(),await mm(),Tm(n.data.guid)}catch(e){km(e.message||`Could not create the deliverable.`,`error`),a.disabled=!1}})}function xm(e,t){return`
    <div class="qm-qa" data-question-id="${G(e.id)}" data-question-type="${G(e.type)}">
      <div class="qm-qa__q">${G(e.label)}${e.help?` <span class="dz-help" style="display:inline;font-weight:400">(${G(e.help)})</span>`:``}</div>
      <div class="qm-qa__a">${Qn(e,t)}</div>
    </div>`}function Sm(e,t){return(e?.sections||[]).map(e=>{let n=(e.questions||[]).filter(e=>e.audience===`am`);return n.length?`
      <div class="qm-section">
        <h3>${G(e.title)}</h3>
        ${e.intro?`<p class="dz-intro">${G(e.intro)}</p>`:``}
        ${n.map(e=>xm(e,t)).join(``)}
      </div>`:``}).join(``)}function Cm(e,t,n){return(e?.sections||[]).map(e=>{let r=(e.questions||[]).filter(e=>{if(e.audience!==`am`)return!0;let n=t?.[e.id];return Array.isArray(n)?n.length>0:!!(n&&String(n).trim())});return r.length?`
      <div class="qm-section">
        <h3>${G(e.title)}</h3>
        ${r.map(e=>`
          <div class="qm-qa">
      ...
      </div>`:``}).join(``)}function wm(e,t){let n=Array.isArray(t)&&t.length?t[t.length-1]:null;if(!n)return``;let r=(e?.sections||[]).flatMap(e=>(e.questions||[]).filter(e=>e.audience!==`am`)).map(e=>({q:e,val:Xn(e,n.answers||{})})).filter(({val:e})=>e);return r.length?`
    ...
    </div>`:``}async function Tm(e){let{drawer:t,close:n}=ym(`Deliverable`),r=t.querySelector(`#dz-dbody`),i=t.querySelector(`#dz-dfoot`),a;try{let t=await fetch(`${Om()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}`),n=await t.json().catch(()=>({}));if(!t.ok||!n.ok)throw Error(n.error||`load failed`);a=n.data||{}}catch{r.innerHTML=Pm(`error`,`Could not load this deliverable. Please try again.`);return}let o=K.rows.find(t=>t.guid===e)?.owner_name||K.clients.find(e=>e.guid===a.owner_guid)?.caption||`Client`;t.querySelector(`#dz-dtitle`).textContent=`${o} · ${a.template?.name||`Deliverable`}`;let s=a.status===`DRAFT`||a.status===`SENT`,c=Number(a.revision)>1?` · Rev ${a.revision}`:``,l=a.status===`SUBMITTED`?`Returned ${a.submitted_tz?O(a.submitted_tz):``}${a.locked_tz?` · Finalized`:``}${c}`:a.status===`SENT`?`Sent ${a.sent_tz?O(a.sent_tz):``} to ${a.recipient_email||``} · awaiting approval${c}`:`${Mm[a.status]||a.status}${c}`;if(t.querySelector(`#dz-dsub`).textContent=l,!s){r.innerHTML=er(a)+Cm(a.template,a.content||{},a.answers||{}),a.status===`SUBMITTED`&&!a.locked_tz?(i.innerHTML=`<button class="dz-btn" type="button" id="dz-revise">Revise plan</button>
        <span class="dz-note" style="margin:0">Archives this review and reopens authoring. The client approves the revised plan on a fresh link.</span>`,i.querySelector(`#dz-revise`).addEventListener(`click`,async t=>{let r=t.target;r.disabled=!0;try{let t=await fetch(`${Om()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}/revise`,{method:`POST`}),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`revise failed`);km(`Reopened for revision. The last review is pinned above the form.`,`success`),n(),await mm(),Tm(e)}catch(e){km(e.message||`Could not revise.`,`error`),r.disabled=!1}})):i.innerHTML=``;return}r.innerHTML=`
    ${wm(a.template,a.rounds)}
    ${a.status===`SENT`&&a.url?`
    ...
    <div id="dz-author-form">${Sm(a.template,a.content||{})}</div>`,i.innerHTML=`
    ...
    <button class="dz-btn--ghost dz-btn--danger" type="button" id="dz-cancel">Cancel deliverable</button>`,r.querySelector(`#dz-copylink`)?.addEventListener(`click`,e=>{let t=r.querySelector(`.dz-linkrow input`);navigator.clipboard?.writeText(t?.value||``).then(()=>{e.target.textContent=`Copied!`,setTimeout(()=>{e.target.textContent=`Copy link`},1500)},()=>km(`Could not copy the link.`,`error`))});let u=async()=>{let t=$n(r.querySelector(`#dz-author-form`)),i=await fetch(`${Om()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}`,{method:`PUT`,headers:{"Content-Type":`application/json`},body:JSON.stringify({content:t})}),a=await i.json().catch(()=>({}));if(!i.ok||!a.ok){if(i.status===409)return km(a.error||`This deliverable is no longer editable.`,`warning`),n(),await mm(),!1;throw Error(a.error||`save failed`)}return!0};i.querySelector(`#dz-save`).addEventListener(`click`,async e=>{let t=e.target;t.disabled=!0;try{await u()&&km(`Draft saved.`,`success`)}catch(e){km(e.message||`Could not save.`,`error`)}t.disabled=!1}),i.querySelector(`#dz-send`).addEventListener(`click`,async t=>{let r=t.target,a=(i.querySelector(`#dz-email`)?.value||``).trim();if(!a){km(`Enter the recipient email first.`,`warning`);return}r.disabled=!0;try{if(!await u())return;let t=await fetch(`${Om()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}/send`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({recipient_email:a})}),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`send failed`);km(r.data?.emailQueued?`Sent. ${a} will receive the review link.`:`Link created, but the email could not be queued. Copy the link and send it yourself.`,r.data?.emailQueued?`success`:`warning`),n(),await mm()}catch(e){km(e.message||`Could not send.`,`error`),r.disabled=!1}}),i.querySelector(`#dz-cancel`).addEventListener(`click`,async t=>{let r=t.target;r.disabled=!0;try{let t=await fetch(`${Om()}/m5t/v5/m5mDeliverable/${encodeURIComponent(e)}/cancel`,{method:`POST`}),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`cancel failed`);km(`Deliverable cancelled. Its link no longer works.`,`info`),n(),await mm()}catch(e){km(e.message||`Could not cancel.`,`error`),r.disabled=!1}})}var Em,Dm,G,Om,km,Am,jm,Mm,K,Nm,Pm,Fm,Im=o((()=>{T(),Em=u(f(),1),m(),ar(),Xt(),Dm={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},G=e=>String(e??``).replace(/[&<>"']/g,e=>Dm[e]),Om=()=>w.getConfig?.().BASE_URL||``,km=(e,t=`info`)=>{try{Em.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},Am=`managed_client`,jm=[[`ALL`,`All`],[`DRAFT`,`In progress`],[`SENT`,`Awaiting approval`],[`SUBMITTED`,`Returned`],[`CANCELLED`,`Cancelled`]],Mm=Object.fromEntries(jm),K={rows:[],clients:[],templates:[],filter:`ALL`,search:``},Nm=!1,Pm=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,Fm={render:fm,init:pm}})),Lm=c({init:()=>Jm,render:()=>qm});function Rm(e){let t=hh(e);if(!t)return``;try{return new Date(t).toLocaleDateString([],{month:`short`,day:`numeric`,year:`numeric`})}catch{return``}}function zm(e){let t=hh(e);if(!t)return``;try{let n=Date.now()-new Date(t).getTime();if(n<0)return Rm(e);let r=n/36e5;return r<1?`${Math.max(1,Math.floor(n/6e4))}m ago`:r<24?`${Math.floor(r)}h ago`:r<48?`Yesterday`:Rm(e)}catch{return Rm(e)}}function Bm(){if(sh)return;let e=document.createElement(`style`);e.id=`m5t-leads-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),sh=!0}function Vm(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function Hm(e){return`<select class="leads-status" data-id="${J(e.id)}" data-act="status" onclick="event.stopPropagation()">
    ${lh.map(t=>`<option value="${t}" ${e.status===t?`selected`:``}>${t[0]}${t.slice(1).toLowerCase()}</option>`).join(``)}
  </select>`}function Um(e){let t=q.magnetNames[e.magnet_guid]||e.lead_source||`—`,n=e.phone?String(e.phone).replace(/[^\d+]/g,``):``;return`
    <tr class="leads-row${String(e.id)===q.selectedId?` selected`:``}" data-id="${J(e.id)}" tabindex="0" role="button" aria-label="Open lead ${J(ph(e))}">
      <td>
        <span class="leads-who">
          <span class="leads-avatar">${J(mh(e))}</span>
          <span><span class="leads-name">${J(ph(e))}</span>${e.company?`<span class="leads-sub">${J(e.company)}</span>`:``}</span>
        </span>
      </td>
      ...
      <td>${q.canManage?Hm(e):`<span class="leads-badge" data-s="${J(e.status)}">${J(e.status)}</span>`}</td>
      <td class="num leads-muted" style="white-space:nowrap" title="${J(Rm(e))}">${gh(e)?`<span class="leads-fresh" title="Captured in the last 24 hours"></span>`:``}${J(zm(e))}</td>
      <td class="leads-open" aria-hidden="true"><span class="material-symbols-outlined">chevron_right</span></td>
    </tr>`}function Wm(e){let t=q.search.trim().toLowerCase();if(!t)return!0;let n=[e.first_name,e.last_name,e.email,e.company,e.phone,e.product_interest,q.magnetNames[e.magnet_guid],q.book?q.ownerNames[e.owner_guid]:``,e.utm_campaign,e.utm_source].filter(Boolean).join(` `).toLowerCase();return t.split(/\s+/).every(e=>n.includes(e))}function Gm(){return q.leads.filter(e=>(q.filter===`ALL`||e.status===q.filter)&&_h(e)&&Wm(e)&&vh(e))}function Km(){if(!q.day)return``;let e=q.day;try{e=new Date(`${q.day}T00:00:00Z`).toLocaleDateString([],{month:`short`,day:`numeric`,year:`numeric`,timeZone:`UTC`})}catch{}return`<button class="leads-daychip" type="button" id="leads-dayclear" title="Show every day again">
  ...
  </button>`}function qm(){Bm(),Bi();let e=j();return`
    <div class="page-shell" data-section="leads">
      ...
      <div class="dashboard-card" style="margin-bottom:24px">
        ...
        <div class="card-body" id="pf-picker-body">${Vm(`hourglass_empty`,`Loading…`)}</div>
      </div>`:``}
      <div class="page-header">
        <div class="page-header-text">
      ...
      <div class="leads-layout">
        <div class="dashboard-card">
          <div class="card-body" id="leads-body">
            ${Vm(`hourglass_empty`,`Loading leads…`)}
          </div>
        </div>
    ...
    </div>`}async function Jm(){document.getElementById(`leads-export`)?.addEventListener(`click`,ah);let e=na(ia.LEADS_SEARCH),t=na(ia.LEADS_DAY);if((e||t)&&(q.search=e||``,q.day=t||``,q.filter=`ALL`,q.clientFilter=`ALL`,q.shown=ch),j()){let e=e=>Number(e||0).toLocaleString();Ki(fh(),[{h:`Leads · wk`,td:t=>e(t.leads.this_week)},{h:`Leads · total`,td:t=>e(t.leads.total)},{h:`Compl.`,td:e=>e.completion_rate==null?`—`:`${e.completion_rate}%`}],{emptyMsg:`No client orgs yet. Create one under Clients first.`})}await Ym()}async function Ym(){let e=document.getElementById(`leads-body`);if(e){try{let[e,t]=await Promise.all([fetch(`${fh()}/m5t/v5/m5mLead`),fetch(`${fh()}/m5t/v5/magnets`).catch(()=>null)]),n=await e.json().catch(()=>({}));if(!n.ok)throw Error(n.message||`Failed to load`);if(q.leads=(n.data||[]).filter(e=>![`DELETED`,`ARCHIVED`].includes(e.status)),q.total=Number.isFinite(n.total)?n.total:q.leads.length,q.truncated=q.total>(n.data||[]).length,q.book=j(),q.ownerNames=q.book?qi():{},t){let e=await t.json().catch(()=>({}));q.magnetNames={},(e.magnets||[]).forEach(e=>{q.magnetNames[e.guid]=e.name,e.owner_guid&&e.owner_name&&!q.ownerNames[e.owner_guid]&&(q.ownerNames[e.owner_guid]=e.owner_name)}),q.canManage=!!e.canManage}}catch{e.innerHTML=Vm(`error`,`Could not load your leads. Please try again.`);return}Xm()}}function Xm(){let e=document.getElementById(`leads-body`),t=document.getElementById(`leads-export`);if(!e)return;if(t&&(t.style.display=q.leads.length?``:`none`),!q.leads.length){e.innerHTML=Vm(`inbox`,`No leads captured yet. Publish a magnet and share it to start collecting.`),rh();return}let n=q.leads.filter(_h),r={ALL:n.length};lh.forEach(e=>{r[e]=n.filter(t=>t.status===e).length});let i=q.book?[...new Set(q.leads.map(e=>e.owner_guid).filter(Boolean))].map(e=>({guid:e,name:q.ownerNames[e]||e})).sort((e,t)=>e.name.localeCompare(t.name)):[];e.innerHTML=`
    <div class="leads-bar">
    ...
    <div id="leads-tablezone"></div>`,e.querySelector(`.leads-filter`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-filter]`);t&&(q.filter=t.dataset.filter,q.shown=ch,Xm())}),e.querySelector(`#leads-clientsel`)?.addEventListener(`change`,e=>{q.clientFilter=e.target.value,q.shown=ch,Xm()}),e.querySelector(`#leads-dayclear`)?.addEventListener(`click`,()=>{q.day=``,q.shown=ch,Xm()}),e.querySelector(`#leads-search`)?.addEventListener(`input`,e=>{q.search=e.target.value,q.shown=ch,Zm()}),Zm()}function Zm(){let e=document.getElementById(`leads-tablezone`),t=document.getElementById(`leads-countlbl`);if(!e)return;let n=Gm(),r=n.slice(0,q.shown),i=q.book?8:7;t&&(t.textContent=`${n.length} of ${q.leads.length}`),e.innerHTML=`
    <div class="leads-tablewrap">
      <table class="leads-table${q.book?` leads-table--book`:``}">
        ...
        <tbody id="leads-tbody">${r.map(Um).join(``)||`<tr><td colspan="${i}"><div class="leads-muted" style="padding:14px 4px">${q.search.trim()?`No leads match your search.`:`No ${q.filter.toLowerCase()} leads.`}</div></td></tr>`}</tbody>
      </table>
    </div>
    ${Jt({total:n.length,shown:q.shown,noun:`leads`})}
    ${q.truncated?`<div class="leads-muted" style="padding:10px 4px">Showing your ${q.leads.length.toLocaleString()} most recent leads of ${q.total.toLocaleString()}. Search and filters cover these; older leads aren't included yet.</div>`:``}`,e.querySelector(`[data-action="list-expand"]`)?.addEventListener(`click`,()=>{q.shown+=25,Zm()}),e.querySelector(`#leads-tbody`)?.addEventListener(`change`,Qm),e.querySelector(`#leads-tbody`)?.addEventListener(`click`,e=>{if(e.target.closest(`.leads-status, a.leads-contactlink`))return;let t=e.target.closest(`.leads-row`);t&&$m(t.dataset.id)}),e.querySelector(`#leads-tbody`)?.addEventListener(`keydown`,e=>{if(e.key!==`Enter`&&e.key!==` `)return;let t=e.target.closest(`.leads-row`);t&&e.target===t&&(e.preventDefault(),$m(t.dataset.id))})}async function Qm(e){let t=e.target.closest(`.leads-status`);if(!t)return;let n=t.dataset.id,r=q.leads.find(e=>String(e.id)===String(n)),i=t.value,a=r?.status;try{if(!(await(await fetch(`${fh()}/m5t/v5/m5mLead/${encodeURIComponent(n)}`,{method:`PUT`,headers:{"Content-Type":`application/json`},body:JSON.stringify({status:i})})).json().catch(()=>({}))).ok)throw Error();r&&(r.status=i),dh(`Marked ${i[0]}${i.slice(1).toLowerCase()}`,`success`)}catch{t.value=a||`NEW`,dh(`Could not update status`,`danger`);return}try{String(n)===q.selectedId&&rh(),q.filter!==`ALL`&&Xm()}catch(e){console.warn(`[leads] re-render after status change failed:`,e)}}function $m(e){q.selectedId=String(e),document.querySelectorAll(`.leads-row`).forEach(e=>e.classList.toggle(`selected`,e.dataset.id===q.selectedId)),rh()}function eh(e,t){let n=Array.isArray(e.tags)?e.tags:[];for(let e=n.length-1;e>=0;e--)if(n[e]&&n[e].tag===t&&n[e].value!=null)return String(n[e].value);return``}function th(e){let t=eh(e,`appointment_start`);if(!t)return null;let n=eh(e,`appointment_status`)||`booked`,r=eh(e,`appointment_timezone`),i=eh(e,`appointment_event_type`),a=new Date(t),o=t;if(!Number.isNaN(a.getTime())){let e={weekday:`short`,month:`short`,day:`numeric`,hour:`numeric`,minute:`2-digit`};try{o=new Intl.DateTimeFormat(void 0,r?{...e,timeZone:r}:e).format(a)}catch{o=new Intl.DateTimeFormat(void 0,e).format(a)}}return{when:o,status:n,type:i,start:t}}function nh(e){let t=Array.isArray(e.tags)?e.tags:[],n=null;for(let e of t)if(!(!e||!e.tag)){if(e.tag===`callrail_call_id`){n={};continue}n&&e.tag.indexOf(`call_`)===0&&e.value!=null&&(n[e.tag]=String(e.value))}if(!n||!n.call_started)return null;let r=n.call_started,i=new Date(r),a=r;Number.isNaN(i.getTime())||(a=new Intl.DateTimeFormat(void 0,{weekday:`short`,month:`short`,day:`numeric`,hour:`numeric`,minute:`2-digit`}).format(i));let o=Number(n.call_duration)||0,s=o?o<60?`${o}s`:`${Math.floor(o/60)}m ${o%60}s`:``;return{when:a,dur:s,source:n.call_source||``,answered:n.call_answered||``,recording:n.call_recording_player||``,started:r}}function rh(){let e=document.getElementById(`leads-panel-body`);if(!e)return;let t=q.leads.find(e=>String(e.id)===q.selectedId),n=document.querySelector(`.leads-layout`);if(n&&(n.dataset.hasSelection=t?`true`:`false`),!t){e.innerHTML=yh();return}let r=q.magnetNames[t.magnet_guid]||t.lead_source||`—`,i=th(t),[a,o]=i?xh[i.status]||xh.booked:[``,``],s=nh(t),c=(e,t)=>t?`<div class="leads-det__field"><span class="leads-det__k">${J(e)}</span><span class="leads-det__v">${J(t)}</span></div>`:``;e.innerHTML=`
    <div class="leads-det__head">
      <span class="leads-avatar leads-det__ava">${J(mh(t))}</span>
      <div>
        <div class="leads-det__name">${J(ph(t))}</div>
    ...
    ${q.canManage?`
      ...
      </div>`:``}`,e.querySelector(`[data-action="delete-lead"]`)?.addEventListener(`click`,ih)}async function ih(e){let t=e.currentTarget?.dataset?.id,n=q.leads.find(e=>String(e.id)===String(t));if(!n)return;let r=ph(n);if(!window.confirm(`Remove ${r} from your leads?\n\nThis takes the lead out of your dashboard, lists and exports. The record is kept in our system, and this address can be captured again if they come back through one of your magnets.`))return;let i=e.currentTarget;i.disabled=!0;try{let e=await fetch(`${fh()}/m5t/v5/m5mLead/${encodeURIComponent(t)}`,{method:`DELETE`}),n=await e.json().catch(()=>({}));if(!e.ok||!n.ok)throw Error(n.error||`HTTP ${e.status}`);q.leads=q.leads.filter(e=>String(e.id)!==String(t)),q.selectedId=null,dh(`${r} removed`,`success`),Xm()}catch{i.disabled=!1,dh(`Could not remove that lead. Please try again.`,`danger`)}}function ah(){let e=Gm();if(!e.length){dh(`Nothing to export`,`info`);return}let t=[`first_name`,`last_name`,`email`,`phone`,`company`,`product_interest`,`lead_source`,`utm_source`,`utm_medium`,`utm_campaign`,`gclid`,`status`],n=e=>`"${String(e??``).replace(/"/g,`""`)}"`,r=[[...q.book?[`client`]:[],...t,`magnet`,`captured`,`appointment`,`last_call`].join(`,`)];for(let i of e){let e=th(i),a=nh(i);r.push([...q.book?[n(q.ownerNames[i.owner_guid]||``)]:[],...t.map(e=>n(i[e])),n(q.magnetNames[i.magnet_guid]||i.lead_source||``),n(Rm(i)),n(e?`${e.start} (${e.status})`:``),n(a?`${a.started} (${a.answered===`no`?`missed`:`answered`})`:``)].join(`,`))}let i=new Blob([r.join(`
`)],{type:`text/csv`}),a=URL.createObjectURL(i),o=document.createElement(`a`);o.href=a,o.download=`leads-${q.filter.toLowerCase()}.csv`,document.body.appendChild(o),o.click(),o.remove(),URL.revokeObjectURL(a),dh(`Exported ${e.length} lead${e.length===1?``:`s`}`,`success`)}var oh,sh,q,ch,lh,uh,J,dh,fh,ph,mh,hh,gh,_h,vh,yh,bh,xh,Sh=o((()=>{T(),oh=u(f(),1),m(),ea(),oa(),Xt(),sh=!1,q={leads:[],magnetNames:{},filter:`ALL`,canManage:!1,book:!1,ownerNames:{},clientFilter:`ALL`,search:``,shown:25,selectedId:null,day:``,total:0,truncated:!1},ch=25,lh=[`NEW`,`WORKING`,`QUALIFIED`,`UNQUALIFIED`,`CONVERTED`,`EXPORTED`],uh={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},J=e=>String(e??``).replace(/[&<>"']/g,e=>uh[e]),dh=(e,t=`info`)=>{try{oh.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},fh=()=>w.getConfig?.().BASE_URL||``,ph=e=>[e.first_name,e.last_name].filter(Boolean).join(` `).trim()||e.email||`(no name)`,mh=e=>(ph(e).split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``)||`?`).toUpperCase(),hh=e=>e.created?.created_tz||``,gh=e=>{let t=hh(e);if(!t)return!1;try{return Date.now()-new Date(t).getTime()<864e5}catch{return!1}},_h=e=>q.clientFilter===`ALL`||e.owner_guid===q.clientFilter,vh=e=>!q.day||hh(e).slice(0,10)===q.day,yh=()=>Vm(`touch_app`,`Click to see lead information.`),bh=()=>`
  <colgroup>
    <col style="width:22%">
  ...
  </colgroup>`,xh={booked:[`QUALIFIED`,`Booked`],rescheduled:[`WORKING`,`Rescheduled`],canceled:[`CANCELED`,`Canceled`]}})),Ch=c({init:()=>kh,render:()=>Oh,tableHTML:()=>Dh});function wh(){if(zh)return;let e=document.createElement(`style`);e.id=`m5t-analytics-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),zh=!0}function Th(e,t){let n=t>0?Math.round(e/t*100):0;return`<svg class="anl-bar" width="72" height="8" viewBox="0 0 72 8" role="img"
    ...
    </svg>`}function Eh(e,t,n){let r=Vh[e.status]?`<span class="anl-chip">${Fh(Vh[e.status])}</span>`:``,i=n&&e.owner_name?`<span class="anl-sub">${Fh(e.owner_name)}</span>`:``,a=e.ab_tests&&(e.ab_tests.running||e.ab_tests.completed)?`${Nh(e.ab_tests.running)} / ${Nh(e.ab_tests.completed)}`:`—`;return`<tr data-guid="${Fh(e.magnet_guid)}">
    <td><a class="anl-name" href="#/magnets">${Fh(e.name)}</a>${r}${i}</td>
    <td>${Th(Number(e.leads?.window||0),t)}</td>
    <td>${Nh(e.leads?.window)}</td>
    <td>${Nh(e.opens)}</td>
    <td>${Ih(e.engagement_rate)}</td>
    <td>${Ih(e.completion_rate)}</td>
    <td>${Ih(e.lead_rate)}</td>
    <td>${a}</td>
  </tr>`}function Dh(e,t){let n=e.reduce((e,t)=>Math.max(e,Number(t.leads?.window||0)),0);return`<div class="anl-wrap"><table class="anl-table">
    <thead><tr>
      <th>Magnet</th><th></th><th>Leads</th><th>Opens</th>
      <th>Engaged</th><th>Completed</th><th>Leads / Open</th><th>A/B run / done</th>
    </tr></thead>
    <tbody>${e.map(e=>Eh(e,n,t)).join(``)}</tbody>
  </table></div>`}function Oh(){return j()?Gi({section:`analytics`,title:`Analytics`,description:`Pick a client to see how their magnets are performing.`}):(wh(),`
    <div class="page-shell" data-section="analytics">
      <div class="page-header">
        ...
        <div class="chart-controls" id="anl-periods">
          ${Lh.map(e=>`<button class="chart-period-btn${e.d===Rh?` active`:``}" type="button" data-days="${e.d}">${e.label}</button>`).join(``)}
        </div>
      </div>
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="anl-body">
          ${Bh(`hourglass_empty`,`Loading magnet performance…`)}
        </div>
      </div>
    ...
    </div>`)}async function kh(){if(j())return Ki(jh(),[{h:`Leads`,td:e=>Nh(e.leads.window??e.leads.total)},{h:`Opens`,td:e=>Nh(e.opens)},{h:`Completed`,td:e=>Ih(e.completion_rate)}]);wh(),await Ah();let e=document.getElementById(`anl-periods`);e&&e.addEventListener(`click`,t=>{let n=t.target.closest(`.chart-period-btn`);n&&(Rh=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>{e.classList.toggle(`active`,Number(e.dataset.days)===Rh)}),Ah())})}async function Ah(){let e=document.getElementById(`anl-body`),t=document.getElementById(`anl-range`);if(!e)return;e.innerHTML=Bh(`hourglass_empty`,`Loading magnet performance…`);let n=new Date,r=Mh(n),i=Mh(new Date(n.getTime()-(Rh-1)*864e5));t&&(t.textContent=`${i} → ${r}`);let a;try{let e=await fetch(`${jh()}/m5t/v5/dashboard/stats?from=${i}&to=${r}&group_by=magnet`);if(a=await e.json(),!e.ok||!a?.ok)throw Error(a?.error||`HTTP ${e.status}`)}catch(t){e.innerHTML=Bh(`cloud_off`,`Could not load analytics. Refresh to try again.`),console.error(`[analytics] load failed:`,t);return}let o=(e,t)=>{let n=document.getElementById(e);n&&(n.textContent=t)};o(`anl-leads`,Nh(a.by_day?.reduce((e,t)=>e+Number(t.leads||0),0))),o(`anl-opens`,Nh(a.funnel?.opens)),o(`anl-completion`,Ih(a.funnel?.completion_rate)),o(`anl-active`,Nh(a.magnets?.active));let s=Array.isArray(a.by_magnet)?a.by_magnet:[];if(!s.length){e.innerHTML=Bh(`widgets`,`No magnets yet. Create one and its numbers will show up here.`);return}let c=s.some(e=>Number(e.opens||0)>0||Number(e.leads?.window||0)>0),l=new Set(s.map(e=>e.owner_guid)).size>1;e.innerHTML=(a.by_magnet_truncated?`<p class="anl-warn">Showing the top ${s.length} magnets by leads. You have more than that, so this list is not complete.</p>`:``)+(c?``:`<p class="anl-warn">No opens or leads for any magnet in this period. Try a longer range.</p>`)+Dh(s,l);let u=e.querySelector(`.anl-table tbody`);u&&D(u,{rowSelector:`tr`,noun:`magnets`,controlHost:e.querySelector(`.anl-wrap`)||e})}var jh,Mh,Nh,Ph,Fh,Ih,Lh,Rh,zh,Bh,Vh,Hh=o((()=>{T(),ea(),ua(),Xt(),jh=()=>w.getConfig?.().BASE_URL||``,Mh=e=>e.toISOString().slice(0,10),Nh=e=>Number(e||0).toLocaleString(),Ph={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Fh=e=>String(e??``).replace(/[&<>"']/g,e=>Ph[e]),Ih=e=>e==null?`—`:`${e}%`,Lh=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],Rh=30,zh=!1,Bh=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,Vh={DRAFT:`Draft`,DISABLED:`Paused`}})),Uh=c({default:()=>dg,init:()=>Kh,render:()=>Gh});function Wh(){if(sg)return;let e=document.createElement(`style`);e.id=`m5t-adperf-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),sg=!0}function Gh(){return j()?Gi({section:`ad-performance`,title:`Ad Performance`,description:`Pick a client to view their spend and lead performance.`}):(Wh(),`
    <div class="page-shell" data-section="ad-performance">
      <div class="page-header">
        ...
        <div class="chart-controls" id="adperf-periods">
          ${lg.map(e=>`<button class="chart-period-btn${e.d===ug?` active`:``}" type="button" data-days="${e.d}">${e.label}</button>`).join(``)}
        </div>
      </div>
    ...
    </div>`)}async function Kh(){if(j()){let e=e=>Number(e||0).toLocaleString();return Ki(tg(),[{h:`Spend`,td:t=>t.ad?`$${e(t.ad.spend)}`:`—`},{h:`Clicks`,td:t=>t.ad?e(t.ad.clicks):`—`},{h:`Leads`,td:t=>e(t.leads.window??t.leads.total)},{h:`CPL`,td:t=>t.ad?.cost_per_lead==null?`—`:`$${e(t.ad.cost_per_lead)}`}],{emptyMsg:`No client orgs yet. Connect a client ad account to see spend here.`})}let e=document.getElementById(`adperf-periods`);e?.addEventListener(`click`,t=>{let n=t.target.closest(`button[data-days]`);n&&(ug=Number(n.dataset.days)||30,e.querySelectorAll(`.chart-period-btn`).forEach(e=>e.classList.toggle(`active`,Number(e.dataset.days)===ug)),qh())}),await qh()}async function qh(){let e=tg(),t=document.getElementById(`adperf-body`),n=document.getElementById(`adperf-range`),r=new Date,i=ng(r),a=ng(new Date(r.getTime()-(ug-1)*864e5));n&&(n.textContent=`${a} → ${i}`);let o,s=[],c=null,l=null,u=null;try{let[t,n,r,d,f]=await Promise.all([fetch(`${e}/m5t/v5/coreAdMetrics/overlay?from=${a}&to=${i}`),fetch(`${e}/m5t/v5/coreAdAccount`),fetch(`${e}/m5t/v5/coreAdMetrics/by-source?from=${a}&to=${i}`),Ls().catch(()=>null),fetch(`${e}/m5t/v5/coreAdMetrics/conversions?from=${a}&to=${i}`).then(e=>e.json()).catch(()=>null)]);if(!t.ok)throw Error(`HTTP ${t.status}`);o=await t.json(),s=((await n.json().catch(()=>({})))?.data||[]).filter(e=>e.status!==`DELETED`),c=await r.json().catch(()=>null),l=d,u=f}catch(e){console.warn(`[ad-performance] load failed:`,e),t&&(t.innerHTML=cg(`error`,`Could not load ad performance. Please try again.`));return}Jh(l,s),Yh(u);let d=o.totals||{};if(Zh(`adperf-spend`,rg(d.spend)),Zh(`adperf-clicks`,ig(d.clicks)),Zh(`adperf-leads`,ig(d.magnet_leads)),Zh(`adperf-cpl`,rg(d.cost_per_lead)),Xh(c),!s.length){t.innerHTML=cg(`link_off`,`No ad account connected yet. Your agency can connect Google Ads or Local Services Ads to see performance here.`);return}if(!o.connected){t.innerHTML=cg(`cloud_sync`,`Account connected. No ad data for this range yet. Metrics appear here once they sync.`);return}t.innerHTML=eg(o.rows||[])}function Jh(e,t){let n=document.getElementById(`adperf-conn`);if(!n)return;if(!e||e.denied){n.innerHTML=``;return}let r=oc.google_ads,i=Ps(e.providers,`google_ads`),a=Fs(r,i,!!e.configured?.google_ads),o=ec()?` <a href="#/settings/integrations/${r.urlSlug}">Manage</a>`:``;if(a===`connected`||a===`broken`){Ys(),n.innerHTML=`<span>Google Ads</span> ${Is(a,i)}${o}`;return}if(t.length){n.innerHTML=`<span>Google Ads</span> <span>${t.length} associated account${t.length===1?``:`s`}</span>`;return}n.innerHTML=``}function Yh(e){let t=document.getElementById(`adperf-conv-card`),n=document.getElementById(`adperf-conv-body`),r=document.getElementById(`adperf-stats`),i=document.getElementById(`adperf-conv-stat`);if(i&&i.remove(),!t||!n)return;if(!e?.ok||!e.has_selection){t.style.display=`none`;return}if(r){let t=document.createElement(`div`);t.innerHTML=sa({icon:`flag`,label:`Ads Conversions`,value:ig(e.totals?.conversions),sub:`Reported by Google Ads`,href:`#/settings/integrations/google-ads`});let n=t.firstElementChild;n.id=`adperf-conv-stat`,r.appendChild(n)}let a=e.actions||[];if(t.style.display=``,!a.length){n.innerHTML=cg(`search_off`,`No conversions in this range for the selected actions.`);return}let o=a.some(e=>e.value>0);n.innerHTML=`
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
        ...
        <tbody>${a.map(e=>`
          <tr>
            <td>${og(e.name||e.id)}</td>
            <td class="adperf-src-sub">${og(String(e.category||``).toLowerCase().replace(/_/g,` `)||`—`)}</td>
            <td class="num"><b>${ig(e.conversions)}</b></td>
            ${o?`<td class="num">${rg(e.value)}</td>`:``}
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>
    <p class="adperf-note">Counted actions are chosen on the Google Ads integration page. These are Google's own numbers and may overlap magnet leads, so they are never added together.</p>`}function Xh(e){let t=document.getElementById(`adperf-sources`),n=document.getElementById(`adperf-src-summary`);if(!t)return;let r=e&&Array.isArray(e.rows)?e.rows:[];if(!r.length){n&&(n.textContent=``),t.innerHTML=cg(`search_off`,`No leads in this range yet. Campaigns appear here as tagged leads (?utm_campaign=…) come in.`);return}let i=e.total_leads||r.reduce((e,t)=>e+(t.leads||0),0),a=e.tracked_leads||0;n&&(n.textContent=`${ig(a)} of ${ig(i)} leads tagged to a source`);let o=e=>e.campaign&&e.campaign!==`(none)`?og(e.campaign):e.source&&e.source!==`(direct)`?og(e.source):`<span class="adperf-src-untracked">Untracked / direct</span>`;t.innerHTML=`
    <div class="adperf-srcwrap">
      <table class="adperf-srctable">
        ...
        <tbody>${r.map(e=>`
          <tr>
            <td>${o(e)}</td>
            <td class="adperf-src-sub">${og(e.source||`—`)}${e.medium?` · ${og(e.medium)}`:``}</td>
            <td class="num">${e.paid_clicks?ig(e.paid_clicks):`<span class="adperf-src-sub">—</span>`}</td>
            <td class="num"><b>${ig(e.leads)}</b></td>
          </tr>`).join(``)}
        </tbody>
      </table>
    </div>`;let s=t.querySelector(`.adperf-srctable tbody`);s&&D(s,{rowSelector:`tr`,noun:`sources`,controlHost:t.querySelector(`.adperf-srcwrap`)||t})}function Zh(e,t){let n=document.getElementById(e);n&&(n.textContent=t)}function Qh(e){if(e<=0)return 1;let t=10**Math.floor(Math.log10(e)),n=e/t;return(n<=1?1:n<=2?2:n<=5?5:10)*t}function $h(e){return e=Math.round(e),e>=1e3?(e/1e3).toFixed(e%1e3==0?0:1)+`k`:String(e)}function eg(e){let t=e.length||1,n=Qh(Math.max(1,...e.map(e=>e.spend||0))),r=Qh(Math.max(1,...e.map(e=>e.magnet_leads||0))),i=738/t,a=Math.min(24,i*.55),o=e=>56+i*(e+.5),s=e=>304-e/n*282,c=e=>304-e/r*282,l=[0,.25,.5,.75,1].map(e=>{let t=304-e*282;return`<line x1="56" y1="${t.toFixed(1)}" x2="794" y2="${t.toFixed(1)}" stroke="#EAF0F7"/><text x="48" y="${(t+3.5).toFixed(1)}" text-anchor="end" class="ax">$${$h(n*e)}</text><text x="802" y="${(t+3.5).toFixed(1)}" text-anchor="start" class="ax">${$h(r*e)}</text>`}).join(``),u=e.map((e,t)=>{let n=s(e.spend||0);return`<rect x="${(o(t)-a/2).toFixed(1)}" y="${n.toFixed(1)}" width="${a.toFixed(1)}" height="${Math.max(0,304-n).toFixed(1)}" rx="3" fill="#0072BA"><title>${e.date}: $${e.spend} spend</title></rect>`}).join(``),d=e.map((e,t)=>`${o(t).toFixed(1)},${c(e.magnet_leads||0).toFixed(1)}`).join(` `),f=t>1?`<polyline points="${d}" fill="none" stroke="#009473" stroke-width="2.5"/>`:``,p=e.map((e,t)=>`<circle cx="${o(t).toFixed(1)}" cy="${c(e.magnet_leads||0).toFixed(1)}" r="3.5" fill="#009473"><title>${e.date}: ${e.magnet_leads} leads</title></circle>`).join(``),m=Math.max(1,Math.ceil(t/6));return`
    ...
    </svg>`}var tg,ng,rg,ig,ag,og,sg,cg,lg,ug,dg,fg=o((()=>{T(),ea(),dc(),ua(),Xt(),tg=()=>w.getConfig?.().BASE_URL||``,ng=e=>e.toISOString().slice(0,10),rg=e=>e==null?`—`:`$`+Number(e).toLocaleString(void 0,{minimumFractionDigits:0,maximumFractionDigits:2}),ig=e=>Number(e||0).toLocaleString(),ag={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},og=e=>String(e??``).replace(/[&<>"']/g,e=>ag[e]),sg=!1,cg=(e,t)=>`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`,lg=[{d:7,label:`7 Days`},{d:30,label:`30 Days`},{d:90,label:`90 Days`}],ug=30,dg={render:Gh,init:Kh}})),pg=c({default:()=>Pg,init:()=>gg,parseBookingCsv:()=>Dg,render:()=>hg});function mg(){if(jg)return;let e=document.createElement(`style`);e.id=`m5t-adacct-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),jg=!0}function hg(){return j()?Gi({section:`ad-accounts`,title:`Ad Accounts`,description:`Pick a client to see their connected ad accounts.`}):(mg(),Ys(),`
...
`&&a++,n.push(r),r=``,n.some(e=>e.trim()!==``)&&t.push(n),n=[]):r+=o}if(n.push(r),n.some(e=>e.trim()!==``)&&t.push(n),t.length<2)return[];let a=t[0].map(e=>e.trim().toLowerCase()),o=a.findIndex(e=>/e-?mail/.test(e)),s=a.findIndex(e=>/phone|mobile|tel/.test(e)),c=a.findIndex(e=>/date|time|booked|appointment|start/.test(e));return o<0&&s<0?[]:t.slice(1).map(e=>({email:o>=0?(e[o]||``).trim():``,phone:s>=0?(e[s]||``).trim():``,at:c>=0?(e[c]||``).trim():``})).filter(e=>e.email||e.phone).slice(0,5e3)}function Og(e,t){let n=new Blob([e],{type:`text/csv;charset=utf-8`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.href=r,i.download=t,document.body.appendChild(i),i.click(),i.remove(),setTimeout(()=>URL.revokeObjectURL(r),4e3)}var kg,Ag,jg,Mg,Ng,Pg,Fg=o((()=>{ea(),Xt(),dc(),kg={google_ads:`Google Ads`,google_lsa:`Local Services Ads`},Ag=e=>{let t=String(e||``).replace(/\D/g,``);return t.length===10?`${t.slice(0,3)}-${t.slice(3,6)}-${t.slice(6)}`:String(e||``)},jg=!1,Mg=[],Ng=null,Pg={render:hg,init:gg}})),Ig=c({default:()=>i_,init:()=>Wg,openInviteModal:()=>Jg,render:()=>Ug});function Lg(e,t=`info`){try{Zg.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}}function Rg(){let e=w.getActiveOrg();if(e&&e.channel===Qg)return{guid:e.guid,name:e.name};let t=w.getMemberships().find(e=>e.channel===Qg);return t?{guid:t.orgGuid,name:t.name}:null}function zg(e){let t=e?.created_tz;if(!t)return``;let n=new Date(t);return Number.isNaN(n.getTime())?``:n.toLocaleDateString(void 0,{year:`numeric`,month:`short`,day:`numeric`})}function Bg(){if(e_)return;let e=document.createElement(`style`);e.id=`m5t-clients-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),tr(),e_=!0}function Vg(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function Hg(e){let t=e.caption||e.guid,n=e.status||`ENABLED`;return`
    <div class="client-row">
      <span class="client-row__avatar">${Y(n_(t))}</span>
      <span class="client-row__main">
        <div class="client-row__name" title="${Y(t)}">${Y(t)}</div>
        <div class="client-row__meta">Added ${Y(zg(e.created))||`—`}</div>
      </span>
    ...
    <div class="page-shell" data-section="clients">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="clients-body">
          ${Vg(`hourglass_empty`,`Loading clients…`)}
        </div>
      </div>
    </div>`}async function Wg(){let e=Rg();document.getElementById(`clients-add-btn`)?.addEventListener(`click`,()=>Kg(e)),await Gg(e)}async function Gg(e){let t=document.getElementById(`clients-body`),n=document.getElementById(`clients-count`);if(t){if(!e?.guid){t.innerHTML=Vg(`info`,`Client provisioning is available to reseller / agency accounts.`),n&&(n.textContent=``);return}try{let r=w.getConfig?.().BASE_URL||``,i=await fetch(`${r}/m5t/v5/acctGroup`);if(!i.ok)throw Error(`HTTP ${i.status}`);let a=await i.json(),o=(Array.isArray(a?.data)?a.data:[]).filter(t=>t.class===$g&&(!t.parent||t.parent===e.guid));if(!o.length){t.innerHTML=Vg(`group_off`,`No clients yet. Add your first client to start managing magnets on their behalf.`),n&&(n.textContent=``);return}t.innerHTML=`<div class="client-list">${o.map(Hg).join(``)}</div>`,D(t.querySelector(`.client-list`),{rowSelector:`.client-row`,noun:`clients`}),n&&(n.textContent=`${o.length} client${o.length===1?``:`s`}`),t.querySelectorAll(`.client-card__invite`).forEach(e=>{e.addEventListener(`click`,()=>{e.dataset.action===`adaccounts`?qg(e.dataset.clientGuid,e.dataset.clientName):e.dataset.action===`questionnaire`?Xg(e.dataset.clientGuid,e.dataset.clientName):Jg(e.dataset.clientGuid,e.dataset.clientName)})})}catch(e){console.warn(`[clients] load failed:`,e),t.innerHTML=Vg(`error`,`Couldn’t load clients. Please try again.`),n&&(n.textContent=``)}}}function Kg(e){if(!e?.guid){Lg(`Only reseller / agency accounts can add clients.`,`error`);return}let t=document.createElement(`div`);t.className=`client-modal-scrim`;let n=document.createElement(`div`);n.className=`client-modal`,n.setAttribute(`role`,`dialog`),n.setAttribute(`aria-modal`,`true`),n.innerHTML=`
    ...
    <div class="client-modal__body" id="qm-body">${Vg(`hourglass_empty`,`Loading…`)}</div>`,document.body.appendChild(r),document.body.appendChild(i),requestAnimationFrame(()=>{r.classList.add(`open`),i.classList.add(`open`)});let a=()=>{r.classList.remove(`open`),i.classList.remove(`open`),document.removeEventListener(`keydown`,o),setTimeout(()=>{r.remove(),i.remove()},200)},o=e=>{e.key===`Escape`&&a()};document.addEventListener(`keydown`,o),r.addEventListener(`click`,a),i.querySelector(`.client-modal__close`)?.addEventListener(`click`,a);let s=i.querySelector(`#qm-body`);function c(){s.innerHTML=`
      ...
      </div>`,s.querySelector(`[data-act="done"]`)?.addEventListener(`click`,a),s.querySelector(`[data-act="resend"]`)?.addEventListener(`click`,()=>c())}function d(e,t=!1){let r=e.editability||{editable:e.status===`SUBMITTED`&&!e.locked_tz,reason:e.locked_tz?`finalized`:null,deadline:null},i=!!r.editable,o=!i&&e.status===`SUBMITTED`?r.reason===`finalized`?`<div class="qm-status" data-status="LOCKED"><span class="material-symbols-outlined" aria-hidden="true">lock</span> Finalized${e.locked_tz?` ${Y(Yg(e.locked_tz))}`:``}${e.locked_by_email?` by ${Y(e.locked_by_email)}`:``} — no further edits.</div>`:`<div class="qm-status" data-status="LOCKED"><span class="material-symbols-outlined" aria-hidden="true">lock_clock</span> Editing closed${r.deadline?` ${Y(Yg(r.deadline))}`:``} — a response locks 14 days after it is submitted.</div>`:``,c=i&&r.deadline?`<p class="client-modal__sub qm-editable-hint">You can edit these answers until ${Y(Yg(r.deadline))}.</p>`:``;s.innerHTML=`
      <div class="qm-status" data-status="SUBMITTED"><span class="material-symbols-outlined" aria-hidden="true">task_alt</span> Submitted ${e.submitted_tz?Y(Yg(e.submitted_tz)):``}</div>
      ${o}
      ${er(e)}
      <form id="qm-form">
      ...
      </div>`,s.querySelector(`[data-act="done"]`)?.addEventListener(`click`,a),s.querySelector(`[data-act="cancel-edit"]`)?.addEventListener(`click`,()=>d(e,!1)),s.querySelector(`[data-act="edit"]`)?.addEventListener(`click`,()=>d(e,!0));let l=s.querySelector(`[data-act="save"]`);l?.addEventListener(`click`,async()=>{let t=$n(s);l.disabled=!0;let r=l.innerHTML;l.innerHTML=`<span class="material-symbols-outlined" aria-hidden="true">progress_activity</span> Saving…`;let i;try{i=await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e.guid)}`,{method:`PUT`,headers:{"Content-Type":`application/json`},body:JSON.stringify({answers:t})});let r=await i.json().catch(()=>({}));if(!i.ok||!r.ok)throw Error(r.error||r.message||`Failed (HTTP ${i.status})`);Lg(`Answers updated`,`success`);let a=await(await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e.guid)}`)).json().catch(()=>({}));d(a.ok?a.data:{...e,answers:t},!1)}catch(t){if(i&&i.status===423){Lg(t.message||`This questionnaire is now locked.`,`info`);let r=await(await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(e.guid)}`)).json().catch(()=>({}));d(r.ok?r.data:e,!1);return}s.querySelector(`#qm-error`).textContent=t.message||`Something went wrong. Please try again.`,l.disabled=!1,l.innerHTML=r}})}try{let t=await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse?owner_guid=${encodeURIComponent(e)}`),r=await t.json().catch(()=>({}));if(!t.ok||!r.ok)throw Error(r.error||`HTTP ${t.status}`);let i=(r.data||[])[0]||null;if(!i){c();return}if(i.status===`DRAFT`){l(i);return}if(i.status===`CANCELLED`){u(i);return}let a=await fetch(`${n}/m5t/v5/m5mQuestionnaireResponse/${encodeURIComponent(i.guid)}`),o=await a.json().catch(()=>({}));if(!a.ok||!o.ok)throw Error(o.error||`HTTP ${a.status}`);d(o.data)}catch(e){console.warn(`[clients] questionnaire load failed:`,e),s.innerHTML=`<p class="client-modal__error">Could not load questionnaire status. ${Y(e.message||``)}</p>`}}var Zg,Qg,$g,e_,t_,Y,n_,r_,i_,a_=o((()=>{T(),Qe(),Zg=u(f(),1),m(),Xt(),ar(),Qg=`reseller_org`,$g=`managed_client`,e_=!1,t_={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},Y=e=>String(e??``).replace(/[&<>"']/g,e=>t_[e]),n_=e=>String(e||`?`).trim().split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``).toUpperCase()||`?`,r_=e=>String(e||``).replace(/[^0-9]/g,``),i_={render:Ug,init:Wg}})),o_=c({default:()=>w_,init:()=>h_,loadTeam:()=>g_,render:()=>m_});function s_(){let e=w.getActiveOrg(),t=e&&e.channel!==y_?e:null;if(!t){let e=w.getMemberships().find(e=>e.channel!==y_);e&&(t={guid:e.orgGuid,channel:e.channel,memberRole:e.memberRole})}if(!t)return null;let n=w.getMemberships().find(e=>e.orgGuid===t.guid)?.name;return{...t,name:n}}function c_(e){return Object.values(Ie).filter(t=>Fe(e,t.value)).sort((e,t)=>t.rank-e.rank)}function l_(){if(b_)return;let e=document.createElement(`style`);e.id=`m5t-team-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e),b_=!0}function u_(e,t){return`<div class="m5t-empty"><span class="material-symbols-outlined" aria-hidden="true">${e}</span><p>${t}</p></div>`}function d_(e,t,n){let r=e.member_role||`member`,i=e.entity_guid&&e.entity_guid===w.getEntity?.()?.guid,a=n&&!i&&c_(t.memberRole).some(e=>e.value===r);return`
    <div class="team-row">
      <span class="team-avatar">${X(C_(e.name||e.email))}</span>
    ...
    </div>`}async function f_(e,t){let{uid:n,email:r}=e.dataset;if(window.confirm(`Revoke the invitation for ${r}?\n\nThe invitation link stops working immediately, including any copy already sent. You can invite this address again afterwards.`)){e.disabled=!0;try{let e=w.getConfig?.().BASE_URL||``,i=await fetch(`${e}/m5t/v5/acctGroup/${encodeURIComponent(t.guid)}/invites/${encodeURIComponent(n)}`,{method:`DELETE`}),a=await i.json().catch(()=>({}));if(!i.ok||!a.ok)throw Error(a.message||a.error||`HTTP ${i.status}`);S_(`Invitation for ${r} revoked`,`success`)}catch(t){e.disabled=!1,S_(t.message||`Could not revoke that invitation.`,`error`);return}try{await g_(t)}catch{}}}async function p_(e,t){let{id:n,name:r}=e.dataset;if(window.confirm(`Remove ${r} from the team?\n\nThey lose access to this workspace immediately. Their account itself is not deleted, and you can invite them again later.`)){e.disabled=!0;try{let e=w.getConfig?.().BASE_URL||``,t=await fetch(`${e}/m5t/v5/acctGroupMember/${encodeURIComponent(n)}`,{method:`DELETE`}),i=await t.json().catch(()=>({}));if(!t.ok||!i.ok)throw Error(i.message||i.error||`HTTP ${t.status}`);S_(`${r} removed from the team`,`success`)}catch(t){e.disabled=!1,S_(t.message||`Could not remove that member.`,`error`);return}try{await g_(t)}catch{}}}function m_(){return l_(),`
    <div class="page-shell" data-section="team">
      ...
      <div class="dashboard-card">
        ...
        <div class="card-body" id="team-body">
          ${u_(`hourglass_empty`,`Loading team…`)}
        </div>
      </div>
    </div>`}async function h_(){let e=s_(),t=document.getElementById(`team-invite-btn`);if(!e){t&&(t.style.display=`none`),document.getElementById(`team-body`).innerHTML=u_(`info`,`Team management is available for agency and workspace accounts.`);return}t?.addEventListener(`click`,()=>__(e)),await g_(e)}async function g_(e){let t=document.getElementById(`team-body`),n=document.getElementById(`team-count`);if(t)try{let r=w.getConfig?.().BASE_URL||``,[i,a]=await Promise.all([fetch(`${r}/m5t/v5/acctGroup/${encodeURIComponent(e.guid)}/members`),fetch(`${r}/m5t/v5/acctGroup/${encodeURIComponent(e.guid)}/invites`)]),[o,s]=await Promise.all([i.json().catch(()=>({})),a.json().catch(()=>({}))]);if(!i.ok||!a.ok)throw Error(`Failed to load team (members: ${i.status}, invites: ${a.status})`);let c=o.members||[],l=s.invites||[],u=c_(e.memberRole).length>0;t.innerHTML=(l.length?`
    ...
    <div class="team-modal__body" id="team-invite-body">
      ...
      <div class="team-modal__actions">
        ...
        </div>`;let u=r.querySelector(`#tm-link`);r.querySelector(`#tm-copy`)?.addEventListener(`click`,async()=>{try{await navigator.clipboard.writeText(u.value),r.querySelector(`#tm-copy`).textContent=`Copied!`}catch{u.select()}}),r.querySelector(`[data-act="done"]`)?.addEventListener(`click`,()=>{i(),g_(e)})}catch(e){console.warn(`[team] invite failed:`,e),o.textContent=e.message||`Something went wrong. Please try again.`,s.disabled=!1,s.innerHTML=l}})}var v_,y_,b_,x_,X,S_,C_,w_,T_=o((()=>{T(),v_=u(f(),1),m(),Qe(),y_=`managed_client`,b_=!1,x_={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},X=e=>String(e??``).replace(/[&<>"']/g,e=>x_[e]),S_=(e,t=`info`)=>{try{v_.default.publish(p.UI_TOAST,{message:e,type:t})}catch{}},C_=e=>String(e||`?`).trim().split(/\s+/).slice(0,2).map(e=>e[0]||``).join(``).toUpperCase()||`?`,w_={render:m_,init:h_}}));m(),we(),T(),Vt(),v(),bn(),Dn(),nt(),window.m5t=Object.create(null),window.m5t.pubsub=y.default,console.log(`StreamSyncEngage SaaS Client - Starting...`),console.log(`Environment:`,`production`),console.log(`Document readyState:`,document.readyState),console.log(`window.mvvLegit:`,typeof window.mvvLegit),console.log(`globalThis.mvvLegit:`,typeof globalThis.mvvLegit);try{console.log(`mvvLegit (direct):`,typeof mvvLegit)}catch(e){console.log(`mvvLegit (direct): ReferenceError -`,e.message)}var E_=Object.keys(window).filter(e=>e.toLowerCase().includes(`mvv`)||e.toLowerCase().includes(`neodigm`)||e.toLowerCase().includes(`legit`));console.log(`Possible Neodigm globals:`,E_);var Z=new te.default(`/`,{hash:!0});window.router=Z;function Q(e){let t=e?`${e.replace(/-/g,`_`)}_route`:`splash_route`;return console.log(`[Router] Checking access: ${e} -> ${t}`),mvvLegit.isRouteAllowed(t)?!0:(console.warn(`[Router] Access denied to route: ${t}`),y.default.publish(p.UI_TOAST,{message:`Please sign in to access this page`,type:`warning`}),!1)}function $(e){console.log(`[Router] Navigating to:`,e),Z.navigate(`/${e}`)}var D_=new Set([`splash`,`signin`,`signup`,`forgot`,`resethash`,`verf-link`,`accept-invite`,`signout`]),O_={},k_={},A_=null,j_={admin:{caption:`Platform Admin`,icon:`shield_person`},reseller:{caption:`Reseller`,icon:`handshake`},managed:{caption:`Managed`,icon:`visibility`},self_serve:{caption:`Self-Serve`,icon:`rocket_launch`}},M_=`__portfolio__`;function N_(){try{Z._setCurrent(null)}catch{}Z.resolve()}function P_(){let e=document.getElementById(`m5t-side-nav`);if(!e)return;let t=!w.getScope()&&w.isAuthenticated(),n=w.getCapabilities(),r=w.getUiMode(),i=Kt.getSideNav(n,{uiMode:r});O_={},k_={};let a=e=>{e.route&&(O_[e.route]={sectionToken:e.token,childToken:null,children:e.children||null},Array.isArray(e.children)&&e.children.forEach(t=>{t.route&&(k_[t.token]=t.route,O_[t.route]={sectionToken:e.token,childToken:t.token,children:e.children})}))},o=e=>e.forEach(e=>{e.group&&Array.isArray(e.items)?o(e.items):a(e)});o(i);let s=w.getMemberships(),c=w.getActiveOrg(),l=w.getManagedClients(),u=w.getStoredActiveOrg(),d=l.length?[{guid:M_,name:`Portfolio (all clients)`},...s.map(e=>({guid:e.orgGuid,name:e.name||e.channel,channel:e.channel,group:`Your organization`})),...l.map(e=>({guid:e.orgGuid,name:e.name||e.orgGuid,channel:e.channel,group:`Clients`}))]:s.map(e=>({guid:e.orgGuid,name:e.name||e.channel,channel:e.channel}));e.setState({buttons:i,orgs:d,activeOrg:l.length?u||M_:c?.guid||null,footerBadge:j_[r]||null,loading:t}),I_(),A_&&L_(A_)}var F_=e=>String(e??``).replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`})[e]);function I_(){let e=document.getElementById(`app-content`);if(!e)return;let t=document.getElementById(`org-context-banner`);t||(t=document.createElement(`div`),t.id=`org-context-banner`,e.insertBefore(t,e.firstChild),dn(t));let n=w.getActiveOrg(),r=w.isPreviewingAsClient(),i=!!n?.viaSubtree&&!!A_&&!D_.has(A_);if(t.hidden=!i,!i){t.innerHTML=``;return}let a=F_(n.name||`Client`);t.innerHTML=`
    <style>
